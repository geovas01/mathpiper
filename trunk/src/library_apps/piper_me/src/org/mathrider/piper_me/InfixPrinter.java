...
class InfixPrinter extends LispPrinter
{
  ...
  void Print(LispPtr aExpression, LispOutput aOutput, int iPrecedence) throws Exception
  {
    ...
    String string = aExpression.getNext().String();
    if (string != null)
    {
    ...
    if (aExpression.getNext().Generic() != null)
    {
        //TODO display genericclass
        WriteToken(aOutput,aExpression.getNext().Generic().TypeName());
        return;
    }

    LispPtr subList = aExpression.getNext().SubList();
    ...
    else
    {
        ...
        string = subList.getNext().String();
        LispInFixOperator prefix  = (LispInFixOperator)iPrefixOperators.get(string);
        LispInFixOperator infix   = (LispInFixOperator)iInfixOperators.get(string);
        LispInFixOperator postfix = (LispInFixOperator)iPostfixOperators.get(string);
        LispInFixOperator bodied  = (LispInFixOperator)iBodiedOperators.get(string);
        ...
        if (op != null)
        {
            ...
            if (prefix != null)
            {
                right = subList.getNext();
            }
            else if (infix != null)
            {
                left  = subList.getNext();
                right = subList.getNext().getNext();
            }
        ...
        {
            LispIterator iter = new LispIterator(subList.getNext());
