...
/**
 * 
 * JYacas - a dockable JPanel, a demonstration of a jEdit plugin.
 *
 */
public class JYacas extends JPanel
    implements EBComponent, JYacasActions, DefaultFocusComponent {
	...
	private JYacasTextArea textArea;

	private JYacasToolPanel toolPanel;
    // }}}
	 ...
	 */
	public JYacas(View view, String position) {
		super(new BorderLayout());
		this.view = view;
		this.floating = position.equals(DockableWindowManager.FLOATING);

		if (jEdit.getSettingsDirectory() != null) {
			this.filename = jEdit.getProperty(JYacasPlugin.OPTION_PREFIX
					+ "filepath");
			if (this.filename == null || this.filename.length() == 0) {
				this.filename = new String(jEdit.getSettingsDirectory()
						+ File.separator + "qn.txt");
				jEdit.setProperty(
						JYacasPlugin.OPTION_PREFIX + "filepath",
						this.filename);
			}
			this.defaultFilename = this.filename;
		}

		this.toolPanel = new JYacasToolPanel(this);
		add(BorderLayout.NORTH, this.toolPanel);

		if (floating)
			this.setPreferredSize(new Dimension(500, 250));

		textArea = new JYacasTextArea();
		textArea.setFont(JYacasOptionPane.makeFont());
	...
	private void propertiesChanged() {
		...
		Font newFont = JYacasOptionPane.makeFont();
	...
	public void saveFile() {
		...
		try {
			...
			Log.log(Log.ERROR, JYacas.class,
	...
	private void readFile() {
		...
		try {
			...
			Log.log(Log.ERROR, JYacas.class, "notepad file " + filename
					+ " does not exist");
		} catch (IOException ioe) {
			Log.log(Log.ERROR, JYacas.class,
