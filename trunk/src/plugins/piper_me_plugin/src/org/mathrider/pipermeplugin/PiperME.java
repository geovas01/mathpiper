 ...
 *
 */
public class PiperME extends JPanel
    implements EBComponent, PiperMEActions, DefaultFocusComponent {
	...
	private PiperMEToolPanel toolPanel;
    // }}}
	 ...
	 */
	public PiperME(View view, String position) {
		super(new BorderLayout());
		this.view = view;
		this.floating = position.equals(DockableWindowManager.FLOATING);

		if (jEdit.getSettingsDirectory() != null) {
			this.filename = jEdit.getProperty(PiperMEPlugin.OPTION_PREFIX
					+ "filepath");
			if (this.filename == null || this.filename.length() == 0) {
				this.filename = new String(jEdit.getSettingsDirectory()
						+ File.separator + "qn.txt");
				jEdit.setProperty(
						PiperMEPlugin.OPTION_PREFIX + "filepath",
						this.filename);
			}
			this.defaultFilename = this.filename;
		}

		this.toolPanel = new PiperMEToolPanel(this);
		...
		textArea.setFont(PiperMEOptionPane.makeFont());
	...
	private void propertiesChanged() {
		...
		Font newFont = PiperMEOptionPane.makeFont();
	...
	public void saveFile() {
		...
		try {
			...
			Log.log(Log.ERROR, PiperME.class,
	...
	private void readFile() {
		...
		try {
			...
			Log.log(Log.ERROR, PiperME.class, "notepad file " + filename
					+ " does not exist");
		} catch (IOException ioe) {
			Log.log(Log.ERROR, PiperME.class,
