...
abstract public class ODE2ndOrder3D extends ODE_3D {

	public ODE2ndOrder3D() {
		super(false,true,"x","y","z","x'","y'","z'");
		initialDataDefault = new double[] { 1, 1, 0, 0, 0, 1, 1 };
		timeSpanDefault = 30;
	}
	...
	protected void nextEulerPoint(double[] pointData, double dt) {
		...
		pointData[3] = xdot + dt*xdotdot(x,y,z,xdot,ydot,zdot);
		pointData[4] = ydot + dt*ydotdot(x,y,z,xdot,ydot,zdot);
		pointData[5] = zdot + dt*zdotdot(x,y,z,xdot,ydot,zdot);
	}
	...
	protected void nextRungeKuttaPoint(double[] pointData, double dt) {
		...
		j1 = dt * ydot;
		m1 = dt * zdot;
		h1 = dt * xdotdot(x,y,z,xdot,ydot,zdot);
		k1 = dt * ydotdot(x,y,z,xdot,ydot,zdot);
		n1 = dt * zdotdot(x,y,z,xdot,ydot,zdot);
		i2 = dt * (xdot + h1/2);
		j2 = dt * (ydot + k1/2);
		m2 = dt * (zdot + n1/2);
		h2 = dt * xdotdot(x+i1/2,y+j1/2,z+m1/2,xdot+h1/2,ydot+k1/2,zdot+n1/2);
		k2 = dt * ydotdot(x+i1/2,y+j1/2,z+m1/2,xdot+h1/2,ydot+k1/2,zdot+n1/2);
		n2 = dt * zdotdot(x+i1/2,y+j1/2,z+m1/2,xdot+h1/2,ydot+k1/2,zdot+n1/2);
		i3 = dt * (xdot + h2/2);
		j3 = dt * (ydot + k2/2);
		m3 = dt * (zdot + n2/2);
		h3 = dt * xdotdot(x+i2/2,y+j2/2,z+m2/2,xdot+h2/2,ydot+k2/2,zdot+n2/2);
		k3 = dt * ydotdot(x+i2/2,y+j2/2,z+m2/2,xdot+h2/2,ydot+k2/2,zdot+n2/2);
		n3 = dt * zdotdot(x+i2/2,y+j2/2,z+m2/2,xdot+h2/2,ydot+k2/2,zdot+n2/2);
		i4 = dt * (xdot + h3);
		j4 = dt * (ydot + k3);
		m4 = dt * (zdot + n3);
		h4 = dt * xdotdot(x+i3,y+j3,z+m3,xdot+h3,ydot+k3,zdot+n3);
		k4 = dt * ydotdot(x+i3,y+j3,z+m3,xdot+h3,ydot+k3,zdot+n3);
		n4 = dt * zdotdot(x+i3,y+j3,z+m3,xdot+h3,ydot+k3,zdot+n3);
	...
	abstract protected double xdotdot(double x, double y, double z, double xdot, double ydot, double zdot);

	abstract protected double ydotdot(double x, double y, double z, double xdot, double ydot, double zdot);
	
	abstract protected double zdotdot(double x, double y, double z, double xdot, double ydot, double zdot);
