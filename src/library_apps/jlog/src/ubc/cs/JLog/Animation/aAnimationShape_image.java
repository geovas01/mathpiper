...
public class aAnimationShape_image extends aAnimationShape {
	...
	g.drawImage(image, p.x, p.y, Math.round(width * m),
		Math.round(height * m), object.getEnvironment());
    };
	...
	return new Rectangle(p.x, p.y, Math.round(width * m), Math.round(height
		* m));
    };
	...
	setStringKeysForTranslation(t, HEIGHT_ATTR_KEY, jReal.class,
		Float.class);
    };
