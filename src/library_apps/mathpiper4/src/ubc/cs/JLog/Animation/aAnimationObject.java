 ...
 * (to determine overlap of the object's image). Animation objects can also keep
 * track of the path they have traversed. Objects are not themselves visible
 * (the enclosed shapes give the visible manifestation), but they do have
 * coordinates and rotation attributes. Coordinates are given the canonical
 * global coordinates (i.e., with magnification of 1.0) and rotation of an
 * object is given in radians. Enclosed shapes have local canonical coordinates
 * and rotations which are offsets from the object values. A display point
 * (non-canonical) is the global point with magnifcation applied.
 * 
...
public class aAnimationObject extends aAttributeTranslation {
     ...
     * 
     * @param ae
     *            The graphical environment that encloses this object. Animation
     *            object can only belong to one environment.
     * @param n
     ...
     * @return An <code>aAnimationShape</code> from the current view, with the
     *         given name.
     */
     ...
     * with this object being its parent object. Shape is added to the current
     * view.
     * 
     * @param as
     *            The <code>aAnimationShape</code> to add.
     */
     ...
     * 
     * @param as
     *            The <code>aAnimationShape</code> to remove.
     */
    public void removeShape(aAnimationShape as) {
	shapes[view].removeElement(as);
	named_shapes[view].remove(as.getName());

	// permits retrieval of overlaid shapes, adds any remaining shape with
	// the same name
    ...
    /**
     * Draw the current view of the object in the <code>Graphics</code>
     * environment provided. Includes any path that the object has recorded.
     * 
     * @param g
     *            The <code>Graphics</code> environment to draw into.
     */
    ...
    /**
     * Returns the bounding rectangle for this object in the current view, given
    ...
    /**
     * Set the current view to display. Does not cause the environment to
     * re-paint.
     * 
     * @param v
	...
	setStringKeysForTranslation(t, ROTATION_ATTR_KEY, jReal.class,
		Float.class);
	setStringKeysForTranslation(t, NAME_ATTR_KEY, jAtom.class, String.class);
	setStringKeysForTranslation(t, LEVEL_ATTR_KEY, jInteger.class,
		Integer.class);
	setStringKeysForTranslation(t, VIEW_ATTR_KEY, jInteger.class,
		Integer.class);
	setStringKeysForTranslation(t, PATHSIZE_ATTR_KEY, jInteger.class,
		Integer.class);
	setStringKeysForTranslation(t, PATHSKIP_ATTR_KEY, jInteger.class,
		Integer.class);
	setStringKeysForTranslation(t, PATHCOLOUR_ATTR_KEY, Color.class,
		Color.class);
    };
	...
	// set location
	{
	    ...
	    if (fx != null || fy != null)
		setPosition((fx != null ? fx.floatValue() : x),
			(fy != null ? fy.floatValue() : y));
	}
	// set rotation
	{
	    Float r;
    ...
    /**
     * Calculates the display coordinates for given local (i.e., shape relative)
     * point and rotation. The display coordinate is the global coordinate after
     * magnification.
     * 
     * @param dx
     *            Local (shape relative) x position offset from the objects
     *            position.
     * @param dy
     *            Local (shape relative) y position offset from the objects
     *            position.
     * @param dr
     *            Local (shape relative) rotation offset.
     */
    public final Point getDisplayPointFromLocalPosition(float dx, float dy,
	    float dr) {
	float gx = x + dx;
	float gy = y + dy;
	...
	return new Point(Math.round((float) Math.sin(a) * d * m),
		Math.round((float) Math.cos(a) * d * m));
    };

    /**
     * Calculates the display coordinates for given local (i.e., shape relative)
     * point. The display coordinate is the global coordinate after
     * magnification.
     * 
     * @param dx
     *            Local (shape relative) x position offset from the objects
    ...
    /**
     * Set the path attributes. The path is a history of previous locations for
     * this object. The path is drawn, unless no historical locations are
     * recorded.
     * 
     * @param sz
     *            The number of previous locations to keep track of.
     * @param sk
     *            The number of previous locations skip over before recording a
     *            location (used to prevent sampling every location).
     * @param c
     *            The colour to draw the path in.
     */
     ...
     *            allowed, false if we just want to know how many locations are
     *            already recorded in the path.
     * @return If <code>allowed</code> is true, then returns the maximum number
     *         of previous locations to record, otherwise returns the number of
     *         previous locations currently recorded.
     */
    ...
    /**
     * Adjust the global location of the object to the given position. Moves the
     * object. Notifies child shapes that the object position has changed.
     * 
     * @param x
     *            The x component of the location.
     * @param y
    ...
    /**
     * Adjust the rotation of the object to the given angle in radians. Adjusts
     * the real location of the child shapes by the given rotation.
     * 
     * @param r
