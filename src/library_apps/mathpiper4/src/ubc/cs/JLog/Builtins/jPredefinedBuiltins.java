 ...
 * <code>jKnowledgeBase</code> and registers all the operator and predicate
 * entries with their corresponding registries for use by the parser.
 * 
...
public class jPredefinedBuiltins extends jPredefined {
	...
	addRules(new jRule[] { get_BAGOFPOSTSORTHELPER1(),
	...
	// if(P,Q,R) :- 'CREATEARRAY'(1,A),'SETARRAYELEMENT'(A,0,fail),
	// 'IF'(P,Q,R,A).
	...
	pt.addPredicate(new jSetArrayElement(A, new jInteger(0), new jAtom(
		"fail")));
	...
	pt = new jPredicateTerms();
	pt.addPredicate(new jGetArrayElement(A, new jInteger(0), new jAtom(
		"fail")));
	...
	return new jBuiltinRule(new jPredicate("retractall", ct1),
		new jPredicateTerms());
    };
	...
	// findall(X,Y,Z) :- A =
	// *compoundterm*,'FINDALL'(X,Y,A),'CONVERTARRAY'(A,Z,[]).
	...
	// findall(X,Y,Z,R) :- A =
	// *compoundterm*,'FINDALL'(X,Y,A),'CONVERTARRAY'(A,Z,R).
	...
	// bagof(X,Y,Z) :- A = *compoundterm*,B = *compoundterm*,C =
	// *compoundterm*,
	...
	// 'BAGOF'(X,Y,C,B) :-
	// findall(Y-X,C,Z),keysort(Z,A,true),'BAGOFPOSTSORT'(_,A,A1),!,
	...
	// 'BAGOFPOSTSORT'(Y,[Y-X|XYs],[[Y,[X|A]]|B]) :-
	// 'BAGOFPOSTSORTHELPER'(Y,XYs,A,B1),!,
	...
	ct1.addTerm(new jListPair(new jListPair(Y, new jListPair(new jListPair(
		X, A), jNullList.NULL_LIST)), B));
	...
	// current_op(X,Y,Z) :-
	// 'CURRENTOPLIST'(A),sort(A,B),!,'CURRENTOP'(B,X,Y,Z).
