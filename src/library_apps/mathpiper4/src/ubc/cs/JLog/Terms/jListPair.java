...
public class jListPair extends jList {
    // head or tail should never be null, except during the list construction
    // phase.
    // complete lists should terminate with a nulllist, or other other term on
    // tail.
	...
	// this is a special case, because a true list may be very long (the
	// other binary predicates
	// have a length limit based on the Java stack size ~3000 calls/terms),
	// so compare
	...
	if (t instanceof jListPair) {
		...
		jListPair l = (jListPair) t; // assume type member value is
					     // correct
		jListPair ll = (jListPair) tt; // assume type member value is
					       // correct
		int result;
	...
	while (t != null) {
	    ...
	    // altough we cannot be certain that t is a jListPair, if it is not
	    // then type was wrong
...

		{
			...
			lp = (jListPair) t2; // assume type member value is
					     // correct from above
		}
	...
	while (t.type == type) {
	    ...
	    t = lp.tail; // no getTerm() since variables should be unbound, and
			 // need to add themselves
	}
	...
	// build stack of list elements to facilitate building the new list from
	// the end.
	while (t.type == type) {
	    jListPair lp = (jListPair) t; // assume type member value is correct
	...
	// for each element of the list, create its duplicate (if neither the
	// head nor tail is
    ...
    /**
     * Returns a <code>jList</code> list representing the given enumeration of
     * objects.
     * 
     * @param e
     *            The enumeration of elements to construct the list from.
     * @param conv
     *            An object to term converter to construct terms in the list
     *            from the objects in the enumeration.
     * 
     * @return The new list where each element in the enumeration is converted
     *         into an element in the list.
     */
    public static jList createListFromEnumeration(Enumeration e,
	    iObjectToTerm conv) {
	jList head = null;
	jListPair next = null;
