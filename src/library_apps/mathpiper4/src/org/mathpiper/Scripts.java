...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (_expression /:: LocPatterns(_patterns)) <--{ MacroSubstituteApply(expression,\"LocPredicate\",\"LocChange\");};10 # (_expression /:: _patterns) <--{ Local(old);  Assign(patterns, LocProcess(patterns));  Assign(old, expression);  Assign(expression, MacroSubstituteApply(expression,\"LocPredicate\",\"LocChange\"));  While (expression !=? old) { Assign(old, expression);  Assign(expression, MacroSubstituteApply(expression,\"LocPredicate\",\"LocChange\")); };  expression;};";
        scriptMap.put("/::",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (_expression /: LocPatterns(_patterns)) <--{ MacroSubstituteApply(expression,\"LocPredicate\",\"LocChange\");};10 # (_expression /: _patterns) <--{ Assign(patterns, LocProcess(patterns));  MacroSubstituteApply(expression,\"LocPredicate\",\"LocChange\");};";
        scriptMap.put("/:",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Expand(x); 10 # PAdicExpand(_x,_y) <--{ Local(coefs); coefs:=PAdicExpandInternal(x,y); Substitute(p,y)Add(coefs*(p^(0 .. Length(coefs))));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # TSumSimplify(TList(Delta(_ind,_ind),_list))_Contains?(indices,ind) <--{  DestructiveDelete(indices,Find(indices,ind)); Ndim*TSumRest(list);};11 # TSumSimplify(TList(Delta(_ind1,_ind2),_list))_ Contains?(indices,ind2) <--{  DestructiveDelete(indices,Find(indices,ind2));  TSumRest( Substitute(ind2,ind1)list );};11 # TSumSimplify(TList(Delta(_ind1,_ind2),_list))_ Contains?(indices,ind1) <--{  DestructiveDelete(indices,Find(indices,ind1));  TSumRest( Substitute(ind1,ind2)list );};1010 # TSumSimplify(TList(_term,_list)) <--{ term*TSumRest(list);};UnFence(\"TSumSimplify\",1); ";
