...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"CoFactor\",[matrix,ii,jj]){ Local(perms,indices,result); indices:=Table(i,i,1,Length(matrix),1); perms:=PermutationsList(indices); result:=0; ForEach(item,perms) Decide(item[ii] =? jj, result:=result+ Product(i,1,Length(matrix), Decide(ii=?i,1,matrix[i][item[i] ]) )*LeviCivita(item)); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HankelMatrix(n):=GenMatrix([[i,j], Decide(i+j-1>?n,0,i+j-1) ], n,n );HankelMatrix(m,n):=GenMatrix([[i,j], Decide(i+j-1>?n,0,i+j-1)], m,n );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WilkinsonMatrix(N):=GenMatrix([[i,j], Decide( Abs(i-j)=?1,1, { Decide(i=?j,Abs( (N-1)/2 - i+1 ),0 ); } )], N,N );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"Table\",[body,var,countfrom,countto,step]) { MacroLocal(var); Local(result,nr,ii); result:=[]; nr := (countto - countfrom) / step; ii := 0; While( ii <=? nr ) { MacroAssign( var, countfrom + ii * step ); DestructiveInsert( result,1,Eval(body) ); Assign(ii,AddN(ii,1)); }; DestructiveReverse(result); };HoldArgumentNumber(\"Table\",5,1); HoldArgumentNumber(\"Table\",5,2); UnFence(\"Table\",5);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " Retract(\"+$\", *); Retract(\"-$\", *); Retract(\"/$\", *); Retract(\"*$\", *); Retract(\"^$\", *);  Infix(\"+$\",70); Infix(\"-$\",70); RightPrecedenceSet(\"-$\",40); Infix(\"/$\",30); Infix(\"*$\",40); Infix(\"^$\",20); LeftPrecedenceSet(\"^$\",19);  RightAssociativeSet(\"^$\"); Prefix(\"+$\",50); Prefix(\"-$\",50); RightPrecedenceSet(\"-$\",40);   RulebaseEvaluateArguments(\"+$\",[lhs, rhs]); RulebaseEvaluateArguments(\"-$\",[lhs, rhs]); RulebaseEvaluateArguments(\"/$\",[lhs, rhs]); RulebaseEvaluateArguments(\"*$\",[lhs, rhs]); RulebaseEvaluateArguments(\"^$\",[lhs, rhs]); RulebaseEvaluateArguments(\"+$\",[operand]); RulebaseEvaluateArguments(\"-$\",[operand]); Position(subTerm, term) :={ Local(positionList);  Check(Function?(term), \"Argument\", \"The argument must be an expression that contains a function.\");  positionList := PositionHelper(subTerm, term,[False]);  positionList := Rest(positionList);  Rest(Reverse(positionList));};PositionHelper(subTerm, term, list) :={  Local(argPosition); If(Atom?(term) And? (subTerm =? term)) { list[1] := True;  list; } Else If(Atom?(term) ) { list; } Else { argPosition := Length(term);  While(argPosition >? 0) {  list := PositionHelper(subTerm, term[argPosition], list);  Decide(list[1] =? True, {list := Append(list, argPosition);argPosition := -1;});  argPosition--; };  list; };};Occurrence(subTerm, term) :={ Check(Function?(term), \"Argument\", \"The argument must be an expression that contains a function.\"); Count(VarListAll(term), subTerm);};SingleOccurrence?(subTerm, term) :={ Occurrence(subTerm, term) =? 1;};Retract(\"SolveEquation\",*);RulebaseListedHoldArguments(\"SolveEquation\",[equation, variable, optionsList]);5 # SolveEquation(_equation, _variable) <-- SolveEquation(equation, variable, []);10 # SolveEquation(_equation, _variable, optionsList_List?) <--{ Local(options, path);  options := OptionsToAssociativeList(optionsList);  equation := AddDollarSigns(equation);  path := Position(variable, equation);  Isolate2(path, equation, variable, options);};20 # SolveEquation(_equation, _variable, _singleOption) <-- SolveEquation(equation, variable, [singleOption]);Isolate(pathNumbers, equation, var) :={ Local(result, rest);  result := [];  DestructiveAppend(result,[ToAtom(\"Solve \\\\ \\\\ \" ~ ToString(TexFormNoDollarSigns(equation)) ~ \" \\\\ \\\\ for \\\\ \" ~ ToString(var) ~ \".\")]);    DestructiveAppend(result,[ToAtom(\" \\\\ \")]);  rest := [equation];  ForEach(pathNumber, pathNumbers) { rest := Isolax(pathNumber, First(rest));  DestructiveAppend(result,[ToAtom(\" \\\\ \")]); DestructiveAppend(result,[ToAtom(ListToString(Remove(StringToList(rest[3]),\"$\")))]);  Decide(Length(rest) =? 4, DestructiveAppend(result,[TexFormNoDollarSigns(rest[4])]));  DestructiveAppend(result,[ToAtom(\" \\\\ \")]);  DestructiveAppend(result,[TexFormNoDollarSigns(rest[1])]);  DestructiveAppend(result,[ToAtom(\" \\\\ \")]); };  result;};Isolate2(pathNumbers, equation, var, options) :={ Local(result, rest, currentEquation, showTree);  result := [];  DestructiveAppend(result, LatexView(\"Solve \\\\ \\\\ \" ~ ToString(TexFormNoDollarSigns(equation)) ~ \" \\\\ \\\\ for \\\\ \" ~ ToString(var) ~ \".\"));   Decide(options[\"showTree\"] =? \"True\", DestructiveAppend(result, TreeView(RemoveDollarSigns(equation), scale -> 2)));      rest := [equation];  ForEach(pathNumber, pathNumbers) { currentEquation := First(rest);  rest := Isolax(pathNumber, currentEquation);    If(Not? List?(rest[1])) { result := Compose(result, rest[1], rest[3], rest[4], Length(rest) =? 4, options[\"showTree\"]); } Else { result := Compose(result, rest[1][1], rest[3][1], rest[4][1], Length(rest) =? 4, options[\"showTree\"]);  result := Compose(result, rest[1][2], rest[3][2], rest[4][2], Length(rest) =? 4, options[\"showTree\"]); };   };  result;};Compose(result, newExpression, explanation, beforeSimplification, lengthFour?, showTree?) :={ DestructiveAppend(result,LatexView(\" \\\\vspace{.75in}\")); DestructiveAppend(result,LatexView(ListToString(Remove(StringToList(explanation),\"$\"))));  Decide(lengthFour?, {DestructiveAppend(result,LatexView(\" \\\\vspace{.10in}\")); DestructiveAppend(result,LatexView(TexFormNoDollarSigns(beforeSimplification)));});  DestructiveAppend(result,LatexView(\" \\\\vspace{.10in}\"));  DestructiveAppend(result, LatexView(TexFormNoDollarSigns(newExpression)));  Decide(showTree? =? \"True\", DestructiveAppend(result, TreeView(RemoveDollarSigns(newExpression), scale -> 2)));  result; };RemoveDollarSigns(equation) :={ equation := Subst(ToAtom(\"+$\"),ToAtom(\"+\")) equation; equation := Subst(ToAtom(\"-$\"),ToAtom(\"-\")) equation; equation := Subst(ToAtom(\"*$\"),ToAtom(\"*\")) equation; equation := Subst(ToAtom(\"/$\"),ToAtom(\"/\")) equation; equation := Subst(ToAtom(\"^$\"),ToAtom(\"^\")) equation;};AddDollarSigns(equation) :={ equation := Subst(ToAtom(\"+\"),ToAtom(\"+$\")) equation; equation := Subst(ToAtom(\"-\"),ToAtom(\"-$\")) equation; equation := Subst(ToAtom(\"*\"),ToAtom(\"*$\")) equation; equation := Subst(ToAtom(\"/\"),ToAtom(\"/$\")) equation; equation := Subst(ToAtom(\"^\"),ToAtom(\"^$\")) equation;};TexFormNoDollarSigns(equation) := { ToAtom(ListToString(Remove(StringToList(TeXForm(RemoveDollarSigns(equation))),\"$\")));};Isolax(side, equation) :={ side : equation /: [  1: -$ _lhs == _rhs <- [lhs == -$ rhs, \"UnaryMinus\", \"Multiply \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ExpressionToString(-1) ~ \":\", -1 *$ -$ lhs == rhs *$ -1],  1: _term1 +$ _term2 == _rhs <- [term1 == rhs -$ term2, \"Addition1\", \"Subtract \\\\ \" ~ ExpressionToString(term2) ~ \" \\\\ from \\\\ both \\\\ sides:\", -$ term2 +$ term1 +$ term2 == rhs -$ term2], 2: _term1 +$ _term2 == _rhs <- [term2 == rhs -$ term1, \"Addition2\", \"Subtract \\\\ \" ~ ExpressionToString(term1) ~ \" \\\\ from \\\\ both \\\\ sides:\", -$ term1 +$ term1 +$ term2 == rhs -$ term1],  1: _term1 -$ _term2 == _rhs <- [term1 == rhs +$ term2, \"Subtraction1\", \"Add \\\\ \" ~ ExpressionToString(term2) ~ \" \\\\ to \\\\ both \\\\ sides:\", term2 +$ term1 -$ term2 == rhs +$ term2], 2: _term1 -$ _term2 == _rhs <- [term2 == term1 -$ rhs, \"Subtraction2\", \"Add \\\\ \" ~ ExpressionToString(term1) ~ \" \\\\ to \\\\ both \\\\ sides:\", term1 +$ term1 -$ term2 == rhs +$ term1],  1: _term1 *$ _term2 == _rhs <- [term1 == rhs /$ term2, \"Multiplication1\", \"Divide \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ExpressionToString(term2) ~ \":\", (term1 *$ term2) /$ term2 == rhs /$ term2], 2: _term1 *$ _term2 == _rhs <- [term2 == rhs /$ term1, \"Multiplication2\", \"Divide \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ExpressionToString(term1) ~ \":\", (term1 *$ term2) /$ term1 == rhs /$ term1],  1: _term1 /$ _term2 == _rhs <- [term1 == rhs *$ term2, \"Division1\", \"Multiply \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ExpressionToString(term2) ~ \":\", term2 *$ (term1 /$ term2) == rhs *$ term2], 2: _term1 /$ _term2 == _rhs <- [[rhs *$ term2 == term1, term2 == term1 /$ rhs], \"Division2\", [\"Multiply \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ExpressionToString(term2) ~ \" \\\\ and \\\\ exchange \\\\ sides:\", \"Divide \\\\ both \\\\ sides \\\\ by \\\\ \" ~ ToString(TexFormNoDollarSigns(rhs)) ~ \":\"], [(term1 /$ term2) *$ term2 == rhs *$ term2, (rhs *$ term2) /$ rhs == term1 /$ rhs]],      1: _term1 ^$ _term2 == _rhs <- [term1 == rhs^$(1/$term2), \"Exponentiation1b\", \"Take \\\\ root \\\\ \" ~ ExpressionToString(term2) ~ \" \\\\ of \\\\ both \\\\ sides:\", (term1 ^$ term2)^$(1/term2) == rhs^$(1/$ term2)],   ]; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BubbleSortIndexed( L_List?, _compare ) <--{ Local(list,indices,pairs,sortedPairs); list := FlatCopy(L); indices := Table(i,i,1,Length(list),1); pairs := Transpose(list~[indices]); sortedPairs := Decide( Apply(compare,[3,5]),  BubbleSort( pairs, Lambda([X,Y],X[1] <? Y[1]) ), BubbleSort( pairs, Lambda([X,Y],X[1] >? Y[1]) ) ); Transpose(sortedPairs);};";
