...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(MathPiperInitLoad,[]);RuleHoldArguments(MathPiperInitLoad, 0, 1, True){  StandardOperatorsLoad(); Constant('True);  Constant('False);  Constant('Empty);  Constant('Infinity);  Constant('Undefined);  Constant('List);  Constant('Block);};";
        scriptMap.put("MathPiperInitLoad",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Nth\",[_alist,_aindex]);RuleHoldArguments(\"Nth\",2,10, And?(Equal?(Function?(alist),True), Equal?(Integer?(aindex),True), Not?(Equal?(First(FunctionToList(alist)),'Nth)) )) MathNth(alist,aindex);RuleHoldArguments(\"Nth\",2,14, And?(Equal?(String?(alist),True),List?(aindex)) ){ Local(result); result:=\"\"; ForEach(i,aindex) { result := result ~ StringMidGet(i,1,alist); }; result;};RuleHoldArguments(\"Nth\",2,15,Equal?(String?(alist),True)){ StringMidGet(aindex,1,alist);};RuleHoldArguments(\"Nth\",2,20,Equal?(List?(aindex),True)){ Map('[[ii],alist[ii]],[aindex]);};RuleHoldArguments(\"Nth\",2,30, And?( Equal?(Generic?(alist),True), Equal?(GenericTypeName(alist),\"Array\"), Equal?(Integer?(aindex),True) ) ){ ArrayGet(alist,aindex);};RuleHoldArguments(\"Nth\",2,40,Equal?(String?(aindex),True)){ Local(as); as := Assoc(aindex,alist); Decide(Not?(Equal?(as,Empty)),Assign(as,Nth(as,2))); as;};";
        scriptMap.put("Nth",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"ArgumentsCount\",[_aLeft]) Length(FunctionToList(aLeft))-1;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"==\",[_left,_right]);";
        scriptMap.put("==",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"--\",[_aVar]){ MacroAssign(aVar,SubtractN(Eval(aVar),1)); Eval(aVar);};UnFence(\"--\",1);HoldArgument(\"--\",\"aVar\");";
        scriptMap.put("--",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(numericMode){ numericMode := False;  LocalSymbols(previousNumericMode, previousPrecision, numericResult)  Macro(\"NM\",[_expression, _precision]) {      Local(previousNumericMode, previousPrecision, numericResult, exception); previousPrecision := BuiltinPrecisionGet();  BuiltinPrecisionSet(@precision+5); AssignCachedConstantsN(); previousNumericMode := numericMode; numericMode := True; exception := False;  ExceptionCatch( numericResult:=Eval(@expression), exception := ExceptionGet() );    Decide(Assigned?(numericResult) And? Decimal?(numericResult), numericResult := RoundToN(numericResult, @precision));  numericMode := previousNumericMode; Decide(Not? numericMode, { ClearCachedConstantsN(); } ); BuiltinPrecisionSet(previousPrecision); Check(exception =? False, exception[\"type\"], exception[\"message\"]); numericResult; };  LocalSymbols(precision,heldExpression)  Macro(\"NM\",[_expression]) { Local(precision, heldExpression); precision := BuiltinPrecisionGet(); heldExpression := Hold(@expression); `NM(@heldExpression, @precision); };   LocalSymbols(result)  Macro(\"NonNM\",[_expression]) { Local(result); GlobalPush(numericMode); numericMode := False; result := (@expression); numericMode := GlobalPop(); result; };  Function(\"NumericMode?\",[]) numericMode;}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"++\",[_aVar]){ MacroAssign(aVar,AddN(Eval(aVar),1)); Eval(aVar);};UnFence(\"++\",1);HoldArgument(\"++\",\"aVar\");";
        scriptMap.put("++",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " RulebaseHoldArguments(\"'\", [_expression]); RuleHoldArguments(\"'\", 1, 100, True) `(Hold(@expression)); HoldArgument(\"'\",\"expression\");";
        scriptMap.put("'",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "51 # -Undefined <-- Undefined;54 # - (- _x) <-- x;55 # (- (x_Number?)) <-- SubtractN(0,x);100 # _x - n_Constant?*(_x) <-- (1-n)*x;100 # n_Constant?*(_x) - _x <-- (n-1)*x;110 # - (_x - _y) <-- y-x;111 # - (x_Number? / _y) <-- (-x)/y; LocalSymbols(x){ 200 # - (x_List?) <-- MapSingle(\"-\",x);};50 # x_Number? - y_Number? <-- SubtractN(x,y);50 # x_Number? - y_Number? <-- SubtractN(x,y);60 # Infinity - Infinity <-- Undefined;100 # 0 - _x <-- -x;100 # _x - 0 <-- x;100 # _x - _x <-- 0;107 # -( (-(_x))/(_y)) <-- x/y;107 # -( (_x)/(-(_y))) <-- x/y;110 # _x - (- _y) <-- x + y;110 # _x - (y_NegativeNumber?) <-- x + (-y);111 # (_x + _y)- _x <-- y;111 # (_x + _y)- _y <-- x;112 # _x - (_x + _y) <-- - y;112 # _y - (_x + _y) <-- - x;113 # (- _x) - _y <-- -(x+y);113 # (x_NegativeNumber?) - _y <-- -((-x)+y);113 # (x_NegativeNumber?)/_y - _z <-- -((-x)/y+z);LocalSymbols(x,y,xarg,yarg){ 10 # ((x_List?) - (y_List?))::(Length(x)=?Length(y)) <-- { Map('[[xarg,yarg],xarg-yarg],[x,y]); };};240 # (x_List? - y_NonObject?)::Not?(List?(y)) <-- -(y-x);241 # (x_NonObject? - y_List?)::Not?(List?(x)) <--{ Local(i,result); result:=[]; For(i:=1,i<=?Length(y),i++) { DestructiveInsert(result,i,x - y[i]); }; result;};250 # z_Infinity? - Complex(_x,_y) <-- Complex(-x+z,-y);250 # Complex(_x,_y) - z_Infinity? <-- Complex(x-z,y);251 # z_Infinity? - _x <-- z;251 # _x - z_Infinity? <-- -z;250 # Undefined - _y <-- Undefined;250 # _x - Undefined <-- Undefined;210 # x_Number? - (y_Number? / z_Number?) <--(x*z-y)/z;210 # (y_Number? / z_Number?) - x_Number? <--(y-x*z)/z;210 # (x_Number? / v_Number?) - (y_Number? / z_Number?) <--(x*z-y*v)/(v*z);";
        scriptMap.put("-",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # + _x <-- x;50 # x_Number? + y_Number? <-- AddN(x,y);100 # 0 + _x <-- x;100 # _x + 0 <-- x;100 # _x + _x <-- 2*x;100 # _x + n_Constant?*(_x) <-- (n+1)*x;100 # n_Constant?*(_x) + _x <-- (n+1)*x;101 # _x + - _y <-- x-y;101 # _x + (- _y)/(_z) <-- x-(y/z);101 # (- _y)/(_z) + _x <-- x-(y/z);101 # (- _x) + _y <-- y-x;102 # _x + y_NegativeNumber? <-- x-(-y);102 # _x + y_NegativeNumber? * _z <-- x-((-y)*z);102 # _x + (y_NegativeNumber?)/(_z) <-- x-((-y)/z);102 # (y_NegativeNumber?)/(_z) + _x <-- x-((-y)/z);102 # (x_NegativeNumber?) + _y <-- y-(-x);150 # _n1 / _d + _n2 / _d <-- (n1+n2)/d;200 # (x_Number? + _y)::Not?(Number?(y)) <-- y+x;200 # ((_y + x_Number?) + _z)::Not?(Number?(y) Or? Number?(z)) <-- (y+z)+x;200 # ((x_Number? + _y) + z_Number?)::Not?(Number?(y)) <-- y+(x+z);200 # ((_x + y_Number?) + z_Number?)::Not?(Number?(x)) <-- x+(y+z);210 # x_Number? + (y_Number? / z_Number?) <--(x*z+y)/z;210 # (y_Number? / z_Number?) + x_Number? <--(x*z+y)/z;210 # (x_Number? / v_Number?) + (y_Number? / z_Number?) <--(x*z+y*v)/(v*z);220 # (xlist_List? + ylist_List?)::(Length(xlist)=?Length(ylist)) <-- Map(\"+\",[xlist,ylist]);SumListSide(_x, y_List?) <--{ Local(i,result); result:=[]; For(i:=1,i<=?Length(y),i++) { DestructiveInsert(result,i,x + y[i]); }; result;};240 # (x_List? + _y)::Not?(List?(y)) <-- SumListSide(y,x);241 # (_x + y_List?)::Not?(List?(x)) <-- SumListSide(x,y);250 # z_Infinity? + Complex(_x,_y) <-- Complex(x+z,y);250 # Complex(_x,_y) + z_Infinity? <-- Complex(x+z,y);251 # z_Infinity? + _x <-- z;251 # _x + z_Infinity? <-- z;250 # Undefined + _y <-- Undefined;250 # _x + Undefined <-- Undefined;";
        scriptMap.put("+",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"CosN\",[_x])Trigonometry(x,0.0,1.0,1.0);";
        scriptMap.put("CosN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"ExpN\", [_x]) Decide(Equal?(x,0),1, Decide(LessThan?(x,0),DivideN(1, ExpN(MathNegate(x))), Decide(GreaterThan?(x,1), MathExpDoubling(MathExpTaylor0(MathMul2Exp(x,MathNegate(MathBitCount(x)))), MathBitCount(x)), MathExpTaylor0(x) )));";
        scriptMap.put("ExpN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathExpDoubling\", [_value, _n]){ Local(shift, result); Assign(shift, n); Assign(result, value); While (GreaterThan?(shift,0))  { Assign(result, MultiplyN(result, result)); Assign(shift, AddN(shift,MathNegate(1))); }; result;};";
        scriptMap.put("MathExpDoubling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathExpTaylor0\",[_x]){ Check(x<=?1 And? x>=?0,\"\",\" Argument must be between 0 and 1 inclusive.\"); Local(i,aResult,term,eps);     Assign(i,0);  Assign(aResult,1.0);  Assign(term,1.0); Assign(eps,MathIntPower(10,MathNegate(BuiltinPrecisionGet())));   While(GreaterThan?(AbsN(term),eps)) {  Assign(i,AddN(i,1));  Assign(term,DivideN(MultiplyN(term,x),i));  Assign(aResult,AddN(aResult,term)); }; aResult;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathIntPower\", [_x,_y]) Decide(Equal?(x,0),0,Decide(Equal?(x,1),1, Decide(Integer?(y),Decide(LessThan?(y,0),  DivideN(1, PositiveIntPower(AddN(x,0.),MathNegate(y))),  PositiveIntPower(x,y) ),  False) ));";
        scriptMap.put("MathIntPower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathMul2Exp\", [_x,_n])  Decide(GreaterThan?(n,0), MultiplyN(x, MathIntPower(2,n)), DivideN(x, MathIntPower(2,MathNegate(n))));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PositiveIntPower\", [_x,_n]){ Local(result,unit); Decide(LessThan?(n,0), False, { Assign(unit,1);  Assign(result, unit); Decide(Equal?(n,0),unit, Decide(Equal?(n,1),x, { While(GreaterThan?(n,0)) { Decide( Equal?(BitAnd(n,1), 1), Assign(result, MultiplyN(result,x)) ); Assign(x, MultiplyN(x,x)); Assign(n,ShiftRight(n,1)); }; result; } ) ); });};";
        scriptMap.put("PositiveIntPower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PowerN\", [_x,_y]) Decide(Equal?(x,0),0, Decide(Equal?(x,1),1, Decide(Integer?(y), MathIntPower(x,y), False) ));";
        scriptMap.put("PowerN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"SinN\",[_x])Trigonometry(x,1.0,x,x);";
        scriptMap.put("SinN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"TanN\",[_x])DivideN(SinN(x),CosN(x));";
        scriptMap.put("TanN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"Trigonometry\",[_x,_i,_sum,_term]){ Local(x2,orig,eps,previousPrec,newPrec); Assign(previousPrec,BuiltinPrecisionGet()); Assign(newPrec,AddN(BuiltinPrecisionGet(),2)); Assign(x2,MultiplyN(x,x)); BuiltinPrecisionSet(newPrec); Assign(eps,MathIntPower(10,MathNegate(previousPrec))); While(GreaterThan?(AbsN(term),eps)) { Assign(term,MultiplyN(term,x2)); Assign(i,AddN(i,1.0)); Assign(term,DivideN(term,i)); Assign(i,AddN(i,1.0)); Assign(term,DivideN(MathNegate(term),i)); BuiltinPrecisionSet(previousPrec); Assign(sum, AddN(sum, term)); BuiltinPrecisionSet(newPrec); }; BuiltinPrecisionSet(previousPrec); sum;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(a,x){Function(\"Conjugate\",[_a]) SubstituteApply(a,[[x],Type(x)=?\"Complex\"],[[x],Complex(x[1],-(x[2]))]);}; ";
        scriptMap.put("Conjugate",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Constant('II);10 # II^n_NegativeInteger? <-- (-II)^(-n);20 # (II^_n)::(Even?(n) =? True) <-- (-1)^(n>>1);20 # (II^_n)::(Odd?(n) =? True) <-- II*(-1)^(n>>1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Magnitude\",[_x]) { Sqrt(Re(x)^2 + Im(x)^2);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Else\",[_ifthen,_otherwise]);0 # (If(_predicate) _body Else _otherwise)::(Eval(predicate) =? True) <-- Eval(body);0 # (If(_predicate) _body Else _otherwise)::(Eval(predicate) =? False) <-- Eval(otherwise);1 # (If(_predicate) _body Else _otherwise) <-- ListToFunction([ToAtom(\"Else\"), ListToFunction([ToAtom(\"If\"), (Eval(predicate)), body]), otherwise]);HoldArgument(\"Else\",\"ifthen\");HoldArgument(\"Else\",\"otherwise\");UnFence(\"Else\",2);";
        scriptMap.put("Else",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"For\",[_start,_predicate,_increment,_body]){ Eval(start); While (Equal?(Eval(predicate),True)) { Eval(body); Eval(increment); };};UnFence(\"For\",4);HoldArgumentNumber(\"For\",4,1);HoldArgumentNumber(\"For\",4,2);HoldArgumentNumber(\"For\",4,3);HoldArgumentNumber(\"For\",4,4);";
        scriptMap.put("For",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"ForEach\",[_item,_listOrString,_body]){ Decide(And?(Equal?(Generic?(listOrString),True), Equal?(GenericTypeName(listOrString),\"Array\") ), `ForEachInArray(@item,listOrString,@body), { MacroLocal(item);  Decide(String?(listOrString), {  Local(index, stringLength);  stringLength := Length(listOrString);  index := 1; While(index <=? stringLength ) { MacroAssign(item,listOrString[index] );  Eval(body);  index++; }; }, { Local(foreachtail); Assign(foreachtail,listOrString); While(Not?(Equal?(foreachtail,[]))) { MacroAssign(item,First(foreachtail)); Eval(body); Assign(foreachtail,Rest(foreachtail)); }; }); });};UnFence(\"ForEach\",3);HoldArgumentNumber(\"ForEach\",3,1);HoldArgumentNumber(\"ForEach\",3,3);";
        scriptMap.put("ForEach",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"If\",[_predicate,_body]);(If(True) _body) <-- Eval(body);HoldArgument(\"If\",\"body\");UnFence(\"If\",2);";
        scriptMap.put("If",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MacroRulebaseHoldArguments(\"Lambda\",[_args,_body]);";
        scriptMap.put("Lambda",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"Until\",[_predicate,_body]){ Eval(body); While (Equal?(Eval(predicate),False)) { Eval(body); }; True;};UnFence(\"Until\",2);HoldArgumentNumber(\"Until\",2,1);HoldArgumentNumber(\"Until\",2,2);";
        scriptMap.put("Until",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(Verbose) { Assign(Verbose,False); Function(\"V\",[_aNumberBody]) { Local(prevVerbose,result); Assign(prevVerbose,Verbose); Assign(Verbose,True); Assign(result,Eval(aNumberBody)); Assign(Verbose,prevVerbose); result; }; Function(\"InVerboseMode\",[]) Verbose;}; HoldArgument(\"V\",\"aNumberBody\");UnFence(\"V\",1);";
        scriptMap.put("V",scriptString);
        scriptMap.put("InVerboseMode",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"AssignArray\", [_listOrAssociationList, _indexOrKey, _newValue]);UnFence(\"AssignArray\", 3);RuleHoldArguments(\"AssignArray\", 3, 1, String?(indexOrKey)){ Local(keyValuePair); keyValuePair := Assoc(indexOrKey, listOrAssociationList); Decide(keyValuePair =? Empty, DestructiveInsert(listOrAssociationList, 1 , [indexOrKey, newValue]), DestructiveReplace(keyValuePair, 2 , newValue) ); True;};RuleHoldArguments(\"AssignArray\",3,1, And?( Equal?(Generic?(listOrAssociationList), True), Equal?(GenericTypeName(listOrAssociationList), \"Array\"))){ ArraySet(listOrAssociationList, indexOrKey, newValue);};RuleHoldArguments(\"AssignArray\",3 , 2, True){ DestructiveReplace(listOrAssociationList, indexOrKey, newValue); True;};";
        scriptMap.put("AssignArray",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MacroRulebaseHoldArguments(\"Defun\",[_func,_args,_body]);RuleHoldArguments(\"Defun\",3,0,True){ Local(nrargs); Assign(nrargs,Length(@args)); Retract(@func, `(@nrargs)); RulebaseHoldArguments(@func,@args); Local(fn,bd); Assign(fn,Hold(@func)); Assign(bd,Hold(@body)); `RuleHoldArguments(@fn, @nrargs, 0,True)(@bd);};";
        scriptMap.put("Defun",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Function\",[_oper,_args,_body]);RuleHoldArguments(\"Function\",3,2047, And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") ))){ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); RulebaseListedEvaluateArguments(oper,args); RuleEvaluateArguments(oper,Length(args),1025,True) body; };RuleHoldArguments(\"Function\",3,2048,True){ Retract(oper,Length(args)); RulebaseEvaluateArguments(oper,args); RuleEvaluateArguments(oper,Length(args),1025,True) body;};RulebaseHoldArguments(\"Function\",[_oper]);RuleHoldArguments(\"Function\",1,2047, And?(Function?(oper), GreaterThan?(Length(oper), 1), Equal?( MathNth(oper, Length(oper)), ToAtom(\"...\") ))){ Local(args); Assign(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  Decide(RulebaseDefined(Type(oper),Length(args)), False,  RulebaseListedEvaluateArguments(Type(oper),args) );};RuleHoldArguments(\"Function\",1,2048, And?(Function?(oper))){ Local(args); Assign(args,Rest(FunctionToList(oper))); Decide(RulebaseDefined(Type(oper),Length(args)), False,  RulebaseEvaluateArguments(Type(oper),args) );};HoldArgument(\"Function\",\"oper\");HoldArgument(\"Function\",\"args\");HoldArgument(\"Function\",\"body\");";
        scriptMap.put("Function",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"HoldArgumentNumber\",[_function,_arity,_index]){ Local(args); args:=RulebaseArgumentsList(function,arity); ApplyFast(\"HoldArgument\",[function,args[index]]);};";
        scriptMap.put("HoldArgumentNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Macro\",[_oper,_args,_body]);HoldArgument(\"Macro\",\"oper\");HoldArgument(\"Macro\",\"args\");HoldArgument(\"Macro\",\"body\");RuleHoldArguments(\"Macro\",3,2047, And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") ))){ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); `MacroRulebaseListedHoldArguments(@oper,@args); RuleEvaluateArguments(oper,Length(args),1025,True) body; };RuleHoldArguments(\"Macro\",3,2048,True){ Retract(oper,Length(args)); `MacroRulebaseHoldArguments(@oper,@args); RuleEvaluateArguments(oper,Length(args),1025,True) body;};RulebaseHoldArguments(\"Macro\",[_oper]);RuleHoldArguments(\"Macro\",1,2047, And?(Function?(oper), GreaterThan?(Length(oper), 1), Equal?( MathNth(oper, Length(oper)), ToAtom(\"...\") ))){ Local(args,name); Assign(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  Assign(name,Type(oper)); Decide(RulebaseDefined(Type(oper),Length(args)), False,  `MacroRulebaseListedHoldArguments(@name,@args) );};RuleHoldArguments(\"Macro\",1,2048, And?(Function?(oper))){ Local(args,name); Assign(args,Rest(FunctionToList(oper))); Assign(name,Type(oper)); Decide(RulebaseDefined(Type(oper),Length(args)), False,  { `MacroRulebaseHoldArguments(@name,@args); } );};";
        scriptMap.put("Macro",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"TemplateFunction\",[_oper,_args,_body]);HoldArgument(\"TemplateFunction\",\"oper\");HoldArgument(\"TemplateFunction\",\"args\");HoldArgument(\"TemplateFunction\",\"body\");RuleHoldArguments(\"TemplateFunction\",3,2047,True){ Retract(oper,Length(args));  Local(arglist);  arglist:=FlatCopy(args);  DestructiveAppend(arglist,[args,ListToFunction(['Hold,body])]);  arglist:=ApplyFast(\"LocalSymbols\",arglist); RulebaseEvaluateArguments(oper,arglist[1]);  RuleEvaluateArguments(oper,Length(args),1025,True) arglist[2];};";
        scriptMap.put("TemplateFunction",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Unholdable\",[_var]);HoldArgument(\"Unholdable\",\"var\");UnFence(\"Unholdable\",1);RuleHoldArguments(\"Unholdable\",1,10,Equal?(Type(Eval(var)),\"Eval\")){ MacroAssign(var,Eval(Eval(var))); };RuleHoldArguments(\"Unholdable\",1,20,True){  True;};";
        scriptMap.put("Unholdable",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\":=\",[_aLeftAssign, _aRightAssign]);UnFence(\":=\", 2);HoldArgument(\":=\", \"aLeftAssign\");HoldArgument(\":=\", \"aRightAssign\");RuleHoldArguments(\":=\", 2, 0, Atom?(aLeftAssign)){ Check( Not? Number?(aLeftAssign), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); MacroAssign(aLeftAssign,Eval(aRightAssign)); Eval(aLeftAssign);};RuleHoldArguments(\":=\",2 , 0, List?(aLeftAssign)){   Local(index, right, result);  Assign(result, []);  Assign(index, 1);  Assign(right, Eval(aRightAssign));  While(index <=? Length(aLeftAssign)) { MacroAssign(aLeftAssign[index], right[index]);  DestructiveAppend(result, right[index]);  index++; };  result;};RuleHoldArguments(\":=\", 2, 10, Function?(aLeftAssign) And? (First(FunctionToList(aLeftAssign)) =? 'Nth)){ Local(listOrAssociationList, indexOrKey, argumentsList); Assign(argumentsList, (FunctionToList(aLeftAssign))); Assign(argumentsList, Rest(argumentsList)); Assign(listOrAssociationList, Eval(First(argumentsList))); Assign(argumentsList, Rest(argumentsList)); Assign(indexOrKey, Eval(First(argumentsList))); AssignArray(listOrAssociationList, indexOrKey, Eval(aRightAssign));};RuleHoldArguments(\":=\", 2, 30, Function?(aLeftAssign) And? Not?(Equal?(aLeftAssign[0], ToAtom(\":=\"))) ){ Check( Not? Equal?(aLeftAssign[0], ToAtom(\"/\")), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); Local(oper,args,arity); Assign(oper,ToString(aLeftAssign[0])); Assign(args,Rest(FunctionToList(aLeftAssign))); Decide( And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") )),  { DestructiveDelete(args, Length(args));  Assign(arity, Length(args)); Retract(oper, arity); RulebaseListedEvaluateArguments(oper, MapSingle(Lambda([x],ToAtom(ConcatStrings(\"_\",ToString(x)))), args)); },  { Assign(arity, Length(args)); Retract(oper, arity); RulebaseEvaluateArguments(oper, MapSingle(Lambda([x],ToAtom(ConcatStrings(\"_\",ToString(x)))), args)); } ); Unholdable(aRightAssign); RuleEvaluateArguments(oper, arity, 1025, True) aRightAssign;};";
        scriptMap.put(":=",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"@\",[_func,_arg]);RuleHoldArguments(\"@\",2,1,List?(arg)) Apply(func,arg);RuleHoldArguments(\"@\",2,2,True ) Apply(func,[arg]);";
        scriptMap.put("@",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"/@\",[_func,_lst]) Apply(\"MapSingle\",[func,lst]);";
        scriptMap.put("/@",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"~\",[_head,_tail]);RuleHoldArguments(\"~\",2,20,List?(head) And? Not? List?(tail) ) Concat(head,[tail]);RuleHoldArguments(\"~\",2,30,List?(tail) ) Concat([head],tail);RuleHoldArguments(\"~\",2,10,String?(tail) And? String?(head)) ConcatStrings(head,tail);UnFence(\"~\",2);";
        scriptMap.put("~",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"DefaultPrint\", [_x]){ DumpErrors(); WriteString(\"Result: \"); Write(x); WriteString(\";\");};HoldArgument(\"DefaultPrint\", \"x\");";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EchoInternal(string_String?) <--{ WriteString(string);};20 # EchoInternal(_item) <--{ Write(item);Space();};RulebaseListedHoldArguments(\"Echo\",[_firstParameter, _parametersList]);5 # Echo(_firstParameter) <-- Echo(firstParameter, []);10 # Echo(_firstParameter, parametersList_List?) <--{ EchoInternal(firstParameter); ForEach(item,parametersList) EchoInternal(item); NewLine(); };20 # Echo(_firstParameter, _secondParameter) <-- Echo(firstParameter, [secondParameter]);Echo() := NewLine();";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"TableForm\",[_list]){ Local(i); ForEach(i,list) { Write(i); NewLine(); }; True;};";
        scriptMap.put("TableForm",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Macro(\"Tell\",[_id]) {Echo(<<,@id,>>);};Macro(\"Tell\",[_id,_x]) {Echo(<<,@id,>>,Hold(@x),\": \",Eval(@x));};";
        scriptMap.put("Tell",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Macro(\"TellNow\",[_id]) {SysOut(\"<< \",@id,\" >>\");};Macro(\"TellNow\",[_id,_x]) {SysOut(\"<< \",@id,\" >> \",Hold(@x),\": \",Eval(@x));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"ZeroVector\",[_n]){ Local(i,result); result:=[]; For(i:=1,i<=?n,i++) { DestructiveInsert(result,1,0); }; result;};";
        scriptMap.put("ZeroVector",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"MatrixColumn\",[_matrix,_col]){ Local(m); m:=matrix[1]; Check(col >? 0, \"Argument\", \"MatrixColumn: column index out of range\"); Check(col <=? Length(m), \"Argument\", \"MatrixColumn: column index out of range\"); Local(i,result); result:=[]; For(i:=1,i<=?Length(matrix),i++) DestructiveAppend(result,matrix[i][col]); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"MatrixRow\",[_matrix,_row]){ Check(row >? 0, \"Argument\", \"MatrixRow: row index out of range\"); Check(row <=? Length(matrix), \"Argument\", \"MatrixRow: row index out of range\"); Local(result); result:=matrix[row]; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Append\",[_list,_element]){ Check(List?(list), \"Argument\", \"The first argument must be a list.\"); Insert(list,Length(list)+1,element);};";
        scriptMap.put("Append",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"BuildList\",[_body,_var,_countfrom,_countto,_step]) { MacroLocal(var); Local(result,nr,ii); result:=[]; nr := (countto - countfrom) / step; ii := 0; While( ii <=? nr ) { MacroAssign( var, countfrom + ii * step ); DestructiveInsert( result,1,Eval(body) ); Assign(ii,AddN(ii,1)); }; DestructiveReverse(result); };HoldArgumentNumber(\"BuildList\",5,1); HoldArgumentNumber(\"BuildList\",5,2); UnFence(\"BuildList\",5);";
        scriptMap.put("BuildList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Contains?\",[_list,_element]){ Local(result); Assign(result,False); While(And?(Not?(result), Not?(Equal?(list, [])))) { Decide(Equal?(First(list),element), Assign(result, True), Assign(list, Rest(list)) ); }; result;};";
        scriptMap.put("Contains?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Count\",[_list,_element]){ Local(result); Assign(result,0); ForEach(item,list) Decide(Equal?(item, element), Assign(result,AddN(result,1))); result;};";
        scriptMap.put("Count",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"DestructiveAppend\",[_list,_element]){ DestructiveInsert(list,Length(list)+1,element);};";
        scriptMap.put("DestructiveAppend",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"DestructiveAppendList\",[_list,_toadd]){ Local(i,nr); nr:=Length(toadd); For(i:=1,i<=?nr,i++) { DestructiveAppend(list,toadd[i]); }; True;};";
        scriptMap.put("DestructiveAppendList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Difference\",[_list1,_list2]){ Local(l2,index,result); l2:=FlatCopy(list2); result:=FlatCopy(list1); ForEach(item,list1) { Assign(index,Find(l2,item)); Decide(index>?0, { DestructiveDelete(l2,index); DestructiveDelete(result,Find(result,item)); } ); }; result;};";
        scriptMap.put("Difference",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Drop\", [_lst, _range]);RuleHoldArguments(\"Drop\", 2, 1, List?(range)) Concat(Take(lst,range[1]-1), Drop(lst, range[2]));RuleHoldArguments(\"Drop\", 2, 2, range >=? 0) Decide( range =? 0 Or? lst =? [], lst, Drop( Rest(lst), range-1 ));RuleHoldArguments(\"Drop\", 2, 2, range <? 0) Take( lst, Decide(AbsN(range) <? Length(lst), Length(lst) + range, 0 ) );";
        scriptMap.put("Drop",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FillList\", [_aItem, _aLength]){ Local(i, aResult); aResult:=[]; For(i:=0, i<?aLength, i++) DestructiveInsert(aResult,1,aItem); aResult;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Macro(\"MacroMapArgs\",[_expr,_oper]){ Local(ex,tl,op); Assign(op,@oper); Assign(ex,FunctionToList(@expr)); Assign(tl,Rest(ex)); ListToFunction(Concat([ex[1]], `MacroMapSingle(@op,Hold(@tl))) );};UnFence(\"MacroMapArgs\",2);HoldArgument(\"MacroMapArgs\",\"oper\");";
        scriptMap.put("MacroMapArgs",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"MacroMapSingle\",[_func,_list]){ Local(mapsingleresult); mapsingleresult:=[]; ForEach(mapsingleitem,list) { DestructiveInsert(mapsingleresult,1, `ApplyFast(func,[Hold(Hold(@mapsingleitem))])); }; DestructiveReverse(mapsingleresult);};UnFence(\"MacroMapSingle\",2);HoldArgument(\"MacroMapSingle\",\"func\");HoldArgument(\"MacroMapSingle\",\"list\");";
        scriptMap.put("MacroMapSingle",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(func,lists,mapsingleresult,mapsingleitem){ TemplateFunction(\"Map\",[_func,_lists]) { Local(mapsingleresult,mapsingleitem); mapsingleresult:=[]; lists:=Transpose(lists); ForEach(mapsingleitem,lists) { DestructiveInsert(mapsingleresult,1,Apply(func,mapsingleitem)); }; DestructiveReverse(mapsingleresult); }; UnFence(\"Map\",2); HoldArgument(\"Map\",\"func\");};";
        scriptMap.put("Map",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"MapArgs\",[_expr,_oper]){ Assign(expr,FunctionToList(expr)); ListToFunction(Concat([expr[1]], Apply(\"MapSingle\",[oper,Rest(expr)]) ) );};UnFence(\"MapArgs\",2);HoldArgument(\"MapArgs\",\"oper\");";
        scriptMap.put("MapArgs",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"MapSingle\",[_func,_list]){ Local(mapsingleresult); mapsingleresult:=[]; ForEach(mapsingleitem,list) { DestructiveInsert(mapsingleresult,1, Apply(func,[mapsingleitem])); }; DestructiveReverse(mapsingleresult);};UnFence(\"MapSingle\",2);HoldArgument(\"MapSingle\",\"func\");";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Pop\",[_stack,_index]){ Local(result); result:=stack[index]; DestructiveDelete(stack,index); result;};";
        scriptMap.put("Pop",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"PopBack\",[_stack]) Pop(stack,Length(stack));";
        scriptMap.put("PopBack",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"PopFront\",[_stack]) Pop(stack,1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Push\",[_stack,_element]){ DestructiveInsert(stack,1,element);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"RemoveDuplicates\",[_list]){ Local(result); Assign(result,[]); ForEach(item,list) Decide(Not?(Contains?(result,item)),DestructiveAppend(result,item)); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(predicate,list,result,item){ Function(\"Select\",[_list,_predicate]) { Local(result); result:=[]; ForEach(item,list) { Decide(Apply(predicate,[item]),DestructiveAppend(result,item)); }; result; }; HoldArgument(\"Select\",\"predicate\"); UnFence(\"Select\",2);};";
        scriptMap.put("Select",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Swap\",[\"list\",_index1,_index2]){ Local(item1,item2); item1:=list[index1]; item2:=list[index2]; list[index1] := item2; list[index2] := item1;};";
        scriptMap.put("Swap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Take\", [_lst, _range]);RuleHoldArguments(\"Take\", 2, 1, List?(range)) Take( Drop(lst, range[1] -1), range[2] - range[1] + 1);RuleHoldArguments(\"Take\", 2, 2, range >=? 0) Decide( Length(lst)=?0 Or? range=?0, [], Concat([First(lst)], Take(Rest(lst), range-1)));RuleHoldArguments(\"Take\", 2, 2, range <? 0) Drop( lst, Decide(AbsN(range) <? Length(lst), Length(lst) + range, 0 ));";
        scriptMap.put("Take",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Union\",[_list1,_list2]){ RemoveDuplicates(Concat(list1,list2));};";
        scriptMap.put("Union",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VarList(_expr) <-- VarList(expr,\"Variable?\");Function(\"VarList\",[_expr,_filter]){ RemoveDuplicates(VarListAll(expr,filter));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Where\",[_left,_right]);UnFence(\"Where\",2);10 # (_body Where var_Atom? == _value) <-- `{      Eval(Substitute(@var, @value) @body);};  20 # (_body Where (_a And? _b)) <--{ Assign(body,`(@body Where @a)); `(@body Where @b);};30 # (_body Where []) <-- [];40 # (_body Where list_List?)::List?(list[1]) <-- { Local(head,rest); head:=First(list); rest:=Rest(list); rest:= `(@body Where @rest); `(@body Where @head) ~ rest; };50 # (_body Where list_List?) <-- { Local(head,rest); While (list !=? []) { head:=First(list); body := `(@body Where @head); list:=Rest(list); }; body; };60 # (_body Where _var == _value) <-- Substitute(var,value)body;";
        scriptMap.put("Where",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"<-\",[_left,_right]);HoldArgument(\"<-\",\"left\");HoldArgument(\"<-\",\"right\");";
        scriptMap.put("<-",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(localResult) { localResult := True;   10 # LocPredicate(exp_Atom?) <-- { Local(patternsList, result);  patternsList := patterns;  result := False;  While (patternsList !=? []) { Decide(First(First(patternsList)) =? exp, { Decide(InVerboseMode() =? True, Echo(Last(First(patternsList))));  Assign(localResult,Eval(First(Rest(First(patternsList)))));  result := True;  patternsList:=[]; }, { patternsList := Rest(patternsList); }); };  result; };   10 # LocPredicate(exp_Function?) <-- { Local(patternsList, result, head);  patternsList := patterns;  result := False;  While (patternsList !=? []) { Assign(head, First(First(patternsList)));  Decide(Not?(Atom?(head)) And? exp[0] =? head[1] And? PatternMatch?(head[2], exp), { Decide(InVerboseMode() =? True, Echo(Last(First(patternsList))));  Assign(localResult, Eval(First(Rest(First(patternsList)))));  Assign(result, True);  Assign(patternsList, []); }, { Assign(patternsList, Rest(patternsList)); }); };  result; };    20 # LocPredicate(_exp) <-- False;   LocChange(_exp) <-- localResult;  }; UnFence(\"LocPredicate\",1);UnFence(\"LocChange\",1);10 # PatternCompile([_pat, _post, _exp]) <-- [ [pat[0], PatternCreate(pat, post)], exp, \"UNNAMED\" ];20 # PatternCompile([pat_Function?, _exp]) <-- [ [pat[0], PatternCreate(pat,True)], exp, \"UNNAMED\" ];30 # PatternCompile([pat_Atom?, _exp]) <-- [ pat, exp, \"UNNAMED\" ];40 # PatternCompile(pat_Function? <- _exp) <-- { Local(justPattern, postPredicate, ruleName);  justPattern := pat;  Decide(Type(justPattern) =? \"::\", {  justPattern := pat[1]; postPredicate := pat[2]; }, {  postPredicate := True; } );    Decide(Type(justPattern) =? \"#\", {  ruleName := pat[1]; justPattern := pat[2];  Decide(Function?(justPattern), { [ [justPattern[0], PatternCreate(justPattern, postPredicate)], exp , ruleName]; }, {  [ justPattern, exp, ruleName ]; }  ); }, { ruleName := \"UNNAMED\"; justPattern := pat; [ [justPattern[0], PatternCreate(justPattern, postPredicate)], exp , ruleName]; } ); };50 # PatternCompile(pat_Atom? <- _exp) <-- [ pat, exp, \"UNNAMED\" ];PatternsCompile(patterns) :={ MapSingle(\"PatternCompile\", patterns);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"WithValue\",[_var,_val,_expr]){ Decide(List?(var), ApplyFast(\"MacroLocal\",var), MacroLocal(var) ); ApplyFast(\":=\",[var,val]); Eval(expr);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(m,n,r, NthRootTable){NthRoot(m_NonNegativeInteger?,n_Integer?)::(n>?1) <--{ Local(r); r:=NthRootRestore(m,n); Decide(Length(r)=?0, { r:=NthRootCalc(m,n); NthRootSave(m,n,r); }); r;};Function(\"NthRootCalc\",[_m,_n]){ Local(i,j,f,r,in); Assign(i,2); Assign(j,Ceil(FastPower(m,NM(1.0/n))+1)); Assign(f,1); Assign(r,m);     While(LessThan?(i,j)) { Assign(in,PowerN(i,n)); While(Equal?(ModuloN(r,in),0)) { Assign(f,MultiplyN(f,i)); Assign(r,QuotientN(r,in)); }; While(Equal?(ModuloN(r,i),0))  Assign(r,QuotientN(r,i));   Assign(i,NextPseudoPrime(i)); Assign(j,Ceil(FastPower(r,NM(1.0/n))+1)); };  List(f,QuotientN(m,PowerN(f,n))); };Function(\"NthRootList\",[_n]){ Decide(Length(NthRootTable)>?0, { Local(p,xx); p:=Select(NthRootTable, '[[xx],First(xx)=?n]); Decide(Length(p)=?1,Rest(p[1]),List()); }, List());};Function(\"NthRootRestore\",[_m,_n]){ Local(p); p:=NthRootList(n); Decide(Length(p)>?0, { Local(r,xx); r:=Select(p, '[[xx],First(xx)=?m]); Decide(Length(r)=?1,First(Rest(r[1])),List()); }, List());};Function(\"NthRootSave\",[_m,_n,_r]){ Local(p); p:=NthRootList(n); Decide(Length(p)=?0,  DestructiveInsert(NthRootTable,1,List(n,List(m,r))), { Local(rr,xx); rr:=Select(p, '[[xx],First(xx)=?m]); Decide(Length(rr)=?0, {  DestructiveAppend(p,List(m,r)); },  False); });};Function(\"NthRootClear\",[]) AssignGlobalLazy(NthRootTable,List());NthRootClear();}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"RationalizeNumber\",[_x]){ Check(Number?(x), \"Argument\", \"RationalizeNumber: Error: \" ~ (PipeToString()Write(x)) ~\" is not a number\"); Local(n,i,bip,m); n := 1; i := 0; bip := BuiltinPrecisionGet();  While(i<=?bip And? Not?(FloatIsInt?(x))) { n := n*10;  x := x*10; i := i+1; }; m := Floor(x+0.5); (m/n);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"::\",[_a,_b]);RulebaseHoldArguments(\"DefinePattern\",[_leftOperand, _rightOperand, _rulePrecedence, _postPredicate]);RuleHoldArguments(\"DefinePattern\",4,9,Equal?(Type(leftOperand),\"::\")){ DefinePattern(leftOperand[1], rightOperand, rulePrecedence, leftOperand[2]);};RuleHoldArguments(\"DefinePattern\",4,10,True){ Local(patternFlat,patternVariables, pattern, patternOperator, arity);  Assign(patternFlat, FunctionToList(leftOperand));   Assign(patternVariables, Rest(patternFlat));   Assign(patternOperator,ToString(First(patternFlat)));   Assign(arity,Length(patternVariables));      Decide(Not?(RulebaseDefined(patternOperator,arity)), { RulebaseEvaluateArguments(patternOperator,MakeVector(_arg,arity)); } );  Assign(pattern,PatternCreate(patternVariables,postPredicate));  RulePatternEvaluateArguments(patternOperator,arity,rulePrecedence, pattern)rightOperand;  True;};";
        scriptMap.put("DefinePattern",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"MakeVector\",[_vec,_dimension]);RuleHoldArguments(\"MakeVector\",2,1,True){ Local(res,i); res:=[]; i:=1; Assign(dimension,AddN(dimension,1)); While(LessThan?(i,dimension)) { DestructiveInsert(res,1,ToAtom(ConcatStrings(ToString(vec),ToString(i)))); Assign(i,AddN(i,1)); }; DestructiveReverse(res);};";
        scriptMap.put("MakeVector",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"<--\",[_leftOperand,_rightOperand]);RuleHoldArguments(\"<--\",2,1,Equal?(Type(leftOperand),\"#\")){ DefinePattern(leftOperand[2],rightOperand,leftOperand[1],True);};RuleHoldArguments(\"<--\",2,2,Function?(leftOperand)){ DefinePattern(leftOperand,rightOperand,0,True);};HoldArgument(\"<--\",\"leftOperand\");HoldArgument(\"<--\",\"rightOperand\");";
        scriptMap.put("<--",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function (\"Boolean?\", [_x]) (x=?True) Or? (x=?False) Or? Function?(x) And? Contains?([\"=?\", \">?\", \"<?\", \">=?\", \"<=?\", \"!=?\", \"And?\", \"Not?\", \"Or?\", \"Implies?\", \"Equivales?\"], Type(x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasExpressionArithmetic?(expr, atom) := HasExpressionSome?(expr, atom, [ToAtom(\"+\"), ToAtom(\"-\"), ToAtom(\"*\"), ToAtom(\"/\")]);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"ZeroVector?\",[_aList]) aList =? ZeroVector(Length(aList));";
        scriptMap.put("ZeroVector?",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(p,x){Function(\"Scalar?\",[_x]) Not?(List?(x));Function(\"Vector?\",[_x]) Decide(List?(x), Length(Select(x, \"List?\"))=?0, False);Function(\"Vector?\",[_p,_x]){ Decide(List?(x), { Local(i,n,result); n:=Length(x); i:=1; result:=True; While(i<=?n And? result) { result:=Apply(p,[x[i]]); i++; }; result; }, False);};Function(\"Matrix?\",[_x])Decide(List?(x) And? Length(x)>?0,{ Local(n); n:=Length(x); Decide(Length(Select(x, \"Vector?\"))=?n, MapSingle(\"Length\",x)=?Length(x[1])+ZeroVector(n), False);},False);Function(\"Matrix?\",[_p,_x])Decide(Matrix?(x),{ Local(i,j,m,n,result); m:=Length(x); n:=Length(x[1]); i:=1; result:=True; While(i<=?m And? result) { j:=1; While(j<=?n And? result) { result:=Apply(p,[x[i][j]]); j++; }; i++; }; result;},False);Function(\"SquareMatrix?\",[_x]) Matrix?(x) And? Length(x)=?Length(x[1]);Function(\"SquareMatrix?\",[_p,_x]) Matrix?(p,x) And? Length(x)=?Length(x[1]);}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(count, iterations, body){RulebaseHoldArguments(\"Repeat\",[_iterations,_body]);RuleHoldArguments(\"Repeat\",2,10,Integer?(iterations) And? iterations >? 0){ Local(count); count := 0; While (iterations >? 0) { Eval(body); iterations--; count++; }; count;};RulebaseHoldArguments(\"Repeat\",[_body]);RuleHoldArguments(\"Repeat\",1,20,True){ Local(count); count := 0; While (True) { Eval(body); count++; }; count;};};UnFence(\"Repeat\",2);HoldArgumentNumber(\"Repeat\",2,2);UnFence(\"Repeat\",1);HoldArgumentNumber(\"Repeat\",1,1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BubbleSort\",[_list,_compare]){ Local(i,j,length,left,right);     list:=FlatCopy(list); length:=Length(list);  For (j:=length,j>?1,j--) { For(i:=1,i<?j,i++) { left:=list[i]; right:=list[i+1];   Decide(Not?(Apply(compare,[left,right])), { DestructiveInsert(DestructiveDelete(list,i),i+1,left); } );  }; }; list;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Exp(x_Number?)::NumericMode?() <-- ExpNum(x);4 # Exp(Ln(_x)) <-- x;200 # Exp(0) <-- 1;200 # Exp(-Infinity) <-- 0;200 # Exp(Infinity) <-- Infinity;200 # Exp(Undefined) <-- Undefined;Exp(xlist_List?) <-- MapSingle(\"Exp\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Object\",[_pred,_x]);RuleHoldArguments(\"Object\",2,0,Apply(pred,[x])=?True) x;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Rem(n_Number?,m_Number?) <-- n-m*Quotient(n,m);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "   Function(\"MacroSubstituteApply\", [_body, _predicate, _change]){ `MacroSubstituteApply((Hold(@body)));};HoldArgument(\"MacroSubstituteApply\", \"predicate\");HoldArgument(\"MacroSubstituteApply\", \"change\");UnFence(\"MacroSubstituteApply\", 3);RulebaseHoldArguments(\"MacroSubstituteApply\", [_body]);UnFence(\"MacroSubstituteApply\", 1);RuleHoldArguments(\"MacroSubstituteApply\", 1, 1, `ApplyFast(predicate, [Hold(Hold(@body))]) =? True){ `ApplyFast(change, [Hold(Hold(@body))]);};RuleHoldArguments(\"MacroSubstituteApply\", 1, 2, `Function?(Hold(@body))){ `ApplyFast(\"MacroMapArgs\", [Hold(Hold(@body)), \"MacroSubstituteApply\"]);};RuleHoldArguments(\"MacroSubstituteApply\", 1, 3, True){ `Hold(@body);};";
        scriptMap.put("MacroSubstituteApply",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"SubstituteApply\", [_body, _predicate, _change]){ SubstituteApply(body);};HoldArgument(\"SubstituteApply\", \"predicate\");HoldArgument(\"SubstituteApply\", \"change\");UnFence(\"SubstituteApply\", 3);RulebaseHoldArguments(\"SubstituteApply\", [_body]);UnFence(\"SubstituteApply\", 1);RuleHoldArguments(\"SubstituteApply\", 1, 1, Apply(predicate, [body]) =? True){ Apply(change,[body]);};RuleHoldArguments(\"SubstituteApply\", 1, 2, Function?(body)){ Apply(\"MapArgs\",[body,\"SubstituteApply\"]);};RuleHoldArguments(\"SubstituteApply\", 1, 3, True) body;";
        scriptMap.put("SubstituteApply",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Add(_val, ...);10 # Add([]) <-- 0;20 # Add(values_List?) <--{ Local(i, sum); sum:=0; ForEach(i, values) { sum := sum + i; }; sum;};30 # Add(_value) <-- value;";
        scriptMap.put("Add",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Maximum(_l1, _l2, _l3, ...);10 # Maximum(_l1, _l2, l3_List?) <-- Maximum(Concat([l1, l2], l3));20 # Maximum(_l1, _l2, _l3) <-- Maximum([l1, l2, l3]);10 # Maximum(l1_List?,l2_List?) <-- Map(\"Maximum\",[l1,l2]);20 # Maximum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- Decide(l1>?l2,l1,l2);30 # Maximum(l1_Constant?,l2_Constant?) <-- Decide(NM(Eval(l1-l2))>?0,l1,l2);10 # Maximum([]) <-- Undefined;20 # Maximum(list_List?) <--{ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Maximum(result,item); result;};30 # Maximum(_x) <-- x;";
        scriptMap.put("Maximum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Minimum(_l1, _l2, _l3, ...);10 # Minimum(_l1, _l2, l3_List?) <-- Minimum(Concat([l1, l2], l3));20 # Minimum(_l1, _l2, _l3) <-- Minimum([l1, l2, l3]);10 # Minimum(l1_List?,l2_List?) <-- Map(\"Minimum\",[l1,l2]);20 # Minimum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- Decide(l1<?l2,l1,l2);30 # Minimum(l1_Constant?,l2_Constant?) <-- Decide(NM(Eval(l1-l2))<?0,l1,l2);10 # Minimum([]) <-- Undefined;20 # Minimum(list_List?) <--{ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Minimum(result,item); result;};30 # Minimum(_x) <-- x;";
        scriptMap.put("Minimum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Product\",[_sumvar,_sumfrom,_sumto,_sumbody]){ Local(sumi,sumsum); sumsum:=1; For(sumi:=sumfrom,sumi<=?sumto And? sumsum!=?0,sumi++) { MacroLocal(sumvar); MacroAssign(sumvar,sumi); sumsum:=sumsum*Eval(sumbody); }; sumsum;};UnFence(\"Product\",4);HoldArgument(\"Product\",\"sumvar\");HoldArgument(\"Product\",\"sumbody\");Product(sumlist_List?) <--{ Local(sumi,sumsum); sumsum:=1; ForEach(sumi,sumlist) { sumsum:=sumsum*sumi; }; sumsum;};";
        scriptMap.put("Product",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Subfactorial\",[_n]){ n! * Sum(k,0,n,(-1)^(k)/k!);};30 # Subfactorial(n_List?) <-- MapSingle(\"Subfactorial\",n);";
        scriptMap.put("Subfactorial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"Sum\",[_sumvararg,_sumfromarg,_sumtoarg,_sumbodyarg]);10 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody)::(sumfrom>?sumto) <-- 0;20 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody)::(sumto<?sumfrom) <-- ApplyFast(\"Sum\",[sumvar,sumto,sumfrom,sumbody]);30 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody) <--LocalSymbols(sumi,sumsum){ Local(sumi,sumsum); sumsum:=0; For(sumi:=sumfrom,sumi<=?sumto,sumi++) { MacroLocal(sumvar); MacroAssign(sumvar,sumi); sumsum:=sumsum+Eval(sumbody); }; sumsum;};UnFence(\"Sum\",4);HoldArgument(\"Sum\",\"sumvararg\");HoldArgument(\"Sum\",\"sumbodyarg\");40 # Sum([]) <-- 0;50 # Sum(values_List?) <--{ Local(i, sum); sum:=0; ForEach(i, values) { sum := sum + i; }; sum;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# (n_PositiveInteger?)!! :: (n <=? 3) <-- n;2# (n_PositiveInteger?)!! <--{ Check(n<=?65535, \"Argument\", \"Double factorial: Error: the argument \" ~ ( PipeToString() Write(n) ) ~ \" is too large, you may want to avoid exact calculation\"); Factorialdouble(2+Modulo(n, 2), n);};3# (_n)!! :: (n =? -1 Or? n =? 0) <-- 1;2# Factorialdouble(_a, _b)::(b-a >=? 6) <-- Factorialdouble(a, Quotient(a+b,2)) * Factorialdouble(Quotient(a+b,2)+1+Modulo(Quotient(a+b,2)+1-a, 2), b);3# Factorialdouble(_a, _b)::(b-a >=? 4) <-- a*(a+2)*(a+4);4# Factorialdouble(_a, _b)::(b-a >=? 2) <-- a*(a+2);5# Factorialdouble(_a, _b) <-- a;30 # (n_List?)!! <-- MapSingle(\"!!\",n);";
        scriptMap.put("!!",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BenchCall\",[_expr]){ Echo(\"In&gt \",expr); WriteString(\"<font color=ff0000>\"); Eval(expr); WriteString(\"</font>\"); True;};HoldArgument(\"BenchCall\",\"expr\");";
        scriptMap.put("BenchCall",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BenchShow\",[_expr]){ Echo(\"In&gt \",expr); WriteString(\"<font color=ff0000> \"); Echo(\"Out&gt \",Eval(expr),\"</font>\"); True;};HoldArgument(\"BenchShow\",\"expr\");";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"KnownFailure\",[_expr]){ Local(rfail); Echo(\"Known failure: \", expr); Assign(rfail,Eval(expr)); Decide(rfail,Echo([\"Failure resolved!\"]));};HoldArgument(\"KnownFailure\",\"expr\");";
        scriptMap.put("KnownFailure",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(TrueFalse){ RulebaseHoldArguments(\"TrueFalse\",[_var,_expr]); 10 # TrueFalse(var_Atom?,_expr) <-- `[(@expr) Where (@var)==False,(@expr) Where (@var)==True]; 20 # TrueFalse([],_expr) <-- `(@expr); 30 # TrueFalse(var_List?,_expr) <-- `{  Local(t,h); Assign(h,First(@var)); Assign(t,Rest(@var)); TrueFalse(h,TrueFalse(t,@expr)); };  HoldArgument(\"TrueFalse\",\"var\"); HoldArgument(\"TrueFalse\",\"expr\"); Macro(\"LogicTest\",[_vars,_expr1,_expr2]) Verify(TrueFalse((@vars),(@expr1)), TrueFalse((@vars),(@expr2)));};";
        scriptMap.put("LogicTest",scriptString);
        scriptMap.put("TrueFalse",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"LogicVerify\",[_aLeft,_aRight]){ Decide(aLeft !=? aRight, Verify(CanProve(aLeft Implies? aRight),True) );};";
        scriptMap.put("LogicVerify",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"NextTest\",[_aLeft]){ WriteString(\"Test suite for \"~aLeft~\" : \"); NewLine();};";
