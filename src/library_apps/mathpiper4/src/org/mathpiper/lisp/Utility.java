...
public class Utility {
    ...
    static double log2_table[] = { 0., 1., 1.5849625007211561814537389439478165087598, 2.,
	    2.3219280948873623478703194294893901758648, 2.5849625007211561814537389439478165087598,
	    2.807354922057604107441969317231830808641, 3., 3.1699250014423123629074778878956330175196,
	    3.3219280948873623478703194294893901758648, 3.4594316186372972561993630467257929587032,
	    3.5849625007211561814537389439478165087598, 3.7004397181410921603968126542566947336284,
	    3.807354922057604107441969317231830808641, 3.9068905956085185293240583734372066846246, 4.,
	    4.0874628412503394082540660108104043540112, 4.1699250014423123629074778878956330175196,
	    4.2479275134435854937935194229068344226935, 4.3219280948873623478703194294893901758648,
	    4.3923174227787602888957082611796473174008, 4.4594316186372972561993630467257929587032,
	    4.5235619560570128722941482441626688444988, 4.5849625007211561814537389439478165087598,
	    4.6438561897747246957406388589787803517296, 4.7004397181410921603968126542566947336284,
	    4.7548875021634685443612168318434495262794, 4.807354922057604107441969317231830808641,
	...
	if (!(oper2.car() instanceof Cons))
	    LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT, args2);
	if (((Cons) oper2.car()) == null)
	    LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT, args2);
    ...
    public static Map optionsListToJavaMap(Environment aEnvironment, int aStackTop, Cons arguments, Map defaultOptions)
	    throws Throwable {
	...
	if (isTrue(aEnvironment, aExpression, aStackTop)) {
	    ...
	    if (isTrue(aEnvironment, aExpression, aStackTop))
		LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT,
			BuiltinFunction.getArgument(aEnvironment, aStackTop, 1));
	    return getTrueAtom(aEnvironment);
	}
    }
	...
	if (aOriginal == null)
	    LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT, "Empty argument.");
	if (aOriginal.charAt(0) != '\"')
	    LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT, "Missing left quote character.");
	int nrc = aOriginal.length() - 1;
	if (aOriginal.charAt(nrc) != '\"')
	    LispError.throwError(aEnvironment, aStackTop, LispError.INVALID_ARGUMENT, "Missing right quote character.");
	...
	try {
	    ...
	    MathPiperParser parser = new MathPiperParser(new MathPiperTokenizer(), aEnvironment.getCurrentInput(), aEnvironment,
		    aEnvironment.iPrefixOperators, aEnvironment.iInfixOperators, aEnvironment.iPostfixOperators,
    ...
    public static void doPatchString(String unpatchedString, MathPiperOutputStream aOutput, Environment aEnvironment, int aStackTop)
	    throws Throwable {
	...
	if (tags.length > 1) {
		...
		if (tag.length > 1) {
		    ...
		    StringInputStream scriptStream = new StringInputStream(scriptCode, aEnvironment.getCurrentInput().iStatus);
    ...
    public static String printMathPiperExpression(int aStackTop, Cons aExpression, Environment aEnvironment, int aMaxChars)
	    throws Throwable {
	if (aExpression == null) {
	    return "NULL";
	}
    ...
    public static String printLispExpression(int aStackTop, Cons aExpression, Environment aEnvironment, int aMaxChars)
	    throws Throwable {
	if (aExpression == null) {
	    return "NULL";
	}
    ...
    /**
     * Convert the number of bits in a given base to the number of digits. To
     * make sure there is no hysteresis, the returned value is rounded down.
     * 
     * @param bits
     * @param base
	...
	BigNumber x = (BigNumber) BuiltinFunction.getArgument(aEnvironment, aStackTop, aArgNr).getNumber(aEnvironment.iPrecision,
		aEnvironment);
	if (x == null)
	    LispError.checkArgument(aEnvironment, aStackTop, aArgNr);
	return x;
    }
	...
	if (!orig.startsWith("\""))
	    LispError.checkArgumentTypeWithError(aEnvironment, aStackTop, 1, "The argument <" + orig + "> must be a string.");

	Cons precedence = aEnvironment.iLispExpressionEvaluator.evaluate(aEnvironment, aStackTop,
		BuiltinFunction.getArgument(aEnvironment, aStackTop, 2));
	if (!(precedence.car() instanceof String))
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	int prec = Integer.parseInt((String) precedence.car(), 10);
	if (prec > MathPiperUnparser.KMaxPrecedence)
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	...
	if (variableString == null)
	    LispError.checkArgument(aEnvironment, aStackTop, 1);
	if (Utility.isNumber(variableString, true))
	    LispError.checkArgument(aEnvironment, aStackTop, 1);
	...
	if (index == null)
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	if (!(index.car() instanceof String))
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	...
	if (consTraverser == null)
	    LispError.throwError(aEnvironment, aStackTop, LispError.NOT_LONG_ENOUGH);
	...
	if (!(evaluated.car() instanceof Cons))
	    LispError.checkArgument(aEnvironment, aStackTop, 1);
     ...
     * work is done by Environment.defineRulebase().
     */
	...
	if (BuiltinFunction.getArgument(aEnvironment, aStackTop, 1) == null)
	    LispError.checkArgument(aEnvironment, aStackTop, 1);
	Cons argument = BuiltinFunction.getArgument(aEnvironment, aStackTop, 1);
	...
	aEnvironment.defineRulebase(aStackTop, Utility.getSymbolName(aEnvironment, functionName), ((Cons) args.car()).cdr(),
		aListed);
	...
	if (arityCons == null)
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	if (!(arityCons.car() instanceof String))
	    LispError.checkArgument(aEnvironment, aStackTop, 2);
	arity = Integer.parseInt((String) arityCons.car(), 10);

	// The precedence
	if (precedenceCons == null)
	    LispError.checkArgument(aEnvironment, aStackTop, 3);
	if (!(precedenceCons.car() instanceof String))
	    LispError.checkArgument(aEnvironment, aStackTop, 3);
	...
	if (aPattern == true) {
	    aEnvironment
		    .definePatternRule(aStackTop, Utility.getSymbolName(aEnvironment, orig), arity, precedence, predicate, body);
	} else {
	    aEnvironment.defineRule(aStackTop, Utility.getSymbolName(aEnvironment, orig), arity, precedence, predicate, body);
	}
    ...
    public static Cons associationListGet(Environment aEnvironment, int aStackTop, Cons key, Cons listCons) throws Throwable {
    ...
    public static void declareFunction(String functionName, String[] parameters, String body, Environment aEnvironment,
