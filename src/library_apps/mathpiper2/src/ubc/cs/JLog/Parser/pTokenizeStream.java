 ...
 * pre tokenized stream (partial tokenizing parse), tokenizer tables for each
 * type of pre token, and a pushback stack (for look aheads).
 * 
...
class pTokenizeStream {
    ...
    protected pTokenizerTable table, string_table, linecomment_table,
	    comment_table;
	...
	do {
		...
		case pPreTokenizeStream.TOKEN_EOF:
		    return new pEOF(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_WORDS:
		...
		case TOKEN_STARTPAREN:
		    return new pStartParen(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_ENDPAREN:
		    return new pEndParen(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_STARTBRACE:
		    tokenizer.pushBackToken(pt);
		    return parseBrace();
		case TOKEN_ENDBRACE:
		    return new pEndBrace(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_STARTBRACKET:
		    return new pStartBracket(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_ENDBRACKET:
		    return new pEndBracket(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_LISTCONS:
		    return new pListCons(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_COMMA:
		    return new pComma(pt.getPosition(), pt.getLine(),
			    pt.getCharPos());
		case TOKEN_PERIOD:
		    tokenizer.pushBackToken(pt);
		    return parsePeriod();
		case TOKEN_SEMICOLON: {
		    ...
		    return new pName(";", predicate, pt.getPosition(),
	...
	if (pt1.getType() != TOKEN_STARTBRACE)
	    throw new TokenizeStreamException(
		    "Error in Tokenizer.  Unexpected token at ",
	...
	if (pt2.getType() == TOKEN_ENDBRACE) {
	    ...
	    return new pName("{}", predicate, pt1.getPosition(), pt1.getLine(),
		    pt1.getCharPos());
	}

	tokenizer.pushBackToken(pt2);
	return new pStartBrace(pt1.getPosition(), pt1.getLine(),
		pt1.getCharPos());
    };
	...
	if (pt2.getType() == TOKEN_NUMBERS) {
	    ...
	    return parseNumbers();
	} else if (pt2.getType() == TOKEN_PERIOD
		|| pt2.getType() == TOKEN_SECONDARYWORDS) {
	    tokenizer.pushBackToken(pt2);
	    tokenizer.pushBackToken(pt1);
	    return parseSecondaryWords();
	}
	...
	if (pt2.getType() == pPreTokenizeStream.TOKEN_EOF)
	    throw new SyntaxErrorException(
		    "Premature end of file.  Comment without end at ",
		    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());
	...
	do {
		...
		|| pt2.getType() == TOKEN_WHITESPACE);

	if (pt2.getType() == pTokenizerTable.TOKEN_EOL)
	    throw new SyntaxErrorException(
		    "Premature end of line.  String without end at ",
		    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());

	if (pt2.getType() == pPreTokenizeStream.TOKEN_EOF)
	    throw new SyntaxErrorException(
		    "Premature end of file.  String without end at ",
		    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());
...

	{
	    ...
	    return new pName(sb.toString(), predicate, pt1.getPosition(),
		    pt1.getLine(), pt1.getCharPos());
	}
    };
	...
	if (pt2.getType() == pTokenizerTable.TOKEN_EOL)
	    throw new SyntaxErrorException(
		    "Premature end of line.  Character array without end at ",
		    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());

	if (pt2.getType() == pPreTokenizeStream.TOKEN_EOF)
	    throw new SyntaxErrorException(
		    "Premature end of file.  Character array without end at ",
		    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());

	tokenizer.popTokenizerTable();
	return new pArray(sb.toString(), pt1.getPosition(), pt1.getLine(),
		pt1.getCharPos());
    };
	...
	if ((sb.charAt(0) >= 'A' && sb.charAt(0) <= 'Z') || sb.charAt(0) == '_')
	    return new pVariable(sb.toString(), pt1.getPosition(),
		    pt1.getLine(), pt1.getCharPos());
	else
	    return new pName(sb.toString(), predicate, pt1.getPosition(),
		    pt1.getLine(), pt1.getCharPos());
	...
	if (pt1.getToken().equals("-") || pt1.getToken().equals("+")) {
		...
		sgn = new pName(sb.toString(), false, pt1.getPosition(),
			pt1.getLine(), pt1.getCharPos());
		unum = parseNumbers();
	...
	if (pt1.getType() == TOKEN_PERIOD) // case: .### or .###e[+,-]###
	{
	    ...
	    pt3 = read();
	    if (pt3.getType() == TOKEN_WORDS
		    && pt3.getToken().equalsIgnoreCase("e")) {
		pTSPreToken pt4;

		pt4 = read();
		if (pt4.getType() == TOKEN_SECONDARYWORDS
			&& (pt4.getToken().equals("-") || pt4.getToken()
				.equals("+"))) {
		    pTSPreToken pt5;
			...
			throw new SyntaxErrorException(
				"Expected number after sign at ",
				pt4.getPosition(), pt4.getLine(),
				pt4.getCharPos());
		    ...
		    throw new SyntaxErrorException(
			    "Expected '+','-', or number after 'e' at ",
			    pt3.getPosition(), pt3.getLine(), pt3.getCharPos());
	    } else
		tokenizer.pushBackToken(pt3);

	    return new pReal(sb.toString(), pt1.getPosition(), pt1.getLine(),
		    pt1.getCharPos());
	} else // case: ### or ###e[+,-]### or ###.### or ###.###e[+,-]### or
	       // ###'nnn
	{
	    pt2 = read();
	    if (pt2.getType() == TOKEN_PERIOD) // case: ###.### or
					       // ###.###e[+,-]###
	    {
		pTSPreToken pt3;

		pt3 = read();
		if (pt3.getType() == TOKEN_NUMBERS) // case: ###.### or
						    // ###.###e[+,-]###
		{
		    pTSPreToken pt4;
		    ...
		    pt4 = read();
		    if (pt4.getType() == TOKEN_WORDS
			    && pt4.getToken().equalsIgnoreCase("e")) {
			pTSPreToken pt5;

			pt5 = read();
			if (pt5.getType() == TOKEN_SECONDARYWORDS
				&& (pt5.getToken().equals("-") || pt5
					.getToken().equals("+"))) {
			    pTSPreToken pt6;
			    ...
			    throw new SyntaxErrorException(
				    "Expected '+', '-', or number after 'e' at ",
				    pt4.getPosition(), pt4.getLine(), pt4
					    .getCharPos());
		    } else
			tokenizer.pushBackToken(pt4);

		    return new pReal(sb.toString(), pt1.getPosition(),
			    pt1.getLine(), pt1.getCharPos());
		} else {
		    tokenizer.pushBackToken(pt3);
		    tokenizer.pushBackToken(pt2);
		    return new pInteger(sb.toString(), pt1.getPosition(),
			    pt1.getLine(), pt1.getCharPos());
		}
		...
		try {
		    ...
		    throw new TokenizeStreamException(
			    "Error in tokenizer.  Expected integer at ",
			    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());
		}
		...
		if (basenum == 0) {
		    ...
		    tokenizer.popTokenizerTable();
		    return new pInteger(base, pt2.getToken(),
			    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());
		    ...
		    throw new SyntaxErrorException(
			    "Number base must be 0 or 2..36 at",
			    pt1.getPosition(), pt1.getLine(), pt1.getCharPos());
	    } else if (pt2.getType() == TOKEN_WORDS
		    && pt2.getToken().equalsIgnoreCase("e")) {
		pTSPreToken pt3; // case: ###e[+,-]###

		pt3 = read();
		if (pt3.getType() == TOKEN_SECONDARYWORDS
			&& (pt3.getToken().equals("-") || pt3.getToken()
				.equals("+"))) {
		    pTSPreToken pt4;
	...
	if (!(pt instanceof pTSPreToken)) {
	    tspt = new pTSPreToken(pt.getToken(), pt.getType(),
		    pt.getPosition(), currentLine, currentChar);
