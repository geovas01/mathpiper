...
/**
 * This is the abstract Application class for the Prolog in Java environment. It
 ...
 * the <code>jPrologServices</code>. Derivative authors should inherit from this
 * class to preserve the <code>getApplicationInfo()</code> credits. This class
 * also supports the use of a single menu bar for all windows of the
 * application. This is not straightforward because Java uses a per window menu
 * bar by default. This class supports the construction of duplicate menu bars,
 * so that each window can have the same menu bar (by appearance, but it is a
 * copy), and also supports updating some common menu items so that they appear
 * the same (e.g., same hilight, same check state) for all menu bar copies.
 * 
 * @author Glendon Holst
 * @version %I%, %G%
 */
abstract public class gJLogApplicationBase extends Frame implements
	iJLogApplBaseServices, iMenuServiceRequester {
     ...
     * string. It must be preserved by authors of derivative works. All
     * applications should be derived from the <code>gJLogApplicationBase</code>
     * class, and if they provide their own <code>getApplicationInfo()</code>
     * function it should invoke this version first. For example:
     * <code>return super.getApplicationInfo() + "derivative information";</code>
     * 
     ...
     * <code>iMenuServiceRequester</code>. The menu bar will have the same
     * layout and items, so that all windows have the same menus, but those
     * menus items not handled by the application will be passed to the
     * iMenuServiceRequester so that it can register a listener and the
     * appropriate action.
     * 
     * @param s
     *            The <code>iMenuServiceRequester</code> that this menu bar is
     *            for.
     ...
     * 
     * @param menubar
     *            The <code>MenuBar</code> to update.
     */
    ...
    /**
     * This function updates the debug portion of the tool menu for all
     * registered windows.
     * 
     * @param dbg
     *            true is the debugging is on.
     * @param unkpred
     *            true if prolog throws an error on unknown predicates. false if
     *            if unknown predicates are false.
     * @param trce
     *            true is trace all goals.
     * @param brk
     *            true if break after each step.
     * @param in_query
     *            true if a query is presently underway.
     */
	...
	while (e.hasMoreElements()) {
	    updateToolMenuDebug(
		    ((gConsultWindow) e.nextElement()).getMenuBar(), dbg,
		    unkpred, trce, brk, in_query);
	}
    };
	...
	while (e.hasMoreElements()) {
	    updateToolMenuQuery(
		    ((gConsultWindow) e.nextElement()).getMenuBar(), in_query,
		    paused);
	}
    };
	...
	while (e.hasMoreElements()) {
	    updateToolMenuConsultAll(
		    ((gConsultWindow) e.nextElement()).getMenuBar(), state);
	}
    };
