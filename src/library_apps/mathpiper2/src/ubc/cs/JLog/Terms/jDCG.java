...
public class jDCG extends jBinaryBuiltinPredicate {
    ...
    // create a DCG for use with makeDCGTerm only. makeDCGRule requires a lhs
    // for the rule head.
    public jDCG(jTerm r) {
    ...
    // this is called during parse operations to create the DCG version of the
    // prolog rules
	...
	head = makeDCGHead(firstv = new jVariable("S0"), lastv = new jVariable(
		"S"));
	if (rhs != null)
    ...
    // used by expand_term. creates jIf if head is not null, otherwise returns
    // the DCG body.
	...
	updateDCGPredicateRegistry(predicate_registry, S0, SN,
		new dcgVariableGenerator());
	return t;
    };
    ...
    protected void makeDCGBaseCompoundTerm(jTerm r, jCompoundTerm terms,
	    Vector pr) {
	jTerm rt = r.getTerm();
	...
	if (rt instanceof jNullList) {
...

		{
		    ...
		    pr.addElement(new dcgNotFork(lv,
			    new dcgUnifyPredicateUpdate(un)));
		}
		...
		terms.addTerm(p = new jPredicate(pt.getName(),
			makeCompoundTerm(pt.getArguments())));
	    ...
	    terms.addTerm(p = new jPredicate(ip.getName(), makeCompoundTerm(ip
		    .getArguments())));
    ...
    // S0 and SN should be the first and last variables/terms used for the base,
    // respectivly.
	...
	if (lt instanceof jPredicate) {
	    ...
	    if ((c.getLHS().getTerm() instanceof jPredicate)
		    && (c.getRHS().getTerm() instanceof iList)) {
		h = (jPredicate) c.getLHS().getTerm();
		l = (c.getRHS().getTerm() instanceof jListPair ? (jListPair) c
			.getRHS().getTerm() : null);
	    } else
		throw new InvalidDCGFormException(
			"Expected predicate,list on LHS.", this);
	} else
	    throw new InvalidDCGFormException(
		    "Expected predicate or predicate,list on LHS.", this);
    ...
    // this should not adjust the existing list src, instead it should create a
    // copy that uses
    ...
    // this should not adjust the existing jPredicate src, instead it should
    // create a copy using
