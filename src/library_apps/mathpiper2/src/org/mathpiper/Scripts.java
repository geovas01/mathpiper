...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(a,b)[10 # MatchLinear(var_Atom?,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchLinear(expr);];20 # MatchLinear(_var,_expr) <-- False;10 # MatchLinear(_expr)_(Degree(expr,var)<?2) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,1); b := Coef(expr,0); True;];20 # MatchLinear(_expr) <-- False;UnFence(\"MatchLinear\",1);UnFence(\"MatchLinear\",2);10 # MatchPureSquared(var_Atom?,_sign2,_sign0,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchPureSquared(expr,sign2,sign0);];20 # MatchPureSquared(_var,_sign2,_sign0,_expr) <-- False;10 # MatchPureSquared(_expr,_sign2,_sign0)_(Degree(expr,var)=?2 And? Coef(expr,1) =? 0 And? Number?(Coef(expr,0)) And? Number?(Coef(expr,2)) And? Coef(expr,0)*sign0 >? 0 And? Coef(expr,2)*sign2 >? 0 ) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,2); b := Coef(expr,0); True;];20 # MatchPureSquared(_expr,_sign2,_sign0) <-- False;UnFence(\"MatchPureSquared\",3);UnFence(\"MatchPureSquared\",4);Matched'a() := a;Matched'b() := b;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "sign'change(x,y,z) :=If( Number?(x) And? Number?(y) And? Number?(z) And? Not? ( x>?y And? y<?z Or?   x<?y And? y>?z ), 0, 1); ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NumericList?(_arg'list) <-- List?(arg'list) And? (\"And?\" @ (MapSingle(Hold({{x},Number?(N(Eval(x)))}), arg'list)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # GeoGebraPoint(name_IsString, x_Number?, y_Number?) <--[ Local(command);  command := PatchString(\"<?Write(ToAtom(name));?>=(<?Write(x);?>,<?Write(y);?>)\");  JavaCall(geogebra,\"evalCommand\",command);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LambertW(0) <-- 0;10 # LambertW(Infinity) <-- Infinity;10 # LambertW(Undefined) <-- Undefined;10 # LambertW(-Infinity) <-- Infinity + I*Pi;10 # LambertW(-Exp(-1)) <-- -1;20 # LambertW(_x * Ln(_x)) <-- Ln(x);20 # LambertW(Ln(_x) * _x) <-- Ln(x);30 # LambertW(x_Constant?) _ InNumericMode() <-- Internal'LambertWNum(Eval(x));10 # Internal'LambertWNum(x_Number?)_(x <? -ExpN(-1)) <-- Undefined;20 # Internal'LambertWNum(x_Number?) <--[ Local(W); NewtonNum( `Hold( { {W}, [ Local(a); a:=W- @x*ExpN(-W); W-a/(W+1-(W+2)/(W+1)*a/2.); ]}),  If( x<?0, x*ExpN(1) / (1+1 / (1 / SqrtN(2*(x*ExpN(1)+1)) - 1 / SqrtN(2) + 1/(ExpN(1)-1))), Internal'LnNum(1+x)*(1-Internal'LnNum(1+Internal'LnNum(1+x))/(2+Internal'LnNum(1+x))) ), 10,  3  );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Ln(0) <-- -Infinity;2 # Ln(1) <-- 0;2 # Ln(Infinity) <-- Infinity;2 # Ln(Undefined) <-- Undefined;2 # Ln(x_NegativeNumber?)_InNumericMode() <-- Complex(Ln(-x), Pi);3 # Ln(x_Number?)_(InNumericMode() And? x>=?1) <-- Internal'LnNum(x);4 # Ln(Exp(_x)) <-- x;3 # Ln(Complex(_r,_i)) <-- Complex(Ln(Abs(Complex(r,i))), Arg(Complex(r,i)));4 # Ln(x_NegativeNumber?) <-- Complex(Ln(-x), Pi);5 # Ln(x_Number?)_(InNumericMode() And? x<?1) <-- - Internal'LnNum(DivideN(1, x));Ln(xlist_List?) <-- MapSingle(\"Ln\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Tanh(_x)_(InNumericMode() And? (Number?(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Sinh(x)/Cosh(x) ));100 # Tanh(_x)*Cosh(_x) <-- Sinh(x);200 # Tanh(0) <-- 0;200 # Tanh(Infinity) <-- 1;200 # Tanh(-Infinity) <-- -1;200 # Tanh(ArcTanh(_x)) <-- x;200 # Tanh(ArcSinh(_x)) <-- x/Sqrt(1+x^2);200 # Tanh(ArcCosh(_x)) <-- Sqrt((x-1)/(x+1))*(x+1)/x;200 # Tanh(Undefined) <-- Undefined;Tanh(xlist_List?) <-- MapSingle(\"Tanh\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'LnNum(x_Number?)_(x>=?1) <-- NewtonLn(x);Internal'LnNum(x_Number?)_(0<?x And? x<?1) <-- - Internal'LnNum(DivideN(1,x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Sum\",{sumvar'arg,sumfrom'arg,sumto'arg,sumbody'arg});10 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody)_(sumfrom>?sumto) <-- 0;20 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody)_(sumto<?sumfrom) <-- ApplyFast(\"Sum\",{sumvar,sumto,sumfrom,sumbody});30 # Sum(_sumvar,sumfrom_Number?,sumto_Number?,_sumbody) <--LocalSymbols(sumi,sumsum)[ Local(sumi,sumsum); sumsum:=0; For(sumi:=sumfrom,sumi<=?sumto,sumi++) [ MacroLocal(sumvar); MacroBind(sumvar,sumi); sumsum:=sumsum+Eval(sumbody); ]; sumsum;];UnFence(\"Sum\",4);HoldArgument(\"Sum\",sumvar'arg);HoldArgument(\"Sum\",sumbody'arg);40 # Sum({}) <-- 0;50 # Sum(values_List?) <--[ Local(i, sum); sum:=0; ForEach(i, values) [ sum := sum + i; ]; sum;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # 0! <-- 1;10 # (Infinity)! <-- Infinity;20 # ((n_PositiveInteger?)!) <-- [ Check(n <=? 65535, \"Argument\", \"Factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); MathFac(n);];25 # ((x_Constant?)!)_(FloatIsInt?(x) And? x>?0) <-- (Round(x)!);30 # ((x_Number?)!)_InNumericMode() <-- Internal'GammaNum(x+1);40 # (n_List?)! <-- MapSingle(\"!\",n);HalfIntegerFactorial(n_Odd?) _ (n>?0) <-- Sqrt(Pi) * ( n!! / 2^((n+1)/2) );HalfIntegerFactorial(n_Odd?) _ (n<?0) <-- Sqrt(Pi) * ( (-1)^((-n-1)/2)*2^((-n-1)/2) / (-n-2)!! );40 # (n_RationalOrNumber?)! _(Denominator(Rationalize(n))=?2) <-- HalfIntegerFactorial(Numerator(Rationalize(n)));";
        scriptMap.put("!",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TD\",{ind});10 # (TD(_i)X(_j)) <-- Delta(i,j);10 # (TD(_i) ( (_f) + (_g) ) ) <-- (TD(i)f) + (TD(i)g);10 # (TD(_i) ( (_f) - (_g) ) ) <-- (TD(i)f) - (TD(i)g);10 # (TD(_i) ( - (_g) ) ) <-- - TD(i)g;10 # (TD(_i) ( (_f) * (_g) ) ) <-- (TD(i)f)*g + f*(TD(i)g);10 # (TD(_i) ( (_f) ^ (n_PositiveInteger?) ) ) <-- n*(TD(i)f)*f^(n-1);10 # (TD(_i)Delta(_j,_k)) <-- 0;10 # (TD(_i)f_Number?) <-- 0;";
