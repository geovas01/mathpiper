...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # 0 / 0 <-- Undefined;52 # x_PositiveNumber? / 0 <-- Infinity;52 # x_NegativeNumber? / 0 <-- -Infinity;55 # (_x / y_IsNumber)_(Zero?(y)) <-- Undefined;55 # 0 / _x <-- 0;56 # (x_NonZeroInteger? / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger? * _expr) / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*expr)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger?) / (y_NonZeroInteger? * _expr))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/(QuotientN(y,gcd)*expr); ];57 # ((x_NonZeroInteger? * _p) / (y_NonZeroInteger? * _q))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*p)/(QuotientN(y,gcd)*q); ];60 # (x_IsDecimal / y_IsNumber) <-- DivideN(x,y);60 # (x_IsNumber / y_IsDecimal) <-- DivideN(x,y); 60 # (x_IsNumber / y_IsNumber)_(InNumericMode()) <-- DivideN(x,y);90 # x_Infinity? / y_Infinity? <-- Undefined;95 # x_Infinity? / y_IsNumber <-- Sign(y)*x;95 # x_Infinity? / y_IsComplex <-- Infinity;90 # Undefined / _y <-- Undefined;90 # _y / Undefined <-- Undefined;100 # _x / _x <-- 1;100 # _x / 1 <-- x;100 # (_x / y_NegativeNumber?) <-- -x/(-y);100 # (_x / - _y) <-- -x/y;150 # (_x) / (_x) ^ (n_Constant?) <-- x^(1-n);150 # (_x) ^ (n_Constant?) * (_x) <-- x^(n-1);150 # Sqrt(_x) / (_x) ^ (n_Constant?) <-- x^(1/2-n);150 # (_x) ^ (n_Constant?) / Sqrt(_x) <-- x^(n-1/2);150 # (_x) / Sqrt(_x) <-- Sqrt(x);200 # (_x / _y)/ _z <-- x/(y*z);230 # _x / (_y / _z) <-- (x*z)/y;240 # (xlist_IsList / ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"/\",{xlist,ylist});250 # (x_IsList / _y)_(Not?(IsList(y))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,x[i] / y); ]; result;];250 # (_x / y_IsList)_(Not?(IsList(x))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x/y[i]); ]; result;];250 # _x / Infinity <-- 0;250 # _x / (-Infinity) <-- 0;400 # 0 / _x <-- 0;";
        scriptMap.put("/",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # IsComplex(x_RationalOrNumber?) <-- True;2 # IsComplex(Complex(_r,_i)) <-- True;3 # IsComplex(_x) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( RationalFunction?(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- [ Local(r); If( RationalFunction?(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # Monomial?(_expr) <-- False;10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( RationalFunction?(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EigenValues(matrix_UpperTriangular?) <-- Diagonal(matrix);10 # EigenValues(matrix_LowerTriangular?) <-- Diagonal(matrix);20 # EigenValues(matrix_IsMatrix) <-- Roots(CharacteristicEquation(matrix,xx));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GuessRational(_x) <-- GuessRational(x, Floor(1/2*BuiltinPrecisionGet()));GuessRational(x_RationalOrNumber?, prec_IsInteger) <-- [ Local(denom'estimate, cf, i); denom'estimate := 1; cf := ContFracList(x); For(i:=2, i<=?Length(cf) And? denom'estimate <? 10^prec, i++) [  denom'estimate := denom'estimate * If( cf[i] =? 1, If( i+2<=?Length(cf),  RoundTo(N(Eval(cf[i]+1/(cf[i+1]+1/cf[i+2]))), 3),  RoundTo(N(Eval(cf[i]+1/cf[i+1])), 3) ),  cf[i] ); ]; If (denom'estimate <? 10^prec,  i--  ); i--;   ContFracEval(Take(cf, i));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NearRational(_x) <-- NearRational(x, Floor(1/2*BuiltinPrecisionGet()));15 # NearRational(x_RationalOrNumber?, prec_IsInteger) <-- [ Local(x1, x2, i, old'prec); old'prec := BuiltinPrecisionGet(); BuiltinPrecisionSet(prec + 8);  x1 := ContFracList(N(Eval(x+10^(-prec)))); x2 := ContFracList(N(Eval(x-10^(-prec))));     For (i:=1, i<=?Length(x1) And? i<=?Length(x2) And? x1[i]=?x2[i], i++ ) True; If( i>?Length(x1),  x1:=x2, If( i>?Length(x2),  True,   x1[i]:=Minimum(x1[i],x2[i]) ) );    If(i>?Length(x1),i:=Length(x1)); x1[i] := x1[i] + If(i=?Length(x1), 0, 1); BuiltinPrecisionSet(old'prec); ContFracEval(Take(x1, i));];20 # NearRational(_z, prec_IsInteger)_ (And?(Im(z)!=?0,RationalOrNumber?(Im(z)),RationalOrNumber?(Re(z)))) <--[ Local(rr,ii); rr := Re(z); ii := Im(z); Complex( NearRational(rr,prec), NearRational(ii,prec) );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"IsGaussianPrime\",{x})[ if( IsGaussianInteger(x) )[ if( Zero?(Re(x)) )[ ( Abs(Im(x)) % 4 =? 3 And? IsPrime(Abs(Im(x))) ); ] else if ( Zero?(Im(x)) ) [ ( Abs(Re(x)) % 4 =? 3 And? IsPrime(Abs(Re(x))) ); ] else [ IsPrime(Re(x)^2 + Im(x)^2); ]; ] else [ False; ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LowerTriangular?(A_IsMatrix) <-- (UpperTriangular?(Transpose(A)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # One?(x_IsNumber) <-- Zero?(SubtractN(x,1));60000 # One?(_x) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RationalOrNumber?(x_IsNumber) <-- True;10 # RationalOrNumber?(x_IsNumber / y_IsNumber) <-- True;10 # RationalOrNumber?(-(x_IsNumber / y_IsNumber)) <-- True;60000 # RationalOrNumber?(_x) <-- False;";
        scriptMap.put("RationalOrNumber?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Rational?(x_IsInteger) <-- True;10 # Rational?(x_IsInteger / y_IsInteger) <-- True;10 # Rational?(-(x_IsInteger / y_IsInteger)) <-- True;60000 # Rational?(_x) <-- False;";
        scriptMap.put("Rational?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SkewSymmetric?(A_IsMatrix) <-- (Transpose(A)=?(-1*A));";
        scriptMap.put("SkewSymmetric?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SumOfTerms?(_var,expr_IsFreeOf(var)) <-- False;12 # SumOfTerms?(_var,expr_IsAtom()) <-- False;14 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"+\") Or? expr[1]=?ToAtom(\"-\")) <-- True;16 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"*\")) <-- Or?(IsFreeOf(var,expr[2]),IsFreeOf(var,expr[3]));18 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"/\")) <-- IsFreeOf(var,expr[3]);20 # SumOfTerms?(_var,expr_IsList()) <-- False;22 # SumOfTerms?(_var,_expr) <-- SumOfTerms?(var,FunctionToList(expr));";
        scriptMap.put("SumOfTerms?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Symmetric?(A_IsMatrix) <-- (Transpose(A)=?A);";
        scriptMap.put("Symmetric?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Unitary?(A_IsMatrix) <-- (Transpose(Conjugate(A))*A =? Identity(Length(A)));";
        scriptMap.put("Unitary?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "UpperTriangular?(A_IsMatrix) <--[ Local(i,j,m,n,result); m:=Length(A); n:=Length(A[1]); i:=2; result:=(m=?n); While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:= (i<=?j Or? A[i][j] =? 0); j++; ]; i++; ]; result;];";
        scriptMap.put("UpperTriangular?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Variable?(_expr) <-- (IsAtom(expr) And? Not?(expr=?Infinity) And? Not?(expr=? -Infinity) And? Not?(expr=?Undefined) And? Not?(IsNumber(N(Eval(expr)))));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zero?(x_IsNumber) <-- ( MathSign(x) =? 0 Or? AbsN(x) <? PowerN(10, -BuiltinPrecisionGet()));60000 # Zero?(_x) <-- False;20 # Zero?(UniVariate(_var,_first,_coefs)) <-- ZeroVector?(coefs);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardErrorOfTheProportion(meanOfSampleProportions, sampleSize) :=[ Check(RationalOrNumber?(meanOfSampleProportions), \"Argument\", \"The first argument must be a number.\");  Check(IsInteger(sampleSize) And? sampleSize >? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  Sqrt((meanOfSampleProportions*(1 - meanOfSampleProportions))/sampleSize);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # ExpandFrac(expr_IsList) <-- MapSingle(\"ExpandFrac\", expr);10 # ExpandFrac(_expr)_Not?(HasFunctionSome?(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- expr;15 # ExpandFrac(a_RationalOrNumber?) <-- a;20 # ExpandFrac(_expr) <-- ExpandFrac'combine(GetNumerDenom(expr));ExpandFrac'combine({_a, _b}) <-- a/b;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # Gamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_Constant?)_(InNumericMode()) <-- Internal'GammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_Constant?)_(InNumericMode()) <-- Internal'LnGammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # TanMap( _n )_(Not?(RationalOrNumber?(n))) <-- ListToFunction({ToAtom(\"Tan\"),n*Pi});2 # TanMap( _n )_(n<?0) <-- -TanMap(-n);2 # TanMap( _n )_(n>?1) <-- TanMap(Modulo(n,1));4 # TanMap( _n )_(n>?1/2) <-- -TanMap(1-n);5 # TanMap( 0 ) <-- 0;5 # TanMap( 1/6 ) <-- 1/3*Sqrt(3);5 # TanMap( 1/4 ) <-- 1;5 # TanMap( 1/3 ) <-- Sqrt(3);5 # TanMap( 1/2 ) <-- Infinity;10 # TanMap(_n) <-- ListToFunction({ToAtom(\"Tan\"),n*Pi});2 # Tan(x_IsNumber)_InNumericMode() <-- TanNum(x);4 # Tan(ArcTan(_x)) <-- x;4 # Tan(ArcSin(_x)) <-- x/Sqrt(1-x^2);4 # Tan(ArcCos(_x)) <-- Sqrt(1-x^2)/x;5 # Tan(- _x)_(Not? Constant?(x)) <-- -Tan(x);6 # (Tan(x_Constant?))_(NegativeNumber?(N(Eval(x)))) <-- -Tan(-x);6 # Tan(Infinity) <-- Undefined;6 # Tan(Undefined) <-- Undefined;110 # Tan(Complex(_r,_i)) <-- Sin(Complex(r,i))/Cos(Complex(r,i));200 # Tan(v_CanBeUni(Pi))_(Not?(InNumericMode()) And? Degree(v,Pi) <? 2 And? Coef(v,Pi,0) =? 0) <-- TanMap(Coef(v,Pi,1));100 # Tan(_x)/Sin(_x) <-- (1/Cos(x));100 # Tan(_x)*Cos(_x) <-- Sin(x);Tan(xlist_IsList) <-- MapSingle(\"Tan\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Round(Infinity) <-- Infinity;5 # Round(-Infinity) <-- -Infinity;5 # Round(Undefined) <-- Undefined;10 # Round(x_RationalOrNumber?) <-- FloorN(N(x+0.5));10 # Round(x_IsList) <-- MapSingle(\"Round\",x);20 # Round(x_IsComplex) _ (RationalOrNumber?(Re(x)) And? RationalOrNumber?(Im(x)) ) <-- FloorN(N(Re(x)+0.5)) + FloorN(N(Im(x)+0.5))*I;";
        scriptMap.put("Round",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Sign(n_PositiveNumber?) <-- 1;10 # Sign(n_Zero?) <-- 0;20 # Sign(n_IsNumber) <-- -1;15 # Sign(n_Infinity?)_(n <? 0) <-- -1;15 # Sign(n_Infinity?)_(n >? 0) <-- 1;15 # Sign(n_IsNumber/m_IsNumber) <-- Sign(n)*Sign(m);20 # Sign(n_IsList) <-- MapSingle(\"Sign\",n);100 # Sign(_a)^n_Even? <-- 1;100 # Sign(_a)^n_Odd? <-- Sign(a);";
        scriptMap.put("Sign",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Sqrt(0) <-- 0;0 # Sqrt(Infinity) <-- Infinity;0 # Sqrt(-Infinity) <-- Complex(0,Infinity);0 # Sqrt(Undefined) <-- Undefined;1 # Sqrt(x_PositiveInteger?)_(IsInteger(SqrtN(x))) <-- SqrtN(x);2 # Sqrt(x_PositiveNumber?)_InNumericMode() <-- SqrtN(x);2 # Sqrt(x_NegativeNumber?) <-- Complex(0,Sqrt(-x));3 # Sqrt(x_IsComplex)_InNumericMode() <-- x^(1/2);Sqrt(xlist_IsList) <-- MapSingle(\"Sqrt\",xlist);90 # (Sqrt(x_Constant?))_(NegativeNumber?(N(x))) <-- Complex(0,Sqrt(-x));400 # x_RationalOrNumber? * Sqrt(y_RationalOrNumber?) <-- Sign(x)*Sqrt(x^2*y);400 # Sqrt(y_RationalOrNumber?) * x_RationalOrNumber? <-- Sign(x)*Sqrt(x^2*y);400 # x_RationalOrNumber? / Sqrt(y_RationalOrNumber?) <-- Sign(x)*Sqrt(x^2/y);400 # Sqrt(y_RationalOrNumber?) / x_RationalOrNumber? <-- Sign(x)*Sqrt(y/(x^2));400 # Sqrt(y_RationalOrNumber?) / Sqrt(x_RationalOrNumber?) <-- Sqrt(y/x);400 # Sqrt(y_RationalOrNumber?) * Sqrt(x_RationalOrNumber?) <-- Sqrt(y*x);400 # Sqrt(x_IsInteger)_IsInteger(SqrtN(x)) <-- SqrtN(x);400 # Sqrt(x_IsInteger/y_IsInteger)_(IsInteger(SqrtN(x)) And? IsInteger(SqrtN(y))) <-- SqrtN(x)/SqrtN(y);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Maximum(l1, l2, l3, ...);10 # Maximum(_l1, _l2, l3_IsList) <-- Maximum(Concat({l1, l2}, l3));20 # Maximum(_l1, _l2, _l3) <-- Maximum({l1, l2, l3});10 # Maximum(l1_IsList,l2_IsList) <-- Map(\"Maximum\",{l1,l2});20 # Maximum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- If(l1>?l2,l1,l2);30 # Maximum(l1_Constant?,l2_Constant?) <-- If(N(Eval(l1-l2))>?0,l1,l2);10 # Maximum({}) <-- Undefined;20 # Maximum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Maximum(result,item); result;];30 # Maximum(_x) <-- x;";
        scriptMap.put("Maximum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Minimum(l1, l2, l3, ...);10 # Minimum(_l1, _l2, l3_IsList) <-- Minimum(Concat({l1, l2}, l3));20 # Minimum(_l1, _l2, _l3) <-- Minimum({l1, l2, l3});10 # Minimum(l1_IsList,l2_IsList) <-- Map(\"Minimum\",{l1,l2});20 # Minimum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- If(l1<?l2,l1,l2);30 # Minimum(l1_Constant?,l2_Constant?) <-- If(N(Eval(l1-l2))<?0,l1,l2);10 # Minimum({}) <-- Undefined;20 # Minimum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Minimum(result,item); result;];30 # Minimum(_x) <-- x;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "n1_RationalOrNumber? *** n2_RationalOrNumber? <--[ Check(n2-n1 <=? 65535, \"Argument\", \"Partial factorial: Error: the range \" : ( PipeToString() Write(n2-n1) ) : \" is too large, you may want to avoid exact calculation\"); If(n2-n1<?0, 1, Factorial'partial(n1, n2) );];2# Factorial'partial(_a, _b) _ (b-a>=?4) <-- Factorial'partial(a, a+((b-a)>>1)) * Factorial'partial(a+((b-a)>>1)+1, b);3# Factorial'partial(_a, _b) _ (b-a>=?3) <-- a*(a+1)*(a+2)*(a+3);4# Factorial'partial(_a, _b) _ (b-a>=?2) <-- a*(a+1)*(a+2);5# Factorial'partial(_a, _b) _ (b-a>=?1) <-- a*(a+1);6# Factorial'partial(_a, _b) _ (b-a>=?0) <-- a;";
        scriptMap.put("***",scriptString);
