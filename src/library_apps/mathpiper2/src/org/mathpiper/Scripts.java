...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # _x ^ (1/2) <-- Sqrt(x);50 # (x_PositiveNumber? ^ (1/2))_IsInteger(SqrtN(x)) <-- SqrtN(x);58 # 1 ^ n_Infinity? <-- Undefined;59 # _x ^ 1 <-- x;59 # 1 ^ _n <-- 1;59 # x_IsZero ^ y_IsZero <-- Undefined;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n>?0) <-- 0;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n<?0) <-- Infinity;59 # _x ^ Undefined <-- Undefined;59 # Undefined ^ _x <-- Undefined;61 # Infinity ^ (y_NegativeNumber?) <-- 0;61 # (-Infinity) ^ (y_NegativeNumber?) <-- 0;61 # x_PositiveNumber? ^ y_PositiveInteger? <-- MathIntPower(x,y);61 # x_PositiveNumber? ^ y_NegativeInteger? <-- 1/MathIntPower(x,-y);65 # (x_PositiveNumber? ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));90 # (-_x)^m_Even? <-- x^m;91 # (x_Constant? ^ (m_Odd? / p_Odd?))_(NegativeNumber?(Re(N(Eval(x))))) <-- -((-x)^(m/p));92 # (x_NegativeNumber? ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));70 # (_x ^ m_IsRationalOrNumber) ^ n_IsRationalOrNumber <-- x^(n*m);80 # (x_IsNumber/y_IsNumber) ^ n_PositiveInteger? <-- x^n/y^n;80 # (x_IsNumber/y_IsNumber) ^ n_NegativeInteger? <-- y^(-n)/x^(-n);80 # x_NegativeNumber? ^ n_Even? <-- (-x)^n;80 # x_NegativeNumber? ^ n_Odd? <-- -((-x)^n);100 # ((_x)*(_x ^ _m)) <-- x^(m+1);100 # ((_x ^ _m)*(_x)) <-- x^(m+1);100 # ((_x ^ _n)*(_x ^ _m)) <-- x^(m+n);100 # ((x_IsNumber)^(n_IsInteger/(_m)))_(n>?1) <-- MathIntPower(x,n)^(1/m);100 # Sqrt(_n)^(m_Even?) <-- n^(m/2);100 # Abs(_a)^n_Even? <-- a^n;100 # Abs(_a)^n_Odd? <-- Sign(a)*a^n;200 # x_IsMatrix ^ n_PositiveInteger? <-- x*(x^(n-1));204 # (xlist_IsList ^ nlist_IsList)_(Length(xlist)=?Length(nlist)) <-- Map(\"^\",{xlist,nlist});205 # (xlist_IsList ^ n_Constant?)_(Not?(IsList(n))) <-- Map({{xx},xx^n},{xlist});206 # (_x ^ n_IsList)_(Not?(IsList(x))) <-- Map({{xx},x^xx},{n});249 # x_Infinity? ^ 0 <-- Undefined;250 # Infinity ^ (_n) <-- Infinity;250 # Infinity ^ (_x_IsComplex) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(Even?(n)) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(Odd?(n)) <-- -Infinity;250 # (x_IsNumber ^ Infinity)_(x>? -1 And? x <? 1) <-- 0;250 # (x_IsNumber ^ Infinity)_(x>? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) <? -1) <-- -Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? -1 And? Magnitude(x) <? 1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? -1 And? x <? 1) <-- Infinity;250 # (x_IsNumber ^ -Infinity)_(x<? -1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? 1) <-- 0;255 # (x_IsComplex ^ Infinity)_(Abs(x) =? 1) <-- Undefined;255 # (x_IsComplex ^ -Infinity)_(Abs(x) =? 1) <-- Undefined;400 # _x ^ 0 <-- 1;";
        scriptMap.put("^",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # 0 / 0 <-- Undefined;52 # x_PositiveNumber? / 0 <-- Infinity;52 # x_NegativeNumber? / 0 <-- -Infinity;55 # (_x / y_IsNumber)_(IsZero(y)) <-- Undefined;55 # 0 / _x <-- 0;56 # (x_NonZeroInteger? / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger? * _expr) / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*expr)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger?) / (y_NonZeroInteger? * _expr))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/(QuotientN(y,gcd)*expr); ];57 # ((x_NonZeroInteger? * _p) / (y_NonZeroInteger? * _q))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*p)/(QuotientN(y,gcd)*q); ];60 # (x_IsDecimal / y_IsNumber) <-- DivideN(x,y);60 # (x_IsNumber / y_IsDecimal) <-- DivideN(x,y); 60 # (x_IsNumber / y_IsNumber)_(InNumericMode()) <-- DivideN(x,y);90 # x_Infinity? / y_Infinity? <-- Undefined;95 # x_Infinity? / y_IsNumber <-- Sign(y)*x;95 # x_Infinity? / y_IsComplex <-- Infinity;90 # Undefined / _y <-- Undefined;90 # _y / Undefined <-- Undefined;100 # _x / _x <-- 1;100 # _x / 1 <-- x;100 # (_x / y_NegativeNumber?) <-- -x/(-y);100 # (_x / - _y) <-- -x/y;150 # (_x) / (_x) ^ (n_Constant?) <-- x^(1-n);150 # (_x) ^ (n_Constant?) * (_x) <-- x^(n-1);150 # Sqrt(_x) / (_x) ^ (n_Constant?) <-- x^(1/2-n);150 # (_x) ^ (n_Constant?) / Sqrt(_x) <-- x^(n-1/2);150 # (_x) / Sqrt(_x) <-- Sqrt(x);200 # (_x / _y)/ _z <-- x/(y*z);230 # _x / (_y / _z) <-- (x*z)/y;240 # (xlist_IsList / ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"/\",{xlist,ylist});250 # (x_IsList / _y)_(Not?(IsList(y))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,x[i] / y); ]; result;];250 # (_x / y_IsList)_(Not?(IsList(x))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x/y[i]); ]; result;];250 # _x / Infinity <-- 0;250 # _x / (-Infinity) <-- 0;400 # 0 / _x <-- 0;";
        scriptMap.put("/",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsBinomials( _x + y_IsFreeOf(x) ) <-- {x+y,1};10 # FactorsBinomials( _x - y_IsFreeOf(x) ) <-- {x-y,1};10 # FactorsBinomials( c_Constant? * _x + y_IsFreeOf(x) ) <-- {c*x+y,1};10 # FactorsBinomials( c_Constant? * _x - y_IsFreeOf(x) ) <-- {c*x-y,1};10 # FactorsBinomials( _x^m_Odd? + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := (y^(1/m)); If(InVerboseMode(),Tell(\" FBinsum1\",{nn,qq})); r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}};];12 # FactorsBinomials( c_Constant? * _x^m_Odd? + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{c,x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := ((y/c)^(1/m)); If(InVerboseMode(),Tell(\" FBinsum.1b\",{nn,qq})); If( c=?1, r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}}, r := {{c,1},{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}} ); ];10 # FactorsBinomials( _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{x,m,y})); Local(pp,qq,r,L); pp := m-1; qq := (y^(1/m)); If(IsNumber(y),qq:=GuessRational(N(qq))); If(InVerboseMode(),Tell(\" FBindif.1\",{pp,qq}));  if (m =? 2) [ L := FunctionToList(y); If(And?(L[1]=?ToAtom(\"^\"),L[3]=?2),qq:=L[2]); r := {{x+qq,1},{x-qq,1}}; ] else if (m =? 4) [r := {{x+qq,1},{x-qq,1},{x^2+qq^2,1}};] else if (m =? 6) [r := {{x+qq,1},{x-qq,1},{x^2+x*qq+qq^2,1},{x^2-x*qq+qq^2,1}};] else [r := {{x-qq,1},{Sum(k,0,pp,qq^k*x^(pp-k)),1}};]; r;];12 # xFactorsBinomials( c_Constant? * _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{c,x,m,y})); Local(aa,bb,c0,r); aa := c^(1/m); bb := ((y)^(1/m)); If(IsNumber(y),bb:=GuessRational(N(bb))); If(InVerboseMode(),Tell(\" FBindif.1b\",{aa,bb})); r := FactorsBinomials( (aa*x)^m - bb^m );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ExtractSubMatrix( mat_IsMatrix, _row1, _col1, _row2, _col2 )_ (And?(PositiveInteger?(row1),PositiveInteger?(col1), PositiveInteger?(row2),PositiveInteger?(col2))) <--[ Local(nrows,ncols,r,row,result); {nrows,ncols} := Dimensions( mat ); Check(And?(row1>?0,col1>?0,row1<?nrows,col1<?ncols), \"Math\", \"ERROR: UL out of range\"); Check(And?(row2>?row1,col2>?col1,row2<=?nrows,col2<=?ncols), \"Math\", \"ERROR: LR out of range\"); result := {}; For(r:=row1,r<=?row2,r++) [ row := Take( MatrixRow(mat,r), {col1,col2} ); result := DestructiveAppend( result, row ); ]; result;];10 # ExtractSubMatrix( mat_IsMatrix, _row1, _col1 )_ (And?(PositiveInteger?(row1),PositiveInteger?(col1))) <--[ Local(nrows,ncols); {nrows,ncols} := Dimensions( mat ); Check(And?(row1>?0,col1>?0,row1<?nrows,col1<?ncols), \"Math\", \"ERROR: UL out of range\"); ExtractSubMatrix( mat, row1, col1, nrows, ncols );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixPower(x_IsSquareMatrix, n_NonNegativeInteger?) <--[ Local(result); result:=Identity(Length(x)); While(n !=? 0) [ If(Odd?(n), result:=Dot(result,x)); x:=Dot(x,x); n:=n>>1; ]; result;];MatrixPower(x_IsSquareMatrix, n_NegativeInteger?) <-- MatrixPower(Inverse(x),-n);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnReplace( M_IsMatrix, jCol_PositiveInteger?, v_IsVector )_(Length(v)=?Dimensions(M)[2]) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnReplace,{M,jCol,v}) ); Local(mRows,nCols,MT); {mRows,nCols} := Dimensions(M); If( jCol <=? nCols, [ MT:=Transpose(M); DestructiveReplace(MT,jCol,v); M:=Transpose(MT); ] ); M;];";
        scriptMap.put("MatrixColumnReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnSwap( M_IsMatrix, jCol1_PositiveInteger?, jCol2_PositiveInteger? )_ (And?(jCol1<=?Dimensions(M)[2],jCol2<=?Dimensions(M)[2])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnSwap,{M,jCol1,jCol2}) ); Local(MT); MT := Transpose(M); MT := MatrixRowSwap(MT,jCol1,jCol2); M := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowReplace( M_IsMatrix, iRow_PositiveInteger?, v_IsVector )_(Length(v)=?Length(M[1])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowReplace,{M,iRow,v}) ); Local(mRows,nCols); {mRows,nCols} := Dimensions(M); If( iRow <=? mRows, DestructiveReplace(M,iRow,v) ); M;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowSwap( M_IsMatrix, iRow1_PositiveInteger?, iRow2_PositiveInteger? )_ (And?(iRow1<=?Dimensions(M)[1],iRow2<=?Dimensions(M)[1])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowSwap,{M,iRow1,iRow2}) ); Local(row1,row2); If( iRow1 !=? iRow2, [ row1 := MatrixRow(M,iRow1); row2 := MatrixRow(M,iRow2); DestructiveReplace(M,iRow1,row2); DestructiveReplace(M,iRow2,row1); ] ); M;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HarmonicNumber(n_IsInteger) <-- HarmonicNumber(n,1);HarmonicNumber(n_IsInteger,r_PositiveInteger?) <--[  if( r=?1 )[ Sum(k,1,n,1/k); ] else [ Sum(k,1,n,1/k^r); ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsAmicablePair(m_PositiveInteger?,n_PositiveInteger?) <-- ( ProperDivisorsSum(m)=?n And? ProperDivisorsSum(n)=?m );";
        scriptMap.put("IsAmicablePair",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsCarmichaelNumber(n_Even?) <-- False;5 # IsCarmichaelNumber(_n)_(n<?561) <-- False;10 # IsCarmichaelNumber(n_PositiveInteger?) <--[ Local(i,factors,length,carmichael); factors:=Factors(n); carmichael:=True; length:=Length(factors); if( length <? 3)[ carmichael:=False; ] else [ For(i:=1,i<=?length And? carmichael,i++)[  If( Modulo(n-1,factors[i][1]-1) !=? 0, carmichael:=False ); If(factors[i][2]>?1,carmichael:=False);  ]; ]; carmichael;];IsCarmichaelNumber(n_IsList) <-- MapSingle(\"IsCarmichaelNumber\",n);";
        scriptMap.put("IsCarmichaelNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsComposite(1) <-- False;10 # IsComposite(n_PositiveInteger?) <-- (Not? IsPrime(n));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsIrregularPrime(p_IsComposite) <-- False;5 # IsIrregularPrime(_p)_(p<?37) <-- False;10 # IsIrregularPrime(p_PositiveInteger?) <--[ Local(i,irregular); i:=1; irregular:=False; While( 2*i + 1 <? p And? (irregular =? False) )[ If( Abs(Numerator(Bernoulli(2*i))) % p =? 0, irregular:=True ); i++; ]; irregular;];";
        scriptMap.put("IsIrregularPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPerfect(n_PositiveInteger?) <-- ProperDivisorsSum(n)=?n;";
        scriptMap.put("IsPerfect",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # IsPrime(_n)_(Not? IsInteger(n) Or? n<=?1) <-- False;3 # IsPrime(n_IsInteger)_(n<=?FastIsPrime(0)) <-- IsSmallPrime(n);5 # IsPrime(n_PositiveInteger?)_(n >? 4 And? Modulo(n^2-1,24)!=?0) <-- False;10 # IsPrime(n_PositiveInteger?) <-- RabinMiller(n);";
        scriptMap.put("IsPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsPrimePower(n_IsPrime) <-- True;10 # IsPrimePower(0) <-- False;10 # IsPrimePower(1) <-- False;20 # IsPrimePower(n_PositiveInteger?) <-- (GetPrimePower(n)[2] >? 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsTwinPrime(n_PositiveInteger?) <-- (IsPrime(n) And? IsPrime(n+2));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# NextPseudoPrime(i_IsInteger)_(i<=?1) <-- 2;2# NextPseudoPrime(2) <-- 3;3# NextPseudoPrime(i_Odd?) <--[  i := i+2; If(Modulo(i,3)=?0, i:=i+2, i);];4# NextPseudoPrime(i_Even?) <-- NextPseudoPrime(i-1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WriteDataItem(tuple_IsList, _options'hash) <--[ Local(item); If(  NumericList?(tuple), ForEach(item,tuple) [ Write(item); Space(); ] ); NewLine();];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NonZeroInteger?(x) := (IsInteger(x) And? x !=? 0);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NotZero?(x_IsNumber) <-- ( AbsN(x) >=? PowerN(10, -BuiltinPrecisionGet()));10 # NotZero?(x_Infinity?) <-- True;60000 # NotZero?(_x) <-- False;";
        scriptMap.put("NotZero?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NumericList?(_arg'list) <-- IsList(arg'list) And? (\"And?\" @ (MapSingle(Hold({{x},IsNumber(N(Eval(x)))}), arg'list)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Odd?(n) := IsInteger(n) And? ( BitAnd(n,1) =? 1 );";
        scriptMap.put("Odd?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # One?(x_IsNumber) <-- IsZero(SubtractN(x,1));60000 # One?(_x) <-- False;";
        scriptMap.put("One?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Orthogonal?(A_IsMatrix) <-- (Transpose(A)*A=?Identity(Length(A)));";
        scriptMap.put("Orthogonal?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PolynomialOverIntegers?(expr_IsFunction) <-- [ Local(x,vars); vars := VarList(expr); If(Length(vars)>?1,vars:=HeapSort(vars,\"IsGreaterThan\"));  x := vars[1]; PolynomialOverIntegers?(expr,x);];15 # PolynomialOverIntegers?(_expr) <-- False;10 # PolynomialOverIntegers?(_expr,_var)_(CanBeUni(var,expr)) <--[ If( AllSatisfy?(\"IsInteger\",Coef(expr,var,0 .. Degree(expr,var))), True, False );];15 # PolynomialOverIntegers?(_expr,_var) <-- False;";
        scriptMap.put("PolynomialOverIntegers?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Polynomial?(expr_IsFunction) <-- [ Local(x,vars); vars := VarList(expr); If(Length(vars)>?1,vars:=HeapSort(vars,\"IsGreaterThan\"));  x := vars[1]; Polynomial?(expr,x);];15 # Polynomial?(_expr) <-- False;10 # Polynomial?(_expr,_var)_(CanBeUni(var,expr)) <-- True;15 # Polynomial?(_expr,_var) <-- False;";
        scriptMap.put("Polynomial?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PositiveInteger?(x):= IsInteger(x) And? x >? 0;";
        scriptMap.put("PositiveInteger?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PositiveNumber?(x):= IsNumber(x) And? x >? 0;";
        scriptMap.put("PositiveNumber?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PositiveReal?(_r) <--[ r:=N(Eval(r)); (IsNumber(r) And? r >=? 0);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # DirichletBeta(1) <-- Pi/4;5 # DirichletBeta(2) <-- Catalan;5 # DirichletBeta(3) <-- Pi^3/32;6 # DirichletBeta(n_Odd?) <-- [ Local(k); k:=(n-1)/2; (-1)^k*Euler(2*k)*(Pi/2)^(2*k+1)/(2*(2*k)!);];10 # DirichletBeta(x_IsRationalOrNumber)_(InNumericMode() And? x>=?1 ) <-- [ Local(prec,eps,term,result,k); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+3); eps:=10^(-prec); result:=0; term:=1; For(k:=0, Abs(term) >? eps, k++ )[ term:=(-1)^k/(2*k+1)^x; Echo(\"term is \",term); result:=result+term; ]; BuiltinPrecisionSet(prec); RoundTo(result,prec);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # Gamma(n_IsRationalOrNumber)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_Constant?)_(InNumericMode()) <-- Internal'GammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_IsRationalOrNumber)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_Constant?)_(InNumericMode()) <-- Internal'LnGammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistribution(_p, _n)_ (If(IsRationalOrNumber(p),p<?0 Or? p>?1, False) Or? (Constant?(n) And? Not? PositiveInteger?(n)) ) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "tDistribution(m_IsRationalOrNumber)_(Not? PositiveInteger?(m)) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Abs(Infinity) <-- Infinity; 10 # Abs(n_IsNumber) <-- AbsN(n);10 # Abs(n_PositiveNumber?/m_PositiveNumber?) <-- n/m;10 # Abs(n_NegativeNumber?/m_PositiveNumber?) <-- (-n)/m;10 # Abs(n_PositiveNumber?/m_NegativeNumber?) <-- n/(-m);10 # Abs( Sqrt(_x)) <-- Sqrt(x);10 # Abs(-Sqrt(_x)) <-- Sqrt(x);10 # Abs(Complex(_r,_i)) <-- Sqrt(r^2 + i^2);10 # Abs(n_Infinity?) <-- Infinity;10 # Abs(Undefined) <-- Undefined;20 # Abs(n_IsList) <-- MapSingle(\"Abs\",n);100 # Abs(_a^_n) <-- Abs(a)^n;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Modulo(_n,m_IsRationalOrNumber)_(m<?0) <-- `Hold(Modulo(@n,@m));1 # Modulo(n_NegativeInteger?,m_PositiveInteger?) <--[ Local(result); result := ModuloN(n,m); If (result <? 0,result := result + m); result;];1 # Modulo(n_PositiveInteger?,m_PositiveInteger?) <-- ModuloN(n,m);2 # Modulo(0,_m) <-- 0;2 # Modulo(n_PositiveInteger?,Infinity) <-- n;3 # Modulo(n_IsInteger,m_IsInteger) <-- ModuloN(n,m);4 # Modulo(n_IsNumber,m_IsNumber) <-- NonN(Modulo(Rationalize(n),Rationalize(m)));5 # Modulo(n_IsRationalOrNumber,m_IsRationalOrNumber) <--[ Local(n1,n2,m1,m2); n1:=Numerator(n); n2:=Denominator(n); m1:=Numerator(m); m2:=Denominator(m); Modulo(n1*m2,m1*n2)/(n2*m2);];6 # Modulo(n_IsList,m_IsList) <-- Map(\"Modulo\",{n,m});7 # Modulo(n_IsList,_m) <-- Map(\"Modulo\",{n,FillList(m,Length(n))});30 # Modulo(n_CanBeUni,m_CanBeUni) <--[ Local(vars); vars:=VarList(n+m); NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];0 # Modulo(n_IsUniVar,m_IsUniVar)_(Degree(n) <? Degree(m)) <-- n;1 # Modulo(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) >=? Degree(m)) <--[ UniVariate(n[1],0, UniDivide(Concat(ZeroVector(n[2]),n[3]), Concat(ZeroVector(m[2]),m[3]))[2]);];10 # Modulo(n_CanBeUni, m_CanBeUni, vars_IsList)_(Length(vars)=?1) <--[ NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# (n_PositiveInteger?)!! _ (n<=?3) <-- n;2# (n_PositiveInteger?)!! <--[ Check(n<=?65535, \"Argument\", \"Double factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); Factorial'double(2+Modulo(n, 2), n);];3# (_n)!! _ (n=? -1 Or? n=?0)<-- 1;2# Factorial'double(_a, _b) _ (b-a>=?6) <-- Factorial'double(a, Quotient(a+b,2)) * Factorial'double(Quotient(a+b,2)+1+Modulo(Quotient(a+b,2)+1-a, 2), b);3# Factorial'double(_a, _b) _ (b-a>=?4) <-- a*(a+2)*(a+4);4# Factorial'double(_a, _b) _ (b-a>=?2) <-- a*(a+2);5# Factorial'double(_a, _b) <-- a;30 # (n_IsList)!! <-- MapSingle(\"!!\",n);";
        scriptMap.put("!!",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # 0! <-- 1;10 # (Infinity)! <-- Infinity;20 # ((n_PositiveInteger?)!) <-- [ Check(n <=? 65535, \"Argument\", \"Factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); MathFac(n);];25 # ((x_Constant?)!)_(FloatIsInt?(x) And? x>?0) <-- (Round(x)!);30 # ((x_IsNumber)!)_InNumericMode() <-- Internal'GammaNum(x+1);40 # (n_IsList)! <-- MapSingle(\"!\",n);HalfIntegerFactorial(n_Odd?) _ (n>?0) <-- Sqrt(Pi) * ( n!! / 2^((n+1)/2) );HalfIntegerFactorial(n_Odd?) _ (n<?0) <-- Sqrt(Pi) * ( (-1)^((-n-1)/2)*2^((-n-1)/2) / (-n-2)!! );40 # (n_IsRationalOrNumber)! _(Denominator(Rationalize(n))=?2) <-- HalfIntegerFactorial(Numerator(Rationalize(n)));";
        scriptMap.put("!",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TD\",{ind});10 # (TD(_i)X(_j)) <-- Delta(i,j);10 # (TD(_i) ( (_f) + (_g) ) ) <-- (TD(i)f) + (TD(i)g);10 # (TD(_i) ( (_f) - (_g) ) ) <-- (TD(i)f) - (TD(i)g);10 # (TD(_i) ( - (_g) ) ) <-- - TD(i)g;10 # (TD(_i) ( (_f) * (_g) ) ) <-- (TD(i)f)*g + f*(TD(i)g);10 # (TD(_i) ( (_f) ^ (n_PositiveInteger?) ) ) <-- n*(TD(i)f)*f^(n-1);10 # (TD(_i)Delta(_j,_k)) <-- 0;10 # (TD(_i)f_IsNumber) <-- 0;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NumericEqual(left_IsDecimal, right_IsDecimal, precision_PositiveInteger?) <--[ If(InVerboseMode(),Tell(\"NumericEqual\",{left,right})); Local(repL,repR,precL,precR,newL,newR,plo,phi,replo,rephi); Local(newhi,newrepL,newlo,newrepR,ans); repL := NumberToRep(left); repR := NumberToRep(right); precL := repL[2]; precR := repR[2]; If(InVerboseMode(),Tell(\" \",{precL,precR,precision})); newL := RoundToPrecision(left, precision ); newR := RoundToPrecision(right, precision ); If(InVerboseMode(),Tell(\" \",{newL,newR})); newrepL := NumberToRep( newL ); newrepR := NumberToRep( newR ); If(InVerboseMode(),Tell(\" \",{newrepL,newrepR})); ans := Verify( newrepL[1] - newrepR[1], 0 ); If(InVerboseMode(),Tell(\" \",ans)); ans;];15 # NumericEqual(left_IsInteger, right_IsInteger, precision_PositiveInteger?) <--[ If(InVerboseMode(),Tell(\"NumericEqualInt\",{left,right})); left =? right;];20 # NumericEqual(left_IsNumber, right_IsNumber, precision_PositiveInteger?) <--[ If(InVerboseMode(),Tell(\"NumericEqualNum\",{left,right})); Local(nI,nD,repI,repD,precI,precD,intAsDec,newDec,newrepI,newrepD,ans); If( IsInteger(left), [nI:=left; nD:=right;], [nI:=right; nD:=left;]);  repI := NumberToRep(nI); repD := NumberToRep(nD); precI := repI[2]; precD := repD[2]; intAsDec := RoundToPrecision(1.0*nI,precision); newDec := RoundToPrecision( nD, precision ); newrepI := NumberToRep( intAsDec ); newrepD := NumberToRep( newDec ); If(InVerboseMode(), [ Tell(\" \",{nI,nD}); Tell(\" \",{repI,repD}); Tell(\" \",{precI,precD}); Tell(\" \",{intAsDec,newDec}); Tell(\" \",{newrepI,newrepD}); ] ); ans := Verify( newrepI[1] - newrepD[1], 0 ); If(InVerboseMode(),Tell(\" \",ans)); ans;];25 # NumericEqual(left_IsComplex, right_IsComplex, precision_PositiveInteger?) <--[ If(InVerboseMode(),Tell(\"NumericEqualC\",{left,right})); Local(rrL,iiL,rrR,iiR,ans); rrL := Re(left); iiL := Im(left); rrR := Re(right); iiR := Im(right); If(InVerboseMode(), [ Tell(\" \",{left,right}); Tell(\" \",{rrL,rrR}); Tell(\" \",{iiL,iiR}); ] ); ans := (NumericEqual(rrL,rrR,precision) And? NumericEqual(iiL,iiR,precision));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RoundTo(x_IsNumber, precision_PositiveInteger?) <--[ Local(oldPrec,result); oldPrec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(precision); Bind(result,DivideN( Round( MultiplyN(x, 10^precision) ), 10^precision )); BuiltinPrecisionSet(oldPrec); result;];10 # RoundTo(Complex(r_IsNumber, i_IsNumber), precision_PositiveInteger?) <-- Complex(RoundTo(r, precision), RoundTo(i, precision));20 # RoundTo( Infinity,precision_PositiveInteger?) <-- Infinity;20 # RoundTo(-Infinity,precision_PositiveInteger?) <-- -Infinity;";
