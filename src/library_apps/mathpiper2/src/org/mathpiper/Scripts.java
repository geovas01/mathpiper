...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathIntPower\", {x,y}) If(IsEqual(x,0),0,If(IsEqual(x,1),1, If(Integer?(y),If(IsLessThan(y,0),  DivideN(1, PositiveIntPower(AddN(x,0.),MathNegate(y))),  PositiveIntPower(x,y) ),  False) ));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PowerN\", {x,y}) If(IsEqual(x,0),0, If(IsEqual(x,1),1, If(Integer?(y), MathIntPower(x,y), False) ));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# FactorizeInt(0) <-- {};1# FactorizeInt(1) <-- {};3# FactorizeInt(n_Integer?) <--[ Local(small'powers); n := Abs(n);    If( Gcd(ProductPrimesTo257(), n) >? 1,  small'powers := TrialFactorize(n, 257),  small'powers := {n}  ); n := small'powers[1];  If(n=?1, Rest(small'powers),  [  SortFactorList( PollardCombineLists(Rest(small'powers), PollardRhoFactorize(n)) ); ] );];";
        scriptMap.put("FactorizeInt",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsBinomials( _x + y_IsFreeOf(x) ) <-- {x+y,1};10 # FactorsBinomials( _x - y_IsFreeOf(x) ) <-- {x-y,1};10 # FactorsBinomials( c_Constant? * _x + y_IsFreeOf(x) ) <-- {c*x+y,1};10 # FactorsBinomials( c_Constant? * _x - y_IsFreeOf(x) ) <-- {c*x-y,1};10 # FactorsBinomials( _x^m_Odd? + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := (y^(1/m)); If(InVerboseMode(),Tell(\" FBinsum1\",{nn,qq})); r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}};];12 # FactorsBinomials( c_Constant? * _x^m_Odd? + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{c,x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := ((y/c)^(1/m)); If(InVerboseMode(),Tell(\" FBinsum.1b\",{nn,qq})); If( c=?1, r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}}, r := {{c,1},{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}} ); ];10 # FactorsBinomials( _x^m_Integer? - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{x,m,y})); Local(pp,qq,r,L); pp := m-1; qq := (y^(1/m)); If(IsNumber(y),qq:=GuessRational(N(qq))); If(InVerboseMode(),Tell(\" FBindif.1\",{pp,qq}));  if (m =? 2) [ L := FunctionToList(y); If(And?(L[1]=?ToAtom(\"^\"),L[3]=?2),qq:=L[2]); r := {{x+qq,1},{x-qq,1}}; ] else if (m =? 4) [r := {{x+qq,1},{x-qq,1},{x^2+qq^2,1}};] else if (m =? 6) [r := {{x+qq,1},{x-qq,1},{x^2+x*qq+qq^2,1},{x^2-x*qq+qq^2,1}};] else [r := {{x-qq,1},{Sum(k,0,pp,qq^k*x^(pp-k)),1}};]; r;];12 # xFactorsBinomials( c_Constant? * _x^m_Integer? - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{c,x,m,y})); Local(aa,bb,c0,r); aa := c^(1/m); bb := ((y)^(1/m)); If(IsNumber(y),bb:=GuessRational(N(bb))); If(InVerboseMode(),Tell(\" FBindif.1b\",{aa,bb})); r := FactorsBinomials( (aa*x)^m - bb^m );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # (count'from_Integer? .. count'to_Integer?)_(count'from <=? count'to) <-- Table(i,i,count'from,count'to,1);20 # (count'from_Integer? .. count'to_Integer?) <-- Table(i,i,count'from,count'to,-1);";
        scriptMap.put("..",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContFracList(_n) <-- ContFracList(n, Infinity);10 # ContFracList(_n, _depth)_(depth <=? 0) <-- {};20 # ContFracList(n_Integer?, _depth) <-- {n};30 # ContFracList(n_IsNumber, _depth) _InNumericMode() <-- NonN(ContFracList(Rationalize(n), depth));40 # ContFracList(n_IsNumber, _depth) <-- ContFracList(Rationalize(n), depth);35 # ContFracList((n_NegativeInteger?) / (m_Integer?), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m)-1);40 # ContFracList((n_Integer?) / (m_Integer?), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GuessRational(_x) <-- GuessRational(x, Floor(1/2*BuiltinPrecisionGet()));GuessRational(x_RationalOrNumber?, prec_Integer?) <-- [ Local(denom'estimate, cf, i); denom'estimate := 1; cf := ContFracList(x); For(i:=2, i<=?Length(cf) And? denom'estimate <? 10^prec, i++) [  denom'estimate := denom'estimate * If( cf[i] =? 1, If( i+2<=?Length(cf),  RoundTo(N(Eval(cf[i]+1/(cf[i+1]+1/cf[i+2]))), 3),  RoundTo(N(Eval(cf[i]+1/cf[i+1])), 3) ),  cf[i] ); ]; If (denom'estimate <? 10^prec,  i--  ); i--;   ContFracEval(Take(cf, i));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NearRational(_x) <-- NearRational(x, Floor(1/2*BuiltinPrecisionGet()));15 # NearRational(x_RationalOrNumber?, prec_Integer?) <-- [ Local(x1, x2, i, old'prec); old'prec := BuiltinPrecisionGet(); BuiltinPrecisionSet(prec + 8);  x1 := ContFracList(N(Eval(x+10^(-prec)))); x2 := ContFracList(N(Eval(x-10^(-prec))));     For (i:=1, i<=?Length(x1) And? i<=?Length(x2) And? x1[i]=?x2[i], i++ ) True; If( i>?Length(x1),  x1:=x2, If( i>?Length(x2),  True,   x1[i]:=Minimum(x1[i],x2[i]) ) );    If(i>?Length(x1),i:=Length(x1)); x1[i] := x1[i] + If(i=?Length(x1), 0, 1); BuiltinPrecisionSet(old'prec); ContFracEval(Take(x1, i));];20 # NearRational(_z, prec_Integer?)_ (And?(Im(z)!=?0,RationalOrNumber?(Im(z)),RationalOrNumber?(Re(z)))) <--[ Local(rr,ii); rr := Re(z); ii := Im(z); Complex( NearRational(rr,prec), NearRational(ii,prec) );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsCoprime(list_IsList) <-- (Lcm(list) =? Product(list));10 # IsCoprime(n_Integer?,m_Integer?) <-- (Gcd(n,m) =? 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # IsPrime(_n)_(Not? Integer?(n) Or? n<=?1) <-- False;3 # IsPrime(n_Integer?)_(n<=?FastIsPrime(0)) <-- IsSmallPrime(n);5 # IsPrime(n_PositiveInteger?)_(n >? 4 And? Modulo(n^2-1,24)!=?0) <-- False;10 # IsPrime(n_PositiveInteger?) <-- RabinMiller(n);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsQuadraticResidue(_a,_p) <--[ Check( Integer?(a) And? Integer?(p) And? p>?2 And? IsCoprime(a,p) And? IsPrime(p), \"Argument\", \"IsQuadraticResidue: Invalid arguments\"); If(a^((p-1)/2) % p =? 1, True, False);];";
        scriptMap.put("IsQuadraticResidue",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # IsSmallPrime(0) <-- False;3 # IsSmallPrime(n_Integer?) <-- (FastIsPrime(n)>?0);";
        scriptMap.put("IsSmallPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSquareFree(n_Integer?) <-- ( Moebius(n) !=? 0 );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LegendreSymbol(_a,_p) <--[ Check( Integer?(a) And? Integer?(p) And? p>?2 And? IsCoprime(a,p) And? IsPrime(p), \"Argument\", \"LegendreSymbol: Invalid arguments\"); If(IsQuadraticResidue(a,p), 1, -1 );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Rationalize(aNumber_IsList) <-- Rationalize /@ aNumber;20 # Rationalize( _aNumber ) <--[ Local(result,n,d); result:=Substitute(aNumber,{{x},IsNumber(x) And? Not?(Integer?(x))},\"RationalizeNumber\"); If(InVerboseMode(),Tell(\"\",result)); If(Length(VarList(aNumber))=?0, [ n:=Numerator(result); If(Type(n)=?\"Numerator\",n:=result); d:=Denominator(result); If(Type(d)=?\"Denominator\",d:=1); result := n*(1/d); ] ); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsGaussianInteger(x_IsList) <-- False;10 # IsGaussianInteger(x_IsComplex) <-- (Integer?(Re(x)) And? Integer?(Im(x)));15 # IsGaussianInteger(_x) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RandomPickVector(list, count) :=[ Check(IsList(list), \"Argument\", \"Argument 1 must be a list.\");  Check(Integer?(count), \"Argument\", \"Argument 2 must be an integer.\");  Table(RandomPick(list),x,1,count,1);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(count, iterations, body)[Rulebase(\"Repeat\",{iterations,body});Rule(\"Repeat\",2,10,Integer?(iterations) And? iterations >? 0)[ Local(count); count := 0; While (iterations >? 0) [ Eval(body); iterations--; count++; ]; count;];Rulebase(\"Repeat\",{body});Rule(\"Repeat\",1,20,True)[ Local(count); count := 0; While (True) [ Eval(body); count++; ]; count;];];UnFence(\"Repeat\",2);HoldArgumentNumber(\"Repeat\",2,2);UnFence(\"Repeat\",1);HoldArgumentNumber(\"Repeat\",1,1);";
        scriptMap.put("Repeat",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Sample(list, sampleSize) :=[ Check(IsList(list), \"Argument\", \"The first argument must be a list.\");  Check(Integer?(sampleSize) And? sampleSize >? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  list := Shuffle(list); Take(list, sampleSize);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GetNumerDenom(_expr, _a) <-- GetNumerDenom(expr)*{a,1};10 # GetNumerDenom(_expr)_Not?(HasFunctionSome?(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- {expr, 1};15 # GetNumerDenom(a_RationalOrNumber?) <-- {a, 1};20 # GetNumerDenom(_a + _b) <-- ExpandFrac'add(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a - _b) <-- ExpandFrac'add(GetNumerDenom(a), GetNumerDenom(b, -1));20 # GetNumerDenom(- _a) <-- GetNumerDenom(a, -1);20 # GetNumerDenom(+ _a) <-- GetNumerDenom(a);20 # GetNumerDenom(_a * _b) <-- ExpandFrac'multiply(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a / _b) <-- ExpandFrac'divide(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a ^ b_Integer?)_(b >? 1) <-- ExpandFrac'multiply(GetNumerDenom(a), GetNumerDenom(a^(b-1)));20 # GetNumerDenom(_a ^ b_Integer?)_(b <? -1) <-- ExpandFrac'divide(GetNumerDenom(1), GetNumerDenom(a^(-b)));20 # GetNumerDenom(_a ^ b_Integer?)_(b =? -1) <-- ExpandFrac'divide(GetNumerDenom(1), GetNumerDenom(a));25 # GetNumerDenom(_a ^ _b) <-- {a^b, 1};ExpandFrac'add({_a, _b}, {_c, _d}) <-- {a*d+b*c, b*d};ExpandFrac'multiply({_a, _b}, {_c, _d}) <-- {a*c, b*d};ExpandFrac'divide({_a, _b}, {_c, _d}) <-- {a*d, b*c};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(Integer?(n) And? n<=?0) <-- Infinity;20 # Gamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_Constant?)_(InNumericMode()) <-- Internal'GammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(Integer?(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_Constant?)_(InNumericMode()) <-- Internal'LnGammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zeta(1) <-- Infinity;10 # Zeta(0) <-- -1/2; 10 # Zeta(3)_InNumericMode() <-- Zeta3(); 10 # Zeta(n_Even?)_(n>?0) <-- Pi^n*(2^(n-1)/n! *Abs(Bernoulli(n)));10 # Zeta(n_Integer?)_(n<?0) <-- -Bernoulli(-n+1)/(-n+1);11 # Zeta(n_Infinity?) <-- 1;20 # Zeta(s_Constant?)_(InNumericMode()) <-- Internal'ZetaNum(N(Eval(s)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Internal'BernoulliArray(n_Integer?)_(n=?0 Or? n=?1) <-- [ Local(B); B:=ArrayCreate(n+1,0); B[1] := 1; If(n=?1, B[2] := -1/2); B;];20 # Internal'BernoulliArray(n_Integer?) <-- [ Local(B, i, k, k2, bin); If (InVerboseMode(), Echo({\"Internal'BernoulliArray: using direct recursion, n = \", n})); B:=ArrayCreate(n+1, 0);     B[1] := 1; B[2] := -1/2; B[3] := 1/6; For(i:=4, i<=?n, i := i+2)  [  bin := 1;   B[i+1] := 1/2-1/(i+1)*(1 + Sum(k, 1, i/2-1, [ bin := bin * (i+3-2*k) * (i+2-2*k)/ (2*k-1) / (2*k); B[2*k+1]*bin;  ] ) ); ]; B;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(BrentCacheOfConstantsN)[ CachedConstant(BrentCacheOfConstantsN, Ln2, Internal'LnNum(2)); ];10 # BrentLn(x_Integer?)_(BuiltinPrecisionGet()>?40) <--[ Local(y, n, k, eps); n := BuiltinPrecisionGet();   k := 1 + Quotient(IntLog(4*10^n, x), 2);  BuiltinPrecisionSet(n+5);  eps := DivideN(1, 10^n);  y := PowerN(x, k);   y := DivideN(Internal'Pi()*y, (2*k)*ArithmeticGeometricMean(4, y, eps)); BuiltinPrecisionSet(n); RoundTo(y, n); ];15 # BrentLn(x_Integer?) <-- LogN(x);20 # BrentLn(_x)_(x<?1) <-- -BrentLn(1/x);30 # BrentLn(_x)_(BuiltinPrecisionGet()>?85) <--[ Local(y, n, n1, k, eps); N([ n := BuiltinPrecisionGet();   n1 := n + IntLog(n,10);   k := 2 + Quotient(n1*28738, 2*8651)   - IntLog(Floor(x), 2);  BuiltinPrecisionSet(n1+2+Quotient(k*3361, 11165));  eps := DivideN(1, 10^(n1+1));  y := x*2^(k-2);    y:=Internal'Pi()*y/(2*ArithmeticGeometricMean(1,y,eps)) - k*Ln2(); BuiltinPrecisionSet(n); ]); y; ];40 # BrentLn(x_IsNumber) <-- LogN(x);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Gcd(0,0) <-- 1;1 # Gcd(0,_m) <-- Abs(m);1 # Gcd(_n,0) <-- Abs(n);1 # Gcd(_m,_m) <-- Abs(m);2 # Gcd(_n,1) <-- 1;2 # Gcd(1,_m) <-- 1;2 # Gcd(n_Integer?,m_Integer?) <-- GcdN(n,m);3 # Gcd(_n,_m)_(IsGaussianInteger(m) And? IsGaussianInteger(n) )<-- GaussianGcd(n,m);4 # Gcd(-(_n), (_m)) <-- Gcd(n,m);4 # Gcd( (_n),-(_m)) <-- Gcd(n,m);4 # Gcd(Sqrt(n_Integer?),Sqrt(m_Integer?)) <-- Sqrt(Gcd(n,m));4 # Gcd(Sqrt(n_Integer?),m_Integer?) <-- Sqrt(Gcd(n,m^2));4 # Gcd(n_Integer?,Sqrt(m_Integer?)) <-- Sqrt(Gcd(n^2,m));5 # Gcd(n_Rational?,m_Rational?) <--[ Gcd(Numerator(n),Numerator(m))/Lcm(Denominator(n),Denominator(m));];10 # Gcd(list_IsList)_(Length(list)>?2) <-- [ Local(first); first:=Gcd(list[1],list[2]); Gcd(first:Rest(Rest(list))); ];14 # Gcd({0}) <-- 1;15 # Gcd({_head}) <-- head;20 # Gcd(list_IsList)_(Length(list)=?2) <-- Gcd(list[1],list[2]);30 # Gcd(n_CanBeUni,m_CanBeUni)_(Length(VarList(n*m))=?1) <--[ Local(vars); vars:=VarList(n+m); NormalForm(Gcd(MakeUni(n,vars),MakeUni(m,vars)));];100 # Gcd(n_Constant?,m_Constant?) <-- 1;110 # Gcd(_m,_n) <--[ Echo(\"Not simplified\");];0 # Gcd(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) <? Degree(m)) <-- Gcd(m,n);1 # Gcd(nn_IsUniVar,mm_IsUniVar)_ (nn[1] =? mm[1] And? Degree(nn) >=? Degree(mm)) <--[ UniVariate(nn[1],0, UniGcd(Concat(ZeroVector(nn[2]),nn[3]), Concat(ZeroVector(mm[2]),mm[3])));];";
        scriptMap.put("Gcd",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Lcm(a_Integer?,b_Integer?) <-- Quotient(a*b,Gcd(a,b));10 # Lcm(list_IsList)_(Length(list)>?2) <--[ Local(first); first:=Lcm(list[1],list[2]); Lcm(first:Rest(Rest(list)));];10 # Lcm(list_IsList)_(Length(list)=?2) <-- Lcm(list[1],list[2]);10 # Lcm(list_IsList)_(Length(list)=?1) <-- Lcm(list[1],1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Sqrt(0) <-- 0;0 # Sqrt(Infinity) <-- Infinity;0 # Sqrt(-Infinity) <-- Complex(0,Infinity);0 # Sqrt(Undefined) <-- Undefined;1 # Sqrt(x_PositiveInteger?)_(Integer?(SqrtN(x))) <-- SqrtN(x);2 # Sqrt(x_PositiveNumber?)_InNumericMode() <-- SqrtN(x);2 # Sqrt(x_NegativeNumber?) <-- Complex(0,Sqrt(-x));3 # Sqrt(x_IsComplex)_InNumericMode() <-- x^(1/2);Sqrt(xlist_IsList) <-- MapSingle(\"Sqrt\",xlist);90 # (Sqrt(x_Constant?))_(NegativeNumber?(N(x))) <-- Complex(0,Sqrt(-x));400 # x_RationalOrNumber? * Sqrt(y_RationalOrNumber?) <-- Sign(x)*Sqrt(x^2*y);400 # Sqrt(y_RationalOrNumber?) * x_RationalOrNumber? <-- Sign(x)*Sqrt(x^2*y);400 # x_RationalOrNumber? / Sqrt(y_RationalOrNumber?) <-- Sign(x)*Sqrt(x^2/y);400 # Sqrt(y_RationalOrNumber?) / x_RationalOrNumber? <-- Sign(x)*Sqrt(y/(x^2));400 # Sqrt(y_RationalOrNumber?) / Sqrt(x_RationalOrNumber?) <-- Sqrt(y/x);400 # Sqrt(y_RationalOrNumber?) * Sqrt(x_RationalOrNumber?) <-- Sqrt(y*x);400 # Sqrt(x_Integer?)_Integer?(SqrtN(x)) <-- SqrtN(x);400 # Sqrt(x_Integer?/y_Integer?)_(Integer?(SqrtN(x)) And? Integer?(SqrtN(y))) <-- SqrtN(x)/SqrtN(y);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TSum\",{indices,body});10 # (TSum({})(_body)) <-- body;20 # (TSum(_indices)(_body))_(Integer?(Ndim)) <-- LocalSymbols(index,i,sum) [ Local(index,i,sum); index:=indices[1]; sum:=0; MacroLocal(index); For(i:=1,i<=?Ndim,i++) [ MacroBind(index,i); sum:=sum+Eval(TSum(Rest(indices))body); ]; sum; ];UnFence(\"TSum\",2);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Coef(uv_IsUniVar,order_IsList) <--[ Local(result); result:={}; ForEach(item,order) [ DestructiveAppend(result,Coef(uv,item)); ]; result;];10 # Coef(uv_IsUniVar,order_Integer?)_(order<?uv[2]) <-- 0;10 # Coef(uv_IsUniVar,order_Integer?)_(order>=?uv[2]+Length(uv[3])) <-- 0;20 # Coef(uv_IsUniVar,order_Integer?) <-- uv[3][(order-uv[2])+1];30 # Coef(uv_CanBeUni,_order)_(Integer?(order) Or? IsList(order)) <-- Coef(MakeUni(uv),order);Function(\"Coef\",{expression,var,order}) NormalForm(Coef(MakeUni(expression,var),order));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RepeatedSquaresMultiply(_a,- (n_Integer?)) <-- 1/RepeatedSquaresMultiply(a,n);15 # RepeatedSquaresMultiply(UniVariate(_var,_first,{_coef}),(n_Integer?)) <-- UniVariate(var,first*n,{coef^n});20 # RepeatedSquaresMultiply(_a,n_Integer?) <--[ Local(m,b); Bind(m,1); Bind(b,1); While(m<=?n) Bind(m,(ShiftLeft(m,1))); Bind(m, ShiftRight(m,1)); While(m>?0) [ Bind(b,b*b); If (Not?(IsEqual(BitAnd(m,n), 0)),Bind(b,b*a)); Bind(m, ShiftRight(m,1)); ]; b;];";
