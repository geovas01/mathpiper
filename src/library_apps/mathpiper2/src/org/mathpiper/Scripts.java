...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "a_NonNegativeInteger? % b_IsPositiveInteger <-- Modulo(a,b);";
        scriptMap.put("%",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # x_IsNumber * y_IsNumber <-- MultiplyN(x,y);100 # 1 * _x <-- x;100 # _x * 1 <-- x;100 # (_f * _x)_(f=? -1) <-- -x;100 # (_x * _f)_(f=? -1) <-- -x;105 # (f_NegativeNumber? * _x) <-- -(-f)*x;105 # (_x * f_NegativeNumber?) <-- -(-f)*x;95 # x_IsMatrix * y_IsMatrix <--[ Local(i,j,k,row,result); result:=ZeroMatrix(Length(x),Length(y[1])); For(i:=1,i<=?Length(x),i++) For(j:=1,j<=?Length(y),j++) For(k:=1,k<=?Length(y[1]),k++) [ row:=result[i]; row[k]:= row[k]+x[i][j]*y[j][k]; ]; result;];96 # x_IsMatrix * y_IsList <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,Dot(x[i], y)); ]; result;];97 # (x_IsList * y_IsNonObject)_Not?(IsList(y)) <-- y*x;98 # (x_IsNonObject * y_IsList)_Not?(IsList(x)) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x * y[i]); ]; result;];50 # _x * Undefined <-- Undefined;50 # Undefined * _y <-- Undefined;100 # 0 * y_Infinity? <-- Undefined;100 # x_Infinity? * 0 <-- Undefined;101 # 0 * (_x) <-- 0;101 # (_x) * 0 <-- 0;100 # x_IsNumber * (y_IsNumber * _z) <-- (x*y)*z;100 # x_IsNumber * (_y * z_IsNumber) <-- (x*z)*y;100 # (_x * _y) * _y <-- x * y^2;100 # (_x * _y) * _x <-- y * x^2;100 # _y * (_x * _y) <-- x * y^2;100 # _x * (_x * _y) <-- y * x^2;100 # _x * (_y / _z) <-- (x*y)/z;100 # (_y / _z) * _x <-- (x*y)/z;100 # (_x * y_IsNumber)_Not?(IsNumber(x)) <-- y*x;100 # (_x) * (_x) ^ (n_Constant?) <-- x^(n+1);100 # (_x) ^ (n_Constant?) * (_x) <-- x^(n+1);100 # (_x * _y)* _x ^ n_Constant? <-- y * x^(n+1);100 # (_y * _x)* _x ^ n_Constant? <-- y * x^(n+1);100 # Sqrt(_x) * (_x) ^ (n_Constant?) <-- x^(n+1/2);100 # (_x) ^ (n_Constant?) * Sqrt(_x) <-- x^(n+1/2);100 # Sqrt(_x) * (_x) <-- x^(3/2);100 # (_x) * Sqrt(_x) <-- x^(3/2);105 # x_IsNumber * -(_y) <-- (-x)*y;105 # (-(_x)) * (y_IsNumber) <-- (-y)*x;106 # _x * -(_y) <-- -(x*y);106 # (- _x) * _y <-- -(x*y);107 # -( (-(_x))/(_y)) <-- x/y;107 # -( (_x)/(-(_y))) <-- x/y;250 # x_IsNumber * y_Infinity? <-- Sign(x)*y;250 # x_Infinity? * y_IsNumber <-- Sign(y)*x;230 # (aLeft_IsList * aRight_IsList)_(Length(aLeft)=?Length(aRight)) <-- Map(\"*\",{aLeft,aRight});242 # (x_IsInteger / y_IsInteger) * (v_IsInteger / w_IsInteger) <-- (x*v)/(y*w);243 # x_IsInteger * (y_IsInteger / z_IsInteger) <-- (x*y)/z;243 # (y_IsInteger / z_IsInteger) * x_IsInteger <-- (x*y)/z;400 # (_x) * (_x) <-- x^2;";
        scriptMap.put("*",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # _x ^ (1/2) <-- Sqrt(x);50 # (x_IsPositiveNumber ^ (1/2))_IsInteger(SqrtN(x)) <-- SqrtN(x);58 # 1 ^ n_Infinity? <-- Undefined;59 # _x ^ 1 <-- x;59 # 1 ^ _n <-- 1;59 # x_IsZero ^ y_IsZero <-- Undefined;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n>?0) <-- 0;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n<?0) <-- Infinity;59 # _x ^ Undefined <-- Undefined;59 # Undefined ^ _x <-- Undefined;61 # Infinity ^ (y_NegativeNumber?) <-- 0;61 # (-Infinity) ^ (y_NegativeNumber?) <-- 0;61 # x_IsPositiveNumber ^ y_IsPositiveInteger <-- MathIntPower(x,y);61 # x_IsPositiveNumber ^ y_NegativeInteger? <-- 1/MathIntPower(x,-y);65 # (x_IsPositiveNumber ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));90 # (-_x)^m_Even? <-- x^m;91 # (x_Constant? ^ (m_IsOdd / p_IsOdd))_(NegativeNumber?(Re(N(Eval(x))))) <-- -((-x)^(m/p));92 # (x_NegativeNumber? ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));70 # (_x ^ m_IsRationalOrNumber) ^ n_IsRationalOrNumber <-- x^(n*m);80 # (x_IsNumber/y_IsNumber) ^ n_IsPositiveInteger <-- x^n/y^n;80 # (x_IsNumber/y_IsNumber) ^ n_NegativeInteger? <-- y^(-n)/x^(-n);80 # x_NegativeNumber? ^ n_Even? <-- (-x)^n;80 # x_NegativeNumber? ^ n_IsOdd <-- -((-x)^n);100 # ((_x)*(_x ^ _m)) <-- x^(m+1);100 # ((_x ^ _m)*(_x)) <-- x^(m+1);100 # ((_x ^ _n)*(_x ^ _m)) <-- x^(m+n);100 # ((x_IsNumber)^(n_IsInteger/(_m)))_(n>?1) <-- MathIntPower(x,n)^(1/m);100 # Sqrt(_n)^(m_Even?) <-- n^(m/2);100 # Abs(_a)^n_Even? <-- a^n;100 # Abs(_a)^n_IsOdd <-- Sign(a)*a^n;200 # x_IsMatrix ^ n_IsPositiveInteger <-- x*(x^(n-1));204 # (xlist_IsList ^ nlist_IsList)_(Length(xlist)=?Length(nlist)) <-- Map(\"^\",{xlist,nlist});205 # (xlist_IsList ^ n_Constant?)_(Not?(IsList(n))) <-- Map({{xx},xx^n},{xlist});206 # (_x ^ n_IsList)_(Not?(IsList(x))) <-- Map({{xx},x^xx},{n});249 # x_Infinity? ^ 0 <-- Undefined;250 # Infinity ^ (_n) <-- Infinity;250 # Infinity ^ (_x_IsComplex) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(Even?(n)) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(IsOdd(n)) <-- -Infinity;250 # (x_IsNumber ^ Infinity)_(x>? -1 And? x <? 1) <-- 0;250 # (x_IsNumber ^ Infinity)_(x>? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) <? -1) <-- -Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? -1 And? Magnitude(x) <? 1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? -1 And? x <? 1) <-- Infinity;250 # (x_IsNumber ^ -Infinity)_(x<? -1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? 1) <-- 0;255 # (x_IsComplex ^ Infinity)_(Abs(x) =? 1) <-- Undefined;255 # (x_IsComplex ^ -Infinity)_(Abs(x) =? 1) <-- Undefined;400 # _x ^ 0 <-- 1;";
        scriptMap.put("^",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "51 # -Undefined <-- Undefined;54 # - (- _x) <-- x;55 # (- (x_IsNumber)) <-- SubtractN(0,x);100 # _x - n_Constant?*(_x) <-- (1-n)*x;100 # n_Constant?*(_x) - _x <-- (n-1)*x;110 # - (_x - _y) <-- y-x;111 # - (x_IsNumber / _y) <-- (-x)/y; LocalSymbols(x)[ 200 # - (x_IsList) <-- MapSingle(\"-\",x);];50 # x_IsNumber - y_IsNumber <-- SubtractN(x,y);50 # x_IsNumber - y_IsNumber <-- SubtractN(x,y);60 # Infinity - Infinity <-- Undefined;100 # 0 - _x <-- -x;100 # _x - 0 <-- x;100 # _x - _x <-- 0;110 # _x - (- _y) <-- x + y;110 # _x - (y_NegativeNumber?) <-- x + (-y);111 # (_x + _y)- _x <-- y;111 # (_x + _y)- _y <-- x;112 # _x - (_x + _y) <-- - y;112 # _y - (_x + _y) <-- - x;113 # (- _x) - _y <-- -(x+y);113 # (x_NegativeNumber?) - _y <-- -((-x)+y);113 # (x_NegativeNumber?)/_y - _z <-- -((-x)/y+z);LocalSymbols(x,y,xarg,yarg)[ 10 # ((x_IsList) - (y_IsList))_(Length(x)=?Length(y)) <-- [ Map({{xarg,yarg},xarg-yarg},{x,y}); ];];240 # (x_IsList - y_IsNonObject)_Not?(IsList(y)) <-- -(y-x);241 # (x_IsNonObject - y_IsList)_Not?(IsList(x)) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x - y[i]); ]; result;];250 # z_Infinity? - Complex(_x,_y) <-- Complex(-x+z,-y);250 # Complex(_x,_y) - z_Infinity? <-- Complex(x-z,y);251 # z_Infinity? - _x <-- z;251 # _x - z_Infinity? <-- -z;250 # Undefined - _y <-- Undefined;250 # _x - Undefined <-- Undefined;210 # x_IsNumber - (y_IsNumber / z_IsNumber) <--(x*z-y)/z;210 # (y_IsNumber / z_IsNumber) - x_IsNumber <--(y-x*z)/z;210 # (x_IsNumber / v_IsNumber) - (y_IsNumber / z_IsNumber) <--(x*z-y*v)/(v*z);";
        scriptMap.put("-",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # + _x <-- x;50 # x_IsNumber + y_IsNumber <-- AddN(x,y);100 # 0 + _x <-- x;100 # _x + 0 <-- x;100 # _x + _x <-- 2*x;100 # _x + n_Constant?*(_x) <-- (n+1)*x;100 # n_Constant?*(_x) + _x <-- (n+1)*x;101 # _x + - _y <-- x-y;101 # _x + (- _y)/(_z) <-- x-(y/z);101 # (- _y)/(_z) + _x <-- x-(y/z);101 # (- _x) + _y <-- y-x;102 # _x + y_NegativeNumber? <-- x-(-y);102 # _x + y_NegativeNumber? * _z <-- x-((-y)*z);102 # _x + (y_NegativeNumber?)/(_z) <-- x-((-y)/z);102 # (y_NegativeNumber?)/(_z) + _x <-- x-((-y)/z);102 # (x_NegativeNumber?) + _y <-- y-(-x);150 # _n1 / _d + _n2 / _d <-- (n1+n2)/d;200 # (x_IsNumber + _y)_Not?(IsNumber(y)) <-- y+x;200 # ((_y + x_IsNumber) + _z)_Not?(IsNumber(y) Or? IsNumber(z)) <-- (y+z)+x;200 # ((x_IsNumber + _y) + z_IsNumber)_Not?(IsNumber(y)) <-- y+(x+z);200 # ((_x + y_IsNumber) + z_IsNumber)_Not?(IsNumber(x)) <-- x+(y+z);210 # x_IsNumber + (y_IsNumber / z_IsNumber) <--(x*z+y)/z;210 # (y_IsNumber / z_IsNumber) + x_IsNumber <--(x*z+y)/z;210 # (x_IsNumber / v_IsNumber) + (y_IsNumber / z_IsNumber) <--(x*z+y*v)/(v*z);220 # (xlist_IsList + ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"+\",{xlist,ylist});SumListSide(_x, y_IsList) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x + y[i]); ]; result;];240 # (x_IsList + _y)_Not?(IsList(y)) <-- SumListSide(y,x);241 # (_x + y_IsList)_Not?(IsList(x)) <-- SumListSide(x,y);250 # z_Infinity? + Complex(_x,_y) <-- Complex(x+z,y);250 # Complex(_x,_y) + z_Infinity? <-- Complex(x+z,y);251 # z_Infinity? + _x <-- z;251 # _x + z_Infinity? <-- z;250 # Undefined + _y <-- Undefined;250 # _x + Undefined <-- Undefined;";
        scriptMap.put("+",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # 0 / 0 <-- Undefined;52 # x_IsPositiveNumber / 0 <-- Infinity;52 # x_NegativeNumber? / 0 <-- -Infinity;55 # (_x / y_IsNumber)_(IsZero(y)) <-- Undefined;55 # 0 / _x <-- 0;56 # (x_IsNonZeroInteger / y_IsNonZeroInteger)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/QuotientN(y,gcd); ];57 # ((x_IsNonZeroInteger * _expr) / y_IsNonZeroInteger)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*expr)/QuotientN(y,gcd); ];57 # ((x_IsNonZeroInteger) / (y_IsNonZeroInteger * _expr))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/(QuotientN(y,gcd)*expr); ];57 # ((x_IsNonZeroInteger * _p) / (y_IsNonZeroInteger * _q))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*p)/(QuotientN(y,gcd)*q); ];60 # (x_IsDecimal / y_IsNumber) <-- DivideN(x,y);60 # (x_IsNumber / y_IsDecimal) <-- DivideN(x,y); 60 # (x_IsNumber / y_IsNumber)_(InNumericMode()) <-- DivideN(x,y);90 # x_Infinity? / y_Infinity? <-- Undefined;95 # x_Infinity? / y_IsNumber <-- Sign(y)*x;95 # x_Infinity? / y_IsComplex <-- Infinity;90 # Undefined / _y <-- Undefined;90 # _y / Undefined <-- Undefined;100 # _x / _x <-- 1;100 # _x / 1 <-- x;100 # (_x / y_NegativeNumber?) <-- -x/(-y);100 # (_x / - _y) <-- -x/y;150 # (_x) / (_x) ^ (n_Constant?) <-- x^(1-n);150 # (_x) ^ (n_Constant?) * (_x) <-- x^(n-1);150 # Sqrt(_x) / (_x) ^ (n_Constant?) <-- x^(1/2-n);150 # (_x) ^ (n_Constant?) / Sqrt(_x) <-- x^(n-1/2);150 # (_x) / Sqrt(_x) <-- Sqrt(x);200 # (_x / _y)/ _z <-- x/(y*z);230 # _x / (_y / _z) <-- (x*z)/y;240 # (xlist_IsList / ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"/\",{xlist,ylist});250 # (x_IsList / _y)_(Not?(IsList(y))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,x[i] / y); ]; result;];250 # (_x / y_IsList)_(Not?(IsList(x))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x/y[i]); ]; result;];250 # _x / Infinity <-- 0;250 # _x / (-Infinity) <-- 0;400 # 0 / _x <-- 0;";
        scriptMap.put("/",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # BinomialCoefficient(0,0) <-- 1;10 # BinomialCoefficient(n_IsPositiveInteger,m_NonNegativeInteger?)_(2*m <=? n) <-- ((n-m+1) *** n) / m!;15 # BinomialCoefficient(n_IsPositiveInteger,m_NonNegativeInteger?)_(2*m >? n And? m <=? n) <-- BinomialCoefficient(n, n-m);20 # BinomialCoefficient(n_IsInteger,m_IsInteger) <-- 0;Combinations(n,m) := BinomialCoefficient(n,m);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Arg(Complex(Cos(_x),Sin(_x))) <-- x;10 # Arg(x_IsZero) <-- Undefined;15 # Arg(x_IsPositiveReal) <-- 0;15 # Arg(x_NegativeReal?) <-- Pi;20 # Arg(Complex(r_IsZero,i_Constant?)) <-- Sign(i)*Pi/2;30 # Arg(Complex(r_IsPositiveReal,i_Constant?)) <-- ArcTan(i/r);40 # Arg(Complex(r_NegativeReal?,i_IsPositiveReal)) <-- Pi+ArcTan(i/r);50 # Arg(Complex(r_NegativeReal?,i_NegativeReal?)) <-- ArcTan(i/r)-Pi;";
        scriptMap.put("Arg",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Complex(_r,i_IsZero) <-- r;2 # Complex(Complex(_r1,_i1),_i2) <-- Complex(r1,i1+i2);2 # Complex(_r1,Complex(_r2,_i2)) <-- Complex(r1-i2,r2);6 # Complex(Undefined,_x) <-- Undefined;6 # Complex(_x,Undefined) <-- Undefined;110 # Complex(_r1,_i1) + Complex(_r2,_i2) <-- Complex(r1+r2,i1+i2);300 # Complex(_r,_i) + x_Constant? <-- Complex(r+x,i);300 # x_Constant? + Complex(_r,_i) <-- Complex(r+x,i);110 # - Complex(_r,_i) <-- Complex(-r,-i);300 # Complex(_r,_i) - x_Constant? <-- Complex(r-x,i);300 # x_Constant? - Complex(_r,_i) <-- Complex((-r)+x,-i);111 # Complex(_r1,_i1) - Complex(_r2,_i2) <-- Complex(r1-r2,i1-i2);110 # Complex(_r1,_i1) * Complex(_r2,_i2) <-- Complex(r1*r2-i1*i2,r1*i2+r2*i1);250 # Complex(r_IsZero,_i) * x_Infinity? <-- Complex(0,i*x);250 # Complex(_r,i_IsZero) * x_Infinity? <-- Complex(r*x,0);251 # Complex(_r,_i) * x_Infinity? <-- Complex(r*x,i*x);250 # x_Infinity? * Complex(r_IsZero,_i) <-- Complex(0,i*x);250 # x_Infinity? * Complex(_r,i_IsZero) <-- Complex(r*x,0);251 # x_Infinity? * Complex(_r,_i) <-- Complex(r*x,i*x);300 # Complex(_r,_i) * y_Constant? <-- Complex(r*y,i*y);300 # y_Constant? * Complex(_r,_i) <-- Complex(r*y,i*y);330 # Complex(_r,_i) * (y_Constant? / _z) <-- (Complex(r*y,i*y))/z;330 # (y_Constant? / _z) * Complex(_r,_i) <-- (Complex(r*y,i*y))/z;110 # x_Constant? / Complex(_r,_i) <-- (x*Conjugate(Complex(r,i)))/(r^2+i^2);300 # Complex(_r,_i) / y_Constant? <-- Complex(r/y,i/y);110 # (_x ^ Complex(_r,_i)) <-- Exp(Complex(r,i)*Ln(x));110 # Sqrt(Complex(_r,_i)) <-- Exp(Ln(Complex(r,i))/2);110 # (Complex(_r,_i) ^ x_IsRationalOrNumber)_(Not?(IsInteger(x))) <-- Exp(x*Ln(Complex(r,i)));123 # Complex(_r, _i) ^ n_NegativeInteger? <-- 1/Complex(r, i)^(-n);124 # Complex(_r, _i) ^ (p_IsZero) <-- 1; 125 # Complex(_r, _i) ^ n_IsPositiveInteger <--[  Local(result, x); x:=Complex(r,i); result:=1; While(n >? 0) [ if ((n&1) =? 1) [ result := result*x; ]; x := x*x; n := n>>1; ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # II^n_NegativeInteger? <-- (-II)^(-n);20 # (II^_n)_(Even?(n) =? True) <-- (-1)^(n>>1);20 # (II^_n)_(IsOdd(n) =? True) <-- II*(-1)^(n>>1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( IsRationalFunction(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- [ Local(r); If( IsRationalFunction(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # Monomial?(_expr) <-- False;10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( IsRationalFunction(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Determinant(_matrix)_(IsUpperTriangular(matrix) Or? LowerTriangular?(matrix)) <--[ Local(result); result:=1; ForEach(i, Diagonal(matrix) ) result:=result*i; result;];15 # Determinant(_matrix)_(Length(Select(Diagonal(matrix), \"IsZero\")) >? 0) <-- SymbolicDeterminant(matrix);16 # Determinant(_matrix)_(VarList(matrix) !=? {}) <-- SymbolicDeterminant(matrix);20 # Determinant(_matrix) <-- GaussianDeterminant(matrix);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContFracList(_n) <-- ContFracList(n, Infinity);10 # ContFracList(_n, _depth)_(depth <=? 0) <-- {};20 # ContFracList(n_IsInteger, _depth) <-- {n};30 # ContFracList(n_IsNumber, _depth) _InNumericMode() <-- NonN(ContFracList(Rationalize(n), depth));40 # ContFracList(n_IsNumber, _depth) <-- ContFracList(Rationalize(n), depth);35 # ContFracList((n_NegativeInteger?) / (m_IsInteger), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m)-1);40 # ContFracList((n_IsInteger) / (m_IsInteger), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EigenValues(matrix_IsUpperTriangular) <-- Diagonal(matrix);10 # EigenValues(matrix_LowerTriangular?) <-- Diagonal(matrix);20 # EigenValues(matrix_IsMatrix) <-- Roots(CharacteristicEquation(matrix,xx));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Euler(0) <-- 1;10 # Euler(n_IsOdd) <-- 0;10 # Euler(n_Even?) <-- - Sum(r,0,n/2-1,BinomialCoefficient(n,2*r)*Euler(2*r));10 # Euler(n_NonNegativeInteger?,_x) <-- Sum(i,0,Round(n/2),BinomialCoefficient(n,2*i)*Euler(2*i)*(x-1/2)^(n-2*i)/2^(2*i));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Idempotent?(A_IsMatrix) <-- (A^2 =? A);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsNotZero(x_IsNumber) <-- ( AbsN(x) >=? PowerN(10, -BuiltinPrecisionGet()));10 # IsNotZero(x_Infinity?) <-- True;60000 # IsNotZero(_x) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LowerTriangular?(A_IsMatrix) <-- (IsUpperTriangular(Transpose(A)));";
        scriptMap.put("LowerTriangular?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)_(Type(expr)=?\"UniVariate\") <-- False;10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- [ Local(r); If( IsRationalFunction(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # Monomial?(_expr) <-- False;";
        scriptMap.put("Monomial?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NegativeInteger?(x):= IsInteger(x) And? x <? 0;";
        scriptMap.put("NegativeInteger?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NegativeNumber?(x):= IsNumber(x) And? x <? 0;";
        scriptMap.put("NegativeNumber?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NegativeReal?(_r) <--[ r:=N(Eval(r)); (IsNumber(r) And? r <=? 0);];";
        scriptMap.put("NegativeReal?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NonNegativeInteger?(x):= IsInteger(x) And? x >=? 0;";
        scriptMap.put("NonNegativeInteger?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NonNegativeNumber?(x):= IsNumber(x) And? x >=? 0;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zeta(1) <-- Infinity;10 # Zeta(0) <-- -1/2; 10 # Zeta(3)_InNumericMode() <-- Zeta3(); 10 # Zeta(n_Even?)_(n>?0) <-- Pi^n*(2^(n-1)/n! *Abs(Bernoulli(n)));10 # Zeta(n_IsInteger)_(n<?0) <-- -Bernoulli(-n+1)/(-n+1);11 # Zeta(n_Infinity?) <-- 1;20 # Zeta(s_Constant?)_(InNumericMode()) <-- Internal'ZetaNum(N(Eval(s)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # ArcSin(x_IsNumber)_(InNumericMode() And? Abs(x)<=?1) <-- ArcSinNum(x);3 # ArcSin(x_IsNumber)_InNumericMode() <-- Sign(x)*(Pi/2+I*ArcCosh(x));110 # ArcSin(Complex(_r,_i)) <-- (- I) * Ln((I*Complex(r,i)) + ((1-(Complex(r,i)^2))^(1/2)));150 # ArcSin(- _x)_(Not? Constant?(x)) <-- -ArcSin(x);160 # (ArcSin(x_Constant?))_(NegativeNumber?(N(Eval(x)))) <-- -ArcSin(-x);200 # ArcSin(0) <-- 0;200 # ArcSin(1/2) <-- Pi/6;200 # ArcSin(Sqrt(1/2)) <-- Pi/4;200 # ArcSin(Sqrt(3/4)) <-- Pi/3;200 # ArcSin(1) <-- Pi/2;200 # ArcSin(_n)_(n =? -1) <-- -Pi/2;200 # ArcSin(_n)_(-n =? Sqrt(3/4)) <-- -Pi/3;200 # ArcSin(_n)_(-n =? Sqrt(1/2)) <-- -Pi/4;200 # ArcSin(_n)_(-n =? 1/2) <-- -Pi/6;ArcSin(xlist_IsList) <-- MapSingle(\"ArcSin\",xlist);200 # ArcSin(Undefined) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Ln(0) <-- -Infinity;2 # Ln(1) <-- 0;2 # Ln(Infinity) <-- Infinity;2 # Ln(Undefined) <-- Undefined;2 # Ln(x_NegativeNumber?)_InNumericMode() <-- Complex(Ln(-x), Pi);3 # Ln(x_IsNumber)_(InNumericMode() And? x>=?1) <-- Internal'LnNum(x);4 # Ln(Exp(_x)) <-- x;3 # Ln(Complex(_r,_i)) <-- Complex(Ln(Abs(Complex(r,i))), Arg(Complex(r,i)));4 # Ln(x_NegativeNumber?) <-- Complex(Ln(-x), Pi);5 # Ln(x_IsNumber)_(InNumericMode() And? x<?1) <-- - Internal'LnNum(DivideN(1, x));Ln(xlist_IsList) <-- MapSingle(\"Ln\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Abs(Infinity) <-- Infinity; 10 # Abs(n_IsNumber) <-- AbsN(n);10 # Abs(n_IsPositiveNumber/m_IsPositiveNumber) <-- n/m;10 # Abs(n_NegativeNumber?/m_IsPositiveNumber) <-- (-n)/m;10 # Abs(n_IsPositiveNumber/m_NegativeNumber?) <-- n/(-m);10 # Abs( Sqrt(_x)) <-- Sqrt(x);10 # Abs(-Sqrt(_x)) <-- Sqrt(x);10 # Abs(Complex(_r,_i)) <-- Sqrt(r^2 + i^2);10 # Abs(n_Infinity?) <-- Infinity;10 # Abs(Undefined) <-- Undefined;20 # Abs(n_IsList) <-- MapSingle(\"Abs\",n);100 # Abs(_a^_n) <-- Abs(a)^n;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Modulo(_n,m_IsRationalOrNumber)_(m<?0) <-- `Hold(Modulo(@n,@m));1 # Modulo(n_NegativeInteger?,m_IsPositiveInteger) <--[ Local(result); result := ModuloN(n,m); If (result <? 0,result := result + m); result;];1 # Modulo(n_IsPositiveInteger,m_IsPositiveInteger) <-- ModuloN(n,m);2 # Modulo(0,_m) <-- 0;2 # Modulo(n_IsPositiveInteger,Infinity) <-- n;3 # Modulo(n_IsInteger,m_IsInteger) <-- ModuloN(n,m);4 # Modulo(n_IsNumber,m_IsNumber) <-- NonN(Modulo(Rationalize(n),Rationalize(m)));5 # Modulo(n_IsRationalOrNumber,m_IsRationalOrNumber) <--[ Local(n1,n2,m1,m2); n1:=Numerator(n); n2:=Denominator(n); m1:=Numerator(m); m2:=Denominator(m); Modulo(n1*m2,m1*n2)/(n2*m2);];6 # Modulo(n_IsList,m_IsList) <-- Map(\"Modulo\",{n,m});7 # Modulo(n_IsList,_m) <-- Map(\"Modulo\",{n,FillList(m,Length(n))});30 # Modulo(n_CanBeUni,m_CanBeUni) <--[ Local(vars); vars:=VarList(n+m); NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];0 # Modulo(n_IsUniVar,m_IsUniVar)_(Degree(n) <? Degree(m)) <-- n;1 # Modulo(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) >=? Degree(m)) <--[ UniVariate(n[1],0, UniDivide(Concat(ZeroVector(n[2]),n[3]), Concat(ZeroVector(m[2]),m[3]))[2]);];10 # Modulo(n_CanBeUni, m_CanBeUni, vars_IsList)_(Length(vars)=?1) <--[ NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Sign(n_IsPositiveNumber) <-- 1;10 # Sign(n_IsZero) <-- 0;20 # Sign(n_IsNumber) <-- -1;15 # Sign(n_Infinity?)_(n <? 0) <-- -1;15 # Sign(n_Infinity?)_(n >? 0) <-- 1;15 # Sign(n_IsNumber/m_IsNumber) <-- Sign(n)*Sign(m);20 # Sign(n_IsList) <-- MapSingle(\"Sign\",n);100 # Sign(_a)^n_Even? <-- 1;100 # Sign(_a)^n_IsOdd <-- Sign(a);";
        scriptMap.put("Sign",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Sqrt(0) <-- 0;0 # Sqrt(Infinity) <-- Infinity;0 # Sqrt(-Infinity) <-- Complex(0,Infinity);0 # Sqrt(Undefined) <-- Undefined;1 # Sqrt(x_IsPositiveInteger)_(IsInteger(SqrtN(x))) <-- SqrtN(x);2 # Sqrt(x_IsPositiveNumber)_InNumericMode() <-- SqrtN(x);2 # Sqrt(x_NegativeNumber?) <-- Complex(0,Sqrt(-x));3 # Sqrt(x_IsComplex)_InNumericMode() <-- x^(1/2);Sqrt(xlist_IsList) <-- MapSingle(\"Sqrt\",xlist);90 # (Sqrt(x_Constant?))_(NegativeNumber?(N(x))) <-- Complex(0,Sqrt(-x));400 # x_IsRationalOrNumber * Sqrt(y_IsRationalOrNumber) <-- Sign(x)*Sqrt(x^2*y);400 # Sqrt(y_IsRationalOrNumber) * x_IsRationalOrNumber <-- Sign(x)*Sqrt(x^2*y);400 # x_IsRationalOrNumber / Sqrt(y_IsRationalOrNumber) <-- Sign(x)*Sqrt(x^2/y);400 # Sqrt(y_IsRationalOrNumber) / x_IsRationalOrNumber <-- Sign(x)*Sqrt(y/(x^2));400 # Sqrt(y_IsRationalOrNumber) / Sqrt(x_IsRationalOrNumber) <-- Sqrt(y/x);400 # Sqrt(y_IsRationalOrNumber) * Sqrt(x_IsRationalOrNumber) <-- Sqrt(y*x);400 # Sqrt(x_IsInteger)_IsInteger(SqrtN(x)) <-- SqrtN(x);400 # Sqrt(x_IsInteger/y_IsInteger)_(IsInteger(SqrtN(x)) And? IsInteger(SqrtN(y))) <-- SqrtN(x)/SqrtN(y);";
        scriptMap.put("Sqrt",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # Undefined <? _x <-- False;1 # Undefined <=? _x <-- False;1 # Undefined >? _x <-- False;1 # Undefined >=? _x <-- False;1 # _x <? Undefined <-- False;1 # _x <=? Undefined <-- False;1 # _x >? Undefined <-- False;1 # _x >=? Undefined <-- False;5 # (n_IsNumber <? m_IsNumber) <-- IsLessThan(n-m,0);LocalSymbols(nNum,mNum)[ 10 # (_n <? _m)_[nNum:=N(Eval(n)); mNum:=N(Eval(m));IsNumber(nNum) And? IsNumber(mNum);] <-- IsLessThan(nNum-mNum,0);];20 # (Infinity <? _n)_(Not?(Infinity?(n))) <-- False;20 # (-Infinity <? _n)_(Not?(Infinity?(n))) <-- True;20 # (_n <? Infinity)_(Not?(Infinity?(n))) <-- True;20 # (_n <? -Infinity)_(Not?(Infinity?(n))) <-- False;30 # (_n1/_n2) <? 0 <-- (n1 <? 0) !=? (n2 <? 0);30 # (_n1*_n2) <? 0 <-- (n1 <? 0) !=? (n2 <? 0);30 # ((_n1+_n2) <? 0)_((n1 <? 0) And? (n2 <? 0)) <-- True;30 # ((_n1+_n2) <? 0)_((n1 >? 0) And? (n2 >? 0)) <-- False;30 # _x^a_IsOdd <? 0 <-- x <? 0;30 # _x^a_Even? <? 0 <-- False; 40 # (Sqrt(_x))_(x >? 0) <? 0 <-- False;40 # (Sin(_x) <? 0)_(Not?(Even?(N(x/Pi))) And? Even?(N(Floor(x/Pi)))) <-- False;40 # (Sin(_x) <? 0)_(Not?(IsOdd (N(x/Pi))) And? IsOdd (N(Floor(x/Pi)))) <-- True;40 # Cos(_x) <? 0 <-- Sin(Pi/2-x) <? 0;40 # (Tan(_x) <? 0)_(Not?(Even?(N(2*x/Pi))) And? Even?(N(Floor(2*x/Pi)))) <-- False;40 # (Tan(_x) <? 0)_(Not?(IsOdd (N(2*x/Pi))) And? IsOdd (N(Floor(2*x/Pi)))) <-- True;40 # (Complex(_a,_b) <? 0)_(b!=?0) <-- False;40 # (Complex(_a,_b) >=? 0)_(b!=?0) <-- False;40 # (Sqrt(_x))_(x <? 0) <? 0 <-- False;40 # (Sqrt(_x))_(x <? 0) >=? 0 <-- False;50 # -(_x) <? 0 <-- Not?((x<?0) Or? (x=?0));50 # _n >? _m <-- m <? n;50 # _n <=? _m <-- m >=? n;50 # _n >=? _m <-- Not?(n<?m);Function(\"!=?\",{aLeft,aRight}) Not?(aLeft=?aRight);";
        scriptMap.put("<?",scriptString);
        scriptMap.put(">?",scriptString);
        scriptMap.put("<=?",scriptString);
        scriptMap.put(">=?",scriptString);
