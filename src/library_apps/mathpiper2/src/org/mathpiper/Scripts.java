...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # Denominator(_x / _y) <-- y;1 # Denominator(-_x/ _y) <-- y;2 # Denominator(x_IsNumber) <-- 1;";
        scriptMap.put("Denominator",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsNonObject(Object(_x)) <-- False;20 # IsNonObject(_x) <-- True;";
        scriptMap.put("IsNonObject",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # Numerator(_x / _y) <-- x;1 # Numerator(-_x/ _y) <-- -x;2 # Numerator(x_IsNumber) <-- x;";
        scriptMap.put("Numerator",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "a_IsNonNegativeInteger & b_IsNonNegativeInteger <-- BitAnd(a,b);";
        scriptMap.put("&",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"==\",{left,right});";
        scriptMap.put("==",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "a_IsNonNegativeInteger % b_IsPositiveInteger <-- Modulo(a,b);";
        scriptMap.put("%",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "a_IsNonNegativeInteger | b_IsNonNegativeInteger <-- BitOr(a,b);";
        scriptMap.put("|",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # x_IsNumber * y_IsNumber <-- MultiplyN(x,y);100 # 1 * _x <-- x;100 # _x * 1 <-- x;100 # (_f * _x)_(f=? -1) <-- -x;100 # (_x * _f)_(f=? -1) <-- -x;105 # (f_IsNegativeNumber * _x) <-- -(-f)*x;105 # (_x * f_IsNegativeNumber) <-- -(-f)*x;95 # x_IsMatrix * y_IsMatrix <--[ Local(i,j,k,row,result); result:=ZeroMatrix(Length(x),Length(y[1])); For(i:=1,i<=?Length(x),i++) For(j:=1,j<=?Length(y),j++) For(k:=1,k<=?Length(y[1]),k++) [ row:=result[i]; row[k]:= row[k]+x[i][j]*y[j][k]; ]; result;];96 # x_IsMatrix * y_IsList <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,Dot(x[i], y)); ]; result;];97 # (x_IsList * y_IsNonObject)_Not?(IsList(y)) <-- y*x;98 # (x_IsNonObject * y_IsList)_Not?(IsList(x)) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x * y[i]); ]; result;];50 # _x * Undefined <-- Undefined;50 # Undefined * _y <-- Undefined;100 # 0 * y_IsInfinity <-- Undefined;100 # x_IsInfinity * 0 <-- Undefined;101 # 0 * (_x) <-- 0;101 # (_x) * 0 <-- 0;100 # x_IsNumber * (y_IsNumber * _z) <-- (x*y)*z;100 # x_IsNumber * (_y * z_IsNumber) <-- (x*z)*y;100 # (_x * _y) * _y <-- x * y^2;100 # (_x * _y) * _x <-- y * x^2;100 # _y * (_x * _y) <-- x * y^2;100 # _x * (_x * _y) <-- y * x^2;100 # _x * (_y / _z) <-- (x*y)/z;100 # (_y / _z) * _x <-- (x*y)/z;100 # (_x * y_IsNumber)_Not?(IsNumber(x)) <-- y*x;100 # (_x) * (_x) ^ (n_Constant?) <-- x^(n+1);100 # (_x) ^ (n_Constant?) * (_x) <-- x^(n+1);100 # (_x * _y)* _x ^ n_Constant? <-- y * x^(n+1);100 # (_y * _x)* _x ^ n_Constant? <-- y * x^(n+1);100 # Sqrt(_x) * (_x) ^ (n_Constant?) <-- x^(n+1/2);100 # (_x) ^ (n_Constant?) * Sqrt(_x) <-- x^(n+1/2);100 # Sqrt(_x) * (_x) <-- x^(3/2);100 # (_x) * Sqrt(_x) <-- x^(3/2);105 # x_IsNumber * -(_y) <-- (-x)*y;105 # (-(_x)) * (y_IsNumber) <-- (-y)*x;106 # _x * -(_y) <-- -(x*y);106 # (- _x) * _y <-- -(x*y);107 # -( (-(_x))/(_y)) <-- x/y;107 # -( (_x)/(-(_y))) <-- x/y;250 # x_IsNumber * y_IsInfinity <-- Sign(x)*y;250 # x_IsInfinity * y_IsNumber <-- Sign(y)*x;230 # (aLeft_IsList * aRight_IsList)_(Length(aLeft)=?Length(aRight)) <-- Map(\"*\",{aLeft,aRight});242 # (x_IsInteger / y_IsInteger) * (v_IsInteger / w_IsInteger) <-- (x*v)/(y*w);243 # x_IsInteger * (y_IsInteger / z_IsInteger) <-- (x*y)/z;243 # (y_IsInteger / z_IsInteger) * x_IsInteger <-- (x*y)/z;400 # (_x) * (_x) <-- x^2;";
        scriptMap.put("*",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # _x ^ (1/2) <-- Sqrt(x);50 # (x_IsPositiveNumber ^ (1/2))_IsInteger(SqrtN(x)) <-- SqrtN(x);58 # 1 ^ n_IsInfinity <-- Undefined;59 # _x ^ 1 <-- x;59 # 1 ^ _n <-- 1;59 # x_IsZero ^ y_IsZero <-- Undefined;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n>?0) <-- 0;60 # (x_IsZero ^ n_IsRationalOrNumber)_(n<?0) <-- Infinity;59 # _x ^ Undefined <-- Undefined;59 # Undefined ^ _x <-- Undefined;61 # Infinity ^ (y_IsNegativeNumber) <-- 0;61 # (-Infinity) ^ (y_IsNegativeNumber) <-- 0;61 # x_IsPositiveNumber ^ y_IsPositiveInteger <-- MathIntPower(x,y);61 # x_IsPositiveNumber ^ y_IsNegativeInteger <-- 1/MathIntPower(x,-y);65 # (x_IsPositiveNumber ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));90 # (-_x)^m_Even? <-- x^m;91 # (x_Constant? ^ (m_IsOdd / p_IsOdd))_(IsNegativeNumber(Re(N(Eval(x))))) <-- -((-x)^(m/p));92 # (x_IsNegativeNumber ^ y_IsNumber)_InNumericMode() <-- Exp(y*Ln(x));70 # (_x ^ m_IsRationalOrNumber) ^ n_IsRationalOrNumber <-- x^(n*m);80 # (x_IsNumber/y_IsNumber) ^ n_IsPositiveInteger <-- x^n/y^n;80 # (x_IsNumber/y_IsNumber) ^ n_IsNegativeInteger <-- y^(-n)/x^(-n);80 # x_IsNegativeNumber ^ n_Even? <-- (-x)^n;80 # x_IsNegativeNumber ^ n_IsOdd <-- -((-x)^n);100 # ((_x)*(_x ^ _m)) <-- x^(m+1);100 # ((_x ^ _m)*(_x)) <-- x^(m+1);100 # ((_x ^ _n)*(_x ^ _m)) <-- x^(m+n);100 # ((x_IsNumber)^(n_IsInteger/(_m)))_(n>?1) <-- MathIntPower(x,n)^(1/m);100 # Sqrt(_n)^(m_Even?) <-- n^(m/2);100 # Abs(_a)^n_Even? <-- a^n;100 # Abs(_a)^n_IsOdd <-- Sign(a)*a^n;200 # x_IsMatrix ^ n_IsPositiveInteger <-- x*(x^(n-1));204 # (xlist_IsList ^ nlist_IsList)_(Length(xlist)=?Length(nlist)) <-- Map(\"^\",{xlist,nlist});205 # (xlist_IsList ^ n_Constant?)_(Not?(IsList(n))) <-- Map({{xx},xx^n},{xlist});206 # (_x ^ n_IsList)_(Not?(IsList(x))) <-- Map({{xx},x^xx},{n});249 # x_IsInfinity ^ 0 <-- Undefined;250 # Infinity ^ (_n) <-- Infinity;250 # Infinity ^ (_x_IsComplex) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(Even?(n)) <-- Infinity;250 # ((-Infinity) ^ (n_IsNumber))_(IsOdd(n)) <-- -Infinity;250 # (x_IsNumber ^ Infinity)_(x>? -1 And? x <? 1) <-- 0;250 # (x_IsNumber ^ Infinity)_(x>? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? 1) <-- Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) <? -1) <-- -Infinity;250 # (x_IsComplex ^ Infinity)_(Magnitude(x) >? -1 And? Magnitude(x) <? 1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? -1 And? x <? 1) <-- Infinity;250 # (x_IsNumber ^ -Infinity)_(x<? -1) <-- 0;250 # (x_IsNumber ^ -Infinity)_(x>? 1) <-- 0;255 # (x_IsComplex ^ Infinity)_(Abs(x) =? 1) <-- Undefined;255 # (x_IsComplex ^ -Infinity)_(Abs(x) =? 1) <-- Undefined;400 # _x ^ 0 <-- 1;";
        scriptMap.put("^",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "51 # -Undefined <-- Undefined;54 # - (- _x) <-- x;55 # (- (x_IsNumber)) <-- SubtractN(0,x);100 # _x - n_Constant?*(_x) <-- (1-n)*x;100 # n_Constant?*(_x) - _x <-- (n-1)*x;110 # - (_x - _y) <-- y-x;111 # - (x_IsNumber / _y) <-- (-x)/y; LocalSymbols(x)[ 200 # - (x_IsList) <-- MapSingle(\"-\",x);];50 # x_IsNumber - y_IsNumber <-- SubtractN(x,y);50 # x_IsNumber - y_IsNumber <-- SubtractN(x,y);60 # Infinity - Infinity <-- Undefined;100 # 0 - _x <-- -x;100 # _x - 0 <-- x;100 # _x - _x <-- 0;110 # _x - (- _y) <-- x + y;110 # _x - (y_IsNegativeNumber) <-- x + (-y);111 # (_x + _y)- _x <-- y;111 # (_x + _y)- _y <-- x;112 # _x - (_x + _y) <-- - y;112 # _y - (_x + _y) <-- - x;113 # (- _x) - _y <-- -(x+y);113 # (x_IsNegativeNumber) - _y <-- -((-x)+y);113 # (x_IsNegativeNumber)/_y - _z <-- -((-x)/y+z);LocalSymbols(x,y,xarg,yarg)[ 10 # ((x_IsList) - (y_IsList))_(Length(x)=?Length(y)) <-- [ Map({{xarg,yarg},xarg-yarg},{x,y}); ];];240 # (x_IsList - y_IsNonObject)_Not?(IsList(y)) <-- -(y-x);241 # (x_IsNonObject - y_IsList)_Not?(IsList(x)) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x - y[i]); ]; result;];250 # z_IsInfinity - Complex(_x,_y) <-- Complex(-x+z,-y);250 # Complex(_x,_y) - z_IsInfinity <-- Complex(x-z,y);251 # z_IsInfinity - _x <-- z;251 # _x - z_IsInfinity <-- -z;250 # Undefined - _y <-- Undefined;250 # _x - Undefined <-- Undefined;210 # x_IsNumber - (y_IsNumber / z_IsNumber) <--(x*z-y)/z;210 # (y_IsNumber / z_IsNumber) - x_IsNumber <--(y-x*z)/z;210 # (x_IsNumber / v_IsNumber) - (y_IsNumber / z_IsNumber) <--(x*z-y*v)/(v*z);";
        scriptMap.put("-",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # + _x <-- x;50 # x_IsNumber + y_IsNumber <-- AddN(x,y);100 # 0 + _x <-- x;100 # _x + 0 <-- x;100 # _x + _x <-- 2*x;100 # _x + n_Constant?*(_x) <-- (n+1)*x;100 # n_Constant?*(_x) + _x <-- (n+1)*x;101 # _x + - _y <-- x-y;101 # _x + (- _y)/(_z) <-- x-(y/z);101 # (- _y)/(_z) + _x <-- x-(y/z);101 # (- _x) + _y <-- y-x;102 # _x + y_IsNegativeNumber <-- x-(-y);102 # _x + y_IsNegativeNumber * _z <-- x-((-y)*z);102 # _x + (y_IsNegativeNumber)/(_z) <-- x-((-y)/z);102 # (y_IsNegativeNumber)/(_z) + _x <-- x-((-y)/z);102 # (x_IsNegativeNumber) + _y <-- y-(-x);150 # _n1 / _d + _n2 / _d <-- (n1+n2)/d;200 # (x_IsNumber + _y)_Not?(IsNumber(y)) <-- y+x;200 # ((_y + x_IsNumber) + _z)_Not?(IsNumber(y) Or? IsNumber(z)) <-- (y+z)+x;200 # ((x_IsNumber + _y) + z_IsNumber)_Not?(IsNumber(y)) <-- y+(x+z);200 # ((_x + y_IsNumber) + z_IsNumber)_Not?(IsNumber(x)) <-- x+(y+z);210 # x_IsNumber + (y_IsNumber / z_IsNumber) <--(x*z+y)/z;210 # (y_IsNumber / z_IsNumber) + x_IsNumber <--(x*z+y)/z;210 # (x_IsNumber / v_IsNumber) + (y_IsNumber / z_IsNumber) <--(x*z+y*v)/(v*z);220 # (xlist_IsList + ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"+\",{xlist,ylist});SumListSide(_x, y_IsList) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x + y[i]); ]; result;];240 # (x_IsList + _y)_Not?(IsList(y)) <-- SumListSide(y,x);241 # (_x + y_IsList)_Not?(IsList(x)) <-- SumListSide(x,y);250 # z_IsInfinity + Complex(_x,_y) <-- Complex(x+z,y);250 # Complex(_x,_y) + z_IsInfinity <-- Complex(x+z,y);251 # z_IsInfinity + _x <-- z;251 # _x + z_IsInfinity <-- z;250 # Undefined + _y <-- Undefined;250 # _x + Undefined <-- Undefined;";
        scriptMap.put("+",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # 0 / 0 <-- Undefined;52 # x_IsPositiveNumber / 0 <-- Infinity;52 # x_IsNegativeNumber / 0 <-- -Infinity;55 # (_x / y_IsNumber)_(IsZero(y)) <-- Undefined;55 # 0 / _x <-- 0;56 # (x_IsNonZeroInteger / y_IsNonZeroInteger)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/QuotientN(y,gcd); ];57 # ((x_IsNonZeroInteger * _expr) / y_IsNonZeroInteger)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*expr)/QuotientN(y,gcd); ];57 # ((x_IsNonZeroInteger) / (y_IsNonZeroInteger * _expr))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/(QuotientN(y,gcd)*expr); ];57 # ((x_IsNonZeroInteger * _p) / (y_IsNonZeroInteger * _q))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*p)/(QuotientN(y,gcd)*q); ];60 # (x_IsDecimal / y_IsNumber) <-- DivideN(x,y);60 # (x_IsNumber / y_IsDecimal) <-- DivideN(x,y); 60 # (x_IsNumber / y_IsNumber)_(InNumericMode()) <-- DivideN(x,y);90 # x_IsInfinity / y_IsInfinity <-- Undefined;95 # x_IsInfinity / y_IsNumber <-- Sign(y)*x;95 # x_IsInfinity / y_IsComplex <-- Infinity;90 # Undefined / _y <-- Undefined;90 # _y / Undefined <-- Undefined;100 # _x / _x <-- 1;100 # _x / 1 <-- x;100 # (_x / y_IsNegativeNumber) <-- -x/(-y);100 # (_x / - _y) <-- -x/y;150 # (_x) / (_x) ^ (n_Constant?) <-- x^(1-n);150 # (_x) ^ (n_Constant?) * (_x) <-- x^(n-1);150 # Sqrt(_x) / (_x) ^ (n_Constant?) <-- x^(1/2-n);150 # (_x) ^ (n_Constant?) / Sqrt(_x) <-- x^(n-1/2);150 # (_x) / Sqrt(_x) <-- Sqrt(x);200 # (_x / _y)/ _z <-- x/(y*z);230 # _x / (_y / _z) <-- (x*z)/y;240 # (xlist_IsList / ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"/\",{xlist,ylist});250 # (x_IsList / _y)_(Not?(IsList(y))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,x[i] / y); ]; result;];250 # (_x / y_IsList)_(Not?(IsList(x))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x/y[i]); ]; result;];250 # _x / Infinity <-- 0;250 # _x / (-Infinity) <-- 0;400 # 0 / _x <-- 0;";
        scriptMap.put("/",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(StandardOperatorsLoad,{});Rule(StandardOperatorsLoad, 0, 1, True)[  Infix(\"=?\",90); Infix(\"And?\",1000); RightAssociativeSet(\"And?\"); Infix(\"Or?\", 1010); Prefix(\"Not?\", 100); Infix(\"<?\",90); Infix(\">?\",90); Infix(\"<=?\",90); Infix(\">=?\",90); Infix(\"!=?\",90); Infix(\":=\",10000); RightAssociativeSet(\":=\"); Infix(\"+\",70); Infix(\"-\",70); RightPrecedenceSet(\"-\",40); Infix(\"/\",30); Infix(\"*\",40); Infix(\"^\",20); LeftPrecedenceSet(\"^\",19);  RightAssociativeSet(\"^\"); Prefix(\"+\",50); Prefix(\"-\",50); RightPrecedenceSet(\"-\",40); Bodied(\"For\",60000); Bodied(\"Until\",60000); Postfix(\"++\",5); Postfix(\"--\",5); Bodied(\"ForEach\",60000); Infix(\"<<\",10); Infix(\">>\",10); Bodied(\"Differentiate\",60000); Bodied(\"Deriv\",60000); Infix(\"X\",30); Infix(\".\",30); Infix(\"o\",30); Postfix(\"!\", 30); Postfix(\"!!\", 30); Infix(\"***\", 50); Bodied(\"Integrate\",60000); Bodied(\"Limit\",60000); Bodied(\"EchoTime\", 60000); Bodied(\"Repeat\", 60000); Infix(\"->\",600);  Infix(\":\",70); RightAssociativeSet(\":\"); Infix(\"@\",600); Infix(\"/@\",600); Infix(\"..\",600); Bodied(\"Taylor\",60000); Bodied(\"Taylor1\",60000); Bodied(\"Taylor2\",60000); Bodied(\"Taylor3\",60000); Bodied(\"InverseTaylor\",60000); Infix(\"<--\",10000); Infix(\"#\",9900); Bodied(\"TSum\",60000); Bodied(\"TExplicitSum\",60000); Bodied(\"TD\",5);   Infix(\"==\",90); Infix(\"!==\",90);  Infix(\"=>\",10000);  Bodied(\"if\",5); Infix(\"else\",60000); RightAssociativeSet(\"else\");  Infix(\"&\",50); Infix(\"|\",50); Infix(\"%\",50);  Infix(\"/:\",20000); Infix(\"/::\",20000); Infix(\"<-\",10000);  Infix(\"<>\", PrecedenceGet(\"=?\")); Infix(\"<=>\", PrecedenceGet(\"=?\"));  Infix(\"Where\", 11000); Infix(\"AddTo\", 2000); Bodied(\"Function\",60000); Bodied(\"Macro\",60000); Bodied(Assert, 60000);  Bodied(\"Defun\",0); Bodied(\"TemplateFunction\",60000); Bodied(\"Taylor3'TaylorCoefs\",0); Infix(\":*:\",3);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Assoc\",{key,list}) Builtin'Assoc(key,list);";
        scriptMap.put("Assoc",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(hash, key, element, hash'expr)[10 # AssocDelete(hash_IsList, element_IsList) <--[ Local(index); index := Find(hash, element); If( index >? 0, DestructiveDelete(hash, index) ); index>?0; ];20 # AssocDelete(hash_IsList, key_IsString) <--[ AssocDelete(hash, Builtin'Assoc(key, hash));];30 # AssocDelete(hash_IsList, Empty) <-- False;]; ";
        scriptMap.put("AssocDelete",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "AssocIndices(associndiceslist_IsList) <-- DestructiveReverse(MapSingle(\"First\",associndiceslist));";
        scriptMap.put("AssocIndices",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "AssocValues(assocvalueslist_IsList) <-- DestructiveReverse(MapSingle(Lambda({x},First(Rest(x))),assocvalueslist));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"ExpN\", {x}) If(IsEqual(x,0),1, If(IsLessThan(x,0),DivideN(1, ExpN(MathNegate(x))), If(IsGreaterThan(x,1), MathExpDoubling(MathExpTaylor0(MathMul2Exp(x,MathNegate(MathBitCount(x)))), MathBitCount(x)), MathExpTaylor0(x) )));";
        scriptMap.put("ExpN",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathExpDoubling\", {value, n})[ Local(shift, result); Bind(shift, n); Bind(result, value); While (IsGreaterThan(shift,0))  [ Bind(result, MultiplyN(result, result)); Bind(shift, AddN(shift,MathNegate(1))); ]; result;];";
        scriptMap.put("MathExpDoubling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathExpTaylor0\",{x})[ Check(x<=?1 And? x>=?0,\"\",\" Argument must be between 0 and 1 inclusive.\"); Local(i,aResult,term,eps);     Bind(i,0);  Bind(aResult,1.0);  Bind(term,1.0); Bind(eps,MathIntPower(10,MathNegate(BuiltinPrecisionGet())));   While(IsGreaterThan(AbsN(term),eps)) [  Bind(i,AddN(i,1));  Bind(term,DivideN(MultiplyN(term,x),i));  Bind(aResult,AddN(aResult,term)); ]; aResult;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathIntPower\", {x,y}) If(IsEqual(x,0),0,If(IsEqual(x,1),1, If(IsInteger(y),If(IsLessThan(y,0),  DivideN(1, PositiveIntPower(AddN(x,0.),MathNegate(y))),  PositiveIntPower(x,y) ),  False) ));";
        scriptMap.put("MathIntPower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathMul2Exp\", {x,n})  If(IsGreaterThan(n,0), MultiplyN(x, MathIntPower(2,n)), DivideN(x, MathIntPower(2,MathNegate(n))));";
        scriptMap.put("MathMul2Exp",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathPi\",{})[    Local(initialPrec,curPrec,result,aPrecision); Bind(aPrecision,BuiltinPrecisionGet()); Bind(initialPrec, aPrecision);  Bind(curPrec, 40);  Bind(result, 3.141592653589793238462643383279502884197169399);   While (IsGreaterThan(initialPrec, MultiplyN(curPrec,3))) [ Bind(initialPrec, FloorN(DivideN(AddN(initialPrec,2),3))); ]; Bind(curPrec, initialPrec); While (Not?(IsGreaterThan(curPrec, aPrecision))) [   BuiltinPrecisionSet(curPrec); Bind(result,AddN(result,SinN(result)));    If (IsEqual(curPrec, aPrecision),  [ Bind(curPrec, AddN(aPrecision,1));  ], [ Bind(curPrec, MultiplyN(curPrec,3));   If (IsGreaterThan(curPrec, aPrecision), [ Bind(curPrec, aPrecision);  ]); ]); ]; BuiltinPrecisionSet(aPrecision); result;];";
        scriptMap.put("MathPi",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PositiveIntPower\", {x,n})[ Local(result,unit); If(IsLessThan(n,0), False, [ Bind(unit,1);  Bind(result, unit); If(IsEqual(n,0),unit, If(IsEqual(n,1),x, [ While(IsGreaterThan(n,0)) [ If( IsEqual(BitAnd(n,1), 1), Bind(result, MultiplyN(result,x)) ); Bind(x, MultiplyN(x,x)); Bind(n,ShiftRight(n,1)); ]; result; ] ) ); ]);];";
        scriptMap.put("PositiveIntPower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PowerN\", {x,y}) If(IsEqual(x,0),0, If(IsEqual(x,1),1, If(IsInteger(y), MathIntPower(x,y), False) ));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Easter(year) := [ Check(IsPositiveInteger(year), \"Argument\", \"The argument must be a positive integer\"); Local(a,b,c,d,e,f,g,h,i,k,L,m,month,day); a := Modulo(year, 19); b := Quotient(year, 100); c := Modulo(year, 100); d := Quotient(b, 4); e := Modulo(b, 4); f := Quotient(b + 8, 25); g := Quotient(b - f + 1, 3); h := Modulo(19*a + b - d - g + 15, 30); i := Quotient(c, 4); k := Modulo(c, 4); L := Modulo(32 + 2*e + 2*i - h - k, 7); m := Quotient(a + 11*h + 22*L, 451); month := Quotient(h + L - 7*m + 114, 31); day := Modulo(h + L - 7*m + 114, 31) + 1; { month, day };];";
        scriptMap.put("Easter",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # BinomialCoefficient(0,0) <-- 1;10 # BinomialCoefficient(n_IsPositiveInteger,m_IsNonNegativeInteger)_(2*m <=? n) <-- ((n-m+1) *** n) / m!;15 # BinomialCoefficient(n_IsPositiveInteger,m_IsNonNegativeInteger)_(2*m >? n And? m <=? n) <-- BinomialCoefficient(n, n-m);20 # BinomialCoefficient(n_IsInteger,m_IsInteger) <-- 0;Combinations(n,m) := BinomialCoefficient(n,m);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Arg(Complex(Cos(_x),Sin(_x))) <-- x;10 # Arg(x_IsZero) <-- Undefined;15 # Arg(x_IsPositiveReal) <-- 0;15 # Arg(x_IsNegativeReal) <-- Pi;20 # Arg(Complex(r_IsZero,i_Constant?)) <-- Sign(i)*Pi/2;30 # Arg(Complex(r_IsPositiveReal,i_Constant?)) <-- ArcTan(i/r);40 # Arg(Complex(r_IsNegativeReal,i_IsPositiveReal)) <-- Pi+ArcTan(i/r);50 # Arg(Complex(r_IsNegativeReal,i_IsNegativeReal)) <-- ArcTan(i/r)-Pi;";
        scriptMap.put("Arg",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Complex(_r,i_IsZero) <-- r;2 # Complex(Complex(_r1,_i1),_i2) <-- Complex(r1,i1+i2);2 # Complex(_r1,Complex(_r2,_i2)) <-- Complex(r1-i2,r2);6 # Complex(Undefined,_x) <-- Undefined;6 # Complex(_x,Undefined) <-- Undefined;110 # Complex(_r1,_i1) + Complex(_r2,_i2) <-- Complex(r1+r2,i1+i2);300 # Complex(_r,_i) + x_Constant? <-- Complex(r+x,i);300 # x_Constant? + Complex(_r,_i) <-- Complex(r+x,i);110 # - Complex(_r,_i) <-- Complex(-r,-i);300 # Complex(_r,_i) - x_Constant? <-- Complex(r-x,i);300 # x_Constant? - Complex(_r,_i) <-- Complex((-r)+x,-i);111 # Complex(_r1,_i1) - Complex(_r2,_i2) <-- Complex(r1-r2,i1-i2);110 # Complex(_r1,_i1) * Complex(_r2,_i2) <-- Complex(r1*r2-i1*i2,r1*i2+r2*i1);250 # Complex(r_IsZero,_i) * x_IsInfinity <-- Complex(0,i*x);250 # Complex(_r,i_IsZero) * x_IsInfinity <-- Complex(r*x,0);251 # Complex(_r,_i) * x_IsInfinity <-- Complex(r*x,i*x);250 # x_IsInfinity * Complex(r_IsZero,_i) <-- Complex(0,i*x);250 # x_IsInfinity * Complex(_r,i_IsZero) <-- Complex(r*x,0);251 # x_IsInfinity * Complex(_r,_i) <-- Complex(r*x,i*x);300 # Complex(_r,_i) * y_Constant? <-- Complex(r*y,i*y);300 # y_Constant? * Complex(_r,_i) <-- Complex(r*y,i*y);330 # Complex(_r,_i) * (y_Constant? / _z) <-- (Complex(r*y,i*y))/z;330 # (y_Constant? / _z) * Complex(_r,_i) <-- (Complex(r*y,i*y))/z;110 # x_Constant? / Complex(_r,_i) <-- (x*Conjugate(Complex(r,i)))/(r^2+i^2);300 # Complex(_r,_i) / y_Constant? <-- Complex(r/y,i/y);110 # (_x ^ Complex(_r,_i)) <-- Exp(Complex(r,i)*Ln(x));110 # Sqrt(Complex(_r,_i)) <-- Exp(Ln(Complex(r,i))/2);110 # (Complex(_r,_i) ^ x_IsRationalOrNumber)_(Not?(IsInteger(x))) <-- Exp(x*Ln(Complex(r,i)));123 # Complex(_r, _i) ^ n_IsNegativeInteger <-- 1/Complex(r, i)^(-n);124 # Complex(_r, _i) ^ (p_IsZero) <-- 1; 125 # Complex(_r, _i) ^ n_IsPositiveInteger <--[  Local(result, x); x:=Complex(r,i); result:=1; While(n >? 0) [ if ((n&1) =? 1) [ result := result*x; ]; x := x*x; n := n>>1; ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # II^n_IsNegativeInteger <-- (-II)^(-n);20 # (II^_n)_(Even?(n) =? True) <-- (-1)^(n>>1);20 # (II^_n)_(IsOdd(n) =? True) <-- II*(-1)^(n>>1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # IsComplex(x_IsRationalOrNumber) <-- True;2 # IsComplex(Complex(_r,_i)) <-- True;3 # IsComplex(_x) <-- False;";
        scriptMap.put("IsComplex",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsComplexII(_c) <-- (ImII(c) !=? 0);";
        scriptMap.put("IsComplexII",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNotComplex(x) := Not?(IsComplex(x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(complexReduce) [ Bind(complexReduce, Hold( { Exp(x_IsComplexII) <- Exp(ReII(x))*(Cos(ImII(x))+II*Sin(ImII(x))) })); NN(_c) <-- [ Local(result); c := (c /:: complexReduce); result := Coef(Expand(c,II),II,{0,1}); result; ];]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SetGlobalLazyVariable(I,Complex(0,1));LocalSymbols(CacheOfConstantsN) [Rulebase(\"CachedConstant\", {C'cache, C'atom, C'func});UnFence(\"CachedConstant\", 3); HoldArgument(\"CachedConstant\", C'func);HoldArgument(\"CachedConstant\", C'cache); Rule(\"CachedConstant\", 3, 10, And?(IsAtom(C'atom), IsFunction(C'func)))[ Local(C'name,C'functionName); Bind(C'name, ToString(C'atom));  Bind(C'functionName,ConcatStrings(\"Internal'\",C'name)); If(  IsAtom(Eval(C'cache)), MacroBind(Eval(C'cache), {}) );  If( IsEqual(Builtin'Assoc(C'name, Eval(C'cache)), Empty),  [  MacroUnbind(C'atom);  DestructiveInsert(Eval(C'cache), 1, {C'name, 0, 0});   MacroRulebase(C'functionName, {}); `( Rule(@C'functionName, 0, 1024, True) [ Local(new'prec, new'C, cached'C); Bind(new'prec, BuiltinPrecisionGet());   Bind(cached'C, Builtin'Assoc(@C'name, @C'cache)); If( MathNth(cached'C, 2) !=? new'prec, [  If(IsEqual(InVerboseMode(),True), Echo(\"CachedConstant: Info: constant \", @C'name, \" is being recalculated at precision \", new'prec)); Bind(new'C, RoundTo(Eval(@C'func),new'prec)); DestructiveReplace(cached'C, 2, new'prec); DestructiveReplace(cached'C, 3, new'C); new'C; ],  MathNth(cached'C, 3) ); ]);   ],  Echo(\"CachedConstant: Warning: constant \", C'atom, \" already defined\") );];Rule(\"CachedConstant\", 3, 20, True) Echo(\"CachedConstant: Error: \", C'atom, \" must be an atom and \", C'func, \" must be a function.\");Function(\"AssignCachedConstantsN\", {})[ Local(var,fname); ForEach(var, AssocIndices(CacheOfConstantsN)) [ MacroUnbind(ToAtom(var)); Bind(fname,ConcatStrings(\"Internal'\",var)); Bind(var,ToAtom(var));  `SetGlobalLazyVariable((@var), ListToFunction({ToAtom(fname)})); ];];UnFence(\"AssignCachedConstantsN\", 0);Function(\"ClearCachedConstantsN\", {})[ Local(c'entry); ForEach(c'entry, CacheOfConstantsN) MacroUnbind(ToAtom(c'entry[1]));];UnFence(\"ClearCachedConstantsN\", 0);CachedConstant(CacheOfConstantsN, Pi,[ Local(result,old'prec); Bind(old'prec,BuiltinPrecisionGet());If(IsEqual(InVerboseMode(),True), Echo(\"Recalculating Pi at precision \",old'prec+5)); BuiltinPrecisionSet(BuiltinPrecisionGet()+5); result := MathPi();If(IsEqual(InVerboseMode(),True),Echo(\"Switching back to precision \",old'prec)); BuiltinPrecisionSet(old'prec); result;]);CachedConstant(CacheOfConstantsN, gamma, GammaConstNum());CachedConstant(CacheOfConstantsN, GoldenRatio, N( (1+Sqrt(5))/2 ) );CachedConstant(CacheOfConstantsN, Catalan, CatalanConstNum() );]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"For\",{start,predicate,increment,body})[ Eval(start); While (IsEqual(Eval(predicate),True)) [ Eval(body); Eval(increment); ];];UnFence(\"For\",4);HoldArgumentNumber(\"For\",4,1);HoldArgumentNumber(\"For\",4,2);HoldArgumentNumber(\"For\",4,3);HoldArgumentNumber(\"For\",4,4);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DefMacroRulebase(\"Defun\",{func,args,body});Rule(\"Defun\",3,0,True)[ Local(nrargs); Bind(nrargs,Length(@args)); Retract(@func, `(@nrargs)); Rulebase(@func,@args); Local(fn,bd); Bind(fn,Hold(@func)); Bind(bd,Hold(@body)); `Rule(@fn, @nrargs, 0,True)(@bd);];";
        scriptMap.put("Defun",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Function\",{oper,args,body});Rule(\"Function\",3,2047, And?(IsGreaterThan(Length(args), 1), IsEqual( MathNth(args, Length(args)), ToAtom(\"...\") )))[ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); MacroRulebaseListed(oper,args); MacroRule(oper,Length(args),1025,True) body; ];Rule(\"Function\",3,2048,True)[ Retract(oper,Length(args)); MacroRulebase(oper,args); MacroRule(oper,Length(args),1025,True) body;];Rulebase(\"Function\",{oper});Rule(\"Function\",1,2047, And?(IsFunction(oper), IsGreaterThan(Length(oper), 1), IsEqual( MathNth(oper, Length(oper)), ToAtom(\"...\") )))[ Local(args); Bind(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebaseListed(Type(oper),args) );];Rule(\"Function\",1,2048, And?(IsFunction(oper)))[ Local(args); Bind(args,Rest(FunctionToList(oper))); If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebase(Type(oper),args) );];HoldArgument(\"Function\",oper);HoldArgument(\"Function\",args);HoldArgument(\"Function\",body);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Macro\",{oper,args,body});HoldArgument(\"Macro\",oper);HoldArgument(\"Macro\",args);HoldArgument(\"Macro\",body);Rule(\"Macro\",3,2047, And?(IsGreaterThan(Length(args), 1), IsEqual( MathNth(args, Length(args)), ToAtom(\"...\") )))[ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); `DefMacroRulebaseListed(@oper,@args); MacroRule(oper,Length(args),1025,True) body; ];Rule(\"Macro\",3,2048,True)[ Retract(oper,Length(args)); `DefMacroRulebase(@oper,@args); MacroRule(oper,Length(args),1025,True) body;];Rulebase(\"Macro\",{oper});Rule(\"Macro\",1,2047, And?(IsFunction(oper), IsGreaterThan(Length(oper), 1), IsEqual( MathNth(oper, Length(oper)), ToAtom(\"...\") )))[ Local(args,name); Bind(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  Bind(name,Type(oper)); If(RulebaseDefined(Type(oper),Length(args)), False,  `DefMacroRulebaseListed(@name,@args) );];Rule(\"Macro\",1,2048, And?(IsFunction(oper)))[ Local(args,name); Bind(args,Rest(FunctionToList(oper))); Bind(name,Type(oper)); If(RulebaseDefined(Type(oper),Length(args)), False,  [ `DefMacroRulebase(@name,@args); ] );];";
        scriptMap.put("Macro",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TemplateFunction\",{oper,args,body});HoldArgument(\"TemplateFunction\",oper);HoldArgument(\"TemplateFunction\",args);HoldArgument(\"TemplateFunction\",body);Rule(\"TemplateFunction\",3,2047,True)[ Retract(oper,Length(args)); Local(arglist); arglist:=FlatCopy(args); DestructiveAppend(arglist,{args,ListToFunction({Hold,body})}); arglist:=ApplyFast(\"LocalSymbols\",arglist); MacroRulebase(oper,arglist[1]); MacroRule(oper,Length(args),1025,True) arglist[2];];";
        scriptMap.put("TemplateFunction",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Unholdable\",{var});HoldArgument(\"Unholdable\",var);UnFence(\"Unholdable\",1);Rule(\"Unholdable\",1,10,IsEqual(Type(Eval(var)),\"Eval\"))[ MacroBind(var,Eval(Eval(var))); ];Rule(\"Unholdable\",1,20,True)[  True;];";
        scriptMap.put("Unholdable",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\":=\",{aLeftAssign,aRightAssign});UnFence(\":=\",2);HoldArgument(\":=\",aLeftAssign);HoldArgument(\":=\",aRightAssign);Rule(\":=\",2,0,IsAtom(aLeftAssign))[ Check( Not? IsNumber(aLeftAssign), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); MacroBind(aLeftAssign,Eval(aRightAssign)); Eval(aLeftAssign);];Rule(\":=\",2,0,IsList(aLeftAssign))[ Map(\":=\",{aLeftAssign,Eval(aRightAssign)});];Rulebase(\"AssignArray\",{setlistterm,setlistindex,setlistresult});UnFence(\"AssignArray\",3);Rule(\"AssignArray\",3,1,IsString(setlistindex))[ Local(item); item:=Assoc(setlistindex,setlistterm); If(item =? Empty, DestructiveInsert(setlistterm,1,{setlistindex,setlistresult}), DestructiveReplace(item,2,setlistresult) ); True;];Rule(\"AssignArray\",3,1, And?( IsEqual(IsGeneric(setlistterm),True), IsEqual(GenericTypeName(setlistterm),\"Array\") ) )[ ArraySet(setlistterm,setlistindex,setlistresult);];Rule(\"AssignArray\",3,2,True)[ DestructiveReplace(setlistterm ,setlistindex, setlistresult); True;];Rule(\":=\",2,10,IsFunction(aLeftAssign) And? (First(FunctionToList(aLeftAssign)) =? Nth))[ Local(frst,scnd); Local(lst); Bind(lst,(FunctionToList(aLeftAssign))); Bind(lst,Rest(lst)); Bind(frst, Eval(First(lst))); Bind(lst,Rest(lst)); Bind(scnd, Eval(First(lst))); AssignArray(frst,scnd,Eval(aRightAssign));];Rule(\":=\",2,30,IsFunction(aLeftAssign) And? Not?(IsEqual(aLeftAssign[0], ToAtom(\":=\"))) )[ Check( Not? IsEqual(aLeftAssign[0], ToAtom(\"/\")), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); Local(oper,args,arity); Bind(oper,ToString(aLeftAssign[0])); Bind(args,Rest(FunctionToList(aLeftAssign))); If( And?(IsGreaterThan(Length(args), 1), IsEqual( MathNth(args, Length(args)), ToAtom(\"...\") )),  [ DestructiveDelete(args,Length(args));  Bind(arity,Length(args)); Retract(oper,arity); MacroRulebaseListed(oper, args); ],  [ Bind(arity,Length(args)); Retract(oper,arity); MacroRulebase(oper, args); ] ); Unholdable(aRightAssign); MacroRule(oper,arity,1025,True) aRightAssign;];";
        scriptMap.put(":=",scriptString);
        scriptMap.put("AssignArray",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Curl\", {aFunc, aBasis});Rule(\"Curl\", 2, 1, Length(aBasis)=?Length(aFunc)) { Apply(\"Differentiate\",{aBasis[2],aFunc[3]})-Apply(\"Differentiate\",{aBasis[3],aFunc[2]}), Apply(\"Differentiate\",{aBasis[3],aFunc[1]})-Apply(\"Differentiate\",{aBasis[1],aFunc[3]}), Apply(\"Differentiate\",{aBasis[1],aFunc[2]})-Apply(\"Differentiate\",{aBasis[2],aFunc[1]}) };";
        scriptMap.put("Curl",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (Deriv(_var,1)_func) <-- Deriv(var)func;5 # (Deriv(_var,0)_func) <-- func;10 # (Deriv(_var,n_IsPositiveInteger)_func) <-- Deriv(var)Deriv(var,n-1)func;10 # (Deriv(_var,n_IsNegativeInteger)_func) <-- Check(0, \"Math\", \"Negative derivative\");0 # (Deriv(_var) (_var)) <-- 1;1 # (Deriv(_var)func_IsAtom) <-- 0;2 # (Deriv(_var)_x + _y) <-- (Deriv(var)x) + (Deriv(var)y);2 # (Deriv(_var)- (_x) ) <-- -Deriv(var)x;2 # (Deriv(_var)_x - _y) <-- (Deriv(var)x) - (Deriv(var)y);2 # (Deriv(_var)_x * _y) <-- (x*Deriv(var)y) + (Deriv(var)x)*y;2 # (Deriv(_var)Sin(_x)) <-- (Deriv(var)x)*Cos(x);2 # (Deriv(_var)Sinh(_x))<-- (Deriv(var)x)*Cosh(x);2 # (Deriv(_var)Cosh(_x))<-- (Deriv(var)x)*Sinh(x);2 # (Deriv(_var)Cos(_x)) <-- -(Deriv(var)x)*Sin(x);2 # (Deriv(_var)Csc(_x)) <-- -(Deriv(var)x)*Csc(x)*Cot(x);2 # (Deriv(_var)Csch(_x)) <-- -(Deriv(var)x)*Csch(x)*Coth(x);2 # (Deriv(_var)Sec(_x)) <-- (Deriv(var)x)*Sec(x)*Tan(x);2 # (Deriv(_var)Sech(_x)) <-- -(Deriv(var)x)*Sech(x)*Tanh(x);2 # (Deriv(_var)Cot(_x)) <-- -(Deriv(var)x)*Csc(x)^2;2 # (Deriv(_var)Coth(_x)) <-- (Deriv(var)x)*Csch(x)^2;2 # (Deriv(_var)Tan(_x)) <-- ((Deriv(var) x) / (Cos(x)^2));2 # (Deriv(_var)Tanh(_x)) <-- (Deriv(var)x)*Sech(x)^2;2 # (Deriv(_var)Exp(_x)) <-- (Deriv(var)x)*Exp(x);2 # (Deriv(_var)(_x / _y))_(IsFreeOf(var,y)) <-- (Deriv(var) x) / y;3 # (Deriv(_var)(_x / _y)) <-- (y* (Deriv(var) x) - x* (Deriv(var) y))/ (y^2);2 # (Deriv(_var)Ln(_x)) <-- ((Deriv(var) x) / x);2 # (Deriv(_var)(_x ^ _n))_(IsRationalOrNumber(n) Or? IsFreeOf(var, n)) <-- n * (Deriv(var) x) * (x ^ (n - 1));2 # (Deriv(_var)(Abs(_x))) <-- (x/Abs(x))*(Deriv(var)x);2 # (Deriv(_var)(Sign(_x))) <-- 0;2 # (Deriv(_var)(if(_cond)(_body))) <-- ListToFunction({ToAtom(\"if\"),cond,Deriv(var)body});2 # (Deriv(_var)((_left) else (_right))) <-- ListToFunction({ToAtom(\"else\"), (Deriv(var)left), (Deriv(var)right) } );3 # (Deriv(_var)(_x ^ _n)) <-- (x^n)*Deriv(var)(n*Ln(x));2 # (Deriv(_var)ArcSin(_x)) <-- (Deriv(var) x)/Sqrt(1 - (x^2));2 # (Deriv(_var)ArcCos(_x)) <-- -(Deriv(var)x)/Sqrt(1 - (x^2));2 # (Deriv(_var)ArcTan(_x)) <-- (Deriv(var) x)/(1 + x^2);2 # (Deriv(_var)ArcSinh(_x)) <-- (Deriv(var) x)/Sqrt((x^2) + 1);2 # (Deriv(_var)ArcCosh(_x)) <-- (Deriv(var) x)/Sqrt((x^2) - 1);2 # (Deriv(_var)ArcTanh(_x)) <-- (Deriv(var) x)/(1 - x^2);2 # (Deriv(_var)Sqrt(_x)) <-- ((Deriv(var)x)/(2*Sqrt(x)));2 # (Deriv(_var)Complex(_r,_i)) <-- Complex(Deriv(var)r,Deriv(var)i);LocalSymbols(var,var2,a,b,y)[ 2 # (Deriv(_var)Integrate(_var)(_y)) <-- y; 2 # (Deriv(_var)Integrate(_var2,_a,_b)(y_IsFreeOf(var))) <-- (Deriv(var)b)*(y Where var2 == b) - (Deriv(var)a)*(y Where var2 == a); 3 # (Deriv(_var)Integrate(_var2,_a,_b)(_y)) <-- (Deriv(var)b)*(y Where var2 == b) - (Deriv(var)a)*(y Where var2 == a) + Integrate(var2,a,b) Deriv(var) y; ];2 # (Deriv(_var)func_IsList)_(Not?(IsList(var))) <-- Map(\"Deriv\",{FillList(var,Length(func)),func});2 # (Deriv(_var)UniVariate(_var,_first,_coefs)) <--[ Local(result,m,i); result:=FlatCopy(coefs); m:=Length(result); For(i:=1,i<=?m,i++) [ result[i] := result[i] * (first+i-1); ]; UniVariate(var,first-1,result);];";
        scriptMap.put("Deriv",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Differentiate\",{aVar,aFunc});Rulebase(\"Differentiate\",{aVar,aCount,aFunc});Rule(\"Differentiate\",2,1,IsList(aVar) And? Not?(IsList(aFunc))) Map(\"Differentiate\",{aVar,FillList(aFunc, Length(aVar))});Rule(\"Differentiate\",2,1,IsList(aVar) And? IsList(aFunc)) Map(\"Differentiate\",{aVar,aFunc});Rule(\"Differentiate\",2,3,True)[ MacroLocal(aVar); Apply(\"Deriv\",{aVar,1,aFunc});];Rule(\"Differentiate\",3,1,IsList(aVar) And? Not?(IsList(aFunc))) Map(\"Differentiate\",{aVar, FillList(aCount, Length(aVar)), FillList(aFunc, Length(aVar))});Rule(\"Differentiate\",3,1,IsList(aVar) And? IsList(aFunc)) Map(\"Differentiate\",{aVar, FillList(aCount, Length(aVar)), aFunc});Rule(\"Differentiate\",3,3,True)[ MacroLocal(aVar); Apply(\"Deriv\",{aVar,aCount,aFunc});];HoldArgument(\"Differentiate\",aVar);HoldArgument(\"Differentiate\",aFunc);";
        scriptMap.put("Differentiate",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Diverge\", {aFunc, aBasis});Rule(\"Diverge\", 2, 1, IsList(aBasis) And? IsList(aFunc) And? Length(aBasis) =? Length(aFunc)) Add(Map(\"Differentiate\", {aBasis,aFunc}));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# FactorizeInt(0) <-- {};1# FactorizeInt(1) <-- {};3# FactorizeInt(n_IsInteger) <--[ Local(small'powers); n := Abs(n);    If( Gcd(ProductPrimesTo257(), n) >? 1,  small'powers := TrialFactorize(n, 257),  small'powers := {n}  ); n := small'powers[1];  If(n=?1, Rest(small'powers),  [  SortFactorList( PollardCombineLists(Rest(small'powers), PollardRhoFactorize(n)) ); ] );];";
        scriptMap.put("FactorizeInt",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsBinomials( _x + y_IsFreeOf(x) ) <-- {x+y,1};10 # FactorsBinomials( _x - y_IsFreeOf(x) ) <-- {x-y,1};10 # FactorsBinomials( c_Constant? * _x + y_IsFreeOf(x) ) <-- {c*x+y,1};10 # FactorsBinomials( c_Constant? * _x - y_IsFreeOf(x) ) <-- {c*x-y,1};10 # FactorsBinomials( _x^m_IsOdd + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := (y^(1/m)); If(InVerboseMode(),Tell(\" FBinsum1\",{nn,qq})); r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}};];12 # FactorsBinomials( c_Constant? * _x^m_IsOdd + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{c,x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := ((y/c)^(1/m)); If(InVerboseMode(),Tell(\" FBinsum.1b\",{nn,qq})); If( c=?1, r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}}, r := {{c,1},{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}} ); ];10 # FactorsBinomials( _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{x,m,y})); Local(pp,qq,r,L); pp := m-1; qq := (y^(1/m)); If(IsNumber(y),qq:=GuessRational(N(qq))); If(InVerboseMode(),Tell(\" FBindif.1\",{pp,qq}));  if (m =? 2) [ L := FunctionToList(y); If(And?(L[1]=?ToAtom(\"^\"),L[3]=?2),qq:=L[2]); r := {{x+qq,1},{x-qq,1}}; ] else if (m =? 4) [r := {{x+qq,1},{x-qq,1},{x^2+qq^2,1}};] else if (m =? 6) [r := {{x+qq,1},{x-qq,1},{x^2+x*qq+qq^2,1},{x^2-x*qq+qq^2,1}};] else [r := {{x-qq,1},{Sum(k,0,pp,qq^k*x^(pp-k)),1}};]; r;];12 # xFactorsBinomials( c_Constant? * _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{c,x,m,y})); Local(aa,bb,c0,r); aa := c^(1/m); bb := ((y)^(1/m)); If(IsNumber(y),bb:=GuessRational(N(bb))); If(InVerboseMode(),Tell(\" FBindif.1b\",{aa,bb})); r := FactorsBinomials( (aa*x)^m - bb^m );];";
        scriptMap.put("FactorsBinomials",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsMonomial(expr_IsMonomial) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( IsRationalFunction(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        scriptMap.put("FactorsMonomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsPolynomialOverIntegers(_expr)_IsPolynomialOverIntegers(expr) <--[ Local(x); x := VarList(expr)[1]; FactorsPolynomialOverIntegers(expr,x);];15 # FactorsPolynomialOverIntegers(_expr) <-- expr;10 # FactorsPolynomialOverIntegers(_expr,_var)_(IsPolynomialOverIntegers(expr,var)) <--[ Local(factorsList,factListTransp,factrs,multiplicities,factrsUnMonic); Local(polyFactors,normalizations,normDivisor,polyFactors,factList); Local(n,result,newResult,gtotal,r,rr,d,g); factorsList := BinaryFactors(expr);        factListTransp := Transpose(factorsList); factrs := factListTransp[1]; multiplicities := factListTransp[2];          factrsUnMonic := MapSingle(\"Together\",factrs);         {polyFactors,normalizations}:=Transpose(MapSingle(\"GetNumerDenom\",factrsUnMonic));          normDivisor := Product(Map(\"^\",{normalizations,multiplicities}));         polyFactors[1] := Simplify(polyFactors[1]/normDivisor);     factListTransp[1] := polyFactors; factList := Transpose(factListTransp);    result := factList;   Local(newResult,gtotal,d,g,rr); newResult := {}; gtotal := 1; ForEach(r,result) [ d := Degree(r[1],var); g := Gcd(Coef(r[1],var,0 .. d)); If( g >? 1,  [ gtotal:=g*gtotal;  r[1]:=Simplify(r[1]/g); ] ); If(d >? 2, [  rr := TryToReduceSpecialPolynomial(r[1]); If( IsList(rr),newResult := Concat(newResult,rr) ); ], If( r !=? {1,1}, newResult := r:newResult ) ); ];  If(gtotal>?1,newResult:={gtotal,1}:newResult); newResult;]; 10 # TryToReduceSpecialPolynomial(_x^4+_x^2+1) <-- {{x^2+x+1,1},{x^2-x+1,1}};10 # TryToReduceSpecialPolynomial(_x^6-1) <-- {{x+1,1},{x-1,1},{x^2+x+1,1},{x^2-x+1,1}};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # IsMonomial(expr_CanBeMonomial) <-- [ Local(r); If( IsRationalFunction(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # IsMonomial(_expr) <-- False;10 # FactorsMonomial(expr_IsMonomial) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( IsRationalFunction(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( IsAtom(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        scriptMap.put("Monomials",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Roots(poly_CanBeUni) <--[ Local(factors,result,uni,root,i,deg); factors:=Factors(poly); result:={}; ForEach(item,factors) [ uni:=MakeUni(item[1]); deg:=Degree(uni); If(deg >? 0 And? deg <? 3, [ root:= PSolve(uni); If(Not? IsList(root),root:={root}); For(i:=0,i<?item[2],i++) result:= Concat(root, result); ] ); ]; result;];";
        scriptMap.put("Roots",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RootsWithMultiples(poly_CanBeUni) <--[ Local(factors,result,uni,root,i,deg); factors:=Factors(poly); result:={}; ForEach(item,factors) [ uni:=MakeUni(item[1]); deg:=Degree(uni); If(deg >? 0 And? deg <? 3, [ root:= PSolve(uni); If(Not? IsList(root),root:={root}); For(i:=1,i<=?Length(root),i++) result:= Concat({{root[i],item[2]}}, result); ] ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # AntiDeriv(_var, poly_CanBeUni(var) ) <-- NormalForm(AntiDeriv(var,`MakeUni(@poly,@var)));5 # AntiDeriv(_var,UniVariate(_var,_first,_coefs)) <--[ Local(result,i); result:=FlatCopy(coefs); For(i:=1,i<=?Length(result),i++) [ result[i]:= result[i]/(first+i); ]; UniVariate(var,first+1,result);];10 # AntiDeriv(_var,_x + _y) <-- AntiDeriv(var,x) + AntiDeriv(var,y);10 # AntiDeriv(_var,_x - _y) <-- AntiDeriv(var,x) - AntiDeriv(var,y);10 # AntiDeriv(_var, - _y) <-- - AntiDeriv(var,y);10 # AntiDeriv(_var,_x/c_IsFreeOf(var) )_(HasExpression?(x,var)) <-- AntiDeriv(var,x)/c;10 # AntiDeriv(_var,c_IsFreeOf(var)/_x )_(HasExpression?(x,var) And? c!=? 1) <-- c*AntiDeriv(var,1/x);1570 # IntegrateMultiplicative(_var,(exy_CanBeUni(var)) * _exx,_dummy1,_dummy2) <-- IntByParts(var,exy*exx,AntiDeriv(var,exx));1570 # IntegrateMultiplicative(_var,_exx * (exy_CanBeUni(var)),_dummy1,_dummy2) <-- IntByParts(var,exy*exx,AntiDeriv(var,exx));10 # IntByParts(_var,_exy * _exx,Integrate(_var)(_something)) <-- `Hold(AntiDeriv(@var,((@exy)*(@exx))));20 # IntByParts(_var,_exy * _exx,_anti)_(Not? IsFreeOf(anti,exx)) <-- `Hold(AntiDeriv(@var,((@exy)*(@exx))));30 # IntByParts(_var,_exy * _exx,_anti) <-- [ Local(cf); cf:=anti*Deriv(var)exy; exy*anti - `(AntiDeriv(@var,@cf)); ];1570 # IntegrateMultiplicative(_var,(exy_CanBeUni(var)) / (exx_CanBeUni(var)),_dummy1,_dummy2) <-- IntRat(var,exy/exx,MakeUni(exy,var),MakeUni(exx,var));10 # IntRat(_var,_exy / _exx,_exyu,_exxu)_ (Degree(exyu) >? Degree(exxu) Or? Degree(Gcd(exyu,exxu)) >? 0) <-- [ Local(gcd); gcd:=Gcd(exxu,exyu); exyu:=Quotient(exyu,gcd); exxu:=Quotient(exxu,gcd); AntiDeriv(var,NormalForm(Quotient(exyu,exxu))) + AntiDeriv(var,NormalForm(Modulo(exyu,exxu))/NormalForm(exxu)); ];11 # IntRat(_var,_exy / _exx,_exyu,_exxu)_ (Degree(exxu,var) >? 1 And? LeadingCoef(exxu)=?1 And? IsNumericList(Coef(exxu,var,0 .. Degree(exxu)))) <--[ Local(ee); ee:=Apart(exy/exx,var); `AntiDeriv(@var,@ee);];20 # IntRat(_var,_exy / _exx,_exyu,_exxu) <-- `Hold(AntiDeriv(@var,((@exy)/(@exx))));30 # AntiDeriv(_var,Deriv(_var)(_expr)) <-- expr;100 # AntiDeriv(_var,_exp) <--[ IntegrateMultiplicative(var,exp,a,b);];10 # IntegrateMultiplicative(_var,if(_cond)(_body),_a,_b) <-- [ body := AntiDeriv(var,body); `Hold(if(@cond)(@body)); ];10 # IntegrateMultiplicative(_var,(_left) else (_right),_a,_b) <-- [ left := AntiDeriv(var,left); right := AntiDeriv(var,right); `Hold( (@left) else (@right) ); ];1600 # IntegrateMultiplicative(_var,_exp,_a,_b) <-- `Hold(Integrate(@var)(@exp));LocalSymbols(intpred)[ intpred := 50; IntFunc(_vr,_from,_to) <-- [ `((@intpred) # IntegrateMultiplicative(_var,@from,_dummy1,_dummy2)_MatchLinear(var,@vr) <-- (@to)/Matched'a()); intpred++; ];];IntPureSquare(_vr,_from,_sign2,_sign0,_to) <--[ `(50 # IntegrateMultiplicative(_var,@from,_dummy1,_dummy2)_MatchPureSquared(var,@sign2,@sign0,@vr) <-- (@to));];IntFunc(x,Sqrt(_x),(2*Sqrt(x)^(3))/3);IntFunc(x,1/Sqrt(_x),2*Sqrt(x));IntFunc(x,1/_x^(_n),x^(1-n)/(1-n) );IntFunc(x,Sin(_x),-Cos(x));IntFunc(x,1/Sin(_x), Ln( 1/Sin(x) - Cos(x)/Sin(x) ) );IntFunc(x,Cos(_x),Sin(x));IntFunc(x,1/Cos(_x),Ln(1/Cos(x)+Tan(x)));IntFunc(x,Tan(_x),-Ln(Cos(x)));IntFunc(x,1/Tan(_x),Ln(Sin(x)) );IntFunc(x,Cos(_x)/Sin(_x),Ln(Sin(x)));IntFunc(x,Exp(_x),Exp(x));IntFunc(x,(C_IsFreeOf(var))^(_x),C^x/Ln(C));IntFunc(x,num_IsFreeOf(var) / (_x),num*Ln(x));IntFunc(x,Ln(_x),x*Ln(x)-x);IntFunc(x,(_x)*Ln(_x),(1/(1+1))*x^(1+1)*Ln(x) - (1/(1+1)^2)*x^(1+1) );IntFunc(x,Ln(_x)*(_x),(1/(1+1))*x^(1+1)*Ln(x) - (1/(1+1)^2)*x^(1+1) );IntFunc(x,1/Sin(_x)^2,-Cos(x)/Sin(x) );IntFunc(x,1/Cos(_x)^2,Tan(x) );IntFunc(x,1/(Sin(_x)*Tan(_x)),-1/Sin(x));IntFunc(x,Tan(_x)/Cos(_x),1/Cos(x));IntFunc(x,1/Sinh(_x)^2,-1/Tanh(x));IntFunc(x,1/Cosh(_x)^2,Tanh(x));IntFunc(x,1/(Sinh(_x)*Tan(_x)),-1/Sinh(x));IntFunc(x,Tanh(_x)/Cosh(_x),-1/Cosh(x));IntFunc(x,1/Sqrt(m_IsFreeOf(x)-_x^2),ArcSin(x/Sqrt(m)) );IntFunc(x,Exp(n_IsNumber*_x)*Sin(m_IsNumber*_x),Exp(n*x)*(n*Sin(m*x)- m*Cos(m*x))/(m^2+n^2) );IntFunc(x,Ln(_x)*(_x)^n_IsNumber,(1/(n+1))*x^(n+1)*Ln(x) - (1/(n+1)^2)*x^(n+1) );IntFunc(x,Ln(A_IsNumber*_x)*(_x)^n_IsNumber,(1/(n+1))*x^(n+1)*Ln(A*x) - (1/(n+1)^2)*x^(n+1) );IntFunc(x,Sin(Ln(_x)),x*Sin(Ln(x))/2 - x*Cos(Ln(x))/2 );IntFunc(x,Cos(Ln(_x)),x*Sin(Ln(x))/2 + x*Cos(Ln(x))/2 );IntFunc(x,1/((_x)*Ln(_x)),Ln(Ln(x)));IntFunc(x,(_x)^(-1),Ln(x));IntFunc(x,(_x)^(n_IsFreeOf(x)),x^(n+1)/(n+1));IntFunc(x,C_IsFreeOf(x)*(_x)^(n_IsFreeOf(x)),C*x^(n+1)/(n+1));IntFunc(x,C_IsFreeOf(x)/(D_IsFreeOf(x)*(_x)^(n_IsFreeOf(x))),(C/D)*x^(1-n)/(1-n));IntFunc(x,Sinh(_x),Cosh(x));IntFunc(x,Sinh(_x)^2,Sinh(2*x)/4 - x/2);IntFunc(x,1/Sinh(_x),Ln(Tanh(x/2)));IntFunc(x,Cosh(_x),Sinh(x));IntFunc(x,Cosh(_x)^2,Sinh(2*x)/4 + x/2);IntFunc(x,1/Cosh(_x),ArcTan(Sinh(x)));IntFunc(x,Tanh(_x),Ln(Cosh(x)));IntFunc(x,Tanh(_x)/Cosh(_x),-1/Cosh(x));IntFunc(x,1/Cosh(_x)^2,Tanh(x));IntFunc(x,1/Tanh(_x),Ln(Sinh(x)));IntFunc(x,Abs(_x),Abs(x)*x/2); IntFunc(x,ArcTan(_x),x*ArcTan(x) - Ln(x^2 + 1)/2);IntFunc(x,ArcCos(_x),x*ArcCos(x) - Sqrt(1-x^2) );IntFunc(x,ArcTanh(_x),x*ArcTanh(x) + Ln(1-x^2)/2 );IntFunc(x,ArcSinh(_x),x*ArcSinh(x) - Sqrt(x^2 + 1) );IntFunc(x,ArcCosh(_x),x*ArcCosh(x) - Sqrt(x-1)*Sqrt(x+1) );IntFunc(x,num_IsFreeOf(var)/(A_IsNumber + B_IsNumber*(_x))^2,-num/(A*b + B^2*x));IntFunc(x,num_IsFreeOf(var)/(n_IsNumber + m_IsNumber*Exp(p_IsNumber*(_x))),num*x/n - num*Ln(n + m*Exp(p*x))/(n*p));IntFunc(x,num_IsFreeOf(var)/(m_IsNumber*Exp(p_IsNumber*(_x)) + n_IsNumber),num*x/n - num*Ln(n + m*Exp(p*x))/(n*p));IntPureSquare(x,num_IsFreeOf(var)/(_x),1,1,(num/(Sqrt(Matched'b()*Matched'a())))*ArcTan(var/Sqrt(Matched'b()/Matched'a())));IntFunc(x,Erf(_x), x*Erf(x)+ 1/(Exp(x^2)*Sqrt(Pi)) );UnFence(\"IntegrateMultiplicative\",4);";
        scriptMap.put("AntiDeriv",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10# (Integrate(_var)(expr_IsList)) <-- Map(\"Integrate\",{FillList(var,Length(expr)),expr});20 # (Integrate(_var)(_expr)) <-- IntSub(var,expr,AntiDeriv(var,IntClean(var,expr)));20 # (Integrate(_var, optionsList_IsList)(_expr)) <--[ Local(result); optionsList := OptionsToAssociativeList(optionsList); result := Integrate(var) expr; If( optionsList[\"logAbs\"] =? \"True\", result := ( result /: {Ln(_x) <- Ln(Abs(x))}) ); result;];10 # IntSub(_var,_expr,Integrate(_var)(_expr2)) <-- `Hold(Integrate(@var)(@expr));20 # IntSub(_var,_expr,_result) <-- result; 10# (Integrate(_var,_from,_to)(expr_IsList)) <-- Map(\"Integrate\",{FillList(var,Length(expr)), FillList(from,Length(expr)), FillList(to,Length(expr)), expr});20 # (Integrate(_var,_from,_to)(_expr)) <-- defIntegrate(var,from,to,expr,a,b);20 # (Integrate(_var,_from,_to,optionsList_IsList)(_expr)) <--[ Local(result); optionsList := OptionsToAssociativeList(optionsList); result := Integrate(var,from,to) expr; If( optionsList[\"logAbs\"] =? \"True\", result := ( result /: {Ln(_x) <- Ln(Abs(x))}) ); result;];10 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(from =? -to And? IsOddFunction(expr,var)) <-- 0;10 # defIntegrate(_var,_from,_from,_expr,_a,_b) <-- 0;12 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(from =? -to And? EvenFunction?(expr,var)) <-- 2*defIntegrate(var,0,to,expr,a,b);100 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(Type(AntiDeriv(var,IntClean(var,expr))) !=? \"AntiDeriv\") <-- IntegrateRange(var,expr,from,to,AntiDeriv(var,IntClean(var,expr)));101 # defIntegrate(_var,_from,_to,_expr,_a,_b) <-- `Hold(Integrate(@var,@from,@to)(@expr));10 # IntegrateRange(_var,_expr,_from,_to,Integrate(_var)_expr2) <-- `Hold(Integrate(@var,@from,@to)@expr);20 # IntegrateRange(_var,_expr,_from,_to,_antideriv) <-- `(@antideriv Where @var == @to) - `(@antideriv Where @var == @from);10 # IntClean(_var,_expr) <--[ if( IsFreeOf(var,expr) Or? IsSumOfTerms(var,expr) )[ expr; ] else if ( HasFunction?(expr,Sin) Or? HasFunction?(expr,Cos) )[ Simplify(TrigSimpCombine(expr)); ] else [ Simplify(expr); ];];";
        scriptMap.put("Integrate",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(a,b)[10 # MatchLinear(var_IsAtom,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchLinear(expr);];20 # MatchLinear(_var,_expr) <-- False;10 # MatchLinear(_expr)_(Degree(expr,var)<?2) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,1); b := Coef(expr,0); True;];20 # MatchLinear(_expr) <-- False;UnFence(\"MatchLinear\",1);UnFence(\"MatchLinear\",2);10 # MatchPureSquared(var_IsAtom,_sign2,_sign0,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchPureSquared(expr,sign2,sign0);];20 # MatchPureSquared(_var,_sign2,_sign0,_expr) <-- False;10 # MatchPureSquared(_expr,_sign2,_sign0)_(Degree(expr,var)=?2 And? Coef(expr,1) =? 0 And? IsNumber(Coef(expr,0)) And? IsNumber(Coef(expr,2)) And? Coef(expr,0)*sign0 >? 0 And? Coef(expr,2)*sign2 >? 0 ) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,2); b := Coef(expr,0); True;];20 # MatchPureSquared(_expr,_sign2,_sign0) <-- False;UnFence(\"MatchPureSquared\",3);UnFence(\"MatchPureSquared\",4);Matched'a() := a;Matched'b() := b;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "(Assert(_error'class, _error'object) _predicate) <--[ CheckErrorTableau(); If(IsEqual(predicate, True),  True, [  DestructiveAppend(GetErrorTableau(), {error'class, error'object}); False; ] );];(Assert(_error'class) _predicate) <-- Assert(error'class, True) predicate;(Assert() _predicate) <-- Assert(\"generic\", True) predicate;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DumpErrors() <--[ Local(error'object, error'word); CheckErrorTableau(); ForEach(error'object, GetErrorTableau()) [  If( IsList(error'object), [ If(  Length(error'object) >? 0 And? error'object[1] =? \"warning\", [ error'word := \"Warning\"; error'object[1] := \"\";  ], error'word := \"Error: \"  ); If(  Length(error'object)=?2 And? error'object[2]=?True, Echo(error'word, error'object[1]), [ Echo(error'word, error'object[1], \": \", PrintList(Rest(error'object))); ] ); ],  Echo(\"Error: \", error'object) ); ]; ClearErrors();];";
        scriptMap.put("DumpErrors",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EchoInternal(string_IsString) <--[ WriteString(string);];20 # EchoInternal(_item) <--[ Write(item);Space();];RulebaseListed(\"Echo\",{firstParameter, parametersList});5 # Echo(_firstParameter) <-- Echo(firstParameter, {});10 # Echo(_firstParameter, parametersList_IsList) <--[ EchoInternal(firstParameter); ForEach(item,parametersList) EchoInternal(item); NewLine(); ];20 # Echo(_firstParameter, _secondParameter) <-- Echo(firstParameter, {secondParameter});Echo() := NewLine();";
        scriptMap.put("Echo",scriptString);
        scriptMap.put("EchoInternal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(ErrorTableau) [  Bind(ErrorTableau, {}); GetErrorTableau() := ErrorTableau; ClearErrors() <-- Bind(ErrorTableau, {});  CheckErrorTableau() <-- If( Not? IsList(ErrorTableau), Bind(ErrorTableau, {{\"general\", \"corrupted ErrorTableau\"}}) );]; GetError(error'class_IsString) <--[ Local(error); error := GetErrorTableau()[error'class]; If( error !=? Empty, error, False );];ClearError(error'class_IsString) <-- AssocDelete(GetErrorTableau(), error'class);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Determinant(_matrix)_(IsUpperTriangular(matrix) Or? IsLowerTriangular(matrix)) <--[ Local(result); result:=1; ForEach(i, Diagonal(matrix) ) result:=result*i; result;];15 # Determinant(_matrix)_(Length(Select(Diagonal(matrix), \"IsZero\")) >? 0) <-- SymbolicDeterminant(matrix);16 # Determinant(_matrix)_(VarList(matrix) !=? {}) <-- SymbolicDeterminant(matrix);20 # Determinant(_matrix) <-- GaussianDeterminant(matrix);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(x,i,n,m,aux,dim,result)[ 1 # Dimensions(x_IsList) <-- [ Local(i,n,m,aux,dim,result); result:=List(Length(x)); If(Length(x)>?0 And? Length(Select(x, IsList))=?Length(x), [ n:=Length(x); dim:=MapSingle(Dimensions,x); m:=Minimum(MapSingle(Length,dim)); For(i:=1,i<=?m,i++) [ aux:=Table(dim[j][i],j,1,n,1); If(Minimum(aux)=?Maximum(aux), result:=DestructiveAppend(result,dim[1][i]), i:=m+1 ); ]; ] ); result; ]; 2 # Dimensions(_x) <-- List();]; ";
        scriptMap.put("Dimensions",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(Dot0,Dot1)[Dot(t1_IsVector,t2_IsVector)_(Length(t1)=?Length(t2)) <-- Dot0(t1,t2,Length(t1));Dot(t1_IsMatrix,t2_IsVector)_(Length(t1[1])=?Length(t2)) <--[ Local(i,n,m,result); n:=Length(t1); m:=Length(t2); result:=List(); For(i:=1,i<=?n,i++) DestructiveInsert(result,1,Dot0(t1[i],t2,m)); DestructiveReverse(result);];Dot(t1_IsVector,t2_IsMatrix)_(Length(t1)=?Length(t2) And? Length(t2[1])>?0) <-- Dot1(t1,t2,Length(t1),Length(t2[1]));Dot(t1_IsMatrix,t2_IsMatrix)_(Length(t1[1])=?Length(t2) And? Length(t2[1])>?0) <--[ Local(i,n,k,l,result); n:=Length(t1); k:=Length(t2); l:=Length(t2[1]); result:=List(); For(i:=1,i<=?n,i++) DestructiveInsert(result,1,Dot1(t1[i],t2,k,l)); DestructiveReverse(result);];Dot0(_t1,_t2,_n) <--[ Local(i,result); result:=0; For(i:=1,i<=?n,i++) result:=result+t1[i]*t2[i]; result;];Dot1(_t1,_t2,_m,_n) <--[ Local(i,j,result); result:=ZeroVector(n); For(i:=1,i<=?n,i++) For(j:=1,j<=?m,j++) result[i]:=result[i]+t1[j]*t2[j][i]; result;];]; ";
        scriptMap.put("Dot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ExtractSubMatrix( mat_IsMatrix, _row1, _col1, _row2, _col2 )_ (And?(IsPositiveInteger(row1),IsPositiveInteger(col1), IsPositiveInteger(row2),IsPositiveInteger(col2))) <--[ Local(nrows,ncols,r,row,result); {nrows,ncols} := Dimensions( mat ); Check(And?(row1>?0,col1>?0,row1<?nrows,col1<?ncols), \"Math\", \"ERROR: UL out of range\"); Check(And?(row2>?row1,col2>?col1,row2<=?nrows,col2<=?ncols), \"Math\", \"ERROR: LR out of range\"); result := {}; For(r:=row1,r<=?row2,r++) [ row := Take( MatrixRow(mat,r), {col1,col2} ); result := DestructiveAppend( result, row ); ]; result;];10 # ExtractSubMatrix( mat_IsMatrix, _row1, _col1 )_ (And?(IsPositiveInteger(row1),IsPositiveInteger(col1))) <--[ Local(nrows,ncols); {nrows,ncols} := Dimensions( mat ); Check(And?(row1>?0,col1>?0,row1<?nrows,col1<?ncols), \"Math\", \"ERROR: UL out of range\"); ExtractSubMatrix( mat, row1, col1, nrows, ncols );];";
        scriptMap.put("ExtractSubMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FrobeniusNorm(matrix_IsMatrix) <--[ Local(i,j,result); result:=0; For(i:=1,i<=?Length(matrix),i++) For(j:=1,j<=?Length(matrix[1]),j++) result:=result+Abs(matrix[i][j])^2; Sqrt(result);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Identity(n_IsNonNegativeInteger) <--[ Local(i,result); result:={}; For(i:=1,i<=?n,i++) [ DestructiveAppend(result,BaseVector(i,n)); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # InfinityNorm( M_IsMatrix ) <--[ Local(sumlist,row); sumlist := {}; ForEach(row,M) Push(sumlist,Sum(Abs(row))); Maximum(sumlist);];10 # InfinityNorm( M_IsVector ) <-- Maximum(Abs(M));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"OrthogonalBasis\",{W})[ Local(V,j,k); V:=ZeroMatrix(Length(W),Length(W[1]) ); V[1]:=W[1]; For(k:=2,k<=?Length(W),k++)[ Check(Not? IsZero(Norm(W[k])), \"Argument\", \"OrthogonalBasis: Input vectors must be linearly independent\"); V[k]:=W[k]-Sum(j,1,k-1,InProduct(W[k],V[j])*V[j]/Norm(V[j])^2); ]; V;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Outer(t1_IsVector, t2_IsVector) <--[ Local(i,j,n,m,result); n:=Length(t1); m:=Length(t2); result:=ZeroMatrix(n,m); For(i:=1,i<=?n,i++) For(j:=1,j<=?m,j++) result[i][j]:=t1[i]*t2[j]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RREF( AM_IsMatrix ) <--[ If( Not? IsBound(iDebug), iDebug := False ); If(iDebug,Tell(\"RREF\",AM)); Local(mRows,nCols,nVars,varVec,ir,jc,col,am,e,ii,pivot); {mRows,nCols} := Dimensions(AM); am := FlatCopy(AM);   Local(cc,cr,col,cnz);  cc := 1;  cr := 1;  While( cr <=? mRows )  [ Local(ic);  If(iDebug, Tell(\" STEP 1\")); cnz := 0;  For(ic:=1,ic<=?nCols,ic++)  [  col := MatrixColumn(am,ic); If( cr >? 1, col := Drop( col, {1,cr-1} ) );  If(Not? IsZeroVector(col), [cnz := ic; Break();]);  ]; cc := cnz; If( iDebug, Tell(\" first non-zero column is \",cc));    If(iDebug, Tell(\" STEP 2\")); For( ir:=cr, ir<=?mRows,ir++ ) [  If( am[ir][cc] !=? 0 And? ir !=? 1,  [ {am[ir],am[cr]} := {am[cr],am[ir]}; If(iDebug, Tell(\" swapping rows \",{cr,ir})); Break(); ] ); ]; If(iDebug,[Tell(\"working matrix\");TableForm(am);]);    If(iDebug, Tell(\" STEP 3\")); am[cr] := am[cr]/am[cr][cc]; If(iDebug,TableForm(am));    If(iDebug, Tell(\" STEP 4\")); For(ir:=cr+1,ir<=?mRows,ir++) [ If( am[ir][cc] !=? 0,  [ am[ir] := am[ir] - am[ir][cc]*am[cr]; ] ); ]; If(iDebug,TableForm(am));     If(iDebug, Tell(\" STEP 5\")); cr := cr + 1; If(iDebug And? cr <=? mRows, [NewLine();Tell(\" \",cr);]);   If( cr=?mRows And? IsZeroVector(am[cr]), Break() ); ];       If(iDebug, Tell(\" STEP 6\")); Local(pc,jr); For(ir:=mRows,ir>?1,ir--) [   If(iDebug,Tell(\"\",{ir,am[ir]})); If(IsZeroVector(am[ir]), [If(iDebug,Tell(\" trailing row of zeros: row \",ir)); ir:=ir-1;Continue();], [ pc := Find(am[ir],1);  If(pc >? 0,  [ For(jr:=ir-1,jr>=?1,jr--) [ If(am[jr][pc]!=?0, am[jr]:=am[jr]-am[jr][pc]*am[ir]); If(iDebug,[NewLine();TableForm(am);]); ]; ] ); ] ); ]; am;]; ";
        scriptMap.put("RREF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # RecursiveDeterminant(_matrix) <--[  Local(result); If(IsEqual(Length(matrix),1),matrix[1][1],[ result:=0; ForEach(i,1 .. Length(matrix))  If(Not?(IsEqual(matrix[1][i],0)),  result:=result+matrix[1][i]*(-1)^(i+1)* RecursiveDeterminant(Transpose(Drop(Transpose(Drop(matrix,{1,1})),{i,i})))); result; ]);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # SymbolicDeterminant(_matrix) <--[ Local(perms,indices,result); Check((IsMatrix(matrix)), \"Argument\", \"Determinant: Argument must be a matrix\"); indices:=Table(i,i,1,Length(matrix),1); perms:=PermutationsList(indices); result:=0; ForEach(item,perms) result:=result+Product(i,1,Length(matrix),matrix[i][item[i] ])* LeviCivita(item); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Trace(matrix_IsList) <--[ Local(i,j,n,d,r,aux,result);  d:=Dimensions(matrix);  r:=Length(d);   n:=Minimum(d);   result:=0;  For(i:=1,i<=?n,i++) [ aux:=matrix[i]; For(j:=2,j<=?r,j++) aux:=aux[i]; result:=result+aux; ]; result;];";
        scriptMap.put("Trace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Transpose(matrix_IsList)_(Length(Dimensions(matrix))>?1) <--[ Local(i,j,result); result:=ZeroMatrix(Length(matrix[1]),Length(matrix)); For(i:=1,i<=?Length(matrix),i++) For(j:=1,j<=?Length(matrix[1]),j++) result[j][i]:=matrix[i][j]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # ZeroMatrix(n_IsNonNegativeInteger) <-- ZeroMatrix(n,n);10 # ZeroMatrix(n_IsNonNegativeInteger,m_IsNonNegativeInteger) <--[ Local(i,result); result:={}; For(i:=1,i<=?n,i++) DestructiveInsert(result,i,ZeroVector(m)); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnAugment( M_IsMatrix, v_IsVector )_(Length(v)=?Dimensions(M)[2]) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnAugment,{M,v}) ); Local(mRows,nCols,newMat,ir); Local(MT,MA); MT := Transpose(M); MT := MatrixRowStack(MT,v); MA := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnReplace( M_IsMatrix, jCol_IsPositiveInteger, v_IsVector )_(Length(v)=?Dimensions(M)[2]) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnReplace,{M,jCol,v}) ); Local(mRows,nCols,MT); {mRows,nCols} := Dimensions(M); If( jCol <=? nCols, [ MT:=Transpose(M); DestructiveReplace(MT,jCol,v); M:=Transpose(MT); ] ); M;];";
        scriptMap.put("MatrixColumnReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnSwap( M_IsMatrix, jCol1_IsPositiveInteger, jCol2_IsPositiveInteger )_ (And?(jCol1<=?Dimensions(M)[2],jCol2<=?Dimensions(M)[2])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnSwap,{M,jCol1,jCol2}) ); Local(MT); MT := Transpose(M); MT := MatrixRowSwap(MT,jCol1,jCol2); M := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowReplace( M_IsMatrix, iRow_IsPositiveInteger, v_IsVector )_(Length(v)=?Length(M[1])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowReplace,{M,iRow,v}) ); Local(mRows,nCols); {mRows,nCols} := Dimensions(M); If( iRow <=? mRows, DestructiveReplace(M,iRow,v) ); M;];";
        scriptMap.put("MatrixRowReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowStack( M_IsMatrix, v_IsVector )_(Length(v)=?Dimensions(M)[1]) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowStack,{M,v}) ); Local(mRows,nCols,newMat,ir); {mRows,nCols} := Dimensions(M); newMat := ZeroMatrix(mRows+1,nCols); For(ir:=1,ir<?mRows+1,ir++) [ MatrixRowReplace(newMat,ir,MatrixRow(M,ir)); ]; MatrixRowReplace(newMat,mRows+1,v); newMat;];";
        scriptMap.put("MatrixRowStack",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowSwap( M_IsMatrix, iRow1_IsPositiveInteger, iRow2_IsPositiveInteger )_ (And?(iRow1<=?Dimensions(M)[1],iRow2<=?Dimensions(M)[1])) <--[ If( Not? IsBound(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowSwap,{M,iRow1,iRow2}) ); Local(row1,row2); If( iRow1 !=? iRow2, [ row1 := MatrixRow(M,iRow1); row2 := MatrixRow(M,iRow2); DestructiveReplace(M,iRow1,row2); DestructiveReplace(M,iRow2,row1); ] ); M;];";
        scriptMap.put("MatrixRowSwap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Append\",{list,element})[ Check(IsList(list), \"Argument\", \"The first argument must be a list.\"); Insert(list,Length(list)+1,element);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Contains\",{list,element})[ Local(result); Bind(result,False); While(And?(Not?(result), Not?(IsEqual(list, {})))) [ If(IsEqual(First(list),element), Bind(result, True), Bind(list, Rest(list)) ); ]; result;];";
        scriptMap.put("Contains",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Count\",{list,element})[ Local(result); Bind(result,0); ForEach(item,list) If(IsEqual(item, element), Bind(result,AddN(result,1))); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Drop\", {lst, range});Rule(\"Drop\", 2, 1, IsList(range)) Concat(Take(lst,range[1]-1), Drop(lst, range[2]));Rule(\"Drop\", 2, 2, range >=? 0) If( range =? 0 Or? lst =? {}, lst, Drop( Rest(lst), range-1 ));Rule(\"Drop\", 2, 2, range <? 0) Take( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Find\",{list,element})[ Local(result,count); Bind(result, -1); Bind(count, 1); While(And?(result<?0, Not?(IsEqual(list, {})))) [ If(IsEqual(First(list), element), Bind(result, count) ); Bind(list,Rest(list)); Bind(count,AddN(count,1)); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FindPredicate\",{list,predicate})[ Local(result,count); Bind(result, -1); Bind(count, 1); While(And?(result<?0, Not?(IsEqual(list, {})))) [ If(Apply(predicate,{First(list)}), Bind(result, count) ); Bind(list,Rest(list)); Bind(count,AddN(count,1)); ]; result;]; ";
        scriptMap.put("FindPredicate",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FuncList(expr_IsAtom) <-- {};20 # FuncList(expr_IsFunction) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, Apply(\"Concat\", MapSingle(\"FuncList\", Rest(FunctionToList(expr))) ) ));10 # FuncList(expr_IsAtom, look'list_IsList) <-- {};20 # FuncList(expr_IsFunction, look'list_IsList)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {ToAtom(Type(expr))};30 # FuncList(expr_IsFunction, look'list_IsList) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, [   Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, FuncList(item, look'list)); result; ] ));HoldArgumentNumber(\"FuncList\", 1, 1);HoldArgumentNumber(\"FuncList\", 2, 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HeapSort(list, compare) := HeapSort(list, ArrayCreate(Length(list), 0), 1, Length(list), compare);1 # HeapSort(_list, _tmplist, _first, _last, _compare) _ (last - first <=? 2) <-- SmallSort(list, first, last, compare);2 # HeapSort(_list, _tmplist, _first, _last, _compare) <--[    Local(mid, ileft, iright, pleft); mid := first+((last-first)>>1); HeapSort(list, tmplist, first, mid, compare); HeapSort(list, tmplist, mid+1, last, compare);  For(ileft := first, ileft <=? mid, ileft++) tmplist[ileft] := list[ileft]; For( [ileft := first; pleft := first; iright := mid+1;], ileft <=? mid,  pleft++  )     If(  iright>?last Or? Apply(compare,{tmplist[ileft],list[iright]}), [  list[pleft] := tmplist[ileft]; ileft++; ], [  list[pleft] := list[iright]; iright++; ] ); list;];";
        scriptMap.put("HeapSort",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Intersection( LoL_IsList )_(AllSatisfy?(\"IsList\",LoL)) <--[  Local(nLists,L0,L1,ii,result,LI); nLists := Length(LoL);  If( nLists =? 1, [ result := LoL[1]; ], [ L0 := FlatCopy(LoL[1]); For( ii:=2,ii<=?nLists,ii++) [ L1 := FlatCopy(LoL[ii]);  LI := Intersection(L0,L1);  L0 := FlatCopy(LI); ];  result := L0; ] ); result;]; 11 # Intersection(list1_IsList,list2_IsList) <--[  Local(l2,index,result); l2:=FlatCopy(list2); result:={}; ForEach(item,list1) [ Bind(index, Find(l2,item)); If(index>?0, [ DestructiveDelete(l2,index); DestructiveInsert(result,1,item); ] ); ]; DestructiveReverse(result);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PrintList(list_IsList) <-- PrintList(list, \", \");10 # PrintList({}, padding_IsString) <-- \"\";20 # PrintList(list_IsList, padding_IsString) <-- PipeToString() [ Local(i); ForEach(i, list) [ If(Not?(IsEqual(i, First(list))), WriteString(padding)); If (IsString(i), WriteString(i), If(IsList(i), WriteString(\"{\" : PrintList(i, padding) : \"}\"), Write(i))); ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"Table\",{body,var,count'from,count'to,step}) [ MacroLocal(var); Local(result,nr,ii); result:={}; nr := (count'to - count'from) / step; ii := 0; While( ii <=? nr ) [ MacroBind( var, count'from + ii * step ); DestructiveInsert( result,1,Eval(body) ); Bind(ii,AddN(ii,1)); ]; DestructiveReverse(result); ];HoldArgumentNumber(\"Table\",5,1); HoldArgumentNumber(\"Table\",5,2); UnFence(\"Table\",5);";
        scriptMap.put("Table",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Take\", {lst, range});Rule(\"Take\", 2, 1, IsList(range)) Take( Drop(lst, range[1] -1), range[2] - range[1] + 1);Rule(\"Take\", 2, 2, range >=? 0) If( Length(lst)=?0 Or? range=?0, {}, Concat({First(lst)}, Take(Rest(lst), range-1)));Rule(\"Take\", 2, 2, range <? 0) Drop( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VarList(_expr) <-- VarList(expr,\"IsVariable\");Function(\"VarList\",{expr,filter})[ RemoveDuplicates(VarListAll(expr,filter));];";
        scriptMap.put("VarList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VarListAll(_expr) <-- VarListAll(expr,\"IsVariable\");10 # VarListAll(_expr,_filter)_(Apply(filter,{expr}) =? True) <-- {expr};20 # VarListAll(expr_IsFunction,_filter) <--[ Local(item,result, flatlist); Bind(flatlist,Rest(FunctionToList(expr))); Bind(result,{}); ForEach(item,flatlist) Bind(result,Concat(result,VarListAll(item,filter))); result;];30 # VarListAll(_expr,_filter) <-- {};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # VarListSome({}, _look'list) <-- {};10 # VarListSome(expr_IsVariable, _look'list) <-- {expr};15 # VarListSome(expr_IsAtom, _look'list) <-- {};20 # VarListSome(expr_IsFunction, look'list_IsList)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {expr};30 # VarListSome(expr_IsFunction, look'list_IsList) <--RemoveDuplicates( [  Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, VarListSome(item, look'list)); result; ]);";
        scriptMap.put("VarListSome",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(GlobalStack, x)[ GlobalStack := {}; GlobalPop(x_IsAtom) <-- [ Check(Length(GlobalStack)>?0, \"Invariant\", \"GlobalPop: Error: empty GlobalStack\"); MacroBind(x, PopFront(GlobalStack)); Eval(x); ]; HoldArgumentNumber(\"GlobalPop\", 1, 1); GlobalPop() <-- [ Check(Length(GlobalStack)>?0, \"Invariant\", \"GlobalPop: Error: empty GlobalStack\"); PopFront(GlobalStack); ]; GlobalPush(_x) <-- [ Push(GlobalStack, x); x; ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # (list_IsList AddTo _rest) <--[ Local(res); res:={}; ForEach(item,list) [ res := Concat(res,item AddTo rest); ]; res;];30 # (_a'item AddTo list_IsList) <--[ MapSingle({{orig},a'item And? orig},list);];40 # (_a'item AddTo _b) <-- a'item And? b;";
        scriptMap.put("AddTo",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(LocResult) [ Bind(LocResult,True); 10 # LocPredicate(exp_IsAtom) <-- [ Local(tr,result); tr:=patterns; result:=False; While (tr !=? {}) [ If (First(First(tr)) =? exp, [ Bind(LocResult,Eval(First(Rest(First(tr))))); result := True; tr:={}; ], [ tr := Rest(tr); ]); ]; result; ]; 10 # LocPredicate(exp_IsFunction) <-- [ Local(tr,result,head); tr:=patterns; result:=False; While (tr !=? {}) [ Bind(head, First(First(tr))); If (Not?(IsAtom(head)) And? exp[0]=?head[1] And? PatternMatches(head[2], exp), [ Bind(LocResult,Eval(First(Rest(First(tr))))); Bind(result, True); Bind(tr,{}); ], [ Bind(tr, Rest(tr)); ]); ]; result; ]; 20 # LocPredicate(_exp) <-- False; LocChange(_exp) <-- LocResult;]; UnFence(\"LocPredicate\",1);UnFence(\"LocChange\",1);10 # LocProcessSingle({_pat,_post,_exp}) <-- { {pat[0],PatternCreate(pat,post)},exp };20 # LocProcessSingle({pat_IsFunction,_exp}) <-- { {pat[0],PatternCreate(pat,True)},exp };30 # LocProcessSingle({pat_IsAtom,_exp}) <-- { pat,exp };40 # LocProcessSingle(pat_IsFunction <- _exp) <-- [ Local(justPattern, postPredicate);  If(Type(pat) =? \"_\", [  justPattern := pat[1]; postPredicate := pat[2]; ], [  justPattern := pat; postPredicate := True; ] );  { {justPattern[0],PatternCreate(justPattern,postPredicate)},exp };];50 # LocProcessSingle(pat_IsAtom <- _exp) <-- { pat,exp };LocProcess(patterns) :=[ MapSingle(\"LocProcessSingle\",patterns);];CompilePatterns(patterns) := LocPatterns(LocProcess(patterns));";
        scriptMap.put("CompilePatterns",scriptString);
        scriptMap.put("LocProcess",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Where\",{left,right});UnFence(\"Where\",2);10 # (_body Where var_IsAtom == _value) <-- `[Local(@var);@var := @value;@body;];20 # (_body Where (_a And? _b)) <--[ Bind(body,`(@body Where @a)); `(@body Where @b);];30 # (_body Where {}) <-- {};40 # (_body Where list_IsList)_IsList(list[1]) <-- [ Local(head,rest); head:=First(list); rest:=Rest(list); rest:= `(@body Where @rest); `(@body Where @head) : rest; ];50 # (_body Where list_IsList) <-- [ Local(head,rest); While (list !=? {}) [ head:=First(list); body := `(@body Where @head); list:=Rest(list); ]; body; ];60 # (_body Where _var == _value) <-- Subst(var,value)body;";
        scriptMap.put("Where",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"<-\",{left,right});HoldArgument(\"<-\",left);HoldArgument(\"<-\",right);";
        scriptMap.put("<-",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Contradict((_x) - (_y) == 0, (_x) - (_z) == 0)_(y !=? z) <-- True;12 # Contradict((_x) == (_y), (_x) == (_z))_(y !=? z) <-- True;13 # Contradict((_x) - (_y) == 0, (_x) - (_z) >=? 0)_(z >? y) <-- True;14 # Contradict((_x) - (_y) == 0, (_x) - (_z) >? 0)_(z >? y) <-- True;14 # Contradict(Not? (_x) - (_y) >=? 0, (_x) - (_z) >? 0)_(z >? y) <-- True;15 # Contradict(_a, _b) <-- IsEqual(SimpleNegate(a), b);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Groebner(f_IsList) <--[ Local(vars,i,j,S,nr,r); nr:=Length(f); vars:=VarList(f); For(i:=1,i<=?nr,i++) [ f[i] := MakeMultiNomial(f[i],vars); ]; S:={}; For(i:=1,i<?nr,i++) For(j:=i+1,j<=?nr,j++) [ r := (MultiDivide(MultiS(f[i],f[j],f[i]),f)[2]); If(NormalForm(r) !=? 0, S:= r:S); f:=Concat(f,S); S:={}; nr:=Length(f); ]; MapSingle(\"NormalForm\",Concat(f));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContFracEval({}, _rest) <-- rest;10 # ContFracEval({{_n, _m}}, _rest) <-- n+m+rest;15 # ContFracEval({_n}, _rest) <-- n+rest;20 # ContFracEval(list_IsList, _rest)_(IsList(First(list))) <-- First(First(list)) + Rest(First(list)) / ContFracEval(Rest(list), rest);30 # ContFracEval(list_IsList, _rest) <-- First(list) + 1 / ContFracEval(Rest(list), rest);ContFracEval(list_IsList) <-- ContFracEval(list, 0);";
        scriptMap.put("ContFracEval",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContFracList(_n) <-- ContFracList(n, Infinity);10 # ContFracList(_n, _depth)_(depth <=? 0) <-- {};20 # ContFracList(n_IsInteger, _depth) <-- {n};30 # ContFracList(n_IsNumber, _depth) _InNumericMode() <-- NonN(ContFracList(Rationalize(n), depth));40 # ContFracList(n_IsNumber, _depth) <-- ContFracList(Rationalize(n), depth);35 # ContFracList((n_IsNegativeInteger) / (m_IsInteger), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m)-1);40 # ContFracList((n_IsInteger) / (m_IsInteger), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m));";
        scriptMap.put("ContFracList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Decimal( n_IsInteger ) <-- {n,{0}};10 # Decimal( (n_IsPositiveInteger) / (d_IsPositiveInteger) ) <--[ Local(result,rev,first,period,repeat,static); result:={Quotient(n,d)}; Decimal(result,Modulo(n,d),d,350); rev:=DecimalFindPeriod(result); first:=rev[1]; period:=rev[2]; repeat:=result[first .. (first+period-1)]; static:=result[1 .. (first-1)]; DestructiveAppend(static,repeat);];20 # Decimal(_n/_m)_((n/m)<?0) <-- \"-\":Decimal(-n/m);10 # Decimal(_result , _n , _d,_count ) <--[ While(count>?0) [ DestructiveAppend(result,Quotient(10*n,d)); n:=Modulo(10*n,d); count--; ];];DecimalFindPeriod(_list) <--[ Local(period,nr,reversed,first,i); reversed:=Rest(DestructiveReverse(FlatCopy(Rest(list)))); nr:=Length(reversed)>>1; period:=1; first:=reversed[1]; For(i:=1,i<?nr,i++) [ If(reversed[i+1] =? first And? DecimalMatches(reversed,i), [ period:=i; i:=nr; ] ); ]; first:=Length(list)-period; While(first>?1 And? list[first] =? list[first+period]) first--; first++; {first,period};];DecimalMatches(_reversed,_period) <--[ Local(nr,matches,first); nr:=0; matches:=True; first:=1; While((nr<?100) And? matches) [ matches := (matches And? (reversed[first .. (first+period-1)] =? reversed[(first+period) .. (first+2*period-1)])); first:=first+period; nr:=nr+period; ]; matches;];";
        scriptMap.put("Decimal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EigenValues(matrix_IsUpperTriangular) <-- Diagonal(matrix);10 # EigenValues(matrix_IsLowerTriangular) <-- Diagonal(matrix);20 # EigenValues(matrix_IsMatrix) <-- Roots(CharacteristicEquation(matrix,xx));";
        scriptMap.put("EigenValues",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "EigenVectors(_matrix,_eigenvalues) <--[ Local(result,n); n:=Length(eigenvalues); result:={}; ForEach(e,eigenvalues) [ Local(possible); possible:=OldSolve(matrix*MakeVector(k,n)==e*MakeVector(k,n),MakeVector(k,n))[1]; If(Not?(IsZeroVector(possible)), DestructiveAppend(result,possible) ); ]; result;];";
        scriptMap.put("EigenVectors",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GuessRational(_x) <-- GuessRational(x, Floor(1/2*BuiltinPrecisionGet()));GuessRational(x_IsRationalOrNumber, prec_IsInteger) <-- [ Local(denom'estimate, cf, i); denom'estimate := 1; cf := ContFracList(x); For(i:=2, i<=?Length(cf) And? denom'estimate <? 10^prec, i++) [  denom'estimate := denom'estimate * If( cf[i] =? 1, If( i+2<=?Length(cf),  RoundTo(N(Eval(cf[i]+1/(cf[i+1]+1/cf[i+2]))), 3),  RoundTo(N(Eval(cf[i]+1/cf[i+1])), 3) ),  cf[i] ); ]; If (denom'estimate <? 10^prec,  i--  ); i--;   ContFracEval(Take(cf, i));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # IsFreeOf({},_expr) <-- True;2 # IsFreeOf(var_IsList, _expr) <-- And?(IsFreeOf(First(var),expr), IsFreeOf(Rest(var),expr));4 # IsFreeOf(_var,{}) <-- True;5 # IsFreeOf(_var,expr_IsList) <-- And?(IsFreeOf(var,First(expr)), IsFreeOf(var,Rest(expr)));10 # IsFreeOf(_expr,_expr) <-- False;11 # IsFreeOf(_var,expr_IsFunction) <-- IsFreeOf(var,Rest(FunctionToList(expr)));12 # IsFreeOf(_var,_expr) <-- True;";
        scriptMap.put("IsFreeOf",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"IsZeroVector\",{aList}) aList =? ZeroVector(Length(aList));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NearRational(_x) <-- NearRational(x, Floor(1/2*BuiltinPrecisionGet()));15 # NearRational(x_IsRationalOrNumber, prec_IsInteger) <-- [ Local(x1, x2, i, old'prec); old'prec := BuiltinPrecisionGet(); BuiltinPrecisionSet(prec + 8);  x1 := ContFracList(N(Eval(x+10^(-prec)))); x2 := ContFracList(N(Eval(x-10^(-prec))));     For (i:=1, i<=?Length(x1) And? i<=?Length(x2) And? x1[i]=?x2[i], i++ ) True; If( i>?Length(x1),  x1:=x2, If( i>?Length(x2),  True,   x1[i]:=Minimum(x1[i],x2[i]) ) );    If(i>?Length(x1),i:=Length(x1)); x1[i] := x1[i] + If(i=?Length(x1), 0, 1); BuiltinPrecisionSet(old'prec); ContFracEval(Take(x1, i));];20 # NearRational(_z, prec_IsInteger)_ (And?(Im(z)!=?0,IsRationalOrNumber(Im(z)),IsRationalOrNumber(Re(z)))) <--[ Local(rr,ii); rr := Re(z); ii := Im(z); Complex( NearRational(rr,prec), NearRational(ii,prec) );];";
        scriptMap.put("NearRational",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NewLine() <-- WriteN(Nl(),1);20 # NewLine(n_IsPositiveInteger) <-- WriteN(Nl(),n);30 # NewLine(_n) <-- Check(False, \"Argument\", \"The argument must be a positive integer \");";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"WithValue\",{var,val,expr})[ If(IsList(var), ApplyFast(\"MacroLocal\",var), MacroLocal(var) ); ApplyFast(\":=\",{var,val}); Eval(expr);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # BellNumber(n_IsInteger) <-- Sum(k,1,n,StirlingNumber2(n,k));";
        scriptMap.put("BellNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CatalanNumber(_n) <--[ Check( IsPositiveInteger(n), \"Argument\", \"CatalanNumber: Error: argument must be positive\" ); BinomialCoefficient(2*n,n)/(n+1);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # DigitalRoot(n_IsPositiveInteger) <-- If(n%9=?0,9,n%9);";
        scriptMap.put("DigitalRoot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Divisors(0) <-- 0;5 # Divisors(1) <-- 1;10 # Divisors(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"Divisors: argument must be positive integer\"); Local(len,sum,factors,i); sum:=1; factors:=Factors(n); len:=Length(factors); For(i:=1,i<=?len,i++)[ sum:=sum*(factors[i][2]+1); ]; sum;];";
        scriptMap.put("Divisors",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # DivisorsSum(0) <-- 0;5 # DivisorsSum(1) <-- 1;10 # DivisorsSum(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"DivisorsSum: argument must be positive integer\"); Local(factors,i,sum,len,p,k); p:=0;k:=0; factors:={}; factors:=Factors(n); len:=Length(factors); sum:=1; For(i:=1,i<=?len,i++)[ p:=factors[i][1]; k:=factors[i][2]; sum:=sum*(p^(k+1)-1)/(p-1); ]; sum;];";
        scriptMap.put("DivisorsSum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Euler(0) <-- 1;10 # Euler(n_IsOdd) <-- 0;10 # Euler(n_Even?) <-- - Sum(r,0,n/2-1,BinomialCoefficient(n,2*r)*Euler(2*r));10 # Euler(n_IsNonNegativeInteger,_x) <-- Sum(i,0,Round(n/2),BinomialCoefficient(n,2*i)*Euler(2*i)*(x-1/2)^(n-2*i)/2^(2*i));";
        scriptMap.put("Euler",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EulerArray(n_IsInteger) <--[ Local(E,i,sum,r); E:=ZeroVector(n+1); E[1]:=1; For(i:=1,2*i<=?n,i++)[ sum:=0; For(r:=0,r<=?i-1,r++)[ sum:=sum+BinomialCoefficient(2*i,2*r)*E[2*r+1]; ]; E[2*i+1] := -sum; ]; E;];";
        scriptMap.put("EulerArray",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Eulerian(n_IsInteger,k_IsInteger) <-- Sum(j,0,k+1,(-1)^j*BinomialCoefficient(n+1,j)*(k-j+1)^n);";
        scriptMap.put("Eulerian",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FermatNumber\",{n})[ Check(IsPositiveInteger(n), \"Argument\", \"FermatNumber: argument must be a positive integer\"); 2^(2^n)+1;];";
        scriptMap.put("FermatNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # GetPrimePower(n_IsPositiveInteger) <--[ Local(s, factors, new'factors);  factors := TrialFactorize(n, 257);  If( Length(factors) >? 1,   If( factors[1] =? 1 And? Length(factors) =? 2,  factors[2], {n, 1} ),  [ factors := CheckIntPower(n, 257);  If( factors[2] >? 1,   If( IsPrime(factors[1]), factors,  [  new'factors := GetPrimePower(factors[1]);  If( new'factors[2] >? 1, {new'factors[1], new'factors[2]*factors[2]},  {n, 1}  ); ] ),  {n, 1} ); ] );];";
        scriptMap.put("GetPrimePower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HarmonicNumber(n_IsInteger) <-- HarmonicNumber(n,1);HarmonicNumber(n_IsInteger,r_IsPositiveInteger) <--[  if( r=?1 )[ Sum(k,1,n,1/k); ] else [ Sum(k,1,n,1/k^r); ];];";
        scriptMap.put("HarmonicNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IntLog(_x, _base) _ (base<=?1) <-- Undefined;20 # IntLog(_x, _base) <--[ Local(result, step, old'step, factor, old'factor); result := 0; old'step := step := 1; old'factor := factor := base;  While (x >=? factor) [ old'factor := factor; factor := factor*factor; old'step := step; step := step*2; ]; If(x >=? base, [ step := old'step; result := step; x := Quotient(x, old'factor); ], step := 0 );  While (step >? 0 And? x !=? 1) [ step := Quotient(step,2);  factor := base^step; If( x >=? factor, [ x:=Quotient(x, factor); result := result + step; ] ); ]; result;];";
        scriptMap.put("IntLog",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IntNthRoot(_n, 2) <-- Floor(SqrtN(n));20 # IntNthRoot(_n, s_IsInteger) <--[ Local(result, k); GlobalPush(BuiltinPrecisionGet());  k := Quotient(IntLog(n, 2), s);  BuiltinPrecisionSet(2+Quotient(k*3361, 11165));  result := Round(ExpN(DivideN(Internal'LnNum(DivideN(n, 2^(k*s))), s))*2^k); BuiltinPrecisionSet(GlobalPop());  If(result^s>?n, result-1, result);];";
        scriptMap.put("IntNthRoot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsAmicablePair(m_IsPositiveInteger,n_IsPositiveInteger) <-- ( ProperDivisorsSum(m)=?n And? ProperDivisorsSum(n)=?m );";
        scriptMap.put("IsAmicablePair",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsCarmichaelNumber(n_Even?) <-- False;5 # IsCarmichaelNumber(_n)_(n<?561) <-- False;10 # IsCarmichaelNumber(n_IsPositiveInteger) <--[ Local(i,factors,length,carmichael); factors:=Factors(n); carmichael:=True; length:=Length(factors); if( length <? 3)[ carmichael:=False; ] else [ For(i:=1,i<=?length And? carmichael,i++)[  If( Modulo(n-1,factors[i][1]-1) !=? 0, carmichael:=False ); If(factors[i][2]>?1,carmichael:=False);  ]; ]; carmichael;];IsCarmichaelNumber(n_IsList) <-- MapSingle(\"IsCarmichaelNumber\",n);";
        scriptMap.put("IsCarmichaelNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsComposite(1) <-- False;10 # IsComposite(n_IsPositiveInteger) <-- (Not? IsPrime(n));";
        scriptMap.put("IsComposite",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsCoprime(list_IsList) <-- (Lcm(list) =? Product(list));10 # IsCoprime(n_IsInteger,m_IsInteger) <-- (Gcd(n,m) =? 1);";
        scriptMap.put("IsCoprime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsIrregularPrime(p_IsComposite) <-- False;5 # IsIrregularPrime(_p)_(p<?37) <-- False;10 # IsIrregularPrime(p_IsPositiveInteger) <--[ Local(i,irregular); i:=1; irregular:=False; While( 2*i + 1 <? p And? (irregular =? False) )[ If( Abs(Numerator(Bernoulli(2*i))) % p =? 0, irregular:=True ); i++; ]; irregular;];";
        scriptMap.put("IsIrregularPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPerfect(n_IsPositiveInteger) <-- ProperDivisorsSum(n)=?n;";
        scriptMap.put("IsPerfect",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # IsPrime(_n)_(Not? IsInteger(n) Or? n<=?1) <-- False;3 # IsPrime(n_IsInteger)_(n<=?FastIsPrime(0)) <-- IsSmallPrime(n);5 # IsPrime(n_IsPositiveInteger)_(n >? 4 And? Modulo(n^2-1,24)!=?0) <-- False;10 # IsPrime(n_IsPositiveInteger) <-- RabinMiller(n);";
        scriptMap.put("IsPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsPrimePower(n_IsPrime) <-- True;10 # IsPrimePower(0) <-- False;10 # IsPrimePower(1) <-- False;20 # IsPrimePower(n_IsPositiveInteger) <-- (GetPrimePower(n)[2] >? 1);";
        scriptMap.put("IsPrimePower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsQuadraticResidue(_a,_p) <--[ Check( IsInteger(a) And? IsInteger(p) And? p>?2 And? IsCoprime(a,p) And? IsPrime(p), \"Argument\", \"IsQuadraticResidue: Invalid arguments\"); If(a^((p-1)/2) % p =? 1, True, False);];";
        scriptMap.put("IsQuadraticResidue",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # IsSmallPrime(0) <-- False;3 # IsSmallPrime(n_IsInteger) <-- (FastIsPrime(n)>?0);";
        scriptMap.put("IsSmallPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSquareFree(n_IsInteger) <-- ( Moebius(n) !=? 0 );";
        scriptMap.put("IsSquareFree",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsTwinPrime(n_IsPositiveInteger) <-- (IsPrime(n) And? IsPrime(n+2));";
        scriptMap.put("IsTwinPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LegendreSymbol(_a,_p) <--[ Check( IsInteger(a) And? IsInteger(p) And? p>?2 And? IsCoprime(a,p) And? IsPrime(p), \"Argument\", \"LegendreSymbol: Invalid arguments\"); If(IsQuadraticResidue(a,p), 1, -1 );];";
        scriptMap.put("LegendreSymbol",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Moebius(1) <-- 1;10 # Moebius(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"Moebius: argument must be positive integer\"); Local(factors,i,repeat); repeat:=0; factors:=Factors(n); len:=Length(factors); For(i:=1,i<=?len,i++)[ If(factors[i][2]>?1,repeat:=1); ]; If(repeat=?0,(-1)^len,0);];";
        scriptMap.put("Moebius",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# NextPrime(_i) <--[ Until(IsPrime(i)) i := NextPseudoPrime(i); i;];";
        scriptMap.put("NextPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# NextPseudoPrime(i_IsInteger)_(i<=?1) <-- 2;2# NextPseudoPrime(2) <-- 3;3# NextPseudoPrime(i_IsOdd) <--[  i := i+2; If(Modulo(i,3)=?0, i:=i+2, i);];4# NextPseudoPrime(i_Even?) <-- NextPseudoPrime(i-1);";
        scriptMap.put("NextPseudoPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(m,n,r, NthRoot'Table)[NthRoot(m_IsNonNegativeInteger,n_IsInteger)_(n>?1) <--[ Local(r); r:=NthRoot'Restore(m,n); If(Length(r)=?0, [ r:=NthRoot'Calc(m,n); NthRoot'Save(m,n,r); ]); r;];Function(\"NthRoot'Calc\",{m,n})[ Local(i,j,f,r,in); Bind(i,2); Bind(j,Ceil(FastPower(m,N(1.0/n))+1)); Bind(f,1); Bind(r,m);     While(IsLessThan(i,j)) [ Bind(in,PowerN(i,n)); While(IsEqual(ModuloN(r,in),0)) [ Bind(f,MultiplyN(f,i)); Bind(r,QuotientN(r,in)); ]; While(IsEqual(ModuloN(r,i),0))  Bind(r,QuotientN(r,i));   Bind(i,NextPseudoPrime(i)); Bind(j,Ceil(FastPower(r,N(1.0/n))+1)); ];  List(f,QuotientN(m,PowerN(f,n))); ];Function(\"NthRoot'List\",{n})[ If(Length(NthRoot'Table)>?0, [ Local(p,xx); p:=Select(NthRoot'Table, {{xx},First(xx)=?n}); If(Length(p)=?1,Rest(p[1]),List()); ], List());];Function(\"NthRoot'Restore\",{m,n})[ Local(p); p:=NthRoot'List(n); If(Length(p)>?0, [ Local(r,xx); r:=Select(p, {{xx},First(xx)=?m}); If(Length(r)=?1,First(Rest(r[1])),List()); ], List());];Function(\"NthRoot'Save\",{m,n,r})[ Local(p); p:=NthRoot'List(n); If(Length(p)=?0,  DestructiveInsert(NthRoot'Table,1,List(n,List(m,r))), [ Local(rr,xx); rr:=Select(p, {{xx},First(xx)=?m}); If(Length(rr)=?0, [  DestructiveAppend(p,List(m,r)); ],  False); ]);];Function(\"NthRoot'Clear\",{}) SetGlobalLazyVariable(NthRoot'Table,List());NthRoot'Clear();]; ";
        scriptMap.put("NthRoot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NumberToRep( N_IsNumber ) <--[  Local(oldPrec,sgn,assoc,typ,val,prec,rep); oldPrec := BuiltinPrecisionGet(); BuiltinPrecisionSet(300);   sgn := Sign(N);  assoc := DumpNumber(Abs(N));  typ := Assoc(\"type\",assoc)[2];  If( typ =? \"BigDecimal\", [ rep := { sgn*Assoc(\"unscaledValue\",assoc)[2], Assoc(\"precision\", assoc)[2], Assoc(\"scale\", assoc)[2]  }; ], [ Local(val,prec); val := Assoc(\"value\",assoc)[2]; prec := Length(ExpressionToString(val)); rep := { sgn*val, prec }; ] );  BuiltinPrecisionSet(oldPrec); rep;];12 # NumberToRep( N_IsComplex ) <-- [ If(IsZero(Re(N)), {NumberToRep(0.0),NumberToRep(Im(N))}, {NumberToRep(Re(N)),NumberToRep(Im(N))} );];";
        scriptMap.put("NumberToRep",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # PartitionsP(n_IsInteger,0) <-- 0;5 # PartitionsP(n_IsInteger,n_IsInteger) <-- 1;5 # PartitionsP(n_IsInteger,1) <-- 1;5 # PartitionsP(n_IsInteger,2) <-- Floor(n/2);5 # PartitionsP(n_IsInteger,3) <-- Round(n^2/12);6 # PartitionsP(n_IsInteger,k_IsInteger)_(k>?n) <-- 0;10 # PartitionsP(n_IsInteger,k_IsInteger) <-- PartitionsP(n-1,k-1)+PartitionsP(n-k,k);5 # PartitionsP(0) <-- 1;5 # PartitionsP(1) <-- 1;10 # PartitionsP(n_IsInteger)_(n<?250) <-- PartitionsP'recur(n);20 # PartitionsP(n_IsInteger) <-- PartitionsP'HR(n);10 # PartitionsP'HR(n_IsPositiveInteger) <--[ Local(P0, A, lambda, mu, mu'k, result, term, j, k, l, prec, epsilon); result:=0; term:=1;  GlobalPush(BuiltinPrecisionGet());   prec := 2+Quotient(IntNthRoot(Quotient(2*n+2,3),2)*161+117,118); BuiltinPrecisionSet(prec);  epsilon := PowerN(10,-prec)*n*10;   lambda := N(Sqrt(n - 1/24)); mu := N(Pi*lambda*Sqrt(2/3));   P0 := N(1-1/mu)*DivideN(ExpN(mu),(n-DivideN(1,24))*4*SqrtN(3));  A := 0;     For(k:=1, k<=?5+Quotient(IntNthRoot(n,2),2) And? (A=?0 Or? Abs(term)>?epsilon), k++) [  A:=0; For(l:=1,l<=?k,l++) [ If( Gcd(l,k)=?1, A := A + Cos(Pi* (  Sum(j,1,k-1, j*(Modulo(l*j,k)/k-1/2)) - 2*l*n  )/k) ); A:=N(A);  ]; term := If( A=?0,  0, N( A*Sqrt(k)*( [ mu'k := mu/k;  Exp(mu'k-mu)*(mu'k-1) + Exp(-mu'k-mu)*(mu'k+1); ] )/(mu-1) ) ); result := result + term; ]; result := result * P0; BuiltinPrecisionSet(GlobalPop()); Round(result);];10 # PartitionsP1(n_IsPositiveInteger) <-- [ Local(C,A,lambda,m,pa,k,h,term); GlobalPush(BuiltinPrecisionGet());  BuiltinPrecisionSet(10 + Floor(N(Sqrt(n))) ); pa:=0; C:=Pi*Sqrt(2/3)/k; lambda:=Sqrt(m - 1/24); term:=1;   For(k:=1,k<=?5+Floor(SqrtN(n)*0.5) And? ( term=?0 Or? Abs(term)>?0.1) ,k++)[ A:=0; For(h:=1,h<=?k,h++)[ if( Gcd(h,k)=?1 )[ A:=A+Exp(I*Pi*Sum(j,1,k-1,(j/k)*((h*j)/k - Floor((h*j)/k) -1/2))- 2*Pi*I*h*n/k ); ]; ]; If(A!=?0, term:= N(A*Sqrt(k)*(Deriv(m) Sinh(C*lambda)/lambda) Where m==n ),term:=0 ); pa:=pa+term; ]; pa:=N(pa/(Pi*Sqrt(2))); BuiltinPrecisionSet(GlobalPop()); Round(pa); ];PartitionsP'recur(number_IsPositiveInteger) <--[  Local(sign, cache, n, k, pentagonal, P); cache:=ArrayCreate(number+1,1);  n := 1; While(n<?number)  [ n++;  P := 0; k := 1; pentagonal := 1;  sign := 1; While(pentagonal<=?n) [ P := P + (cache[n-pentagonal+1]+If(pentagonal+k<=?n, cache[n-pentagonal-k+1], 0))*sign; pentagonal := pentagonal + 3*k+1; k++; sign := -sign; ]; cache[n+1] := P;  ]; cache[number+1];];PartitionsP'recur(0) <-- 1;";
        scriptMap.put("PartitionsP",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(p, q)[  ProductPrimesTo257() := 2*3*[ If( IsInteger(p), p, p := Product(Select( 5 .. 257, {{q}, Modulo(q^2,24)=?1 And? IsSmallPrime(q)})) ); ];];";
        scriptMap.put("ProductPrimesTo257",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ProperDivisors(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"ProperDivisors: argument must be positive integer\"); Divisors(n)-1;];";
        scriptMap.put("ProperDivisors",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ProperDivisorsSum(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"ProperDivisorsSum: argument must be positive integer\"); DivisorsSum(n)-n;];";
        scriptMap.put("ProperDivisorsSum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Rationalize(aNumber_IsList) <-- Rationalize /@ aNumber;20 # Rationalize( _aNumber ) <--[ Local(result,n,d); result:=Substitute(aNumber,{{x},IsNumber(x) And? Not?(IsInteger(x))},\"RationalizeNumber\"); If(InVerboseMode(),Tell(\"\",result)); If(Length(VarList(aNumber))=?0, [ n:=Numerator(result); If(Type(n)=?\"Numerator\",n:=result); d:=Denominator(result); If(Type(d)=?\"Denominator\",d:=1); result := n*(1/d); ] ); result;];";
        scriptMap.put("Rationalize",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"RationalizeNumber\",{x})[ Check(IsNumber(x), \"Argument\", \"RationalizeNumber: Error: \" : (PipeToString()Write(x)) :\" is not a number\"); Local(n,i,bip,m); n := 1; i := 0; bip := BuiltinPrecisionGet();  While(i<=?bip And? Not?(FloatIsInt?(x))) [ n := n*10;  x := x*10; i := i+1; ]; m := Floor(x+0.5); (m/n);];";
        scriptMap.put("RationalizeNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RepToNumber( rep_IsListOfLists ) <--[  RepToNumber(rep[1])+I*RepToNumber(rep[2]);];12 # RepToNumber( rep_IsList ) <--[  Local(bigInt,precision,scale,strBI,sgn,index,ans); Local(first,secnd,third,LS,numStr); precision := rep[2]; scale := 0; bigInt := rep[1]; precision := rep[2]; sgn := Sign(bigInt); If( Length(rep) >? 2, scale := rep[3] ); strBI := ExpressionToString(Abs(bigInt)); LS := Length(strBI);  If( Length(rep)=?2, [ numStr := strBI; ], [ index := precision-scale; first := strBI[1]; secnd := StringMidGet(2,LS-1,strBI);  third := ExpressionToString(index-1);  if ( index >? 0 ) [ if ( index <? precision ) [  numStr := ConcatStrings(first,\".\",secnd,\"E\",third); ] else if ( index >=? precision ) [  numStr := ConcatStrings(first,\".\",secnd,\"E+\",third); ]; ] else if ( index <? 0 ) [  numStr := ConcatStrings(first,\".\",secnd,\"E\",third); ] else [  first := \"0.\" ;  secnd := strBI; numStr := ConcatStrings(first,secnd); ]; ] ); ans := sgn * ToAtom(numStr);  ans;];";
        scriptMap.put("RepToNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Repunit(0) <-- 0;Repunit(n_IsPositiveInteger) <--[ (10^n-1)/9;];";
        scriptMap.put("Repunit",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RoundToPlace( N_IsDecimal, place_IsInteger ) <--[  Local(rep,sgn,oldInt,oldPrec,oldScale,oldPlaces,strOInt,LS); Local(newInt,newScale,newRep,ans); sgn := Sign(N); rep := NumberToRep( Abs(N) ); oldInt := rep[1]; oldPrec := rep[2]; oldScale := rep[3]; oldPlaces:= oldPrec - oldScale; strOInt := ExpressionToString(oldInt); LS := Length(strOInt);           If(oldPlaces+place>?0, ans := RoundToPrecision(N,oldPlaces+place), ans := 0. ); ans;];15 # RoundToPlace( N_IsInteger, place_IsInteger )_(place <=? 0) <--[  Local(oldRep,oldPrec,decN,newDecN,ans); oldRep := NumberToRep(N); oldPrec := oldRep[2]; decN := N*1.0; newDecN := RoundToPlace( decN, place );     If( place <=? oldPrec,  ans := Round(newDecN), ans := Round(newDecN*10^(place-oldPrec)) ); ans;];20 # RoundToPlace( N_IsComplex, place_IsInteger )_(Not? IsInteger(N)) <--[  Local(rr,ii); rr := Re(N); ii := Im(N); Complex(RoundToPlace(rr,place),RoundToPlace(ii,place));];";
        scriptMap.put("RoundToPlace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RoundToPrecision( N_IsDecimal, newPrec_IsPositiveInteger ) <--[  Local(rep,sgn,oldInt,oldPrec,oldScale,strOInt,LS,BIP0); Local(newInt,newScale,newRep,ans); BIP0 := BuiltinPrecisionGet(); sgn := Sign(N); rep := NumberToRep( If(sgn<?0,-N,N) ); oldInt := rep[1]; oldPrec := rep[2]; oldScale := rep[3]; If( newPrec >? oldPrec, BuiltinPrecisionSet(newPrec) ); strOInt := ExpressionToString(oldInt); LS := Length(strOInt);           Local(first,secnd,rem,ad); if ( newPrec =? oldPrec ) [ ans := N; ] else if ( newPrec <? oldPrec ) [ first := StringMidGet(1, newPrec, strOInt);  secnd := StringMidGet(newPrec+1, LS-newPrec, strOInt); rem := ToAtom(ConcatStrings(\".\",secnd)); ad := If(rem>=?0.5, 1, 0 ); newInt := sgn * ( ToAtom(first) + ad ); newScale := oldScale - ( oldPrec - newPrec );  newRep := {newInt,newPrec,newScale}; ans := RepToNumber(newRep);         ] else [  Local(diffPrec); diffPrec := oldPrec - newPrec; newInt := sgn * ToAtom(strOInt) * 10^(-diffPrec) ; newScale := oldScale - diffPrec; newRep := {newInt,newPrec,newScale};  ans := RepToNumber(newRep); ]; BuiltinPrecisionSet(BIP0); ans;];15 # RoundToPrecision( N_IsInteger, newPrec_IsPositiveInteger ) <--[  Local(oldRep,oldPrec,decN,newDecN,ans); oldRep := NumberToRep(N); oldPrec := oldRep[2]; decN := N*1.0; newDecN := RoundToPrecision( decN, newPrec );     If( newPrec <=? oldPrec,  ans := Round(newDecN), ans := Round(newDecN*10^(newPrec-oldPrec)) ); ans;];20 # RoundToPrecision( N_IsComplex, newPrec_IsPositiveInteger ) <--[  Local(rr,ii); rr := Re(N); ii := Im(N); Complex(RoundToPrecision(rr,newPrec),RoundToPrecision(ii,newPrec));];";
        scriptMap.put("RoundToPrecision",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # StirlingNumber1(n_IsInteger,0) <-- If(n=?0,1,0);10 # StirlingNumber1(n_IsInteger,1) <-- (-1)^(n-1)*(n-1)!;10 # StirlingNumber1(n_IsInteger,2) <-- (-1)^n*(n-1)! * HarmonicNumber(n-1);10 # StirlingNumber1(n_IsInteger,n-1) <-- -BinomialCoefficient(n,2);10 # StirlingNumber1(n_IsInteger,3) <-- (-1)^(n-1)*(n-1)! * (HarmonicNumber(n-1)^2 - HarmonicNumber(n-1,2))/2;20 # StirlingNumber1(n_IsInteger,m_IsInteger) <-- Sum(k,0,n-m,(-1)^k*BinomialCoefficient(k+n-1,k+n-m)*BinomialCoefficient(2*n-m,n-k-m)*StirlingNumber2(k-m+n,k));";
        scriptMap.put("StirlingNumber1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # StirlingNumber2(n_IsInteger,0) <-- If(n=?0,1,0);20 # StirlingNumber2(n_IsInteger,k_IsInteger) <-- Sum(i,0,k-1,(-1)^i*BinomialCoefficient(k,i)*(k-i)^n)/ k! ;";
        scriptMap.put("StirlingNumber2",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Totient(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"Totient: argument must be positive integer\"); Local(i,sum,factors,len); sum:=n; factors:=Factors(n); len:=Length(factors); For(i:=1,i<=?len,i++)[ sum:=sum*(1-1/factors[i][1]); ]; sum;];";
        scriptMap.put("Totient",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FactorGaussianInteger\",{x}) [ Check( IsGaussianInteger(x), \"Argument\", \"FactorGaussianInteger: argument must be a Gaussian integer\"); Local(re,im,norm,a,b,d,i,j); re:=Re(x);im:=Im(x); If(re<?0, re:=(-re) ); If(im<?0, im:=(-im) ); norm:=re^2+im^2; if( IsComposite(norm) )[ For(i:=0, i^2 <=? norm, i++ )[  For(j:=0, i^2 + j^2 <=? norm, j++)[  if( Not?( (i =? re And? j =? im) Or? (i =? im And? j =? re) ) )[  d:=i^2+j^2; if( d >? 1 )[ a := re * i + im * j; b := im * i - re * j; While( (Modulo(a,d) =? 0) And? (Modulo(b,d) =? 0) ) [ FactorGaussianInteger(Complex(i,j)); re:= a/d; im:= b/d; a := re * i + im * j; b := im * i - re * j; norm := re^2 + im^2; ]; ]; ]; ]; ]; If( re !=? 1 Or? im !=? 0, Echo(Complex(re,im)) ); ] else [ Echo(Complex(re,im)); ];];";
        scriptMap.put("FactorGaussianInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GaussianFactorPrime(p_IsInteger) <-- [ Local(a,i); a := 1; For (i:=2,i<=?(p-1)/2,i++) a := Modulo(a*i,p); GaussianGcd(a+I,p);];";
        scriptMap.put("GaussianFactorPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "AddGaussianFactor(L_IsList,z_IsGaussianInteger,p_IsGaussianInteger) <--[ Local(e); e :=0; While (IsGaussianInteger(z:= z/p)) e++; If (e !=? 0, DestructiveAppend(L,{p,e}));];10 # GaussianFactors(n_IsInteger) <--[  Local(ifactors,gfactors,p,alpha); ifactors := FactorizeInt(n);  gfactors := {}; ForEach(p,ifactors) [ If (p[1]=?2, [ DestructiveAppend(gfactors,{1+I,p[2]}); DestructiveAppend(gfactors,{1-I,p[2]}); ]); If (Modulo(p[1],4)=?3, DestructiveAppend(gfactors,p)); If (Modulo(p[1],4)=?1, [ alpha := GaussianFactorPrime(p[1]); DestructiveAppend(gfactors,{alpha,p[2]}); DestructiveAppend(gfactors,{Conjugate(alpha),p[2]}); ]); ];gfactors;];20 # GaussianFactors(z_IsGaussianInteger) <--[ Local(n,nfactors,gfactors,p); gfactors :={}; n := GaussianNorm(z); nfactors := Factors(n); ForEach(p,nfactors) [ If (p[1]=?2, [ AddGaussianFactor(gfactors,z,1+I);]); If (Modulo(p[1],4)=?3, AddGaussianFactor(gfactors,z,p[1])); If (Modulo(p[1],4)=?1, [ Local(alpha); alpha := GaussianFactorPrime(p[1]); AddGaussianFactor(gfactors,z,alpha); AddGaussianFactor(gfactors,z,Conjugate(alpha)); ]); ]; gfactors;];";
        scriptMap.put("GaussianFactors",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # GaussianGcd(n_IsGaussianInteger,m_IsGaussianInteger) <--[ If(N(Abs(m))=?0,n, GaussianGcd(m,n - m*Round(n/m) ) );];";
        scriptMap.put("GaussianGcd",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GaussianMod(z_IsGaussianInteger,w_IsGaussianInteger) <-- z - w * Round(z/w);";
        scriptMap.put("GaussianMod",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GaussianNorm(z_IsGaussianInteger) <-- Re(z)^2+Im(z)^2;";
        scriptMap.put("GaussianNorm",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsGaussianInteger(x_IsList) <-- False;10 # IsGaussianInteger(x_IsComplex) <-- (IsInteger(Re(x)) And? IsInteger(Im(x)));15 # IsGaussianInteger(_x) <-- False;";
        scriptMap.put("IsGaussianInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"IsGaussianPrime\",{x})[ if( IsGaussianInteger(x) )[ if( IsZero(Re(x)) )[ ( Abs(Im(x)) % 4 =? 3 And? IsPrime(Abs(Im(x))) ); ] else if ( IsZero(Im(x)) ) [ ( Abs(Re(x)) % 4 =? 3 And? IsPrime(Abs(Re(x))) ); ] else [ IsPrime(Re(x)^2 + Im(x)^2); ]; ] else [ False; ];];";
        scriptMap.put("IsGaussianPrime",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsGaussianUnit(z_IsGaussianInteger) <-- GaussianNorm(z)=?1;";
        scriptMap.put("IsGaussianUnit",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DivisorsList(n_IsPositiveInteger) <--[ Local(nFactors,f,result,oldresult,x); nFactors:= Factors(n); result := {1}; ForEach (f,nFactors) [ oldresult := result; For (k:=1,k<=?f[2],k++) ForEach (x,oldresult) result:=Append(result,x*f[1]^k); ]; result;];";
        scriptMap.put("DivisorsList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # JacobiSymbol(_a, 1) <-- 1;15 # JacobiSymbol(0, _b) <-- 0;18 # JacobiSymbol(_a, _b) _ (Gcd(a,b)>?1) <-- 0;20 # JacobiSymbol(_a, b_IsOdd)_(a>=?Abs(b) Or? a<?0) <-- JacobiSymbol(Modulo(a,Abs(b)),Abs(b));30 # JacobiSymbol(a_Even?, b_IsOdd) <--[ Local(c, s);  {c,s}:=FindPrimeFactorSimple(a, 2);  If(Modulo(s,2)=?1 And? Abs(Modulo(b,8)-4)=?1, -1, 1) * JacobiSymbol(c,b);];40 # JacobiSymbol(a_IsOdd, b_IsOdd) <-- If(Modulo(a,4)=?3 And? Modulo(b,4)=?3, -1, 1) * JacobiSymbol(b,a);";
        scriptMap.put("JacobiSymbol",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MoebiusDivisorsList(n_IsPositiveInteger) <--[ Local(nFactors,f,result,oldresult,x); nFactors:= Factors(n); result := {{1,1}}; ForEach (f,nFactors) [ oldresult := result; ForEach (x,oldresult) result:=Append(result,{x[1]*f[1],-x[2]}); ]; result;];";
        scriptMap.put("MoebiusDivisorsList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RamanujanSum(k_IsPositiveInteger,0) <-- Totient(k);20 # RamanujanSum(k_IsPositiveInteger,n_IsPositiveInteger) <--[ Local(s,gcd,d); s:= 0; gcd := Gcd(n,k); ForEach (d,DivisorsList(gcd)) s:=s+d*Moebius(k/d); s;];";
        scriptMap.put("RamanujanSum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SquareFreeDivisorsList(n_IsPositiveInteger) <--[ Local(nFactors,f,result,oldresult,x); nFactors:= Factors(n); result := {1}; ForEach (f,nFactors) [ oldresult := result; ForEach (x,oldresult) result:=Append(result,x*f[1]); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EvaluateHornerScheme({}, _x) <-- 0;10 # EvaluateHornerScheme({_coeffs}, _x) <-- coeffs;20 # EvaluateHornerScheme(coeffs_IsList, _x) <-- First(coeffs)+x*EvaluateHornerScheme(Rest(coeffs), x);OrthoP(n_IsInteger, _x)_(n>=?0) <-- OrthoP(n, 0, 0, x);OrthoP(n_IsInteger, a_IsRationalOrNumber, b_IsRationalOrNumber, _x)_(n>=?0 And? a>? -1 And? b>? -1) <-- OrthoPoly(\"Jacobi\", n, {a, b}, x);OrthoG(n_IsInteger, a_IsRationalOrNumber, _x)_(n>=?0 And? a>? -1/2) <-- OrthoPoly(\"Gegenbauer\", n, {a}, x);OrthoH(n_IsInteger, _x)_(n>=?0) <-- OrthoPoly(\"Hermite\", n, {}, x);OrthoL(n_IsInteger, a_IsRationalOrNumber, _x)_(n>=?0 And? a>? -1) <-- OrthoPoly(\"Laguerre\", n, {a}, x);OrthoT(n_IsInteger, _x)_(n>=?0) <-- OrthoPoly(\"Tscheb1\", n, {}, x);OrthoU(n_IsInteger, _x)_(n>=?0) <-- OrthoPoly(\"Tscheb2\", n, {}, x);OrthoPSum(c_IsList, _x) <-- OrthoP(c, 0, 0, x);OrthoPSum(c_IsList, a_IsRationalOrNumber, b_IsRationalOrNumber, _x)_(a>? -1 And? b>? -1) <-- OrthoPolySum(\"Jacobi\", c, {a, b}, x);OrthoGSum(c_IsList, a_IsRationalOrNumber, _x)_(a>? -1/2) <-- OrthoPolySum(\"Gegenbauer\", c, {a}, x);OrthoHSum(c_IsList, _x) <-- OrthoPolySum(\"Hermite\", c, {}, x);OrthoLSum(c_IsList, a_IsRationalOrNumber, _x)_(a>? -1) <-- OrthoPolySum(\"Laguerre\", c, {a}, x);OrthoTSum(c_IsList, _x) <-- OrthoPolySum(\"Tscheb1\", c, {}, x);OrthoUSum(c_IsList, _x) <-- OrthoPolySum(\"Tscheb2\", c, {}, x);LocalSymbols(knownOrthoPoly) [ knownOrthoPoly := Hold({ {\"Jacobi\", {{n, p}, {{(p[1]-p[2])/2, 1+(p[1]+p[2])/2}, {(2*n+p[1]+p[2]-1)*((p[1])^2-(p[2])^2)/(2*n*(n+p[1]+p[2])*(2*n+p[1]+p[2]-2)), (2*n+p[1]+p[2]-1)*(2*n+p[1]+p[2])/(2*n*(n+p[1]+p[2])), -(n+p[1]-1)*(n+p[2]-1)*(2*n+p[1]+p[2])/(n*(n+p[1]+p[2])*(2*n+p[1]+p[2]-2))}}}}, {\"Gegenbauer\", {{n, p}, {{0, 2*p[1]}, {0, 2+2*(p[1]-1)/n, -1-2*(p[1]-1)/n}}}}, {\"Laguerre\", {{n, p}, {{p[1]+1, -1}, {2+(p[1]-1)/n, -1/n, -1-(p[1]-1)/n}}}}, {\"Hermite\", {{n, p}, {{0,2}, {0, 2, -2*(n-1)}}}}, {\"Tscheb1\", {{n, p}, {{0,1}, {0,2,-1}}}}, {\"Tscheb2\", {{n, p}, {{0,2}, {0,2,-1}}}} }); KnownOrthoPoly() := knownOrthoPoly;]; 10 # OrthoPoly(name_IsString, _n, p_IsList, x_IsRationalOrNumber) _ (KnownOrthoPoly()[name] !=? Empty) <-- OrthoPolyNumeric(name, n, p, x);20 # OrthoPoly(name_IsString, _n, p_IsList, _x) _ (KnownOrthoPoly()[name] !=? Empty) <-- EvaluateHornerScheme(OrthoPolyCoeffs(name, n, p), x);10 # OrthoPolySum(name_IsString, c_IsList, p_IsList, x_IsRationalOrNumber) _ (KnownOrthoPoly()[name] !=? Empty) <-- OrthoPolySumNumeric(name, c, p, x);20 # OrthoPolySum(name_IsString, c_IsList, p_IsList, _x) _ (KnownOrthoPoly()[name] !=? Empty) <-- EvaluateHornerScheme(OrthoPolySumCoeffs(name, c, p), x);OrthoPolyNumeric(name_IsString, n_IsInteger, p_IsList, _x) <-- [ Local(value1, value2, value3, ruleCoeffs, index); value1 := 1; ruleCoeffs := Apply(KnownOrthoPoly()[name], {n, p})[1]; value2 := ruleCoeffs[1] + x*ruleCoeffs[2]; index := 1;  While(index<?n) [ index := index + 1; ruleCoeffs := Apply(KnownOrthoPoly()[name], {index, p})[2]; value3 := (ruleCoeffs[1] + x*ruleCoeffs[2])*value2 + ruleCoeffs[3]*value1; value1 := value2; value2 := value3; ]; value2;];OrthoPolySumNumeric(name_IsString, c_IsList, p_IsList, _x) <-- [ Local(value1, value2, value3, ruleCoeffs, ruleCoeffs1, index); value1 := 0; value2 := 0; index := Length(c) - 1;  While(index>=?1) [ ruleCoeffs := Apply(KnownOrthoPoly()[name], {index+1, p})[2]; ruleCoeffs1 := Apply(KnownOrthoPoly()[name], {index+2, p})[2]; value3 := (ruleCoeffs[1] + x*ruleCoeffs[2])*value2 + ruleCoeffs1[3]*value1 + c[index+1]; value1 := value2; value2 := value3; index := index - 1; ];  ruleCoeffs := Apply(KnownOrthoPoly()[name], {1, p})[1]; ruleCoeffs1 := Apply(KnownOrthoPoly()[name], {2, p})[2]; value2 := (ruleCoeffs[1] + x*ruleCoeffs[2])*value2 + ruleCoeffs1[3]*value1 + c[1]; value2;];10 # OrthoPolyCoeffs(name_IsString, 0, p_IsList) <-- {1};10 # OrthoPolyCoeffs(name_IsString, 1, p_IsList) <-- Apply(KnownOrthoPoly()[name], {1, p})[1];20 # OrthoPolyCoeffs(name_IsString, n_IsInteger, p_IsList)_(n>?1) <-- [ Local(ruleCoeffs, tmpCoeffs, newCoeffs, prevCoeffs, index, jndex, tmptmpCoeffs, prevCoeffsA, newCoeffsA, tmpCoeffsA);  prevCoeffsA := ZeroVector(n+1); newCoeffsA := ZeroVector(n+1); tmpCoeffsA := ZeroVector(n+1);  prevCoeffs := prevCoeffsA; newCoeffs := newCoeffsA; tmpCoeffs := tmpCoeffsA;  prevCoeffs[1] := 1; ruleCoeffs := Apply(KnownOrthoPoly()[name], {n, p})[1]; newCoeffs[1] := ruleCoeffs[1]; newCoeffs[2] := ruleCoeffs[2];  index := 1;  While(index <? n) [ index := index + 1;  ruleCoeffs := Apply(KnownOrthoPoly()[name], {index, p})[2]; tmpCoeffs[1] := ruleCoeffs[1]*newCoeffs[1] + ruleCoeffs[3]*prevCoeffs[1];  For(jndex:=2, jndex <=? index, jndex:=jndex+1) [ tmpCoeffs[jndex] := ruleCoeffs[1]*newCoeffs[jndex] + ruleCoeffs[3]*prevCoeffs[jndex] + ruleCoeffs[2]*newCoeffs[jndex-1]; ]; tmpCoeffs[index+1] := ruleCoeffs[2]*newCoeffs[index]; tmptmpCoeffs := prevCoeffs; prevCoeffs := newCoeffs; newCoeffs := tmpCoeffs; tmpCoeffs := tmptmpCoeffs; ]; newCoeffs;];OrthoPolySumCoeffs(name_IsString, c_IsList, p_IsList) <-- [ Local(n, ruleCoeffs, ruleCoeffs1, tmpCoeffs, newCoeffs, prevCoeffs, index, jndex, tmptmpCoeffs, prevCoeffsA, newCoeffsA, tmpCoeffsA);  n := Length(c) - 1;  prevCoeffsA := ZeroVector(n+1); newCoeffsA := ZeroVector(n+1); tmpCoeffsA := ZeroVector(n+1);  prevCoeffs := prevCoeffsA; newCoeffs := newCoeffsA; tmpCoeffs := tmpCoeffsA;   For(index:=n, index >=? 1, index:=index-1) [  ruleCoeffs := Apply(KnownOrthoPoly()[name], {index+1, p})[2]; ruleCoeffs1 := Apply(KnownOrthoPoly()[name], {index+2, p})[2]; tmpCoeffs[1] := c[index+1] + ruleCoeffs[1]*newCoeffs[1] + ruleCoeffs1[3]*prevCoeffs[1];  For(jndex:=2, jndex <=? n-index, jndex:=jndex+1) [ tmpCoeffs[jndex] := ruleCoeffs[1]*newCoeffs[jndex] + ruleCoeffs1[3]*prevCoeffs[jndex] + ruleCoeffs[2]*newCoeffs[jndex-1]; ]; If(n-index>?0, tmpCoeffs[n-index+1] := ruleCoeffs[2]*newCoeffs[n-index]); tmptmpCoeffs := prevCoeffs; prevCoeffs := newCoeffs; newCoeffs := tmpCoeffs; tmpCoeffs := tmptmpCoeffs; ];  index:=0; ruleCoeffs := Apply(KnownOrthoPoly()[name], {index+1, p})[1]; ruleCoeffs1 := Apply(KnownOrthoPoly()[name], {index+2, p})[2]; tmpCoeffs[1] := c[index+1] + ruleCoeffs[1]*newCoeffs[1] + ruleCoeffs1[3]*prevCoeffs[1];  For(jndex:=2, jndex <=? n-index, jndex:=jndex+1) [ tmpCoeffs[jndex] := ruleCoeffs[1]*newCoeffs[jndex] + ruleCoeffs1[3]*prevCoeffs[jndex] + ruleCoeffs[2]*newCoeffs[jndex-1]; ]; tmpCoeffs[n-index+1] := ruleCoeffs[2]*newCoeffs[n-index]; tmpCoeffs;];8# OrthoPolyCoeffs(\"Jacobi\", n_IsInteger, {0,0}) <--[ Local(i, result); result := ZeroVector(n+1); result[n+1] := (2*n-1)!! /n!;  i := 1; While(2*i<=?n) [  result[n+1-2*i] := -(result[n+3-2*i]*(n-2*i+1)*(n-2*i+2)) / ((2*n-2*i+1)*2*i); i++; ]; result;];OrthoPolyCoeffs(\"Hermite\", n_IsInteger, {}) <-- HermiteCoeffs(n);HermiteCoeffs(n_Even?)_(n>?0) <--[ Local(i, k, result); k := Quotient(n,2); result := ZeroVector(n+1); result[1] := (-2)^k*(n-1)!!;  For(i:=1,i<=?k,i++)  result[2*i+1] := Quotient(-2*result[2*i-1] * (k-i+1), (2*i-1)*i);  result;];HermiteCoeffs(n_IsOdd)_(n>?0) <--[ Local(i, k, result); k := Quotient(n,2); result := ZeroVector(n+1); result[2] := 2*(-2)^k*(n!!);  For(i:=1,i<=?k,i++)  result[2*i+2] := Quotient(-2*result[2*i] * (k-i+1), i*(2*i+1));  result;];OrthoPolyCoeffs(\"Laguerre\", n_IsInteger, {_k}) <--[ Local(i, result); result := ZeroVector(n+1); result[n+1] := (-1)^n/n!;  For(i:=n,i>=?1,i--)  result[i] := -(result[i+1]*i*(k+i))/(n-i+1); result;];OrthoPolyCoeffs(\"Tscheb1\", n_IsInteger, {}) <-- ChebTCoeffs(n);OrthoPolyCoeffs(\"Tscheb2\", n_IsInteger, {}) <-- ChebUCoeffs(n);1 # ChebTCoeffs(0) <-- {1};2 # ChebTCoeffs(n_IsInteger) <--[ Local(i, result); result := ZeroVector(n+1); result[n+1] := 2^(n-1);  i := 1; While(2*i<=?n) [  result[n+1-2*i] := -(result[n+3-2*i]*(n-2*i+2)*(n-2*i+1)) / ((n-i)*4*i); i++; ]; result;];1 # ChebUCoeffs(0) <-- {1};2 # ChebUCoeffs(n_IsInteger) <--[ Local(i, result); result := ZeroVector(n+1); result[n+1] := 2^n;  i := 1; While(2*i<=?n) [  result[n+1-2*i] := -(result[n+3-2*i]*(n-2*i+2)*(n-2*i+1)) / ((n-i+1)*4*i); i++; ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PartFracExpand(_g,_f) <--[ Local(mlist,vlist,res); mlist:=Map(\"^\",Transpose(Factors(f))); vlist:=Map(\"Rem\",{FillList(g,Length(mlist)),mlist}); If(IsInteger(f), res:=ChineseRemainderInteger(mlist,vlist)[2], res:=ChineseRemainderPoly(mlist,vlist)[2] ); Local(result,divlist); divlist:=Map(\"/\",{res,mlist}); If(Length(divlist)<?2, Add(divlist), [ result:=divlist[1]; ForEach(item,Rest(divlist)) [ result:=ListToFunction({ToAtom(\"+\"),result,item}); ]; result; ]);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"_\",{a});Rulebase(\"_\",{a,b});Rulebase(\"DefinePattern\",{leftOperand, rightOperand, rulePrecedence, postPredicate});Rule(\"DefinePattern\",4,9,IsEqual(Type(leftOperand),\"_\"))[ DefinePattern(leftOperand[1], rightOperand, rulePrecedence, leftOperand[2]);];Rule(\"DefinePattern\",4,10,True)[ Local(patternFlat,patternVariables, pattern, patternOperator, arg, arity);  Bind(patternFlat, FunctionToList(leftOperand));   Bind(patternVariables, Rest(patternFlat));   Bind(patternOperator,ToString(First(patternFlat)));   Bind(arity,Length(patternVariables));      If(Not?(RulebaseDefined(patternOperator,arity)), [ MacroRulebase(patternOperator,MakeVector(arg,arity)); ] );  Bind(pattern,PatternCreate(patternVariables,postPredicate));  MacroRulePattern(patternOperator,arity,rulePrecedence, pattern)rightOperand;  True;];";
        scriptMap.put("DefinePattern",scriptString);
        scriptMap.put("_",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"MakeVector\",{vec,dimension});Rule(\"MakeVector\",2,1,True)[ Local(res,i); res:={}; i:=1; Bind(dimension,AddN(dimension,1)); While(IsLessThan(i,dimension)) [ DestructiveInsert(res,1,ToAtom(ConcatStrings(ToString(vec),ToString(i)))); Bind(i,AddN(i,1)); ]; DestructiveReverse(res);];";
        scriptMap.put("MakeVector",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"<--\",{leftOperand,rightOperand});Rule(\"<--\",2,1,IsEqual(Type(leftOperand),\"#\"))[ DefinePattern(leftOperand[2],rightOperand,leftOperand[1],True);];Rule(\"<--\",2,2,IsFunction(leftOperand))[ DefinePattern(leftOperand,rightOperand,0,True);];HoldArgument(\"<--\",leftOperand);HoldArgument(\"<--\",rightOperand);";
        scriptMap.put("<--",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "OptionsListToHash(list) :=[ Local(item, result); result := {}; ForEach(item, list) If( IsFunction(item) And? (Type(item) =? \"->\" ) And? IsAtom(item[1]), result[ToString(item[1])] := If( IsAtom(item[2]) And? Not? IsNumber(item[2]) And? Not? IsString(item[2]), ToString(item[2]), item[2] ), Echo({\"OptionsListToHash: Error: item \", item, \" is not of the format a -> b.\"}) );  result;];HoldArgumentNumber(\"OptionsListToHash\", 1, 1);";
        scriptMap.put("OptionsListToHash",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RemoveRepeated({}) <-- {};10 # RemoveRepeated({_x}) <-- {x};20 # RemoveRepeated(list_IsList) <-- [ Local(i, done); done := False; For(i:=0, Not? done, i++) [ While(i<?Length(list) And? list[i]=?list[i+1]) DestructiveDelete(list, i); If(i=?Length(list), done := True); ]; list;];";
        scriptMap.put("RemoveRepeated",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WriteDataItem(tuple_IsList, _options'hash) <--[ Local(item); If(  IsNumericList(tuple), ForEach(item,tuple) [ Write(item); Space(); ] ); NewLine();];";
        scriptMap.put("WriteDataItem",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(options)[ options := { {\"default\", \"data\"}, {\"data\", \"Plot2DData\"}, {\"java\", \"Plot2DJava\"}, {\"geogebra\", \"Plot2DGeoGebra\"}, {\"jfreechart\", \"Plot2DJFreeChart\"},}; Plot2DOutputs() := options;];Plot2DData(values_IsList, _optionsHash) <-- values;Plot2DJava(values_IsList, _optionsHash) <--[ Local(result,count); count := 0; result:=\"$plot2d:\"; result := result:\" pensize 2.0 \"; ForEach(function,values) [ result := result:ColorForGraphNr(count); count++; result:=result:\" lines2d \":ToString(Length(function)); function:=Select(function, Lambda({item},item[2] !=? Undefined)); ForEach(item,function) [ result := result:\" \":ToString(item[1]):\" \":ToString(item[2]):\" \"; ]; ]; WriteString(result:\"$\"); True;];10 # ColorForGraphNr(0) <-- \" pencolor 64 64 128 \";10 # ColorForGraphNr(1) <-- \" pencolor 128 64 64 \";10 # ColorForGraphNr(2) <-- \" pencolor 64 128 64 \";20 # ColorForGraphNr(_count) <-- ColorForGraphNr(Modulo(count,3));Plot2DGeogebra(values_IsList, _optionsHash) <--[ Local(result,count); count := 0; result:=\"\"; ForEach(function,values) [ function:=Select(function, Lambda({item},item[2] !=? Undefined)); ForEach(item,function) [ result := result:\"(\":ToString(item[1]):\",\":ToString(item[2]):\")\":Nl(); ]; ]; WriteString(result); True;];Plot2DJFreeChart(values_IsList, _optionsHash) <--[ Local(rangeList, domainList, function, allProcessedFunctionData, lineChartCallListForm);     ForEach(name, {\"xrange\", \"xname\", \"yname\", \"output\", \"precision\", \"points\", \"depth\"}) [ AssocDelete(optionsHash, name); ];     allProcessedFunctionData := {};  ForEach(function,values) [ rangeList := {};  domainList := {};  function := Select(function, Lambda({item},item[2] !=? Undefined));  ForEach(item,function) [ rangeList := Append(rangeList, item[1]);  domainList := Append(domainList, item[2]); ];  allProcessedFunctionData := Append(allProcessedFunctionData, rangeList); allProcessedFunctionData := Append(allProcessedFunctionData, domainList);  ];   lineChartCallListForm := {LineChart, allProcessedFunctionData };     ForEach(key, AssocIndices(optionsHash)) [ lineChartCallListForm := Append(lineChartCallListForm, Apply(\"->\", {key, optionsHash[key]})); ];     Eval(ListToFunction(lineChartCallListForm)); ];";
        scriptMap.put("Plot2DOutputs",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(var, func, range, option, options'list, delta, options'hash, c, fc, all'values, dummy)[Function() Plot2D(func);Function() Plot2D(func, range);Function() Plot2D(func, range, options, ...);1 # Plot2D(_func) <-- (\"Plot2D\" @ {func, -5:5});2 # Plot2D(_func, _range) <-- (\"Plot2D\" @ {func, range, {}});3 # Plot2D(_func, _range, option_IsFunction) _ (Type(option) =? \"->\" ) <-- (\"Plot2D\" @ {func, range, {option}});5 # Plot2D(_func, _range, options'list_IsList)_(Not? IsList(func)) <-- (\"Plot2D\" @ {{func}, range, options'list});4 # Plot2D(func'list_IsList, _range, options'list_IsList) <--[ Local(var, func, delta, options'hash, c, fc, all'values, dummy); all'values := {}; options'hash := \"OptionsListToHash\" @ {options'list};    options'hash[\"xname\"] := \"\";  options'hash[\"yname\"] := {};  If ( Type(range) =? \"->\",  [  options'hash[\"xname\"] := ToString(range[1]); range := range[2]; ] ); If( Type(range) =? \":\",  range := N(Eval({range[1], range[2]})) );  If( options'hash[\"points\"] =? Empty, options'hash[\"points\"] := 23 ); If( options'hash[\"depth\"] =? Empty, options'hash[\"depth\"] := 5 ); If( options'hash[\"precision\"] =? Empty, options'hash[\"precision\"] := 0.0001 ); If( options'hash[\"output\"] =? Empty Or? IsString(options'hash[\"output\"]) And? Plot2DOutputs()[options'hash[\"output\"]] =? Empty, options'hash[\"output\"] := Plot2DOutputs()[\"default\"] );  If( options'hash[\"output\"] =? \"datafile\" And? options'hash[\"filename\"] =? Empty, options'hash[\"filename\"] := \"output.data\" );   options'hash[\"points\"] := N(Eval(Quotient(options'hash[\"points\"]+3, 4)));   options'hash[\"precision\"] := N(Eval(options'hash[\"precision\"]));   options'hash[\"xrange\"] := {range[1], range[2]};   delta := N(Eval( (range[2] - range[1]) / (options'hash[\"points\"]) ));   Check(IsNumber(range[1]) And? IsNumber(range[2]) And? IsNumber(options'hash[\"points\"]) And? IsNumber(options'hash[\"precision\"]), \"Argument\", \"Plot2D: Error: plotting range '\" :(PipeToString()Write(range)) :\"' and/or the number of points '\" :(PipeToString()Write(options'hash[\"points\"])) :\"' and/or precision '\" :(PipeToString()Write(options'hash[\"precision\"])) :\"' is not numeric\" );  ForEach(func, func'list) [  var := VarList(func);  Check(Length(var)<=?1, \"Argument\", \"Plot2D: Error: expression is not a function of one variable: \" :(PipeToString()Write(func)) );  If(Length(var)=?0, var:={dummy});  If( options'hash[\"xname\"] =? \"\", options'hash[\"xname\"] := ToString(VarList(var)[1]) );  DestructiveAppend(options'hash[\"yname\"], PipeToString()Write(func));  c := range[1]; fc := N(Eval(Apply({var, func}, {c}))); Check(IsNumber(fc) Or? fc=?Infinity Or? fc=? -Infinity Or? fc=?Undefined, \"Argument\",  \"Plot2D: Error: cannot evaluate function '\" :(PipeToString()Write(func)) :\"' at point '\" :(PipeToString()Write(c)) :\"' to a number, instead got '\" :(PipeToString()Write(fc)) :\"'\" );  DestructiveAppend(all'values, Plot2D'get'data(func, var, c, fc, delta, options'hash) );  If(InVerboseMode(), Echo({\"Plot2D: using \", Length(all'values[Length(all'values)]), \" points for function \", func}), True); ];   Plot2DOutputs()[options'hash[\"output\"]] @ {all'values, options'hash};];HoldArgumentNumber(\"Plot2D\", 2, 2);HoldArgumentNumber(\"Plot2D\", 3, 2);HoldArgumentNumber(\"Plot2D\", 3, 3);Plot2D'get'data(_func, _var, _x'init, _y'init, _delta'x, _options'hash) <--[ Local(i, a, fa, b, fb, c, fc, result);  result := { {c,fc} := {x'init, y'init} }; For(i:=0, i<?options'hash[\"points\"], i++) [ {a,fa} := {c, fc};   {b, c} := N(Eval({x'init + (i+1/2)*delta'x, x'init + (i+1)*delta'x}));  {fb, fc} := N(Eval(MapSingle({var, func}, {b, c}))); result := Concat(result, Rest(Plot2D'adaptive(func, var, {a,b,c}, {fa, fb, fc}, options'hash[\"depth\"],  options'hash[\"precision\"]*options'hash[\"points\"] ))); ];  result;];Plot2D'adaptive(_func, _var, {_a,_b,_c}, {_fa, _fb, _fc}, _depth, _epsilon) <--[ Local(a1, b1, fa1, fb1); a1 := N(Eval((a+b)/2)); b1 := N(Eval((b+c)/2)); {fa1, fb1} := N(Eval(MapSingle({var, func}, {a1, b1}))); If( depth<=?0 Or? (  sign'change(fa, fa1, fb) + sign'change(fa1, fb, fb1) + sign'change(fb, fb1, fc) <=? 2 And?  N(Eval(Abs( (fa-5*fa1+9*fb-7*fb1+2*fc)/24 ) ))  <=? N(Eval( epsilon*(   (5*fb+8*fb1-fc)/12  - Minimum({fa,fa1,fb,fb1,fc}) ) ) ) ),  {{a,fa}, {a1,fa1}, {b,fb}, {b1,fb1}, {c,fc}},  Concat(  Plot2D'adaptive(func, var, {a, a1, b}, {fa, fa1, fb}, depth-1, epsilon*2),  Rest(Plot2D'adaptive(func, var, {b, b1, c}, {fb, fb1, fc}, depth-1, epsilon*2)) ) );];]; ";
        scriptMap.put("Plot2D",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Plot3DS'outputs() := { {\"default\", \"data\"}, {\"data\", \"Plot3DS'data\"},};Plot3DS'data(values_IsList, _options'hash) <-- values;";
        scriptMap.put("Plot3DS'outputs",scriptString);
        scriptMap.put("Plot3DS'data",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " LocalSymbols(var, func, xrange, yrange, option, options'list, xdelta, ydelta, options'hash, cx, cy, fc, all'values, dummy)[Function() Plot3DS(func);Function() Plot3DS(func, xrange, yrange);Function() Plot3DS(func, xrange, yrange, options, ...);1 # Plot3DS(_func) <-- (\"Plot3DS\" @ {func, -5:5, -5:5});2 # Plot3DS(_func, _xrange, _yrange) <-- (\"Plot3DS\" @ {func, xrange, yrange, {}});3 # Plot3DS(_func, _xrange, _yrange, option_IsFunction) _ (Type(option) =? \"->\" ) <-- (\"Plot3DS\" @ {func, xrange, yrange, {option}});5 # Plot3DS(_func, _xrange, _yrange, options'list_IsList)_(Not? IsList(func)) <-- (\"Plot3DS\" @ {{func}, xrange, yrange, options'list});4 # Plot3DS(func'list_IsList, _xrange, _yrange, options'list_IsList) <--[ Local(var, func, xdelta, ydelta, options'hash, cx, cy, fc, all'values, dummy);  all'values := {}; options'hash := \"OptionsListToHash\" @ {options'list};  options'hash[\"xname\"] := \"\"; options'hash[\"yname\"] := \"\";  options'hash[\"zname\"] := {};  If ( Type(xrange) =? \"->\",  [  options'hash[\"xname\"] := ToString(xrange[1]); xrange := xrange[2]; ] ); If ( Type(yrange) =? \"->\" ,  [  options'hash[\"yname\"] := ToString(yrange[1]); yrange := yrange[2]; ] ); If( Type(xrange) =? \":\",  xrange := N(Eval({xrange[1], xrange[2]})) ); If( Type(yrange) =? \":\",  yrange := N(Eval({yrange[1], yrange[2]})) );  If( options'hash[\"points\"] =? Empty, options'hash[\"points\"] := 10  ); If( options'hash[\"xpoints\"] =? Empty, options'hash[\"xpoints\"] := options'hash[\"points\"] ); If( options'hash[\"ypoints\"] =? Empty, options'hash[\"ypoints\"] := options'hash[\"points\"] ); If( options'hash[\"depth\"] =? Empty, options'hash[\"depth\"] := 2 ); If( options'hash[\"precision\"] =? Empty, options'hash[\"precision\"] := 0.0001 ); If( options'hash[\"hidden\"] =? Empty Or? Not? Boolean?(options'hash[\"hidden\"]), options'hash[\"hidden\"] := True ); If( options'hash[\"output\"] =? Empty Or? IsString(options'hash[\"output\"]) And? Plot3DS'outputs()[options'hash[\"output\"]] =? Empty, options'hash[\"output\"] := Plot3DS'outputs()[\"default\"] );  If( options'hash[\"output\"] =? \"datafile\" And? options'hash[\"filename\"] =? Empty, options'hash[\"filename\"] := \"output.data\" ); options'hash[\"used depth\"] := options'hash[\"depth\"];  options'hash[\"xpoints\"] := N(Eval(Quotient(options'hash[\"xpoints\"]+1, 2))); options'hash[\"ypoints\"] := N(Eval(Quotient(options'hash[\"ypoints\"]+1, 2)));  options'hash[\"precision\"] := N(Eval(options'hash[\"precision\"]));  options'hash[\"xrange\"] := {xrange[1], xrange[2]}; options'hash[\"yrange\"] := {yrange[1], yrange[2]};  xdelta := N(Eval( (xrange[2] - xrange[1]) / (options'hash[\"xpoints\"]) ) ); ydelta := N(Eval( (yrange[2] - yrange[1]) / (options'hash[\"ypoints\"]) ) );  Check(IsNumericList({xrange[1], xrange[2], options'hash[\"xpoints\"], options'hash[\"ypoints\"], options'hash[\"precision\"]}), \"Argument\",  \"Plot3DS: Error: plotting ranges '\" :(PipeToString()Write(xrange, yrange)) :\"' and/or the number of points '\" :(PipeToString()Write(options'hash[\"xpoints\"], options'hash[\"ypoints\"])) :\"' and/or precision '\" :(PipeToString()Write(options'hash[\"precision\"])) :\"' is not numeric\" );  ForEach(func, func'list) [  var := VarList(func);  Check(Length(var)<=?2, \"Argument\", \"Plot3DS: Error: expression is not a function of at most two variables: \" :(PipeToString()Write(func)) );  If(Length(var)=?0, var:={dummy, dummy}); If(Length(var)=?1, var:={var[1], dummy});  If( options'hash[\"xname\"] =? \"\", options'hash[\"xname\"] := ToString(var[1]) ); If( options'hash[\"yname\"] =? \"\", options'hash[\"yname\"] := ToString(var[2]) );  DestructiveAppend(options'hash[\"zname\"], PipeToString()Write(func));  cx := xrange[1]; cy := yrange[1]; fc := N(Eval(Apply({var, func}, {cx, cy}))); Check(IsNumber(fc) Or? fc=?Infinity Or? fc=? -Infinity Or? fc=?Undefined, \"Argument\",  \"Plot3DS: Error: cannot evaluate function '\" :(PipeToString()Write(func)) :\"' at point '\" :(PipeToString()Write(cx, cy)) :\"' to a number, instead got '\" :(PipeToString()Write(fc)) :\"'\" );  DestructiveAppend(all'values, RemoveRepeated(HeapSort( Plot3DS'get'data(func, var, {cx, cy, fc}, {xdelta, ydelta}, options'hash), Hold({{x,y},x[1]<?y[1] Or? x[1] =? y[1] And? x[2] <=? y[2] } ) )) ); If(InVerboseMode(), Echo({\"Plot3DS: using \", Length(all'values[Length(all'values)]), \" points for function \", func}), True); If(InVerboseMode(), Echo({\"Plot3DS: max. used \", 2^(options'hash[\"depth\"] - options'hash[\"used depth\"]), \"subdivisions for \", func}), True); ];  Plot3DS'outputs()[options'hash[\"output\"]] @ {all'values, options'hash};];HoldArgumentNumber(\"Plot3DS\", 3, 2);HoldArgumentNumber(\"Plot3DS\", 3, 3);HoldArgumentNumber(\"Plot3DS\", 4, 2);HoldArgumentNumber(\"Plot3DS\", 4, 3);HoldArgumentNumber(\"Plot3DS\", 4, 4);Plot3DS'get'data(_func, _var, _init'values, _deltas, _options'hash) <--[ Local(i, j, xa, ya, fa, xb, yb, fb, result, row'cache);      row'cache := {init'values}; For(i:=1, i<=?options'hash[\"ypoints\"], i++) [ ya := N(Eval(init'values[2]+i*deltas[2])); DestructiveAppend(row'cache, {init'values[1], ya, N(Eval(Apply({var, func}, {init'values[1], ya})))}); ]; result := row'cache;  For(i:=1, i<=?options'hash[\"xpoints\"], i++) [   xa := N(Eval(init'values[1]+i*deltas[1])); ya := init'values[2]; fa := N(Eval(Apply({var, func}, {xa, ya}))); DestructiveAppend(result, {xa, ya, fa});  For(j:=1, j<=?options'hash[\"ypoints\"], j++) [     yb := N(Eval(init'values[2] + j*deltas[2])); fb := N(Eval(Apply({var, func}, {xa, yb}))); result := Concat(result, Plot3DS'adaptive(func, var, {row'cache[j][1], ya, xa, yb, row'cache[j][3], row'cache[j+1][3], fa, fb}, options'hash[\"depth\"],  options'hash[\"precision\"] * options'hash[\"xpoints\"] * options'hash[\"ypoints\"], options'hash ));  row'cache[j] := {xa, ya, fa}; ya := yb; fa := fb; DestructiveAppend(result, {xa, ya, fa}); ]; ]; result;];10 # Plot3DS'adaptive(_func, _var, _square, 0, _epsilon, _options'hash) <-- {};20 # Plot3DS'adaptive(_func, _var, {_x1, _y1, _x2, _y2, _f11, _f12, _f21, _f22}, _depth, _epsilon, _options'hash) <--[ Local(x3, y3, f13, f31, f33, f32, f23, result);  options'hash[\"used depth\"] := depth-1;  x3 := N(Eval((x1+x2)/2)); y3 := N(Eval((y1+y2)/2));   f13 := N(Eval(Apply({var, func}, {x1, y3}))); f31 := N(Eval(Apply({var, func}, {x3, y1}))); f33 := N(Eval(Apply({var, func}, {x3, y3}))); f32 := N(Eval(Apply({var, func}, {x3, y2}))); f23 := N(Eval(Apply({var, func}, {x2, y3}))); result := {{x1,y3,f13}, {x3, y1, f31}, {x3, y3, f33}, {x3, y2, f32}, {x2, y3, f23}}; If(  sign'change(f11,f13,f12) + sign'change(f13,f12,f32) + sign'change(f12,f32,f22) <=? 2 And? sign'change(f22,f23,f21) + sign'change(f23,f21,f31) + sign'change(f21,f31,f11) <=? 2 And?    N(Eval(Abs( (f11-f23)/2-(f12-f21)/3+(f22-f13)/6+2*(f32-f33)/3 ))) <=? N(Eval( epsilon*(   (f11 + f12 + f21 + f22)/12 + 2*f33/3 - Minimum({f11, f12, f21, f22, f13, f31, f33, f32, f23}) ) ) ) ,  result,  Concat(  result,  Plot3DS'adaptive(func, var, {x1, y1, x3, y3, f11, f13, f31, f33}, depth-1, epsilon*4, options'hash), Plot3DS'adaptive(func, var, {x1, y3, x3, y2, f13, f12, f33, f32}, depth-1, epsilon*4, options'hash), Plot3DS'adaptive(func, var, {x3, y1, x2, y3, f31, f33, f21, f23}, depth-1, epsilon*4, options'hash), Plot3DS'adaptive(func, var, {x3, y3, x2, y2, f33, f32, f23, f22}, depth-1, epsilon*4, options'hash) ) );];]; ";
        scriptMap.put("Plot3DS",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "sign'change(x,y,z) :=If( IsNumber(x) And? IsNumber(y) And? IsNumber(z) And? Not? ( x>?y And? y<?z Or?   x<?y And? y>?z ), 0, 1); ";
        scriptMap.put("sign'change",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # AllSatisfy?(pred_IsString,lst_IsList) <-- Apply(\"And?\",(MapSingle(pred,lst)));20 # AllSatisfy?(_pred,_lst) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function (\"Boolean?\", {x}) (x=?True) Or? (x=?False) Or? IsFunction(x) And? Contains({\"=?\", \">?\", \"<?\", \">=?\", \"<=?\", \"!=?\", \"And?\", \"Not?\", \"Or?\"}, Type(x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Diagonal?(A_IsMatrix) <--[ Local(i,j,m,n,result); m := Length(A); n := Length(A[1]); result := (m=?n);   i:=2; While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:= (i=?j Or? A[i][j] =? 0); j++; ]; i++; ]; If(m=?2, [ result := result And? (A=?Transpose(A)); ] ); result;];";
        scriptMap.put("Diagonal?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Equation?(expr_IsAtom) <-- False;12 # Equation?(_expr) <-- FunctionToList(expr)[1] =? == ;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Even?(n) := IsInteger(n) And? ( BitAnd(n,1) =? 0 );";
        scriptMap.put("Even?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FloatIsInt?(_x) <-- [ x:=N(Eval(x)); Local(prec,result,n); Bind(prec,BuiltinPrecisionGet()); If(IsZero(x),Bind(n,2), If(x>?0, Bind(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Bind(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); BuiltinPrecisionSet(n+prec); Bind(result,IsZero(RoundTo(x-Floor(x),prec)) Or? IsZero(RoundTo(x-Ceil(x),prec))); BuiltinPrecisionSet(prec); result; ];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasExpressionSome?(_expr, _atom, _look'list) _ IsEqual(expr, atom) <-- True;15 # HasExpressionSome?(expr_IsAtom, _atom, _look'list) <-- IsEqual(expr, atom);19 # HasExpressionSome?({}, _atom, _look'list) <-- False;20 # HasExpressionSome?(expr_IsList, _atom, _look'list) <-- HasExpressionSome?(First(expr), atom, look'list) Or? HasExpressionSome?(Rest(expr), atom, look'list);25 # HasExpressionSome?(expr_IsFunction, _atom, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- False;30 # HasExpressionSome?(expr_IsFunction, _atom, _look'list) <-- HasExpressionSome?(Rest(FunctionToList(expr)), atom, look'list);";
        scriptMap.put("HasExpressionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasExpression?(_expr, _atom) _ IsEqual(expr, atom) <-- True;15 # HasExpression?(expr_IsAtom, _atom) <-- IsEqual(expr, atom);19 # HasExpression?({}, _atom) <-- False;20 # HasExpression?(expr_IsList, _atom) <-- HasExpression?(First(expr), atom) Or? HasExpression?(Rest(expr), atom);30 # HasExpression?(expr_IsFunction, _atom) <-- HasExpression?(Rest(FunctionToList(expr)), atom);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFunctionSome?(_expr, string_IsString, _look'list) <-- HasFunctionSome?(expr, ToAtom(string), look'list);10 # HasFunctionSome?(expr_IsAtom, atom_IsAtom, _look'list) <-- False;15 # HasFunctionSome?(expr_IsFunction, atom_IsAtom, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- IsEqual(First(FunctionToList(expr)), atom);20 # HasFunctionSome?(expr_IsFunction, atom_IsAtom, _look'list) <-- IsEqual(First(FunctionToList(expr)), atom) Or? ListHasFunctionSome?(Rest(FunctionToList(expr)), atom, look'list);";
        scriptMap.put("HasFunctionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFunction?(_expr, string_IsString) <-- HasFunction?(expr, ToAtom(string));10 # HasFunction?(expr_IsAtom, atom_IsAtom) <-- False;20 # HasFunction?(expr_IsFunction, atom_IsAtom) <-- IsEqual(First(FunctionToList(expr)), atom) Or? ListHasFunction?(Rest(FunctionToList(expr)), atom);";
        scriptMap.put("HasFunction?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsHermitian(A_IsMatrix) <-- (Conjugate(Transpose(A))=?A);";
        scriptMap.put("IsHermitian",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsIdempotent(A_IsMatrix) <-- (A^2 =? A);";
        scriptMap.put("IsIdempotent",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsInfinity(Infinity) <-- True;10 # IsInfinity(-(_x)) <-- IsInfinity(x);11 # IsInfinity(Sign(_x)*y_IsInfinity) <-- True;60000 # IsInfinity(_x) <-- False;";
        scriptMap.put("IsInfinity",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsLowerTriangular(A_IsMatrix) <-- (IsUpperTriangular(Transpose(A)));";
        scriptMap.put("IsLowerTriangular",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)_(Type(expr)=?\"UniVariate\") <-- False;10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # IsMonomial(expr_CanBeMonomial) <-- [ Local(r); If( IsRationalFunction(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # IsMonomial(_expr) <-- False;";
        scriptMap.put("IsMonomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNegativeInteger(x):= IsInteger(x) And? x <? 0;";
        scriptMap.put("IsNegativeInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNegativeNumber(x):= IsNumber(x) And? x <? 0;";
        scriptMap.put("IsNegativeNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNegativeReal(_r) <--[ r:=N(Eval(r)); (IsNumber(r) And? r <=? 0);];";
        scriptMap.put("IsNegativeReal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNonNegativeInteger(x):= IsInteger(x) And? x >=? 0;";
        scriptMap.put("IsNonNegativeInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNonNegativeNumber(x):= IsNumber(x) And? x >=? 0;";
        scriptMap.put("IsNonNegativeNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNonZeroInteger(x) := (IsInteger(x) And? x !=? 0);";
        scriptMap.put("IsNonZeroInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsNotZero(x_IsNumber) <-- ( AbsN(x) >=? PowerN(10, -BuiltinPrecisionGet()));10 # IsNotZero(x_IsInfinity) <-- True;60000 # IsNotZero(_x) <-- False;";
        scriptMap.put("IsNotZero",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsNumericList(_arg'list) <-- IsList(arg'list) And? (\"And?\" @ (MapSingle(Hold({{x},IsNumber(N(Eval(x)))}), arg'list)));";
        scriptMap.put("IsNumericList",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsOdd(n) := IsInteger(n) And? ( BitAnd(n,1) =? 1 );";
        scriptMap.put("IsOdd",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsOddFunction(f,x):= (f =? Eval(-Subst(x,-x)f));";
        scriptMap.put("IsOddFunction",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsOne(x_IsNumber) <-- IsZero(SubtractN(x,1));60000 # IsOne(_x) <-- False;";
        scriptMap.put("IsOne",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsOrthogonal(A_IsMatrix) <-- (Transpose(A)*A=?Identity(Length(A)));";
        scriptMap.put("IsOrthogonal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsPolynomial(expr_IsFunction) <-- [ Local(x,vars); vars := VarList(expr); If(Length(vars)>?1,vars:=HeapSort(vars,\"IsGreaterThan\"));  x := vars[1]; IsPolynomial(expr,x);];15 # IsPolynomial(_expr) <-- False;10 # IsPolynomial(_expr,_var)_(CanBeUni(var,expr)) <-- True;15 # IsPolynomial(_expr,_var) <-- False;";
        scriptMap.put("IsPolynomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsPolynomialOverIntegers(expr_IsFunction) <-- [ Local(x,vars); vars := VarList(expr); If(Length(vars)>?1,vars:=HeapSort(vars,\"IsGreaterThan\"));  x := vars[1]; IsPolynomialOverIntegers(expr,x);];15 # IsPolynomialOverIntegers(_expr) <-- False;10 # IsPolynomialOverIntegers(_expr,_var)_(CanBeUni(var,expr)) <--[ If( AllSatisfy?(\"IsInteger\",Coef(expr,var,0 .. Degree(expr,var))), True, False );];15 # IsPolynomialOverIntegers(_expr,_var) <-- False;";
        scriptMap.put("IsPolynomialOverIntegers",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPositiveInteger(x):= IsInteger(x) And? x >? 0;";
        scriptMap.put("IsPositiveInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPositiveNumber(x):= IsNumber(x) And? x >? 0;";
        scriptMap.put("IsPositiveNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPositiveReal(_r) <--[ r:=N(Eval(r)); (IsNumber(r) And? r >=? 0);];";
        scriptMap.put("IsPositiveReal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsRational(x_IsInteger) <-- True;10 # IsRational(x_IsInteger / y_IsInteger) <-- True;10 # IsRational(-(x_IsInteger / y_IsInteger)) <-- True;60000 # IsRational(_x) <-- False;";
        scriptMap.put("IsRational",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsRationalFunction(_expr)_(Length(VarList(expr))=?0) <-- False;15 # IsRationalFunction(_expr) <-- IsRationalFunction(expr,VarList(expr));10 # IsRationalFunction(expr_IsRationalOrNumber,_var) <-- False;15 # IsRationalFunction(_expr,var_IsAtom)_(Type(expr)=?\"/\" Or? Type(-expr)=?\"/\") <--[ If (IsPolynomial(Numerator(expr),var) And? IsPolynomial(Denominator(expr),var), Contains(VarList(Denominator(expr)),var), False );];20 # IsRationalFunction(_expr,vars_IsList)_(Type(expr)=?\"/\" Or? Type(-expr)=?\"/\") <--[ If (IsPolynomial(Numerator(expr),vars) And? IsPolynomial(Denominator(expr),vars), Intersection(vars, VarList(expr)) !=? {}, False );];60000 # IsRationalFunction(_expr,_var) <-- False;";
        scriptMap.put("IsRationalFunction",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsRationalOrNumber(x_IsNumber) <-- True;10 # IsRationalOrNumber(x_IsNumber / y_IsNumber) <-- True;10 # IsRationalOrNumber(-(x_IsNumber / y_IsNumber)) <-- True;60000 # IsRationalOrNumber(_x) <-- False;";
        scriptMap.put("IsRationalOrNumber",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSkewSymmetric(A_IsMatrix) <-- (Transpose(A)=?(-1*A));";
        scriptMap.put("IsSkewSymmetric",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsSumOfTerms(_var,expr_IsFreeOf(var)) <-- False;12 # IsSumOfTerms(_var,expr_IsAtom()) <-- False;14 # IsSumOfTerms(_var,expr_IsList())_(expr[1]=?ToAtom(\"+\") Or? expr[1]=?ToAtom(\"-\")) <-- True;16 # IsSumOfTerms(_var,expr_IsList())_(expr[1]=?ToAtom(\"*\")) <-- Or?(IsFreeOf(var,expr[2]),IsFreeOf(var,expr[3]));18 # IsSumOfTerms(_var,expr_IsList())_(expr[1]=?ToAtom(\"/\")) <-- IsFreeOf(var,expr[3]);20 # IsSumOfTerms(_var,expr_IsList()) <-- False;22 # IsSumOfTerms(_var,_expr) <-- IsSumOfTerms(var,FunctionToList(expr));";
        scriptMap.put("IsSumOfTerms",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSymmetric(A_IsMatrix) <-- (Transpose(A)=?A);";
        scriptMap.put("IsSymmetric",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsUnitary(A_IsMatrix) <-- (Transpose(Conjugate(A))*A =? Identity(Length(A)));";
        scriptMap.put("IsUnitary",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsUpperTriangular(A_IsMatrix) <--[ Local(i,j,m,n,result); m:=Length(A); n:=Length(A[1]); i:=2; result:=(m=?n); While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:= (i<=?j Or? A[i][j] =? 0); j++; ]; i++; ]; result;];";
        scriptMap.put("IsUpperTriangular",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsVariable(_expr) <-- (IsAtom(expr) And? Not?(expr=?Infinity) And? Not?(expr=? -Infinity) And? Not?(expr=?Undefined) And? Not?(IsNumber(N(Eval(expr)))));";
        scriptMap.put("IsVariable",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsZero(x_IsNumber) <-- ( MathSign(x) =? 0 Or? AbsN(x) <? PowerN(10, -BuiltinPrecisionGet()));60000 # IsZero(_x) <-- False;20 # IsZero(UniVariate(_var,_first,_coefs)) <-- IsZeroVector(coefs);";
        scriptMap.put("IsZero",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunctionSome?({}, _atom, _look'list) <-- False;20 # ListHasFunctionSome?(expr_IsList, atom_IsAtom, _look'list) <-- HasFunctionSome?(First(expr), atom, look'list) Or? ListHasFunctionSome?(Rest(expr), atom, look'list);";
        scriptMap.put("ListHasFunctionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunction?({}, _atom) <-- False;20 # ListHasFunction?(expr_IsList, atom_IsAtom) <-- HasFunction?(First(expr), atom) Or? ListHasFunction?(Rest(expr), atom);";
        scriptMap.put("ListHasFunction?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NoneSatisfy(pred_IsString,lst_IsList) <-- Not? Apply(\"Or?\",(MapSingle(pred,lst)));20 # NoneSatisfy(_pred,_lst) <-- True;";
        scriptMap.put("NoneSatisfy",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(p,x)[Function(\"IsScalar\",{x}) Not?(IsList(x));Function(\"IsVector\",{x}) If(IsList(x), Length(Select(x, IsList))=?0, False);Function(\"IsVector\",{p,x})[ If(IsList(x), [ Local(i,n,result); n:=Length(x); i:=1; result:=True; While(i<=?n And? result) [ result:=Apply(p,{x[i]}); i++; ]; result; ], False);];Function(\"IsMatrix\",{x})If(IsList(x) And? Length(x)>?0,[ Local(n); n:=Length(x); If(Length(Select(x, IsVector))=?n, MapSingle(Length,x)=?Length(x[1])+ZeroVector(n), False);],False);Function(\"IsMatrix\",{p,x})If(IsMatrix(x),[ Local(i,j,m,n,result); m:=Length(x); n:=Length(x[1]); i:=1; result:=True; While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:=Apply(p,{x[i][j]}); j++; ]; i++; ]; result;],False);Function(\"IsSquareMatrix\",{x}) IsMatrix(x) And? Length(x)=?Length(x[1]);Function(\"IsSquareMatrix\",{p,x}) IsMatrix(p,x) And? Length(x)=?Length(x[1]);]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CDF(BernoulliDistribution(_p), x_IsNumber) <-- If(x<=?0,0,If(x>?0 And? x<=?1, p,1));11 # CDF(BernoulliDistribution(_p), _x) <-- Hold(If(x<=?0,0,If(x>?0 And? x<=?1, p,1)));10 # CDF(BinomialDistribution(_p,_n),m_IsNumber)_(m<?0) <-- 0;10 # CDF(BinomialDistribution(_p,n_IsInteger),m_IsNumber)_(m>?n) <-- 1;10 # CDF(BinomialDistribution(_p,_n),_m) <-- Sum @ { i, 0, Floor(m), PMF(BinomialDistribution(p,n),i)};10 # CDF(DiscreteUniformDistribution( a_IsNumber, b_IsNumber), x_IsNumber)_(x<=?a) <-- 0;10 # CDF(DiscreteUniformDistribution( a_IsNumber, b_IsNumber), x_IsNumber)_(x>?b) <-- 1;10 # CDF(DiscreteUniformDistribution( a_IsNumber, b_IsNumber), x_IsNumber)_(a<?x And? x<=?b) <-- (x-a)/(b-a+1);11 # CDF(DiscreteUniformDistribution( _a, _b), _x) <--Hold(If(x<=?a,0,If(x<=?b,(x-a)/(b-a),1)));10 # CDF(PoissonDistribution(_l), x_IsNumber)_(x<=?0) <-- 0;10 # CDF(PoissonDistribution(_l), _x) <-- Sum @ {i,0,x,PMF(PoissonDistribution(l),i)};10 # CDF(ChiSquareDistribution(_m), _x) <-- IncompleteGamma(m/2,x/2)/Gamma(x/2);10 # CDF(DiscreteDistribution( dom_IsList, prob_IsList), _x) <-- [ Local(i,cdf,y); i := 1; cdf:=0; y:=dom[i]; While(y<?x) [cdf:=cdf+prob[i];i++;]; cdf; ];10 # CDF(HypergeometricDistribution( N_IsNumber, M_IsNumber, n_IsNumber), x_IsNumber)_(M <=? N And? n <=? N) <-- [ Sum @ {i,0,x,PMF(HypergeometricDistribution(N, M, n),i)};];10 # CDF(NormalDistribution(_m,_s), _x) <-- 1/2 + 1/2 * ErrorFunction((x - m)/(s*Sqrt(2))); ";
        scriptMap.put("CDF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Expectation(BernoulliDistribution(_p)) <-- 1-p;10 # Expectation(BinomialDistribution(_p,_n)) <-- n*p;10 # Expectation( DiscreteDistribution( dom_IsList, prob_IsList))_( Length(dom)=?Length(prob) And? Simplify(Sum(prob))=?1) <-- Sum @ {i,1,Length(dom),dom[i]*prob[i]};";
        scriptMap.put("Expectation",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PDF(ExponentialDistribution(_l), _x) <-- If(x<?0,0,l*Exp(-l*x));10 # PDF(NormalDistribution(_m,_s),_x) <-- Exp(-(x-m)^2/(2*s^2))/Sqrt(2*Pi*s^2); 10 # PDF(ContinuousUniformDistribution(_a,_b),x)_(a<?b) <-- If(x<?a Or? x>?b,0,1/(b-a));10 # PDF(DiscreteDistribution( dom_IsList, prob_IsList), _x)_( Length(dom)=?Length(prob) And? Simplify(Add(prob))=?1) <-- [ Local(i); i:=Find(dom,x); If(i =? -1,0,prob[i]); ];10 # PDF( ChiSquareDistribution( _m),x_IsRationalOrNumber)_(x<=?0) <-- 0;20 # PDF( ChiSquareDistribution( _m),_x) <-- x^(m/2-1)*Exp(-x/2)/2^(m/2)/Gamma(m/2);10 # PDF(tDistribution(_m),x) <-- Gamma((m+1)/2)*(1+x^2/m)^(-(m+1)/2)/Gamma(m/2)/Sqrt(Pi*m);";
        scriptMap.put("PDF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PMF(BernoulliDistribution(_p),0) <-- p;10 # PMF(BernoulliDistribution(_p),1) <-- 1-p;10 # PMF(BernoulliDistribution(_p),x_IsNumber)_(x !=? 0 And? x !=? 1) <-- 0;10 # PMF(BernoulliDistribution(_p),_x) <-- Hold(If(x=?0,p,If(x=?1,1-p,0)));10 # PMF(BinomialDistribution(_p,_n),_k) <-- BinomialCoefficient(n,k)*p^k*(1-p)^(n-k);10 # PMF(DiscreteUniformDistribution(_a,_b), x_IsNumber) <-- If(x<?a Or? x>?b, 0 ,1/(b-a+1));11 # PMF(DiscreteUniformDistribution(_a,_b), _x) <-- Hold(If(x<?a Or? x>?b, 0 ,1/(b-a+1)));10 # PMF(PoissonDistribution(_l), n_IsNumber) <-- If(n<?0,0,Exp(-l)*l^n/n!);11 # PMF(PoissonDistribution(_l),_n) <-- Exp(-l)*l^n/n!;10 # PMF(GeometricDistribution(_p),_n) <--If(n<?0,0,p*(1-p)^n);10 # PMF(DiscreteDistribution( dom_IsList, prob_IsList), _x)_( Length(dom)=?Length(prob) And? Simplify(Add(prob))=?1) <-- [ Local(i); i:=Find(dom,x); If(i =? -1,0,prob[i]); ]; 10 # PMF(HypergeometricDistribution( N_IsNumber, M_IsNumber, n_IsNumber), x_IsNumber)_(M <=? N And? n <=? N) <-- (BinomialCoefficient(M,x) * BinomialCoefficient(N-M, n-x))/BinomialCoefficient(N,n);";
        scriptMap.put("PMF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Manipulate\",{symbolicEquation});HoldArgument(\"Manipulate\",symbolicEquation);10 # Manipulate(_symbolicEquation)_HasFunction?(Eval(symbolicEquation), \"==\") <--[ Local(listForm, operator, operand, left, right, leftManipulated, rightManipulated, operandIndex, equationIndex, leftOrder, rightOrder); listForm := FunctionToList(symbolicEquation);  operator := listForm[1];  If(HasFunction?(Eval(listForm[2]),\"==\" ), [operandIndex := 3; equationIndex := 2; ], [ operandIndex := 2; equationIndex := 3;]);  operand := listForm[operandIndex]; equation := Eval(listForm[equationIndex]); left := EquationLeft(equation); right := EquationRight(equation);  If(operandIndex =? 3, [ leftOrder := `({left,operand});rightOrder := `({right,operand});], [leftOrder := `({operand,left}); rightOrder := `({operand,right});]);   leftManipulated := ExpandBrackets(Simplify(Apply(ToString(operator), leftOrder))); rightManipulated := ExpandBrackets(Simplify(Apply(ToString(operator), rightOrder)));   leftManipulated == rightManipulated;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Distance(PointA_IsPoint,PointB_IsPoint) <--[ Local(x1,x2,y1,y2,distance); x1 := PointA[1]; x2 := PointB[1]; y1 := PointA[2]; y2 := PointB[2];  distance := Sqrt((x2 - x1)^2 + (y2 - y1)^2);];";
        scriptMap.put("Distance",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPoint(p) := If(IsList(p) And? (Length(p) =? 2 Or? Length(p) =? 3),True,False);";
        scriptMap.put("IsPoint",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSegment(list_IsList) <--[ If(IsList(list[1]) And? Length(list[1])=?2 And? IsList(list[2]) And? Length(list[2])=?2,True,False);];";
        scriptMap.put("IsSegment",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Midpoint(PointA_IsPoint,PointB_IsPoint) <-- [ Local(x1,x2,y1,y2,midpointX,midpointY);  x1 := PointA[1]; x2 := PointB[1]; y1 := PointA[2]; y2 := PointB[2]; midpointX := (x1 + x2)/2; midpointY := (y1 + y2)/2;  {midpointX,midpointY};];Midpoint(segment_IsSegment) <-- [ Local(x1,x2,y1,y2,midpointX,midpointY);  x1 := segment[1][1]; x2 := segment[2][1]; y1 := segment[1][2]; y2 := segment[2][2]; midpointX := (x1 + x2)/2; midpointY := (y1 + y2)/2;  {midpointX,midpointY};];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Segment(PointA_IsPoint,PointB_IsPoint) <--[ Local(x1,x2,y1,y2);  x1 := PointA[1]; x2 := PointB[1]; y1 := PointA[2]; y2 := PointB[2]; {{x1,y1},{x2,y2}};];";
        scriptMap.put("Segment",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Slope(PointA_IsPoint,PointB_IsPoint) <--[ Local(x1,x2,y1,y2,slope); x1 := PointA[1]; x2 := PointB[1]; y1 := PointA[2]; y2 := PointB[2];  slope := (y2 - y1)/(x2 - x1);];10 # Slope(segment_IsList)_(Length(segment) =? 2 And? Length(segment[1]) =? 2 And? Length(segment[2]) =? 2) <--[ Local(x1,x2,y1,y2,slope);  x1 := segment[1][1];  x2 := segment[2][1];    y1 := segment[1][2];  y2 := segment[2][2];  slope := (y2 - y1)/(x2 - x1);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistributionMean(probability,numberOfTrials) :=[ Check(IsRationalOrNumber(probability) And? p >=? 0 And? p <=? 1, \"Argument\", \"The first argument must be a number between 0 and 1.\");  Check(IsInteger(numberOfTrials) And? numberOfTrials >=? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  numberOfTrials * probability;]; ";
        scriptMap.put("BinomialDistributionMean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistributionStandardDeviation(probability,numberOfTrials) :=[ Check(IsRationalOrNumber(probability) And? p >=? 0 And? p <=? 1, \"Argument\", \"The first argument must be a number between 0 and 1.\");  Check(IsInteger(numberOfTrials) And? numberOfTrials >=? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  SqrtN(numberOfTrials * probability * (1 - probability));]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CoefficientOfDetermination(x,y) :=[  Check(IsList(x), \"Argument\", \"The first argument must be a list.\");  Check(IsList(y), \"Argument\", \"The second argument must be a list.\");  Check(Length(x) =? Length(y), \"Argument\", \"The lists for argument 1 and argument 2 must have the same length.\");  N(CorrelationCoefficient(x,y)^2);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSubset(bigList, littleList) :=[ Local(result); result := True;  ForEach(element, littleList) [ If(Not? Contains(bigList,element), result := False); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Permutations(n) :=[ Check(IsInteger(n), \"Argument\", \"Argument must be an integer\"); n!;];Permutations(n, r) :=[ Check(IsInteger(n), \"Argument\", \"Argument 1 must be an integer\");  Check(IsInteger(r), \"Argument\", \"Argument 2 must be an integer\");  n! /(n-r)!;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardErrorOfTheEstimate(xList,yList) :=[  Check(IsList(xList), \"Argument\", \"The first argument must be a list.\");  Check(IsList(yList), \"Argument\", \"The second argument must be a list.\");  Check(Length(xList) =? Length(yList), \"Argument\", \"The lists for argument 1 and argument 2 must have the same length.\");  Local(n,a,b,regressionLine);  regressionLine := RegressionLine(xList,yList);  n := regressionLine[\"count\"];  a := regressionLine[\"yIntercept\"];  b := regressionLine[\"slope\"];  N(Sqrt((Sum(yList^2) - a*Sum(yList) - b*Sum(xList*yList))/(n-2)));];";
        scriptMap.put("StandardErrorOfTheEstimate",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardErrorOfTheMean(sigma, sampleSize) :=[ Check(sigma >? 0, \"Argument\", \"The first argument must be a number which is greater than 0.\");  Check(IsInteger(sampleSize) And? sampleSize >? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  sigma/Sqrt(sampleSize);];";
        scriptMap.put("StandardErrorOfTheMean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardErrorOfTheProportion(meanOfSampleProportions, sampleSize) :=[ Check(IsRationalOrNumber(meanOfSampleProportions), \"Argument\", \"The first argument must be a number.\");  Check(IsInteger(sampleSize) And? sampleSize >? 0, \"Argument\", \"The second argument must be an integer which is greater than 0.\");  Sqrt((meanOfSampleProportions*(1 - meanOfSampleProportions))/sampleSize);];";
        scriptMap.put("StandardErrorOfTheProportion",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardErrorOfTheSlope(xList,yList) :=[  Check(IsList(xList), \"Argument\", \"The first argument must be a list.\");  Check(IsList(yList), \"Argument\", \"The second argument must be a list.\");  Check(Length(xList) =? Length(yList), \"Argument\", \"The lists for argument 1 and argument 2 must have the same length.\");  Local(standardErrorOfTheEstimate,n,xMean);  standardErrorOfTheEstimate := StandardErrorOfTheEstimate(xList,yList);  n := Length(xList);  xMean := Mean(xList);  N(standardErrorOfTheEstimate/Sqrt(Sum(xList^2) - n*xMean^2));];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FastModularPower(a_IsPositiveInteger, b_IsPositiveInteger, n_IsPositiveInteger) <--[ Local(p, j, r); p := a; j := b; r := 1; While (j >? 0) [ If (IsOdd(j), r := ModuloN(r*p, n)); p := ModuloN(p*p, n); j := ShiftRight(j, 1); ]; r;];IsStronglyProbablyPrime(b_IsPositiveInteger, n_IsPositiveInteger) <--[ Local(m, q, r, a, flag, i, root); m := n-1; q := m; r := 0; root := 0;  While (Even?(q)) [ q := ShiftRight(q, 1); r++; ]; a := FastModularPower(b, q, n); flag := (a =? 1 Or? a =? m); i := 1; While (Not?(flag) And? (i <? r)) [ root := a;  a := ModuloN(a*a, n); flag := (a =? m); i++; ]; {root, flag}; ];10 # RabinMillerSmall(1) <-- False;10 # RabinMillerSmall(2) <-- True;20 # RabinMillerSmall(n_Even?) <-- False;20 # RabinMillerSmall(3) <-- True;30 # RabinMillerSmall(n_IsPositiveInteger) <--[ Local(continue, prime, i, primetable, pseudotable, root); continue := True; prime := True; i := 1; primetable := {2, 3, 5, 7, 11, 13, 17}; pseudotable := {2047, 1373653, 25326001, 3215031751, 2152302898747, 3474749660383, 34155071728321};  While (continue And? prime And? (i <? 8)) [  {root, prime} := IsStronglyProbablyPrime(primetable[i], n);  continue := (n >=? pseudotable[i]); i++; ];  If (continue And? (i =? 8), Overflow, prime);];10 # RabinMillerProbabilistic(1, _p) <-- False;10 # RabinMillerProbabilistic(2, _p) <-- True;20 # RabinMillerProbabilistic(n_Even?, _p) <-- False;20 # RabinMillerProbabilistic(3, _p) <-- True;30 # RabinMillerProbabilistic(n_IsPositiveInteger, p_IsPositiveInteger) <--[ Local(k, prime, b, roots'of'minus1, root); k := 1+IntLog(IntLog(n,2),4)+p;  b := 1; prime := True; roots'of'minus1 := {0};  While (prime And? k>?0) [ b := NextPseudoPrime(b);  {root, prime} := IsStronglyProbablyPrime(b, n); If(prime, roots'of'minus1 := Union(roots'of'minus1, {root})); If(Length(roots'of'minus1)>?3, prime := False);  If(  InVerboseMode() And? Length(roots'of'minus1)>?3, [  Local(factor); roots'of'minus1 := Difference(roots'of'minus1,{0});  factor := Gcd(n, If( roots'of'minus1[1]+roots'of'minus1[2]=?n, roots'of'minus1[1]+roots'of'minus1[3], roots'of'minus1[1]+roots'of'minus1[2] )); Echo(n, \" = \", factor, \" * \", n/factor); ] ); k--; ]; prime;];RabinMiller(n_IsPositiveInteger) <--[  If( n <? 34155071728321, RabinMillerSmall(n), RabinMillerProbabilistic(n, 40)  );];";
        scriptMap.put("RabinMiller",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RadSimp(_n)_(Length(VarList(n))<?1) <--[ Local(max, result); Bind(max, CeilN(N(Eval(n^2)))); Bind(result,0); Bind(result,RadSimpTry(n,0,1,max)); if (CheckRadicals(n,result)) result else n;];20 # RadSimp(_n) <-- n;CheckRadicals(_n,_test) <-- Abs(N(Eval(n-test),20)) <? 0.000001;10 # ClampRadicals(_r)_(N(Eval(Abs(r)), 20)<?0.000001) <-- 0;20 # ClampRadicals(_r) <-- r;RadSimpTry(_n,_result,_current,_max)<--[ if (IsLessThan(N(Eval(result-n)), 0)) [ Local(i);  i:=BSearch(max,Hold({{try},ClampRadicals(N(Eval((result+Sqrt(try))-n),20))})); If(i>?0, [ Bind(result,result+Sqrt(i)); Bind(i,AddN(max,1)); Bind(current,AddN(max,1)); ]);  if (IsLessThan(N(Eval(result-n)), 0)) [ For (Bind(i,current),i<=?max,Bind(i,AddN(i,1))) [ Local(new, test); Bind(test,result+Sqrt(i)); Bind(new,RadSimpTry(n,test,i,max)); if (CheckRadicals(n,new)) [ Bind(result,new); Bind(i,AddN(max,1)); ]; ]; ]; ]; result;];";
        scriptMap.put("RadSimp",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RandomInteger(_n) <--[ Check(IsPositiveInteger(n), \"Argument\", \"The argument must be a positive integer.\"); CeilN(Random() * n);];10 # RandomInteger(_lowerBoundInclusive, _upperBoundInclusive) <--[ Check(IsInteger(lowerBoundInclusive) And? IsInteger(upperBoundInclusive), \"Argument\", \"Both arguments must be integers.\"); Check(lowerBoundInclusive <? upperBoundInclusive, \"Argument\", \"The first argument must be less than the second argument.\"); FloorN(lowerBoundInclusive + Random() * (upperBoundInclusive + 1 - lowerBoundInclusive) );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(knownRNGEngines, knownRNGDists) [ knownRNGEngines := { { \"default\", \"RNGEngine'LCG'2\"}, { \"advanced\", \"RNGEngine'L'Ecuyer\"}, }; knownRNGDists := { {\"default\", \"FlatRNGDist\"}, {\"flat\", \"FlatRNGDist\"},  {\"gauss\", \"GaussianRNGDist\"}, }; KnownRNGDists() := knownRNGDists; KnownRNGEngines() := knownRNGEngines;];Function() RngCreate();Function() RngCreate(seed, ...);Function() RngSeed(r, seed);Function() Rng(r);RngCreate() <-- RngCreate(0);10 # RngCreate(a'seed_IsInteger) <-- (RngCreate @ {seed -> a'seed});20 # RngCreate(_key -> _value) <-- `(RngCreate({@key -> value}));30 # RngCreate(options_IsList) <--[ options := OptionsListToHash @ {options};  If( options[\"seed\"] =? Empty Or? options[\"seed\"] <=? 0, options[\"seed\"] := 76544321  ); If( options[\"engine\"] =? Empty Or? Not? (Assert(\"warning\", {\"RngCreate: invalid engine\", options[\"engine\"]}) KnownRNGEngines()[options[\"engine\"] ] !=? Empty), options[\"engine\"] := \"default\" ); If( options[\"dist\"] =? Empty Or? Not? (Assert(\"warning\", {\"RngCreate: invalid distribution\", options[\"dist\"]}) KnownRNGDists()[options[\"dist\"] ] !=? Empty), options[\"dist\"] := \"default\" );   { KnownRNGDists()[options[\"dist\"] ], KnownRNGEngines()[options[\"engine\"] ],  KnownRNGEngines()[options[\"engine\"] ] @ { options[\"seed\"] } };];Rng(_r) <--[ Local(state, result); {state, result} := (r[1] @ {r});  DestructiveReplace(r, 3, state);  result; ];RngSeed(_r, seed_IsInteger) <--[ Local(state); (Assert(\"warning\", {\"RngSeed: seed must be positive\", seed}) seed >? 0 ) Or? (seed:=76544321); state := (r[2] @ {seed});  DestructiveReplace(r, 3, state);  True;];FlatRNGDist(_r) <-- (r[2] @ {r[3]}); GaussianRNGDist(_rng) <--[    Local(a,b,m, new'state, rnumber); new'state := rng[3];  m:=0; While(m=?0 Or? m>=?1)  [ {new'state, rnumber} := (rng[2] @ {new'state}); a:=2*rnumber-1; {new'state, rnumber} := (rng[2] @ {new'state}); b:=2*rnumber-1; m:=a*a+b*b; ]; {new'state, (a+I*b)*SqrtN(-2*DivideN(Internal'LnNum(m),m))};];RNGEngine'LCG'1(seed_IsInteger) <-- {seed};RNGEngine'LCG'1(state_IsList) <-- LCG'1(state);RNGEngine'LCG'2(seed_IsInteger) <-- {seed};RNGEngine'LCG'2(state_IsList) <-- LCG'2(state);RNGEngine'LCG'3(seed_IsInteger) <-- {seed};RNGEngine'LCG'3(state_IsList) <-- LCG'3(state);RNGEngine'LCG'4(seed_IsInteger) <-- {seed};RNGEngine'LCG'4(state_IsList) <-- LCG'4(state);LCG'1(state) := RandomLCG(state, 2147483647,950706376,0);LCG'2(state) := RandomLCG(state, 4294967296,1099087573,0);LCG'3(state) := RandomLCG(state, 281474976710656,68909602460261,0);LCG'4(state) := RandomLCG(state, 18014398509481984,2783377640906189,0);RandomLCG(_state, _im, _ia, _ic) <--{ DestructiveReplace(state,1, ModuloN(state[1]*ia+ic,im)), DivideN(state[1], im) };RNGEngine'L'Ecuyer(a'seed_IsInteger) <--[  Local(rng'aux, result); rng'aux := (RngCreate @ {a'seed});  result:=ZeroVector(6);  Local(i); For(i:=1, i<=?6, i++) [ Rng(rng'aux); result[i] := rng'aux[3][1];  ];  result;];RNGEngine'L'Ecuyer(state_IsList) <--[ Local(new'state, result); new'state := { Modulo(1403580*state[2]-810728*state[3], 4294967087), state[1], state[2], Modulo(527612*state[4]-1370589*state[6], 4294944433), state[4], state[5] }; result:=Modulo(state[1]-state[4], 4294967087); { new'state, DivideN(If(result=?0, 4294967087, result), 4294967088) };];LocalSymbols(RandSeed) [  RandSeed := SystemTimer();   Function(\"RandomSeed\", {seed}) Bind(RandSeed, seed);  RandomLCG(_im, _ia, _ic) <-- [ RandSeed:=ModuloN(RandSeed*ia+ic,im); DivideN(RandSeed,im);  ];]; Function(\"Random1\",{}) RandomLCG(4294967296,1103515245,12345);Function(\"Random6\",{}) RandomLCG(1771875,2416,374441);Function(\"Random2\",{}) RandomLCG(2147483647,950706376,0);Function(\"Random3\",{}) RandomLCG(4294967296,1099087573,0);Function(\"Random4\",{}) RandomLCG(281474976710656,68909602460261,0);Function(\"Random5\",{}) RandomLCG(18014398509481984,2783377640906189,0);Function(\"Random\",{}) Random3();";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Combine(expr_IsZero) <-- 0;20 # Combine(_expr) <-- [ Local(L); L := ReassembleListTerms(DisassembleExpression(expr)); UnFlatten(L,\"+\",0);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(AssembleTerms, AssembleTermsRecursive)[ AssembleTerms(list) := [ Check(IsList(list), \"Argument\", \"The argument must be a list.\"); If(Length(list) =? 1, First(list), AssembleTermsRecursive(Reverse(list))  ); ]; AssembleTermsRecursive(list) := [ If(Type(list[1]) =? \"-\" Or? IsNegativeNumber(list[1]) Or? Type(list[1]) =? \"/\" And? (Type(Numerator(list[1])) =? \"-\" Or? IsNegativeNumber(Numerator(list[1]))), If(Length(list) =? 2, ListToFunction({ToAtom(\"-\"), list[2], -list[1]} ), ListToFunction({ToAtom(\"-\"), AssembleTermsRecursive(Rest(list)), -First(list)} ) ), If(Length(list) =? 2, ListToFunction({ToAtom(\"+\"), list[2], list[1]} ), ListToFunction({ToAtom(\"+\"), AssembleTermsRecursive(Rest(list)), First(list)} ) ) ); ];10 # ExpandBrackets(xx_IsZero) <-- 0;20 # ExpandBrackets(_xx)_(Type(xx)=?\"/\" Or? Type(-xx)=?\"/\") <--[ Local(N,D,t); N := ReassembleListTerms(DisassembleExpression(Numerator(xx))); D := ExpandBrackets(Denominator(xx)); AssembleTerms(MapSingle({{t}, t / D}, N));];30 # ExpandBrackets(_xx) <-- AssembleTerms(ReassembleListTerms(DisassembleExpression(xx)));];";
        scriptMap.put("ExpandBrackets",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # ExpandFrac(expr_IsList) <-- MapSingle(\"ExpandFrac\", expr);10 # ExpandFrac(_expr)_Not?(HasFunctionSome?(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- expr;15 # ExpandFrac(a_IsRationalOrNumber) <-- a;20 # ExpandFrac(_expr) <-- ExpandFrac'combine(GetNumerDenom(expr));ExpandFrac'combine({_a, _b}) <-- a/b;";
        scriptMap.put("ExpandFrac",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"DoFlatten\",{doflattenx});UnFence(\"DoFlatten\",1);10 # DoFlatten(_doflattenx)_(Type(doflattenx)=?flattenoper) <-- Apply(\"Concat\",MapSingle(\"DoFlatten\",Rest(FunctionToList(doflattenx))));20 # DoFlatten(_doflattenx) <-- { doflattenx };Function(\"Flatten\",{body,flattenoper})[ DoFlatten(body);];";
        scriptMap.put("Flatten",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GetNumerDenom(_expr, _a) <-- GetNumerDenom(expr)*{a,1};10 # GetNumerDenom(_expr)_Not?(HasFunctionSome?(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- {expr, 1};15 # GetNumerDenom(a_IsRationalOrNumber) <-- {a, 1};20 # GetNumerDenom(_a + _b) <-- ExpandFrac'add(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a - _b) <-- ExpandFrac'add(GetNumerDenom(a), GetNumerDenom(b, -1));20 # GetNumerDenom(- _a) <-- GetNumerDenom(a, -1);20 # GetNumerDenom(+ _a) <-- GetNumerDenom(a);20 # GetNumerDenom(_a * _b) <-- ExpandFrac'multiply(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a / _b) <-- ExpandFrac'divide(GetNumerDenom(a), GetNumerDenom(b));20 # GetNumerDenom(_a ^ b_IsInteger)_(b >? 1) <-- ExpandFrac'multiply(GetNumerDenom(a), GetNumerDenom(a^(b-1)));20 # GetNumerDenom(_a ^ b_IsInteger)_(b <? -1) <-- ExpandFrac'divide(GetNumerDenom(1), GetNumerDenom(a^(-b)));20 # GetNumerDenom(_a ^ b_IsInteger)_(b =? -1) <-- ExpandFrac'divide(GetNumerDenom(1), GetNumerDenom(a));25 # GetNumerDenom(_a ^ _b) <-- {a^b, 1};ExpandFrac'add({_a, _b}, {_c, _d}) <-- {a*d+b*c, b*d};ExpandFrac'multiply({_a, _b}, {_c, _d}) <-- {a*c, b*d};ExpandFrac'divide({_a, _b}, {_c, _d}) <-- {a*d, b*c};";
        scriptMap.put("GetNumerDenom",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"SimpAdd\",{x,y});";
        scriptMap.put("SimpAdd",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"SimpDiv\",{x,y});";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SimpFlatten((_x)+(_y)) <-- SimpAdd(SimpFlatten(x),SimpFlatten(y));10 # SimpFlatten((_x)-(_y)) <-- SimpAdd(SimpFlatten(x),SimpMul(-1,SimpFlatten(y)));10 # SimpFlatten( -(_y)) <-- SimpMul(-1,SimpFlatten(y));10 # SimpFlatten((_x)*(_y)) <-- SimpMul(SimpFlatten(x),SimpFlatten(y));10 # SimpFlatten((_x)/(_y)) <-- SimpDiv(SimpFlatten(x),SimpFlatten(y));10 # SimpFlatten((_x)^(n_IsPositiveInteger)) <-- SimpMul(SimpFlatten(x),SimpFlatten(x^(n-1)));100 # SimpFlatten(_x) <--[ x;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"SimpMul\",{x,y});";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Solve'System(_eqns, _vars) <-- Solve'SimpleBackSubstitution(eqns,vars);10 # Solve'SimpleBackSubstitution'FindAlternativeForms((_lx) == (_rx)) <--[ Local(newEq); newEq := (Simplify(lx) == Simplify(rx)); If (newEq !=? (lx == rx) And? newEq !=? (0==0),DestructiveAppend(eq,newEq)); newEq := (Simplify(lx - rx) == 0); If (newEq !=? (lx == rx) And? newEq !=? (0==0),DestructiveAppend(eq,newEq));];20 # Solve'SimpleBackSubstitution'FindAlternativeForms(_equation) <--[];UnFence(\"Solve'SimpleBackSubstitution'FindAlternativeForms\",1);10 # Solve'SimpleBackSubstitution(eq_IsList,var_IsList) <--[ If(InVerboseMode(), Echo({\"Entering Solve'SimpleBackSubstitution\"})); Local(result,i,j,nrvar,nreq,sub,nrSet,origEq); eq:=FlatCopy(eq); origEq:=FlatCopy(eq); nrvar:=Length(var); result:={FlatCopy(var)}; nrSet := 0; ForEach(equation,origEq) [ Solve'SimpleBackSubstitution'FindAlternativeForms(equation); ]; nreq:=Length(eq);  For(j:=1,j<=?nreq And? nrSet <? nrvar,j++) [ Local(vlist); vlist:=VarListAll(eq[j],`Lambda({pt},Contains(@var,pt))); For(i:=1,i<=?nrvar And? nrSet <? nrvar,i++) [ If(Count(vlist,var[i]) =? 1, [ sub := FunctionToList(eq[j]); sub := sub[2]-sub[3]; sub:=SuchThat(sub,var[i]); If(InVerboseMode(), Echo({\"From \",eq[j],\" it follows that \",var[i],\" = \",sub})); If(SolveFullSimplify=?True, result:=Simplify(Subst(var[i],sub)result), result[1][i]:=sub ); nrSet++; Local(k,reset); reset:=False; For(k:=1,k<=?nreq And? nrSet <? nrvar,k++) If(Contains(VarListAll(eq[k],`Lambda({pt},Contains(@var,pt))),var[i]), [ Local(original); original:=eq[k]; eq[k]:=Subst(var[i],sub)eq[k]; If(Simplify(Simplify(eq[k])) =? (0 == 0), eq[k] := (0 == 0), Solve'SimpleBackSubstitution'FindAlternativeForms(eq[k]) ); If(original!=?(0==0) And? eq[k] =? (0 == 0),reset:=True); If(InVerboseMode(), Echo({\" \",original,\" simplifies to \",eq[k]})); ]); nreq:=Length(eq); vlist:=VarListAll(eq[j],`Lambda({pt},Contains(@var,pt))); i:=nrvar+1;  If(reset,j:=1); ]); ]; ]; Local(zeroeq,tested); tested:={}; ForEach(item,result) [ DestructiveAppend(tested,Map(\"==\",{var,item})); ]; If(InVerboseMode(), Echo({\"Leaving Solve'SimpleBackSubstitution\"})); tested;];10 # OldSolve(eq_IsList,var_IsList) <-- Solve'SimpleBackSubstitution(eq,var);90 # OldSolve((left_IsList) == right_IsList,_var) <-- OldSolve(Map(\"==\",{left,right}),var);100 # OldSolve(_left == _right,_var) <-- SuchThat(left - right , 0 , var);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SolveSystem( eqns_IsList, vars_IsList )_(Length(eqns)=?1 And? Length(vars)=?1) <--[ {Solve(eqns[1],vars[1])};];12 # SolveSystem( eqns_IsList, vars_IsList ) <--[  If(InVerboseMode(),Tell(\"SolveSystem\",{eqns,vars}));  Local(eq,expr,exprns,VaD,isLinearSet,ans);  exprns := {}; ForEach(eq,eqns) [ expr := If( Equation?(eq), EquationLeft(eq)-EquationRight(eq), eq ); DestructiveAppend(exprns,expr); ]; If(InVerboseMode(),Tell(\" \",exprns));   VaD := VarsAndDegs(exprns,vars); If(InVerboseMode(),Tell(\" \",VaD)); isLinearSet := Maximum(Flatten(VaD,\"List\"))=?1; If(InVerboseMode(),Tell(\" \",isLinearSet));  If( isLinearSet, ans:=SolveLinearSystem( exprns, vars ), ans:=SolveNonlinearSystem( exprns, vars ) ); If(ans !=? {}, ans := Map(\"==\",{vars,ans})); If(InVerboseMode(),Tell(\"\",ans)); ans;];10 # VarsAndDegs(exs_IsList,vars_IsList) <--[ Local(ex,v,result); result := {}; ForEach(ex,exs) [ Local(res); res := {}; ForEach(v,vars) [ DestructiveAppend(res,Apply(\"Degree\",{ex,v})); ]; DestructiveAppend(result,res); ]; result;];10 # SolveLinearSystem( polys_IsList, vars_IsList ) <--[ Local(A, E); If(InVerboseMode(),Tell(\" SolveLinearSystem\",{polys,vars}));  Local(lhs,rhs,zeros); lhs := MakeCoefMatrix(polys,vars); If(InVerboseMode(),Tell(\" \",lhs)); zeros := ZeroVector(Length(vars)); rhs := -WithValue(vars,zeros,polys); If(InVerboseMode(),Tell(\" \",rhs));  A := Transpose(Concat(Transpose(lhs),{rhs})); E := RREF(A); If(Contains(E,BaseVector(Dimensions(E)[2],Dimensions(E)[2])), {}, MatrixColumn(E,Dimensions(E)[2]) - (ExtractSubMatrix(E, 1, 1, Length(E), Length(E)) - Identity(Length(E))) * vars);];10 # MakeCoefMatrix(polys_IsList,vars_IsList) <--[ If(InVerboseMode(),Tell(\" MakeCoefMatrix\",{polys,vars})); Local(p,v,result); result := {}; ForEach(p,polys) [ Local(res); res := {}; ForEach(v,vars) [ DestructiveAppend(res,Apply(\"Coef\",{p,v,1})); ]; DestructiveAppend(result,res); ]; result;];";
        scriptMap.put("SolveSystem",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContainsExpression(_body,_body) <-- True;15 # ContainsExpression(body_IsAtom,_expr) <-- False;20 # ContainsExpression(body_IsFunction,_expr) <--[ Local(result,args); result:=False; args:=Rest(FunctionToList(body)); While(args !=? {}) [ result:=ContainsExpression(First(args),expr); args:=Rest(args); if (result =? True) (args:={}); ]; result;];SuchThat(_function,_var) <-- SuchThat(function,0,var);10 # SuchThat(_left,_right,_var)_(left =? var) <-- right;20 # SuchThat(left_IsAtom,_right,_var) <-- var;30 # SuchThat((_x) + (_y),_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right-y , var);30 # SuchThat((_y) + (_x),_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right-y , var);30 # SuchThat(Complex(_r,_i),_right,_var)_ContainsExpression(r,var) <-- SuchThat(r , right-I*i , var);30 # SuchThat(Complex(_r,_i),_right,_var)_ContainsExpression(i,var) <-- SuchThat(i , right+I*r , var);30 # SuchThat(_x * _y,_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right/y , var);30 # SuchThat(_y * _x,_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right/y , var);30 # SuchThat(_x ^ _y,_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right^(1/y) , var);30 # SuchThat(_x ^ _y,_right,_var)_ContainsExpression(y,var) <-- SuchThat(y , Ln(right)/Ln(x) , var);30 # SuchThat(Sin(_x),_right,_var) <-- SuchThat(x , ArcSin(right) , var);30 # SuchThat(ArcSin(_x),_right,_var) <-- SuchThat(x , Sin(right) , var);30 # SuchThat(Cos(_x),_right,_var) <-- SuchThat(x , ArcCos(right) , var);30 # SuchThat(ArcCos(_x),_right,_var) <-- SuchThat(x , Cos(right) , var);30 # SuchThat(Tan(_x),_right,_var) <-- SuchThat(x , ArcTan(right) , var);30 # SuchThat(ArcTan(_x),_right,_var) <-- SuchThat(x , Tan(right) , var);30 # SuchThat(Exp(_x),_right,_var) <-- SuchThat(x , Ln(right) , var);30 # SuchThat(Ln(_x),_right,_var) <-- SuchThat(x , Exp(right) , var);30 # SuchThat(_x / _y,_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right*y , var);30 # SuchThat(_y / _x,_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , y/right , var);30 # SuchThat(- (_x),_right,_var) <-- SuchThat(x , -right , var);30 # SuchThat((_x) - (_y),_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , right+y , var);30 # SuchThat((_y) - (_x),_right,_var)_ContainsExpression(x,var) <-- SuchThat(x , y-right , var);30 # SuchThat(Sqrt(_x),_right,_var) <-- SuchThat(x , right^2 , var);";
        scriptMap.put("SuchThat",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"YacasPSolve\",{uni});Rule(\"YacasPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 0) {};Rule(\"YacasPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 1) -Coef(uni,0)/Coef(uni,1);Rule(\"YacasPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 2) [ Local(a,b,c,d,q,r); c:=Coef(uni,0); b:=Coef(uni,1); a:=Coef(uni,2); d:=b*b-4*a*c; q:=Sqrt(d)/(2*a);     r:=-b/(2*a); If(InVerboseMode(),[ Tell(\" \",{c,b,a,d}); Tell(\" \",{q,r}); ]); {r+q,r-q}; ];Rule(\"YacasPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 3 ) [ Local(p,q,r,w,ww,a,b); Local(coef0,coef1,coef3,adjust); adjust := (-Coef(uni,2))/(3*Coef(uni,3)); coef3 := Coef(uni,3); coef1 := 2*Coef(uni,2)*adjust+3*Coef(uni,3)*adjust^2+Coef(uni,1); coef0 := Coef(uni,2)*adjust^2+Coef(uni,3)*adjust^3+ adjust*Coef(uni,1)+Coef(uni,0); p:=coef3; q:=coef1/p; r:=coef0/p; w:=Complex(-1/2,Sqrt(3/4)); ww:=Complex(-1/2,-Sqrt(3/4)); a:=(-r/2 + Sqrt(q^3/27+ r*r/4))^(1/3); b:=(-r/2 - Sqrt(q^3/27+ r*r/4))^(1/3); {a+b+adjust,w*a+ww*b+adjust,ww*a+w*b+adjust};];Rule(\"YacasPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 4 )[ Local(coef4,a1,a2,a3,a4,y,y1,z,t,s); coef4:=Coef(uni,4); a1:=Coef(uni,3)/coef4; a2:=Coef(uni,2)/coef4; a3:=Coef(uni,1)/coef4; a4:=Coef(uni,0)/coef4;  y1:=First(YacasPSolve(y^3-a2*y^2+(a1*a3-4*a4)*y+(4*a2*a4-a3^2-a1^2*a4),y)); t := Sqrt(y1^2/4-a4); If(t=?0, s:=Sqrt(y1+a1^2/4-a2), s:=(a1*y1-2*a3)/(4*t)); Concat(YacasPSolve(z^2+(a1/2+s)*z+y1/2+t,z), YacasPSolve(z^2+(a1/2-s)*z+y1/2-t,z));];Function(\"YacasPSolve\",{uni,var}) [ Local(u, factors, f, r, s); u := MakeUni(uni, var); If(Type(u) =? \"UniVariate\" And? (And? @ (Lambda({x}, IsNumber(x) Or? IsRational(x)) /@ u[3])), [ Local(coeffs); coeffs := Rationalize(u[3]); coeffs := If(Length(coeffs) >? 1,  Lcm(Denominator /@ coeffs) * coeffs,  (Denominator /@ coeffs) * coeffs); DestructiveReplace(u, 3, coeffs); factors := If(Degree(u)>?0,  Factors(NormalForm(u)),  {NormalForm(u), 1}); ], [ factors := {{uni, 1}}; ]); r := {}; ForEach(f, factors) [ s := YacasPSolve(MakeUni(f[1],var)); r := Union(r, If(IsList(s), s, {s})); ]; If(Length(r) =? 1, r[1], r);];";
        scriptMap.put("YacasPSolve",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # jSolveUniPoly( _lhs==_rhs, var_IsAtom ) <--[ jSolveUniPoly(lhs-rhs,var);];15 # jSolveUniPoly( poly_IsPolynomial, var_IsAtom )_(Length(VarList(poly))=?1) <--[ If(InVerboseMode(),Tell(jSolveUniPoly,{poly,var})); Local(factorList,f,fac,mult,ii,answer); factorList := Factors(poly); If(InVerboseMode(),Tell(\" \",factorList)); answer := {}; ForEach(f,factorList) [ {fac,mult} := f; soln := Solve(fac,var); If(InVerboseMode(),[Tell(\" \",{fac,mult});Tell(\" \",soln);]); ForEach(ii,1 .. mult) [ DestructiveAppend(answer,soln); ]; ]; answer;];20 # jSolveUniPoly( poly_IsPolynomial, var_IsAtom ) <-- Failed;";
        scriptMap.put("jSolveUniPoly",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(res)[ 10 # Solve(expr_IsList, var_IsList) <-- SolveSystem(expr, var); 12 # Solve(_expr, var_IsList)_(Length(var)=?1) <-- [ {Solve(expr,var[1])}; ];  20 # Solve(_expr, _var)_(IsNumber(var) Or? IsString(var)) <-- [ Assert(\"Solve'TypeError\", \"Second argument, \":(PipeToString() Write(var)):\", is not the name of a variable\") False; {}; ]; 22 # Solve(_expr, _var)_(Not? IsAtom(var) And? Not? HasExpression?(expr,var)) <-- [ Assert(\"Solve'TypeError\", \"Second argument, \":(PipeToString() Write(var)):\", is not the name of a variable\") False; {}; ]; 24 # Solve(False,_var) <-- Check(False, \"Argument\", \"Bad input: possibly '=' instead of '==' \"); 30 # Solve(_lhs == _rhs, _var) <-- Solve(lhs - rhs, var); 40 # Solve(0, _var) <-- {var == var}; 41 # Solve(a_Constant?, _var) <-- {}; 42 # Solve(_expr, _var)_(Not? HasExpression?(expr,var)) <-- [ Assert(\"Solve\", \"expression \":(PipeToString() Write(expr)):\" does not depend on \":PipeToString() Write(var)) False; {}; ]; 50 # Solve(_expr, _var)_((res := Solve'Poly(expr, var)) !=? Failed) <-- res; 60 # Solve(_e1 * _e2, _var) <-- [ Local(t,u,s); t := Union(Solve(e1,var), Solve(e2,var)); u := {}; ForEach(s, t) [ Local(v1,v2); v1 := WithValue(var, s[2], e1); v2 := WithValue(var, s[2], e2); If(Not? (IsInfinity(v1) Or? (v1 =? Undefined) Or? IsInfinity(v2) Or? (v2 =? Undefined)), DestructiveAppend(u, s)); ]; u; ]; 70 # Solve(_e1 / _e2, _var) <-- [ Local(tn, t, s); tn := Solve(e1, var); t := {}; ForEach(s, tn) If(Not?(IsZero(WithValue(var, s[2], e2))),  DestructiveAppend(t, s) ); t; ]; 80 # Solve(_e1 + _e2, _var)_(Not? HasExpression?(e2,var) And? (res := Solve'Simple(e1,-e2,var)) !=? Failed) <-- res; 80 # Solve(_e1 + _e2, _var)_(Not? HasExpression?(e1,var) And? (res := Solve'Simple(e2,-e1,var)) !=? Failed) <-- res; 80 # Solve(_e1 - _e2, _var)_(Not? HasExpression?(e2,var) And? (res := Solve'Simple(e1,e2,var)) !=? Failed) <-- res; 80 # Solve(_e1 - _e2, _var)_(Not? HasExpression?(e1,var) And? (res := Solve'Simple(e2,e1,var)) !=? Failed) <-- res; 85 # Solve(_expr, _var)_((res := Solve'Simple(expr, 0, var)) !=? Failed) <-- res; 90 # Solve(_expr, _var)_((res := Solve'Reduce(expr, var)) !=? Failed) <-- res; 95 # Solve(_expr, _var)_((res := Solve'Divide(expr, var)) !=? Failed) <-- res; 100 # Solve(_expr, _var)_((res := Simplify(expr)) !=? expr) <-- Solve(res, var); 110 # Solve(_expr, _var) <-- [ Assert(\"Solve'Fails\", \"cannot solve equation \":(PipeToString() Write(expr)):\" for \":PipeToString() Write(var)) False; {}; ];];10 # Solve'Poly(_expr, _var)_(Not? CanBeUni(var, expr)) <-- Failed;20 # Solve'Poly(_expr, _var) <--LocalSymbols(x)[ Local(roots); roots := PSolve(expr, var); If(Type(roots) =? \"YacasPSolve\", If(roots =? YacasPSolve(0), {var == var}, Failed),  If(Type(roots) =? \"List\", MapSingle({{x},var==x}, roots),  {var == roots})); ];10 # Solve'Reduce(_expr, _var) <--[ ClearError(\"Solve'Fails\");  Local(context, expr2, var2, res, sol, sol2, i); context := Solve'Context(expr, var); If(context =? False, res := Failed, [ expr2 := Eval(Subst(context, var2) expr); If(CanBeUni(var2, expr2) And? (Degree(expr2, var2) =? 0 Or? (Degree(expr2, var2) =? 1 And? Coef(expr2, var2, 1) =? 1)), res := Failed,  [ sol2 := Solve(expr2, var2); If(IsError(\"Solve'Fails\"), [ ClearError(\"Solve'Fails\"); res := Failed; ], [ res := {}; i := 1; While(i <=? Length(sol2) And? res !=? Failed) [ sol := Solve(context == (var2 Where sol2[i]), var); If(IsError(\"Solve'Fails\"), [ ClearError(\"Solve'Fails\"); res := Failed; ], res := Union(res, sol)); i++; ]; ]); ]); ]); res;];10 # Solve'Context(expr_IsAtom, _var) <-- If(expr=?var, var, {});20 # Solve'Context(_expr, _var) <--[ Local(lst, foundVarP, context, i, res); lst := FunctionToList(expr); foundVarP := False; i := 2; While(i <=? Length(lst) And? Not? foundVarP) [ foundVarP := (lst[i] =? var); i++; ]; If(foundVarP, context := expr, [ context := {}; i := 2; While(i <=? Length(lst) And? context !=? False) [ res := Solve'Context(lst[i], var); If(res !=? {} And? context !=? {} And? res !=? context, context := False); If(res !=? {} And? context =? {}, context := res); i++; ]; ]); context;];20 # Solve'Simple(_e1 + _e2, _rhs, _var)_(e1 =? var And? Not? HasExpression?(e2,var)) <-- { var == rhs-e2 };20 # Solve'Simple(_e1 + _e2, _rhs, _var)_(e2 =? var And? Not? HasExpression?(e1,var)) <-- { var == rhs-e1 };20 # Solve'Simple(_e1 - _e2, _rhs, _var)_(e1 =? var And? Not? HasExpression?(e2,var)) <-- { var == rhs+e2 };20 # Solve'Simple(_e1 - _e2, _rhs, _var)_(e2 =? var And? Not? HasExpression?(e1,var)) <-- { var == e1-rhs };20 # Solve'Simple(-(_e1), _rhs, _var)_(e1 =? var) <-- { var == -rhs };20 # Solve'Simple(_e1 * _e2, _rhs, _var)_(e1 =? var And? Not? HasExpression?(e2,var)) <-- { var == rhs/e2 };20 # Solve'Simple(_e1 * _e2, _rhs, _var)_(e2 =? var And? Not? HasExpression?(e1,var)) <-- { var == rhs/e1 };20 # Solve'Simple(_e1 / _e2, _rhs, _var)_(e1 =? var And? Not? HasExpression?(e2,var)) <-- { var == rhs*e2 };10 # Solve'Simple(_e1 / _e2, 0, _var)_(e2 =? var And? Not? HasExpression?(e1,var)) <-- { };20 # Solve'Simple(_e1 / _e2, _rhs, _var)_(e2 =? var And? Not? HasExpression?(e1,var)) <-- { var == e1/rhs };LocalSymbols(x)[ 20 # Solve'Simple(_e1 ^ _n, _rhs, _var)_(e1 =? var And? IsPositiveInteger(n)) <-- MapSingle({{x}, var == rhs^(1/n)*x}, Exp(2*Pi*I*(1 .. n)/n)); 20 # Solve'Simple(_e1 ^ _n, _rhs, _var)_(e1 =? var And? IsNegativeInteger(n)) <-- MapSingle({{x}, var == rhs^(1/n)*x}, Exp(2*Pi*I*(1 .. (-n))/(-n)));];20 # Solve'Simple(_e1 ^ _e2, _rhs, _var) _ (IsPositiveReal(e1) And? e1 !=? 0 And? e2 =? var And? IsPositiveReal(rhs) And? rhs !=? 0) <-- { var == Ln(rhs)/Ln(e1) };10 # Solve'Simple(Sin(_e1), 1, _var)_(e1 =? var) <-- { var == 1/2*Pi };10 # Solve'Simple(Sin(_e1), _rhs, _var)_(e1 =? var And? rhs =? -1) <-- { var == 3/2*Pi };20 # Solve'Simple(Sin(_e1), _rhs, _var)_(e1 =? var) <-- { var == ArcSin(rhs), var == Pi-ArcSin(rhs) };10 # Solve'Simple(Cos(_e1), 1, _var)_(e1 =? var) <-- { var == 0 };10 # Solve'Simple(Cos(_e1), _rhs, _var)_(e1 =? var And? rhs =? -1) <-- { var == Pi };20 # Solve'Simple(Cos(_e1), _rhs, _var)_(e1 =? var) <-- { var == ArcCos(rhs), var == -ArcCos(rhs) };20 # Solve'Simple(Tan(_e1), _rhs, _var)_(e1 =? var) <-- { var == ArcTan(rhs) };20 # Solve'Simple(ArcSin(_e1), _rhs, _var)_(e1 =? var) <-- { var == Sin(rhs) };20 # Solve'Simple(ArcCos(_e1), _rhs, _var)_(e1 =? var) <-- { var == Cos(rhs) };20 # Solve'Simple(ArcTan(_e1), _rhs, _var)_(e1 =? var) <-- { var == Tan(rhs) };10 # Solve'Simple(Exp(_e1), 0, _var)_(e1 =? var) <-- { };20 # Solve'Simple(Exp(_e1), _rhs, _var)_(e1 =? var) <-- { var == Ln(rhs) };20 # Solve'Simple(_b^_e1, _rhs, _var)_(e1 =? var And? IsFreeOf(var,b) And? Not? IsZero(b)) <-- { var == Ln(rhs) / Ln(b) };20 # Solve'Simple(Ln(_e1), _rhs, _var)_(e1 =? var) <-- { var == Exp(rhs) };20 # Solve'Simple(Sqrt(_e1), _rhs, _var)_(e1 =? var And? IsPositiveReal(Re(rhs)) And? Re(rhs) !=? 0) <-- { var == rhs^2 };20 # Solve'Simple(Sqrt(_e1), _rhs, _var)_(e1 =? var And? Re(rhs)=?0 And? IsPositiveReal(Im(rhs))) <-- { var == rhs^2 };20 # Solve'Simple(Sqrt(_e1), _rhs, _var)_(e1 =? var And? Re(rhs)=?0 And? IsNegativeReal(Im(rhs)) And? Im(rhs) !=? 0) <-- { };20 # Solve'Simple(Sqrt(_e1), _rhs, _var)_(e1 =? var And? IsNegativeReal(Re(rhs)) And? Re(rhs) !=? 0) <-- { };30 # Solve'Simple(_lhs, _rhs, _var) <-- Failed;10 # Solve'Divide(_e1 + _e2, _var)_(HasExpression?(e1, var) And? HasExpression?(e2, var) And? Not? (HasExpression?(Simplify(1 + (e2/e1)), e1) Or? HasExpression?(Simplify(1 + (e2/e1)), e2))) <-- Solve(1 + (e2/e1), var);10 # Solve'Divide(_e1 - _e2, _var)_(HasExpression?(e1, var) And? HasExpression?(e2, var) And? Not? (HasExpression?(Simplify(1 - (e2/e1)), e1) Or? HasExpression?(Simplify(1 - (e2/e1)), e2))) <-- Solve(1 - (e2/e1), var);20 # Solve'Divide(_e, _v) <-- Failed;";
        scriptMap.put("Solve",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " Rulebase(\"xPSolve\",{uni});Rule(\"xPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 1)[ If(iDebug,Tell(\" xPSolve_1\",uni)); {-Coef(uni,0)/Coef(uni,1)};];Rule(\"xPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 2)[ If(iDebug,Tell(\" xPSolve_2\",uni)); Local(a,b,c,d,q,r); c:=Coef(uni,0); b:=Coef(uni,1); a:=Coef(uni,2); If(iDebug,Tell(\" \",{a,b,c})); d:=b*b-4*a*c; If(iDebug,Tell(\" \",d));  q:=Sqrt(d)/(2*a); If(iDebug,Tell(\" \",q)); r:=Simplify(-b/(2*a)); If(iDebug,Tell(\" \",r)); {r+q,r-q};];Rule(\"xPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 3 )[ If(iDebug,Tell(\" xPSolve_3\",uni)); Local(p,q,r,s,t,w1,w2,a,b); Local(c0,c1,c3,adjust);  w1 := (1/2)*Complex(-1, Sqrt(3)); w2 := Conjugate(w1); If( iDebug, Tell(\" \",{w1,w2}) );   adjust := (-uni[3][3])/(3*uni[3][4]); If( iDebug, Tell(\" \",adjust)); c3 := uni[3][4]; c1 := (3*uni[3][4]*adjust+2*uni[3][3])*adjust+uni[3][2]; c0 :=((uni[3][4]*adjust+uni[3][3])*adjust+uni[3][2])*adjust+uni[3][1]; If( iDebug, Tell(\" \",{c0,c1,c3}));   Assert(\"Invariant\", \"Coefficients Must be Real\") And?(Im(c0)=?0,Im(c1)=?0,Im(c2)=?0); If( IsError(\"Invariant\"), DumpErrors() ); p :=c3; q :=c1/p; r :=c0/p; If( iDebug, Tell(\" \",{p,q,r})); Local(a3,b3,qq,r1,r2,r3); qq := Sqrt(q^3/27 + r^2/4); a3 := -r/2 + qq; b3 := -r/2 - qq;  If( iDebug, [Tell(\" \",{qq,a3,b3}); Tell(\" \",N(a3+b3+r)); Tell(\" \",N(a3-b3-2*qq));]); a := (a3)^(1/3); b := (b3)^(1/3); If( iDebug, Tell(\" \",{a,b})); r1 := a+b+adjust; r2 := w1*a+w2*b+adjust; r3 := w2*a+w1*b+adjust;  If( iDebug, [ Tell(\" \",r1); Tell(\" \",r2); Tell(\" \",r3); ] ); {r1,r2,r3};];Rule(\"xPSolve\",1,1,IsUniVar(uni) And? Degree(uni) =? 4 )[ If(iDebug,Tell(\" xPSolve_4\",uni)); Local(coef4,a1,a2,a3,a4,y,y1,z,t,s); coef4:=Coef(uni,4); a1:=Coef(uni,3)/coef4; a2:=Coef(uni,2)/coef4; a3:=Coef(uni,1)/coef4; a4:=Coef(uni,0)/coef4; If( iDebug, Tell(\" \",{a1,a2,a3,a4}));  Local(ys); ys := xPSolveCubic(y^3-a2*y^2+(a1*a3-4*a4)*y+(4*a2*a4-a3^2-a1^2*a4)); If( iDebug, [NewLine(); Tell(\" \",ys[1]);] ); y1:=First(ys); If( iDebug, Tell(\" \",y1)); t := Sqrt(y1^2/4-a4); If( iDebug, Tell(\" \",t)); If(t=?0, s:=Sqrt(y1+a1^2/4-a2), s:=(a1*y1-2*a3)/(4*t)); If( iDebug, Tell(\" \",s));  Local(q11,q12,q21,q2,quad1,quad2); q11 := a1/2+s; q12 := y1/2+t; q21 := a1/2-s; q22 := y1/2-t; If( iDebug, Tell(\" \",{q11,q12})); If( iDebug, Tell(\" \",{q21,q22})); quad1 := z^2 + q11*z + q12; quad2 := z^2 + q21*z + q22; If( iDebug, Tell(\" \",{quad1,quad2}));  Local(r1,r2,r3,r4); {r1,r2} := xPSolve( quad1, z ); {r3,r4} := xPSolve( quad2, z ); r1 := NearRational(N(r1,10),8); r2 := NearRational(N(r2,10),8); r3 := NearRational(N(r3,10),8); r4 := NearRational(N(r4,10),8);  {r1,r2,r3,r4};];Function(\"xPSolve\",{expr,var})[ If( Not? IsBound(iDebug), iDebug := False ); If(iDebug,Tell(\"xPSolve_notUni\",{expr,var}));  Local(lhs,rhs,cc,pp,uni,solnpp,solncc,soln); If( Equation?(expr),  [ If(iDebug,Tell(\" is Equation\")); lhs := EquationLeft(expr); rhs := EquationRight(expr); expr := lhs - rhs; ] ); If(iDebug,Tell(\" \",expr)); cc := xContent(expr); pp := xPrimitivePart(expr,cc); If(iDebug,Tell(\" \",{cc,pp})); solnpp := xPSolve(MakeUni(pp,var)); If(iDebug,Tell(\" \",solnpp)); If( Length(VarList(cc)) >? 0 And? Contains(VarList(cc),var ), [ solncc := xPSolve(MakeUni(cc,var)); If(iDebug,Tell(\" \",solncc)); soln := Concat(solncc,solnpp); ], [ soln := solnpp; ] ); soln;];10 # xPSolveCubic( poly_IsPolynomial )_ (Length(VarList(poly))=?1 And? Degree(poly)=?3) <--[ If( iDebug, Tell(\" xPSolveCubic\",poly) ); Local(var,coeffs,ans); var := VarList(poly)[1]; coeffs := Coef(poly,var,3 .. 0); If( iDebug, Tell(\" \",{var,coeffs})); ans := xPSC1(coeffs); ];UnFence(\"xPSolveCubic\",1);10 # xPSC1( coeffs_IsList ) <--[ If( iDebug, Tell(\" xPSC1\",coeffs) );  Local(f,g,h,j,iType,ans); f := coeffs[2]/coeffs[1]/3; g := coeffs[3]/coeffs[1]/3 - f^2; h := coeffs[4]/coeffs[1]/2 + f^3 - f * coeffs[3]/coeffs[1]/2; j := g^3 + h^2; If( iDebug, Tell(\" \",{f,g,h,j}) ); ans := xPSC2( {f,g,h,j} );];10 # xPSC2( xs_IsList )_(xs[4]=?0) <--[ If( iDebug, Tell(\" Type 1\",xs) );  Local(f,g,h,j,m,r1,r2,r3,ans); {f,g,h,j} := FlatCopy(xs); m := 2*(-h)^(1/3); r1 := NearRational(N(m - f,10),8); r2 := NearRational(N(-m/2 - f,10),8); r3 := NearRational(N(-m/2 - f,10),8); ans := {r1,r2,r3};];10 # xPSC2( xs_IsList )_(xs[4]>?0) <--[ If( iDebug, Tell(\" Type 2\",xs) );  Local(f,g,h,j,k,l1,l2,m,n,r1,r2,r3,ans);  {f,g,h,j} := FlatCopy(xs); k := Sqrt(j); l1 := (-h + k)^(1/3); l2 := (-h - k)^(1/3); m := l1 + l2; n := (l1 - l2)*Sqrt(3)/2; r1 := NearRational(N(m - f,10),8); r2 := NearRational(N(-m/2 - f + I*n,10),8); r3 := NearRational(N(Conjugate(r2),10),8); ans := {r1,r2,r3}; ];10 # xPSC2( xs_IsList )_(xs[4]<?0 And? xs[3]=?0) <--[ If( iDebug, Tell(\" Type 3a\",xs) ); Local(f,g,h,j,p,r1,r2,r3,ans); {f,g,h,j} := FlatCopy(xs); p := 2*Sqrt(-g); r1 := NearRational(N(-f,10),8); r2 := NearRational(N( p*Sqrt(3)/2 - f,10),8); r3 := NearRational(N(-p*Sqrt(3)/2 - f,10),8); ans := {r1,r2,r3};];10 # xPSC2( xs_IsList )_(xs[4]<?0 And? xs[3]>?0) <--[ If( iDebug, Tell(\" Type 3b\",xs) ); Local(p,x,alpha,beta,gama,r1,r2,r3,ans); {f,g,h,j} := FlatCopy(xs); p := 2*Sqrt(-g); k := Sqrt(-j); alpha := ArcTan(k/(-h));  beta := Pi + alpha; gama := beta / 3; If( iDebug, [ Tell(\" \",{p,k}); Tell(\" \",{alpha,beta,gama}); Tell(\" \",57.2957795*N({alpha,beta,gama})); Tell(\" \",N(Cos(gama))); ] ); r1 := NearRational(N(p * Cos(gama) - f,10),8); r2 := NearRational(N(p * Cos(gama+2*Pi/3) - f,10),8); r3 := NearRational(N(p * Cos(gama+4*Pi/3) - f,10),8); ans := {r1,r2,r3};];10 # xPSC2( xs_IsList )_(xs[4]<?0 And? xs[3]<?0) <--[ If( iDebug, Tell(\" Type 3c\",xs) ); Local(f,g,h,j,p,k,alpha,beta,gama,r1,r2,r3,ans); {f,g,h,j} := FlatCopy(xs); p := 2*Sqrt(-g); k := Sqrt(-j); alpha := ArcTan(k/(-h));  beta := alpha; gama := beta / 3; If(iDebug,[Tell(\" \",{p,k}); Tell(\" \",{alpha,beta,gama});]); r1 := NearRational(N(p * Cos(gama) - f,10),8); r2 := NearRational(N(p * Cos(gama+2*Pi/3) - f,10),8); r3 := NearRational(N(p * Cos(gama+4*Pi/3) - f,10),8); ans := {r1,r2,r3};];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Bernoulli(0) <-- 1;10 # Bernoulli(1) <-- -1/2;15 # Bernoulli(n_IsInteger)_(n<?0) <-- Undefined;30 # Bernoulli(n_IsOdd) <-- 0;20 # Bernoulli(n_Even?)_(n<=?Bernoulli1Threshold()) <-- Internal'BernoulliArray(n)[n+1];20 # Bernoulli(n_Even?)_(n>?Bernoulli1Threshold()) <-- Bernoulli1(n);LocalSymbols(bernoulli1Threshold) [  If(Not? IsBound(bernoulli1Threshold), bernoulli1Threshold := 20); Bernoulli1Threshold() := bernoulli1Threshold; SetBernoulli1Threshold(threshold) := [ bernoulli1Threshold := threshold;];] ; Bernoulli(n_IsInteger, _x) <-- [ Local(B, i, result); B := Internal'BernoulliArray(n); result := B[1]; For(i:=n-1, i>=?0, i--) [ result := result * x + B[n-i+1]*BinomialCoefficient(n,i); ]; result;];";
        scriptMap.put("Bernoulli",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # BesselI(0,0) <-- 1;10 # BesselI(_n,0)_(n>?0) <-- 0;10 # BesselI(_n,0)_(n<?0 And? IsInteger(n)) <-- 0;10 # BesselI(_n,0)_(n<?0 And? Not? IsInteger(n)) <-- Infinity;20 # BesselI(1/2,_x) <-- Sqrt(2/(x*Pi))*Sinh(x);20 # BesselI(3/2,_x) <-- Sqrt(2/(x*Pi))*(Cosh(x) - Sinh(x)/x);20 # BesselI(5/2,_x) <-- Sqrt(2/(x*Pi))*((3/x^2 + 1)*Sinh(x) - 3*Cosh(x)/x );30 # BesselI(_n,_z)_(n<?0 And? IsInteger(n) ) <-- BesselI(-n,z);40 # BesselI(_n,x_IsComplex)_(Constant?(x) And? Abs(x)<=? 2*Gamma(n) ) <--[ApproxInfSum((x/2)^(2*k+c[1])/(k! * Gamma(k+c[1]+1) ),0,x,{n} );];";
        scriptMap.put("BesselI",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # BesselJ(0,0) <-- 1;10 # BesselJ(_n,0)_(n>?0) <-- 0;10 # BesselJ(_n,0)_(n<?0 And? IsInteger(n)) <-- 0;10 # BesselJ(_n,0)_(n<?0 And? Not? IsInteger(n)) <-- Infinity;10 # BesselJ(0,Infinity)<-- 0;20 # BesselJ(1/2,_x) <-- Sqrt(2/(x*Pi))*Sin(x);20 # BesselJ(-1/2,_x) <-- Sqrt(2/(x*Pi))*Cos(x);20 # BesselJ(3/2,_x) <-- Sqrt(2/(x*Pi))*(Sin(x)/x - Cos(x));20 # BesselJ(-3/2,_x) <-- Sqrt(2/(x*Pi))*(Cos(x)/x + Sin(x));20 # BesselJ(5/2,_x) <-- Sqrt(2/(x*Pi))*((3/x^2 - 1)*Sin(x) - 3*Cos(x)/x );20 # BesselJ(-5/2,_x) <-- Sqrt(2/(x*Pi))*( (3/x^2 -1)*Cos(x) + 3*Sin(x)/x );30 # BesselJ(_n,_x)_(Constant?(x) And? IsInteger(n) And? N(Abs(x) >? 2*Gamma(n))) <-- N((2*(n+1)/x)*BesselJ(n+1,x) - BesselJ(n+2,x));30 # BesselJ(_n,_z)_(n<?0 And? IsInteger(n) ) <-- (-1)^n*BesselJ(-n,z);40 # BesselJ(_n,x_IsComplex)_(N(Abs(x)<=? 2*Gamma(n)) ) <--[ApproxInfSum((-1)^k*(x/2)^(2*k+c[1])/(k! * Gamma(k+c[1]+1) ),0,x,{n} );];50 # BesselJ(0,x_IsComplex)_(InNumericMode()) <-- N(BesselJN0(x));";
        scriptMap.put("BesselJ",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "40 # BesselY(_n,x_IsComplex)_(Abs(x)<=? 2*Gamma(n) ) <-- N((Cos(n*Pi)*BesselJ(n,x) - BesselJ(-n,x))/Sin(Pi*n));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Digamma(_n)_(IsPositiveInteger(n)) <-- Sum(m,1,n-1,1/m) - gamma;";
        scriptMap.put("Digamma",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # DirichletBeta(1) <-- Pi/4;5 # DirichletBeta(2) <-- Catalan;5 # DirichletBeta(3) <-- Pi^3/32;6 # DirichletBeta(n_IsOdd) <-- [ Local(k); k:=(n-1)/2; (-1)^k*Euler(2*k)*(Pi/2)^(2*k+1)/(2*(2*k)!);];10 # DirichletBeta(x_IsRationalOrNumber)_(InNumericMode() And? x>=?1 ) <-- [ Local(prec,eps,term,result,k); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+3); eps:=10^(-prec); result:=0; term:=1; For(k:=0, Abs(term) >? eps, k++ )[ term:=(-1)^k/(2*k+1)^x; Echo(\"term is \",term); result:=result+term; ]; BuiltinPrecisionSet(prec); RoundTo(result,prec);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Erf(0) <-- 0;10 # Erf(Infinity) <-- 1;10 # Erf(Undefined) <-- Undefined;10 # Erf(x_IsNumber)_(x<?0) <-- -Erf(-x);LocalSymbols(k)[ 40 # Erf(_x)_(InNumericMode() And? (IsNumber(x) Or? IsComplex(x)) And? Abs(x) <=? 1) <--[ Local(prec); prec := BuiltinPrecisionGet();  2 / SqrtN(Internal'Pi()) * x * SumTaylorNum(x^2, 1, {{k}, -(2*k-1)/(2*k+1)/k},  N(1+87/32*Exp(LambertW(prec*421/497)), 20) );];]; LocalSymbols(n'max, k)[ 50 # Erf(_x)_(InNumericMode() And? (IsNumber(x) Or? IsComplex(x)) And? ( [  n'max := 0; Re(x^2) >? BuiltinPrecisionGet()*3295/1431+0.121; ] Or? [  n'max := N(Minimum((BuiltinPrecisionGet()*3295/1431+0.121)/Internal'LnNum(Abs(x)), 2*Internal'LnNum(Abs(x))), 10); 2*Abs(x)+Re(x^2) >? BuiltinPrecisionGet()*3295/1431+0.121; ] Or? [   n'max := N(({{k}, k+Internal'LnNum(k)} @ BuiltinPrecisionGet()*3295/1431)/2 - 3/2, 10); Abs(x) >? n'max+3/2; ] ) ) <-- If(Re(x)!=?0, Sign(Re(x)), 0) - Exp(-x^2)/x/SqrtN(Internal'Pi())  * SumTaylorNum(1/x^2, 1, {{k}, -(2*k-1)/2 }, Maximum(0, Floor(n'max)));]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FresnelCos(0) <-- 0;10 # FresnelCos(Infinity) <-- 1/2;10 # FresnelCos(x_IsNumber)_(x<?0) <-- -FresnelCos(x);40 # FresnelCos(x_IsNumber)_(Abs(x) <=? 1) <-- N(Sqrt(2/Pi)*ApproxInfSum((-1)^(k+1)*x^(4*k-3)/((4*k-3) * (2*k-2)! ),1,x));";
        scriptMap.put("FresnelCos",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FresnelSin(0) <-- 0;10 # FresnelSin(Infinity) <-- 1/2;10 # FresnelSin(x_IsNumber)_(x<?0) <-- -FresnelSin(x);40 # FresnelSin(x_IsNumber)_(Abs(x) <=? 1) <-- N(Sqrt(2/Pi)*ApproxInfSum((-1)^(k+1)*x^(2*k+1)/(k! * (2*k+1)),1,x));";
        scriptMap.put("FresnelSin",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # Gamma(n_IsRationalOrNumber)_(IsPositiveInteger(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_Constant?)_(InNumericMode()) <-- Internal'GammaNum(N(Eval(x)));";
        scriptMap.put("Gamma",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LambertW(0) <-- 0;10 # LambertW(Infinity) <-- Infinity;10 # LambertW(Undefined) <-- Undefined;10 # LambertW(-Infinity) <-- Infinity + I*Pi;10 # LambertW(-Exp(-1)) <-- -1;20 # LambertW(_x * Ln(_x)) <-- Ln(x);20 # LambertW(Ln(_x) * _x) <-- Ln(x);30 # LambertW(x_Constant?) _ InNumericMode() <-- Internal'LambertWNum(Eval(x));10 # Internal'LambertWNum(x_IsNumber)_(x <? -ExpN(-1)) <-- Undefined;20 # Internal'LambertWNum(x_IsNumber) <--[ Local(W); NewtonNum( `Hold( { {W}, [ Local(a); a:=W- @x*ExpN(-W); W-a/(W+1-(W+2)/(W+1)*a/2.); ]}),  If( x<?0, x*ExpN(1) / (1+1 / (1 / SqrtN(2*(x*ExpN(1)+1)) - 1 / SqrtN(2) + 1/(ExpN(1)-1))), Internal'LnNum(1+x)*(1-Internal'LnNum(1+Internal'LnNum(1+x))/(2+Internal'LnNum(1+x))) ), 10,  3  );];";
        scriptMap.put("LambertW",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_IsRationalOrNumber)_(IsPositiveInteger(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_Constant?)_(InNumericMode()) <-- Internal'LnGammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zeta(1) <-- Infinity;10 # Zeta(0) <-- -1/2; 10 # Zeta(3)_InNumericMode() <-- Zeta3(); 10 # Zeta(n_Even?)_(n>?0) <-- Pi^n*(2^(n-1)/n! *Abs(Bernoulli(n)));10 # Zeta(n_IsInteger)_(n<?0) <-- -Bernoulli(-n+1)/(-n+1);11 # Zeta(n_IsInfinity) <-- 1;20 # Zeta(s_Constant?)_(InNumericMode()) <-- Internal'ZetaNum(N(Eval(s)));";
        scriptMap.put("Zeta",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Bernoulli1(n_Even?)_(n>=?2) <-- [ Local(B, prec); prec := BuiltinPrecisionGet();   BuiltinPrecisionSet(10); BuiltinPrecisionSet( Ceil(N((1/2*Ln(8*Pi*n)-n+n*Ln(n/2/Pi))/Ln(10)))+3  ); If (InVerboseMode(), Echo({\"Bernoulli: using zeta funcion, precision \", BuiltinPrecisionSet(), \", n = \", n})); B := Floor(N(  If(  n>?250,  Internal'ZetaNum2(n, n/17+1),    Internal'ZetaNum1(n, n/17+1)  ) *N(2*n! /(2*Pi)^n)))  * (2*Modulo(n/2,2)-1)  + BernoulliFracPart(n);  BuiltinPrecisionSet(prec);  B;];";
        scriptMap.put("Bernoulli1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BernoulliFracPart(n_Even?)_(n>=?2) <-- [ Local(p, sum);  sum := 1/2+1/3;  If(IsPrime(n+1), sum := sum+1/(n+1)); If(IsPrime(n/2+1), sum := sum+1/(n/2+1));   For(p:=5, p<=?n/3+1, p:=NextPrime(p)) If(Modulo(n, p-1)=?0, sum := sum + 1/p);   Quotient(Numerator(sum), Denominator(sum)) - sum + Modulo(n/2,2);  ];";
        scriptMap.put("BernoulliFracPart",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Internal'BernoulliArray(n_IsInteger)_(n=?0 Or? n=?1) <-- [ Local(B); B:=ArrayCreate(n+1,0); B[1] := 1; If(n=?1, B[2] := -1/2); B;];20 # Internal'BernoulliArray(n_IsInteger) <-- [ Local(B, i, k, k2, bin); If (InVerboseMode(), Echo({\"Internal'BernoulliArray: using direct recursion, n = \", n})); B:=ArrayCreate(n+1, 0);     B[1] := 1; B[2] := -1/2; B[3] := 1/6; For(i:=4, i<=?n, i := i+2)  [  bin := 1;   B[i+1] := 1/2-1/(i+1)*(1 + Sum(k, 1, i/2-1, [ bin := bin * (i+3-2*k) * (i+2-2*k)/ (2*k-1) / (2*k); B[2*k+1]*bin;  ] ) ); ]; B;];";
        scriptMap.put("Internal'BernoulliArray",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'BernoulliArray1(n_Even?) _ (n>=?2) <--[ Local(C, f, k, j, denom, sum); C := ArrayCreate(n+1, 0); f := ArrayCreate(n/2, 0); C[1] := 1; C[2] := -1/2; C[3] := 1/12;  f[1] := 2;  For(k:=2, k<=?n/2, k++)  [  f[k] := f[k-1] * (2*k)*(2*k-1);  C[2*k+1] := 1/(1-4^(-k))/2*( [ denom := 4;  sum := 0; For(j:=1, j<=?k-1, j++) [ sum := sum + C[2*(k-j)+1]/denom/f[j];  denom := denom * 4; ]; (2*k-1)/denom/f[k] - sum; ] ); ];  For(k:=1, k<=?n/2, k++) C[2*k+1] := C[2*k+1] * f[k];  C;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'GammaNum(z) := N(Exp(Internal'LnGammaNum(z)));Internal'GammaNum(z,a) := N(Exp(Internal'LnGammaNum(z,a)));";
        scriptMap.put("Internal'GammaNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Internal'LnGammaNum(_z, _a)_(N(Re(z))<?0) <-- [ If (InVerboseMode(), Echo({\"Internal'LnGammaNum: using 1-z identity\"})); N(Ln(Pi/Sin(Pi*z)) - Internal'LnGammaNum(1-z, a));];20 # Internal'LnGammaNum(_z, _a) <-- [ Local(e, k, tmpcoeff, coeff, result); a := Maximum(a, 4);  If (InVerboseMode(), Echo({\"Internal'LnGammaNum: precision parameter = \", a})); e := N(Exp(1)); k:=Ceil(a);  result := 0;   While(k>?1) [  k:=k-1; result := N( PowerN(a-k,k)/((z+k)*Sqrt(a-k))-result/(e*k) ); ]; N(Ln(1+Exp(a-1)/Sqrt(2*Pi)*result) + Ln(2*Pi)/2 -a-z+(z+1/2)*Ln(z+a) - Ln(z));];Internal'LnGammaNum(z) := [ Local(a, prec, result); prec := BuiltinPrecisionGet(); a:= Quotient((prec-IntLog(prec,10))*659, 526) + 0.4;     BuiltinPrecisionSet(Ceil(prec*1.4));  result := Internal'LnGammaNum(z,a); BuiltinPrecisionSet(prec); result;];";
        scriptMap.put("Internal'LnGammaNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GammaConstNum() :=[ Local(k, n, A, B, U'old, U, V'old, V, prec, result); prec:=BuiltinPrecisionGet(); NonN([ BuiltinPrecisionSet(prec+IntLog(prec,10)+3);  n:= 1+Ceil(prec*0.5757+0.2862);  A:= -Internal'LnNum(n); B:=1; U:=A; V:=1; k:=0; U'old := 0;  V'old := 0; While(U'old-U !=? 0 Or? V'old-V !=? 0) [ k++; U'old:=U; V'old:=V;  B:=MultiplyNum(B,n^2/k^2);   A:=MultiplyNum(MultiplyNum(A,n^2/k)+B, 1/k);  U:=U+A; V:=V+B; ]; If(InVerboseMode(), Echo(\"GammaConstNum: Info: used\", k, \"iterations at working precision\", BuiltinPrecisionGet())); result:=DivideN(U,V);  ]); BuiltinPrecisionSet(prec);  RoundTo(result, prec); ];";
        scriptMap.put("GammaConstNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'ZetaNum(_s, n_IsInteger) <-- [ Local(result, j, sign); If (InVerboseMode(), Echo({\"Internal'ZetaNum: Borwein's method, precision \", BuiltinPrecisionGet(), \", n = \", n})); result := 0; sign := 1;  For(j:=0, j<=?2*n-1, j++) [  result := N(result + sign*Internal'ZetaNumCoeffEj(j,n)/(1+j)^s ); sign := -sign; ]; N(result/(2^n)/(1-2^(1-s)));];Internal'ZetaNum1(s, limit) := [ Local(i, sum); If (InVerboseMode(), Echo({\"Internal'ZetaNum: direct method (sum), precision \", BuiltinPrecisionGet(), \", N = \", limit})); sum := 0; limit := Ceil(N(limit)); For(i:=2, i<=?limit, i++) sum := sum+N(1/PowerN(i, s)); sum+1; ];Internal'ZetaNum2(s, limit) :=[ Local(i, prod); If (InVerboseMode(), Echo({\"Internal'ZetaNum: direct method (product), precision \", BuiltinPrecisionGet(), \", N = \", limit})); prod := N( (1-1/PowerN(2, s))*(1-1/PowerN(3,s)) ); limit := Ceil(N(limit)); For(i:=5, i<=?limit, i:= NextPrime(i)) prod := prod*N(1-1/PowerN(i, s)); 1/prod;];Internal'ZetaNumCoeffEj(j,n) := [ Local(k); 2^n-If(j<?n, 0, Sum(k,0,j-n,BinomialCoefficient(n,k))  );];Zeta3() :=[ Local(result, old'result, k, term); N([ For( [ k:=1; result := 1; old'result := -1; term := 1; ], old'result!=?result, k++ ) [ old'result := result; term := -term * k^2 / ((2*k+1)*(2*k)); result := result + term/(k+1)^2; ]; result := 5/4*result; ], BuiltinPrecisionGet()+IntLog(BuiltinPrecisionGet(),10)+1); result;];10 # Internal'ZetaNum(_s) _ (N(s)=?0) <-- -0.5;10 # Internal'ZetaNum(_s) _ (N(s)=?1) <-- Infinity;20 # Internal'ZetaNum(_s) <-- [ Local(n, prec, result); prec := BuiltinPrecisionGet(); If(  N(Re(s)) <? 0.5,  [ If(InVerboseMode(), Echo({\"Internal'ZetaNum: using s->1-s identity, s=\", s, \", precision \", prec})); result := 2*Exp(Internal'LnGammaNum(1-s)-(1-s)*Ln(2*Internal'Pi()))*Sin(Internal'Pi()*s/2) * Internal'ZetaNum(1-s); ],  If (N(Re(s)) >? N(1+(prec*Ln(10))/(Ln(prec)+0.1), 6), [  n:= N(10^(prec/(s-1)), 6)+2;  BuiltinPrecisionSet(prec+2);  result := Internal'ZetaNum1(s, n); ], [  n := Ceil( N( prec*Ln(10)/Ln(8) + 2, 6 ) );  BuiltinPrecisionSet(prec+2);  result := Internal'ZetaNum(s, n); ] ) ); BuiltinPrecisionSet(prec); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GeometricMean(list) := [ Check(IsList(list), \"Argument\", \"Argument must be a list.\");  Product(list)^(1/Length(list));];";
        scriptMap.put("GeometricMean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Mean(list) := [ Check(IsList(list), \"Argument\", \"Argument must be a list.\");  Sum(list)/Length(list);];";
        scriptMap.put("Mean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Median(list) :=[ Check(IsList(list), \"Argument\", \"Argument must be a list.\");  Local(sx,n,n2);   sx := HeapSort(list,\"<?\");  n := Length(list);  n2 := (n>>1);  If(Modulo(n,2) =? 1, sx[n2+1], (sx[n2]+sx[n2+1])/2);];";
        scriptMap.put("Median",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "StandardDeviation(list) := [ Check(IsList(list), \"Argument\", \"Argument must be a list.\"); Sqrt(UnbiasedVariance(list));];";
        scriptMap.put("StandardDeviation",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "UnbiasedVariance(list) := [ Check(IsList(list), \"Argument\", \"Argument must be a list.\");  Sum((list - Mean(list))^2)/(Length(list)-1);];";
        scriptMap.put("UnbiasedVariance",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Variance(list) := [ Check(IsList(list), \"Argument\", \"Argument must be a list.\"); Sum((list - Mean(list))^2)/Length(list);];";
        scriptMap.put("Variance",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BernoulliDistribution(p_IsRationalOrNumber)_(p<?0 Or? p>?1) <-- Undefined;";
        scriptMap.put("BernoulliDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistribution(_p, _n)_ (If(IsRationalOrNumber(p),p<?0 Or? p>?1, False) Or? (Constant?(n) And? Not? IsPositiveInteger(n)) ) <-- Undefined;";
        scriptMap.put("BinomialDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ChiSquareDistribution(m_IsRationalOrNumber)_(m<=?0) <-- Undefined;";
        scriptMap.put("ChiSquareDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ContinuousUniformDistribution(a_IsRationalOrNumber, b_IsRationalOrNumber)_(a>=?b) <-- Undefined;";
        scriptMap.put("ContinuousUniformDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DiscreteDistribution( dom_IsRationalOrNumber , prob_IsRationalOrNumber) <-- Undefined;";
        scriptMap.put("DiscreteDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DiscreteUniformDistribution(a_IsRationalOrNumber, b_IsRationalOrNumber)_(a>=?b) <-- Undefined;";
        scriptMap.put("DiscreteUniformDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ExponentialDistribution(l_IsRationalOrNumber)_(l<?0) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GeometricDistribution(p_IsRationalOrNumber)_(p<?0 Or? p>?1) <-- Undefined;";
        scriptMap.put("GeometricDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HypergeometricDistribution(N_IsRationalOrNumber, M_IsRationalOrNumber, n_IsRationalOrNumber)_(M >? N Or? n >? N) <-- Undefined;";
        scriptMap.put("HypergeometricDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NormalDistribution( _m , s2_IsRationalOrNumber)_(s2<=?0) <-- Undefined;";
        scriptMap.put("NormalDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PoissonDistribution(l_IsRationalOrNumber)_(l<=?0) <-- Undefined;";
        scriptMap.put("PoissonDistribution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "tDistribution(m_IsRationalOrNumber)_(Not? IsPositiveInteger(m)) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ExpressionDepth(expression_IsFunction) <--[ Local(result); result:=0; ForEach(item,Rest(FunctionToList(expression))) [ Local(newresult); newresult:=ExpressionDepth(item); result:=Maximum(result,newresult); ]; result+1;];20 # ExpressionDepth(_expression) <-- 1;UnFence(\"ExpressionDepth\",1);";
        scriptMap.put("ExpressionDepth",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # ArcCos(x_IsNumber)_InNumericMode() <-- Internal'Pi()/2-ArcSin(x); 200 # ArcCos(0) <-- Pi/2;200 # ArcCos(1/2) <-- Pi/3;200 # ArcCos(Sqrt(1/2)) <-- Pi/4;200 # ArcCos(Sqrt(3/4)) <-- Pi/6;200 # ArcCos(1) <-- 0;200 # ArcCos(_n)_(n =? -1) <-- Pi;200 # ArcCos(_n)_(-n =? Sqrt(3/4)) <-- 5/6*Pi;200 # ArcCos(_n)_(-n =? Sqrt(1/2)) <-- 3/4*Pi;200 # ArcCos(_n)_(-n =? 1/2) <-- 2/3*Pi;200 # ArcCos(Undefined) <-- Undefined;ArcCos(xlist_IsList) <-- MapSingle(\"ArcCos\",xlist);110 # ArcCos(Complex(_r,_i)) <-- (- I)*Ln(Complex(r,i) + (Complex(r,i)^2 - 1)^(1/2));";
        scriptMap.put("ArcCos",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcCosh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Ln(x+Sqrt(x^2-1)) ));200 # ArcCosh(Infinity) <-- Infinity;200 # ArcCosh(-Infinity) <-- Infinity+I*Pi/2;200 # ArcCosh(Undefined) <-- Undefined;ArcCosh(xlist_IsList) <-- MapSingle(\"ArcCosh\",xlist);";
        scriptMap.put("ArcCosh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # ArcSin(x_IsNumber)_(InNumericMode() And? Abs(x)<=?1) <-- ArcSinNum(x);3 # ArcSin(x_IsNumber)_InNumericMode() <-- Sign(x)*(Pi/2+I*ArcCosh(x));110 # ArcSin(Complex(_r,_i)) <-- (- I) * Ln((I*Complex(r,i)) + ((1-(Complex(r,i)^2))^(1/2)));150 # ArcSin(- _x)_(Not? Constant?(x)) <-- -ArcSin(x);160 # (ArcSin(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- -ArcSin(-x);200 # ArcSin(0) <-- 0;200 # ArcSin(1/2) <-- Pi/6;200 # ArcSin(Sqrt(1/2)) <-- Pi/4;200 # ArcSin(Sqrt(3/4)) <-- Pi/3;200 # ArcSin(1) <-- Pi/2;200 # ArcSin(_n)_(n =? -1) <-- -Pi/2;200 # ArcSin(_n)_(-n =? Sqrt(3/4)) <-- -Pi/3;200 # ArcSin(_n)_(-n =? Sqrt(1/2)) <-- -Pi/4;200 # ArcSin(_n)_(-n =? 1/2) <-- -Pi/6;ArcSin(xlist_IsList) <-- MapSingle(\"ArcSin\",xlist);200 # ArcSin(Undefined) <-- Undefined;";
        scriptMap.put("ArcSin",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcSinh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Ln(x+Sqrt(x^2+1)) ));200 # ArcSinh(Infinity) <-- Infinity;200 # ArcSinh(-Infinity) <-- -Infinity;200 # ArcSinh(Undefined) <-- Undefined;ArcSinh(xlist_IsList) <-- MapSingle(\"ArcSinh\",xlist);";
        scriptMap.put("ArcSinh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (ArcTan(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- -ArcTan(-x);4 # ArcTan(-Tan(_x)) <-- -ArcTan(Tan(x));110 # ArcTan(Complex(_r,_i)) <-- (- I*0.5)*Ln(Complex(1,Complex(r,i))/ Complex(1, - Complex(r,i)));150 # ArcTan(- _x)_(Not? Constant?(x)) <-- -ArcTan(x);160 # (ArcTan(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- -ArcTan(-x);200 # ArcTan(Sqrt(3)) <-- Pi/3;200 # ArcTan(-Sqrt(3)) <-- -Pi/3;200 # ArcTan(1) <-- Pi/4;200 # ArcTan(0) <-- 0;200 # ArcTan(_n)_(n =? -1) <-- -Pi/4;200 # ArcTan(Infinity) <-- Pi/2;200 # ArcTan(-Infinity) <-- -Pi/2;200 # ArcTan(Undefined) <-- Undefined;ArcTan(xlist_IsList) <-- MapSingle(\"ArcTan\",xlist);2 # ArcTan(x_IsNumber)_InNumericMode() <-- ArcTanNum(x);";
        scriptMap.put("ArcTan",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcTanh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Ln((1+x)/(1-x))/2 ));200 # ArcTanh(Infinity) <-- Infinity+I*Pi/2;200 # ArcTanh(-Infinity) <-- -Infinity-I*Pi/2; 200 # ArcTanh(Undefined) <-- Undefined;ArcTanh(xlist_IsList) <-- MapSingle(\"ArcTanh\",xlist);";
        scriptMap.put("ArcTanh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # CosMap( _n )_(Not?(IsRationalOrNumber(n))) <-- ListToFunction({ToAtom(\"Cos\"),n*Pi});2 # CosMap( _n )_(n<?0) <-- CosMap(-n);2 # CosMap( _n )_(n>?2) <-- CosMap(Modulo(n,2));3 # CosMap( _n )_(n>?1) <-- CosMap(2-n);4 # CosMap( _n )_(n>?1/2) <-- -CosMap(1-n);5 # CosMap( 0 ) <-- 1;5 # CosMap( 1/6 ) <-- Sqrt(3)/2;5 # CosMap( 1/4 ) <-- Sqrt(2)/2;5 # CosMap( 1/3 ) <-- 1/2;5 # CosMap( 1/2 ) <-- 0;5 # CosMap( 2/5 ) <-- (Sqrt(5)-1)/4;10 # CosMap(_n) <-- ListToFunction({ToAtom(\"Cos\"),n*Pi});2 # Cos(x_IsNumber)_InNumericMode() <-- CosNum(x);4 # Cos(ArcCos(_x)) <-- x;4 # Cos(ArcSin(_x)) <-- Sqrt(1-x^2);4 # Cos(ArcTan(_x)) <-- 1/Sqrt(1+x^2);5 # Cos(- _x)_(Not? Constant?(x)) <-- Cos(x);6 # (Cos(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- Cos(-x);110 # Cos(Complex(_r,_i)) <-- (Exp(I*Complex(r,i)) + Exp(- I*Complex(r,i))) / (2) ;6 # Cos(x_IsInfinity) <-- Undefined;6 # Cos(Undefined) <-- Undefined;200 # Cos(v_CanBeUni(Pi))_(Not?(InNumericMode()) And? Degree(v,Pi) <? 2 And? Coef(v,Pi,0) =? 0) <-- CosMap(Coef(v,Pi,1));400 # Cos(x_IsRationalOrNumber) <-- [ Local(ll); ll:= FloorN(N(Eval(x/Pi))); If(Even?(ll),x:=(x - Pi*ll),x:=(-x + Pi*(ll+1))); ListToFunction({Cos,x}); ];400 # Cos(x_IsRationalOrNumber) <-- [ Local(ll); ll:= FloorN(N(Eval(Abs(x)/Pi))); If(Even?(ll),x:=(Abs(x) - Pi*ll),x:=(-Abs(x) + Pi*(ll+1))); ListToFunction({Cos,x}); ];100 # Cos(_x)*Tan(_x) <-- Sin(x);100 # Cos(_x)/Sin(_x) <-- (1/Tan(x));Cos(xlist_IsList) <-- MapSingle(\"Cos\",xlist);";
        scriptMap.put("Cos",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Cosh(- _x) <-- Cosh(x);100 # Cosh(_x)*Sech(_x) <-- 1;200 # Cosh(0) <-- 1;200 # Cosh(Infinity) <-- Infinity;200 # Cosh(-Infinity) <-- Infinity;200 # Cosh(ArcCosh(_x)) <-- x;200 # Cosh(ArcSinh(_x)) <-- Sqrt(1+x^2);200 # Cosh(ArcTanh(_x)) <-- 1/Sqrt(1-x^2);200 # Cosh(Undefined) <-- Undefined;Cosh(xlist_IsList) <-- MapSingle(\"Cosh\",xlist);2 # Cosh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( (Exp(x)+Exp(-x))/2 ));";
        scriptMap.put("Cosh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # 1/Cot(_x) <-- Tan(x);100 # Cot(_x) <-- 1/Tan(x);";
        scriptMap.put("Cot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # 1/Coth(_x) <-- Tanh(x);100 # Coth(_x) <-- 1/Tanh(x);100 # Coth(_x)*Sinh(_x) <-- Cosh(x);";
        scriptMap.put("Coth",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # 1/Csc(_x) <-- Sin(x);100 # Csc(_x) <-- 1/Sin(x);";
        scriptMap.put("Csc",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # 1/Csch(_x) <-- Sinh(x);100 # Csch(_x) <-- 1/Sinh(x);";
        scriptMap.put("Csch",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Exp(x_IsNumber)_InNumericMode() <-- ExpNum(x);4 # Exp(Ln(_x)) <-- x;110 # Exp(Complex(_r,_i)) <-- Exp(r)*(Cos(i) + I*Sin(i));200 # Exp(0) <-- 1;200 # Exp(-Infinity) <-- 0;200 # Exp(Infinity) <-- Infinity;200 # Exp(Undefined) <-- Undefined;Exp(xlist_IsList) <-- MapSingle(\"Exp\",xlist);";
        scriptMap.put("Exp",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Ln(0) <-- -Infinity;2 # Ln(1) <-- 0;2 # Ln(Infinity) <-- Infinity;2 # Ln(Undefined) <-- Undefined;2 # Ln(x_IsNegativeNumber)_InNumericMode() <-- Complex(Ln(-x), Pi);3 # Ln(x_IsNumber)_(InNumericMode() And? x>=?1) <-- Internal'LnNum(x);4 # Ln(Exp(_x)) <-- x;3 # Ln(Complex(_r,_i)) <-- Complex(Ln(Abs(Complex(r,i))), Arg(Complex(r,i)));4 # Ln(x_IsNegativeNumber) <-- Complex(Ln(-x), Pi);5 # Ln(x_IsNumber)_(InNumericMode() And? x<?1) <-- - Internal'LnNum(DivideN(1, x));Ln(xlist_IsList) <-- MapSingle(\"Ln\",xlist);";
        scriptMap.put("Ln",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # 1/Sec(_x) <-- Cos(x);100 # Sec(_x) <-- 1/Cos(x);";
        scriptMap.put("Sec",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "100 # Sech(_x) <-- 1/Cosh(x);100 # 1/Sech(_x) <-- Cosh(x);";
        scriptMap.put("Sech",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # SinMap( _n )_(Not?(IsRationalOrNumber(n))) <-- ListToFunction({ToAtom(\"Sin\"),n*Pi});2 # SinMap( _n )_(n<?0) <-- -SinMap(-n);2 # SinMap( _n )_(n>?2) <-- SinMap(Modulo(n,2));3 # SinMap( _n )_(n>?1) <-- SinMap(n-2);4 # SinMap( _n )_(n>?1/2) <-- SinMap(1-n);5 # SinMap( n_IsInteger ) <-- 0;5 # SinMap( 1/6 ) <-- 1/2;5 # SinMap( 1/4 ) <-- Sqrt(2)/2;5 # SinMap( 1/3 ) <-- Sqrt(3)/2;5 # SinMap( 1/2 ) <-- 1;5 # SinMap( 1/10) <-- (Sqrt(5)-1)/4;10 # SinMap(_n) <-- ListToFunction({ToAtom(\"Sin\"),n*Pi});2 # Sin(x_IsNumber)_InNumericMode() <-- SinNum(x);4 # Sin(ArcSin(_x)) <-- x;4 # Sin(ArcCos(_x)) <-- Sqrt(1-x^2);4 # Sin(ArcTan(_x)) <-- x/Sqrt(1+x^2);5 # Sin(- _x)_(Not? Constant?(x)) <-- -Sin(x);6 # (Sin(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- -Sin(-x);6 # Sin(x_IsInfinity) <-- Undefined;6 # Sin(Undefined) <-- Undefined;110 # Sin(Complex(_r,_i)) <-- (Exp(I*Complex(r,i)) - Exp(- I*Complex(r,i))) / (I*2) ;200 # Sin(v_CanBeUni(Pi))_(Not?(InNumericMode()) And? Degree(v,Pi) <? 2 And? Coef(v,Pi,0) =? 0) <--[ SinMap(Coef(v,Pi,1));];100 # Sin(_x)/Tan(_x) <-- Cos(x);100 # Sin(_x)/Cos(_x) <-- Tan(x);Sin(xlist_IsList) <-- MapSingle(\"Sin\",xlist);";
        scriptMap.put("Sin",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Sinh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( (Exp(x)-Exp(-x))/2 ));5 # Sinh(- _x) <-- -Sinh(x);5 # Sinh(- _x) <-- -Sinh(x);100 # Sinh(_x)^2-Cosh(_x)^2 <-- 1;100 # Sinh(_x)+Cosh(_x) <-- Exp(x);100 # Sinh(_x)-Cosh(_x) <-- Exp(-x);100 # Sinh(_x)/Cosh(_x) <-- Tanh(x);100 # Sinh(_x)*Csch(_x) <-- 1;200 # Sinh(0) <-- 0;200 # Sinh(Infinity) <-- Infinity;200 # Sinh(-Infinity) <-- -Infinity;200 # Sinh(ArcSinh(_x)) <-- x;200 # Sinh(ArcCosh(_x)) <-- Sqrt((x-1)/(x+1))*(x+1);200 # Sinh(ArcTanh(_x)) <-- x/Sqrt(1-x^2);200 # Sinh(Undefined) <-- Undefined;Sinh(xlist_IsList) <-- MapSingle(\"Sinh\",xlist);";
        scriptMap.put("Sinh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # TanMap( _n )_(Not?(IsRationalOrNumber(n))) <-- ListToFunction({ToAtom(\"Tan\"),n*Pi});2 # TanMap( _n )_(n<?0) <-- -TanMap(-n);2 # TanMap( _n )_(n>?1) <-- TanMap(Modulo(n,1));4 # TanMap( _n )_(n>?1/2) <-- -TanMap(1-n);5 # TanMap( 0 ) <-- 0;5 # TanMap( 1/6 ) <-- 1/3*Sqrt(3);5 # TanMap( 1/4 ) <-- 1;5 # TanMap( 1/3 ) <-- Sqrt(3);5 # TanMap( 1/2 ) <-- Infinity;10 # TanMap(_n) <-- ListToFunction({ToAtom(\"Tan\"),n*Pi});2 # Tan(x_IsNumber)_InNumericMode() <-- TanNum(x);4 # Tan(ArcTan(_x)) <-- x;4 # Tan(ArcSin(_x)) <-- x/Sqrt(1-x^2);4 # Tan(ArcCos(_x)) <-- Sqrt(1-x^2)/x;5 # Tan(- _x)_(Not? Constant?(x)) <-- -Tan(x);6 # (Tan(x_Constant?))_(IsNegativeNumber(N(Eval(x)))) <-- -Tan(-x);6 # Tan(Infinity) <-- Undefined;6 # Tan(Undefined) <-- Undefined;110 # Tan(Complex(_r,_i)) <-- Sin(Complex(r,i))/Cos(Complex(r,i));200 # Tan(v_CanBeUni(Pi))_(Not?(InNumericMode()) And? Degree(v,Pi) <? 2 And? Coef(v,Pi,0) =? 0) <-- TanMap(Coef(v,Pi,1));100 # Tan(_x)/Sin(_x) <-- (1/Cos(x));100 # Tan(_x)*Cos(_x) <-- Sin(x);Tan(xlist_IsList) <-- MapSingle(\"Tan\",xlist);";
        scriptMap.put("Tan",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Tanh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Sinh(x)/Cosh(x) ));100 # Tanh(_x)*Cosh(_x) <-- Sinh(x);200 # Tanh(0) <-- 0;200 # Tanh(Infinity) <-- 1;200 # Tanh(-Infinity) <-- -1;200 # Tanh(ArcTanh(_x)) <-- x;200 # Tanh(ArcSinh(_x)) <-- x/Sqrt(1+x^2);200 # Tanh(ArcCosh(_x)) <-- Sqrt((x-1)/(x+1))*(x+1)/x;200 # Tanh(Undefined) <-- Undefined;Tanh(xlist_IsList) <-- MapSingle(\"Tanh\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ArcTanN'Taylor(x) :=[ Local(num'terms); num'terms := QuotientN(BuiltinPrecisionGet()*1068, -643*MathBitCount(x))+1;   x*SumTaylorNum(-MultiplyN(x,x), {{k}, 1/(2*k+1)}, num'terms);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # BisectSqrt(0) <-- 0;10 # BisectSqrt(1) <-- 1;20 # BisectSqrt(N_IsPositiveInteger) <--[ Local(l2,u,v,u2,v2,uv2,n);  u := N; l2 := MathBitCount(u)-1;     l2 := l2>>1;  u := 1 << l2; u2 := u << l2;  While( l2 !=? 0 ) [ l2--;  v := 1<<l2; v2 := v<<l2;    uv2 := u<<(l2 + 1);   n := u2 + uv2 + v2;     if( n <=? N ) [ u := u+v;  u2 := n;  ]; ]; u; ];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CosN'Doubling(value, n) :=[ Local(shift, result); shift := n; result := value; While (shift>?0)  [ result := MultiplyN(MathMul2Exp(result, 1), 2 - result); shift--; ]; result;];";
        scriptMap.put("CosN'Doubling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CosN'Taylor(x) :=[ Local(num'terms, prec, Bx); prec := QuotientN(BuiltinPrecisionGet()*3919, 1702);  Bx := -QuotientN(MathBitCount(x)*1143, 1649)-2;  num'terms := QuotientN( QuotientN( prec-1, QuotientN( MathBitCount( prec-1)*1588, 2291)+Bx), 2)+1;  SumTaylorNum(MultiplyN(x,x), 1, {{k}, -1/(2*k*(2*k-1))}, num'terms);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ExpN'Doubling1(value, n) :=[ Local(shift, result); shift := n; result := value; While (shift>?0)  [ result := MathMul2Exp(result, 1) + MultiplyN(result, result); shift--; ]; result;];";
        scriptMap.put("ExpN'Doubling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ExpN'Taylor(x) :=[ Local(num'terms, prec, Bx); prec := QuotientN(BuiltinPrecisionGet()*3919, 1702);  Bx := -QuotientN(MathBitCount(x)*1143, 1649)-2;  num'terms := QuotientN( prec-1, QuotientN( MathBitCount( prec-1)*1588, 2291)+Bx)+1;  x*SumTaylorNum(x, 1, {{k}, 1/(k+1)}, num'terms);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MathLn'Doubling(value, n) :=[ Local(shift, result); shift := n; result := value; While (shift>?0)  [ result := MultiplyN(result, result); shift--; ]; result;];";
        scriptMap.put("MathLn'Doubling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MathLn'Taylor(x) :=[ Local(num'terms, y); y := x-1; num'terms := QuotientN(BuiltinPrecisionGet()*2136, -643*MathBitCount(y))+1;   y*SumTaylorNum(-y, {{k}, 1/(k+1)}, num'terms);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SinN'Taylor(x) :=[ Local(num'terms, prec, Bx); prec := QuotientN(BuiltinPrecisionGet()*3919, 1702);  Bx := -QuotientN(MathBitCount(x)*1143, 1649)-2;  num'terms := QuotientN( QuotientN( prec+Bx, QuotientN( MathBitCount( prec+Bx)*1588, 2291)+Bx)+1, 2)+1;  x*SumTaylorNum(MultiplyN(x,x), 1, {{k}, -1/(2*k*(2*k+1))}, num'terms);];";
        scriptMap.put("SinN'Taylor",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SinN'Tripling(value, n) :=[ Local(shift, result); shift := n; result := value; While (shift>?0)  [  result := MultiplyN(result, 3 - MathMul2Exp(MultiplyN(result,result), 2) ); shift--; ]; result;];";
        scriptMap.put("SinN'Tripling",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SqrtN(x) := MathSqrt1(x); 10 # MathSqrt1(0) <-- 0;100 # MathSqrt1(_x) <-- [ Echo(\"SqrtN: invalid argument: \", x); False;];30 # MathSqrt1(x_IsPositiveNumber) <-- x*NewtonNum({{r}, r+r*(1-x*r^2)/2}, FastPower(x,-0.5), 4, 2);30 # MathSqrt1(x_IsPositiveNumber) <-- MathSqrtFloat(x);20 # MathSqrt1(x_IsInteger) _ (IsGreaterThan(x,0)) <--[ Local(result); If(ModuloN(x,4)<?2 And? ModuloN(x,3)<?2 And? ModuloN(x+1,5)<?3,  [  GlobalPush(BuiltinPrecisionGet()); If(MathBitCount(x)+3>?DigitsToBits(BuiltinPrecisionGet(), 10), BuiltinPrecisionSet(BitsToDigits(MathBitCount(x), 10)+1));    result := MathSqrtFloat(x+0.);  If(FloatIsInt?(SetExactBitsN(result, GetExactBitsN(result)-3)), result:= Floor(result+0.5)); BuiltinPrecisionSet(GlobalPop()); ],  result := MathSqrtFloat(x+0.) );  SetExactBitsN(result, DigitsToBits(BuiltinPrecisionGet(),10));];";
        scriptMap.put("SqrtN",scriptString);
        scriptMap.put("MathSqrt1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ArcSinNum(x) :=[  If( 239*Abs(x) >=? 169,   Sign(x)*(Internal'Pi()/2-ArcSinN(Sqrt(1-x^2))), ArcSinN(x) );];";
        scriptMap.put("ArcSinNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ArcTanNum(x) :=[  If( Abs(x)>?1, Sign(x)*(Internal'Pi()/2-ArcSin(1/Sqrt(x^2+1))), ArcSin(x/Sqrt(x^2+1)) );];";
        scriptMap.put("ArcTanNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(BrentCacheOfConstantsN)[ CachedConstant(BrentCacheOfConstantsN, Ln2, Internal'LnNum(2)); ];10 # BrentLn(x_IsInteger)_(BuiltinPrecisionGet()>?40) <--[ Local(y, n, k, eps); n := BuiltinPrecisionGet();   k := 1 + Quotient(IntLog(4*10^n, x), 2);  BuiltinPrecisionSet(n+5);  eps := DivideN(1, 10^n);  y := PowerN(x, k);   y := DivideN(Internal'Pi()*y, (2*k)*ArithmeticGeometricMean(4, y, eps)); BuiltinPrecisionSet(n); RoundTo(y, n); ];15 # BrentLn(x_IsInteger) <-- LogN(x);20 # BrentLn(_x)_(x<?1) <-- -BrentLn(1/x);30 # BrentLn(_x)_(BuiltinPrecisionGet()>?85) <--[ Local(y, n, n1, k, eps); N([ n := BuiltinPrecisionGet();   n1 := n + IntLog(n,10);   k := 2 + Quotient(n1*28738, 2*8651)   - IntLog(Floor(x), 2);  BuiltinPrecisionSet(n1+2+Quotient(k*3361, 11165));  eps := DivideN(1, 10^(n1+1));  y := x*2^(k-2);    y:=Internal'Pi()*y/(2*ArithmeticGeometricMean(1,y,eps)) - k*Ln2(); BuiltinPrecisionSet(n); ]); y; ];40 # BrentLn(x_IsNumber) <-- LogN(x);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ExpNum(x_IsNumber) _ (x >? MathExpThreshold()) <-- [ Local(i, y); i:=0; For(i:=0, x >? MathExpThreshold(), i++) x := DivideN(x, 2.); For(y:= ExpN(x), i>?0, i--) y := MultiplyN(y, y); y;];20 # ExpNum(x_IsNumber) _ (2*x <? -MathExpThreshold()) <-- DivideN(1, ExpNum(-x));30 # ExpNum(x_IsNumber) <-- ExpN(x);";
        scriptMap.put("ExpNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'LnNum(x_IsNumber)_(x>=?1) <-- NewtonLn(x);Internal'LnNum(x_IsNumber)_(0<?x And? x<?1) <-- - Internal'LnNum(DivideN(1,x));";
        scriptMap.put("Internal'LnNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LogN(x) := Internal'LnNum(x);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TruncRadian(_r) <--[ Local(twopi);  N([ r:=Eval(r); twopi:=2*Internal'Pi(); r:=r-FloorN(r/twopi)*twopi; ], BuiltinPrecisionGet() + IntLog(Ceil(Abs(N(Eval(r), 1))), 10)); r;];HoldArgument(\"TruncRadian\",r);";
        scriptMap.put("TruncRadian",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(mathExpThreshold) [  mathExpThreshold := If(Not? IsBound(mathExpThreshold), 500); MathExpThreshold() := mathExpThreshold; SetMathExpThreshold(threshold) := [mathExpThreshold:= threshold; ];];";
        scriptMap.put("MathExpThreshold",scriptString);
        scriptMap.put("SetMathExpThreshold",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ArithmeticGeometricMean(a, b, eps) :=[ Check(IsPositiveReal(a) And? IsPositiveReal(b), \"Argument\", \"The first two arguments must be positive real numbers.\");  Check(IsPositiveInteger(eps), \"Argument\", \"The precision argument must be a positive integer.\");  a := N(a, eps);  b := N(b, eps);  Local(a1, b1);  If(InVerboseMode(), Echo(\"ArithmeticGeometricMean: Info: at prec. \", BuiltinPrecisionGet()));   While(Abs(a-b)>=?eps) [ a1 := DivideN(a+b, 2); b1 := SqrtN(MultiplyN(a, b));  a := a1; b := b1; ];  DivideN(a+b, 2);];";
        scriptMap.put("ArithmeticGeometricMean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IntPowerNum(_x, 0, _func, _unity) <-- unity;10 # IntPowerNum(_x, n_IsInteger, _func, _unity) <--[  Local(result);  While(n >? 0) [ If( (n&1) =? 1, If( IsBound(result),  result := Apply(func, {result,x}), result := x,  ) ); x := Apply(func, {x,x}); n := n>>1; ]; result;];";
        scriptMap.put("IntPowerNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() MultiplyNum(x, y, ...);Function() MultiplyNum(x);10 # MultiplyNum(x_IsList)_(Length(x)>?1) <-- MultiplyNum(First(x), Rest(x));10 # MultiplyNum(x_IsRational, y_IsRationalOrNumber) <--[ If( Type(y) =? \"/\",  DivideN(Numerator(x)*Numerator(y), Denominator(x)*Denominator(y)),   If( Numerator(x)=?1, DivideN(y, Denominator(x)), If( Denominator(x)=?1, MultiplyN(y, Numerator(x)), DivideN(MultiplyN(y, Numerator(x)), Denominator(x)) ) ) );];20 # MultiplyNum(x_IsNumber, y_IsRational) <-- MultiplyNum(y, x);25 # MultiplyNum(x_IsNumber, y_IsNumber) <-- MultiplyN(x,y);30 # MultiplyNum(Complex(r_IsNumber, i_IsNumber), y_IsRationalOrNumber) <-- Complex(MultiplyNum(r, y), MultiplyNum(i, y));35 # MultiplyNum(y_IsNumber, Complex(r_IsNumber, i_IsRationalOrNumber)) <-- MultiplyNum(Complex(r, i), y);40 # MultiplyNum(Complex(r1_IsNumber, i1_IsNumber), Complex(r2_IsNumber, i2_IsNumber)) <-- Complex(MultiplyNum(r1,r2)-MultiplyNum(i1,i2), MultiplyNum(r1,i2)+MultiplyNum(i1,r2));30 # MultiplyNum(x_IsRationalOrNumber, y_IsNumericList)_(Length(y)>?1) <-- MultiplyNum(MultiplyNum(x, First(y)), Rest(y));40 # MultiplyNum(x_IsRationalOrNumber, y_IsNumericList)_(Length(y)=?1) <-- MultiplyNum(x, First(y));";
        scriptMap.put("MultiplyNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Common'digits(x,y) :=[ Local(diff); diff := Abs(x-y); If( diff=?0, Infinity,  Quotient(IntLog(FloorN(DivideN(Maximum(Abs(x), Abs(y)), diff)), 2)*351, 1166) ); ];NewtonNum(_func, _x0) <-- NewtonNum(func, x0, 5); NewtonNum(_func, _x0, _prec0) <-- NewtonNum(func, x0, prec0, 2);NewtonNum(_func, _x'init, _prec0, _order) <--[ Check(prec0>=?4, \"Argument\", \"NewtonNum: Error: initial precision must be at least 4\"); Check(IsInteger(order) And? order>?1, \"Argument\", \"NewtonNum: Error: convergence order must be an integer and at least 2\"); Local(x0, x1, prec, exact'digits, int'part, initial'tries); N([ x0 := x'init; prec := BuiltinPrecisionGet(); int'part := IntLog(Ceil(Abs(x0)), 10);   BuiltinPrecisionSet(2+prec0-int'part);  x1 := (func @ x0);   exact'digits := 0; initial'tries := 5;  While(exact'digits*order <? prec0 And? initial'tries>?0) [ initial'tries--; x0 := x1; x1 := (func @ x0); exact'digits := Common'digits(x0, x1);  ];  If( Assert(\"value\", {\"NewtonNum: Error: need a more accurate initial value than\", x'init}) exact'digits >=? 1, [ exact'digits :=Minimum(exact'digits, prec0+2);  int'part := IntLog(Ceil(Abs(x1)), 10);  While(exact'digits*order <=? prec) [ exact'digits := exact'digits*order; BuiltinPrecisionSet(2+Minimum(exact'digits, Quotient(prec,order)+1)-int'part); x0 := x1; x1 := (func @ x0);  ];  BuiltinPrecisionSet(2+prec); x1 := RoundTo( (func @ x1), prec); ],  x1 := x'init ); BuiltinPrecisionSet(prec); ]); x1;];";
        scriptMap.put("NewtonNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SumTaylorNum0(_x, _nth'term'func, _n'terms) <-- SumTaylorNum0(x, nth'term'func, {}, n'terms);SumTaylorNum1(_x, _nth'term'func, _n'terms) <-- SumTaylorNum1(x, nth'term'func, {}, n'terms);SumTaylorNum(_x, _nth'term'func, _n'terms) <--If( n'terms >=? 30,   SumTaylorNum1(x, nth'term'func, n'terms), SumTaylorNum0(x, nth'term'func, n'terms));SumTaylorNum(_x, _nth'term'func, _next'term'factor, _n'terms) <--If( n'terms >=? 5,  SumTaylorNum1(x, nth'term'func, next'term'factor, n'terms), SumTaylorNum0(x, nth'term'func, next'term'factor, n'terms));1# SumTaylorNum0(_x, _nth'term'func, {}, _n'terms) <--[ Local(sum, k); N([  x:=Eval(x); sum := 0; For(k:=n'terms, k>=?0, k--) sum := AddN(sum*x, nth'term'func @ k); ]); sum;];2# SumTaylorNum0(_x, _nth'term'func, _next'term'factor, _n'terms) <--[ Local(sum, k, term, delta); N([ x:=Eval(x);  If (Constant?(nth'term'func), term := nth'term'func, term := (nth'term'func @ {0}), ); sum := term;  ]); NonN([ delta := 1; For(k:=1, k<=?n'terms And? delta !=? 0, k++) [ term := MultiplyNum(term, next'term'factor @ {k}, x);  delta := sum; sum := sum + term;  delta := Abs(sum-delta);  ]; ]); sum;];SumTaylorNum0(_x, _nth'term'func, _n'terms) <-- SumTaylorNum0(x, nth'term'func, {}, n'terms);SumTaylorNum1(x, nth'term'func, next'term'factor, n'terms) :=[  Local(sum, rows, cols, rows'tmp, last'power, i, j, x'power, term'tmp); N([  x:=Eval(x);  rows := IntNthRoot(n'terms+1, 2); cols := Quotient(n'terms+rows, rows);  Check(rows>?1 And? cols>?1, \"Argument\", \"SumTaylorNum1: Internal error: number of Taylor sum terms must be at least 4\"); rows'tmp := ArrayCreate(rows, 0); x'power := x ^ rows;      If( next'term'factor =? {}, term'tmp := 1,  If (Constant?(nth'term'func), term'tmp := nth'term'func, term'tmp := (nth'term'func @ {0}), ) ); ]); NonN([   For(i:=0, i<?cols, i++) [  For(j:=0, j<?rows And? (i<?cols-1 Or? i*rows+j<=?n'terms), j++)  [   If( next'term'factor =? {},  [ rows'tmp[j+1] := rows'tmp[j+1] + MultiplyNum(term'tmp, nth'term'func @ {i*rows+j}); ], [ rows'tmp[j+1] := rows'tmp[j+1] + term'tmp;  term'tmp := MultiplyNum(term'tmp, next'term'factor @ {i*rows+j+1});  ] ); ];  term'tmp := term'tmp*x'power;  ];   For([j:=rows; sum:=0;], j>?0, j--) sum := sum*x + rows'tmp[j]; ]); sum;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Abs(Infinity) <-- Infinity; 10 # Abs(n_IsNumber) <-- AbsN(n);10 # Abs(n_IsPositiveNumber/m_IsPositiveNumber) <-- n/m;10 # Abs(n_IsNegativeNumber/m_IsPositiveNumber) <-- (-n)/m;10 # Abs(n_IsPositiveNumber/m_IsNegativeNumber) <-- n/(-m);10 # Abs( Sqrt(_x)) <-- Sqrt(x);10 # Abs(-Sqrt(_x)) <-- Sqrt(x);10 # Abs(Complex(_r,_i)) <-- Sqrt(r^2 + i^2);10 # Abs(n_IsInfinity) <-- Infinity;10 # Abs(Undefined) <-- Undefined;20 # Abs(n_IsList) <-- MapSingle(\"Abs\",n);100 # Abs(_a^_n) <-- Abs(a)^n;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Floor(Infinity) <-- Infinity;5 # Floor(-Infinity) <-- -Infinity;5 # Floor(Undefined) <-- Undefined;10 # Floor(x_IsRationalOrNumber) <-- [ x:=N(Eval(x)); Local(prec,result,n); Bind(prec,BuiltinPrecisionGet()); If(IsZero(x), Bind(n,2), If(x>?0, Bind(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Bind(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); If(n>?prec,BuiltinPrecisionSet(n)); Bind(result,FloorN(x)); BuiltinPrecisionSet(prec); result; ];";
        scriptMap.put("Floor",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Gcd(0,0) <-- 1;1 # Gcd(0,_m) <-- Abs(m);1 # Gcd(_n,0) <-- Abs(n);1 # Gcd(_m,_m) <-- Abs(m);2 # Gcd(_n,1) <-- 1;2 # Gcd(1,_m) <-- 1;2 # Gcd(n_IsInteger,m_IsInteger) <-- GcdN(n,m);3 # Gcd(_n,_m)_(IsGaussianInteger(m) And? IsGaussianInteger(n) )<-- GaussianGcd(n,m);4 # Gcd(-(_n), (_m)) <-- Gcd(n,m);4 # Gcd( (_n),-(_m)) <-- Gcd(n,m);4 # Gcd(Sqrt(n_IsInteger),Sqrt(m_IsInteger)) <-- Sqrt(Gcd(n,m));4 # Gcd(Sqrt(n_IsInteger),m_IsInteger) <-- Sqrt(Gcd(n,m^2));4 # Gcd(n_IsInteger,Sqrt(m_IsInteger)) <-- Sqrt(Gcd(n^2,m));5 # Gcd(n_IsRational,m_IsRational) <--[ Gcd(Numerator(n),Numerator(m))/Lcm(Denominator(n),Denominator(m));];10 # Gcd(list_IsList)_(Length(list)>?2) <-- [ Local(first); first:=Gcd(list[1],list[2]); Gcd(first:Rest(Rest(list))); ];14 # Gcd({0}) <-- 1;15 # Gcd({_head}) <-- head;20 # Gcd(list_IsList)_(Length(list)=?2) <-- Gcd(list[1],list[2]);30 # Gcd(n_CanBeUni,m_CanBeUni)_(Length(VarList(n*m))=?1) <--[ Local(vars); vars:=VarList(n+m); NormalForm(Gcd(MakeUni(n,vars),MakeUni(m,vars)));];100 # Gcd(n_Constant?,m_Constant?) <-- 1;110 # Gcd(_m,_n) <--[ Echo(\"Not simplified\");];0 # Gcd(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) <? Degree(m)) <-- Gcd(m,n);1 # Gcd(nn_IsUniVar,mm_IsUniVar)_ (nn[1] =? mm[1] And? Degree(nn) >=? Degree(mm)) <--[ UniVariate(nn[1],0, UniGcd(Concat(ZeroVector(nn[2]),nn[3]), Concat(ZeroVector(mm[2]),mm[3])));];";
        scriptMap.put("Gcd",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Lcm(a_IsInteger,b_IsInteger) <-- Quotient(a*b,Gcd(a,b));10 # Lcm(list_IsList)_(Length(list)>?2) <--[ Local(first); first:=Lcm(list[1],list[2]); Lcm(first:Rest(Rest(list)));];10 # Lcm(list_IsList)_(Length(list)=?2) <-- Lcm(list[1],list[2]);10 # Lcm(list_IsList)_(Length(list)=?1) <-- Lcm(list[1],1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # LnExpand(Ln(x_IsInteger)) <-- Add(Map({{n,m},m*Ln(n)},Transpose(Factors(x))));1 # LnExpand(Ln(_a*_b)) <-- LnExpand(Ln(a))+LnExpand(Ln(b));1 # LnExpand(Ln(_a/_b)) <-- LnExpand(Ln(a))-LnExpand(Ln(b));1 # LnExpand(Ln(_a^_n)) <-- LnExpand(Ln(a))*n;2 # LnExpand(_a) <-- a;";
        scriptMap.put("LnExpand",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Modulo(_n,m_IsRationalOrNumber)_(m<?0) <-- `Hold(Modulo(@n,@m));1 # Modulo(n_IsNegativeInteger,m_IsPositiveInteger) <--[ Local(result); result := ModuloN(n,m); If (result <? 0,result := result + m); result;];1 # Modulo(n_IsPositiveInteger,m_IsPositiveInteger) <-- ModuloN(n,m);2 # Modulo(0,_m) <-- 0;2 # Modulo(n_IsPositiveInteger,Infinity) <-- n;3 # Modulo(n_IsInteger,m_IsInteger) <-- ModuloN(n,m);4 # Modulo(n_IsNumber,m_IsNumber) <-- NonN(Modulo(Rationalize(n),Rationalize(m)));5 # Modulo(n_IsRationalOrNumber,m_IsRationalOrNumber) <--[ Local(n1,n2,m1,m2); n1:=Numerator(n); n2:=Denominator(n); m1:=Numerator(m); m2:=Denominator(m); Modulo(n1*m2,m1*n2)/(n2*m2);];6 # Modulo(n_IsList,m_IsList) <-- Map(\"Modulo\",{n,m});7 # Modulo(n_IsList,_m) <-- Map(\"Modulo\",{n,FillList(m,Length(n))});30 # Modulo(n_CanBeUni,m_CanBeUni) <--[ Local(vars); vars:=VarList(n+m); NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];0 # Modulo(n_IsUniVar,m_IsUniVar)_(Degree(n) <? Degree(m)) <-- n;1 # Modulo(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) >=? Degree(m)) <--[ UniVariate(n[1],0, UniDivide(Concat(ZeroVector(n[2]),n[3]), Concat(ZeroVector(m[2]),m[3]))[2]);];10 # Modulo(n_CanBeUni, m_CanBeUni, vars_IsList)_(Length(vars)=?1) <--[ NormalForm(Modulo(MakeUni(n,vars),MakeUni(m,vars)));];";
        scriptMap.put("Modulo",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Object\",{pred,x});Rule(\"Object\",2,0,Apply(pred,{x})=?True) x;";
        scriptMap.put("Object",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Quotient(n_IsInteger,m_IsInteger) <-- QuotientN(n,m);1 # Quotient(0 ,_m) <-- 0;2 # Quotient(n_IsRationalOrNumber,m_IsRationalOrNumber) <--[ Local(n1,n2,m1,m2,sgn1,sgn2); n1:=Numerator(n); n2:=Denominator(n); m1:=Numerator(m); m2:=Denominator(m); sgn1 := Sign(n1*m2); sgn2 := Sign(m1*n2); sgn1*sgn2*Floor(DivideN(sgn1*n1*m2,sgn2*m1*n2));];30 # Quotient(n_CanBeUni,m_CanBeUni)_(Length(VarList(n*m))=?1) <--[ Local(vars,nl,ml); vars:=VarList(n*m); nl := MakeUni(n,vars); ml := MakeUni(m,vars); NormalForm(Quotient(nl,ml));];0 # Quotient(n_IsUniVar,m_IsUniVar)_(Degree(n) <? Degree(m)) <-- 0;1 # Quotient(n_IsUniVar,m_IsUniVar)_ (n[1] =? m[1] And? Degree(n) >=? Degree(m)) <--[ UniVariate(n[1],0, UniDivide(Concat(ZeroVector(n[2]),n[3]), Concat(ZeroVector(m[2]),m[3]))[1]);];";
        scriptMap.put("Quotient",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Rem(n_IsNumber,m_IsNumber) <-- n-m*Quotient(n,m);30 # Rem(n_CanBeUni,m_CanBeUni) <-- Modulo(n,m);";
        scriptMap.put("Rem",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Round(Infinity) <-- Infinity;5 # Round(-Infinity) <-- -Infinity;5 # Round(Undefined) <-- Undefined;10 # Round(x_IsRationalOrNumber) <-- FloorN(N(x+0.5));10 # Round(x_IsList) <-- MapSingle(\"Round\",x);20 # Round(x_IsComplex) _ (IsRationalOrNumber(Re(x)) And? IsRationalOrNumber(Im(x)) ) <-- FloorN(N(Re(x)+0.5)) + FloorN(N(Im(x)+0.5))*I;";
        scriptMap.put("Round",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Sign(n_IsPositiveNumber) <-- 1;10 # Sign(n_IsZero) <-- 0;20 # Sign(n_IsNumber) <-- -1;15 # Sign(n_IsInfinity)_(n <? 0) <-- -1;15 # Sign(n_IsInfinity)_(n >? 0) <-- 1;15 # Sign(n_IsNumber/m_IsNumber) <-- Sign(n)*Sign(m);20 # Sign(n_IsList) <-- MapSingle(\"Sign\",n);100 # Sign(_a)^n_Even? <-- 1;100 # Sign(_a)^n_IsOdd <-- Sign(a);";
        scriptMap.put("Sign",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Sqrt(0) <-- 0;0 # Sqrt(Infinity) <-- Infinity;0 # Sqrt(-Infinity) <-- Complex(0,Infinity);0 # Sqrt(Undefined) <-- Undefined;1 # Sqrt(x_IsPositiveInteger)_(IsInteger(SqrtN(x))) <-- SqrtN(x);2 # Sqrt(x_IsPositiveNumber)_InNumericMode() <-- SqrtN(x);2 # Sqrt(x_IsNegativeNumber) <-- Complex(0,Sqrt(-x));3 # Sqrt(x_IsComplex)_InNumericMode() <-- x^(1/2);Sqrt(xlist_IsList) <-- MapSingle(\"Sqrt\",xlist);90 # (Sqrt(x_Constant?))_(IsNegativeNumber(N(x))) <-- Complex(0,Sqrt(-x));400 # x_IsRationalOrNumber * Sqrt(y_IsRationalOrNumber) <-- Sign(x)*Sqrt(x^2*y);400 # Sqrt(y_IsRationalOrNumber) * x_IsRationalOrNumber <-- Sign(x)*Sqrt(x^2*y);400 # x_IsRationalOrNumber / Sqrt(y_IsRationalOrNumber) <-- Sign(x)*Sqrt(x^2/y);400 # Sqrt(y_IsRationalOrNumber) / x_IsRationalOrNumber <-- Sign(x)*Sqrt(y/(x^2));400 # Sqrt(y_IsRationalOrNumber) / Sqrt(x_IsRationalOrNumber) <-- Sqrt(y/x);400 # Sqrt(y_IsRationalOrNumber) * Sqrt(x_IsRationalOrNumber) <-- Sqrt(y*x);400 # Sqrt(x_IsInteger)_IsInteger(SqrtN(x)) <-- SqrtN(x);400 # Sqrt(x_IsInteger/y_IsInteger)_(IsInteger(SqrtN(x)) And? IsInteger(SqrtN(y))) <-- SqrtN(x)/SqrtN(y);";
        scriptMap.put("Sqrt",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # Undefined <? _x <-- False;1 # Undefined <=? _x <-- False;1 # Undefined >? _x <-- False;1 # Undefined >=? _x <-- False;1 # _x <? Undefined <-- False;1 # _x <=? Undefined <-- False;1 # _x >? Undefined <-- False;1 # _x >=? Undefined <-- False;5 # (n_IsNumber <? m_IsNumber) <-- IsLessThan(n-m,0);LocalSymbols(nNum,mNum)[ 10 # (_n <? _m)_[nNum:=N(Eval(n)); mNum:=N(Eval(m));IsNumber(nNum) And? IsNumber(mNum);] <-- IsLessThan(nNum-mNum,0);];20 # (Infinity <? _n)_(Not?(IsInfinity(n))) <-- False;20 # (-Infinity <? _n)_(Not?(IsInfinity(n))) <-- True;20 # (_n <? Infinity)_(Not?(IsInfinity(n))) <-- True;20 # (_n <? -Infinity)_(Not?(IsInfinity(n))) <-- False;30 # (_n1/_n2) <? 0 <-- (n1 <? 0) !=? (n2 <? 0);30 # (_n1*_n2) <? 0 <-- (n1 <? 0) !=? (n2 <? 0);30 # ((_n1+_n2) <? 0)_((n1 <? 0) And? (n2 <? 0)) <-- True;30 # ((_n1+_n2) <? 0)_((n1 >? 0) And? (n2 >? 0)) <-- False;30 # _x^a_IsOdd <? 0 <-- x <? 0;30 # _x^a_Even? <? 0 <-- False; 40 # (Sqrt(_x))_(x >? 0) <? 0 <-- False;40 # (Sin(_x) <? 0)_(Not?(Even?(N(x/Pi))) And? Even?(N(Floor(x/Pi)))) <-- False;40 # (Sin(_x) <? 0)_(Not?(IsOdd (N(x/Pi))) And? IsOdd (N(Floor(x/Pi)))) <-- True;40 # Cos(_x) <? 0 <-- Sin(Pi/2-x) <? 0;40 # (Tan(_x) <? 0)_(Not?(Even?(N(2*x/Pi))) And? Even?(N(Floor(2*x/Pi)))) <-- False;40 # (Tan(_x) <? 0)_(Not?(IsOdd (N(2*x/Pi))) And? IsOdd (N(Floor(2*x/Pi)))) <-- True;40 # (Complex(_a,_b) <? 0)_(b!=?0) <-- False;40 # (Complex(_a,_b) >=? 0)_(b!=?0) <-- False;40 # (Sqrt(_x))_(x <? 0) <? 0 <-- False;40 # (Sqrt(_x))_(x <? 0) >=? 0 <-- False;50 # -(_x) <? 0 <-- Not?((x<?0) Or? (x=?0));50 # _n >? _m <-- m <? n;50 # _n <=? _m <-- m >=? n;50 # _n >=? _m <-- Not?(n<?m);Function(\"!=?\",{aLeft,aRight}) Not?(aLeft=?aRight);";
        scriptMap.put("<?",scriptString);
        scriptMap.put(">?",scriptString);
        scriptMap.put("<=?",scriptString);
        scriptMap.put(">=?",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"->\",{left,right});HoldArgument(\"->\",left);";
        scriptMap.put("->",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "n_IsInteger << m_IsInteger <-- ShiftLeft(n,m);n_IsInteger >> m_IsInteger <-- ShiftRight(n,m);";
        scriptMap.put("<<",scriptString);
        scriptMap.put(">>",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"MacroSubstitute\",{body,predicate,change})[ `MacroSubstitute((Hold(@body)));];HoldArgument(\"MacroSubstitute\",predicate);HoldArgument(\"MacroSubstitute\",change);UnFence(\"MacroSubstitute\",3);Rulebase(\"MacroSubstitute\",{body});UnFence(\"MacroSubstitute\",1);Rule(\"MacroSubstitute\",1,1,`ApplyFast(predicate,{Hold(Hold(@body))}) =? True)[ `ApplyFast(change,{Hold(Hold(@body))});];Rule(\"MacroSubstitute\",1,2,`IsFunction(Hold(@body)))[ `ApplyFast(\"MacroMapArgs\",{Hold(Hold(@body)),\"MacroSubstitute\"});];Rule(\"MacroSubstitute\",1,3,True)[ `Hold(@body);];";
        scriptMap.put("MacroSubstitute",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Substitute\",{body,predicate,change})[ Substitute(body);];HoldArgument(\"Substitute\",predicate);HoldArgument(\"Substitute\",change);UnFence(\"Substitute\",3);Rulebase(\"Substitute\",{body});UnFence(\"Substitute\",1);Rule(\"Substitute\",1,1,Apply(predicate,{body}) =? True)[ Apply(change,{body});];Rule(\"Substitute\",1,2,IsFunction(body))[ Apply(\"MapArgs\",{body,\"Substitute\"});];Rule(\"Substitute\",1,3,True) body;";
        scriptMap.put("Substitute",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Add(val, ...);10 # Add({}) <-- 0;20 # Add(values_IsList) <--[ Local(i, sum); sum:=0; ForEach(i, values) [ sum := sum + i; ]; sum;];30 # Add(_value) <-- value;";
        scriptMap.put("Add",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Maximum(l1, l2, l3, ...);10 # Maximum(_l1, _l2, l3_IsList) <-- Maximum(Concat({l1, l2}, l3));20 # Maximum(_l1, _l2, _l3) <-- Maximum({l1, l2, l3});10 # Maximum(l1_IsList,l2_IsList) <-- Map(\"Maximum\",{l1,l2});20 # Maximum(l1_IsRationalOrNumber,l2_IsRationalOrNumber) <-- If(l1>?l2,l1,l2);30 # Maximum(l1_Constant?,l2_Constant?) <-- If(N(Eval(l1-l2))>?0,l1,l2);10 # Maximum({}) <-- Undefined;20 # Maximum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Maximum(result,item); result;];30 # Maximum(_x) <-- x;";
        scriptMap.put("Maximum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Minimum(l1, l2, l3, ...);10 # Minimum(_l1, _l2, l3_IsList) <-- Minimum(Concat({l1, l2}, l3));20 # Minimum(_l1, _l2, _l3) <-- Minimum({l1, l2, l3});10 # Minimum(l1_IsList,l2_IsList) <-- Map(\"Minimum\",{l1,l2});20 # Minimum(l1_IsRationalOrNumber,l2_IsRationalOrNumber) <-- If(l1<?l2,l1,l2);30 # Minimum(l1_Constant?,l2_Constant?) <-- If(N(Eval(l1-l2))<?0,l1,l2);10 # Minimum({}) <-- Undefined;20 # Minimum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Minimum(result,item); result;];30 # Minimum(_x) <-- x;";
        scriptMap.put("Minimum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Product\",{sumvar,sumfrom,sumto,sumbody})[ Local(sumi,sumsum); sumsum:=1; For(sumi:=sumfrom,sumi<=?sumto And? sumsum!=?0,sumi++) [ MacroLocal(sumvar); MacroBind(sumvar,sumi); sumsum:=sumsum*Eval(sumbody); ]; sumsum;];UnFence(\"Product\",4);HoldArgument(\"Product\",sumvar);HoldArgument(\"Product\",sumbody);Product(sumlist_IsList) <--[ Local(sumi,sumsum); sumsum:=1; ForEach(sumi,sumlist) [ sumsum:=sumsum*sumi; ]; sumsum;];";
        scriptMap.put("Product",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Subfactorial\",{n})[ n! * Sum(k,0,n,(-1)^(k)/k!);];30 # Subfactorial(n_IsList) <-- MapSingle(\"Subfactorial\",n);";
        scriptMap.put("Subfactorial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Sum\",{sumvar'arg,sumfrom'arg,sumto'arg,sumbody'arg});10 # Sum(_sumvar,sumfrom_IsNumber,sumto_IsNumber,_sumbody)_(sumfrom>?sumto) <-- 0;20 # Sum(_sumvar,sumfrom_IsNumber,sumto_IsNumber,_sumbody)_(sumto<?sumfrom) <-- ApplyFast(\"Sum\",{sumvar,sumto,sumfrom,sumbody});30 # Sum(_sumvar,sumfrom_IsNumber,sumto_IsNumber,_sumbody) <--LocalSymbols(sumi,sumsum)[ Local(sumi,sumsum); sumsum:=0; For(sumi:=sumfrom,sumi<=?sumto,sumi++) [ MacroLocal(sumvar); MacroBind(sumvar,sumi); sumsum:=sumsum+Eval(sumbody); ]; sumsum;];UnFence(\"Sum\",4);HoldArgument(\"Sum\",sumvar'arg);HoldArgument(\"Sum\",sumbody'arg);40 # Sum({}) <-- 0;50 # Sum(values_IsList) <--[ Local(i, sum); sum:=0; ForEach(i, values) [ sum := sum + i; ]; sum;];";
        scriptMap.put("Sum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(c,d,expr,from,to,summand,sum,predicate,n,r,var,x) [Function() SumFunc(k,from,to,summand, sum, predicate );Function() SumFunc(k,from,to,summand, sum);HoldArgument(SumFunc,predicate);HoldArgument(SumFunc,sum);HoldArgument(SumFunc,summand);SumFunc(_sumvar,sumfrom_IsInteger,_sumto,_sumbody,_sum) <--[    `(40 # Sum(@sumvar,@sumfrom,@sumto,@sumbody ) <-- Eval(@sum) ); `(41 # Sum(@sumvar,p_IsInteger,@sumto,@sumbody)_(p >? @sumfrom) <-- [ Local(sub); (sub := Eval(ListToFunction({Sum,sumvar'arg,@sumfrom,p-1,sumbody'arg}))); Simplify(Eval(@sum) - sub ); ]);];SumFunc(_sumvar,sumfrom_IsInteger,_sumto,_sumbody,_sum,_condition) <--[ `(40 # Sum(@sumvar,@sumfrom,@sumto,@sumbody)_(@condition) <-- Eval(@sum) ); `(41 # Sum(@sumvar,p_IsInteger,@sumto,@sumbody )_(@condition And? p >? @sumfrom) <-- [ Local(sub); `(sub := Eval(ListToFunction({Sum,sumvar'arg,@sumfrom,p-1,sumbody'arg}))); Simplify(Eval(@sum) - sub ); ]);];SumFunc(_k,1,_n,_c + _d, Eval(ListToFunction({Sum,sumvar'arg,1,n,c})) + Eval(ListToFunction({Sum,sumvar'arg,1,n,d})));SumFunc(_k,1,_n,_c*_expr,Eval(c*ListToFunction({Sum,sumvar'arg,1,n,expr})), IsFreeOf(k,c) );SumFunc(_k,1,_n,_expr/_c,Eval(ListToFunction({Sum,sumvar'arg,1,n,expr})/c), IsFreeOf(k,c) );SumFunc(_k,1,Infinity,1/_k,Infinity);SumFunc(_k,1,_n,_c,c*n,IsFreeOf(k,c) );SumFunc(_k,1,_n,_k, n*(n+1)/2 );SumFunc(_k,1,_n,_k^_p,(Bernoulli(p+1,n+1) - Bernoulli(p+1))/(p+1), IsInteger(p) );SumFunc(_k,1,_n,2*_k-1, n^2 );SumFunc(_k,1,_n,HarmonicNumber(_k),(n+1)*HarmonicNumber(n) - n );SumFunc(_k,0,_n,(r_IsFreeOf(k))^(_k), (1-r^(n+1))/(1-r) );SumFunc(_k,1,Infinity,1/(_k^_d), Zeta(d), IsFreeOf(k,d) );SumFunc(_k,1,Infinity,_k^(-_d), Zeta(d), IsFreeOf(k,d) );SumFunc(_k,0,Infinity,_x^(2*_k+1)/(2*_k+1)!,Sinh(x) );SumFunc(_k,0,Infinity,(-1)^k*_x^(2*_k+1)/(2*_k+1)!,Sin(x) );SumFunc(_k,0,Infinity,_x^(2*_k)/(2*_k)!,Cosh(x) );SumFunc(_k,0,Infinity,(-1)^k*_x^(2*_k)/(2*_k)!,Cos(x) );SumFunc(_k,0,Infinity,_x^(2*_k+1)/(2*_k+1),ArcTanh(x) );SumFunc(_k,0,Infinity,1/(_k)!,Exp(1) );SumFunc(_k,0,Infinity,_x^_k/(_k)!,Exp(x) );40 # Sum(_var,_from,Infinity,_expr)_( `(Limit(@var,Infinity)(@expr)) =? Infinity) <-- Infinity;SumFunc(_k,1,Infinity,1/BinomialCoefficient(2*_k,_k), (2*Pi*Sqrt(3)+9)/27 );SumFunc(_k,1,Infinity,1/(_k*BinomialCoefficient(2*_k,_k)), (Pi*Sqrt(3))/9 );SumFunc(_k,1,Infinity,1/(_k^2*BinomialCoefficient(2*_k,_k)), Zeta(2)/3 );SumFunc(_k,1,Infinity,1/(_k^3*BinomialCoefficient(2*_k,_k)), 17*Zeta(4)/36 );SumFunc(_k,1,Infinity,(-1)^(_k-1)/_k, Ln(2) );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Taylor'LPS\",{order, coeffs, var, expr});Rulebase(\"Taylor'LPS'Inverse\",{lps});Rulebase(\"Taylor'LPS'Add\",{lps1,lps2});Rulebase(\"Taylor'LPS'Multiply\",{lps1,lps2});Rulebase(\"Taylor'LPS'ScalarMult\",{a,lps});Rulebase(\"Taylor'LPS'Compose\",{lps1,lps2});10 # (Taylor2(_x, _a, _n) _y) _ (Not?(IsPositiveInteger(n) Or? IsZero(n))) <-- Check(False, \"Argument\", \"Third argument to Taylor should be a nonnegative integer\");20 # (Taylor2(_x, 0, _n) _y) <--[ Local(res); res := Taylor'LPS'PowerSeries(Taylor'LPS'Construct(x, y), n, x); If (ClearError(\"singularity\"), Echo(y, \"has a singularity at\", x, \"= 0.\")); If (ClearError(\"dunno\"), Echo(\"Cannot determine power series of\", y)); res;];30 # (Taylor2(_x, _a, _n) _y) <-- Subst(x,x-a) Taylor2(x,0,n) Subst(x,x+a) y;Taylor'LPS'Param1() := 15;10 # Taylor'LPS'Construct(_var, _expr) <-- Taylor'LPS(Undefined, {}, var, Taylor'LPS'NormalizeExpr(var, expr));Taylor'LPS'Coeffs(_lps, _n1, _n2) <--[ Local(res, finished, order, j, k, n, tmp, c1, c2); finished := False;  If (lps[1] =? Infinity, [ res := FillList(0, n2-n1+1); finished := True; ]);  If (Not? finished And? lps[1] !=? Undefined And? n2 <? lps[1]+Length(lps[2]), [ If (n1 >=? lps[1], res := Take(lps[2], {n1-lps[1]+1, n2-lps[1]+1}), If (n2 >=? lps[1], res := Concat(FillList(0, lps[1]-n1), Take(lps[2], n2-lps[1]+1)), res := FillList(0, n2-n1+1))); finished := True; ]);  If (Not? finished, [  order := Taylor'LPS'CompOrder(lps[3], lps[4]); If (Not? ClearError(\"dunno\"), [ If (lps[1] =? Undefined, [ lps[1] := order; If (order <=? n2, [ lps[2] := Table(Taylor'LPS'CompCoeff(lps[3], lps[4], n), n, order, n2, 1); ]); ],[ tmp := Table(Taylor'LPS'CompCoeff(lps[3], lps[4], n), n, lps[1]+Length(lps[2]), n2, 1); lps[2] := Concat(lps[2], tmp); ]); finished := True; ]);  If (Not? finished And? lps[4][0] =? Taylor'LPS'Add, [ lps[1] := Minimum(Taylor'LPS'GetOrder(lps[4][1])[1], Taylor'LPS'GetOrder(lps[4][2])[1], n2); If (IsError(\"dunno\"), [ ClearError(\"dunno\"); ClearError(\"dunno\"); ],[ If (lps[1] <=? n2, [ c1 := Taylor'LPS'Coeffs(lps[4][1], lps[1] + Length(lps[2]), n2); c2 := Taylor'LPS'Coeffs(lps[4][2], lps[1] + Length(lps[2]), n2); lps[2] := Concat(lps[2], c1 + c2); ]); finished := True; ]); ]);  If (Not? finished And? lps[4][0] =? Taylor'LPS'ScalarMult, [ lps[1] := Minimum(Taylor'LPS'GetOrder(lps[4][2])[1], n2); If (Not? ClearError(\"dunno\"), [ If (lps[1] <=? n2, [ tmp := Taylor'LPS'Coeffs(lps[4][2], lps[1] + Length(lps[2]), n2); tmp := lps[4][1] * tmp; lps[2] := Concat(lps[2], tmp); ]); finished := True; ]); ]);  If (Not? finished And? lps[4][0] =? Taylor'LPS'Multiply, [ lps[1] := Taylor'LPS'GetOrder(lps[4][1])[1] + Taylor'LPS'GetOrder(lps[4][2])[1]; If (IsError(\"dunno\"), [ ClearError(\"dunno\"); ClearError(\"dunno\"); ],[ If (lps[1] <=? n2, [ c1 := Taylor'LPS'Coeffs(lps[4][1], lps[4][1][1], n2 - lps[4][2][1]); c2 := Taylor'LPS'Coeffs(lps[4][2], lps[4][2][1], n2 - lps[4][1][1]); tmp := lps[2]; ForEach(k, (Length(lps[2])+1) .. Length(c1)) tmp := Append(tmp, Sum(j, 1, k, c1[j]*c2[k+1-j])); lps[2] := tmp; ]); finished := True; ]); ]);  If (Not? finished And? lps[4][0] =? Taylor'LPS'Inverse, [ If (lps[4][1][1] =? Infinity, [ Assert(\"div-by-zero\") False; finished := True; ]); If (Not? finished And? lps[2] =? {}, [ order := Taylor'LPS'GetOrder(lps[4][1])[1]; n := order; c1 := Taylor'LPS'Coeffs(lps[4][1], n, n)[1]; While (c1 =? 0 And? n <? order + Taylor'LPS'Param1()) [ n := n + 1; c1 := Taylor'LPS'Coeffs(lps[4][1], n, n)[1]; ]; If (c1 =? 0, [ Assert(\"maybe-div-by-zero\") False; finished := True; ]); ]); If (Not? finished, [ lps[1] := -lps[4][1][1]; c1 := Taylor'LPS'Coeffs(lps[4][1], lps[4][1][1], lps[4][1][1]+n2-lps[1]); tmp := lps[2]; If (tmp =? {}, tmp := {1/c1[1]}); If (Length(c1)>?1, [ ForEach(k, (Length(tmp)+1) .. Length(c1)) [ n := -Sum(j, 1, k-1, c1[k+1-j]*tmp[j]) / c1[1]; tmp := Append(tmp, n); ]; ]); lps[2] := tmp; finished := True; ]); ]);  If (Not? finished And? lps[4][0] =? Taylor'LPS'Compose, [ j := Taylor'LPS'GetOrder(lps[4][1])[1]; Check(j >=? 0, \"Math\", \"Expansion of f(g(x)) where f has a\" : \"singularity is not implemented\"); k := Taylor'LPS'GetOrder(lps[4][2])[1]; c1 := {j, Taylor'LPS'Coeffs(lps[4][1], j, n2)}; c2 := {k, Taylor'LPS'Coeffs(lps[4][2], k, n2)}; c1 := Taylor'TPS'Compose(c1, c2); lps[1] := c1[1]; lps[2] := c1[2]; finished := True; ]);  If (finished, [  While (lps[2] !=? {} And? lps[2][1] =? 0) [ lps[1] := lps[1] + 1; lps[2] := Rest(lps[2]); ];  If (Not? IsError(\"dunno\") And? Not? IsError(\"div-by-zero\") And? Not? IsError(\"maybe-div-by-zero\"), [ If (lps[1] <=? n1, res := Take(lps[2], {n1-lps[1]+1, n2-lps[1]+1}), If (lps[1] <=? n2, res := Concat(FillList(0, lps[1]-n1), lps[2]), res := FillList(0, n2-n1+1))); ]); ],[ Assert(\"dunno\") False; res := False; ]); ]);  res;];10 # Taylor'TPS'GetCoeff({_n,_c}, _k) _ (k <? n) <-- 0;10 # Taylor'TPS'GetCoeff({_n,_c}, _k) _ (k >=? n+Length(c)) <-- Undefined;20 # Taylor'TPS'GetCoeff({_n,_c}, _k) <-- c[k-n+1];10 # Taylor'TPS'Add({_n1,_c1}, {_n2,_c2}) <--[ Local(n, len, c1b, c2b); n := Minimum(n1,n2); len := Minimum(n1+Length(c1), n2+Length(c2)) - n; c1b := Take(Concat(FillList(0, n1-n), c1), len); c2b := Take(Concat(FillList(0, n2-n), c2), len); {n, c1b+c2b};];10 # Taylor'TPS'ScalarMult(_a, {_n2,_c2}) <-- {n2, a*c2};10 # Taylor'TPS'Multiply({_n1,_c1}, {_n2,_c2}) <--[ Local(j,k,c); c := {}; For (k:=1, k<=?Minimum(Length(c1), Length(c2)), k++) [ c := c : Sum(j, 1, k, c1[j]*c2[k+1-j]); ]; {n1+n2, c};];10 # Taylor'TPS'Compose({_n1,_c1}, {_n2,_c2}) <--[ Local(res, tps, tps2, k, n); n := Minimum(n1+Length(c1)-1, n2+Length(c2)-1); tps := {0, 1 : FillList(0, n)};  res := Taylor'TPS'ScalarMult(Taylor'TPS'GetCoeff({n1,c1}, 0), tps); For (k:=1, k<=?n, k++) [ tps := Taylor'TPS'Multiply(tps, {n2,c2}); tps2 := Taylor'TPS'ScalarMult(Taylor'TPS'GetCoeff({n1,c1}, k), tps); res := Taylor'TPS'Add(res, tps2); ]; res;];5 # Taylor'LPS'NormalizeExpr(_var, _e1) _ [Taylor'LPS'CompOrder(var,e1); Not? ClearError(\"dunno\");] <-- e1;10 # Taylor'LPS'NormalizeExpr(_var, _e1 + _e2) <-- Taylor'LPS'Add(Taylor'LPS'Construct(var, e1), Taylor'LPS'Construct(var, e2));10 # Taylor'LPS'NormalizeExpr(_var, - _e1) <-- Taylor'LPS'ScalarMult(-1, Taylor'LPS'Construct(var, e1));10 # Taylor'LPS'NormalizeExpr(_var, _e1 - _e2) <-- (Taylor'LPS'Add(Taylor'LPS'Construct(var, e1), Taylor'LPS'Construct(var, e3)) Where e3 == Taylor'LPS'ScalarMult(-1, Taylor'LPS'Construct(var, e2)));10 # Taylor'LPS'NormalizeExpr(_var, e1_IsFreeOf(var) * _e2) <-- Taylor'LPS'ScalarMult(e1, Taylor'LPS'Construct(var, e2));10 # Taylor'LPS'NormalizeExpr(_var, _e1 * e2_IsFreeOf(var)) <-- Taylor'LPS'ScalarMult(e2, Taylor'LPS'Construct(var, e1));20 # Taylor'LPS'NormalizeExpr(_var, _e1 * _e2) <-- Taylor'LPS'Multiply(Taylor'LPS'Construct(var, e1), Taylor'LPS'Construct(var, e2));10 # Taylor'LPS'NormalizeExpr(_var, _e1 / e2_IsFreeOf(var)) <-- Taylor'LPS'ScalarMult(1/e2, Taylor'LPS'Construct(var, e1));20 # Taylor'LPS'NormalizeExpr(_var, 1 / _e1) <-- Taylor'LPS'Inverse(Taylor'LPS'Construct(var, e1));30 # Taylor'LPS'NormalizeExpr(_var, _e1 / _e2) <-- (Taylor'LPS'Multiply(Taylor'LPS'Construct(var, e1), Taylor'LPS'Construct(var, e3)) Where e3 == Taylor'LPS'Inverse(Taylor'LPS'Construct(var, e2)));10 # Taylor'LPS'NormalizeExpr(_var, _e1 ^ (n_IsPositiveInteger)) _ (e1 !=? var) <-- Taylor'LPS'Multiply(Taylor'LPS'Construct(var, e1), Taylor'LPS'Construct(var, e1^(n-1)));10 # Taylor'LPS'NormalizeExpr(_var, Tan(_x)) <-- (Taylor'LPS'Multiply(Taylor'LPS'Construct(var, Sin(x)), Taylor'LPS'Construct(var, e3)) Where e3 == Taylor'LPS'Inverse(Taylor'LPS'Construct(var, Cos(x))));LocalSymbols(res)[50 # Taylor'LPS'NormalizeExpr(_var, _e1)_[ Local(c, lps1, lps2, lps3, success); success := True; If (IsAtom(e1), success := False); If (success And? Length(e1) !=? 1, success := False); If (success And? IsAtom(e1[1]), success := False); If (success And? CanBeUni(var, e1[1]) And? Degree(e1[1], var) =? 1, [ success := False; ]); If (success, [ lps2 := Taylor'LPS'Construct(var, e1[1]); c := Taylor'LPS'Coeffs(lps2, 0, 0)[1]; If (IsError(), [ ClearErrors(); success := False; ]); If (success And? Taylor'LPS'GetOrder(lps2)[1] <? 0, [ success := False; ],[ If (c =? 0, [ lps1 := Taylor'LPS'Construct(var, Apply(e1[0], {var})); res := Taylor'LPS'Compose(lps1, lps2); ],[ lps1 := Taylor'LPS'Construct(var, Apply(e1[0], {var+c})); lps3 := Taylor'LPS'Construct(var, -c); lps2 := Taylor'LPS'Construct(var, Taylor'LPS'Add(lps2, lps3)); res := Taylor'LPS'Compose(lps1, lps2); ]); ]); ]); success; ] <-- res;];60000 # Taylor'LPS'NormalizeExpr(_var, _e1) <-- e1;5 # Taylor'LPS'CompCoeff(_var, _expr, _n) _ (n <? Taylor'LPS'CompOrder(var, expr)) <-- 0;10 # Taylor'LPS'CompOrder(_x, 0) <-- Infinity;20 # Taylor'LPS'CompOrder(_x, e_IsFreeOf(x)) <-- 0;20 # Taylor'LPS'CompCoeff(_x, e_IsFreeOf(x), 0) <-- e;21 # Taylor'LPS'CompCoeff(_x, e_IsFreeOf(x), _n) <-- 0;30 # Taylor'LPS'CompOrder(_x, _x) <-- 1;30 # Taylor'LPS'CompCoeff(_x, _x, 1) <-- 1;31 # Taylor'LPS'CompCoeff(_x, _x, _n) <-- 0;40 # Taylor'LPS'CompOrder(_x, _x^(k_IsPositiveInteger)) <-- k;40 # Taylor'LPS'CompCoeff(_x, _x^(k_IsPositiveInteger), _k) <-- 1;41 # Taylor'LPS'CompCoeff(_x, _x^(k_IsPositiveInteger), _n) <-- 0;50 # Taylor'LPS'CompOrder(_x, Sqrt(_y)) _ (CanBeUni(x,y) And? Degree(y,x) =? 1 And? Coef(y,x,0) !=? 0) <-- 0;50 # Taylor'LPS'CompCoeff(_x, Sqrt(_y), 0) _ (CanBeUni(x,y) And? Degree(y,x) =? 1 And? Coef(y,x,0) !=? 0) <-- Sqrt(Coef(y,x,0));51 # Taylor'LPS'CompCoeff(_x, Sqrt(_y), _n) _ (CanBeUni(x,y) And? Degree(y,x) =? 1 And? Coef(y,x,0) !=? 0) <--[ Local(j); Coef(y,x,0)^(1/2-n) * Product(j,0,n-1,1/2-j) * Coef(y,x,1)^n/n!;];60 # Taylor'LPS'CompOrder(_x, Exp(_x)) <-- 0;60 # Taylor'LPS'CompCoeff(_x, Exp(_x), _n) <-- 1/n!;70 # Taylor'LPS'CompOrder(_x, Exp(_y))_(CanBeUni(x,y) And? Degree(y,x) =? 1) <-- 0;70 # Taylor'LPS'CompCoeff(_x, Exp(_y), _n)_(CanBeUni(x,y) And? Degree(y,x) =? 1) <-- Exp(Coef(y,x,0)) * Coef(y,x,1)^n / n!;80 # Taylor'LPS'CompOrder(_x, Ln(_x+1)) <-- 1;80 # Taylor'LPS'CompCoeff(_x, Ln(_x+1), _n) <-- (-1)^(n+1)/n;90 # Taylor'LPS'CompOrder(_x, Sin(_x)) <-- 1;90 # Taylor'LPS'CompCoeff(_x, Sin(_x), n_IsOdd) <-- (-1)^((n-1)/2) / n!;90 # Taylor'LPS'CompCoeff(_x, Sin(_x), n_Even?) <-- 0;100 # Taylor'LPS'CompOrder(_x, Cos(_x)) <-- 0;100 # Taylor'LPS'CompCoeff(_x, Cos(_x), n_IsOdd) <-- 0;100 # Taylor'LPS'CompCoeff(_x, Cos(_x), n_Even?) <-- (-1)^(n/2) / n!;110 # Taylor'LPS'CompOrder(_x, 1/_x) <-- -1;110 # Taylor'LPS'CompCoeff(_x, 1/_x, -1) <-- 1;111 # Taylor'LPS'CompCoeff(_x, 1/_x, _n) <-- 0;Taylor'LPS'AcceptDeriv(_expr) <-- (Contains({\"ArcTan\"},Type(expr)));200 # Taylor'LPS'CompOrder(_x, (_expr))_(Taylor'LPS'AcceptDeriv(expr)) <-- [ Local(n); n:=0; While ((Limit(x,0)expr) =? 0 And? n<?Taylor'LPS'Param1()) [ expr := Deriv(x)expr; n++; ]; n; ];200 # Taylor'LPS'CompCoeff(_x, (_expr), _n)_ (Taylor'LPS'AcceptDeriv(expr) And? n>=?0 ) <-- [  Local(result); result:=(Limit(x,0)(Deriv(x,n)expr))/(n!);Echo(expr,\" \",n,\" \",result); result; ];60000 # Taylor'LPS'CompOrder(_var, _expr) <-- Assert(\"dunno\") False;60000 # Taylor'LPS'CompCoeff(_var, _expr, _n) <-- Check(False, \"Argument\", \"Taylor'LPS'CompCoeff'FallThrough\" : PipeToString() Write({var,expr,n}));20 # Taylor'LPS'GetOrder(Taylor'LPS(_order, _coeffs, _var, _expr)) _ (order !=? Undefined) <-- {order, coeffs !=? {}};40 # Taylor'LPS'GetOrder(_lps) <--[ Local(res, computed, exact, res1, res2); computed := False; res := Taylor'LPS'CompOrder(lps[3], lps[4]); If (Not? ClearError(\"dunno\"), [ res := {res, True}; computed := True; ]); If (Not? computed And? lps[4][0] =? Taylor'LPS'Add, [ res1 := Taylor'LPS'GetOrder(lps[4][1]); If (Not? ClearError(\"dunno\"), [ res2 := Taylor'LPS'GetOrder(lps[4][2]); If (Not? ClearError(\"dunno\"), [ res := {Minimum(res1[1],res2[1]), False};  computed := True; ]); ]); ]); If (Not? computed And? lps[4][0] =? Taylor'LPS'ScalarMult, [ res := Taylor'LPS'GetOrder(lps[4][2]); If (Not? ClearError(\"dunno\"), computed := True); ]); If (Not? computed And? lps[4][0] =? Taylor'LPS'Multiply, [ res1 := Taylor'LPS'GetOrder(lps[4][1]); If (Not? ClearError(\"dunno\"), [ res2 := Taylor'LPS'GetOrder(lps[4][2]); If (Not? ClearError(\"dunno\"), [ res := {res1[1]+res2[1], res1[1] And? res2[1]}; computed := True; ]); ]); ]); If (Not? computed And? lps[4][0] =? Taylor'LPS'Inverse, [ res := Taylor'LPS'GetOrder(lps[4][1]); If (Not? ClearError(\"dunno\"), [ If (res[1] =? Infinity, [ res[1] =? Undefined; Assert(\"div-by-zero\") False; computed := True; ]); If (Not? computed And? res[2] =? False, [ Local(c, n); n := res[1]; c := Taylor'LPS'Coeffs(lps[4][1], res[1], res[1])[1]; While (c =? 0 And? res[1] <? n + Taylor'LPS'Param1()) [ res[1] := res[1] + 1; c := Taylor'LPS'Coeffs(lps[4][1], res[1], res[1])[1]; ]; If (c =? 0, [ res[1] := Undefined; Assert(\"maybe-div-by-zero\") False; computed := True; ]); ]); If (Not? computed, [ res := {-res[1], True}; computed := True; ]); ]); ]); If (Not? computed And? lps[4][0] =? Taylor'LPS'Compose, [ res1 := Taylor'LPS'GetOrder(lps[4][1]); If (Not? ClearError(\"dunno\"), [ res2 := Taylor'LPS'GetOrder(lps[4][2]); If (Not? ClearError(\"dunno\"), [ res := {res1[1]*res2[1], res1[1] And? res2[1]}; computed := True; ]); ]); ]); If (computed, lps[1] := res[1]); Assert(\"dunno\") computed; res;];10 # Taylor'LPS'PowerSeries(_lps, _n, _var) <--[ Local(ord, k, coeffs); coeffs := Taylor'LPS'Coeffs(lps, 0, n); If (IsError(\"dunno\"), [ False; ],[ If (lps[1] <? 0, [ Assert(\"singularity\") False; Undefined; ],[ Sum(k, 0, n, coeffs[k+1]*var^k); ]); ]);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"Taylor3'MultiplyCoefs\",{coefs1,coefs2,degree})[ Local(result,i,j,jset,ilimit,jlimit); Bind(result, ArrayCreate(AddN(degree,1),0)); Bind(i,1); Bind(ilimit,AddN(degree,2)); While (Not? IsEqual(i,ilimit)) [ Bind(j,1); Bind(jlimit,AddN(degree,SubtractN(3,i))); While (Not? IsEqual(j,jlimit)) [ Bind(jset,AddN(i,SubtractN(j,1))); ArraySet(result,jset,ArrayGet(result,jset) + ArrayGet(coefs1,i)*ArrayGet(coefs2,j)); Bind(j,AddN(j,1)); ]; Bind(i,AddN(i,1)); ]; result;];10 # (Taylor3'TaylorCoefs(_var,_degree)(_var)) <--[ Local(result); Bind(result,ArrayCreate(degree+1,0)); ArraySet(result,2, 1); result;];20 # (Taylor3'TaylorCoefs(_var,_degree)(_atom))_(IsFreeOf(var,atom)) <-- [ Local(result); Bind(result,ArrayCreate(degree+1,0)); ArraySet(result,1, atom); result; ];30 # (Taylor3'TaylorCoefs(_var,_degree)(_X + _Y)) <-- [ Local(result,add,i); Bind(result,Taylor3'TaylorCoefs(var,degree)(X)); Bind(add, Taylor3'TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) [ ArraySet(result,i,ArrayGet(result,i)+ArrayGet(add,i)); ]; result; ];30 # (Taylor3'TaylorCoefs(_var,_degree)(_X - _Y)) <-- [ Local(result,add,i); Bind(result,Taylor3'TaylorCoefs(var,degree)(X)); Bind(add, Taylor3'TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) [ ArraySet(result,i,ArrayGet(result,i)-ArrayGet(add,i)); ]; result; ];30 # (Taylor3'TaylorCoefs(_var,_degree)( - _Y)) <-- [ Local(result,add,i); Bind(result,Taylor3'TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) [ ArraySet(result,i,-ArrayGet(result,i)); ]; result; ];30 # (Taylor3'TaylorCoefs(_var,_degree)(_X * _Y)) <-- Taylor3'MultiplyCoefs( Taylor3'TaylorCoefs(var,degree)(X), Taylor3'TaylorCoefs(var,degree)(Y), degree);30 # (Taylor3'TaylorCoefs(_var,_degree)((_X) ^ N_IsPositiveInteger)) <--[ Local(result,factor); factor:=Taylor3'TaylorCoefs(var,degree)(X); result:=ArrayCreate(degree+1,0); result[1] := 1;  While(N>?0) [ result:=Taylor3'MultiplyCoefs(result,factor,degree); N--; ]; result;];60 # Taylor3'UniFunction(\"Exp\") <-- True;60 # Taylor3'CompCoeff(\"Exp\", _n) <-- 1/n!;80 # Taylor3'UniFunction(\"Ln\") <-- False; 80 # Taylor3'CompCoeff(\"Ln\", 0) <-- 0;81 # Taylor3'CompCoeff(\"Ln\", _n) <-- (-1)^(n+1)/n;90 # Taylor3'UniFunction(\"Sin\") <-- True;90 # Taylor3'CompCoeff(\"Sin\", n_IsOdd) <-- (-1)^((n-1)/2) / n!;90 # Taylor3'CompCoeff(\"Sin\", n_Even?) <-- 0;100 # Taylor3'UniFunction(\"Cos\") <-- True;100 # Taylor3'CompCoeff(\"Cos\", n_IsOdd) <-- 0;100 # Taylor3'CompCoeff(\"Cos\", n_Even?) <-- (-1)^(n/2) / n!;210 # Taylor3'UniFunction(_any)_ ( [ Local(result); result:= Deriv(var)ListToFunction({ToAtom(any),var}); Type(result) !=? \"Deriv\"; ] ) <-- [ True; ];210 # Taylor3'CompCoeff(_any, n_IsInteger) <-- [ Limit(var,0)(Deriv(var,n)(ListToFunction({ToAtom(any),var}))/n!); ];60000 # Taylor3'UniFunction(_any) <-- False;Taylor3'FuncCoefs(_fname,_degree) <--[ Local(sins,i); Bind(sins, ArrayCreate(degree+1,0)); For (i:=0,i<=?degree,Bind(i,i+1)) [ ArraySet(sins,i+1, Taylor3'CompCoeff(fname,i)); ]; sins;];100 # (Taylor3'TaylorCoefs(_var,_degree)(Ln(_f)))_(Simplify(f-1) =? var) <-- Taylor3'FuncCoefs(\"Ln\",degree);110 # (Taylor3'TaylorCoefs(_var,_degree)(f_IsFunction))_(ArgumentsCount(f) =? 1 And? (Taylor3'UniFunction(Type(f)))) <--[ Local(sins,i,j,result,xx,expr,sinfact); expr := f[1]; sins:=Taylor3'FuncCoefs(Type(f),degree); expr:=Taylor3'TaylorCoefs(var,degree)expr; result:=ArrayCreate(degree+1,0); ArraySet(result,1, ArrayGet(sins,1)); xx:=expr; For (i:=2,i<=?degree+1,i++) [ Bind(sinfact,sins[i]); For (j:=1,j<=?degree+1,j++) [ ArraySet(result,j,ArrayGet(result,j) + (ArrayGet(xx,j) * sinfact)); ]; Bind(xx,Taylor3'MultiplyCoefs(xx,expr,degree)); ]; result;];(Taylor3(_var,_degree)(_expr)) <-- Add((Taylor3'TaylorCoefs(var,degree)(expr))[1 .. degree+1]*var^(0 .. degree));10 # (Taylor3(_x, 0, _n) _y) <-- Taylor3(x,n) y;20 # (Taylor3(_x, _a, _n) _y) <-- Subst(x,x-a) Taylor3(x,n) Subst(x,x+a) y;";
        scriptMap.put("Taylor3",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "n1_IsRationalOrNumber *** n2_IsRationalOrNumber <--[ Check(n2-n1 <=? 65535, \"Argument\", \"Partial factorial: Error: the range \" : ( PipeToString() Write(n2-n1) ) : \" is too large, you may want to avoid exact calculation\"); If(n2-n1<?0, 1, Factorial'partial(n1, n2) );];2# Factorial'partial(_a, _b) _ (b-a>=?4) <-- Factorial'partial(a, a+((b-a)>>1)) * Factorial'partial(a+((b-a)>>1)+1, b);3# Factorial'partial(_a, _b) _ (b-a>=?3) <-- a*(a+1)*(a+2)*(a+3);4# Factorial'partial(_a, _b) _ (b-a>=?2) <-- a*(a+1)*(a+2);5# Factorial'partial(_a, _b) _ (b-a>=?1) <-- a*(a+1);6# Factorial'partial(_a, _b) _ (b-a>=?0) <-- a;";
        scriptMap.put("***",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# (n_IsPositiveInteger)!! _ (n<=?3) <-- n;2# (n_IsPositiveInteger)!! <--[ Check(n<=?65535, \"Argument\", \"Double factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); Factorial'double(2+Modulo(n, 2), n);];3# (_n)!! _ (n=? -1 Or? n=?0)<-- 1;2# Factorial'double(_a, _b) _ (b-a>=?6) <-- Factorial'double(a, Quotient(a+b,2)) * Factorial'double(Quotient(a+b,2)+1+Modulo(Quotient(a+b,2)+1-a, 2), b);3# Factorial'double(_a, _b) _ (b-a>=?4) <-- a*(a+2)*(a+4);4# Factorial'double(_a, _b) _ (b-a>=?2) <-- a*(a+2);5# Factorial'double(_a, _b) <-- a;30 # (n_IsList)!! <-- MapSingle(\"!!\",n);";
        scriptMap.put("!!",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # 0! <-- 1;10 # (Infinity)! <-- Infinity;20 # ((n_IsPositiveInteger)!) <-- [ Check(n <=? 65535, \"Argument\", \"Factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); MathFac(n);];25 # ((x_Constant?)!)_(FloatIsInt?(x) And? x>?0) <-- (Round(x)!);30 # ((x_IsNumber)!)_InNumericMode() <-- Internal'GammaNum(x+1);40 # (n_IsList)! <-- MapSingle(\"!\",n);HalfIntegerFactorial(n_IsOdd) _ (n>?0) <-- Sqrt(Pi) * ( n!! / 2^((n+1)/2) );HalfIntegerFactorial(n_IsOdd) _ (n<?0) <-- Sqrt(Pi) * ( (-1)^((-n-1)/2)*2^((-n-1)/2) / (-n-2)!! );40 # (n_IsRationalOrNumber)! _(Denominator(Rationalize(n))=?2) <-- HalfIntegerFactorial(Numerator(Rationalize(n)));";
        scriptMap.put("!",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ApplyDelta(_result,Delta(_i,_j)) <-- DestructiveInsert(result,1,Delta(i,j));20 # ApplyDelta(_result,(_x) ^ (n_IsInteger))_(n>?0) <-- [ Local(i); For(i:=1,i<=?n,i++) [ ApplyDelta(result,x); ]; ];100 # ApplyDelta(_result,_term) <-- DestructiveAppend(result,term);";
        scriptMap.put("ApplyDelta",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Delta\",{ind1,ind2});";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TD\",{ind});10 # (TD(_i)X(_j)) <-- Delta(i,j);10 # (TD(_i) ( (_f) + (_g) ) ) <-- (TD(i)f) + (TD(i)g);10 # (TD(_i) ( (_f) - (_g) ) ) <-- (TD(i)f) - (TD(i)g);10 # (TD(_i) ( - (_g) ) ) <-- - TD(i)g;10 # (TD(_i) ( (_f) * (_g) ) ) <-- (TD(i)f)*g + f*(TD(i)g);10 # (TD(_i) ( (_f) ^ (n_IsPositiveInteger) ) ) <-- n*(TD(i)f)*f^(n-1);10 # (TD(_i)Delta(_j,_k)) <-- 0;10 # (TD(_i)f_IsNumber) <-- 0;";
        scriptMap.put("TD",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "(TExplicitSum(Ndim_IsInteger)(_body)) <-- Eval(body);";
        scriptMap.put("TExplicitSum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TList\",{head,tail});";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"TSum\",{indices,body});10 # (TSum({})(_body)) <-- body;20 # (TSum(_indices)(_body))_(IsInteger(Ndim)) <-- LocalSymbols(index,i,sum) [ Local(index,i,sum); index:=indices[1]; sum:=0; MacroLocal(index); For(i:=1,i<=?Ndim,i++) [ MacroBind(index,i); sum:=sum+Eval(TSum(Rest(indices))body); ]; sum; ];UnFence(\"TSum\",2);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EqualAsSets( A_IsList, B_IsList )_(Length(A)=?Length(B)) <--[ Local(Acopy,b,nba,result); Acopy := FlatCopy(A); result := True; ForEach(b,B) [ nba := Find(Acopy,b); If( nba <? 0, [ result := False; Break(); ] ); DestructiveDelete(Acopy,nba); ]; If( Not? result, result := Length(Acopy)=?0 ); result;];20 # EqualAsSets( _A, _B ) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(TrueFalse)[ MacroRulebase(TrueFalse,{var,expr}); 10 # TrueFalse(var_IsAtom,_expr) <-- `{(@expr) Where (@var)==False,(@expr) Where (@var)==True}; 20 # TrueFalse({},_expr) <-- `(@expr); 30 # TrueFalse(var_IsList,_expr) <-- `[ Local(t,h); Bind(h,First(@var)); Bind(t,Rest(@var)); TrueFalse(h,TrueFalse(t,@expr)); ]; Macro(LogicTest,{vars,expr1,expr2}) Verify(TrueFalse((@vars),(@expr1)), TrueFalse((@vars),(@expr2)));];";
        scriptMap.put("LogicTest",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"LogicVerify\",{aLeft,aRight})[ If(aLeft !=? aRight, Verify(CanProve(aLeft => aRight),True) );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RoundTo(x_IsNumber, precision_IsPositiveInteger) <--[ Local(oldPrec,result); oldPrec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(precision); Bind(result,DivideN( Round( MultiplyN(x, 10^precision) ), 10^precision )); BuiltinPrecisionSet(oldPrec); result;];10 # RoundTo(Complex(r_IsNumber, i_IsNumber), precision_IsPositiveInteger) <-- Complex(RoundTo(r, precision), RoundTo(i, precision));20 # RoundTo( Infinity,precision_IsPositiveInteger) <-- Infinity;20 # RoundTo(-Infinity,precision_IsPositiveInteger) <-- -Infinity;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Verify\",{aLeft,aRight})[ Local(result); result := Eval(aLeft);  If (Not?(IsEqual(result,aRight)), [ WriteString(\"******************\"); NewLine(); ShowLine(); NewLine(); Write(aLeft); NewLine(); WriteString(\" evaluates to \"); NewLine(); Write(result); NewLine(); WriteString(\" which differs from \"); NewLine(); Write(aRight); NewLine(); WriteString(\"******************\"); NewLine(); False; ], True );];HoldArgument(\"Verify\",aLeft);UnFence(\"Verify\",2);Macro(\"Verify\", {a,b,message})[ Echo(\"test \", @message); Verify(@a, @b);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RepeatedSquaresMultiply(_a,- (n_IsInteger)) <-- 1/RepeatedSquaresMultiply(a,n);15 # RepeatedSquaresMultiply(UniVariate(_var,_first,{_coef}),(n_IsInteger)) <-- UniVariate(var,first*n,{coef^n});20 # RepeatedSquaresMultiply(_a,n_IsInteger) <--[ Local(m,b); Bind(m,1); Bind(b,1); While(m<=?n) Bind(m,(ShiftLeft(m,1))); Bind(m, ShiftRight(m,1)); While(m>?0) [ Bind(b,b*b); If (Not?(IsEqual(BitAnd(m,n), 0)),Bind(b,b*a)); Bind(m, ShiftRight(m,1)); ]; b;];";
        scriptMap.put("RepeatedSquaresMultiply",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"UniDivide\",{u,v})[ Local(m,n,q,r,k,j); m := Length(u)-1; n := Length(v)-1; While (m>?0 And? IsZero(u[m+1])) m--; While (n>?0 And? IsZero(v[n+1])) n--; q := ZeroVector(m-n+1); r := FlatCopy(u);  For(k:=m-n,k>=?0,k--) [ q[k+1] := r[n+k+1]/v[n+1]; For (j:=n+k-1,j>=?k,j--) [ r[j+1] := r[j+1] - q[k+1]*v[j-k+1]; ]; ]; Local(end); end:=Length(r); While (end>?n) [ DestructiveDelete(r,end); end:=end-1; ]; {q,r};];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"SparseUniVar\",{var,termlist});300 # SparseUniVar(_var,_terms1) * SparseUniVar(_var,_terms2) <--SparseUniVar(var, MultiplyTerms(terms1,terms2));300 # SparseUniVar(_var,_terms1) + SparseUniVar(_var,_terms2) <--SparseUniVar(var, AddTerms(terms1,terms2));300 # SparseUniVar(_var,_terms1) - SparseUniVar(_var,_terms2) <--SparseUniVar(var, SubstractTerms(terms1,terms2));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BoundRealRoots(_p) <--[ BoundRealRoots(p,MinimumBound(p),MaximumBound(p));];BoundRealRoots(_p,_Mmin,_Mmax) <--[ Local(S,N,work,result,Vmin,Vmax,a,b,Va,Vb,c,Vc,x); result:={}; if (IsZero(p Where x==0)) [ p:=Simplify(p/x); result:={{0,0}}; ]; S:=SturmSequence(p); Vmin := SturmVariations(S,-Infinity); Vmax := SturmVariations(S,Infinity); N:=Vmin - Vmax; work:={}; if (N=?1) [ result:={{-Mmax,Mmax}}; ]; if (N>?1) [ work := { {-Mmax,-Mmin,Vmin,SturmVariations(S,-Mmin)}, { Mmin, Mmax,SturmVariations(S, Mmin),Vmax} }; ]; While(work !=? {}) [ {a,b,Va,Vb} := First(work); work := Rest(work); c:=(a+b)/2; Vc := SturmVariations(S,c); if (IsZero(p Where x == c)) [ Local(M,Vcmin,Vcplus,pnew); pnew := Simplify((p Where x == x+c)/x); M:=MinimumBound(pnew); Vcmin := SturmVariations(S, c-M); Vcplus := SturmVariations(S, c+M); result:=Concat(result,{{c,c}}); if (Va =? Vcmin+1) [ result:=Concat(result,{{a,c-M}}); ]; if (Va >? Vcmin+1) [ work:=Concat(work,{{a,c-M,Va,Vcmin}}); ]; if (Vb =? Vcplus-1) [ result:=Concat(result,{{c+M,b}}); ]; if (Vb <? Vcplus-1) [ work:=Concat(work,{{c+M,b,Vcplus,Vb}}); ]; ] else [ if (Va =? Vc+1) [ result:=Concat(result,{{a,c}}); ]; if (Va >? Vc+1) [ work:=Concat(work,{{a,c,Va,Vc}}); ]; if (Vb =? Vc-1) [ result:=Concat(result,{{c,b}}); ]; if (Vb <? Vc-1) [ work:=Concat(work,{{c,b,Vc,Vb}}); ]; ]; ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 5 # MaximumBound(_p)_(IsZero(p Where x==0)) <-- MaximumBound(Simplify(p/x));10 # MaximumBound(_p)_(Degree(p)>?0) <--[ Local(an); an:=Coef(p,(Degree(p)-1) .. 0)/Coef(p,Degree(p)); an := N(Eval(Abs(an)^(1/(1 .. Degree(p))))); Rationalize(2*Maximum(an));];20 # MaximumBound(_p) <-- Infinity;";
        scriptMap.put("MaximumBound",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # MinimumBound(_p)_(IsZero(p Where x==0)) <-- 0;20 # MinimumBound(_p)_(Degree(p)>?0) <--[ Local(an,result); an:=Coef(p,1 .. (Degree(p)))/Coef(p,0); an := N(Eval(Abs(an)^(1/(1 .. Degree(p))))); result:=0; an:=2*Maximum(an); if(Not? IsZero(an)) [result := 1/an;]; Simplify(Rationalize(result));];30 # MinimumBound(_p) <-- -Infinity;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SturmVariations(_S,Infinity) <--[ Local(i,s); s:=FillList(0,Length(S)); For(i:=1,i<=?Length(S),i++) [ s[i] := LeadingCoef(S[i]); ]; SturmVariations(s);];10 # SturmVariations(_S,-Infinity) <--[ Local(i,s); s:=FillList(0,Length(S)); For(i:=1,i<=?Length(S),i++) [ s[i] := ((-1)^Degree(S[i]))*LeadingCoef(S[i]); ]; SturmVariations(s);];20 # SturmVariations(_S,_x) <-- SturmVariations(Eval(S));SturmVariations(_S) <--[ Local(result,prev); result:=0; While(Length(S)>?0 And? IsZero(S[1])) S:=Rest(S); if (Length(S)>?0) [ prev:=S[1]; ForEach(item,Rest(S)) [ if(Not? IsZero(item)) [ if (prev*item <? 0) [result++;]; prev:=item; ]; ]; ]; result;];";
