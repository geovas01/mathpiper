...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnAugment( M_IsMatrix, v_IsVector )_(Length(v)=?Dimensions(M)[2]) <--[ If( Not? Bound?(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnAugment,{M,v}) ); Local(mRows,nCols,newMat,ir); Local(MT,MA); MT := Transpose(M); MT := MatrixRowStack(MT,v); MA := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnReplace( M_IsMatrix, jCol_PositiveInteger?, v_IsVector )_(Length(v)=?Dimensions(M)[2]) <--[ If( Not? Bound?(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnReplace,{M,jCol,v}) ); Local(mRows,nCols,MT); {mRows,nCols} := Dimensions(M); If( jCol <=? nCols, [ MT:=Transpose(M); DestructiveReplace(MT,jCol,v); M:=Transpose(MT); ] ); M;];";
        scriptMap.put("MatrixColumnReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnSwap( M_IsMatrix, jCol1_PositiveInteger?, jCol2_PositiveInteger? )_ (And?(jCol1<=?Dimensions(M)[2],jCol2<=?Dimensions(M)[2])) <--[ If( Not? Bound?(iDebug), iDebug := False ); If( iDebug, Tell(MatrixColumnSwap,{M,jCol1,jCol2}) ); Local(MT); MT := Transpose(M); MT := MatrixRowSwap(MT,jCol1,jCol2); M := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowReplace( M_IsMatrix, iRow_PositiveInteger?, v_IsVector )_(Length(v)=?Length(M[1])) <--[ If( Not? Bound?(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowReplace,{M,iRow,v}) ); Local(mRows,nCols); {mRows,nCols} := Dimensions(M); If( iRow <=? mRows, DestructiveReplace(M,iRow,v) ); M;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowSwap( M_IsMatrix, iRow1_PositiveInteger?, iRow2_PositiveInteger? )_ (And?(iRow1<=?Dimensions(M)[1],iRow2<=?Dimensions(M)[1])) <--[ If( Not? Bound?(iDebug), iDebug := False ); If( iDebug, Tell(MatrixRowSwap,{M,iRow1,iRow2}) ); Local(row1,row2); If( iRow1 !=? iRow2, [ row1 := MatrixRow(M,iRow1); row2 := MatrixRow(M,iRow2); DestructiveReplace(M,iRow1,row2); DestructiveReplace(M,iRow2,row1); ] ); M;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Hermitian?(A_IsMatrix) <-- (Conjugate(Transpose(A))=?A);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IntPowerNum(_x, 0, _func, _unity) <-- unity;10 # IntPowerNum(_x, n_IsInteger, _func, _unity) <--[  Local(result);  While(n >? 0) [ If( (n&1) =? 1, If( Bound?(result),  result := Apply(func, {result,x}), result := x,  ) ); x := Apply(func, {x,x}); n := n>>1; ]; result;];";
