...
public class MultipleArityRulebase {
     ...
     */
    public SingleArityRulebase getUserFunction(int aArity, int aStackBase, Environment aEnvironment) throws Exception {
        int ruleIndex;
        ...
        for (ruleIndex = 0; ruleIndex < numberOfRules; ruleIndex++) {
            if(iFunctions.get(ruleIndex) == null) LispError.lispAssert(aEnvironment, aStackBase);
     ...
     */
    public void holdArgument(String aVariable, int aStackBase, Environment aEnvironment) throws Exception {
        int ruleIndex;
     ...
     */
    public void addRulebaseEntry(Environment aEnvironment, int aStackBase, SingleArityRulebase aNewFunction) throws Exception {
        int ruleIndex;
        //Find function body with the right arity
        int numberOfRules = iFunctions.size();
        for (ruleIndex = 0; ruleIndex < numberOfRules; ruleIndex++) {
            if(((SingleArityRulebase) iFunctions.get(ruleIndex)) == null) LispError.lispAssert(aEnvironment, aStackBase);
            if(aNewFunction == null) LispError.lispAssert(aEnvironment, aStackBase);
            if(((SingleArityRulebase) iFunctions.get(ruleIndex)).isArity(aNewFunction.arity())) LispError.throwError(aEnvironment, aStackBase, "ARITY ALREADY DEFINED FOR FUNCTION: " + aNewFunction.functionName);
            if(aNewFunction.isArity(((SingleArityRulebase) iFunctions.get(ruleIndex)).arity())) LispError.throwError(aEnvironment, aStackBase, "ARITY ALREADY DEFINED FOR FUNCTION: " + aNewFunction.functionName);
        }
     ...
     */
    public void deleteRulebaseEntry(int aArity, int aStackBase, Environment aEnvironment) throws Exception {
