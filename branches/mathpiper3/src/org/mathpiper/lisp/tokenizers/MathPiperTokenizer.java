...
public class MathPiperTokenizer {
    ...
    /// or an empty list.
    public String nextToken(Environment aEnvironment, int aStackBase, MathPiperInputStream aInput) throws Exception {
         ...
         //System.out.println(streamCharacter);
            ...
            if (streamCharacter == '(') {
                ...
                while (true) {
                    ...
                    if(aInput.endOfStream()) LispError.throwError(aEnvironment, aStackBase, LispError.COMMENT_TO_END_OF_FILE, "");
                    if (aInput.peek() == '/') {
                        aInput.next();  // consume /
                        redo = true;
                        break;
                ...
                while (aInput.peek() != '\"') {
                    if (aInput.peek() == '\\') {
                        aInput.next();
                        if(aInput.endOfStream()) LispError.throwError(aEnvironment, aStackBase, LispError.PARSING_INPUT, aInput);
                    ...
                    if(aInput.endOfStream()) LispError.throwError(aEnvironment, aStackBase, LispError.PARSING_INPUT, "Last character read was <" + aResult + ">.");
                }
                //TODO FIXME is following append char correct?
