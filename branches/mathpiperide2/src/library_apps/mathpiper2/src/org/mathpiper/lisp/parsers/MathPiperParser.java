...
public class MathPiperParser extends Parser
{
    ...
    void readExpression(Environment aEnvironment,int aStackTop, int depth) throws Exception
    {
        ...
        for (;;)
        {
            ...
            if (iLookAhead == iEnvironment.iProgOpenAtom.car())
            {
                ...
                {
                    LispError.raiseError("Expecting a ] close bracket for program block, but got " + iLookAhead + " instead." + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, aEnvironment);
                    return;
                }
                matchToken(aStackTop, iLookAhead);
                // Build into Ntn(...)
            ...
            } else
            {
                ...
                if (op == null)
                {
                    ...
                    if(iLookAhead.equals(""))
                    {
                       ...
                       LispError.raiseError("Expression must end with a semi-colon (;) " + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, aEnvironment);
                        return;
                    }
    ...
    void readAtom(Environment aEnvironment, int aStackTop) throws Exception
    {
        ...
        else if (iLookAhead == iEnvironment.iListOpenAtom.car())
        {
            ...
            while (iLookAhead != iEnvironment.iListCloseAtom.car())
            {
                ...
                {
                    LispError.raiseError("Expecting a } close bracket for a list, but got " + iLookAhead + " instead." + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, aEnvironment);
                    return;
                }
            }
            matchToken(aStackTop, iLookAhead);
        ...
        else if (iLookAhead == iEnvironment.iProgOpenAtom.car())
        {
            ...
            while (iLookAhead != iEnvironment.iProgCloseAtom.car())
            {
                ...
                {
                    LispError.raiseError("Expecting ; end of statement in program block, but got " + iLookAhead + " instead." + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, aEnvironment);
                    return;
                }
            }
            matchToken(aStackTop, iLookAhead);
        ...
        else
        {
            ...
            if (iLookAhead == iEnvironment.iBracketOpenAtom.car())
            {
                ...
                while (iLookAhead != iEnvironment.iBracketCloseAtom.car())
                {
                    ...
                    {
                        LispError.raiseError("Expecting ) closing bracket for sub-expression, but got " + iLookAhead + " instead. " + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, aEnvironment);
                        return;
                    }
                }
                matchToken(aStackTop, iLookAhead);
    ...
    void fail(int aStackTop) throws Exception // called when parsing fails, raising an exception
    {
        ...
        if (iLookAhead != null)
        {
            LispError.raiseError("Error parsing expression, near token " + iLookAhead + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, iEnvironment);
        }
        LispError.raiseError("Error parsing expression." + " Function name: " + iInput.iStatus.getFileName() + ", " + " Line number: " + iInput.iStatus.getLineNumber() + ", " + " Line index: " + iInput.iStatus.getLineIndex() + ". ", "[INTERNAL]", aStackTop, iEnvironment);
    }
};
