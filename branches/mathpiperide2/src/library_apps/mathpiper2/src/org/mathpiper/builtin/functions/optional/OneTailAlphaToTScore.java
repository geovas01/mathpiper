...
public class OneTailAlphaToTScore extends BuiltinFunction{
    ...
    public void evaluate(Environment aEnvironment, int aStackTop) throws Exception
    {
        ...
        if(!degreesOfFreedom.isInteger() || degreesOfFreedom.toInt() < 0) LispError.throwError(aEnvironment, aStackTop, "The first argument must be an integer which is greater than 0.", "OneTailAlphaToTScore");

        BigNumber alpha = org.mathpiper.lisp.Utility.getNumber(aEnvironment, aStackTop, 2);

        if(alpha.toDouble() < 0 || alpha.toDouble() > .5) LispError.throwError(aEnvironment, aStackTop, "The second argument must be greater than 0 and less than or equal to .5.", "OneTailAlphaToTScore");
