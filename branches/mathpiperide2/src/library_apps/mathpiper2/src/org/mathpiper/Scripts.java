...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString[0] = null;
        scriptString[1] = "Rulebase(MathPiperInitLoad,{});RuleHoldArguments(MathPiperInitLoad, 0, 1, True)[  StandardOperatorsLoad(); Constant(True);];";
        scriptMap.put("MathPiperInitLoad",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Nth\",{alist,aindex});RuleHoldArguments(\"Nth\",2,10, And?(Equal?(Function?(alist),True), Equal?(Integer?(aindex),True), Not?(Equal?(First(FunctionToList(alist)),Nth)) )) MathNth(alist,aindex);RuleHoldArguments(\"Nth\",2,14, And?(Equal?(String?(alist),True),List?(aindex)) )[ Local(result); result:=\"\"; ForEach(i,aindex) [ result := result : StringMidGet(i,1,alist); ]; result;];RuleHoldArguments(\"Nth\",2,15,Equal?(String?(alist),True))[ StringMidGet(aindex,1,alist);];RuleHoldArguments(\"Nth\",2,20,Equal?(List?(aindex),True))[ Map({{ii},alist[ii]},{aindex});];RuleHoldArguments(\"Nth\",2,30, And?( Equal?(Generic?(alist),True), Equal?(GenericTypeName(alist),\"Array\"), Equal?(Integer?(aindex),True) ) )[ ArrayGet(alist,aindex);];RuleHoldArguments(\"Nth\",2,40,Equal?(String?(aindex),True))[ Local(as); as := Assoc(aindex,alist); If (Not?(Equal?(as,Empty)),Bind(as,Nth(as,2))); as;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " Rulebase(\"'\", {expression}); RuleHoldArguments(\"'\", 1, 100, True) `(Hold(@expression)); HoldArgument(\"'\",expression);";
        scriptMap.put("'",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DefMacroRulebase(\"Defun\",{func,args,body});RuleHoldArguments(\"Defun\",3,0,True)[ Local(nrargs); Bind(nrargs,Length(@args)); Retract(@func, `(@nrargs)); Rulebase(@func,@args); Local(fn,bd); Bind(fn,Hold(@func)); Bind(bd,Hold(@body)); `RuleHoldArguments(@fn, @nrargs, 0,True)(@bd);];";
        scriptMap.put("Defun",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Function\",{oper,args,body});RuleHoldArguments(\"Function\",3,2047, And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") )))[ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); MacroRulebaseListed(oper,args); MacroRule(oper,Length(args),1025,True) body; ];RuleHoldArguments(\"Function\",3,2048,True)[ Retract(oper,Length(args)); MacroRulebase(oper,args); MacroRule(oper,Length(args),1025,True) body;];Rulebase(\"Function\",{oper});RuleHoldArguments(\"Function\",1,2047, And?(Function?(oper), GreaterThan?(Length(oper), 1), Equal?( MathNth(oper, Length(oper)), ToAtom(\"...\") )))[ Local(args); Bind(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebaseListed(Type(oper),args) );];RuleHoldArguments(\"Function\",1,2048, And?(Function?(oper)))[ Local(args); Bind(args,Rest(FunctionToList(oper))); If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebase(Type(oper),args) );];HoldArgument(\"Function\",oper);HoldArgument(\"Function\",args);HoldArgument(\"Function\",body);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"@\",{func,arg});RuleHoldArguments(\"@\",2,1,List?(arg)) Apply(func,arg);RuleHoldArguments(\"@\",2,2,True ) Apply(func,{arg});";
        scriptMap.put("@",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\":\",{head,tail});RuleHoldArguments(\":\",2,20,List?(head) And? Not? List?(tail) ) Concat(head,{tail});RuleHoldArguments(\":\",2,30,List?(tail) ) Concat({head},tail);RuleHoldArguments(\":\",2,10,String?(tail) And? String?(head)) ConcatStrings(head,tail);UnFence(\":\",2);";
        scriptMap.put(":",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Drop\", {lst, range});RuleHoldArguments(\"Drop\", 2, 1, List?(range)) Concat(Take(lst,range[1]-1), Drop(lst, range[2]));RuleHoldArguments(\"Drop\", 2, 2, range >=? 0) If( range =? 0 Or? lst =? {}, lst, Drop( Rest(lst), range-1 ));RuleHoldArguments(\"Drop\", 2, 2, range <? 0) Take( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Take\", {lst, range});RuleHoldArguments(\"Take\", 2, 1, List?(range)) Take( Drop(lst, range[1] -1), range[2] - range[1] + 1);RuleHoldArguments(\"Take\", 2, 2, range >=? 0) If( Length(lst)=?0 Or? range=?0, {}, Concat({First(lst)}, Take(Rest(lst), range-1)));RuleHoldArguments(\"Take\", 2, 2, range <? 0) Drop( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"MakeVector\",{vec,dimension});RuleHoldArguments(\"MakeVector\",2,1,True)[ Local(res,i); res:={}; i:=1; Bind(dimension,AddN(dimension,1)); While(LessThan?(i,dimension)) [ DestructiveInsert(res,1,ToAtom(ConcatStrings(ToString(vec),ToString(i)))); Bind(i,AddN(i,1)); ]; DestructiveReverse(res);];";
        scriptMap.put("MakeVector",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"<--\",{leftOperand,rightOperand});RuleHoldArguments(\"<--\",2,1,Equal?(Type(leftOperand),\"#\"))[ DefinePattern(leftOperand[2],rightOperand,leftOperand[1],True);];RuleHoldArguments(\"<--\",2,2,Function?(leftOperand))[ DefinePattern(leftOperand,rightOperand,0,True);];HoldArgument(\"<--\",leftOperand);HoldArgument(\"<--\",rightOperand);";
        scriptMap.put("<--",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Object\",{pred,x});RuleHoldArguments(\"Object\",2,0,Apply(pred,{x})=?True) x;";
