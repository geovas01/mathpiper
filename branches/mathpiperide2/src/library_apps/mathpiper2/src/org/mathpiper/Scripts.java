...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Nth\",{alist,aindex});Rule(\"Nth\",2,10, And?(Equal?(Function?(alist),True), Equal?(Integer?(aindex),True), Not?(Equal?(First(FunctionToList(alist)),Nth)) )) MathNth(alist,aindex);Rule(\"Nth\",2,14, And?(Equal?(IsString(alist),True),IsList(aindex)) )[ Local(result); result:=\"\"; ForEach(i,aindex) [ result := result : StringMidGet(i,1,alist); ]; result;];Rule(\"Nth\",2,15,Equal?(IsString(alist),True))[ StringMidGet(aindex,1,alist);];Rule(\"Nth\",2,20,Equal?(IsList(aindex),True))[ Map({{ii},alist[ii]},{aindex});];Rule(\"Nth\",2,30, And?( Equal?(Generic?(alist),True), Equal?(GenericTypeName(alist),\"Array\"), Equal?(Integer?(aindex),True) ) )[ ArrayGet(alist,aindex);];Rule(\"Nth\",2,40,Equal?(IsString(aindex),True))[ Local(as); as := Assoc(aindex,alist); If (Not?(Equal?(as,Empty)),Bind(as,Nth(as,2))); as;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # 0 / 0 <-- Undefined;52 # x_PositiveNumber? / 0 <-- Infinity;52 # x_NegativeNumber? / 0 <-- -Infinity;55 # (_x / y_IsNumber)_(Zero?(y)) <-- Undefined;55 # 0 / _x <-- 0;56 # (x_NonZeroInteger? / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger? * _expr) / y_NonZeroInteger?)_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*expr)/QuotientN(y,gcd); ];57 # ((x_NonZeroInteger?) / (y_NonZeroInteger? * _expr))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); QuotientN(x,gcd)/(QuotientN(y,gcd)*expr); ];57 # ((x_NonZeroInteger? * _p) / (y_NonZeroInteger? * _q))_(GcdN(x,y) >? 1) <-- [ Local(gcd); Bind(x,x); Bind(y,y); Bind(gcd,GcdN(x,y)); (QuotientN(x,gcd)*p)/(QuotientN(y,gcd)*q); ];60 # (x_Decimal? / y_IsNumber) <-- DivideN(x,y);60 # (x_IsNumber / y_Decimal?) <-- DivideN(x,y); 60 # (x_IsNumber / y_IsNumber)_(InNumericMode()) <-- DivideN(x,y);90 # x_Infinity? / y_Infinity? <-- Undefined;95 # x_Infinity? / y_IsNumber <-- Sign(y)*x;95 # x_Infinity? / y_IsComplex <-- Infinity;90 # Undefined / _y <-- Undefined;90 # _y / Undefined <-- Undefined;100 # _x / _x <-- 1;100 # _x / 1 <-- x;100 # (_x / y_NegativeNumber?) <-- -x/(-y);100 # (_x / - _y) <-- -x/y;150 # (_x) / (_x) ^ (n_Constant?) <-- x^(1-n);150 # (_x) ^ (n_Constant?) * (_x) <-- x^(n-1);150 # Sqrt(_x) / (_x) ^ (n_Constant?) <-- x^(1/2-n);150 # (_x) ^ (n_Constant?) / Sqrt(_x) <-- x^(n-1/2);150 # (_x) / Sqrt(_x) <-- Sqrt(x);200 # (_x / _y)/ _z <-- x/(y*z);230 # _x / (_y / _z) <-- (x*z)/y;240 # (xlist_IsList / ylist_IsList)_(Length(xlist)=?Length(ylist)) <-- Map(\"/\",{xlist,ylist});250 # (x_IsList / _y)_(Not?(IsList(y))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(x),i++) [ DestructiveInsert(result,i,x[i] / y); ]; result;];250 # (_x / y_IsList)_(Not?(IsList(x))) <--[ Local(i,result); result:={}; For(i:=1,i<=?Length(y),i++) [ DestructiveInsert(result,i,x/y[i]); ]; result;];250 # _x / Infinity <-- 0;250 # _x / (-Infinity) <-- 0;400 # 0 / _x <-- 0;";
        scriptMap.put("/",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SetGlobalLazyVariable(I,Complex(0,1));LocalSymbols(CacheOfConstantsN) [Rulebase(\"CachedConstant\", {C'cache, C'atom, C'func});UnFence(\"CachedConstant\", 3); HoldArgument(\"CachedConstant\", C'func);HoldArgument(\"CachedConstant\", C'cache); Rule(\"CachedConstant\", 3, 10, And?(Atom?(C'atom), Function?(C'func)))[ Local(C'name,C'functionName); Bind(C'name, ToString(C'atom));  Bind(C'functionName,ConcatStrings(\"Internal'\",C'name)); If(  Atom?(Eval(C'cache)), MacroBind(Eval(C'cache), {}) );  If( Equal?(Builtin'Assoc(C'name, Eval(C'cache)), Empty),  [  MacroUnbind(C'atom);  DestructiveInsert(Eval(C'cache), 1, {C'name, 0, 0});   MacroRulebase(C'functionName, {}); `( Rule(@C'functionName, 0, 1024, True) [ Local(new'prec, new'C, cached'C); Bind(new'prec, BuiltinPrecisionGet());   Bind(cached'C, Builtin'Assoc(@C'name, @C'cache)); If( MathNth(cached'C, 2) !=? new'prec, [  If(Equal?(InVerboseMode(),True), Echo(\"CachedConstant: Info: constant \", @C'name, \" is being recalculated at precision \", new'prec)); Bind(new'C, RoundTo(Eval(@C'func),new'prec)); DestructiveReplace(cached'C, 2, new'prec); DestructiveReplace(cached'C, 3, new'C); new'C; ],  MathNth(cached'C, 3) ); ]);   ],  Echo(\"CachedConstant: Warning: constant \", C'atom, \" already defined\") );];Rule(\"CachedConstant\", 3, 20, True) Echo(\"CachedConstant: Error: \", C'atom, \" must be an atom and \", C'func, \" must be a function.\");Function(\"AssignCachedConstantsN\", {})[ Local(var,fname); ForEach(var, AssocIndices(CacheOfConstantsN)) [ MacroUnbind(ToAtom(var)); Bind(fname,ConcatStrings(\"Internal'\",var)); Bind(var,ToAtom(var));  `SetGlobalLazyVariable((@var), ListToFunction({ToAtom(fname)})); ];];UnFence(\"AssignCachedConstantsN\", 0);Function(\"ClearCachedConstantsN\", {})[ Local(c'entry); ForEach(c'entry, CacheOfConstantsN) MacroUnbind(ToAtom(c'entry[1]));];UnFence(\"ClearCachedConstantsN\", 0);CachedConstant(CacheOfConstantsN, Pi,[ Local(result,old'prec); Bind(old'prec,BuiltinPrecisionGet());If(Equal?(InVerboseMode(),True), Echo(\"Recalculating Pi at precision \",old'prec+5)); BuiltinPrecisionSet(BuiltinPrecisionGet()+5); result := MathPi();If(Equal?(InVerboseMode(),True),Echo(\"Switching back to precision \",old'prec)); BuiltinPrecisionSet(old'prec); result;]);CachedConstant(CacheOfConstantsN, gamma, GammaConstNum());CachedConstant(CacheOfConstantsN, GoldenRatio, N( (1+Sqrt(5))/2 ) );CachedConstant(CacheOfConstantsN, Catalan, CatalanConstNum() );]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"ForEach\",{item,listOrString,body})[ If(And?(Equal?(Generic?(listOrString),True), Equal?(GenericTypeName(listOrString),\"Array\") ), `ForEachInArray(@item,listOrString,@body), [ MacroLocal(item);  If(IsString(listOrString), [  Local(index, stringLength);  stringLength := Length(listOrString);  index := 1; While(index <=? stringLength ) [ MacroBind(item,listOrString[index] );  Eval(body);  index++; ]; ], [ Local(foreachtail); Bind(foreachtail,listOrString); While(Not?(Equal?(foreachtail,{}))) [ MacroBind(item,First(foreachtail)); Eval(body); Bind(foreachtail,Rest(foreachtail)); ]; ]); ]);];UnFence(\"ForEach\",3);HoldArgumentNumber(\"ForEach\",3,1);HoldArgumentNumber(\"ForEach\",3,3);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(TraceStart,TraceEnter,TraceLeave,DebugStart,DebugEnter, DebugLeave,ProfileStart,ProfileEnter,result, WriteLines,ClearScreenString,Debug'FileLoaded, Debug'FileLines, Debug'NrLines, debugstepoverfile, debugstepoverline) [TraceStart() := [indent := 0;];TraceEnter() :=[ indent++; Space(2*indent); Echo(\"Enter \",CustomEval'Expression());];TraceLeave() :=[ Space(2*indent); Echo(\"Leave \",CustomEval'Result()); indent--;];Macro(TraceExp,{expression})[ TraceStart(); CustomEval(TraceEnter(),TraceLeave(),CustomEval'Stop(),@expression);];DebugStart():=[ debugging:=True; debugstopdepth := -1; breakpoints:={}; filebreakpoints := {}; debugstopped:=False; debugverbose:=False; debugcallstack:={}; breakpredicate:=False;];DebugRun():= [debugging:=False;True;];DebugStep():=[debugging:=False;nextdebugging:=True;];DebugStepOver():=[ debugging:=False; debugstepoverfile := DebugFile(CustomEval'Expression()); debugstepoverline := DebugLine(CustomEval'Expression()); debugstopdepth := Length(debugcallstack);];DebugBreakAt(file,line):=[ Check(DebugMode?(), \"Mode\", \"DebugBreakAt only supported in the debug build of MathPiper\"); If(filebreakpoints[file] =? Empty,filebreakpoints[file]:={}); DestructiveAppend(filebreakpoints[file],line);];DebugRemoveBreakAt(file,line):=[ Check(DebugMode?(), \"Mode\", \"DebugRemoveBreakAt only supported in the debug build of MathPiper\"); If(filebreakpoints[file] =? Empty,filebreakpoints[file]:={}); filebreakpoints[file] := Difference(filebreakpoints[file],{line});];DebugStop():=[debugging:=False;debugstopped:=True;CustomEval'Stop();];DebugVerbose(verbose):=[debugverbose:=verbose;];DebugAddBreakpoint(fname_IsString) <-- [ breakpoints := fname:breakpoints;];Macro(DebugBreakIf,{predicate})[ breakpredicate:= Hold(@predicate);];BreakpointsClear() <--[ breakpredicate:=False; breakpoints := {};];Macro(DebugLocals,{})[ Echo(\"\"); Echo(\"*************** Current locals on the stack ****************\"); ForEach(item,CustomEval'Locals()) [ Echo(\" \",item,\" : \",Eval(item)); ]; Echo(\"\");];DebugCallstack() <--[ Echo(\"\"); Echo(\"*************** Function call stack ****************\"); ForEach(item,debugcallstack) [ if(Function?(item)) Echo(\" Function \",Type(item),\" : \",item) else Echo(\" Variable \",item); ]; Echo(\"\");];Macro(DebugEnter,{})[ debugcallstack := CustomEval'Expression():debugcallstack;  If(debugging =? False And? Eval(breakpredicate) =? True, [ breakpredicate:=False; debugging:=True; ]); If(debugging =? False And? DebugMode?(), [ Local(file,line); file := DebugFile(CustomEval'Expression()); If(filebreakpoints[file] !=? Empty, [ line := DebugLine(CustomEval'Expression()); If(Not?(file =? debugstepoverfile And? line =? debugstepoverline) And? Contains(filebreakpoints[file],line), [ debugging:=True; ] ); ]); ]);  If(debugging =? False And? Function?(CustomEval'Expression()) And? Contains(breakpoints,Type(CustomEval'Expression())), debugging:=True); nextdebugging:=False; If (debugging, [ If(DebugMode?(),DebugShowCode()); Echo(\">>> \",CustomEval'Expression()); While(debugging) [ Echo(\"DebugResult: \",Eval(PipeFromString(ReadCmdLineString(\"Debug> \"):\";\")Read()));  If(IsExitRequested(),debugging:=False); ]; ]); debugging:=nextdebugging; If(IsExitRequested(),debugstopped:=True);];Macro(DebugLeave,{})[ If(debugging =? False And? debugstopdepth >=? 0 And? Length(debugcallstack) =? debugstopdepth, [ debugstepoverline := -1; debugging := True; debugstopdepth := -1; ]); debugcallstack := Rest(debugcallstack); If(debugverbose,Echo(CustomEval'Result(),\" <-- \",CustomEval'Expression()));];Macro(Debug,{expression})PipeToStdout()[ DebugStart(); CustomEval(DebugEnter(),DebugLeave(),If(debugstopped,Check(False, \"Debug\", \"\"),[debugging:=True;debugcallstack := Rest(debugcallstack);]),@expression);];ProfileStart():=[ profilefn:={};];10 # ProfileEnter()_(Function?(CustomEval'Expression())) <--[ Local(fname); fname:=Type(CustomEval'Expression()); If(profilefn[fname]=?Empty,profilefn[fname]:=0); profilefn[fname] := profilefn[fname]+1;];Macro(Profile,{expression})[ ProfileStart(); CustomEval(ProfileEnter(),True,CustomEval'Stop(),@expression); ForEach(item,profilefn) Echo(\"Function \",item[1],\" called \",item[2],\" times\");];Macro(EchoTime,{expression})[ Local(result); Echo(Time()Bind(result, @expression), \"seconds taken.\"); result;];ClearScreenString := UnicodeToString(27):\"[2J\":UnicodeToString(27):\"[1;1H\";WriteLines(filename,lines,from,nrlines,breakpoints,current):=[ Local(i,nr); nr:=Length(lines); WriteString(ClearScreenString); Echo(\"File \",filename,\" at line \",current); For(i:=from,i<?from+nrlines And? i<?nr,i++) [ if (current =? i) WriteString(\">\") else WriteString(\" \"); if (Contains(breakpoints,i)) WriteString(\"*\") else WriteString(\" \"); WriteString(\"| \"); Echo(lines[i][1]); ];];Debug'FileLoaded := \"\";Debug'FileLines := {};Debug'NrLines:=20;DebugShowCode():=[ False;];]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Function\",{oper,args,body});Rule(\"Function\",3,2047, And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") )))[ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); MacroRulebaseListed(oper,args); MacroRule(oper,Length(args),1025,True) body; ];Rule(\"Function\",3,2048,True)[ Retract(oper,Length(args)); MacroRulebase(oper,args); MacroRule(oper,Length(args),1025,True) body;];Rulebase(\"Function\",{oper});Rule(\"Function\",1,2047, And?(Function?(oper), GreaterThan?(Length(oper), 1), Equal?( MathNth(oper, Length(oper)), ToAtom(\"...\") )))[ Local(args); Bind(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebaseListed(Type(oper),args) );];Rule(\"Function\",1,2048, And?(Function?(oper)))[ Local(args); Bind(args,Rest(FunctionToList(oper))); If(RulebaseDefined(Type(oper),Length(args)), False,  MacroRulebase(Type(oper),args) );];HoldArgument(\"Function\",oper);HoldArgument(\"Function\",args);HoldArgument(\"Function\",body);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Macro\",{oper,args,body});HoldArgument(\"Macro\",oper);HoldArgument(\"Macro\",args);HoldArgument(\"Macro\",body);Rule(\"Macro\",3,2047, And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") )))[ DestructiveDelete(args,Length(args));  Retract(oper,Length(args)); `DefMacroRulebaseListed(@oper,@args); MacroRule(oper,Length(args),1025,True) body; ];Rule(\"Macro\",3,2048,True)[ Retract(oper,Length(args)); `DefMacroRulebase(@oper,@args); MacroRule(oper,Length(args),1025,True) body;];Rulebase(\"Macro\",{oper});Rule(\"Macro\",1,2047, And?(Function?(oper), GreaterThan?(Length(oper), 1), Equal?( MathNth(oper, Length(oper)), ToAtom(\"...\") )))[ Local(args,name); Bind(args,Rest(FunctionToList(oper))); DestructiveDelete(args,Length(args));  Bind(name,Type(oper)); If(RulebaseDefined(Type(oper),Length(args)), False,  `DefMacroRulebaseListed(@name,@args) );];Rule(\"Macro\",1,2048, And?(Function?(oper)))[ Local(args,name); Bind(args,Rest(FunctionToList(oper))); Bind(name,Type(oper)); If(RulebaseDefined(Type(oper),Length(args)), False,  [ `DefMacroRulebase(@name,@args); ] );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Unholdable\",{var});HoldArgument(\"Unholdable\",var);UnFence(\"Unholdable\",1);Rule(\"Unholdable\",1,10,Equal?(Type(Eval(var)),\"Eval\"))[ MacroBind(var,Eval(Eval(var))); ];Rule(\"Unholdable\",1,20,True)[  True;];";
        scriptMap.put("Unholdable",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\":=\",{aLeftAssign,aRightAssign});UnFence(\":=\",2);HoldArgument(\":=\",aLeftAssign);HoldArgument(\":=\",aRightAssign);Rule(\":=\",2,0,Atom?(aLeftAssign))[ Check( Not? IsNumber(aLeftAssign), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); MacroBind(aLeftAssign,Eval(aRightAssign)); Eval(aLeftAssign);];Rule(\":=\",2,0,IsList(aLeftAssign))[ Map(\":=\",{aLeftAssign,Eval(aRightAssign)});];Rulebase(\"AssignArray\",{setlistterm,setlistindex,setlistresult});UnFence(\"AssignArray\",3);Rule(\"AssignArray\",3,1,IsString(setlistindex))[ Local(item); item:=Assoc(setlistindex,setlistterm); If(item =? Empty, DestructiveInsert(setlistterm,1,{setlistindex,setlistresult}), DestructiveReplace(item,2,setlistresult) ); True;];Rule(\"AssignArray\",3,1, And?( Equal?(Generic?(setlistterm),True), Equal?(GenericTypeName(setlistterm),\"Array\") ) )[ ArraySet(setlistterm,setlistindex,setlistresult);];Rule(\"AssignArray\",3,2,True)[ DestructiveReplace(setlistterm ,setlistindex, setlistresult); True;];Rule(\":=\",2,10,Function?(aLeftAssign) And? (First(FunctionToList(aLeftAssign)) =? Nth))[ Local(frst,scnd); Local(lst); Bind(lst,(FunctionToList(aLeftAssign))); Bind(lst,Rest(lst)); Bind(frst, Eval(First(lst))); Bind(lst,Rest(lst)); Bind(scnd, Eval(First(lst))); AssignArray(frst,scnd,Eval(aRightAssign));];Rule(\":=\",2,30,Function?(aLeftAssign) And? Not?(Equal?(aLeftAssign[0], ToAtom(\":=\"))) )[ Check( Not? Equal?(aLeftAssign[0], ToAtom(\"/\")), \"Argument\", \"Only a variable can be placed on the left side of an := operator.\" ); Local(oper,args,arity); Bind(oper,ToString(aLeftAssign[0])); Bind(args,Rest(FunctionToList(aLeftAssign))); If( And?(GreaterThan?(Length(args), 1), Equal?( MathNth(args, Length(args)), ToAtom(\"...\") )),  [ DestructiveDelete(args,Length(args));  Bind(arity,Length(args)); Retract(oper,arity); MacroRulebaseListed(oper, args); ],  [ Bind(arity,Length(args)); Retract(oper,arity); MacroRulebase(oper, args); ] ); Unholdable(aRightAssign); MacroRule(oper,arity,1025,True) aRightAssign;];";
        scriptMap.put(":=",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( RationalFunction?(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( Atom?(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- [ Local(r); If( RationalFunction?(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # Monomial?(_expr) <-- False;10 # FactorsMonomial(expr_Monomial?) <--[ If(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); If( RationalFunction?(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); If(InVerboseMode(),Tell(\" \",{num,den})); Ns := Select(num, \"IsComplex\"); If(InVerboseMode(),Tell(\" \",Ns)); If( Ns =? {}, If( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); If(InVerboseMode(),Tell(\" \",num)); result := {}; ForEach(f,num) [ If( IsComplex(f),  DestructiveAppend(result,{(f),1}), If( Atom?(f), DestructiveAppend(result,{f,1}), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Apply(_applyoper,_applyargs) _ (Or?(IsString(applyoper), IsList(applyoper))) <-- ApplyFast(applyoper,applyargs);20 # Apply(applyoper_Atom?,_applyargs) <-- ApplyFast(ToString(applyoper),applyargs);30 # Apply(Lambda(_args,_body),_applyargs) <-- `ApplyFast(Hold({@args,@body}),applyargs);UnFence(\"Apply\",2);";
        scriptMap.put("Apply",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(NFunction'Numberize)[NFunction(new'name_IsString, old'name_IsString, arg'list_IsList) <-- [ MacroRulebase(new'name, arg'list); MacroRule(new'name, Length(arg'list), 0,  ListToFunction({NumericList?, arg'list}) )  NFunction'Numberize(ListToFunction({ToAtom(\"@\"), old'name, arg'list})); ];10 # NFunction'Numberize(x_IsNumber) <-- x;20 # NFunction'Numberize(x_Atom?) <-- Undefined;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(a,b)[10 # MatchLinear(var_Atom?,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchLinear(expr);];20 # MatchLinear(_var,_expr) <-- False;10 # MatchLinear(_expr)_(Degree(expr,var)<?2) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,1); b := Coef(expr,0); True;];20 # MatchLinear(_expr) <-- False;UnFence(\"MatchLinear\",1);UnFence(\"MatchLinear\",2);10 # MatchPureSquared(var_Atom?,_sign2,_sign0,expr_CanBeUni(var)) <--[ Bind(expr,MakeUni(expr,var)); MatchPureSquared(expr,sign2,sign0);];20 # MatchPureSquared(_var,_sign2,_sign0,_expr) <-- False;10 # MatchPureSquared(_expr,_sign2,_sign0)_(Degree(expr,var)=?2 And? Coef(expr,1) =? 0 And? IsNumber(Coef(expr,0)) And? IsNumber(Coef(expr,2)) And? Coef(expr,0)*sign0 >? 0 And? Coef(expr,2)*sign2 >? 0 ) <--[ Check(IsUniVar(expr), \"Argument\", PipeToString()Echo({\"Incorrect argument \",expr,\" passed to MatchLinear\"})); a := Coef(expr,2); b := Coef(expr,0); True;];20 # MatchPureSquared(_expr,_sign2,_sign0) <-- False;UnFence(\"MatchPureSquared\",3);UnFence(\"MatchPureSquared\",4);Matched'a() := a;Matched'b() := b;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "(Assert(_error'class, _error'object) _predicate) <--[ CheckErrorTableau(); If(Equal?(predicate, True),  True, [  DestructiveAppend(GetErrorTableau(), {error'class, error'object}); False; ] );];(Assert(_error'class) _predicate) <-- Assert(error'class, True) predicate;(Assert() _predicate) <-- Assert(\"generic\", True) predicate;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Find\",{list,element})[ Local(result,count); Bind(result, -1); Bind(count, 1); While(And?(result<?0, Not?(Equal?(list, {})))) [ If(Equal?(First(list), element), Bind(result, count) ); Bind(list,Rest(list)); Bind(count,AddN(count,1)); ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FuncList(expr_Atom?) <-- {};20 # FuncList(expr_Function?) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, Apply(\"Concat\", MapSingle(\"FuncList\", Rest(FunctionToList(expr))) ) ));10 # FuncList(expr_Atom?, look'list_IsList) <-- {};20 # FuncList(expr_Function?, look'list_IsList)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {ToAtom(Type(expr))};30 # FuncList(expr_Function?, look'list_IsList) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, [   Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, FuncList(item, look'list)); result; ] ));HoldArgumentNumber(\"FuncList\", 1, 1);HoldArgumentNumber(\"FuncList\", 2, 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PrintList(list_IsList) <-- PrintList(list, \", \");10 # PrintList({}, padding_IsString) <-- \"\";20 # PrintList(list_IsList, padding_IsString) <-- PipeToString() [ Local(i); ForEach(i, list) [ If(Not?(Equal?(i, First(list))), WriteString(padding)); If (IsString(i), WriteString(i), If(IsList(i), WriteString(\"{\" : PrintList(i, padding) : \"}\"), Write(i))); ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # VarListSome({}, _look'list) <-- {};10 # VarListSome(expr_Variable?, _look'list) <-- {expr};15 # VarListSome(expr_Atom?, _look'list) <-- {};20 # VarListSome(expr_Function?, look'list_IsList)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {expr};30 # VarListSome(expr_Function?, look'list_IsList) <--RemoveDuplicates( [  Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, VarListSome(item, look'list)); result; ]);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # IsFreeOf({},_expr) <-- True;2 # IsFreeOf(var_IsList, _expr) <-- And?(IsFreeOf(First(var),expr), IsFreeOf(Rest(var),expr));4 # IsFreeOf(_var,{}) <-- True;5 # IsFreeOf(_var,expr_IsList) <-- And?(IsFreeOf(var,First(expr)), IsFreeOf(var,Rest(expr)));10 # IsFreeOf(_expr,_expr) <-- False;11 # IsFreeOf(_var,expr_Function?) <-- IsFreeOf(var,Rest(FunctionToList(expr)));12 # IsFreeOf(_var,_expr) <-- True;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"MakeVector\",{vec,dimension});Rule(\"MakeVector\",2,1,True)[ Local(res,i); res:={}; i:=1; Bind(dimension,AddN(dimension,1)); While(LessThan?(i,dimension)) [ DestructiveInsert(res,1,ToAtom(ConcatStrings(ToString(vec),ToString(i)))); Bind(i,AddN(i,1)); ]; DestructiveReverse(res);];";
        scriptMap.put("MakeVector",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"<--\",{leftOperand,rightOperand});Rule(\"<--\",2,1,Equal?(Type(leftOperand),\"#\"))[ DefinePattern(leftOperand[2],rightOperand,leftOperand[1],True);];Rule(\"<--\",2,2,Function?(leftOperand))[ DefinePattern(leftOperand,rightOperand,0,True);];HoldArgument(\"<--\",leftOperand);HoldArgument(\"<--\",rightOperand);";
        scriptMap.put("<--",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "OptionsListToHash(list) :=[ Local(item, result); result := {}; ForEach(item, list) If( Function?(item) And? (Type(item) =? \"->\" ) And? Atom?(item[1]), result[ToString(item[1])] := If( Atom?(item[2]) And? Not? IsNumber(item[2]) And? Not? IsString(item[2]), ToString(item[2]), item[2] ), Echo({\"OptionsListToHash: Error: item \", item, \" is not of the format a -> b.\"}) );  result;];HoldArgumentNumber(\"OptionsListToHash\", 1, 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function (\"Boolean?\", {x}) (x=?True) Or? (x=?False) Or? Function?(x) And? Contains({\"=?\", \">?\", \"<?\", \">=?\", \"<=?\", \"!=?\", \"And?\", \"Not?\", \"Or?\"}, Type(x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasExpressionSome?(_expr, _atom, _look'list) _ Equal?(expr, atom) <-- True;15 # HasExpressionSome?(expr_Atom?, _atom, _look'list) <-- Equal?(expr, atom);19 # HasExpressionSome?({}, _atom, _look'list) <-- False;20 # HasExpressionSome?(expr_IsList, _atom, _look'list) <-- HasExpressionSome?(First(expr), atom, look'list) Or? HasExpressionSome?(Rest(expr), atom, look'list);25 # HasExpressionSome?(expr_Function?, _atom, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- False;30 # HasExpressionSome?(expr_Function?, _atom, _look'list) <-- HasExpressionSome?(Rest(FunctionToList(expr)), atom, look'list);";
        scriptMap.put("HasExpressionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasExpression?(_expr, _atom) _ Equal?(expr, atom) <-- True;15 # HasExpression?(expr_Atom?, _atom) <-- Equal?(expr, atom);19 # HasExpression?({}, _atom) <-- False;20 # HasExpression?(expr_IsList, _atom) <-- HasExpression?(First(expr), atom) Or? HasExpression?(Rest(expr), atom);30 # HasExpression?(expr_Function?, _atom) <-- HasExpression?(Rest(FunctionToList(expr)), atom);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFunctionSome?(_expr, string_IsString, _look'list) <-- HasFunctionSome?(expr, ToAtom(string), look'list);10 # HasFunctionSome?(expr_Atom?, atom_Atom?, _look'list) <-- False;15 # HasFunctionSome?(expr_Function?, atom_Atom?, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- Equal?(First(FunctionToList(expr)), atom);20 # HasFunctionSome?(expr_Function?, atom_Atom?, _look'list) <-- Equal?(First(FunctionToList(expr)), atom) Or? ListHasFunctionSome?(Rest(FunctionToList(expr)), atom, look'list);";
        scriptMap.put("HasFunctionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFunction?(_expr, string_IsString) <-- HasFunction?(expr, ToAtom(string));10 # HasFunction?(expr_Atom?, atom_Atom?) <-- False;20 # HasFunction?(expr_Function?, atom_Atom?) <-- Equal?(First(FunctionToList(expr)), atom) Or? ListHasFunction?(Rest(FunctionToList(expr)), atom);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunctionSome?({}, _atom, _look'list) <-- False;20 # ListHasFunctionSome?(expr_IsList, atom_Atom?, _look'list) <-- HasFunctionSome?(First(expr), atom, look'list) Or? ListHasFunctionSome?(Rest(expr), atom, look'list);";
        scriptMap.put("ListHasFunctionSome?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunction?({}, _atom) <-- False;20 # ListHasFunction?(expr_IsList, atom_Atom?) <-- HasFunction?(First(expr), atom) Or? ListHasFunction?(Rest(expr), atom);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Polynomial?(expr_Function?) <-- [ Local(x,vars); vars := VarList(expr); If(Length(vars)>?1,vars:=HeapSort(vars,\"GreaterThan?\"));  x := vars[1]; Polynomial?(expr,x);];15 # Polynomial?(_expr) <-- False;10 # Polynomial?(_expr,_var)_(CanBeUni(var,expr)) <-- True;15 # Polynomial?(_expr,_var) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SumOfTerms?(_var,expr_IsFreeOf(var)) <-- False;12 # SumOfTerms?(_var,expr_Atom?()) <-- False;14 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"+\") Or? expr[1]=?ToAtom(\"-\")) <-- True;16 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"*\")) <-- Or?(IsFreeOf(var,expr[2]),IsFreeOf(var,expr[3]));18 # SumOfTerms?(_var,expr_IsList())_(expr[1]=?ToAtom(\"/\")) <-- IsFreeOf(var,expr[3]);20 # SumOfTerms?(_var,expr_IsList()) <-- False;22 # SumOfTerms?(_var,_expr) <-- SumOfTerms?(var,FunctionToList(expr));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Variable?(_expr) <-- (Atom?(expr) And? Not?(expr=?Infinity) And? Not?(expr=? -Infinity) And? Not?(expr=?Undefined) And? Not?(IsNumber(N(Eval(expr)))));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"MacroSubstitute\",{body,predicate,change})[ `MacroSubstitute((Hold(@body)));];HoldArgument(\"MacroSubstitute\",predicate);HoldArgument(\"MacroSubstitute\",change);UnFence(\"MacroSubstitute\",3);Rulebase(\"MacroSubstitute\",{body});UnFence(\"MacroSubstitute\",1);Rule(\"MacroSubstitute\",1,1,`ApplyFast(predicate,{Hold(Hold(@body))}) =? True)[ `ApplyFast(change,{Hold(Hold(@body))});];Rule(\"MacroSubstitute\",1,2,`Function?(Hold(@body)))[ `ApplyFast(\"MacroMapArgs\",{Hold(Hold(@body)),\"MacroSubstitute\"});];Rule(\"MacroSubstitute\",1,3,True)[ `Hold(@body);];";
        scriptMap.put("MacroSubstitute",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Substitute\",{body,predicate,change})[ Substitute(body);];HoldArgument(\"Substitute\",predicate);HoldArgument(\"Substitute\",change);UnFence(\"Substitute\",3);Rulebase(\"Substitute\",{body});UnFence(\"Substitute\",1);Rule(\"Substitute\",1,1,Apply(predicate,{body}) =? True)[ Apply(change,{body});];Rule(\"Substitute\",1,2,Function?(body))[ Apply(\"MapArgs\",{body,\"Substitute\"});];Rule(\"Substitute\",1,3,True) body;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VerifySolve(_e1, _e2) <--If (VerifySolve'Equal(Eval(e1), Eval(e2)),  True, [  WriteString(\"******************\"); NewLine(); ShowLine(); NewLine(); Write(e1); NewLine(); WriteString(\" evaluates to \"); NewLine(); Write(Eval(e1)); NewLine(); WriteString(\" which differs from \"); NewLine(); Write(e2); NewLine(); WriteString(\"******************\"); NewLine(); False;  ]); HoldArgumentNumber(\"VerifySolve\", 2, 1);10 # VerifySolve'Equal({}, {}) <-- True;20 # VerifySolve'Equal({}, e2_IsList) <-- False;30 # VerifySolve'Equal(e1_IsList, e2_IsList) <--[ Local(i, found); found := False; i := 0; While(i <? Length(e2) And? Not? found) [ i++; found := VerifySolve'Equal(First(e1), e2[i]); ]; If (found, VerifySolve'Equal(Rest(e1), Delete(e2, i)), False);];40 # VerifySolve'Equal(_l1 == _r1, _l2 == _r2) <-- Equal?(l1,l2) And? Simplify(r1-r2)=?0;50 # VerifySolve'Equal(_e1, _e2) <-- Simplify(e1-e2) =? 0;";
