...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasExpr(_expr, _atom) _ IsEqual(expr, atom) <-- True;15 # HasExpr(expr_IsAtom, _atom) <-- IsEqual(expr, atom);19 # HasExpr({}, _atom) <-- False;20 # HasExpr(expr_IsList, _atom) <-- HasExpr(First(expr), atom) Or? HasExpr(Rest(expr), atom);30 # HasExpr(expr_IsFunction, _atom) <-- HasExpr(Rest(FunctionToList(expr)), atom);";
        scriptMap.put("HasExpr",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasExprArith(expr, atom) := HasExprSome(expr, atom, {ToAtom(\"+\"), ToAtom(\"-\"), *, /});";
        scriptMap.put("HasExprArith",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasExprSome(_expr, _atom, _look'list) _ IsEqual(expr, atom) <-- True;15 # HasExprSome(expr_IsAtom, _atom, _look'list) <-- IsEqual(expr, atom);19 # HasExprSome({}, _atom, _look'list) <-- False;20 # HasExprSome(expr_IsList, _atom, _look'list) <-- HasExprSome(First(expr), atom, look'list) Or? HasExprSome(Rest(expr), atom, look'list);25 # HasExprSome(expr_IsFunction, _atom, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- False;30 # HasExprSome(expr_IsFunction, _atom, _look'list) <-- HasExprSome(Rest(FunctionToList(expr)), atom, look'list);";
        scriptMap.put("HasExprSome",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFunc(_expr, string_IsString) <-- HasFunc(expr, ToAtom(string));10 # HasFunc(expr_IsAtom, atom_IsAtom) <-- False;20 # HasFunc(expr_IsFunction, atom_IsAtom) <-- IsEqual(First(FunctionToList(expr)), atom) Or? ListHasFunc(Rest(FunctionToList(expr)), atom);";
        scriptMap.put("HasFunc",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HasFuncArith(expr, atom) := HasFuncSome(expr, atom, {ToAtom(\"+\"), ToAtom(\"-\"), *, /});";
        scriptMap.put("HasFuncArith",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # HasFuncSome(_expr, string_IsString, _look'list) <-- HasFuncSome(expr, ToAtom(string), look'list);10 # HasFuncSome(expr_IsAtom, atom_IsAtom, _look'list) <-- False;15 # HasFuncSome(expr_IsFunction, atom_IsAtom, _look'list)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- IsEqual(First(FunctionToList(expr)), atom);20 # HasFuncSome(expr_IsFunction, atom_IsAtom, _look'list) <-- IsEqual(First(FunctionToList(expr)), atom) Or? ListHasFuncSome(Rest(FunctionToList(expr)), atom, look'list);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)_(Type(expr)=?\"UniVariate\") <-- False;10 # CanBeMonomial(_expr)<--Not? (HasFunc(expr,ToAtom(\"+\")) Or? HasFunc(expr,ToAtom(\"-\")));10 # IsMonomial(expr_CanBeMonomial) <-- [ Local(r); If( IsRationalFunction(expr), r := (VarList(Denominator(expr)) =? {}), r := True );];15 # IsMonomial(_expr) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunc({}, _atom) <-- False;20 # ListHasFunc(expr_IsList, atom_IsAtom) <-- HasFunc(First(expr), atom) Or? ListHasFunc(Rest(expr), atom);";
        scriptMap.put("ListHasFunc",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFuncSome({}, _atom, _look'list) <-- False;20 # ListHasFuncSome(expr_IsList, atom_IsAtom, _look'list) <-- HasFuncSome(First(expr), atom, look'list) Or? ListHasFuncSome(Rest(expr), atom, look'list);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # ExpandFrac(expr_IsList) <-- MapSingle(\"ExpandFrac\", expr);10 # ExpandFrac(_expr)_Not?(HasFuncSome(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- expr;15 # ExpandFrac(a_IsRationalOrNumber) <-- a;20 # ExpandFrac(_expr) <-- ExpandFrac'combine(GetNumerDenom(expr));ExpandFrac'combine({_a, _b}) <-- a/b;";
