...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(hash, key, element, hash'expr)[10 # AssocDelete(hash_List?, element_List?) <--[ Local(index); index := Find(hash, element); If( index >? 0, DestructiveDelete(hash, index) ); index>?0; ];20 # AssocDelete(hash_List?, key_String?) <--[ AssocDelete(hash, Builtin'Assoc(key, hash));];30 # AssocDelete(hash_List?, Empty) <-- False;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\":\",{head,tail});Rule(\":\",2,20,List?(head) And? Not? List?(tail) ) Concat(head,{tail});Rule(\":\",2,30,List?(tail) ) Concat({head},tail);Rule(\":\",2,10,String?(tail) And? String?(head)) ConcatStrings(head,tail);UnFence(\":\",2);";
        scriptMap.put(":",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(ErrorTableau) [  Bind(ErrorTableau, {}); GetErrorTableau() := ErrorTableau; ClearErrors() <-- Bind(ErrorTableau, {});  CheckErrorTableau() <-- If( Not? List?(ErrorTableau), Bind(ErrorTableau, {{\"general\", \"corrupted ErrorTableau\"}}) );]; GetError(error'class_String?) <--[ Local(error); error := GetErrorTableau()[error'class]; If( error !=? Empty, error, False );];ClearError(error'class_String?) <-- AssocDelete(GetErrorTableau(), error'class);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsError() <--[ CheckErrorTableau(); Length(GetErrorTableau())>?0;];IsError(error'class_String?) <--[ CheckErrorTableau(); GetErrorTableau()[error'class] !=? Empty;];";
