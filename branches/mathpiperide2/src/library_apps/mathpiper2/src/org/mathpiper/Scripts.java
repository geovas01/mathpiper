...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(hash, key, element, hash'expr)[10 # AssocDelete(hash_List?, element_List?) <--[ Local(index); index := Find(hash, element); If( index >? 0, DestructiveDelete(hash, index) ); index>?0; ];20 # AssocDelete(hash_List?, key_IsString) <--[ AssocDelete(hash, Builtin'Assoc(key, hash));];30 # AssocDelete(hash_List?, Empty) <-- False;]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Apply(_applyoper,_applyargs) _ (Or?(IsString(applyoper), List?(applyoper))) <-- ApplyFast(applyoper,applyargs);20 # Apply(applyoper_Atom?,_applyargs) <-- ApplyFast(ToString(applyoper),applyargs);30 # Apply(Lambda(_args,_body),_applyargs) <-- `ApplyFast(Hold({@args,@body}),applyargs);UnFence(\"Apply\",2);";
        scriptMap.put("Apply",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(NFunction'Numberize)[NFunction(new'name_IsString, old'name_IsString, arg'list_List?) <-- [ MacroRulebase(new'name, arg'list); MacroRule(new'name, Length(arg'list), 0,  ListToFunction({NumericList?, arg'list}) )  NFunction'Numberize(ListToFunction({ToAtom(\"@\"), old'name, arg'list})); ];10 # NFunction'Numberize(x_IsNumber) <-- x;20 # NFunction'Numberize(x_Atom?) <-- Undefined;]; ";
        scriptMap.put("NFunction",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"@\",{func,arg});Rule(\"@\",2,1,List?(arg)) Apply(func,arg);Rule(\"@\",2,2,True ) Apply(func,{arg});";
        scriptMap.put("@",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\":\",{head,tail});Rule(\":\",2,20,List?(head) And? Not? List?(tail) ) Concat(head,{tail});Rule(\":\",2,30,List?(tail) ) Concat({head},tail);Rule(\":\",2,10,IsString(tail) And? IsString(head)) ConcatStrings(head,tail);UnFence(\":\",2);";
        scriptMap.put(":",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Drop\", {lst, range});Rule(\"Drop\", 2, 1, List?(range)) Concat(Take(lst,range[1]-1), Drop(lst, range[2]));Rule(\"Drop\", 2, 2, range >=? 0) If( range =? 0 Or? lst =? {}, lst, Drop( Rest(lst), range-1 ));Rule(\"Drop\", 2, 2, range <? 0) Take( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FuncList(expr_Atom?) <-- {};20 # FuncList(expr_Function?) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, Apply(\"Concat\", MapSingle(\"FuncList\", Rest(FunctionToList(expr))) ) ));10 # FuncList(expr_Atom?, look'list_List?) <-- {};20 # FuncList(expr_Function?, look'list_List?)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {ToAtom(Type(expr))};30 # FuncList(expr_Function?, look'list_List?) <--RemoveDuplicates( Concat( {First(FunctionToList(expr))}, [   Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, FuncList(item, look'list)); result; ] ));HoldArgumentNumber(\"FuncList\", 1, 1);HoldArgumentNumber(\"FuncList\", 2, 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PrintList(list_List?) <-- PrintList(list, \", \");10 # PrintList({}, padding_IsString) <-- \"\";20 # PrintList(list_List?, padding_IsString) <-- PipeToString() [ Local(i); ForEach(i, list) [ If(Not?(Equal?(i, First(list))), WriteString(padding)); If (IsString(i), WriteString(i), If(List?(i), WriteString(\"{\" : PrintList(i, padding) : \"}\"), Write(i))); ];];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"Take\", {lst, range});Rule(\"Take\", 2, 1, List?(range)) Take( Drop(lst, range[1] -1), range[2] - range[1] + 1);Rule(\"Take\", 2, 2, range >=? 0) If( Length(lst)=?0 Or? range=?0, {}, Concat({First(lst)}, Take(Rest(lst), range-1)));Rule(\"Take\", 2, 2, range <? 0) Drop( lst, Length(lst) + range );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # VarListSome({}, _look'list) <-- {};10 # VarListSome(expr_Variable?, _look'list) <-- {expr};15 # VarListSome(expr_Atom?, _look'list) <-- {};20 # VarListSome(expr_Function?, look'list_List?)_(Not? Contains(look'list, ToAtom(Type(expr)))) <-- {expr};30 # VarListSome(expr_Function?, look'list_List?) <--RemoveDuplicates( [  Local(item, result); result := {}; ForEach(item, expr) result := Concat(result, VarListSome(item, look'list)); result; ]);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # (list_List? AddTo _rest) <--[ Local(res); res:={}; ForEach(item,list) [ res := Concat(res,item AddTo rest); ]; res;];30 # (_a'item AddTo list_List?) <--[ MapSingle({{orig},a'item And? orig},list);];40 # (_a'item AddTo _b) <-- a'item And? b;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # IsFreeOf({},_expr) <-- True;2 # IsFreeOf(var_List?, _expr) <-- And?(IsFreeOf(First(var),expr), IsFreeOf(Rest(var),expr));4 # IsFreeOf(_var,{}) <-- True;5 # IsFreeOf(_var,expr_List?) <-- And?(IsFreeOf(var,First(expr)), IsFreeOf(var,Rest(expr)));10 # IsFreeOf(_expr,_expr) <-- False;11 # IsFreeOf(_var,expr_Function?) <-- IsFreeOf(var,Rest(FunctionToList(expr)));12 # IsFreeOf(_var,_expr) <-- True;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsCoprime(list_List?) <-- (Lcm(list) =? Product(list));10 # IsCoprime(n_Integer?,m_Integer?) <-- (Gcd(n,m) =? 1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Rationalize(aNumber_List?) <-- Rationalize /@ aNumber;20 # Rationalize( _aNumber ) <--[ Local(result,n,d); result:=Substitute(aNumber,{{x},IsNumber(x) And? Not?(Integer?(x))},\"RationalizeNumber\"); If(InVerboseMode(),Tell(\"\",result)); If(Length(VarList(aNumber))=?0, [ n:=Numerator(result); If(Type(n)=?\"Numerator\",n:=result); d:=Denominator(result); If(Type(d)=?\"Denominator\",d:=1); result := n*(1/d); ] ); result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IsGaussianInteger(x_List?) <-- False;10 # IsGaussianInteger(x_IsComplex) <-- (Integer?(Re(x)) And? Integer?(Im(x)));15 # IsGaussianInteger(_x) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WriteDataItem(tuple_List?, _options'hash) <--[ Local(item); If(  NumericList?(tuple), ForEach(item,tuple) [ Write(item); Space(); ] ); NewLine();];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # AllSatisfy?(pred_IsString,lst_List?) <-- Apply(\"And?\",(MapSingle(pred,lst)));20 # AllSatisfy?(_pred,_lst) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "19 # ListHasFunctionSome?({}, _atom, _look'list) <-- False;20 # ListHasFunctionSome?(expr_List?, atom_Atom?, _look'list) <-- HasFunctionSome?(First(expr), atom, look'list) Or? ListHasFunctionSome?(Rest(expr), atom, look'list);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # NoneSatisfy?(pred_IsString,lst_List?) <-- Not? Apply(\"Or?\",(MapSingle(pred,lst)));20 # NoneSatisfy?(_pred,_lst) <-- True;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "NumericList?(_arg'list) <-- List?(arg'list) And? (\"And?\" @ (MapSingle(Hold({{x},IsNumber(N(Eval(x)))}), arg'list)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SumOfTerms?(_var,expr_IsFreeOf(var)) <-- False;12 # SumOfTerms?(_var,expr_Atom?()) <-- False;14 # SumOfTerms?(_var,expr_List?())_(expr[1]=?ToAtom(\"+\") Or? expr[1]=?ToAtom(\"-\")) <-- True;16 # SumOfTerms?(_var,expr_List?())_(expr[1]=?ToAtom(\"*\")) <-- Or?(IsFreeOf(var,expr[2]),IsFreeOf(var,expr[3]));18 # SumOfTerms?(_var,expr_List?())_(expr[1]=?ToAtom(\"/\")) <-- IsFreeOf(var,expr[3]);20 # SumOfTerms?(_var,expr_List?()) <-- False;22 # SumOfTerms?(_var,_expr) <-- SumOfTerms?(var,FunctionToList(expr));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(p,x)[Function(\"IsScalar\",{x}) Not?(List?(x));Function(\"IsVector\",{x}) If(List?(x), Length(Select(x, List?))=?0, False);Function(\"IsVector\",{p,x})[ If(List?(x), [ Local(i,n,result); n:=Length(x); i:=1; result:=True; While(i<=?n And? result) [ result:=Apply(p,{x[i]}); i++; ]; result; ], False);];Function(\"IsMatrix\",{x})If(List?(x) And? Length(x)>?0,[ Local(n); n:=Length(x); If(Length(Select(x, IsVector))=?n, MapSingle(Length,x)=?Length(x[1])+ZeroVector(n), False);],False);Function(\"IsMatrix\",{p,x})If(IsMatrix(x),[ Local(i,j,m,n,result); m:=Length(x); n:=Length(x[1]); i:=1; result:=True; While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:=Apply(p,{x[i][j]}); j++; ]; i++; ]; result;],False);Function(\"IsSquareMatrix\",{x}) IsMatrix(x) And? Length(x)=?Length(x[1]);Function(\"IsSquareMatrix\",{p,x}) IsMatrix(p,x) And? Length(x)=?Length(x[1]);]; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsPoint(p) := If(List?(p) And? (Length(p) =? 2 Or? Length(p) =? 3),True,False);";
        scriptMap.put("IsPoint",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsSegment(list_List?) <--[ If(List?(list[1]) And? Length(list[1])=?2 And? List?(list[2]) And? Length(list[2])=?2,True,False);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Slope(PointA_IsPoint,PointB_IsPoint) <--[ Local(x1,x2,y1,y2,slope); x1 := PointA[1]; x2 := PointB[1]; y1 := PointA[2]; y2 := PointB[2];  slope := (y2 - y1)/(x2 - x1);];10 # Slope(segment_List?)_(Length(segment) =? 2 And? Length(segment[1]) =? 2 And? Length(segment[2]) =? 2) <--[ Local(x1,x2,y1,y2,slope);  x1 := segment[1][1];  x2 := segment[2][1];    y1 := segment[1][2];  y2 := segment[2][2];  slope := (y2 - y1)/(x2 - x1);];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ListToString(list_List?)_(Length(list) =? 0) <-- \"\";20 # ListToString(list_List?) <--[ Local(resultString, character);  resultString := \"\";  ForEach(element, list) [ If(IsString(element), character := element, character := ToString(element));  resultString := resultString : character; ];  resultString;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # ExpandFrac(expr_List?) <-- MapSingle(\"ExpandFrac\", expr);10 # ExpandFrac(_expr)_Not?(HasFunctionSome?(expr, \"/\", {ToAtom(\"+\"), ToAtom(\"-\"), *, /, ^})) <-- expr;15 # ExpandFrac(a_RationalOrNumber?) <-- a;20 # ExpandFrac(_expr) <-- ExpandFrac'combine(GetNumerDenom(expr));ExpandFrac'combine({_a, _b}) <-- a/b;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CheckSolution( _expr, _var, solution_List? )_(Not? IsFreeOf(var,expr)) <-- [ Local(expr0,result,s,r); If( Equation?(expr), Bind(expr0,EquationLeft(expr)-EquationRight(expr)), Bind(expr0,expr) ); result := {}; ForEach(s,solution)  [ r := ( expr0 Where s ); If(r=?0,Push(result,s)); ]; Reverse(result); ];  20 # CheckSolution( _expr, _var, _solution ) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcCosh(_x)_(InNumericMode() And? (IsNumber(x) Or? Type(x) =? \"Complex\")) <-- N(Eval( Ln(x+Sqrt(x^2-1)) ));200 # ArcCosh(Infinity) <-- Infinity;200 # ArcCosh(-Infinity) <-- Infinity+I*Pi/2;200 # ArcCosh(Undefined) <-- Undefined;ArcCosh(xlist_List?) <-- MapSingle(\"ArcCosh\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Exp(x_IsNumber)_InNumericMode() <-- ExpNum(x);4 # Exp(Ln(_x)) <-- x;110 # Exp(Complex(_r,_i)) <-- Exp(r)*(Cos(i) + I*Sin(i));200 # Exp(0) <-- 1;200 # Exp(-Infinity) <-- 0;200 # Exp(Infinity) <-- Infinity;200 # Exp(Undefined) <-- Undefined;Exp(xlist_List?) <-- MapSingle(\"Exp\",xlist);";
        scriptMap.put("Exp",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # Ln(0) <-- -Infinity;2 # Ln(1) <-- 0;2 # Ln(Infinity) <-- Infinity;2 # Ln(Undefined) <-- Undefined;2 # Ln(x_NegativeNumber?)_InNumericMode() <-- Complex(Ln(-x), Pi);3 # Ln(x_IsNumber)_(InNumericMode() And? x>=?1) <-- Internal'LnNum(x);4 # Ln(Exp(_x)) <-- x;3 # Ln(Complex(_r,_i)) <-- Complex(Ln(Abs(Complex(r,i))), Arg(Complex(r,i)));4 # Ln(x_NegativeNumber?) <-- Complex(Ln(-x), Pi);5 # Ln(x_IsNumber)_(InNumericMode() And? x<?1) <-- - Internal'LnNum(DivideN(1, x));Ln(xlist_List?) <-- MapSingle(\"Ln\",xlist);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Round(Infinity) <-- Infinity;5 # Round(-Infinity) <-- -Infinity;5 # Round(Undefined) <-- Undefined;10 # Round(x_RationalOrNumber?) <-- FloorN(N(x+0.5));10 # Round(x_List?) <-- MapSingle(\"Round\",x);20 # Round(x_IsComplex) _ (RationalOrNumber?(Re(x)) And? RationalOrNumber?(Im(x)) ) <-- FloorN(N(Re(x)+0.5)) + FloorN(N(Im(x)+0.5))*I;";
        scriptMap.put("Round",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Sign(n_PositiveNumber?) <-- 1;10 # Sign(n_Zero?) <-- 0;20 # Sign(n_IsNumber) <-- -1;15 # Sign(n_Infinity?)_(n <? 0) <-- -1;15 # Sign(n_Infinity?)_(n >? 0) <-- 1;15 # Sign(n_IsNumber/m_IsNumber) <-- Sign(n)*Sign(m);20 # Sign(n_List?) <-- MapSingle(\"Sign\",n);100 # Sign(_a)^n_Even? <-- 1;100 # Sign(_a)^n_Odd? <-- Sign(a);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# (n_PositiveInteger?)!! _ (n<=?3) <-- n;2# (n_PositiveInteger?)!! <--[ Check(n<=?65535, \"Argument\", \"Double factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); Factorial'double(2+Modulo(n, 2), n);];3# (_n)!! _ (n=? -1 Or? n=?0)<-- 1;2# Factorial'double(_a, _b) _ (b-a>=?6) <-- Factorial'double(a, Quotient(a+b,2)) * Factorial'double(Quotient(a+b,2)+1+Modulo(Quotient(a+b,2)+1-a, 2), b);3# Factorial'double(_a, _b) _ (b-a>=?4) <-- a*(a+2)*(a+4);4# Factorial'double(_a, _b) _ (b-a>=?2) <-- a*(a+2);5# Factorial'double(_a, _b) <-- a;30 # (n_List?)!! <-- MapSingle(\"!!\",n);";
        scriptMap.put("!!",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # 0! <-- 1;10 # (Infinity)! <-- Infinity;20 # ((n_PositiveInteger?)!) <-- [ Check(n <=? 65535, \"Argument\", \"Factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); MathFac(n);];25 # ((x_Constant?)!)_(FloatIsInt?(x) And? x>?0) <-- (Round(x)!);30 # ((x_IsNumber)!)_InNumericMode() <-- Internal'GammaNum(x+1);40 # (n_List?)! <-- MapSingle(\"!\",n);HalfIntegerFactorial(n_Odd?) _ (n>?0) <-- Sqrt(Pi) * ( n!! / 2^((n+1)/2) );HalfIntegerFactorial(n_Odd?) _ (n<?0) <-- Sqrt(Pi) * ( (-1)^((-n-1)/2)*2^((-n-1)/2) / (-n-2)!! );40 # (n_RationalOrNumber?)! _(Denominator(Rationalize(n))=?2) <-- HalfIntegerFactorial(Numerator(Rationalize(n)));";
        scriptMap.put("!",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VerifySolve(_e1, _e2) <--If (VerifySolve'Equal(Eval(e1), Eval(e2)),  True, [  WriteString(\"******************\"); NewLine(); ShowLine(); NewLine(); Write(e1); NewLine(); WriteString(\" evaluates to \"); NewLine(); Write(Eval(e1)); NewLine(); WriteString(\" which differs from \"); NewLine(); Write(e2); NewLine(); WriteString(\"******************\"); NewLine(); False;  ]); HoldArgumentNumber(\"VerifySolve\", 2, 1);10 # VerifySolve'Equal({}, {}) <-- True;20 # VerifySolve'Equal({}, e2_List?) <-- False;30 # VerifySolve'Equal(e1_List?, e2_List?) <--[ Local(i, found); found := False; i := 0; While(i <? Length(e2) And? Not? found) [ i++; found := VerifySolve'Equal(First(e1), e2[i]); ]; If (found, VerifySolve'Equal(Rest(e1), Delete(e2, i)), False);];40 # VerifySolve'Equal(_l1 == _r1, _l2 == _r2) <-- Equal?(l1,l2) And? Simplify(r1-r2)=?0;50 # VerifySolve'Equal(_e1, _e2) <-- Simplify(e1-e2) =? 0;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Coef(uv_IsUniVar,order_List?) <--[ Local(result); result:={}; ForEach(item,order) [ DestructiveAppend(result,Coef(uv,item)); ]; result;];10 # Coef(uv_IsUniVar,order_Integer?)_(order<?uv[2]) <-- 0;10 # Coef(uv_IsUniVar,order_Integer?)_(order>=?uv[2]+Length(uv[3])) <-- 0;20 # Coef(uv_IsUniVar,order_Integer?) <-- uv[3][(order-uv[2])+1];30 # Coef(uv_CanBeUni,_order)_(Integer?(order) Or? List?(order)) <-- Coef(MakeUni(uv),order);Function(\"Coef\",{expression,var,order}) NormalForm(Coef(MakeUni(expression,var),order));";
