...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Arg(Complex(Cos(_x),Sin(_x))) <-- x;10 # Arg(x_IsZero) <-- Undefined;15 # Arg(x_IsPositiveReal) <-- 0;15 # Arg(x_IsNegativeReal) <-- Pi;20 # Arg(Complex(r_IsZero,i_IsConstant)) <-- Sign(i)*Pi/2;30 # Arg(Complex(r_IsPositiveReal,i_IsConstant)) <-- ArcTan(i/r);40 # Arg(Complex(r_IsNegativeReal,i_IsPositiveReal)) <-- Pi+ArcTan(i/r);50 # Arg(Complex(r_IsNegativeReal,i_IsNegativeReal)) <-- ArcTan(i/r)-Pi;";
        scriptMap.put("Arg",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # Complex(_r,i_IsZero) <-- r;2 # Complex(Complex(_r1,_i1),_i2) <-- Complex(r1,i1+i2);2 # Complex(_r1,Complex(_r2,_i2)) <-- Complex(r1-i2,r2);6 # Complex(Undefined,_x) <-- Undefined;6 # Complex(_x,Undefined) <-- Undefined;110 # Complex(_r1,_i1) + Complex(_r2,_i2) <-- Complex(r1+r2,i1+i2);300 # Complex(_r,_i) + x_IsConstant <-- Complex(r+x,i);300 # x_IsConstant + Complex(_r,_i) <-- Complex(r+x,i);110 # - Complex(_r,_i) <-- Complex(-r,-i);300 # Complex(_r,_i) - x_IsConstant <-- Complex(r-x,i);300 # x_IsConstant - Complex(_r,_i) <-- Complex((-r)+x,-i);111 # Complex(_r1,_i1) - Complex(_r2,_i2) <-- Complex(r1-r2,i1-i2);110 # Complex(_r1,_i1) * Complex(_r2,_i2) <-- Complex(r1*r2-i1*i2,r1*i2+r2*i1);250 # Complex(r_IsZero,_i) * x_IsInfinity <-- Complex(0,i*x);250 # Complex(_r,i_IsZero) * x_IsInfinity <-- Complex(r*x,0);251 # Complex(_r,_i) * x_IsInfinity <-- Complex(r*x,i*x);250 # x_IsInfinity * Complex(r_IsZero,_i) <-- Complex(0,i*x);250 # x_IsInfinity * Complex(_r,i_IsZero) <-- Complex(r*x,0);251 # x_IsInfinity * Complex(_r,_i) <-- Complex(r*x,i*x);300 # Complex(_r,_i) * y_IsConstant <-- Complex(r*y,i*y);300 # y_IsConstant * Complex(_r,_i) <-- Complex(r*y,i*y);330 # Complex(_r,_i) * (y_IsConstant / _z) <-- (Complex(r*y,i*y))/z;330 # (y_IsConstant / _z) * Complex(_r,_i) <-- (Complex(r*y,i*y))/z;110 # x_IsConstant / Complex(_r,_i) <-- (x*Conjugate(Complex(r,i)))/(r^2+i^2);300 # Complex(_r,_i) / y_IsConstant <-- Complex(r/y,i/y);110 # (_x ^ Complex(_r,_i)) <-- Exp(Complex(r,i)*Ln(x));110 # Sqrt(Complex(_r,_i)) <-- Exp(Ln(Complex(r,i))/2);110 # (Complex(_r,_i) ^ x_IsRationalOrNumber)_(Not?(IsInteger(x))) <-- Exp(x*Ln(Complex(r,i)));123 # Complex(_r, _i) ^ n_IsNegativeInteger <-- 1/Complex(r, i)^(-n);124 # Complex(_r, _i) ^ (p_IsZero) <-- 1; 125 # Complex(_r, _i) ^ n_IsPositiveInteger <--[  Local(result, x); x:=Complex(r,i); result:=1; While(n >? 0) [ if ((n&1) =? 1) [ result := result*x; ]; x := x*x; n := n>>1; ]; result;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # II^n_IsNegativeInteger <-- (-II)^(-n);20 # (II^_n)_(IsEven(n) =? True) <-- (-1)^(n>>1);20 # (II^_n)_(IsOdd(n) =? True) <-- II*(-1)^(n>>1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsBinomials( _x + y_IsFreeOf(x) ) <-- {x+y,1};10 # FactorsBinomials( _x - y_IsFreeOf(x) ) <-- {x-y,1};10 # FactorsBinomials( c_IsConstant * _x + y_IsFreeOf(x) ) <-- {c*x+y,1};10 # FactorsBinomials( c_IsConstant * _x - y_IsFreeOf(x) ) <-- {c*x-y,1};10 # FactorsBinomials( _x^m_IsOdd + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := (y^(1/m)); If(InVerboseMode(),Tell(\" FBinsum1\",{nn,qq})); r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}};];12 # FactorsBinomials( c_IsConstant * _x^m_IsOdd + _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialssum\",{c,x,m,y})); Local(nn,qq, r); nn := (m-1)/2; qq := ((y/c)^(1/m)); If(InVerboseMode(),Tell(\" FBinsum.1b\",{nn,qq})); If( c=?1, r := {{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}}, r := {{c,1},{x+qq,1},{Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1}} ); ];10 # FactorsBinomials( _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{x,m,y})); Local(pp,qq,r,L); pp := m-1; qq := (y^(1/m)); If(IsNumber(y),qq:=GuessRational(N(qq))); If(InVerboseMode(),Tell(\" FBindif.1\",{pp,qq}));  if (m =? 2) [ L := FunctionToList(y); If(And?(L[1]=?ToAtom(\"^\"),L[3]=?2),qq:=L[2]); r := {{x+qq,1},{x-qq,1}}; ] else if (m =? 4) [r := {{x+qq,1},{x-qq,1},{x^2+qq^2,1}};] else if (m =? 6) [r := {{x+qq,1},{x-qq,1},{x^2+x*qq+qq^2,1},{x^2-x*qq+qq^2,1}};] else [r := {{x-qq,1},{Sum(k,0,pp,qq^k*x^(pp-k)),1}};]; r;];12 # xFactorsBinomials( c_IsConstant * _x^m_IsInteger - _y ) <--[ If(InVerboseMode(),Tell(\"FactorsBinomialsdif\",{c,x,m,y})); Local(aa,bb,c0,r); aa := c^(1/m); bb := ((y)^(1/m)); If(IsNumber(y),bb:=GuessRational(N(bb))); If(InVerboseMode(),Tell(\" FBindif.1b\",{aa,bb})); r := FactorsBinomials( (aa*x)^m - bb^m );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Euler(0) <-- 1;10 # Euler(n_IsOdd) <-- 0;10 # Euler(n_IsEven) <-- - Sum(r,0,n/2-1,BinomialCoefficient(n,2*r)*Euler(2*r));10 # Euler(n_IsNonNegativeInteger,_x) <-- Sum(i,0,Round(n/2),BinomialCoefficient(n,2*i)*Euler(2*i)*(x-1/2)^(n-2*i)/2^(2*i));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# NextPseudoPrime(i_IsInteger)_(i<=?1) <-- 2;2# NextPseudoPrime(2) <-- 3;3# NextPseudoPrime(i_IsOdd) <--[  i := i+2; If(Modulo(i,3)=?0, i:=i+2, i);];4# NextPseudoPrime(i_IsEven) <-- NextPseudoPrime(i-1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # JacobiSymbol(_a, 1) <-- 1;15 # JacobiSymbol(0, _b) <-- 0;18 # JacobiSymbol(_a, _b) _ (Gcd(a,b)>?1) <-- 0;20 # JacobiSymbol(_a, b_IsOdd)_(a>=?Abs(b) Or? a<?0) <-- JacobiSymbol(Modulo(a,Abs(b)),Abs(b));30 # JacobiSymbol(a_IsEven, b_IsOdd) <--[ Local(c, s);  {c,s}:=FindPrimeFactorSimple(a, 2);  If(Modulo(s,2)=?1 And? Abs(Modulo(b,8)-4)=?1, -1, 1) * JacobiSymbol(c,b);];40 # JacobiSymbol(a_IsOdd, b_IsOdd) <-- If(Modulo(a,4)=?3 And? Modulo(b,4)=?3, -1, 1) * JacobiSymbol(b,a);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "0 # IsBoolType(True) <-- True;0 # IsBoolType(False) <-- True;1 # IsBoolType(_anythingelse) <-- False;";
        scriptMap.put("IsBoolType",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function (\"IsBoolean\", {x}) (x=?True) Or? (x=?False) Or? IsFunction(x) And? Contains({\"=?\", \">?\", \"<?\", \">=?\", \"<=?\", \"!=?\", \"And?\", \"Not?\", \"Or?\"}, Type(x));";
        scriptMap.put("IsBoolean",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsConstant(_n) <-- (VarList(n) =? {});";
        scriptMap.put("IsConstant",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsDiagonal(A_IsMatrix) <--[ Local(i,j,m,n,result); m := Length(A); n := Length(A[1]); result := (m=?n);   i:=2; While(i<=?m And? result) [ j:=1; While(j<=?n And? result) [ result:= (i=?j Or? A[i][j] =? 0); j++; ]; i++; ]; If(m=?2, [ result := result And? (A=?Transpose(A)); ] ); result;];";
        scriptMap.put("IsDiagonal",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IsEquation(expr_IsAtom) <-- False;12 # IsEquation(_expr) <-- FunctionToList(expr)[1] =? == ;";
        scriptMap.put("IsEquation",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsEven(n) := IsInteger(n) And? ( BitAnd(n,1) =? 0 );";
        scriptMap.put("IsEven",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "IsEvenFunction(f,x):= (f =? Eval(Subst(x,-x)f));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FastModularPower(a_IsPositiveInteger, b_IsPositiveInteger, n_IsPositiveInteger) <--[ Local(p, j, r); p := a; j := b; r := 1; While (j >? 0) [ If (IsOdd(j), r := ModuloN(r*p, n)); p := ModuloN(p*p, n); j := ShiftRight(j, 1); ]; r;];IsStronglyProbablyPrime(b_IsPositiveInteger, n_IsPositiveInteger) <--[ Local(m, q, r, a, flag, i, root); m := n-1; q := m; r := 0; root := 0;  While (IsEven(q)) [ q := ShiftRight(q, 1); r++; ]; a := FastModularPower(b, q, n); flag := (a =? 1 Or? a =? m); i := 1; While (Not?(flag) And? (i <? r)) [ root := a;  a := ModuloN(a*a, n); flag := (a =? m); i++; ]; {root, flag}; ];10 # RabinMillerSmall(1) <-- False;10 # RabinMillerSmall(2) <-- True;20 # RabinMillerSmall(n_IsEven) <-- False;20 # RabinMillerSmall(3) <-- True;30 # RabinMillerSmall(n_IsPositiveInteger) <--[ Local(continue, prime, i, primetable, pseudotable, root); continue := True; prime := True; i := 1; primetable := {2, 3, 5, 7, 11, 13, 17}; pseudotable := {2047, 1373653, 25326001, 3215031751, 2152302898747, 3474749660383, 34155071728321};  While (continue And? prime And? (i <? 8)) [  {root, prime} := IsStronglyProbablyPrime(primetable[i], n);  continue := (n >=? pseudotable[i]); i++; ];  If (continue And? (i =? 8), Overflow, prime);];10 # RabinMillerProbabilistic(1, _p) <-- False;10 # RabinMillerProbabilistic(2, _p) <-- True;20 # RabinMillerProbabilistic(n_IsEven, _p) <-- False;20 # RabinMillerProbabilistic(3, _p) <-- True;30 # RabinMillerProbabilistic(n_IsPositiveInteger, p_IsPositiveInteger) <--[ Local(k, prime, b, roots'of'minus1, root); k := 1+IntLog(IntLog(n,2),4)+p;  b := 1; prime := True; roots'of'minus1 := {0};  While (prime And? k>?0) [ b := NextPseudoPrime(b);  {root, prime} := IsStronglyProbablyPrime(b, n); If(prime, roots'of'minus1 := Union(roots'of'minus1, {root})); If(Length(roots'of'minus1)>?3, prime := False);  If(  InVerboseMode() And? Length(roots'of'minus1)>?3, [  Local(factor); roots'of'minus1 := Difference(roots'of'minus1,{0});  factor := Gcd(n, If( roots'of'minus1[1]+roots'of'minus1[2]=?n, roots'of'minus1[1]+roots'of'minus1[3], roots'of'minus1[1]+roots'of'minus1[2] )); Echo(n, \" = \", factor, \" * \", n/factor); ] ); k--; ]; prime;];RabinMiller(n_IsPositiveInteger) <--[  If( n <? 34155071728321, RabinMillerSmall(n), RabinMillerProbabilistic(n, 40)  );];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CheckSolution( _expr, _var, solution_IsList )_(Not? IsFreeOf(var,expr)) <-- [ Local(expr0,result,s,r); If( IsEquation(expr), Bind(expr0,EquationLeft(expr)-EquationRight(expr)), Bind(expr0,expr) ); result := {}; ForEach(s,solution)  [ r := ( expr0 Where s ); If(r=?0,Push(result,s)); ]; Reverse(result); ];  20 # CheckSolution( _expr, _var, _solution ) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # Gamma(n_IsRationalOrNumber)_(IsPositiveInteger(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_IsConstant)_(InNumericMode()) <-- Internal'GammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(IsInteger(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_IsRationalOrNumber)_(IsPositiveInteger(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_IsConstant)_(InNumericMode()) <-- Internal'LnGammaNum(N(Eval(x)));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zeta(1) <-- Infinity;10 # Zeta(0) <-- -1/2; 10 # Zeta(3)_InNumericMode() <-- Zeta3(); 10 # Zeta(n_IsEven)_(n>?0) <-- Pi^n*(2^(n-1)/n! *Abs(Bernoulli(n)));10 # Zeta(n_IsInteger)_(n<?0) <-- -Bernoulli(-n+1)/(-n+1);11 # Zeta(n_IsInfinity) <-- 1;20 # Zeta(s_IsConstant)_(InNumericMode()) <-- Internal'ZetaNum(N(Eval(s)));";
        scriptMap.put("Zeta",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Bernoulli1(n_IsEven)_(n>=?2) <-- [ Local(B, prec); prec := BuiltinPrecisionGet();   BuiltinPrecisionSet(10); BuiltinPrecisionSet( Ceil(N((1/2*Ln(8*Pi*n)-n+n*Ln(n/2/Pi))/Ln(10)))+3  ); If (InVerboseMode(), Echo({\"Bernoulli: using zeta funcion, precision \", BuiltinPrecisionSet(), \", n = \", n})); B := Floor(N(  If(  n>?250,  Internal'ZetaNum2(n, n/17+1),    Internal'ZetaNum1(n, n/17+1)  ) *N(2*n! /(2*Pi)^n)))  * (2*Modulo(n/2,2)-1)  + BernoulliFracPart(n);  BuiltinPrecisionSet(prec);  B;];";
        scriptMap.put("Bernoulli1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BernoulliFracPart(n_IsEven)_(n>=?2) <-- [ Local(p, sum);  sum := 1/2+1/3;  If(IsPrime(n+1), sum := sum+1/(n+1)); If(IsPrime(n/2+1), sum := sum+1/(n/2+1));   For(p:=5, p<=?n/3+1, p:=NextPrime(p)) If(Modulo(n, p-1)=?0, sum := sum + 1/p);   Quotient(Numerator(sum), Denominator(sum)) - sum + Modulo(n/2,2);  ];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Internal'BernoulliArray1(n_IsEven) _ (n>=?2) <--[ Local(C, f, k, j, denom, sum); C := ArrayCreate(n+1, 0); f := ArrayCreate(n/2, 0); C[1] := 1; C[2] := -1/2; C[3] := 1/12;  f[1] := 2;  For(k:=2, k<=?n/2, k++)  [  f[k] := f[k-1] * (2*k)*(2*k-1);  C[2*k+1] := 1/(1-4^(-k))/2*( [ denom := 4;  sum := 0; For(j:=1, j<=?k-1, j++) [ sum := sum + C[2*(k-j)+1]/denom/f[j];  denom := denom * 4; ]; (2*k-1)/denom/f[k] - sum; ] ); ];  For(k:=1, k<=?n/2, k++) C[2*k+1] := C[2*k+1] * f[k];  C;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistribution(_p, _n)_ (If(IsRationalOrNumber(p),p<?0 Or? p>?1, False) Or? (IsConstant(n) And? Not? IsPositiveInteger(n)) ) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SumTaylorNum0(_x, _nth'term'func, _n'terms) <-- SumTaylorNum0(x, nth'term'func, {}, n'terms);SumTaylorNum1(_x, _nth'term'func, _n'terms) <-- SumTaylorNum1(x, nth'term'func, {}, n'terms);SumTaylorNum(_x, _nth'term'func, _n'terms) <--If( n'terms >=? 30,   SumTaylorNum1(x, nth'term'func, n'terms), SumTaylorNum0(x, nth'term'func, n'terms));SumTaylorNum(_x, _nth'term'func, _next'term'factor, _n'terms) <--If( n'terms >=? 5,  SumTaylorNum1(x, nth'term'func, next'term'factor, n'terms), SumTaylorNum0(x, nth'term'func, next'term'factor, n'terms));1# SumTaylorNum0(_x, _nth'term'func, {}, _n'terms) <--[ Local(sum, k); N([  x:=Eval(x); sum := 0; For(k:=n'terms, k>=?0, k--) sum := AddN(sum*x, nth'term'func @ k); ]); sum;];2# SumTaylorNum0(_x, _nth'term'func, _next'term'factor, _n'terms) <--[ Local(sum, k, term, delta); N([ x:=Eval(x);  If (IsConstant(nth'term'func), term := nth'term'func, term := (nth'term'func @ {0}), ); sum := term;  ]); NonN([ delta := 1; For(k:=1, k<=?n'terms And? delta !=? 0, k++) [ term := MultiplyNum(term, next'term'factor @ {k}, x);  delta := sum; sum := sum + term;  delta := Abs(sum-delta);  ]; ]); sum;];SumTaylorNum0(_x, _nth'term'func, _n'terms) <-- SumTaylorNum0(x, nth'term'func, {}, n'terms);SumTaylorNum1(x, nth'term'func, next'term'factor, n'terms) :=[  Local(sum, rows, cols, rows'tmp, last'power, i, j, x'power, term'tmp); N([  x:=Eval(x);  rows := IntNthRoot(n'terms+1, 2); cols := Quotient(n'terms+rows, rows);  Check(rows>?1 And? cols>?1, \"Argument\", \"SumTaylorNum1: Internal error: number of Taylor sum terms must be at least 4\"); rows'tmp := ArrayCreate(rows, 0); x'power := x ^ rows;      If( next'term'factor =? {}, term'tmp := 1,  If (IsConstant(nth'term'func), term'tmp := nth'term'func, term'tmp := (nth'term'func @ {0}), ) ); ]); NonN([   For(i:=0, i<?cols, i++) [  For(j:=0, j<?rows And? (i<?cols-1 Or? i*rows+j<=?n'terms), j++)  [   If( next'term'factor =? {},  [ rows'tmp[j+1] := rows'tmp[j+1] + MultiplyNum(term'tmp, nth'term'func @ {i*rows+j}); ], [ rows'tmp[j+1] := rows'tmp[j+1] + term'tmp;  term'tmp := MultiplyNum(term'tmp, next'term'factor @ {i*rows+j+1});  ] ); ];  term'tmp := term'tmp*x'power;  ];   For([j:=rows; sum:=0;], j>?0, j--) sum := sum*x + rows'tmp[j]; ]); sum;];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Maximum(l1, l2, l3, ...);10 # Maximum(_l1, _l2, l3_IsList) <-- Maximum(Concat({l1, l2}, l3));20 # Maximum(_l1, _l2, _l3) <-- Maximum({l1, l2, l3});10 # Maximum(l1_IsList,l2_IsList) <-- Map(\"Maximum\",{l1,l2});20 # Maximum(l1_IsRationalOrNumber,l2_IsRationalOrNumber) <-- If(l1>?l2,l1,l2);30 # Maximum(l1_IsConstant,l2_IsConstant) <-- If(N(Eval(l1-l2))>?0,l1,l2);10 # Maximum({}) <-- Undefined;20 # Maximum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Maximum(result,item); result;];30 # Maximum(_x) <-- x;";
        scriptMap.put("Maximum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Minimum(l1, l2, l3, ...);10 # Minimum(_l1, _l2, l3_IsList) <-- Minimum(Concat({l1, l2}, l3));20 # Minimum(_l1, _l2, _l3) <-- Minimum({l1, l2, l3});10 # Minimum(l1_IsList,l2_IsList) <-- Map(\"Minimum\",{l1,l2});20 # Minimum(l1_IsRationalOrNumber,l2_IsRationalOrNumber) <-- If(l1<?l2,l1,l2);30 # Minimum(l1_IsConstant,l2_IsConstant) <-- If(N(Eval(l1-l2))<?0,l1,l2);10 # Minimum({}) <-- Undefined;20 # Minimum(list_IsList) <--[ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Minimum(result,item); result;];30 # Minimum(_x) <-- x;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # 0! <-- 1;10 # (Infinity)! <-- Infinity;20 # ((n_IsPositiveInteger)!) <-- [ Check(n <=? 65535, \"Argument\", \"Factorial: Error: the argument \" : ( PipeToString() Write(n) ) : \" is too large, you may want to avoid exact calculation\"); MathFac(n);];25 # ((x_IsConstant)!)_(FloatIsInt?(x) And? x>?0) <-- (Round(x)!);30 # ((x_IsNumber)!)_InNumericMode() <-- Internal'GammaNum(x+1);40 # (n_IsList)! <-- MapSingle(\"!\",n);HalfIntegerFactorial(n_IsOdd) _ (n>?0) <-- Sqrt(Pi) * ( n!! / 2^((n+1)/2) );HalfIntegerFactorial(n_IsOdd) _ (n<?0) <-- Sqrt(Pi) * ( (-1)^((-n-1)/2)*2^((-n-1)/2) / (-n-2)!! );40 # (n_IsRationalOrNumber)! _(Denominator(Rationalize(n))=?2) <-- HalfIntegerFactorial(Numerator(Rationalize(n)));";
        scriptMap.put("!",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LaplaceTransform(_var1,_var2, _expr ) <-- LapTran(var1,var2,expr);10 # LapTran(_var1,_var2,_x + _y) <-- LapTran(var1,var2,x) + LapTran(var1,var2,y);10 # LapTran(_var1,_var2,_x - _y) <-- LapTran(var1,var2,x) - LapTran(var1,var2,y);10 # LapTran(_var1,_var2, - _y) <-- LapTran(var1,var2,y);10 # LapTran(_var1,_var2, c_IsConstant*_y) <-- c*LapTran(var1,var2,y);10 # LapTran(_var1,_var2, _y*c_IsConstant) <-- c*LapTran(var1,var2,y);10 # LapTran(_var1,_var2, _y/c_IsConstant) <-- LapTran(var1,var2,y)/c;10 # LapTran(_var1,_var2, Exp(c_IsConstant*_var1)*_expr ) <-- LapTran(var1,var2-c,expr);10 # LapTran(_var1,_var2, Exp(-c_IsConstant*_var1)*_expr ) <-- LapTran(var1,var2+c,expr);10 # LapTran(_var1,_var2, _expr*Exp(c_IsConstant*_var1) ) <-- LapTran(var1,var2-c,expr);10 # LapTran(_var1,_var2, _expr*Exp(-c_IsConstant*_var1) ) <-- LapTran(var1,var2+c,expr);10 # LapTran(_var1,_var2, _expr/_var1 ) <-- Integrate(var2,var2,Infinity) LapTran(var1,var2,expr) ;10 # LapTran(_var1,_var2, _var1*_expr ) <-- - Deriv(var2) LapTran(var1,var2,expr);10 # LapTran(_var1,_var2, _var1^(n_IsInteger)*_expr ) <-- (-1)^n * Deriv(var2,n) LapTran(var1,var2,expr);10 # LapTran(_var1,_var2, _expr*_var1 ) <-- - Deriv(var2) LapTran(var1,var2,expr);10 # LapTran(_var1,_var2, _expr*_var1^(n_IsInteger) ) <-- (-1)^n * Deriv(var2,n) LapTran(var1,var2,expr);100 # LapTran(_var1,_var2, _expr ) <-- `Hold(LaplaceTransform(@var1,@var2,@expr));LapTranDef(_in,_out) <--[ Local(i,o);  `(50 # LapTran(_t,_s,@in) <-- @out ); i:=Subst(_t,c_IsPositiveInteger*_t) in; o:=Subst(s,s/c) out;  `(50 # LapTran(_t,_s,@i ) <-- @o/c ); i:=Subst(_t,_t/c_IsPositiveInteger) in; o:=Subst(s,s*c) out;  `(50 # LapTran(_t,_s,@i ) <-- @o*c );];LapTranDef( (_t)^(n_IsConstant), Gamma(n+1)/s^(n+1) );LapTranDef( _t, 1/s^2 );LapTranDef( Sqrt(_t), Sqrt(Pi)/(2*s^(3/2)) );LapTranDef( c_IsFreeOf({t,s}), c/s );LapTranDef( Sin(_t), 1/(s^2+1) );LapTranDef( Cos(_t), s/(s^2+1) );LapTranDef( Sinh(_t), c/(s^2-1) );LapTranDef( Cosh(_t), s/(s^2-1) );LapTranDef( Exp(_t), 1/(s-1) );LapTranDef( BesselJ(n_IsConstant,_t), (Sqrt(s^2+1)-s)^n /Sqrt(s^2+1) );LapTranDef( BesselI(n_IsConstant,_t), (s-Sqrt(s^2+1))^n /Sqrt(s^2-1) );LapTranDef( Ln(_t), -(gamma+Ln(s))/s);LapTranDef( Ln(_t)^2, Pi^2/(6*s)+(gamma+Ln(s))/s );LapTranDef( Erf(_t), Exp(s^2/4)*Erfc(s/2)/s );LapTranDef( Erf(Sqrt(_t)), 1/(Sqrt(s+1)*s) );";
