...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Rulebase(\"YacasPSolve\",{uni});Rule(\"YacasPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 0) {};Rule(\"YacasPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 1) -Coef(uni,0)/Coef(uni,1);Rule(\"YacasPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 2) [ Local(a,b,c,d,q,r); c:=Coef(uni,0); b:=Coef(uni,1); a:=Coef(uni,2); d:=b*b-4*a*c; q:=Sqrt(d)/(2*a);     r:=-b/(2*a); If(InVerboseMode(),[ Tell(\" \",{c,b,a,d}); Tell(\" \",{q,r}); ]); {r+q,r-q}; ];Rule(\"YacasPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 3 ) [ Local(p,q,r,w,ww,a,b); Local(coef0,coef1,coef3,adjust); adjust := (-Coef(uni,2))/(3*Coef(uni,3)); coef3 := Coef(uni,3); coef1 := 2*Coef(uni,2)*adjust+3*Coef(uni,3)*adjust^2+Coef(uni,1); coef0 := Coef(uni,2)*adjust^2+Coef(uni,3)*adjust^3+ adjust*Coef(uni,1)+Coef(uni,0); p:=coef3; q:=coef1/p; r:=coef0/p; w:=Complex(-1/2,Sqrt(3/4)); ww:=Complex(-1/2,-Sqrt(3/4)); a:=(-r/2 + Sqrt(q^3/27+ r*r/4))^(1/3); b:=(-r/2 - Sqrt(q^3/27+ r*r/4))^(1/3); {a+b+adjust,w*a+ww*b+adjust,ww*a+w*b+adjust};];Rule(\"YacasPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 4 )[ Local(coef4,a1,a2,a3,a4,y,y1,z,t,s); coef4:=Coef(uni,4); a1:=Coef(uni,3)/coef4; a2:=Coef(uni,2)/coef4; a3:=Coef(uni,1)/coef4; a4:=Coef(uni,0)/coef4;  y1:=First(YacasPSolve(y^3-a2*y^2+(a1*a3-4*a4)*y+(4*a2*a4-a3^2-a1^2*a4),y)); t := Sqrt(y1^2/4-a4); If(t=?0, s:=Sqrt(y1+a1^2/4-a2), s:=(a1*y1-2*a3)/(4*t)); Concat(YacasPSolve(z^2+(a1/2+s)*z+y1/2+t,z), YacasPSolve(z^2+(a1/2-s)*z+y1/2-t,z));];Function(\"YacasPSolve\",{uni,var}) [ Local(u, factors, f, r, s); u := MakeUni(uni, var); If(Type(u) =? \"UniVariate\" And? (And? @ (Lambda({x}, Number?(x) Or? Rational?(x)) /@ u[3])), [ Local(coeffs); coeffs := Rationalize(u[3]); coeffs := If(Length(coeffs) >? 1,  Lcm(Denominator /@ coeffs) * coeffs,  (Denominator /@ coeffs) * coeffs); DestructiveReplace(u, 3, coeffs); factors := If(Degree(u)>?0,  Factors(NormalForm(u)),  {NormalForm(u), 1}); ], [ factors := {{uni, 1}}; ]); r := {}; ForEach(f, factors) [ s := YacasPSolve(MakeUni(f[1],var)); r := Union(r, If(List?(s), s, {s})); ]; If(Length(r) =? 1, r[1], r);];";
