...
/**
 * The pre parsing phase that converts a stream of tokens into a tree of
 * packets.
 * 
...
class pPreParseStream {
    ...
    /**
     * Performs the next parsing operation, returning one packet at a time.
     * Returns the next packet in the stream.
     * 
     * @return The next <code>pPacket</code> from the stream, or null if there
    ...
    /**
     * Internal method that parses the stream begining with a parenthesis, and
     * returns the packet representing it.
     * 
    ...
    /**
     * Internal method that parses the stream begining with a curley brace, and
     * returns the packet representing it.
     * 
	...
	if (pt2 instanceof pEndBrace)
	    return new pPredicate(predicates, "{}", pt1, new pParens(
		    (pStartBrace) pt1, pak));
	else
	    throw new SyntaxErrorException("Expected '}' at ",
		    pt2.getPosition(), pt2.getLine(), pt2.getCharPos());
    ...
    /**
     * Internal method that parses the stream begining with a bracket, and
     * returns the packet representing it.
     * 
	...
	throw new SyntaxErrorException("Expected ']' at ", pt2.getPosition(),
		pt2.getLine(), pt2.getCharPos());
    };

    /**
     * Internal method that parses the stream and returns the packet
     * representing the next term.
     * 
     * @param single
     *            Affects the treatment of commas. If true, then a comma
	...
	do {
	    ...
	    if (pt instanceof pName && !need_lhsop
		    && ((pName) pt).isPotentialPredicate())
	    ...
	    else if ((pt instanceof pReal || pt instanceof pInteger || pt instanceof pUnaryNumber)
		    && !need_lhsop)
		pp = new pNumber(pt);
    ...
    /**
     * Internal method which updates the given stack of packets used to store
     * operands for operators. Because of the previous tests, we can make a
     * number of assumptions when reordering the stack. Assumptions include:
     * only the first and only element on the stack can ever be a non-operator,
     * no incompletely bound operators are on the stack if the current pp
     * operator requires a lhs, if all operators on the stack are complete then
     * pp is a lhs operator, only the top operator on the stack will be
     * incomplete, if the top operator is incomplete, then the next package will
     * not be an operator requiring a lhs. the stack has the property that each
     * operator on the stack is the rhs for the previous operator, the priority
     * goes in increasing order from top to bottom of stack, and operators that
     * are non associative to the right do not have any operators higher on the
     * stack with the same priority.
     * 
     * @param paks
     *            The stack of packets.
     * @param pp
     *            The packet to add to the stack (may be an operator that takes
     *            other elements of the stack as operands).
     */
	...
	if (paks.empty())
		...
		do {
		    if (ppo.getPriority() < sppo.getPriority()
			    || (ppo.getPriority() == sppo.getPriority() && !sppo
			...
			paks.push(ppo);
			break;
		    } else if ((ppo.getPriority() > sppo.getPriority() && paks
			    .empty())
			    || (ppo.getPriority() == sppo.getPriority() && !ppo
			...
			if (!paks.empty())
			    ...
			    throw new SyntaxErrorException(
				    "Parentheses required. Associativity rules do not permit use of operator at ",
				    opt.getPosition(), opt.getLine(), opt
					    .getCharPos());
			}
		    }
		} while (true);
	    } else {// this happens when the first and only element on the stack
		    // is not an operator
		ppo.setLHS(spp);
		paks.push(ppo);
	    }
	} else // is not operator or not lhs operator
	{
		...
		if (ppo.getPriority() > sppo.getPriority()
			|| (ppo.getPriority() == sppo.getPriority() && sppo
				.isNonAssociativeRight()))
		    throw new SyntaxErrorException(
			    "Operator with lower priority number or parenthesis expected for operator at ",
    ...
    /**
     * Internal method which produces a final packet representing the given
     * stack of packets.
     * 
     * @param paks
     *            The stack of packets.
    ...
    /**
     * Internal method which produces a final packet representing the given
     * stack of packets. Should only return operators or predicates (or atoms).
     * Should not touch the tokenizer stream, and should not call any other
     * functions that might.
     * 
     * @param paks
     *            The stack of packets.
     * @param need_lhsop
     *            If true then return an operator which requires a lhs or return
     *            null if a lhs operator was required. If false, return a packet
     *            which doesn't required a lhs or return null.
     * 
