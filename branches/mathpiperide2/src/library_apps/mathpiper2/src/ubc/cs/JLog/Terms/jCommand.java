...
public class jCommand extends jTerm {
	...
	// only equiv with other cons terms of same type
	if (type != t.type)
	    return false;

	// altough we cannot be certain that term is a jCommand, if it is not
	// then type
	// was wrong so this warrents a failing exception.
	{
	    ...
	    // equivalence each element of cons term, exit on first failure.

	    return rhs.equivalence(cterm.rhs, v);
	}
    };
