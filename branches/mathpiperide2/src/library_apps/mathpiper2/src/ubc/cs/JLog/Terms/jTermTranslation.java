...
/**
 * This class aggregates iObjectToTerm and iTermToObject converters into a
 * single conversion class which chooses the correct converter object for the
 * for desired conversion. The fundamental idea behind the converter mappings is
 * that typically each prolog term object has a single corresponding Java
 * object, and each Java object has a single corresponding prolog term. To
 * peform an object->term conversion we look up the object class in the term
 * converter look-up table and use the associated converter. To peform a
 * term->object conversion we look up the term class in the object converter
 * table and use the associated converter. Because the source object class is
 * just a key for a converter lookup, the idea is extended to arbitrary keys for
 * converters. It is possible to specify which converter to use via these keys
 * (see the two parameter versions of <code>createTermFromObject</code> and
 * <code>createObjectFromTerm</code>). For each conversion direction (i.e.,
 * object->term and term->object) there is a default converter, used if the not
 * matching converters are found in the look-up table.
 * 
...
public class jTermTranslation implements iObjectToTerm, iTermToObject {
    ...
    /**
     * Sets default translators in the create*Keys hashtables. Existing
     * registered translators remain, provided they were not registered for the
     * default keys. This method establishes the default translation behaviour.
     */
    ...
    /**
     * Sets the default <code>iTermToObject</code> conversion objects for
     * converting Prolog jTerms into their corresponding Java objects. Also sets
     * the default <code>iTermToObject</code> conversion object. The following
     * conversion mappings are registered (Keys : ObjectClass):<br>
	...
	// this converter creates a Boolean, based on jTrue, jFail, or a jAtom
	// name "true" or "fail".
	{
	    iTermToObject c;
		...
		public Object createObjectFromTerm(jTerm term) {
		    if (term instanceof jTrue
			    || (term instanceof jAtom && term.getName().equals(
				    "true")))
			return Boolean.TRUE;
	...
	// this converter returns a vector for a list.
	{
		...
		public Object createObjectFromTerm(jTerm term) {
			...
			Enumeration e = ((jList) term)
				.elements(jTermTranslation.this);
			Vector v = new Vector();
	...
	// default translation, create string representation of term
	{
		...
		public Object createObjectFromTerm(jTerm term) {
		    if (term instanceof jAtom
			    || (term instanceof jPredicate && ((jPredicate) term)
				    .getArity() == 0))
    ...
    /**
     * Sets the default <code>iObjectToTerm</code> conversion objects for
     * converting Java objects into their corresponding Prolog jTerm objects.
     * Also sets the default <code>iObjectToTerm</code> conversion object. The
     * following conversion mappings are registered (Keys : ObjectClass):<br>
     ...
     * ([jTerm] : jTerm)<br>
     * (["String",jAtom,jTerm] : jAtom,jTerm) default if no PrologServices
     * specified<br>
	...
	// this converter creates a true or fail term, from Boolean or String
	// with "true" or "fail".
	{
	    iObjectToTerm c;
		...
		public jTerm createTermFromObject(Object obj) {
			...
			return (((Boolean) obj).booleanValue() ? (jTerm) jTrue.TRUE
				: (jTerm) jFail.FAIL);
	...
	// this converter returns a list given a vector, or an enumeration.
	{
		...
		public jTerm createTermFromObject(Object obj) {
			...
			return jListPair.createListFromEnumeration(
				(Enumeration) obj, jTermTranslation.this);
	...
	// this converter returns the original object if it is a jTerm.
	{
		...
		public jTerm createTermFromObject(Object obj) {
		    ...
		    throw new TranslationFailureException(
			    "Expected jTerm object.");
		}
	    };

	    RegisterObjectToTermConverter(jTerm.class, c);
	}
	// this converter returns an atom (from String representation of object)
	// or original jTerm.
	{
	    iObjectToTerm c;
	...
	// this default converter returns parses Strings to their jTerm
	// representation.
	// prolog services must exist for this converter to exist and be the
	// default.
	if (prolog != null) {
	    iObjectToTerm c;
	...
	throw new TranslationFailureException(
		"No object to term conversion unit found.");
    };
	...
	throw new TranslationFailureException(
		"No term to object conversion unit found.");
    };
};
