...
public class gTracePanel extends Panel {
    ...
    protected Button hideshow, update, settrace, setbreak, cleartrace,
	    clearbreak;
	...
	hideshow.setLabel((hideshow.getLabel() == HIDEBUTTON ? SHOWBUTTON
		: HIDEBUTTON));
	try {
...

	{
		...
		try {
		    ...
		    internal_sortInsertNameArity(bv,
			    new gTraceItem(na2, ti.getTrace(), ti.getBreak()));
		} catch (InvalidTraceItemException e1) {
		    internal_sortInsertNameArity(bv, new gTraceItem(na2, false,
			    false));
		}
	    }
		...
		if (rd.isBuiltin()) {
			...
			internal_sortInsertNameArity(
				bv,
				new gTraceItem(rd, ti.getTrace(), ti.getBreak()));
		    } catch (InvalidTraceItemException e2) {
			internal_sortInsertNameArity(bv, new gTraceItem(rd,
				false, false));
		    }
		}
	    }
	}
	{
		...
		if (!rd.isBuiltin()) {
			...
			internal_sortInsertNameArity(uv, new gTraceItem(rd,
				false, false));
		    }
		}
	    }
	...
	// the design of this method and the methods it calls, requires that the
	// instance
	// predicate vectors are updated at the end, after the old versions are
	// used to lookup
	// the trace items.
	user_predicates = uv;
	...
	traceitem_hash = new Hashtable(user_predicates.size()
		+ builtin_predicates.size() + 1);
	{
	    Enumeration e;
    ...
    protected synchronized void internal_updatePredicateList(
	    iNameArity keepselected) {
	predlist.removeAll();
    ...
    protected boolean internal_sortCompareGreaterThan(iNameArity na1,
	    iNameArity na2) {
	String s1 = na1.getName().toLowerCase(), s2 = na2.getName()
		.toLowerCase();
	int result;
    ...
    protected synchronized gTraceItem internal_getNameArityTraceItem(
	    iNameArity na) {

	{
		...
		if (na.getName().equals(ti.getName())
			&& na.getArity() == ti.getArity())
		    return ti;
	    }
	}
