 ...
 *
 */
public class Clojure extends JPanel
    implements EBComponent, ClojureActions, DefaultFocusComponent {
	...
	private ClojureTextArea textArea;

	private ClojureToolPanel toolPanel;
    // }}}
	 ...
	 */
	public Clojure(View view, String position) {
		super(new BorderLayout());
		this.view = view;
		this.floating = position.equals(DockableWindowManager.FLOATING);

		if (jEdit.getSettingsDirectory() != null) {
			this.filename = jEdit.getProperty(ClojurePlugin.OPTION_PREFIX
					+ "filepath");
			if (this.filename == null || this.filename.length() == 0) {
				this.filename = new String(jEdit.getSettingsDirectory()
						+ File.separator + "qn.txt");
				jEdit.setProperty(
						ClojurePlugin.OPTION_PREFIX + "filepath",
						this.filename);
			}
			this.defaultFilename = this.filename;
		}

		this.toolPanel = new ClojureToolPanel(this);
		add(BorderLayout.NORTH, this.toolPanel);

		if (floating)
			this.setPreferredSize(new Dimension(500, 250));

		textArea = new ClojureTextArea();
		textArea.setFont(ClojureOptionPane.makeFont());
	...
	private void propertiesChanged() {
		...
		Font newFont = ClojureOptionPane.makeFont();
	...
	public void saveFile() {
		...
		try {
			...
			Log.log(Log.ERROR, Clojure.class,
	...
	private void readFile() {
		...
		try {
			...
			Log.log(Log.ERROR, Clojure.class, "notepad file " + filename
					+ " does not exist");
		} catch (IOException ioe) {
			Log.log(Log.ERROR, Clojure.class,
