...
/**
 * 
 * maxima - a dockable JPanel, a demonstration of a jEdit plugin.
 *
 */
public class Maxima extends JPanel
    implements EBComponent, MaximaActions, DefaultFocusComponent {
	...
	private MaximaTextArea textArea;

	private MaximaToolPanel toolPanel;
    // }}}
	 ...
	 */
	public Maxima(View view, String position) {
		super(new BorderLayout());
		this.view = view;
		this.floating = position.equals(DockableWindowManager.FLOATING);

		if (jEdit.getSettingsDirectory() != null) {
			this.filename = jEdit.getProperty(MaximaPlugin.OPTION_PREFIX
					+ "filepath");
			if (this.filename == null || this.filename.length() == 0) {
				this.filename = new String(jEdit.getSettingsDirectory()
						+ File.separator + "qn.txt");
				jEdit.setProperty(
						MaximaPlugin.OPTION_PREFIX + "filepath",
						this.filename);
			}
			this.defaultFilename = this.filename;
		}

		this.toolPanel = new MaximaToolPanel(this);
		add(BorderLayout.NORTH, this.toolPanel);

		if (floating)
			this.setPreferredSize(new Dimension(500, 250));

		textArea = new MaximaTextArea();
		textArea.setFont(MaximaOptionPane.makeFont());
	...
	private void propertiesChanged() {
		...
		Font newFont = MaximaOptionPane.makeFont();
	...
	public void saveFile() {
		...
		try {
			...
			Log.log(Log.ERROR, Maxima.class,
	...
	private void readFile() {
		...
		try {
			...
			Log.log(Log.ERROR, Maxima.class, "notepad file " + filename
					+ " does not exist");
		} catch (IOException ioe) {
			Log.log(Log.ERROR, Maxima.class,
