...
/**
 * This class really just represents a list of points that can be drawn as dots
 * or a connected sequence of line segments.  It is possible to add points and
 * have those points show up in a View without redrawing the entire contents 
 ...
 * an ODE in two dimensions. 
 */
public class OrbitPoints2D  {
	
	/**
	 * A constant for selecting whether the points should be drawn as individual dots or
	...
	private ArrayList<Point2D> points = new ArrayList<Point2D>();  // This is the list of points.
	...
	private double dotDiameter = 2.5;  // diameter of ovals used to draw dots when style == DOTS
	...
	/**
	 * Adds all the points from an array to the curve by calling {@link #addPoint(Point2D)} for
	...
	public void drawPoint(View view, int index) {
		...
		switch (style) {
		case LINES:
			p0 = index == 0 ? p1 : (Point2D)points.get(index-1);
			if (p0 == null)
				return;
			view.drawLine(p0,p1);
			break;
