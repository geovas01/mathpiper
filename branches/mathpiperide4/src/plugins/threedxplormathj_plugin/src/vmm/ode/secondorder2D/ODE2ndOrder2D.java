...
abstract public class ODE2ndOrder2D extends ODE_2D {

	public ODE2ndOrder2D() {
		super(false,true,"x","y","x'","y'");
		initialDataDefault = new double[] { 1, 0, 0, 1 };
		timeSpanDefault = 30;
	}
	...
	protected void nextEulerPoint(double[] pointData, double dt) {
		...
		double xdot = pointData[2];
		double ydot = pointData[3];
		pointData[0] = x + dt*xdot;
		pointData[1] = y + dt*ydot;
		pointData[2] = xdot + dt*xdotdot(x,y,xdot,ydot);
		pointData[3] = ydot + dt*ydotdot(x,y,xdot,ydot);
	}
	...
	protected void nextRungeKuttaPoint(double[] pointData, double dt) {
		...
		i1 = dt * xdot;
		j1 = dt * ydot;
		h1 = dt * xdotdot(x,y,xdot,ydot);
		k1 = dt * ydotdot(x,y,xdot,ydot);
		i2 = dt * (xdot + h1/2);
		j2 = dt * (ydot + k1/2);
		h2 = dt * xdotdot(x+i1/2,y+j1/2,xdot+h1/2,ydot+k1/2);
		k2 = dt * ydotdot(x+i1/2,y+j1/2,xdot+h1/2,ydot+k1/2);
		i3 = dt * (xdot + h2/2);
		j3 = dt * (ydot + k2/2);
		h3 = dt * xdotdot(x+i2/2,y+j2/2,xdot+h2/2,ydot+k2/2);
		k3 = dt * ydotdot(x+i2/2,y+j2/2,xdot+h2/2,ydot+k2/2);
		i4 = dt * (xdot + h3);
		j4 = dt * (ydot + k3);
		h4 = dt * xdotdot(x+i3,y+j3,xdot+h3,ydot+k3);
		k4 = dt * ydotdot(x+i3,y+j3,xdot+h3,ydot+k3);
		...
		pointData[0] = x + dx;
		pointData[1] = y + dy;
		pointData[2] = xdot + dxdot;
		pointData[3] = ydot + dydot;
	}
	
	abstract protected double xdotdot(double x, double y, double xdot, double ydot);

	abstract protected double ydotdot(double x, double y, double xdot, double ydot);
	...
	private class ThrowIt extends BasicMouseTask2D {
		...
		public void doMouseUp(MouseEvent evt, Display display, View view, int width, int height) {
			if (throwing) {
				...
				double[] data = new double[] { pt1.getX(), pt1.getY(), pt2.getX()-pt1.getX(), pt2.getY()-pt1.getY() };
