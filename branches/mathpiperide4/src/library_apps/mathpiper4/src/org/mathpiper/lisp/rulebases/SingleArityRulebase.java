...
/**
 * A function (usually mathematical) which is defined by one or more rules. This
 * is the basic class which implements functions. Evaluation is done by
 * consulting a set of rewritng rules. The body of the first rule that matches
 * is evaluated and its result is returned as the function's result.
 */
public class SingleArityRulebase extends Evaluator {
     ...
     * 
     * @param aParameters
     *            linked list containing the names of the arguments
	...
	while (parameters != null) {
	    ...
	    try {
		if (!(parameters.car() instanceof String))
    ...
    /**
     * Evaluate the function with the given arguments. First, all arguments are
     * evaluated by the evaluator associated with aEnvironment, unless the iHold
     * flag of the corresponding parameter is true. Then a new LocalFrame is
     * constructed, in which the actual arguments are assigned to the names of
     * the formal arguments, as stored in iName. Then all rules in <b>iRules</b>
     * are tried one by one. The body of the first rule that matches is
     * evaluated, and the result is put in aResult. If no rule matches, aResult
     * will receive a new expression with evaluated arguments.
     * 
     * @param aResult
     *            (on output) the result of the evaluation
     * @param aEnvironment
     ...
     * @throws java.lang.Exception
     */
    public Cons evaluate(Environment aEnvironment, int aStackTop, Cons aArguments)
	    throws Throwable {
	...
	// Create a new local variables frame that has the same fenced state as
	// this function.
	...
	try {
		...
		//if (functionName.equals("Cos") && thisRule.getPrecedence() == 200) {
		//    int xx = 3;
		//}
		...
		if (matches) {
			...
			aResult = aEnvironment.iLispExpressionEvaluator.evaluate(aEnvironment, aStackTop, thisRule.getBody()); // ***
															       // User
															       // function
															       // is
															       // called
			...
			// todo:tk:note that user functions currently return
			// their results in aResult, not on the stack.
	    ...
	    // ex.printStackTrace(); System.exit(1);//todo:tk:uncomment for
	    // debugging.
    ...
    protected Cons[] evaluateArguments(Environment aEnvironment, int aStackTop, Cons aArguments)
	    throws Throwable {
	int arity = arity();
	int parameterIndex;
	...
	for (parameterIndex = 0; parameterIndex < arity; parameterIndex++) {
		...
		LispError.throwError(aEnvironment, aStackTop, LispError.WRONG_NUMBER_OF_ARGUMENTS, "Expected arity: " + arity + ".");

	    if (((ParameterName) iParameters.get(parameterIndex)).iHold) {
		// If the parameter is on hold, don't evaluate it and place a
		// copy of it in argumentsArray.
		...
		// LispError.WRONG_NUMBER_OF_ARGUMENTS, "Expected arity: " +
		// arity + ".", "INTERNAL");

		// Evaluate each argument and place the result into
		// argumentsResultArray[i];
    ...
    /**
     * Put an argument on hold. The \c iHold flag of the corresponding argument
     * is set. This implies that this argument is not evaluated by evaluate().
     * 
     * @param aVariable
     *            name of argument to put un hold
     */
    ...
    /**
     * Add a PredicateRule to the list of rules. See: insertRule()
     * 
     * @param aPrecedence
     * @param aPredicate
     * @param aBody
     * @throws java.lang.Exception
     */
    public void definePredicateRule(int aStackTop, int aPrecedence, Cons aPredicate, Cons aBody)
	    throws Throwable {
	// New branching rule.
	PredicateRule newRule = new PredicateRule(iEnvironment, aPrecedence, aPredicate, aBody);
	// LispError.check(iEnvironment, aStackTop, newRule != null,
	// LispError.CREATING_RULE, "INTERNAL");

	insertRule(aPrecedence, newRule);
    }

    /**
     * Add a TrueRule to the list of rules. See: insertRule()
     * 
     * @param aPrecedence
     * @param aBody
     * @throws java.lang.Exception
     */
    public void defineTrueRule(int aStackTop, int aPrecedence, Cons aBody)
	    throws Throwable {
	// New branching rule.
    ...
    /**
     * Add a PatternRule to the list of rules. See: insertRule()
     * 
     * @param aPrecedence
     * @param aPredicate
     * @param aBody
     * @throws java.lang.Exception
     */
    public void definePatternRule(int aStackTop, int aPrecedence, Cons aPredicate, Cons aBody)
	    throws Throwable {
	// New branching rule.
    ...
    /**
     * Insert any Rule object in the list of rules. This function does the real
     * work for defineAlwaysTrueRule() and definePattern(): it inserts the rule
     * in <b>iRules</b>, while keeping it sorted. The algorithm is O(log n),
     * where n denotes the number of rules.
     * 
     * @param aPrecedence
     * @param newRule
     */
