...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(i,nr){ TemplateFunction(\"ForEachInArray\",[item,list,body]) { Local(i,nr); MacroLocal(item); Assign(i,1); Assign(nr,Length(list)); While(i<=?nr) { MacroAssign(item,list[i]); Eval(body); Assign(i,AddN(i,1)); }; };};UnFence(\"ForEachInArray\",3);HoldArgumentNumber(\"ForEachInArray\",3,1);HoldArgumentNumber(\"ForEachInArray\",3,3);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(TraceStart,TraceEnter,TraceLeave,DebugStart,DebugEnter, DebugLeave,ProfileStart,ProfileEnter,result, WriteLines,ClearScreenString,DebugFileLoaded, DebugFileLines, DebugNrLines, debugstepoverfile, debugstepoverline) {TraceStart() := {indent := 0;};TraceEnter() :={ indent++; Space(2*indent); Echo(\"Enter \",CustomEvalExpression());};TraceLeave() :={ Space(2*indent); Echo(\"Leave \",CustomEvalResult()); indent--;};Macro(TraceExp,[expression]){ TraceStart(); CustomEval(TraceEnter(),TraceLeave(),CustomEvalStop(),@expression);};DebugStart():={ debugging:=True; debugstopdepth := -1; breakpoints:=[]; filebreakpoints := []; debugstopped:=False; debugverbose:=False; debugcallstack:=[]; breakpredicate:=False;};DebugRun():= {debugging:=False;True;};DebugStep():= {debugging:=False;nextdebugging:=True;};DebugStepOver():={ debugging:=False; debugstepoverfile := DebugFile(CustomEvalExpression()); debugstepoverline := DebugLine(CustomEvalExpression()); debugstopdepth := Length(debugcallstack);};DebugBreakAt(file,line):={ Check(DebugMode?(), \"Mode\", \"DebugBreakAt only supported in the debug build of MathPiper\"); Decide(filebreakpoints[file] =? Empty,filebreakpoints[file]:=[]); DestructiveAppend(filebreakpoints[file],line);};DebugRemoveBreakAt(file,line):={ Check(DebugMode?(), \"Mode\", \"DebugRemoveBreakAt only supported in the debug build of MathPiper\"); Decide(filebreakpoints[file] =? Empty,filebreakpoints[file]:=[]); filebreakpoints[file] := Difference(filebreakpoints[file],[line]);};DebugStop():= {debugging:=False;debugstopped:=True;CustomEvalStop();};DebugVerbose(verbose):= {debugverbose:=verbose;};DebugAddBreakpoint(fname_String?) <-- { breakpoints := fname~breakpoints;};Macro(DebugBreakIf,[predicate]){ breakpredicate:= Hold(@predicate);};BreakpointsClear() <--{ breakpredicate:=False; breakpoints := [];};Macro(DebugLocals,[]){ Echo(\"\"); Echo(\"*************** Current locals on the stack ****************\"); ForEach(item,CustomEvalLocals()) { Echo(\" \",item,\" : \",Eval(item)); }; Echo(\"\");};DebugCallstack() <--{ Echo(\"\"); Echo(\"*************** Function call stack ****************\"); ForEach(item,debugcallstack) { If(Function?(item)) Echo(\" Function \",Type(item),\" : \",item) Else Echo(\" Variable \",item); }; Echo(\"\");};Macro(DebugEnter,[]){ debugcallstack := CustomEvalExpression()~debugcallstack;  Decide(debugging =? False And? Eval(breakpredicate) =? True, { breakpredicate:=False; debugging:=True; }); Decide(debugging =? False And? DebugMode?(), { Local(file,line); file := DebugFile(CustomEvalExpression()); Decide(filebreakpoints[file] !=? Empty, { line := DebugLine(CustomEvalExpression()); Decide(Not?(file =? debugstepoverfile And? line =? debugstepoverline) And? Contains?(filebreakpoints[file],line), { debugging:=True; } ); }); });  Decide(debugging =? False And? Function?(CustomEvalExpression()) And? Contains?(breakpoints,Type(CustomEvalExpression())), debugging:=True); nextdebugging:=False; Decide(debugging, { Decide(DebugMode?(),DebugShowCode()); Echo(\">>> \",CustomEvalExpression()); While(debugging) { Echo(\"DebugResult: \",Eval(PipeFromString(ReadCmdLineString(\"Debug> \")~\";\")ParseMathPiper()));  Decide(IsExitRequested(),debugging:=False); }; }); debugging:=nextdebugging; Decide(IsExitRequested(),debugstopped:=True);};Macro(DebugLeave,[]){ Decide(debugging =? False And? debugstopdepth >=? 0 And? Length(debugcallstack) =? debugstopdepth, { debugstepoverline := -1; debugging := True; debugstopdepth := -1; }); debugcallstack := Rest(debugcallstack); Decide(debugverbose,Echo(CustomEvalResult(),\" <-- \",CustomEvalExpression()));};Macro(Debug,[expression])PipeToStdout(){ DebugStart(); CustomEval(DebugEnter(),DebugLeave(),Decide(debugstopped,Check(False, \"Debug\", \"\"),{debugging:=True;debugcallstack := Rest(debugcallstack);}),@expression);};ProfileStart():={ profilefn:=[];};10 # ProfileEnter()_(Function?(CustomEvalExpression())) <--{ Local(fname); fname:=Type(CustomEvalExpression()); Decide(profilefn[fname]=?Empty,profilefn[fname]:=0); profilefn[fname] := profilefn[fname]+1;};Macro(Profile,[expression]){ ProfileStart(); CustomEval(ProfileEnter(),True,CustomEvalStop(),@expression); ForEach(item,profilefn) Echo(\"Function \",item[1],\" called \",item[2],\" times\");};Macro(EchoTime,[expression]){ Local(result); Echo(Time()Assign(result, @expression), \"seconds taken.\"); result;};ClearScreenString := UnicodeToString(27)~\"[2J\"~UnicodeToString(27)~\"[1;1H\";WriteLines(filename,lines,from,nrlines,breakpoints,current):={ Local(i,nr); nr:=Length(lines); WriteString(ClearScreenString); Echo(\"File \",filename,\" at line \",current); For(i:=from,i<?from+nrlines And? i<?nr,i++) { If(current =? i) WriteString(\">\") Else WriteString(\" \"); If(Contains?(breakpoints,i)) WriteString(\"*\") Else WriteString(\" \"); WriteString(\"| \"); Echo(lines[i][1]); };};DebugFileLoaded := \"\";DebugFileLines := [];DebugNrLines:=20;DebugShowCode():={ False;};}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(lastcoef,OrdBuild, AddFoundSolutionSingle , AddFoundSolution, Fct, MkfactD,p){LastCoef(_vector,_p) <--{ Local(n); n:=Length(vector); Add(vector*p^(0 .. (n-1)));};OrdBuild(vector,q):={ Local(i,result,n); Assign(i,2); Assign(result, 0); Assign(n, Length(vector)); While (i<=?n) { Assign(result,result+(i-1)*vector[i]*p^(i-2)); Assign(i, i+2); }; q*result;};Function(AddFoundSolutionSingle,[p]){ Local(calc); Assign(calc, Eval(lastcoef)); Decide(Equal?(calc, 0), { Local(newlist,count,root); count:=0; root := p; Local(rem); rem:=[-root,1]; [testpoly,rem]:=MkfactD(testpoly,rem); rem:=[-root,1]; [newlist,rem]:=MkfactD(poly,rem); While (rem =? []) { count++; Assign(poly,newlist); rem:=[-root,1]; [newlist,rem]:=MkfactD(poly,rem); }; Local(lgcd,lc); Assign(lgcd,Gcd([andiv,an,root])); Assign(lc,Quotient(an,lgcd)); Assign(result,[var+ (-(Quotient(root,lgcd)/lc)),count]~result); Assign(andiv,Quotient(andiv,lgcd^count)); Assign(anmul,anmul*lc^count); Local(p,q); Assign(lastcoef, LastCoef(testpoly,p)); Assign(ord, OrdBuild(testpoly,q)); });};UnFence(AddFoundSolutionSingle,1);Function(AddFoundSolution,[p]){ AddFoundSolutionSingle(p); AddFoundSolutionSingle(-2*q+p);};UnFence(AddFoundSolution,1);Function(Fct,[poly,var]){ Local(maxNrRoots,result,ord,p,q,accu,calc,twoq,mask); Local(gcd); { Assign(gcd,Gcd(poly)); Decide(poly[Length(poly)] <? 0,Assign(gcd, gcd * -1)); Assign(poly,poly/gcd); }; Local(unrat); Assign(unrat,Lcm(MapSingle(\"Denominator\",poly))); Assign(poly,unrat*poly); Local(origdegree); Assign(origdegree,Length(poly)-1); Local(an,andiv,anmul); Assign(an,poly[Length(poly)]); Assign(poly,poly* (an^((origdegree-1) .. -1))); Assign(andiv,an^(origdegree-1)); Assign(anmul,1); Local(leadingcoef,lowestcoef); Assign(leadingcoef,poly[Length(poly)]); { Local(i); Assign(i,1); Assign(lowestcoef,Abs(poly[i])); While (lowestcoef =? 0 And? i<=?Length(poly)) { Assign(i,i+1); Assign(lowestcoef,Abs(poly[i])); }; };    Local(testpoly); Local(deriv);  deriv:=Rest(poly); { Local(i); For (i:=1,i<=?Length(deriv),i++) { deriv[i] := deriv[i]*i; }; }; { Local(q,r,next); q:=poly; r:=deriv; While(r !=? []) { next := MkfactD(q,r)[2]; q:=r; r:=next; };   q:=q/q[Length(q)]; testpoly:=MkfactD(poly,q)[1]; }; Assign(maxNrRoots,Length(testpoly)-1); Assign(result, []); Assign(lastcoef, LastCoef(testpoly,p)); Assign(ord, OrdBuild(testpoly,q)); Assign(accu,[]); Assign(q,1); Assign(twoq,MultiplyN(q,2)); Assign(mask,AddN(twoq,MathNegate(1))); If(Even?(testpoly[1])) { Assign(accu,0~accu); AddFoundSolutionSingle(0); }; Assign(p,1); Assign(calc, Eval(lastcoef)); Decide(Even?(calc), { Assign(accu,1~accu); AddFoundSolution(1); }); Assign(q,twoq); Assign(twoq,MultiplyN(q,2)); Assign(mask,AddN(twoq,MathNegate(1))); While(Length(result)<?maxNrRoots And? Length(accu)>?0 And? q<=?Abs(testpoly[1])) { Local(newaccu); Assign(newaccu,[]); ForEach(p,accu) { Assign(calc,Eval(lastcoef)); Decide(LessThan?(calc,0), Assign(calc, AddN(calc,MultiplyN(twoq,QuotientN(AddN(MathNegate(calc),twoq),twoq)))) ); Assign(calc, BitAnd(calc, mask)); Decide( Equal?(calc, 0), { Assign(newaccu, p~newaccu); AddFoundSolutionSingle(-2*q+p); }); Assign(calc, AddN(calc, Eval(ord))); Decide(LessThan?(calc,0), Assign(calc, AddN(calc,MultiplyN(twoq,QuotientN(AddN(MathNegate(calc),twoq),twoq)))) ); Assign(calc, BitAnd(calc, mask)); Decide( Equal?(calc, 0), { Assign(newaccu, AddN(p,q)~newaccu); AddFoundSolution(AddN(p,q)); }); }; Assign(accu, newaccu); Assign(q,twoq); Assign(twoq,MultiplyN(q,2)); Assign(mask,AddN(twoq,MathNegate(1))); };   Assign(poly,poly*an^(0 .. (Length(poly)-1))); Assign(poly,gcd*anmul*poly);  Decide(Not? Zero?(unrat * andiv ),Assign(poly,poly/(unrat * andiv ))); Decide(poly !=? [1], { result:=[(Add(poly*var^(0 .. (Length(poly)-1)))),1]~result; }); result;};BinaryFactors(expr):={ Local(result,uni,coefs); uni:=MakeUni(expr,VarList(expr)[1]); uni:=FunctionToList(uni); coefs:=uni[4]; coefs:=Concat(ZeroVector(uni[3]),coefs); result:=Fct(coefs,uni[2]); result;};MkfactD(numer,denom):={ Local(q,r,i,j,ln,ld,nq); DropEndZeroes(numer); DropEndZeroes(denom); Assign(numer,Reverse(numer)); Assign(denom,Reverse(denom)); Assign(ln,Length(numer)); Assign(ld,Length(denom)); Assign(q,FillList(0,ln)); Assign(r,FillList(0,ln)); Assign(i,1); Decide(ld>?0, { While(Length(numer)>=?Length(denom)) { Assign(nq,numer[1]/denom[1]); q[ln-(Length(numer)-ld)] := nq; For(j:=1,j<=?Length(denom),j++) { numer[j] := (numer[j] - nq*denom[j]); }; r[i] := r[1] + numer[1]; Assign(numer, Rest(numer)); i++; }; }); For(j:=0,j<?Length(numer),j++) { r[i+j] := r[i+j] + numer[j+1]; }; Assign(q,Reverse(q)); Assign(r,Reverse(r)); DropEndZeroes(q); DropEndZeroes(r); [q,r];};}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorCancel( p_Rational? ) <-- Factor(p);15 # FactorCancel( p_RationalFunction? ) <--{ Decide(InVerboseMode(),Tell(FactorCancel,p)); Local(pp,ff,n,d,fn,fd,f,tnu,newn,newd,s,k); pp := Simplify(p); Decide(InVerboseMode(),Tell(\" \",pp)); ff := Factors(pp); Decide(InVerboseMode(),Tell(\" \",ff)); tnu := [ff[1]]; Decide(ListOfLists?(ff), tnu := RemoveDuplicates(Transpose(ff)[1])); newn := []; newd := []; Decide(InVerboseMode(),Tell(\" \",tnu)); ForEach(f,tnu) { s := Select(ff,Lambda([X],X[1]=?f)); Decide(InVerboseMode(),Tell(\" \",[f,s])); Decide( s !=? [], { k := Sum(Transpose(s)[2]); Decide(InVerboseMode(),Tell(\" \",[s,k])); Decide( k >? 0, DestructiveAppend(newn,[f,k]) ); Decide( k <? 0, DestructiveAppend(newd,[f,-k]) ); }, { k := 1; DestructiveAppend(newn,[f,k]); } ); }; Decide(InVerboseMode(),Tell(\" \",[newn,newd])); FW(newn)/FW(newd);};20 # FactorCancel( _p ) <-- Factor(p);";
        scriptMap.put("FactorCancel",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FactorQS\",[n]){ Local(x,k,fb,j);   k:=Round(NM(Sqrt(Exp(Sqrt(Ln(n)*Ln(Ln(n))))))); fb:=ZeroVector(k); For(j:=1,j<=?k,j++){ fb[j]:=NextPrime(j); };};";
        scriptMap.put("FactorQS",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1# FactorizeInt(0) <-- [];1# FactorizeInt(1) <-- [];3# FactorizeInt(n_Integer?) <--{ Local(smallpowers); n := Abs(n);    Decide( Gcd(ProductPrimesTo257(), n) >? 1,  smallpowers := TrialFactorize(n, 257),  smallpowers := [n]  ); n := smallpowers[1];  Decide(n=?1, Rest(smallpowers),  {  SortFactorList( PollardCombineLists(Rest(smallpowers), PollardRhoFactorize(n)) ); } );};";
        scriptMap.put("FactorizeInt",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsBinomials( _x + y_FreeOf?(x) ) <-- [x+y,1];10 # FactorsBinomials( _x - y_FreeOf?(x) ) <-- [x-y,1];10 # FactorsBinomials( c_Constant? * _x + y_FreeOf?(x) ) <-- [c*x+y,1];10 # FactorsBinomials( c_Constant? * _x - y_FreeOf?(x) ) <-- [c*x-y,1];10 # FactorsBinomials( _x^m_Odd? + _y ) <--{ Decide(InVerboseMode(),Tell(\"FactorsBinomialssum\",[x,m,y])); Local(nn,qq, r); nn := (m-1)/2; qq := (y^(1/m)); Decide(InVerboseMode(),Tell(\" FBinsum1\",[nn,qq])); r := [[x+qq,1],[Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1]];};12 # FactorsBinomials( c_Constant? * _x^m_Odd? + _y ) <--{ Decide(InVerboseMode(),Tell(\"FactorsBinomialssum\",[c,x,m,y])); Local(nn,qq, r); nn := (m-1)/2; qq := ((y/c)^(1/m)); Decide(InVerboseMode(),Tell(\" FBinsum.1b\",[nn,qq])); Decide( c=?1, r := [[x+qq,1],[Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1]], r := [[c,1],[x+qq,1],[Sum(k,0,m-1,(-1)^k*qq^k*x^(m-1-k)),1]] ); };10 # FactorsBinomials( _x^m_Integer? - _y ) <--{ Decide(InVerboseMode(),Tell(\"FactorsBinomialsdif\",[x,m,y])); Local(pp,qq,r,L); pp := m-1; qq := (y^(1/m)); Decide(Number?(y),qq:=GuessRational(NM(qq))); Decide(InVerboseMode(),Tell(\" FAssignif.1\",[pp,qq]));  if (m =? 2) { L := FunctionToList(y); Decide(And?(L[1]=?ToAtom(\"^\"),L[3]=?2),qq:=L[2]); r := [[x+qq,1],[x-qq,1]]; } Else If(m =? 4) {r := [[x+qq,1],[x-qq,1],[x^2+qq^2,1]];} Else If(m =? 6) {r := [[x+qq,1],[x-qq,1],[x^2+x*qq+qq^2,1],[x^2-x*qq+qq^2,1]];} Else {r := [[x-qq,1],[Sum(k,0,pp,qq^k*x^(pp-k)),1]];}; r;};12 # xFactorsBinomials( c_Constant? * _x^m_Integer? - _y ) <--{ Decide(InVerboseMode(),Tell(\"FactorsBinomialsdif\",[c,x,m,y])); Local(aa,bb,c0,r); aa := c^(1/m); bb := ((y)^(1/m)); Decide(Number?(y),bb:=GuessRational(NM(bb))); Decide(InVerboseMode(),Tell(\" FAssignif.1b\",[aa,bb])); r := FactorsBinomials( (aa*x)^m - bb^m );};";
        scriptMap.put("FactorsBinomials",scriptString);
        scriptMap.put("xFactorsBinomials",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsMonomial(expr_Monomial?) <--[ Decide(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); Decide( RationalFunction?(expr), [ den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); ], [ den := 1; num := Flatten(expr,\"*\"); ] ); Decide(InVerboseMode(),Tell(\" \",[num,den])); Ns := Select(num, \"Complex?\"); Decide(InVerboseMode(),Tell(\" \",Ns)); Decide( Ns =? [], Decide( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); Decide(InVerboseMode(),Tell(\" \",num)); result := []; ForEach(f,num) [ Decide( Complex?(f),  DestructiveAppend(result,[(f),1]), Decide( Atom?(f), DestructiveAppend(result,[f,1]), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); ]; result;];";
        scriptMap.put("FactorsMonomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsPolynomialOverIntegers(_expr)_PolynomialOverIntegers?(expr) <--{ Local(x); x := VarList(expr)[1]; FactorsPolynomialOverIntegers(expr,x);};15 # FactorsPolynomialOverIntegers(_expr) <-- expr;10 # FactorsPolynomialOverIntegers(_expr,_var)_(PolynomialOverIntegers?(expr,var)) <--{ Local(factorsList,factListTransp,factrs,multiplicities,factrsUnMonic); Local(polyFactors,normalizations,normDivisor,polyFactors,factList); Local(n,result,newResult,gtotal,r,rr,d,g); factorsList := BinaryFactors(expr);        factListTransp := Transpose(factorsList); factrs := factListTransp[1]; multiplicities := factListTransp[2];          factrsUnMonic := MapSingle(\"Together\",factrs);         [polyFactors,normalizations]:=Transpose(MapSingle(\"GetNumerDenom\",factrsUnMonic));          normDivisor := Product(Map(\"^\",[normalizations,multiplicities]));         polyFactors[1] := Simplify(polyFactors[1]/normDivisor);     factListTransp[1] := polyFactors; factList := Transpose(factListTransp);    result := factList;   Local(newResult,gtotal,d,g,rr); newResult := []; gtotal := 1; ForEach(r,result) { d := Degree(r[1],var); g := Gcd(Coef(r[1],var,0 .. d)); Decide( g >? 1,  { gtotal:=g*gtotal;  r[1]:=Simplify(r[1]/g); } ); Decide(d >? 2, {  rr := TryToReduceSpecialPolynomial(r[1]); Decide( List?(rr),newResult := Concat(newResult,rr) ); }, Decide( r !=? [1,1], newResult := r~newResult ) ); };  Decide(gtotal>?1,newResult:=[gtotal,1]~newResult); newResult;}; 10 # TryToReduceSpecialPolynomial(_x^4+_x^2+1) <-- [[x^2+x+1,1],[x^2-x+1,1]];10 # TryToReduceSpecialPolynomial(_x^6-1) <-- [[x+1,1],[x-1,1],[x^2+x+1,1],[x^2-x+1,1]];";
        scriptMap.put("FactorsPolynomialOverIntegers",scriptString);
        scriptMap.put("TryToReduceSpecialPolynomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FactorsSmallInteger( N_Integer? ) <--{ Local(n, power, prime, result, limit); n := Abs(N);  limit := Ceil(SqrtN(n));  prime := 2;  result := []; While( prime <=? limit And? n >? 1 And? prime*prime <=? n ) {  [n, power] := FindPrimeFactor(n, prime); Decide( power >? 0, DestructiveAppend(result, [prime,power]) ); prime := NextPseudoPrime(prime);  };  Decide( n >? 1, DestructiveAppend(result, [n,1]) ); result;};";
        scriptMap.put("FactorsSmallInteger",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FindPrimeFactor(n, prime) :={ Local(power, factor, oldfactor, step); power := 1; oldfactor := 1;  factor := prime;  While(Modulo(n, factor)=?0)  { oldfactor := factor;  factor := factor^2; power := power*2; }; power := Quotient(power,2); factor := oldfactor; n := Quotient(n, factor);  step := Quotient(power,2); While(step>?0 And? n >? 1) { factor := prime^step; Decide( Modulo(n, factor)=?0, { n := Quotient(n, factor); power := power + step; } ); step := Quotient(step, 2); }; [n, power];};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- { Local(r); Decide( RationalFunction?(expr), r := (VarList(Denominator(expr)) =? []), r := True );};15 # Monomial?(_expr) <-- False;10 # FactorsMonomial(expr_Monomial?) <--{ Decide(InVerboseMode(),Tell(\"FactorsMonomial\",expr)); Local(den,num,Ns,flat,prod,quot,result,f,ff); Decide( RationalFunction?(expr), { den := Denominator(expr); num := Flatten(Numerator(expr),\"*\"); }, { den := 1; num := Flatten(expr,\"*\"); } ); Decide(InVerboseMode(),Tell(\" \",[num,den])); Ns := Select(num, \"Complex?\"); Decide(InVerboseMode(),Tell(\" \",Ns)); Decide( Ns =? [], Decide( den !=? 1, DestructiveInsert(num,1,1/den)), DestructiveReplace(num,Find(num,Ns[1]),Ns[1]/den) ); Decide(InVerboseMode(),Tell(\" \",num)); result := []; ForEach(f,num) { Decide( Complex?(f),  DestructiveAppend(result,[(f),1]), Decide( Atom?(f), DestructiveAppend(result,[f,1]), DestructiveAppend(result,DestructiveDelete(FunctionToList(f),1)) ) ); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Roots(poly_CanBeUni) <--{ Local(factors,result,uni,root,i,deg); factors:=Factors(poly); result:=[]; ForEach(item,factors) { uni:=MakeUni(item[1]); deg:=Degree(uni); Decide(deg >? 0 And? deg <? 3, { root:= PSolve(uni); Decide(Not? List?(root),root:=[root]); For(i:=0,i<?item[2],i++) result:= Concat(root, result); } ); }; result;};";
        scriptMap.put("Roots",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RootsWithMultiples(poly_CanBeUni) <--{ Local(factors,result,uni,root,i,deg); factors:=Factors(poly); result:=[]; ForEach(item,factors) { uni:=MakeUni(item[1]); deg:=Degree(uni); Decide(deg >? 0 And? deg <? 3, { root:= PSolve(uni); Decide(Not? List?(root),root:=[root]); For(i:=1,i<=?Length(root),i++) result:= Concat([[root[i],item[2]]], result); } ); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TrialFactorize(n, limit) :={ Local(power, prime, result); result := [n];  prime := 2;  While(prime <=? limit And? n>?1 And? prime*prime <=? n) {  [n, power] := FindPrimeFactor(n, prime); Decide( power>?0, DestructiveAppend(result, [prime,power]) ); prime := NextPseudoPrime(prime);  };  DestructiveReplace(result, 1, n);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # Factors(p_PositiveInteger?) <-- FactorizeInt(p);11 # Factors(p_Integer?) <-- FactorizeInt(p);12 # Factors(p_Rational?)_(Denominator(p) !=? 1) <-- [[YacasFactor(Numerator(p)) /YacasFactor(Denominator(p)) , 1]]; 14 # Factors(p_GaussianInteger?) <-- GaussianFactors(p);20 # Factors(p_CanBeUni)_(Length(VarList(p)) =? 1) <--{ Local(x,d,coeffs,nterms,factorsList,result); x := VarList(p)[1]; d := Degree(p,x);   coeffs := Coef(p,x,0 .. Degree(p,x)); nterms := Length(Select(coeffs, \"NotZero?\"));  Decide( nterms =? 2 And? d >? 2, { result := FactorsBinomials(p); },  {  factorsList := BinaryFactors(p);    Decide( AllSatisfy?(\"Integer?\",coeffs), {  result := FactorsPolynomialOverIntegers(p,x); }, {   Local(notInt,rat,dd,lcm,newCoeffs,newPoly,facs); notInt := Select(coeffs, Lambda([i],Not? Integer?(i))); rat := Rationalize(coeffs);  dd := MapSingle(\"Denominator\",rat); lcm := Lcm(dd); newCoeffs := lcm * rat; newPoly := NormalForm(UniVariate(x,0,newCoeffs)); facs := FactorsPolynomialOverIntegers(newPoly); Decide( InVerboseMode(), { Echo(\"coeffs \",coeffs); Echo(\"notInt \",notInt); Echo(\"rat \",rat); Echo(\"dd \",dd); Echo(\"lcm \",lcm); Echo(\"newCoeffs \",newCoeffs); Echo(\"newPoly \",newPoly); Echo(\"facs \",facs); } ); result := [(1/lcm),1]~facs;  } ); } ); CombineNumericalFactors( result );};30 # Factors(p_CanBeUni) <--{  Local(vl,nvars,coeffs,result); vl := VarList(p); nvars := Length(vl); coeffs := Coef(p,x,0 .. 8); Decide(InVerboseMode(),Tell(\"CBU\",[vl,nvars,coeffs])); Decide(nvars >? 1,  { Decide( InVerboseMode(), Echo(\" special \",p)); result := FactorsMultivariateSpecialCases(p); },  result := FactorsSomethingElse(p)  ); CombineNumericalFactors( result );};40 # Factors(_p) <--{  Decide( InVerboseMode(), Echo(\"Possibly trying to factor polynomial with non-integral exponents\") ); Local( result);   result := [[p,1]]; CombineNumericalFactors( result ); };10 # FactorsMultivariateSpecialCases(-_expr) <-- [-1,1]~FactorsMultivariateSpecialCases(expr);10 # FactorsMultivariateSpecialCases(x_Atom? + y_Atom?) <-- {Decide(InVerboseMode(),Tell(1));[[x+y,1]];};10 # FactorsMultivariateSpecialCases(x_Atom? - y_Atom?) <-- {Decide(InVerboseMode(),Tell(2));[[x-y,1]];};10 # FactorsMultivariateSpecialCases(_n*_x^p_Integer? + _n*_y) <-- {Decide(InVerboseMode(),Tell(3));[n,1]~FactorsMultivariateSpecialCases(x+y);};10 # FactorsMultivariateSpecialCases(_n*_x^p_Integer? - _n*_y) <-- {Decide(InVerboseMode(),Tell(4));[n,1]~FactorsMultivariateSpecialCases(x-y);};10 # FactorsMultivariateSpecialCases(n_Integer?*_x + m_Integer?*_y)_(Gcd(n,m)>?1) <-- [[Gcd(n,m),1],[(Simplify((n*x+m*y)/Gcd(n,m))),1]];10 # FactorsMultivariateSpecialCases(n_Integer?*_x - m_Integer?*_y)_(Gcd(n,m)>?1) <-- [[Gcd(n,m),1],[(Simplify((n*x-m*y)/Gcd(n,m))),1]];10 # FactorsMultivariateSpecialCases(_n*_x + _n*_y) <-- [n,1]~FactorsMultivariateSpecialCases(x+y);10 # FactorsMultivariateSpecialCases(_n*_x - _n*_y) <-- [n,1]~FactorsMultivariateSpecialCases(x-y);10 # FactorsMultivariateSpecialCases(_x^n_Integer? - _y) <-- FactorsBinomials(x^n - y); 10 # FactorsMultivariateSpecialCases(_x^n_Integer? + _y) <-- FactorsBinomials(x^n + y); 20 # FactorsSomethingElse(_p) <--  { Decide( InVerboseMode(), { ECHO(\" *** FactorsSomethingElse: NOT IMPLEMENTED YET ***\"); } ); p; };10 # CombineNumericalFactors( factrs_List? ) <-- { Decide( InVerboseMode(), Tell(\"Combine\",factrs) ); Local(q,a,b,t,f,err); err := False; t := 1; f := []; ForEach(q,factrs) { Decide( InVerboseMode(), Tell(1,q) ); Decide( List?(q) And? Length(q)=?2, { [a,b] := q; Decide( InVerboseMode(), Echo(\" \",[a,b]) ); Decide( NumericList?( [a,b] ), t := t * a^b, f := [a,b]~f ); }, err := True ); }; Decide( InVerboseMode(), { Echo(\" t = \",t); Echo(\" f = \",f); Echo(\" err = \",err); } ); Decide(Not? err And? t !=? 1, [t,1]~Reverse(f), factrs); };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "50 # jFactorsPoly( poly_CanBeUni ) <--{ Decide(InVerboseMode(),Tell(\"jFactorsPoly_100612\",poly)); Local(content,pp,ppFactors,monomialFactors,result,moreDetails); Local(vars,nvars,disassem,degrees,mpoly,nterms,allCoeffs,allPowers); Local(hasRealCoefficients,hasRationalCoefficients,isHomogeneous); Local(extraFactor);  moreDetails := False; disassem := DisassembleExpression(poly); allCoeffs := disassem[3];   hasRealCoefficients := HasRealCoefficients(poly); Decide( hasRealCoefficients,  { Local(realPoly); realPoly := poly;  allPowers := Flatten(disassem[2],\"List\"); poly := RealToRationalConvert(poly);  hasRealCoefficients := False; } );   hasRationalCoefficients := HasRationalCoefficients(poly); Decide(hasRationalCoefficients,  { Local(rationalPoly); rationalPoly := poly;  Decide(InVerboseMode(),Tell(\" RATIONAL\",rationalPoly));  poly := RationalToIntegerConvert(poly);  hasRationalCoefficients := False;  } );  content := xContent( poly ); pp := Simplify( poly / content ); Decide(Assigned?(extraFactor),content := content / extraFactor); Decide(InVerboseMode(),Tell(\" \",[content,pp]));  Decide( Length(VarList(pp))=?0, result := [[pp*content,1]], {  vars := VarList(pp); nvars := Length(vars); disassem := DisassembleExpression(pp); nterms := Length(disassem[3]); degrees := []; allCoeffs := disassem[3]; allPowers := Flatten(disassem[2],\"List\"); Decide(nvars >? 0, { ForEach(v,vars) { DestructiveAppend(degrees,Degree(pp,v)); }; isHomogeneous := {   Local(sd,cmp);  sd := Sum /@ disassem[2]; cmp := FillList(sd[1],Length(sd)); ZeroVector?(sd - cmp); }; } );  Decide(InVerboseMode() And? moreDetails, { Tell(\" \",vars); Tell(\" \",nvars); Tell(\" \",nterms); Tell(\" \",degrees); Tell(\" \",disassem); Tell(\" \",allCoeffs); Tell(\" \",allPowers); Tell(\" \",isHomogeneous); NewLine(); } );    monomialFactors := FactorsMonomial(content); Decide(InVerboseMode(),Tell(\" \",monomialFactors));   ppFactors := jFactorsPrimitivePart( pp ); Decide(InVerboseMode(),Tell(\" \",ppFactors)); Decide( Not? ListOfLists?(ppFactors), { Local(L,op,var,exp); L := Decide(Atom?(ppFactors[1]), ppFactors, FunctionToList(ppFactors[1]) ); Decide(InVerboseMode(),Tell(\" \",L)); Decide( L[1] =? ^, ppFactors := [L[2],L[3]] ); ppFactors := [ppFactors]; } ); Decide(InVerboseMode(),Tell(\" \",ppFactors));   Decide( monomialFactors[1][1] =? 1, result := ppFactors,   result := Concat(monomialFactors,ppFactors)  ); } );  Decide(InVerboseMode(), { NewLine(); Tell(\" \",monomialFactors); Tell(\" \",ppFactors); Tell(\" final \",result); } ); result;};UnFence(\"jFactorsPoly\",1); 60 # jFactorsPrimitivePart( _pp )_(isHomogeneous And? nterms=?2 And? nvars=?2) <--{ Decide(InVerboseMode(),Tell(\"Bivariate Binomial\",pp)); Local(ppFactors,isDiagonal); isDiagonal := Diagonal?(disassem[2]);  ppFactors := Decide(isDiagonal,jFactorsBivariateBinomial(pp),jasFactorsInt(pp) );};UnFence(\"jFactorsPrimitivePart\",1); 65 # jFactorsPrimitivePart( _pp )_(isHomogeneous And? nterms>?1 And? nvars=?2) <--{ Decide(InVerboseMode(),Tell(\"Homogeneous and Bivariate\")); Local(ppFactors); ppFactors := jFactorsHomogeneousBivariate(disassem);};UnFence(\"jFactorsPrimitivePart\",1); 70 # jFactorsPrimitivePart( _pp )_(nvars=?0) <--{ Local(ppFactors); ppfactors := [];}; 100 # jFactorsPrimitivePart( _pp ) <-- { Decide(InVerboseMode(),Tell(\"jFactorsPrimitivePart_usingJAS\",pp)); Local(answer); answer := Decide(Monomial?(pp),[pp,1],jasFactorsInt(pp)); Decide(InVerboseMode(),Tell(\" \",answer)); answer;};UnFence(\"jFactorsPrimitivePart\",1);10 # jFactorsHomogeneousBivariate( dis_List? ) <-- { Decide(InVerboseMode(),{NewLine();Tell(\"jFactorsHomogeneousBivariate\",dis);}); Local(dis1,f,dis2,poly1,ppFactors,residuals); Local(ii,lst,f,preassem); dis1 := [[xi],[[X],[X[1]]] /@ dis[2],dis[3]]; Decide(InVerboseMode(),Tell(\" \",dis1)); poly1 := Sum(ReassembleListTerms(dis1)); Decide(InVerboseMode(),Tell(\" \",poly1)); ppFactors := BinaryFactors(poly1); [ppFactors,residuals] := FixUpMonicFactors(ppFactors); For(ii:=1,ii<=?Length(ppFactors),ii++) { f := ppFactors[ii]; Decide(InVerboseMode(),Tell(\" \",f[1])); lst := DisassembleExpression(f[1]); Decide(InVerboseMode(), { Tell(\" \",lst); Tell(\" \",dis[1]); } ); DestructiveReplace(lst,1,dis[1]); DestructiveAppend(lst[2][1],0); DestructiveAppend(lst[2][2],1); Decide(Length(lst[2])=?3, DestructiveAppend(lst[2][3],2)); Decide(InVerboseMode(),Tell(\" \",lst)); preassem := Sum(ReassembleListTerms(lst)) ; Decide(InVerboseMode(),Tell(\" \",preassem)); ppFactors[ii][1] := preassem; }; Decide(InVerboseMode(),{Tell(\" \",ppFactors); Tell(\" \",residuals);NewLine();} );  ppFactors;};UnFence(\"jFactorsHomogeneousBivariate\",1);10 # RealToRationalConvert( poly_Polynomial? ) <--{   Decide(InVerboseMode(),{NewLine();Tell(\" REAL\",poly);}); Local(coeffs,gcd,lcm); coeffs := Rationalize /@ (allCoeffs); Decide(InVerboseMode(),{Tell(\" to-Q\",coeffs);Tell(\" to-Z\",coeffs);}); Local(gcd,lcm); gcd := Gcd(Numerator /@ coeffs); lcm := Lcm(Denominator /@ coeffs); Decide(InVerboseMode(),{Tell(\" \",gcd);Tell(\" \",lcm);}); disassem[3] := coeffs; allCoeffs := coeffs; poly := Sum(ReassembleListTerms(disassem)); Decide(InVerboseMode(),Tell(\" new\",poly)); poly;};UnFence(\"RealToRationalConvert\",1);10 # RationalToIntegerConvert( poly_Polynomial? ) <--{   Local(coeffs,gcd,lcm); coeffs := allCoeffs; Decide(InVerboseMode(),Tell(\" \",coeffs)); lcm := Lcm(Denominator /@ coeffs); extraFactor := lcm; Decide(InVerboseMode(),{Tell(\" \",extraFactor);}); poly := Simplify(extraFactor*poly); Decide(InVerboseMode(),Tell(\" new \",poly)); poly;};UnFence(\"RationalToIntegerConvert\",1);100 # CombineNumericalFactors( factrs_List? ) <--{ Decide( InVerboseMode(), Tell(\"Combine\",factrs) ); Local(q,a,b,t,f,ff,err); err := False; t := 1; f := []; ForEach(q,factrs) { Decide( InVerboseMode(), Tell(1,q) ); Decide( List?(q) And? Length(q)=?2, { [a,b] := q; Decide( InVerboseMode(), Echo(\" \",[a,b]) ); Decide( NumericList?( [a,b] ), t := t * a^b, f := [a,b]~f ); }, err := True ); }; Decide( InVerboseMode(), { Echo(\" t = \",t); Echo(\" f = \",f); Echo(\" err = \",err); } ); ff := Decide(Not? err And? t !=? 1, [t,1]~Reverse(f), factrs); ff := Select(Lambda([x],x!=?[1,1]),ff); Decide(ff[1]<?0,ff[1]:=-ff[1]);};150 # jFactors( expr_RationalFunction? )_ (Polynomial?(Numerator(expr)) And? Polynomial?(Denominator(expr))) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"jFactors_Rational_Function\",expr);}); Local(Numer,Denom,fNumer,fDenom); Numer := Numerator(expr); Denom := Denominator(expr); fNumer := jFactors(Numer); fDenom := jFactors(Denom); Decide(InVerboseMode(),{Tell(\" \",fNumer); Tell(\" \",fDenom);}); fNumer/fDenom;};152 # jFactors( expr_RationalFunction? )_ (Constant?(Numerator(expr)) And? Polynomial?(Denominator(expr))) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"jFactors_Rational_Denom\",expr);}); Local(Numer,Denom,fNumer,fDenom); Numer := Numerator(expr); Denom := Denominator(expr); fNumer := jFactors(Numer); fDenom := jFactors(Denom); Decide(InVerboseMode(),{Tell(\" \",fNumer); Tell(\" \",fDenom);}); fNumer/fDenom;};200 # jFactors( _expr )_(Length(VarList(expr)) =? 1) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"Some other kind of expression\",expr);}); Local(dis,X,pows); dis := DisassembleExpression(expr); X := VarList(expr)[1]; pows := matchPower /@ dis[1]; rats := NearRational /@ pows; dis[1] := x^rats; p := Sum(ReassembleListTerms(dis)); Decide(InVerboseMode(),Tell(\" new \",p)); jFactors(p);};10 # jFactorsBivariateBinomial( poly_Polynomial? )_(Length(VarList(poly))=?2) <--{ Decide(InVerboseMode(),Tell(jFactorsBivariateBinomial,poly)); Local(dis,n,X,Y,vars,A,B,s,Ar,Br,Arr,Brr,DAr,DBr,result); dis := DisassembleExpression(poly); Decide(InVerboseMode(),Tell(\" \",dis)); n := Maximum(dis[2])[1]; X := dis[1][1]; Y := dis[1][2]; vars := dis[1]; A := Abs(dis[3][1]); B := Abs(dis[3][2]); s := Sign(dis[3][1]*dis[3][2]);   Ar := NM(A^(1/n)); Arr := Round(Ar); DAr := Abs(Ar-Arr); Br := NM(B^(1/n)); Brr := Round(Br); DBr := Abs(Br-Brr); Decide(InVerboseMode(), { Tell(\" \",[n,X,Y]);  Tell(\" \",[vars,A,B]); Tell(\" \",[Ar,Br,s]); Tell(\" \",[Arr,Brr]); Tell(\" \",[DAr,DBr]); Tell(\" \",dis); } ); result := Decide( DAr <? 10^(-9) And? DBr <? 10^(-9), jFB(dis), [[poly,1]] ); result;};UnFence(\"jFactorsBivariateBinomial\",1);50 # jFB( dis_List? )_(Length(dis)=?3 And? Length(dis[3])=?2) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"jFB\",dis);}); Local(ns,ii,fn,mx,my,fac); Decide(InVerboseMode(), { Tell(\" \",n); Tell(\" \",[X,Y]); Tell(\" \",[A,B,s]); Tell(\" \",[Ar,Br]); } ); X := Arr*X; Y := Brr*Y; Decide(InVerboseMode(),Tell(\" \",[X,Y]));  fac := jFac( X/Y,n,s);  Decide(InVerboseMode(), { NewLine(); Tell(\" \",X/Y); Tell(\" \",fac); } );   Decide( Y !=? 1, { Local(f,d,fs); For(ii:=1,ii<=?Length(fac),ii++) { f := fac[ii][1]; d := Degree(f,x); Decide(InVerboseMode(),Tell(\" \",[ii,f,d])); fs := Subst(x,X/Y) f; Decide(InVerboseMode(),Tell(\" \",[fs,d])); fac[ii][1] := Simplify(Simplify(Y^d*fs)); }; } ); fac;};UnFence(\"jFB\",1);60 # jFac( _var, n_PositiveInteger?, s_Integer? ) <--{  Decide(InVerboseMode(),{NewLine();Tell(\"jFac\",[var,n,s]);}); Local(x,poly,result); poly := x^n+s; Decide(InVerboseMode(),Tell(\" \",poly)); result := jasFactorsInt(poly); };UnFence(\"jFac\",3);10 # IsPureRational( N_Rational? )_(Not? Integer?(N)) <-- True;12 # IsPureRational( _N ) <-- False;10 # HasRealCoefficients( poly_Polynomial? ) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"Decimal?\")) >? 0);};10 # HasRealCoefficients( poly_Monomial? ) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"Decimal?\")) >? 0);};10 # HasRationalCoefficients( poly_Polynomial? ) <--{ Local(disassem,answer); Decide(InVerboseMode(),Tell(\" HasRationalCoefficients\",poly)); disassem := DisassembleExpression(poly);  answer := (Length(Select(disassem[3],\"IsPureRational\")) >? 0); Decide(InVerboseMode(),Tell(\" \",answer)); answer;};10 # HasRationalCoefficients( poly_Monomial?) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"IsPureRational\")) >? 0);};10 # FixUpMonicFactors( factrs_List? ) <--{ Decide(InVerboseMode(),{ NewLine(); Tell(\" doing monic fixup\"); } ); Local(factrsnew,residuals,f,uni,); factrsnew := []; residuals := []; ForEach(f,factrs) { Decide(InVerboseMode(),Tell(\" \",f)); uni := MakeUni(f[1]); Decide(InVerboseMode(),Tell(\" \",uni)); Decide( Degree(f[1])=?1, { Local(cc,lcm,fnew); Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); cc := Coef(f[1],uni[1],0 .. 1);  lcm := Lcm( Denominator /@ cc ); uni[3] := lcm * cc; fnew := NormalForm(uni); Decide( hasRationalCoefficients, { DestructiveAppend(factrsnew,f); }, {  DestructiveAppend(factrsnew,[fnew,f[2]]); } ); } ); Decide( Degree(f[1])=?2, { Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); Local(pq); pq := PrimitivePart(f[1]); DestructiveAppend(factrsnew,[pq,f[2]]); } );   Decide( Degree(f[1]) >? 2, { Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); Local(pq); pq := PrimitivePart(f[1]); DestructiveAppend(residuals,[pq,f[2]]); Decide(InVerboseMode(),Tell(\" appending to residuals\",pq)); } );  }; [factrsnew,residuals];};UnFence(\"FixUpMonicFactors\",1);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PollardRhoPolynomial(_x) <-- x^2+1;2# PollardRhoFactorize(n_PrimePower?) <-- [GetPrimePower(n)];3# PollardRhoFactorize(_n) <--{ Local(x,y,restarts,gcd,repeat); gcd:=1; restarts := 100;  While(gcd =? 1 And? restarts>=?0)  { restarts--;  x:= RandomInteger(n-1);  gcd:=1; y:=x; repeat := 4;   While(gcd =? 1 And? repeat>=?0) { x:= Modulo( PollardRhoPolynomial(x), n); y:= Modulo( PollardRhoPolynomial( Modulo( PollardRhoPolynomial(y), n)  ), n); Decide(x-y =? 0, { gcd := 1; repeat--;  }, gcd:=Gcd(x-y,n) ); }; Decide(InVerboseMode() And? repeat<=?0, Echo([\"PollardRhoFactorize: Warning: stalled while factorizing \", n, \"; counters \", x, y])); }; Check(restarts>?0, \"Math\", \"PollardRhoFactorize: Error: failed to factorize \" ~ ToString(n)); Decide(InVerboseMode() And? gcd >? 1, Echo([\"PollardRhoFactorize: Info: while factorizing \", n, \" found factor \", gcd]));  PollardCombineLists(PollardRhoFactorize(gcd), PollardRhoFactorize(Quotient(n,gcd)));};";
        scriptMap.put("PollardRhoFactorize",scriptString);
        scriptMap.put("PollardRhoPolynomial",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # xContent( poly_CanBeUni ) <--{ Local(disassem,gcdCoefs,lc,minExpts); disassem := DisassembleExpression(poly); gcdCoefs := Gcd(disassem[3]); lc := LeadingCoef(poly); Decide(NegativeNumber?(lc) And? gcdCoefs >? 0, gcdCoefs:=-gcdCoefs);   gcdCoefs;};";
        scriptMap.put("xContent",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # xFactor( p_CanBeUni ) <-- FW(xFactors(p));10 # xFactor( p_RationalFunction? ) <--{ Local(fs,n,d); fs := xFactors( p ); n := FW(Numerator(fs)); d := FW(Denominator(fs)); n/d;};10 # xFactor( L_List? ) <-- { Local (result,x,f); result := []; ForEach(x,L) { f := xFactors(x); Decide( f =? [], f := 0, f := FW(f) ); DestructiveAppend(result,f); }; result;};20 # xFactor( _expr ) <-- expr;";
        scriptMap.put("xFactor",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # xFactors( L_List? ) <-- xFactors /@ L;10 # xFactors(p_PositiveInteger?) <-- { Decide( p <? 1600, FactorsSmallInteger(p), FactorizeInt(p) );};12 # xFactors(p_NegativeInteger?) <-- xFactors(-p);14 # xFactors(p_Rational?)_(Denominator(p) !=? 1) <--  [ [xFactor(Numerator(p)) / xFactor(Denominator(p) ) , 1] ];  16 # xFactors(p_GaussianInteger?) <-- GaussianFactors(p);18 # xFactors(_p)_(Length(VarList(p))=?0) <-- [[p,1]];21 # xFactors( poly_CanBeUni ) <--{ Decide(InVerboseMode(),Tell(\"xFactors_can_be_uni_100122\",poly)); Local(content,pp,ppFactors,monomialFactors,result); Local(vars,nvars,disassem,degrees,mpoly,nterms,allCoeffs,allPowers); Local(hasRealCoefficients,hasRationalCoefficients,isHomogeneous);   hasRealCoefficients := HasRealCoefficients(poly); Decide( hasRealCoefficients,  { Local(realPoly); realPoly := poly;  disassem := DisassembleExpression(poly); allCoeffs := disassem[3]; allPowers := Flatten(disassem[2],\"List\"); poly := ApproximateRealCoefficients(poly); } );   hasRationalCoefficients := HasRationalCoefficients(poly);   content := xContent( poly ); pp := xPrimitivePart( poly, content ); Decide(InVerboseMode(),Tell(\" \",[content,pp])); vars := VarList(pp); nvars := Length(vars); disassem := DisassembleExpression(pp); nterms := Length(disassem[3]); degrees := []; allCoeffs := disassem[3]; allPowers := Flatten(disassem[2],\"List\"); Decide(nvars >? 0, { ForEach(v,vars) { DestructiveAppend(degrees,Degree(pp,v)); }; isHomogeneous := {   Local(sd,cmp);  sd := Sum /@ disassem[2]; cmp := FillList(sd[1],Length(sd)); ZeroVector?(sd - cmp); }; } );     pp := MetaSet(pp,\"nvars\",nvars); pp := MetaSet(pp,\"nterms\",nterms); pp := MetaSet(pp,\"degrees\",degrees); pp := MetaSet(pp,\"isHomogeneous\",isHomogeneous);  Decide(InVerboseMode(), { Tell(\" \",vars); Tell(\" \",nvars); Tell(\" \",nterms); Tell(\" \",degrees); Tell(\" \",disassem); Tell(\" \",allCoeffs); Tell(\" \",allPowers); Tell(\" \",isHomogeneous); NewLine(); } );   ppFactors := xFactorsPrimitivePart( pp ); Decide(InVerboseMode(),{NewLine();Tell(\" \",ppFactors);});   Decide(InVerboseMode(),NewLine()); monomialFactors := FactorsMonomial(content); Decide(InVerboseMode(),{Tell(\" \",monomialFactors);}); Decide( monomialFactors[1][1] =? 1, result := ppFactors,  result := Concat(monomialFactors,ppFactors) );  Decide(InVerboseMode(),{NewLine();Tell(\" final \",result);}); result;}; 10 # xFactorsPrimitivePart( _pp )_(nterms=?2) <--{ Decide(InVerboseMode(),Tell(\"Binomial\")); Local(ppFactors); ppFactors := xFactorsBinomial(pp);};UnFence(\"xFactorsPrimitivePart\",1); 12 # xFactorsPrimitivePart( _pp )_(isHomogeneous And? nvars=?2) <--{ Decide(InVerboseMode(),Tell(\"Homogeneous and Bivariate\")); Local(ppFactors); ppFactors := xFactorsHomogeneousBivariate(disassem);};UnFence(\"xFactorsPrimitivePart\",1); 14 # xFactorsPrimitivePart( _pp )_(nvars=?0) <--{ Local(ppFactors); ppfactors := [];}; 16 # xFactorsPrimitivePart( _pp )_(nvars=?1) <-- xFactorsUnivariate(pp);UnFence(\"xFactorsPrimitivePart\",1); 18 # xFactorsPrimitivePart( _pp )_(nvars>?1) <-- xFactorsMultivariate(pp);UnFence(\"xFactorsPrimitivePart\",1); 20 # xFactorsPrimitivePart( _pp ) <-- Tell(\"Should never get here!\");UnFence(\"xFactorsPrimitivePart\",1);30 # xFactorsUnivariate( poly_CanBeUni )_(Length(VarList(poly))=?1) <--{ Local(factrs,coeffs,deg,X,residuals,factrsnew); Decide(InVerboseMode(), { NewLine(); Tell(\"xFactorsUnivariate\",poly); Tell(\" \",allCoeffs); } );     factrs := BinaryFactors(poly); Decide(InVerboseMode(),Tell(\" output of BinaryFactors\",factrs));      [factrsnew,residuals] := FixUpMonicFactors(factrs);   Local(residOut); residOut := []; Decide(Length(residuals) >? 0, residOut := xFactorsResiduals( residuals ) );  Decide(InVerboseMode(), { NewLine(); Tell(\" just before end of univariate factoring\"); Tell(\" \",factrs); Tell(\" \",factrsnew); Tell(\" \",residOut); } );   Local(final); Decide(Length(Union(factrsnew,residOut)) >? 0, final := Concat(factrsnew,residOut), final := factrs ); CheckForSpecialForms( final );}; UnFence(\"xFactorsUnivariate\",1);40 # xFactorsMultivariate( poly_CanBeUni )_(Length(VarList(poly))>?1) <--{ Local(factrs);  Decide(InVerboseMode(),{NewLine();Tell(\"xFactorsMultivariate\",poly);}); Decide( nterms =? 2, { Decide(InVerboseMode(),Tell(\" Is Binomial\")); factrs := xFactorsBinomial(poly);  }, { Decide(InVerboseMode(),Tell(\" Has more than 2 terms\")); } ); factrs;};UnFence(\"xFactorsMultivariate\",1);10 # xFactorsHomogeneousBivariate( dis_List? ) <-- { Decide(InVerboseMode(),{NewLine();Tell(\"xFactorsHomogeneousBivariate\",dis);}); Local(dis1,f,lst,dis2,poly1,ppFactors,residuals,ii,preassem); dis1 := [[xi],[[X],[X[1]]] /@ dis[2],dis[3]]; Decide(InVerboseMode(),Tell(\" \",dis1)); poly1 := Sum(ReassembleListTerms(dis1)); Decide(InVerboseMode(),Tell(\" \",poly1)); ppFactors := BinaryFactors(poly1); [ppFactors,residuals] := FixUpMonicFactors(ppFactors); For(ii:=1,ii<=?Length(ppFactors),ii++) { f := ppFactors[ii]; Decide(InVerboseMode(),Tell(\" \",f[1])); lst := DisassembleExpression(f[1]); Decide(InVerboseMode(), { Tell(\" \",lst); Tell(\" \",dis[1]); } ); DestructiveReplace(lst,1,dis[1]); DestructiveAppend(lst[2][1],0); DestructiveAppend(lst[2][2],1); Decide(InVerboseMode(),Tell(\" \",lst)); preassem := Sum(ReassembleListTerms(lst)) ; Decide(InVerboseMode(),Tell(\" \",preassem)); ppFactors[ii][1] := preassem; }; Decide(InVerboseMode(),{Tell(\" \",ppFactors); Tell(\" \",residuals);NewLine();} );  ppFactors;};UnFence(\"xFactorsHomogeneousBivariate\",1);10 # CheckForSpecialForms( final_List? ) <-- { Decide(InVerboseMode(),{NewLine();Tell(\"CheckForSpecialForms\",final);}); Local(LL,ii,fact,mult,dis,new); new := []; LL := Length(final); For(ii:=1,ii<=?LL,ii++) { fact := final[ii][1]; mult := final[ii][2]; Decide(InVerboseMode(),Tell(\" \",[fact,mult])); dis := DisassembleExpression( fact ); Decide(InVerboseMode(),Tell(\" \",dis)); Local(var); var := dis[1][1]; If( dis[2]=?[[4],[2],[0]] And? dis[3]=?[1,1,1] ) { Local(new1,new2); new1 := [var^2-var+1,mult]; new2 := [var^2+var+1,mult]; DestructiveAppend(new,new1); DestructiveAppend(new,new2); Decide(InVerboseMode(),Tell(\" \",new)); } Else { Decide(InVerboseMode(),Tell(\" no special form\")); DestructiveAppend(new,[fact,mult]); }; ); }; new;};10 # ApproximateRealCoefficients( poly_Polynomial? ) <--{   Decide(InVerboseMode(),{NewLine();Tell(\" REAL\",poly);}); Local(coeffs,gcd,lcm); coeffs := Rationalize /@ (allCoeffs); Decide(InVerboseMode(),{Tell(\" to-Q\",coeffs);Tell(\" to-Z\",coeffs);}); Local(gcd,lcm); gcd := Gcd(Numerator /@ coeffs); lcm := Lcm(Denominator /@ coeffs); Decide(InVerboseMode(),{Tell(\" \",gcd);Tell(\" \",lcm);}); disassem[3] := coeffs; allCoeffs := coeffs; poly := Sum(ReassembleListTerms(disassem)); Decide(InVerboseMode(),Tell(\" new\",poly)); poly;};UnFence(\"ApproximateRealCoefficients\",1);100 # CombineNumericalFactors( factrs_List? ) <--{ Decide( InVerboseMode(), Tell(\"Combine\",factrs) ); Local(q,a,b,t,f,ff,err); err := False; t := 1; f := []; ForEach(q,factrs) { Decide( InVerboseMode(), Tell(1,q) ); Decide( List?(q) And? Length(q)=?2, { [a,b] := q; Decide( InVerboseMode(), Echo(\" \",[a,b]) ); Decide( NumericList?( [a,b] ), t := t * a^b, f := [a,b]~f ); }, err := True ); }; Decide( InVerboseMode(), { Echo(\" t = \",t); Echo(\" f = \",f); Echo(\" err = \",err); } ); ff := Decide(Not? err And? t !=? 1, [t,1]~Reverse(f), factrs); ff := Select(Lambda([x],x!=?[1,1]),ff); Decide(ff[1]<?0,ff[1]:=-ff[1]);};150 # xFactors( expr_RationalFunction? )_ (Polynomial?(Numerator(expr)) And? Polynomial?(Denominator(expr))) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"xFactors_Rational_Function\",expr);}); Local(Numer,Denom,fNumer,fDenom); Numer := Numerator(expr); Denom := Denominator(expr); fNumer := xFactors(Numer); fDenom := xFactors(Denom); Decide(InVerboseMode(),{Tell(\" \",fNumer); Tell(\" \",fDenom);}); fNumer/fDenom;};152 # xFactors( expr_RationalFunction? )_ (Constant?(Numerator(expr)) And? Polynomial?(Denominator(expr))) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"xFactors_Rational_Denom\",expr);}); Local(Numer,Denom,fNumer,fDenom); Numer := Numerator(expr); Denom := Denominator(expr); fNumer := xFactors(Numer); fDenom := xFactors(Denom); Decide(InVerboseMode(),{Tell(\" \",fNumer); Tell(\" \",fDenom);}); fNumer/fDenom;};200 # xFactors( _expr )_(Length(VarList(expr)) =? 1) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"Some other kind of expression\",expr);}); Local(dis,X,pows); dis := DisassembleExpression(expr); X := VarList(expr)[1]; pows := matchPower /@ dis[1]; rats := NearRational /@ pows; dis[1] := x^rats; p := Sum(ReassembleListTerms(dis)); Decide(InVerboseMode(),Tell(\" new \",p)); xFactors(p);};10 # IsPureRational( N_Rational? )_(Not? Integer?(N)) <-- True;12 # IsPureRational( _N ) <-- False;10 # HasRealCoefficients( poly_Polynomial? ) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"Decimal?\")) >? 0);};10 # HasRealCoefficients( poly_Monomial? ) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"Decimal?\")) >? 0);};10 # HasRationalCoefficients( poly_Polynomial? ) <--{ Local(disassem); disassem := DisassembleExpression(poly);  (Length(Select(disassem[3],\"IsPureRational\")) >? 0);};10 # HasRationalCoefficients( poly_Monomial?) <--{ Local(disassem); disassem := DisassembleExpression(poly); (Length(Select(disassem[3],\"IsPureRational\")) >? 0);};10 # FixUpMonicFactors( factrs_List? ) <--{ Decide(InVerboseMode(),{ NewLine(); Tell(\" doing monic fixup\"); } ); Local(factrsnew,residuals,uni); factrsnew := []; residuals := []; ForEach(f,factrs) { Decide(InVerboseMode(),Tell(\" \",f)); uni := MakeUni(f[1]); Decide(InVerboseMode(),Tell(\" \",uni)); Decide( Degree(f[1])=?1, { Local(cc,lcm,fnew); Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); cc := Coef(f[1],uni[1],0 .. 1);  lcm := Lcm( Denominator /@ cc ); uni[3] := lcm * cc; fnew := NormalForm(uni); Decide( hasRationalCoefficients, { DestructiveAppend(factrsnew,f); }, {  DestructiveAppend(factrsnew,[fnew,f[2]]); } ); } ); Decide( Degree(f[1])=?2, { Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); Local(pq); pq := PrimitivePart(f[1]); DestructiveAppend(factrsnew,[pq,f[2]]); } );   Decide( Degree(f[1]) >? 2, { Decide(InVerboseMode(),Tell(\" \",Degree(f[1]))); Local(pq); pq := PrimitivePart(f[1]); DestructiveAppend(residuals,[pq,f[2]]); Decide(InVerboseMode(),Tell(\" appending to residuals\",pq)); } );  }; [factrsnew,residuals];};UnFence(\"FixUpMonicFactors\",1);10 # IsIrreducible( poly_Polynomial? )_(Length(VarList(poly))=?1) <--{      Decide(InVerboseMode(),Tell(\"IsIrreducible\",poly)); Local(var,deg,coeffs,num1); var := VarList(poly)[1]; deg := Degree(poly); coeffs := Coef(poly,var,deg .. 0); Decide(InVerboseMode(),Tell(\" \",deg)); Local(ii,res,nprimes); nprimes := 0; For(ii:=-3*deg,ii<=?3*deg,ii:=ii+3) { res := NM(Subst(x,ii) poly);  Decide(Abs(res)=?1 Or? Prime?(res), nprimes := nprimes + 1, ); }; Tell(\" \",nprimes); Decide(nprimes >? 2*deg, True, False );};10 # matchPower(_Z^n_Number?) <-- n;15 # matchPower(_Z) <-- 1;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Retract(\"xFactorsBinomial\",*);Retract(\"xFB1\",*);Retract(\"xFB2\",*);Retract(\"IsPowerOf2\",*);10 # xFactorsBinomial( poly_Polynomial? )_(Length(VarList(poly))=?1) <-- { Decide(InVerboseMode(),Tell(xFactorsBinomial,poly)); Local(dis,n,X,var,A,B,s,Ar,Br); dis := DisassembleExpression(poly); Decide(InVerboseMode(),Tell(\" \",dis)); n := Maximum(dis[2])[1]; X := dis[1][1]; var := dis[1][1]; A := Abs(dis[3][1]); B := Abs(dis[3][2]); s := Sign(dis[3][1]*dis[3][2]); Ar := NearRational(NM(A^(1/n),20)); Br := NearRational(NM(B^(1/n),20)); Decide(InVerboseMode(),{Tell(\" \",[n,X,var,A,B]); Tell(\" \",[Ar,Br,s]);}); Decide( Integer?(Ar) And? Integer?(Br), xFB1(dis), [[poly,1]] );};10 # xFactorsBinomial( poly_Polynomial? )_(Length(VarList(poly))=?2) <-- { Decide(InVerboseMode(),Tell(xFactorsBinomial,poly)); Local(dis,n,X,Y,vars,A,B,s,Ar,Br); dis := DisassembleExpression(poly); Decide(InVerboseMode(),Tell(\" \",dis)); n := Maximum(dis[2])[1]; X := dis[1][1]; Y := dis[1][2]; vars := dis[1]; A := Abs(dis[3][1]); B := Abs(dis[3][2]); s := Sign(dis[3][1]*dis[3][2]); Ar := NearRational(NM(A^(1/n))); Br := NearRational(NM(B^(1/n))); Decide(InVerboseMode(), { Tell(\" \",[n,X,Y]);  Tell(\" \",[vars,A,B]); Tell(\" \",[Ar,Br,s]); } ); Decide( Integer?(Ar) And? Integer?(Br), xFB2(dis), [[poly,1]] );};12 # xFB1( dis_List? )_(Length(dis)=?3 And? Length(dis[3])=?2) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"xFB1\",dis);}); Local(Y,y,ii,fac1); X := Ar*X; Y := Br; Unassign(y); y := 1; Decide(InVerboseMode(), Tell(\" \",[X,Y])); fac1 := xFB1( X/Y,n,s);   Decide( InVerboseMode(),Tell(\" \",fac1));   Decide( Y !=? 1, { Local(f,d); For(ii:=1,ii<=?Length(fac1),ii++) { f := fac1[ii][1]; d := Degree(f,var); Decide(InVerboseMode(),Tell(\" \",[ii,f,d])); fac1[ii][1] := Simplify(Y^d*f); }; } ); fac1;};UnFence(\"xFB1\",1);15 # xFB1(_X,n_SmallPrime?,s_NotZero?)_(Odd?(n)) <--{ Local(ans,k); Decide(InVerboseMode(),{NewLine();Tell(\" xFB1prime\",[X,n,s]);}); ans := [[X+s,1]]; Decide( n >? 1, ans := Concat(ans,[[Sum(k,0,n-1,(-s)^k*X^(n-1-k)),1]]) ); Decide(InVerboseMode(),Tell(\" \",ans)); ans;};UnFence(\"xFB1\",3);20 # xFB1(_X, n_Odd?, s_PositiveInteger?) <--{ Local(ans,ans1); Decide(InVerboseMode(),{NewLine(); Tell(\" xFB1oddsum\",[X,Y,n]);}); If( n =? 9 ) { ans := [[X+1,1],[X^2-X+1,1],[X^6-X^3+1,1]]; } Else If( n =? 15 ) { ans := [[X+1,1],[X^2-X+1,1],[X^4-X^3+X^2-X+1,1],[X^8+X^7-X^5-X^4-X^3+X+1,1]]; } Else If( n =? 21 ) { ans := [[X+1,1],[X^2-X+1,1],[X^6-X^5+X^4-X^3+X^2-X+1,1],[X^12+X^11-X^9-X^8+X^6-X^4-X^3+X+1,1]]; } Else If( n =? 25 ) { ans := [[X+1,1],[X^4-X^3+X^2-X+1,1],[X^20-X^15+X^10-X^5+1,1]]; } Else If( n =? 35 ) { ans := [[X+1,1],[X^4-X^3+X^2-X+1,1],[X^6-X^5+X^4-X^3+X^2-X+1,1],[X^24+X^23-X^19-X^18-X^17-X^16+X^14+X^13+X^12+X^11+X^10-X^8-X^7-X^6-X^5+X+1,1]]; } Else If( n =? 45 ) { ans := [[X+1,1],[X^2-X+1,1],[X^4-X^3+X^2-X+1,1],[X^6-X^3+1,1],[X^8+X^7-X^5-X^4-X^3+X+1,1],[X^24+X^21-X^15-X^12-X^9+X^3+1,1]]; } Else { ans := BinaryFactors(X^n+1); };  ans;};25 # xFB1(_X,n_Odd?, s_NegativeInteger?) <--{ Local(ans); Decide(InVerboseMode(),{NewLine(); Tell(\" xFB1odddif\",[X,n]);}); If( n =? 9 ) { ans := [[X-1,1],[X^2+X+1,1],[X^6+X^3+1,1]]; } Else If( n =? 15 ) { ans := [[X-1,1],[X^2+X+1,1],[X^4+X^3+X^2+X+1,1],[X^8-X^7+X^5-X^4+X^3-X+1,1]]; } Else If( n =? 21 ) { ans := [[X-1,1],[X^2+X+1,1],[X^6+X^5+X^4+X^3+X^2+X+1,1],[X^12-X^11+X^9-X^8+X^6-X^4+X^3-X+1,1]]; } Else If( n =? 25 ) { ans := [[X-1,1],[X^4+X^3+X^2+X+1,1],[X^20+X^15+X^10+X^5+1,1]]; } Else If( n =? 35 ) { ans := [[X-1,1],[X^4+X^3+X^2+X+1,1],[X^6+X^5+X^4+X^3+X^2+X+1,1],[X^24-X^23+X^19-X^18+X^17-X^16+X^14-X^13+X^12-X^11+X^10-X^8+X^7-X^6+X^5-X+1,1]]; } Else If( n =? 45 ) { ans := [[X-1,1],[X^2+X+1,1],[X^4+X^3+X^2+X+1,1],[X^6+X^3+1,1],[X^8-X^7+X^5-X^4+X^3-X+1,1],[X^24-X^21+X^15-X^12+X^9-X^3+1,1]]; } Else { ans := BinaryFactors(X^n-1); };  ans; Decide(InVerboseMode(),Tell(\" \",ans)); ans;};30 # xFB1(_X, n_Even?, s_PositiveInteger?) <--{ Local(ans,fn,mx,my); Decide(InVerboseMode(),{NewLine(); Tell(\" xFB1evensum\",[X,n]);}); fn := [[1,1]]; Decide( n >? 1, fn := FactorsSmallInteger(n) ); Decide(Length(fn)=?1 And? Odd?(fn[1][1]), mx:= fn[1][1]^(fn[1][2]-1)); Decide(Length(fn)>?1, ForEach(f,fn) { Decide( Odd?(f[1]), mx := f[1]^f[2] ); }); my := n/mx; Decide(InVerboseMode(),Tell(\" \",[mx,my])); Decide( IsPowerOf2(n), {  ans := [[X^n+1,1]]; }, {  If( n =? 6 ) { ans := [[X^2+1,1],[X^4-X^2+1,1]]; } Else If( n =? 10 ) { ans := [[X^2+1,1],[X^8-X^6+X^4-X^2+1,1]]; } Else If( n =? 20 ) { ans := [[X^4+1,1],[X^16-X^12+X^8-X^4+1,1]]; } Else If( n =? 30 ) { ans := [[X^2+1,1],[X^4-x^2+1,1],[X^8-X^6+X^4-X^2+1,1],[X^16+X^14-X^10-X^8-X^6+X^2+1,1]]; } Else If( n =? 40 ) { ans := [[X^8+1,1],[X^32-X^24+X^16-X^8+1,1]]; } Else If( n =? 50 ) { ans := [[X^2+1,1],[X^8-X^6+X^4-X^2+1,1],[X^40-X^30+X^20-X^10+1,1]]; } Else If( n =? 100 ) { ans := [[X^4+1,1],[X^16-X^12+X^8-X^4+1,1],[X^80-X^60+X^40-X^20+1,1]]; } Else { ans := [[X^my+1,1],[Sum(k,0,mx-1,X^(n-my-k*my)*(-1)^k),1]]; }; } ); Decide(InVerboseMode(),Tell(\" \",ans)); ans;};35 # xFB1(_X, n_Even?, s_NegativeInteger?) <--{ Local(ans); Decide(InVerboseMode(),{NewLine(); Tell(\" xFB1evendif\",[X,n]);}); If( n =? 2 ) { ans := [[X-1,1],[X+1,1]]; } Else If( n =? 10 ) { ans := [[X-1,1],[X+1,1],[X^4+X^3+X^2+X+1,1],[X^4-X^3+X^2-X+1,1]]; } Else If( n =? 20 ) { ans := [[X-1,1],[X+1,1],[X^2+1,1],[X^4+X^3+X^2+X+1,1],[X^4-X^3+X^2-X+1,1],[X^8-X^6+X^4-X^2+1,1]]; } Else If( n =? 30 ) { ans := [[X-1,1],[X+1,1],[X^2+X+1,1],[X^2-X+1,1],[X^4+X^3+x^2+X+1,1],[X^4-X^3+x^2-X+1,1],[X^8-X^7+X^5-X^4+X^3-X+1,1],[X^8+X^7-X^5-X^4-X^3+X+1,1]]; } Else If( n =? 40 ) { ans := [[X-1,1],[X+1,1],[X^2+1,1],[X^4+1,1],[X^4+X^3+X^2+X+1,1],[X^4-X^3+X^2-X+1,1],[X^8-X^6+X^4-X^2+1,1],[X^16-X^12+X^8-X^4+1,1]]; } Else If( n =? 50 ) { ans := [[X-1,1],[X+1,1],[X^4+X^3+X^2+X+1,1],[X^4-X^3+X^2-X+1,1],[X^20+X^15+X^10+X^5+1,1],[X^20-X^15+X^10-X^5+1,1]]; } Else If( n =? 100 ) { ans := [[X-1,1],[X+1,1],[X^2+1,1],[X^4+X^3+X^2+X+1,1],[X^4-X^3+X^2-X+1,1],[X^8-X^6+X^4-X^2+1,1],[X^20+X^15+X^10+X^5+1,1],[X^20-X^15+X^10-X^5+1,1],[X^40-X^30+X^20-X^10+1,1]];} Else { ans := Concat( xFB1(X,n/2,1), xFB1(X,n/2,-1) ); };   Decide(InVerboseMode(),Tell(\" \",ans)); ans;};50 # xFB2( dis_List? )_(Length(dis)=?3 And? Length(dis[3])=?2) <--{ Decide(InVerboseMode(),{NewLine();Tell(\"xFB2\",dis);}); Local(ns,ii,fn,mx,my,fac2); Decide(InVerboseMode(), { Tell(\" \",n); Tell(\" \",[X,Y]); Tell(\" \",[A,B,s]); Tell(\" \",[Ar,Br]); } ); X := Ar*X; Y := Br*Y; Decide(InVerboseMode(),Tell(\" \",[X,Y]));  fac2 := xFB1( X/Y,n,s);  Decide(InVerboseMode(),Tell(\" \",fac2));   Decide( Y !=? 1, { Local(f,d); For(ii:=1,ii<=?Length(fac2),ii++) { f := fac2[ii][1]; d := Degree(f,vars[1]); Decide(InVerboseMode(),Tell(\" \",[ii,f,d])); fac2[ii][1] := Simplify(Simplify(Y^d*f)); }; } ); fac2;};UnFence(\"xFB2\",1);IsPowerOf2( n_PositiveInteger? ) <-- { Count(StringToList(ToBase(2,n)),\"1\") =? 1; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # xFactorsResiduals( residualList_List? ) <--{ Decide(InVerboseMode(),{NewLine(); Tell(\"Residuals\",residualList);}); Decide(InVerboseMode(),Tell(\" --\",content)); Decide(InVerboseMode(),Tell(\" --\",factrs)); Decide(InVerboseMode(),Tell(\" --\",factrsnew)); Decide(InVerboseMode(),Tell(\" --\",residuals)); Decide(InVerboseMode(),Tell(\" -- original \",degrees)); Local(resid,sqf,sqfGood,rrGood);   residualList := trySQF(residualList);   Decide(InVerboseMode(), { Tell(\" after trying SQF on all residuals\"); Tell(\" \",sqfGood); Tell(\" \",factrsnew); Tell(\" \",residualList); NewLine(); } );   tryRealRoots(residualList);  Decide(InVerboseMode(), { Tell(\" after trying for REAL roots on all residuals\"); Tell(\" \",rrGood); Tell(\" \",factrsnew); Tell(\" \",residuals); Tell(\" \",residualList); NewLine(); } ); residOut;};UnFence(\"xFactorsResiduals\",1);10 # trySQF( residualList_List? ) <--{  Local(resid,sqf); Decide(InVerboseMode(),{NewLine(); Tell(\"trySQF\",residualList);}); ForEach(resid,residualList) { Decide(InVerboseMode(),Tell(\" \",resid));  sqf := SquareFree(resid[1]); Decide(InVerboseMode(), { Tell(\" trying SQF\"); Tell(\" \",resid[1]); Tell(\" \",sqf); } ); Decide(Degree(sqf) <? Degree(resid[1]), { Decide(InVerboseMode(),Tell(\" sqf helps factor resid\")); sqfGood := True; Local(f1,f2); f1 := sqf; f2 := Simplify(resid[1]/sqf); Decide( f2 =? f1, factrsnew := Concat([[f1,2*resid[2]]],factrsnew), factrsnew := Concat([[f1,resid[2]],[f2,resid[2]]],factrsnew) );  residuals := Difference(residuals,[resid]); Decide(InVerboseMode(),Tell(\" new\",residuals)); residualList := residuals; }, { Decide(InVerboseMode(), { Tell(\" sqf DOES NOT HELP factor resid\"); sqfGood := False; } ); } ); Decide(InVerboseMode(),Tell(\" after sqf \",factrsnew)); Decide(InVerboseMode(),Tell(\" \",residuals)); Decide(InVerboseMode(),Tell(\" \",residualList));  }; residualList; };UnFence(\"trySQF\",1);10 # tryRealRoots(residualList_List?)_(Length(residualList)>?0) <--{  Decide(InVerboseMode(),{NewLine(); Tell(\"tryRealRoots\",residualList);}); ForEach(resid,residualList) { Local(nrr,rr,ptry,uptry); nrr := RealRootsCount(resid[1]); Decide(InVerboseMode(), { Tell(\" this \",resid[1]); Tell(\" \",nrr); } ); Decide( nrr >? 0, rr := FindRealRoots(resid[1]), rr := [] ); processRealRoots(rr);  Decide( nrr =? 2, { Decide( nrr =? 0, {  Decide(InVerboseMode(), { NewLine(); Tell(\" NO real solutions\"); Tell(\" try something Else\"); } );  Local(u,X); u := MakeUni(resid[1]); X := u[1]; Decide( u[2]=?0 And? u[3]=?[1,0,1,0,1], {  DestructiveAppend(residOut,[X^2-X+1,1]); DestructiveAppend(residOut,[X^2+X+1,1]); Decide(InVerboseMode(), { Tell(\" found \",factrsnew); Tell(\" \",resid); Tell(\" \",factrs); Tell(\" \",residOut); } ); } ); }, {  rr := FindRealRoots(resid[1]); Decide(InVerboseMode(),Tell(\" \",rr));  goodptry := []; For(ii:=1,ii<?nrr,ii++) For(jj:=ii+1,jj<=?nrr,jj++) { Decide(InVerboseMode(),Tell(\" \",[ii,jj])); ptry := Expand((x-rr[ii])*(x-rr[jj])); uptry := MakeUni(ptry); uptry[3] := \"NearRational\" /@ uptry[3]; Decide(InVerboseMode(),Tell(\" rat\",uptry[3])); Decide(InVerboseMode(),Tell(\" \",Maximum(Denominator /@ uptry[3]))); Decide( Maximum(Denominator /@ uptry[3]) <? 100, { DestructiveAppend(goodptry,NormalForm(uptry)); } ); }; Decide(InVerboseMode(),Tell(\" \",goodptry)); Decide(Length(goodptry) >? 0, { ForEach(pt,goodptry) { DestructiveAppend(residOut,[pt,1]); }; } ); } );  } );  }; };UnFence(\"tryRealRoots\",1);10 # processRealRoots( rr_NumericList? )_(Length(rr) =? 1) <--{   Decide(InVerboseMode(),Tell(\" Only 1 real root\",rr)); Local(root); root := rr[1]; rrGood := False; Decide(Integer?(root),  { Decide(InVerboseMode(),Tell(\" integer \",root)); rrGood := True; }, { Local(rroot); rroot := NearRational(root); Decide(InVerboseMode(),Tell(\" rational \",rroot)); Decide(Denominator(rroot) <? 100, {root := rroot; rrGood:=True;} ); } ); };UnFence(\"processRealRoots\",1);10 # processRealRoots( rr_NumericList? )_(Length(rr) =? 2) <--{  ptry := Expand((x-rr[1])*(x-rr[2])); Decide(InVerboseMode(),{Tell(\" \",rr);Tell(\" \",ptry);}); uptry := MakeUni(ptry); uptry[3] := \"NearRational\" /@ uptry[3]; ptry := NormalForm(uptry); Decide(InVerboseMode(),Tell(\" \",ptry)); Decide( Abs(Lcm(uptry[3])) <? 100, {  Local(f1,f2,new); f1 := ptry; f2 := Simplify(resid[1]/f1); new := [[f1,resid[2]],[f2,resid[2]]]; Decide(InVerboseMode(),Tell(\" \",new)); resid := new; residOut := new; Decide(InVerboseMode(),Tell(\" \",residOut));  } );};UnFence(\"processRealRoots\",1);10 # processRealRoots( rr_NumericList? )_(Length(rr) >=? 4) <--{  Decide(InVerboseMode(),Tell(\" \",rr));  goodptry := []; For(ii:=1,ii<?nrr,ii++) For(jj:=ii+1,jj<=?nrr,jj++) { Decide(InVerboseMode(),Tell(\" \",[ii,jj])); ptry := Expand((x-rr[ii])*(x-rr[jj])); uptry := MakeUni(ptry); uptry[3] := \"NearRational\" /@ uptry[3]; Decide(InVerboseMode(),Tell(\" rat\",uptry[3])); Decide(InVerboseMode(),Tell(\" \",Maximum(Denominator /@ uptry[3]))); Decide( Maximum(Denominator /@ uptry[3]) <? 100, { DestructiveAppend(goodptry,NormalForm(uptry)); } ); }; Decide(InVerboseMode(),Tell(\" \",goodptry)); Decide(Length(goodptry) >? 0, { ForEach(pt,goodptry) { DestructiveAppend(residOut,[pt,1]); }; } ); };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # xPrimitivePart(poly_CanBeUni) <-- { Local(cont,pp); Decide(InVerboseMode(),Tell(\" xPrimitivePart1\",poly)); cont := xContent(poly); pp := poly / cont; };Macro(\"xPrimitivePart\",[poly,xcont]){ Local(pp); Decide(InVerboseMode(),Tell(\" xPrimitivePart2\",[poly,xcont]));  Decide( Assigned?(@xcont),  { pp := Eval(@poly) / Eval(@xcont); }, { Local(xCont); xCont := xContent(Eval(@poly)); @xcont := xCont; pp := Eval(@poly) / xCont; } ); pp; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "anchor:=[];anchor[\"0\"]:=\"a\";anchor[\"name\"]:=\"\";link:=[];link[\"0\"]:=\"a\";link[\"href\"]:=\"\";frameset:=[];frameset[\"0\"]:=\"frameset\";frameset[\"border\"]:=\"0\";frame:=[];frame[\"0\"]:=\"frame\";caption:=[];caption[\"0\"]:=\"caption\";table:=[];table[\"0\"]:=\"table\";form:=[];form[\"0\"]:=\"form\";textarea:=[];textarea[\"0\"]:=\"textarea\";textfield:=[];textfield[\"0\"]:=\"input\";textfield[\"TYPE\"]:=\"text\";button:=[];button[\"0\"]:=\"input\";button[\"TYPE\"]:=\"submit\";bullets:=[];bullets[\"0\"]:=\"ul\";bullet:=[];bullet[\"0\"]:=\"li\";newline:=\"\";Gt():=\"&gt;\";Lt():=\"&lt;\";HtmlNewParagraph():= (newline ~ \"<p>\" ~ newline);HtmlTitle(title):={\"<head> <title>\" ~ title ~ \"</title> <link rel=\\\"stylesheet\\\" href=\\\"piper.css\\\" TYPE=\\\"text/css\\\" MEDIA=\\\"screen\\\"></head>\";};HtmlAnchor(name):={ anchor[\"name\"]:=name; HtmlTag(anchor,\"\");};HtmlTable(cellpadding,width,body):={ table[\"cellpadding\"]:=ToString(cellpadding); table[\"width\"]:=width; HtmlTag(table,body);};Bullets(list):=HtmlTag(bullets,list);Bullet (list):=HtmlTag(bullet ,list);HtmlCaption(title):={ HtmlTag(caption,title);};HtmlForm(action,body):={ form[\"method\"]:=\"get\"; form[\"action\"]:=action; HtmlTag(form,body);};HtmlTextArea(name,width,height,body) :={ textarea[\"name\"]:=name; textarea[\"cols\"]:=ToString(width); textarea[\"rows\"]:=ToString(height); HtmlTag(textarea,body);};HtmlTextField(name,size,value):={ textfield[\"name\"]:=name; textfield[\"size\"]:=ToString(size); textfield[\"value\"]:=value; HtmlTag(textfield,\"\");};HtmlSubmitButton(name,value):={ button[\"name\"]:=name; button[\"value\"]:=value; HtmlTag(button,\"\");};HtmlLink(description,file,tag,target):={ Decide(tag !=? \"\", link[\"href\"]:= file ~ \"#\" ~ tag, link[\"href\"]:= file); Decide(target !=? \"\",link[\"target\"] :=target); HtmlTag(link,description);};HtmlFrameSetRows(columns,body):={ frameset[\"cols\"]:=\"\"; frameset[\"rows\"]:=columns; HtmlTag(frameset,body);};HtmlFrameSetCols(columns,body):={ frameset[\"cols\"]:=columns; frameset[\"rows\"]:=\"\"; HtmlTag(frameset,body);};HtmlFrame(source,name):={ frame[\"src\"]:=source; frame[\"name\"]:=name; HtmlTag(frame,\"\");};HtmlTag(tags,content):={ Local(result,tag,analytics); result:=\"<\" ~ tags[\"0\"]; ForEach(tag,AssocIndices(tags)) { Decide(tag !=? \"0\" And? tags[tag] !=? \"\", result:= result ~ \" \" ~ tag ~ \"=?\" ~ \"\\\"\" ~ tags[tag] ~ \"\\\"\" ); }; analytics:=\"\"; Decide(tags[\"0\"] =? \"body\", analytics:=\"<script src=\\\"http:</script><script type=\\\"text/javascript\\\">_uacct = \\\"UA-2425144-1\\\";urchinTracker();</script>\"); result:= result ~ \">\" ~ newline ~ content ~ newline ~ analytics ~ \"</\" ~ tags[\"0\"] ~ \">\" ~ newline; result;};htmldir:=\"\";SetHtmlDirectory(dir):= {htmldir:=dir;};HtmlFile(file) := {htmldir ~ file;};site:=[];ClearSite() := {site:=[];};LoadSite():={ PipeFromFile(\"siteall\") { site:=ParseMathPiper(); };};SaveSite():={ PipeToFile(\"siteall\") { Write(site); WriteString(\";\"); };};MySQLQuery(pidstr,string):={ Local(result); PipeToFile(\"sqlin\"~pidstr) WriteString(string); SystemCall(\"mysql mysql < \"~\"sqlin\"~pidstr~\" > sqlout\"~pidstr); SystemCall(FindFile(\"tools/mysqlstubs\")~\" sqlout\"~pidstr~\" sqlout_\"~pidstr); result:= PipeFromFile(\"sqlout_\"~pidstr)ParseMathPiper(); SystemCall(\"rm -rf sqlin\"~pidstr); SystemCall(\"rm -rf sqlout\"~pidstr); SystemCall(\"rm -rf sqlout_\"~pidstr); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LoadLibraryFunction(\"AntiDeriv\");10 # (Integrate(_var)(expr_List?)) <-- Map(\"Integrate\",[FillList(var,Length(expr)),expr]);20 # (Integrate(_var)(_expr)) <-- IntSub(var,expr,AntiDeriv(var,IntClean(var,expr)));20 # (Integrate(_var, optionsList_List?)(_expr)) <--{ Local(result); optionsList := OptionsToAssociativeList(optionsList); result := Integrate(var) expr; Decide( optionsList[\"logAbs\"] =? \"True\", result := ( result /: [Ln(_x) <- Ln(Abs(x))]) ); result;};10 # IntSub(_var,_expr,Integrate(_var)(_expr2)) <-- `Hold(Integrate(@var)(@expr));20 # IntSub(_var,_expr,_result) <-- result; 10# (Integrate(_var,_from,_to)(expr_List?)) <-- Map(\"Integrate\",[FillList(var,Length(expr)), FillList(from,Length(expr)), FillList(to,Length(expr)), expr]);20 # (Integrate(_var,_from,_to)(_expr)) <-- defIntegrate(var,from,to,expr,a,b);20 # (Integrate(_var,_from,_to,optionsList_List?)(_expr)) <--{ Local(result); optionsList := OptionsToAssociativeList(optionsList); result := Integrate(var,from,to) expr; Decide( optionsList[\"logAbs\"] =? \"True\", result := ( result /: [Ln(_x) <- Ln(Abs(x))]) ); result;};10 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(from =? -to And? OddFunction?(expr,var)) <-- 0;10 # defIntegrate(_var,_from,_from,_expr,_a,_b) <-- 0;12 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(from =? -to And? EvenFunction?(expr,var)) <-- 2*defIntegrate(var,0,to,expr,a,b);100 # defIntegrate(_var,_from,_to,_expr,_a,_b)_(Type(AntiDeriv(var,IntClean(var,expr))) !=? \"AntiDeriv\") <-- IntegrateRange(var,expr,from,to,AntiDeriv(var,IntClean(var,expr)));101 # defIntegrate(_var,_from,_to,_expr,_a,_b) <-- `Hold(Integrate(@var,@from,@to)(@expr));10 # IntegrateRange(_var,_expr,_from,_to,Integrate(_var)_expr2) <-- `Hold(Integrate(@var,@from,@to)@expr);20 # IntegrateRange(_var,_expr,_from,_to,_antideriv) <-- `(@antideriv Where @var == @to) - `(@antideriv Where @var == @from);10 # IntClean(_var,_expr) <--{ If( FreeOf?(var,expr) Or? SumOfTerms?(var,expr) ){ expr; } Else If( HasFunction?(expr,Sin) Or? HasFunction?(expr,Cos) ){ Simplify(TrigSimpCombine(expr)); } Else { Simplify(expr); };};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(a,b){10 # MatchLinear(var_Atom?,expr_CanBeUni(var)) <--{ Assign(expr,MakeUni(expr,var)); MatchLinear(expr);};20 # MatchLinear(_var,_expr) <-- False;10 # MatchLinear(_expr)_(Degree(expr,var)<?2) <--{ Check(UniVar?(expr), \"Argument\", PipeToString()Echo([\"Incorrect argument \",expr,\" passed to MatchLinear\"])); a := Coef(expr,1); b := Coef(expr,0); True;};20 # MatchLinear(_expr) <-- False;UnFence(\"MatchLinear\",1);UnFence(\"MatchLinear\",2);10 # MatchPureSquared(var_Atom?,_sign2,_sign0,expr_CanBeUni(var)) <--{ Assign(expr,MakeUni(expr,var)); MatchPureSquared(expr,sign2,sign0);};20 # MatchPureSquared(_var,_sign2,_sign0,_expr) <-- False;10 # MatchPureSquared(_expr,_sign2,_sign0)_(Degree(expr,var)=?2 And? Coef(expr,1) =? 0 And? Number?(Coef(expr,0)) And? Number?(Coef(expr,2)) And? Coef(expr,0)*sign0 >? 0 And? Coef(expr,2)*sign2 >? 0 ) <--{ Check(UniVar?(expr), \"Argument\", PipeToString()Echo([\"Incorrect argument \",expr,\" passed to MatchLinear\"])); a := Coef(expr,2); b := Coef(expr,0); True;};20 # MatchPureSquared(_expr,_sign2,_sign0) <-- False;UnFence(\"MatchPureSquared\",3);UnFence(\"MatchPureSquared\",4);Matcheda() := a;Matchedb() := b;}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BuildMatrix\",[func,m,n]){ Local(i,j,result); result:=ZeroMatrix(m,n); For(i:=1,i<=?m,i++) For(j:=1,j<=?n,j++) result[i][j]:=ApplyFast(func,[i,j]); result;};HoldArgument(\"BuildMatrix\",func);UnFence(\"BuildMatrix\",3);";
        scriptMap.put("BuildMatrix",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Cholesky(A_Matrix?) <--{ Local(matrix,n,k,j); n:=Length(A); matrix:=ZeroMatrix(n);  ForEach(i,1 .. n) ForEach(j,1 .. n) matrix[i][j] := A[i][j];  ForEach(i,1 .. n){ For(k:=1,k<=?(i-1),k++) matrix[i][i] := matrix[i][i] - matrix[k][i]^2; Check( matrix[i][i] >? 0, \"Math\", \"Cholesky: Matrix is not positive definite\"); matrix[i][i] := Sqrt(matrix[i][i]);  For(j:=i+1,j<=?n,j++){ For(k:=1,k<=?(i-1),k++) matrix[i][j]:= matrix[i][j] - matrix[k][i]*matrix[k][j]; matrix[i][j] := matrix[i][j]/matrix[i][i];  }; };  ForEach(i,1 .. n) ForEach(j,1 .. n) Decide(i>?j,matrix[i][j] := 0); matrix;};";
        scriptMap.put("Cholesky",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"CoFactor\",[matrix,ii,jj]){ Local(perms,indices,result); indices:=BuildList(i,i,1,Length(matrix),1); perms:=PermutationsList(indices); result:=0; ForEach(item,perms) Decide(item[ii] =? jj, result:=result+ Product(i,1,Length(matrix), Decide(ii=?i,1,matrix[i][item[i] ]) )*LeviCivita(item)); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HankelMatrix(n):=BuildMatrix([[i,j], Decide(i+j-1>?n,0,i+j-1) ], n,n );HankelMatrix(m,n):=BuildMatrix([[i,j], Decide(i+j-1>?n,0,i+j-1)], m,n );";
        scriptMap.put("HankelMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HessianMatrix(f,v):=BuildMatrix([[i,j], Deriv(v[i]) Deriv(v[j]) f],Length(v),Length(v));";
        scriptMap.put("HessianMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HilbertInverseMatrix(n):=BuildMatrix([[i,j], (-1)^(i+j)*(i+j-1)*BinomialCoefficient(n+i-1,n-j)*BinomialCoefficient(n+j-1,n-i)*BinomialCoefficient(i+j-2,i-1)^2],n,n);";
        scriptMap.put("HilbertInverseMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "HilbertMatrix(n):=BuildMatrix([[i,j], 1/(i+j-1)], n,n );HilbertMatrix(m,n):=BuildMatrix([[i,j], 1/(i+j-1)], m,n );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Inverse\",[matrix]){ Local(perms,indices,inv,det,n); n:=Length(matrix); indices:=BuildList(i,i,1,n,1); perms:=PermutationsList(indices); inv:=ZeroMatrix(n,n); det:=0; ForEach(item,perms) { Local(i,lc); lc := LeviCivita(item); det:=det+Product(i,1,n,matrix[i][item[i] ])* lc; For(i:=1,i<=?n,i++) { inv[item[i] ][i] := inv[item[i] ][i]+ Product(j,1,n, Decide(j=?i,1,matrix[j][item[j] ]))*lc; }; }; Check(det !=? 0, \"Math\", \"Zero determinant\"); (1/det)*inv;};";
        scriptMap.put("Inverse",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "JacobianMatrix(f,v):=BuildMatrix([[i,j],Deriv(v[j])f[i]],Length(f),Length(f));";
        scriptMap.put("JacobianMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LU(A_SquareMatrix?) <--{ Local(n,matrix,L,U); n:=Length(A); L:=ZeroMatrix(n,n); U:=ZeroMatrix(n,n); matrix:=ZeroMatrix(n,n); ForEach(i,1 .. n) ForEach(j,1 .. n) matrix[i][j] := A[i][j];  ForEach(i,1 ..(n-1)){  ForEach(k,i+1 .. n){  matrix[k][i] := matrix[k][i] / matrix[i][i];  ForEach(j,i+1 .. n){ matrix[k][j] := matrix[k][j] - matrix[k][i]*matrix[i][j]; }; }; }; ForEach(i,1 .. n){ ForEach(j,1 .. n){ Decide(i<=?j,U[i][j]:=matrix[i][j],L[i][j]:=matrix[i][j]); };  L[i][i]:=1; }; [L,U];};";
        scriptMap.put("LU",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"LeviCivita\",[indices]){ Local(i,j,length,left,right,factor); length:=Length(indices); factor:=1; For (j:=length,j>?1,j--) { For(i:=1,i<?j,i++) { left:=indices[i]; right:=indices[i+1]; Decide(Equal?(left,right), { factor := 0 ; }, { Decide(Not?(Apply(\"<?\",[left,right])), { indices:=Insert(Delete(indices,i),i+1,left); factor:= -factor; }); }); }; }; factor;};";
        scriptMap.put("LeviCivita",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixPower(x_SquareMatrix?, n_NonNegativeInteger?) <--{ Local(result); result:=Identity(Length(x)); While(n !=? 0) { Decide(Odd?(n), result:=Dot(result,x)); x:=Dot(x,x); n:=n>>1; }; result;};MatrixPower(x_SquareMatrix?, n_NegativeInteger?) <-- MatrixPower(Inverse(x),-n);";
        scriptMap.put("MatrixPower",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # MatrixSolve(matrix_Diagonal?,b_Vector?) <--{ Decide(InVerboseMode(),Tell(\" MatrixSolve_diag\",[matrix,b])); Local(rowsm,rowsb,x); rowsm:=Length(matrix); rowsb:=Length(b); Check(rowsm=?rowsb, \"Argument\", \"MatrixSolve: Matrix and vector must have same number of rows\"); x:=ZeroVector(rowsb); ForEach(i,1 .. rowsb) x[i]:=b[i]/matrix[i][i]; x;};15 # MatrixSolve(matrix_UpperTriangular?,b_Vector?) <--{ Decide(InVerboseMode(),Tell(\" MatrixSolve_ut\",[matrix,b])); Local(rowsm,rowsb,x,s); rowsm:=Length(matrix); rowsb:=Length(b); Check(rowsm=?rowsb, \"Argument\", \"MatrixSolve: Matrix and vector must have same number of rows\"); x:=ZeroVector(rowsb); x[rowsb]:=b[rowsb]/matrix[rowsb][rowsb]; Decide(InVerboseMode(),Echo([\"set x[\",rowsb,\"] =? \",b[rowsb]/matrix[rowsb][rowsb]])); ForEach(i,(rowsb-1) .. 1 ){ s:=b[i]; ForEach(j,i+1 .. rowsb ){ s:= s - matrix[i][j]*x[j]; }; x[i]:= s/matrix[i][i]; Decide(InVerboseMode(),Echo([\"set x[\",i,\"] =? \",s/matrix[i][i]])); }; x;};15 # MatrixSolve(matrix_LowerTriangular?,b_Vector?) <--{ Decide(InVerboseMode(),Tell(\" MatrixSolve_lt\",[matrix,b])); Local(rowsm,rowsb,x,s); rowsm:=Length(matrix); rowsb:=Length(b); Check(rowsm=?rowsb, \"Argument\", \"MatrixSolve: Matrix and vector must have same number of rows\"); x:=ZeroVector(rowsb); x[1]:=b[1]/matrix[1][1]; Decide(InVerboseMode(),Echo([\"set x[1] =? \",b[1]/matrix[1][1]])); ForEach(i,2 .. rowsb ){ s:=b[i]; ForEach(j,1 .. (i-1) ){ s:= s - matrix[i][j]*x[j]; }; x[i]:= s/matrix[i][i]; Decide(InVerboseMode(),Echo([\"set x[\",i,\"] =? \",s/matrix[i][i]])); }; x;};20 # MatrixSolve(matrix_Matrix?,b_Vector?) <--{ Decide(InVerboseMode(),Tell(\" MatrixSolve\",[matrix,b])); Local(aug,rowsm,rowsb,x,s); rowsm:=Length(matrix); rowsb:=Length(b); Check(rowsm=?rowsb, \"Argument\", \"MatrixSolve: Matrix and vector must have same number of rows\"); aug:=ZeroMatrix(rowsb,rowsb+1); x:=ZeroVector(rowsb);  ForEach(i, 1 .. rowsb ) ForEach(j, 1 .. rowsb ) aug[i][j] := matrix[i][j]; ForEach(i, 1 .. rowsb ) aug[i][rowsb+1] := b[i];  ForEach(i, 1 .. (rowsb-1) ){   Decide(aug[i][i] =? 0, { Local(p,tmp); p:=i+1; While( aug[p][p] =? 0 ){ p++; }; Decide(InVerboseMode(), Echo([\"switching row \",i,\"with \",p]) ); tmp:=aug[i]; aug[i]:=aug[p]; aug[p]:=tmp; }); ForEach(k, (i+1) .. rowsb ){ s:=aug[k][i]; ForEach(j, i .. (rowsb+1) ){ aug[k][j] := aug[k][j] - (s/aug[i][i])*aug[i][j];   }; }; };  x[rowsb]:=aug[rowsb][rowsb+1]/aug[rowsb][rowsb]; Decide(InVerboseMode(),Echo([\"set x[\",rowsb,\"] =? \",x[rowsb] ])); ForEach(i,(rowsb-1) .. 1 ){ s:=aug[i][rowsb+1]; ForEach(j,i+1 .. rowsb){ s := s - aug[i][j]*x[j]; }; x[i]:=Simplify(s/aug[i][i]); Decide(InVerboseMode(),Echo([\"set x[\",i,\"] = \",x[i] ])); }; x;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"PNorm\",[v,p]){ Local(result,i); Check(p>=?1, \"Argument\", \"PNorm: p must be >=? 1\"); result:=0; For(i:=1,i<=?Length(v),i++){ result:=result+Abs(v[i])^p; };  Decide(p=?2,Sqrt(result),(result)^(1/p) );};";
        scriptMap.put("PNorm",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RREF( AM_Matrix? ) <--{ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide(iDebug,Tell(\"RREF\",AM)); Local(mRows,nCols,nVars,varVec,ir,jc,col,am,e,ii,pivot); [mRows,nCols] := Dimensions(AM); am := FlatCopy(AM);   Local(cc,cr,col,cnz);  cc := 1;  cr := 1;  While( cr <=? mRows )  { Local(ic);  Decide(iDebug, Tell(\" STEP 1\")); cnz := 0;  For(ic:=1,ic<=?nCols,ic++)  {  col := MatrixColumn(am,ic); Decide( cr >? 1, col := Drop( col, [1,cr-1] ) );  Decide(Not? ZeroVector?(col), {cnz := ic; Break();});  }; cc := cnz; Decide( iDebug, Tell(\" first non-zero column is \",cc));    Decide(iDebug, Tell(\" STEP 2\")); For( ir:=cr, ir<=?mRows,ir++ ) {  Decide( am[ir][cc] !=? 0 And? ir !=? 1,  { [am[ir],am[cr]] := [am[cr],am[ir]]; Decide(iDebug, Tell(\" swapping rows \",[cr,ir])); Break(); } ); }; Decide(iDebug,{Tell(\"working matrix\");TableForm(am);});    Decide(iDebug, Tell(\" STEP 3\")); am[cr] := am[cr]/am[cr][cc]; Decide(iDebug,TableForm(am));    Decide(iDebug, Tell(\" STEP 4\")); For(ir:=cr+1,ir<=?mRows,ir++) { Decide( am[ir][cc] !=? 0,  { am[ir] := am[ir] - am[ir][cc]*am[cr]; } ); }; Decide(iDebug,TableForm(am));     Decide(iDebug, Tell(\" STEP 5\")); cr := cr + 1; Decide(iDebug And? cr <=? mRows, {NewLine();Tell(\" \",cr);});   Decide( cr=?mRows And? ZeroVector?(am[cr]), Break() ); };       Decide(iDebug, Tell(\" STEP 6\")); Local(pc,jr); For(ir:=mRows,ir>?1,ir--) {   Decide(iDebug,Tell(\"\",[ir,am[ir]])); Decide(ZeroVector?(am[ir]), {Decide(iDebug,Tell(\" trailing row of zeros: row \",ir)); ir:=ir-1;Continue();}, { pc := Find(am[ir],1);  Decide(pc >? 0,  { For(jr:=ir-1,jr>=?1,jr--) { Decide(am[jr][pc]!=?0, am[jr]:=am[jr]-am[jr][pc]*am[ir]); Decide(iDebug,{NewLine();TableForm(am);}); }; } ); } ); }; am;}; ";
        scriptMap.put("RREF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # RecursiveDeterminant(_matrix) <--{  Local(result); Decide(Equal?(Length(matrix),1),matrix[1][1],{ result:=0; ForEach(i,1 .. Length(matrix))  Decide(Not?(Equal?(matrix[1][i],0)),  result:=result+matrix[1][i]*(-1)^(i+1)* RecursiveDeterminant(Transpose(Drop(Transpose(Drop(matrix,[1,1])),[i,i])))); result; });};";
        scriptMap.put("RecursiveDeterminant",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Sparsity\",[matrix]){ Local(rows,cols,nonzero); nonzero:=0; rows:=Length(matrix); cols:=Length(matrix[1]); ForEach(i, 1 .. rows ) ForEach(j, 1 .. cols ) Decide(matrix[i][j] !=? 0, nonzero:=nonzero+1 ); NM(1 - nonzero/(rows*cols));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # SymbolicDeterminant(_matrix) <--{ Local(perms,indices,result); Check((Matrix?(matrix)), \"Argument\", \"Determinant: Argument must be a matrix\"); indices:=BuildList(i,i,1,Length(matrix),1); perms:=PermutationsList(indices); result:=0; ForEach(item,perms) result:=result+Product(i,1,Length(matrix),matrix[i][item[i] ])* LeviCivita(item); result;};";
        scriptMap.put("SymbolicDeterminant",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ToeplitzMatrix(N):=BuildMatrix([[i,j],N[Abs(i-j)+1]], Length(N), Length(N) );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WilkinsonMatrix(N):=BuildMatrix([[i,j], Decide( Abs(i-j)=?1,1, { Decide(i=?j,Abs( (N-1)/2 - i+1 ),0 ); } )], N,N );";
        scriptMap.put("WilkinsonMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "WronskianMatrix(f,v):=BuildMatrix([[i,j], Deriv(v,i-1) f[j]], Length(f), Length(f) );";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnAugment( M_Matrix?, v_Vector? )_(Length(v)=?Dimensions(M)[2]) <--[ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnAugment,[M,v]) ); Local(mRows,nCols,newMat,ir); Local(MT,MA); MT := Transpose(M); MT := MatrixRowStack(MT,v); MA := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnReplace( M_Matrix?, jCol_PositiveInteger?, v_Vector? )_(Length(v)=?Dimensions(M)[2]) <--{ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnReplace,[M,jCol,v]) ); Local(mRows,nCols,MT); [mRows,nCols] := Dimensions(M); Decide( jCol <=? nCols, { MT:=Transpose(M); DestructiveReplace(MT,jCol,v); M:=Transpose(MT); } ); M;};";
        scriptMap.put("MatrixColumnReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnSwap( M_Matrix?, jCol1_PositiveInteger?, jCol2_PositiveInteger? )_ (And?(jCol1<=?Dimensions(M)[2],jCol2<=?Dimensions(M)[2])) <--{ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnSwap,[M,jCol1,jCol2]) ); Local(MT); MT := Transpose(M); MT := MatrixRowSwap(MT,jCol1,jCol2); M := Transpose(MT);};";
        scriptMap.put("MatrixColumnSwap",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowReplace( M_Matrix?, iRow_PositiveInteger?, v_Vector? )_(Length(v)=?Length(M[1])) <--{ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixRowReplace,[M,iRow,v]) ); Local(mRows,nCols); [mRows,nCols] := Dimensions(M); Decide( iRow <=? mRows, DestructiveReplace(M,iRow,v) ); M;};";
        scriptMap.put("MatrixRowReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowStack( M_Matrix?, v_Vector? )_(Length(v)=?Dimensions(M)[1]) <--{ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixRowStack,[M,v]) ); Local(mRows,nCols,newMat,ir); [mRows,nCols] := Dimensions(M); newMat := ZeroMatrix(mRows+1,nCols); For(ir:=1,ir<?mRows+1,ir++) { MatrixRowReplace(newMat,ir,MatrixRow(M,ir)); }; MatrixRowReplace(newMat,mRows+1,v); newMat;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(max,f,result){ BSearch(max,f) := { Local(result); Assign(result, FindIsq(max,f)); Decide(Apply(f,[result])!=?0,Assign(result,-1)); result; };};UnFence(\"BSearch\",2);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(max,f,low,high,mid,current){FindIsq(max,f) :={ Local(low,high,mid,current); low:=1; high:=max+1; Assign(mid,((high+low)>>1)); While(high>?low And? mid>?1) { Assign(mid,((high+low)>>1)); Assign(current,Apply(f,[mid])); Decide(current =? 0, high:=low-1, Decide(current >? 0, Assign(high,mid), Assign(low,mid+1) ) ); }; mid;};};UnFence(\"FindIsq\",2);";
        scriptMap.put("FindIsq",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FindPredicate\",[list,predicate]){ Local(result,count); Assign(result, -1); Assign(count, 1); While(And?(result<?0, Not?(Equal?(list, [])))) { Decide(Apply(predicate,[First(list)]), Assign(result, count) ); Assign(list,Rest(list)); Assign(count,AddN(count,1)); }; result;}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "VarListAll(_expr) <-- VarListAll(expr,\"Variable?\");10 # VarListAll(_expr,_filter)_(Apply(filter,[expr]) =? True) <-- [expr];20 # VarListAll(expr_Function?,_filter) <--{ Local(item,result, flatlist); Assign(flatlist,Rest(FunctionToList(expr))); Assign(result,[]); ForEach(item,flatlist) Assign(result,Concat(result,VarListAll(item,filter))); result;};30 # VarListAll(_expr,_filter) <-- [];";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(localResult) { localResult := True;   10 # LocPredicate(exp_Atom?) <-- { Local(tr, result);  tr := patterns;  result := False;  While (tr !=? []) { Decide(First(First(tr)) =? exp, { Decide(InVerboseMode() =? True, Echo(Last(First(tr))));  Assign(localResult,Eval(First(Rest(First(tr)))));  result := True;  tr:=[]; }, { tr := Rest(tr); }); };  result; };   10 # LocPredicate(exp_Function?) <-- { Local(tr, result, head);  tr := patterns;  result := False;  While (tr !=? []) { Assign(head, First(First(tr)));  Decide(Not?(Atom?(head)) And? exp[0] =? head[1] And? PatternMatches(head[2], exp), { Decide(InVerboseMode() =? True, Echo(Last(First(tr))));  Assign(localResult, Eval(First(Rest(First(tr)))));  Assign(result, True);  Assign(tr, []); }, { Assign(tr, Rest(tr)); }); };  result; };    20 # LocPredicate(_exp) <-- False;   LocChange(_exp) <-- localResult;  }; UnFence(\"LocPredicate\",1);UnFence(\"LocChange\",1);10 # LocProcessSingle([_pat, _post, _exp]) <-- [ [pat[0], PatternCreate(pat, post)], exp, \"UNNAMED\" ];20 # LocProcessSingle([pat_Function?, _exp]) <-- [ [pat[0], PatternCreate(pat,True)], exp, \"UNNAMED\" ];30 # LocProcessSingle([pat_Atom?, _exp]) <-- [ pat, exp, \"UNNAMED\" ];40 # LocProcessSingle(pat_Function? <- _exp) <-- { Local(justPattern, postPredicate, ruleName);  justPattern := pat;  Decide(Type(justPattern) =? \"_\", {  justPattern := pat[1]; postPredicate := pat[2]; }, {  postPredicate := True; } );    Decide(Type(justPattern) =? \"#\", {  ruleName := pat[1]; justPattern := pat[2];  Decide(Function?(justPattern), { [ [justPattern[0], PatternCreate(justPattern, postPredicate)], exp , ruleName]; }, {  [ justPattern, exp, ruleName ]; }  ); }, { ruleName := \"UNNAMED\"; justPattern := pat; [ [justPattern[0], PatternCreate(justPattern, postPredicate)], exp , ruleName]; } ); };50 # LocProcessSingle(pat_Atom? <- _exp) <-- [ pat, exp, \"UNNAMED\" ];LocProcess(patterns) :={ MapSingle(\"LocProcessSingle\", patterns);};CompilePatterns(patterns) := LocPatterns(LocProcess(patterns));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (_expression /:: LocPatterns(_patterns)) <--{ MacroSubstitute(expression,\"LocPredicate\",\"LocChange\");};10 # (_expression /:: _patterns) <--{ Local(old);  Assign(patterns, LocProcess(patterns));  Assign(old, expression);  Assign(expression, MacroSubstitute(expression,\"LocPredicate\",\"LocChange\"));  While (expression !=? old) { Assign(old, expression);  Assign(expression, MacroSubstitute(expression,\"LocPredicate\",\"LocChange\")); };  expression;};";
        scriptMap.put("/::",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (_expression /: LocPatterns(_patterns)) <--{ MacroSubstitute(expression,\"LocPredicate\",\"LocChange\");};10 # (_expression /: _patterns) <--{ Assign(patterns, LocProcess(patterns));  MacroSubstitute(expression,\"LocPredicate\",\"LocChange\");};";
        scriptMap.put("/:",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "DoUnitSubsumptionAndResolution(_list) <--{ Local(i, j, k, isFalse, isTrue, changed); Assign(isFalse, False); Assign(isTrue, False); Assign(changed, True);  While(changed) { Assign(changed, False); For(i:=1, (Not? isFalse And? Not? isTrue) And? i <=? Length(list), i++) { Decide(Length(list[i]) =? 1, { Local(x); Assign(x, list[i][1]);    For(j:=1, (Not? isFalse And? Not? isTrue) And? j <=? Length(list), j++) { Decide(i !=?j, { Local(deletedClause); Assign(deletedClause, False); For(k:=1, (Not? isFalse And? Not? isTrue And? Not? deletedClause) And? k <=? Length(list[j]), k++) {    Decide(Subsumes(x, list[j][k]), {  DestructiveDelete(list, j); j--; Decide(i>?j, i--);  Assign(deletedClause, True); Assign(changed, True); Decide(Length(list) =? 0, {Assign(isTrue, True);}); },  Decide(Contradict(x, list[j][k]), {  DestructiveDelete(list[j], k); k--; Assign(changed, True); Decide(Length(list[j]) =? 0, {Assign(isFalse, True);}); }) ); }; }); }; }); }; }; list;};";
        scriptMap.put("DoUnitSubsumptionAndResolution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LogicCombine(_list) <--{ Local(i, j); For(Assign(i,1), i<=?Length(list), Assign(i,AddN(i,1))) {  For(j := 1, (j<=?Length(list[i])), j++) { Local(tocombine, n, k); Assign(n, list[i][j]); [tocombine, k] := LogicFindWith(list, i, n);  Decide(tocombine !=? -1, { Local(combination); Check(k !=? -1, \"Math\", \"k is -1\"); Assign(combination, LogicRemoveTautologies(Concat(list[i], list[tocombine]))); Decide(combination =? [],  { Assign(list, [[]]); Assign(i, Length(list)+1);}, { }); }); }; }; list;};";
        scriptMap.put("LogicCombine",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LogicFindWith(_list, _i, _n) <--{ Local(result, index, j); Assign(result, -1); Assign(index, -1); For(j := i+1, (result<?0) And? (j <=? Length(list)), j++) { Local(k, len); Assign(len, Length(list[j])); For(k := 1, (result<?0) And? (k<=?len), k++) { Local(el); Assign(el, list[j][k]); Decide(Contradict(n, el), {Assign(result, j); Assign(index, k);}); }; }; [result, index];};";
        scriptMap.put("LogicFindWith",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 1 # SimpleNegate(Not? (_x)) <-- x;2 # SimpleNegate(_x) <-- Not?(x);LocalCmp(_e1, _e2) <-- LessThan?(PipeToString() Write(e1), PipeToString() Write(e2));100 # SimplifyExpression(_x) <-- x;LogicRemoveTautologies(_e) <--{ Local(i, len, negationfound); Assign(len, Length(e)); Assign(negationfound, False);  e := BubbleSort(e, \"LocalCmp\"); For(Assign(i, 1), (i <=? len) And? (Not? negationfound), i++) { Local(x, n, j);   Assign(x, MathNth(e,i)); Assign(n, SimpleNegate(x));  For(Assign(j, i+1), (j <=? len) And? (Not? negationfound), j++) { Local(y); Assign(y, MathNth(e,j)); Decide(Equal?(y, n), {  Assign(negationfound, True);  }, Decide(Equal?(y, x), {  DestructiveDelete(e, j); Assign(len,SubtractN(len,1)); }) ); }; Check(len =? Length(e), \"Math\", \"The length computation is incorrect\"); }; Decide(negationfound, [True], e); };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # LogicSimplify(_proposition, _level)_(level<?2) <-- CNF(proposition);20 # LogicSimplify(_proposition, _level) <--{ Local(cnf, list, clauses); Check(level >? 1, \"Argument\", \"Wrong level\");  Assign(cnf, CNF(proposition)); Decide(level <=? 1, cnf, { Assign(list, Flatten(cnf, \"And?\")); Assign(clauses, []); ForEach(clause, list) { Local(newclause);  Assign(newclause, LogicRemoveTautologies(Flatten(clause, \"Or?\"))); Decide(newclause !=? [True], DestructiveAppend(clauses, newclause)); };  Assign(clauses, RemoveDuplicates(clauses)); Decide(Equal?(level, 3) And? (Length(clauses) !=? 0), { Assign(clauses, DoUnitSubsumptionAndResolution(clauses)); Assign(clauses, LogicCombine(clauses)); }); Assign(clauses, RemoveDuplicates(clauses)); Decide(Equal?(Length(clauses), 0), True, {  Local(result); Assign(result, True); ForEach(item,clauses) { Assign(result, result And? UnFlatten(item, \"Or?\", False)); }; result; }); });};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ContFracList(_n) <-- ContFracList(n, Infinity);10 # ContFracList(_n, _depth)_(depth <=? 0) <-- [];20 # ContFracList(n_Integer?, _depth) <-- [n];30 # ContFracList(n_Number?, _depth) _NumericMode?() <-- NonNM(ContFracList(Rationalize(n), depth));40 # ContFracList(n_Number?, _depth) <-- ContFracList(Rationalize(n), depth);35 # ContFracList((n_NegativeInteger?) / (m_Integer?), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m)-1);40 # ContFracList((n_Integer?) / (m_Integer?), _depth) <-- Push( ContFracList(m/Modulo(n,m), depth-1) , Quotient(n,m));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "EigenVectors(_matrix,_eigenvalues) <--{ Local(result,n); n:=Length(eigenvalues); result:=[]; ForEach(e,eigenvalues) { Local(possible); possible:=OldSolve(matrix*MakeVector(k,n)==e*MakeVector(k,n),MakeVector(k,n))[1]; Decide(Not?(ZeroVector?(possible)), DestructiveAppend(result,possible) ); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # CarmichaelNumber?(n_Even?) <-- False;5 # CarmichaelNumber?(_n)_(n<?561) <-- False;10 # CarmichaelNumber?(n_PositiveInteger?) <--{ Local(i,factors,length,carmichael); factors:=Factors(n); carmichael:=True; length:=Length(factors); If( length <? 3){ carmichael:=False; } Else { For(i:=1,i<=?length And? carmichael,i++){  Decide( Modulo(n-1,factors[i][1]-1) !=? 0, carmichael:=False ); Decide(factors[i][2]>?1,carmichael:=False);  }; }; carmichael;};CarmichaelNumber?(n_List?) <-- MapSingle(\"CarmichaelNumber?\",n);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CheckIntPower(n, limit) :={ Local(s0, s, root); Decide(limit<=?1, limit:=2);   s0 := IntLog(n, limit);  root := 0; s := 0; While(root =? 0 And? NextPseudoPrime(s)<=?s0)  { s := NextPseudoPrime(s); root := IntNthRoot(n, s); Decide( root^s =? n,  True, root := 0 ); };  Decide( root=?0, [n, 1], [root, s] );};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # DigitalRoot(n_PositiveInteger?) <-- Decide(n%9=?0,9,n%9);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "20 # GetPrimePower(n_PositiveInteger?) <--{ Local(s, factors, newfactors);  factors := TrialFactorize(n, 257);  Decide( Length(factors) >? 1,   Decide( factors[1] =? 1 And? Length(factors) =? 2,  factors[2], [n, 1] ),  { factors := CheckIntPower(n, 257);  Decide( factors[2] >? 1,   Decide( Prime?(factors[1]), factors,  {  newfactors := GetPrimePower(factors[1]);  Decide( newfactors[2] >? 1, [newfactors[1], newfactors[2]*factors[2]],  [n, 1]  ); } ),  [n, 1] ); } );};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # IntNthRoot(_n, 2) <-- Floor(SqrtN(n));20 # IntNthRoot(_n, s_Integer?) <--{ Local(result, k); GlobalPush(BuiltinPrecisionGet());  k := Quotient(IntLog(n, 2), s);  BuiltinPrecisionSet(2+Quotient(k*3361, 11165));  result := Round(ExpN(DivideN(InternalLnNum(DivideN(n, 2^(k*s))), s))*2^k); BuiltinPrecisionSet(GlobalPop());  Decide(result^s>?n, result-1, result);};";
        scriptMap.put("IntNthRoot",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IrregularPrime?(p_Composite?) <-- False;5 # IrregularPrime?(_p)_(p<?37) <-- False;10 # IrregularPrime?(p_PositiveInteger?) <--{ Local(i,irregular); i:=1; irregular:=False; While( 2*i + 1 <? p And? (irregular =? False) ){ Decide( Abs(Numerator(Bernoulli(2*i))) % p =? 0, irregular:=True ); i++; }; irregular;};";
        scriptMap.put("IrregularPrime?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LegendreSymbol(_a,_p) <--{ Check( Integer?(a) And? Integer?(p) And? p>?2 And? Coprime?(a,p) And? Prime?(p), \"Argument\", \"LegendreSymbol: Invalid arguments\"); Decide(QuadraticResidue?(a,p), 1, -1 );};";
        scriptMap.put("LegendreSymbol",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Moebius(1) <-- 1;10 # Moebius(_n) <--{ Check(PositiveInteger?(n), \"Argument\", \"Moebius: argument must be positive integer\"); Local(factors,i,repeat); repeat:=0; factors:=Factors(n); len:=Length(factors); For(i:=1,i<=?len,i++){ Decide(factors[i][2]>?1,repeat:=1); }; Decide(repeat=?0,(-1)^len,0);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # QuadraticResidue?(_a,_p) <--{ Check( Integer?(a) And? Integer?(p) And? p>?2 And? Coprime?(a,p) And? Prime?(p), \"Argument\", \"QuadraticResidue?: Invalid arguments\"); Decide(a^((p-1)/2) % p =? 1, True, False);};";
        scriptMap.put("QuadraticResidue?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Rationalize(aNumber_List?) <-- Rationalize /@ aNumber;20 # Rationalize( _aNumber ) <--{ Local(result,n,d); result:=Substitute(aNumber,[[x],Number?(x) And? Not?(Integer?(x))],\"RationalizeNumber\"); Decide(InVerboseMode(),Tell(\"\",result)); Decide(Length(VarList(aNumber))=?0, { n:=Numerator(result); Decide(Type(n)=?\"Numerator\",n:=result); d:=Denominator(result); Decide(Type(d)=?\"Denominator\",d:=1); result := n*(1/d); } ); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RepToNumber( rep_ListOfLists? ) <--{  RepToNumber(rep[1])+I*RepToNumber(rep[2]);};12 # RepToNumber( rep_List? ) <--{  Local(bigInt,precision,scale,strBI,sgn,index,ans); Local(first,secnd,third,LS,numStr); precision := rep[2]; scale := 0; bigInt := rep[1]; precision := rep[2]; sgn := Sign(bigInt); Decide( Length(rep) >? 2, scale := rep[3] ); strBI := ExpressionToString(Abs(bigInt)); LS := Length(strBI);  Decide( Length(rep)=?2, { numStr := strBI; }, { index := precision-scale; first := strBI[1]; secnd := StringMidGet(2,LS-1,strBI);  third := ExpressionToString(index-1);  If( index >? 0 ) { If( index <? precision ) {  numStr := ConcatStrings(first,\".\",secnd,\"E\",third); } Else If( index >=? precision ) {  numStr := ConcatStrings(first,\".\",secnd,\"E+\",third); }; } Else If( index <? 0 ) {  numStr := ConcatStrings(first,\".\",secnd,\"E\",third); } Else {  first := \"0.\" ;  secnd := strBI; numStr := ConcatStrings(first,secnd); }; } ); ans := sgn * ToAtom(numStr);  ans;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # StirlingNumber1(n_Integer?,0) <-- Decide(n=?0,1,0);10 # StirlingNumber1(n_Integer?,1) <-- (-1)^(n-1)*(n-1)!;10 # StirlingNumber1(n_Integer?,2) <-- (-1)^n*(n-1)! * HarmonicNumber(n-1);10 # StirlingNumber1(n_Integer?,n-1) <-- -BinomialCoefficient(n,2);10 # StirlingNumber1(n_Integer?,3) <-- (-1)^(n-1)*(n-1)! * (HarmonicNumber(n-1)^2 - HarmonicNumber(n-1,2))/2;20 # StirlingNumber1(n_Integer?,m_Integer?) <-- Sum(k,0,n-m,(-1)^k*BinomialCoefficient(k+n-1,k+n-m)*BinomialCoefficient(2*n-m,n-k-m)*StirlingNumber2(k-m+n,k));";
        scriptMap.put("StirlingNumber1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # StirlingNumber2(n_Integer?,0) <-- Decide(n=?0,1,0);20 # StirlingNumber2(n_Integer?,k_Integer?) <-- Sum(i,0,k-1,(-1)^i*BinomialCoefficient(k,i)*(k-i)^n)/ k! ;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FactorGaussianInteger\",[x]) { Check( GaussianInteger?(x), \"Argument\", \"FactorGaussianInteger: argument must be a Gaussian integer\"); Local(re,im,norm,a,b,d,i,j); re:=Re(x);im:=Im(x); Decide(re<?0, re:=(-re) ); Decide(im<?0, im:=(-im) ); norm:=re^2+im^2; If( Composite?(norm) ){ For(i:=0, i^2 <=? norm, i++ ){  For(j:=0, i^2 + j^2 <=? norm, j++){  If( Not?( (i =? re And? j =? im) Or? (i =? im And? j =? re) ) ){  d:=i^2+j^2; If( d >? 1 ){ a := re * i + im * j; b := im * i - re * j; While( (Modulo(a,d) =? 0) And? (Modulo(b,d) =? 0) ) { FactorGaussianInteger(Complex(i,j)); re:= a/d; im:= b/d; a := re * i + im * j; b := im * i - re * j; norm := re^2 + im^2; }; }; }; }; }; Decide( re !=? 1 Or? im !=? 0, Echo(Complex(re,im)) ); } Else { Echo(Complex(re,im)); };};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "AddGaussianFactor(L_List?,z_GaussianInteger?,p_GaussianInteger?) <--{ Local(e); e :=0; While (GaussianInteger?(z:= z/p)) e++; Decide(e !=? 0, DestructiveAppend(L,[p,e]));};10 # GaussianFactors(n_Integer?) <--{  Local(ifactors,gfactors,p,alpha); ifactors := FactorizeInt(n);  gfactors := []; ForEach(p,ifactors) { Decide(p[1]=?2, { DestructiveAppend(gfactors,[1+I,p[2]]); DestructiveAppend(gfactors,[1-I,p[2]]); }); Decide(Modulo(p[1],4)=?3, DestructiveAppend(gfactors,p)); Decide(Modulo(p[1],4)=?1, { alpha := GaussianFactorPrime(p[1]); DestructiveAppend(gfactors,[alpha,p[2]]); DestructiveAppend(gfactors,[Conjugate(alpha),p[2]]); }); };gfactors;};20 # GaussianFactors(z_GaussianInteger?) <--{ Local(n,nfactors,gfactors,p); gfactors :=[]; n := GaussianNorm(z); nfactors := Factors(n); ForEach(p,nfactors) { Decide(p[1]=?2, { AddGaussianFactor(gfactors,z,1+I);}); Decide(Modulo(p[1],4)=?3, AddGaussianFactor(gfactors,z,p[1])); Decide(Modulo(p[1],4)=?1, { Local(alpha); alpha := GaussianFactorPrime(p[1]); AddGaussianFactor(gfactors,z,alpha); AddGaussianFactor(gfactors,z,Conjugate(alpha)); }); }; gfactors;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # JacobiSymbol(_a, 1) <-- 1;15 # JacobiSymbol(0, _b) <-- 0;18 # JacobiSymbol(_a, _b) _ (Gcd(a,b)>?1) <-- 0;20 # JacobiSymbol(_a, b_Odd?)_(a>=?Abs(b) Or? a<?0) <-- JacobiSymbol(Modulo(a,Abs(b)),Abs(b));30 # JacobiSymbol(a_Even?, b_Odd?) <--{ Local(c, s);  [c,s]:=FindPrimeFactorSimple(a, 2);  Decide(Modulo(s,2)=?1 And? Abs(Modulo(b,8)-4)=?1, -1, 1) * JacobiSymbol(c,b);};40 # JacobiSymbol(a_Odd?, b_Odd?) <-- Decide(Modulo(a,4)=?3 And? Modulo(b,4)=?3, -1, 1) * JacobiSymbol(b,a);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Apart(_f) <-- Apart(f,x);Apart(_f,_var) <--{ Local(rat);  rat := [Numerator(f),Denominator(f)]; Decide(Degree(rat[1],var) =? 0 And? Degree(rat[2],var) =? 0, { rat:=[Coef(rat[1],var,0),Coef(rat[2],var,0)]; Local(summed,add); summed := Eval(PartFracExpand(Rem(rat[1],rat[2]),rat[2])); add:=(rat[1]/rat[2] - summed); add + summed; } , {  Expand(Quotient(rat[1],rat[2])) + PartFracExpand(Rem(rat[1],rat[2]),rat[2]); } );};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "PartFracExpand(_g,_f) <--{ Local(mlist,vlist,res); mlist:=Map(\"^\",Transpose(Factors(f))); vlist:=Map(\"Rem\",[FillList(g,Length(mlist)),mlist]); Decide(Integer?(f), res:=ChineseRemainderInteger(mlist,vlist)[2], res:=ChineseRemainderPoly(mlist,vlist)[2] ); Local(result,divlist); divlist:=Map(\"/\",[res,mlist]); Decide(Length(divlist)<?2, Add(divlist), { result:=divlist[1]; ForEach(item,Rest(divlist)) { result:=ListToFunction([ToAtom(\"+\"),result,item]); }; result; });};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Diagonal?(A_Matrix?) <--{ Local(i,j,m,n,result); m := Length(A); n := Length(A[1]); result := (m=?n);   i:=2; While(i<=?m And? result) { j:=1; While(j<=?n And? result) { result:= (i=?j Or? A[i][j] =? 0); j++; }; i++; }; Decide(m=?2, { result := result And? (A=?Transpose(A)); } ); result;};";
        scriptMap.put("Diagonal?",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CanBeMonomial(_expr)_(Type(expr)=?\"UniVariate\") <-- False;10 # CanBeMonomial(_expr)<--Not? (HasFunction?(expr,ToAtom(\"+\")) Or? HasFunction?(expr,ToAtom(\"-\")));10 # Monomial?(expr_CanBeMonomial) <-- { Local(r); Decide( RationalFunction?(expr), r := (VarList(Denominator(expr)) =? []), r := True );};15 # Monomial?(_expr) <-- False;";
        scriptMap.put("Monomial?",scriptString);
        scriptMap.put("CanBeMonomial",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CDF(BernoulliDistribution(_p), x_Number?) <-- Decide(x<=?0,0,Decide(x>?0 And? x<=?1, p,1));11 # CDF(BernoulliDistribution(_p), _x) <-- Hold(Decide(x<=?0,0,Decide(x>?0 And? x<=?1, p,1)));10 # CDF(BinomialDistribution(_p,_n),m_Number?)_(m<?0) <-- 0;10 # CDF(BinomialDistribution(_p,n_Integer?),m_Number?)_(m>?n) <-- 1;10 # CDF(BinomialDistribution(_p,_n),_m) <-- Sum @ [ i, 0, Floor(m), PMF(BinomialDistribution(p,n),i)];10 # CDF(DiscreteUniformDistribution( a_Number?, b_Number?), x_Number?)_(x<=?a) <-- 0;10 # CDF(DiscreteUniformDistribution( a_Number?, b_Number?), x_Number?)_(x>?b) <-- 1;10 # CDF(DiscreteUniformDistribution( a_Number?, b_Number?), x_Number?)_(a<?x And? x<=?b) <-- (x-a)/(b-a+1);11 # CDF(DiscreteUniformDistribution( _a, _b), _x) <--Hold(Decide(x<=?a,0,Decide(x<=?b,(x-a)/(b-a),1)));10 # CDF(PoissonDistribution(_l), x_Number?)_(x<=?0) <-- 0;10 # CDF(PoissonDistribution(_l), _x) <-- Sum @ [i,0,x,PMF(PoissonDistribution(l),i)];10 # CDF(ChiSquareDistribution(_m), _x) <-- IncompleteGamma(m/2,x/2)/Gamma(x/2);10 # CDF(DiscreteDistribution( dom_List?, prob_List?), _x) <-- { Local(i,cdf,y); i := 1; cdf:=0; y:=dom[i]; While(y<?x) {cdf:=cdf+prob[i];i++;}; cdf; };10 # CDF(HypergeometricDistribution( N_Number?, M_Number?, n_Number?), x_Number?)_(M <=? N And? n <=? N) <-- { Sum @ [i,0,x,PMF(HypergeometricDistribution(N, M, n),i)];};10 # CDF(NormalDistribution(_m,_s), _x) <-- 1/2 + 1/2 * ErrorFunction((x - m)/(s*Sqrt(2))); ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PDF(ExponentialDistribution(_l), _x) <-- Decide(x<?0,0,l*Exp(-l*x));10 # PDF(NormalDistribution(_m,_s),_x) <-- Exp(-(x-m)^2/(2*s^2))/Sqrt(2*Pi*s^2); 10 # PDF(ContinuousUniformDistribution(_a,_b),x)_(a<?b) <-- Decide(x<?a Or? x>?b,0,1/(b-a));10 # PDF(DiscreteDistribution( dom_List?, prob_List?), _x)_( Length(dom)=?Length(prob) And? Simplify(Add(prob))=?1) <-- { Local(i); i:=Find(dom,x); Decide(i =? -1,0,prob[i]); };10 # PDF( ChiSquareDistribution( _m),x_RationalOrNumber?)_(x<=?0) <-- 0;20 # PDF( ChiSquareDistribution( _m),_x) <-- x^(m/2-1)*Exp(-x/2)/2^(m/2)/Gamma(m/2);10 # PDF(tDistribution(_m),x) <-- Gamma((m+1)/2)*(1+x^2/m)^(-(m+1)/2)/Gamma(m/2)/Sqrt(Pi*m);";
        scriptMap.put("PDF",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PMF(BernoulliDistribution(_p),0) <-- p;10 # PMF(BernoulliDistribution(_p),1) <-- 1-p;10 # PMF(BernoulliDistribution(_p),x_Number?)_(x !=? 0 And? x !=? 1) <-- 0;10 # PMF(BernoulliDistribution(_p),_x) <-- Hold(Decide(x=?0,p,Decide(x=?1,1-p,0)));10 # PMF(BinomialDistribution(_p,_n),_k) <-- BinomialCoefficient(n,k)*p^k*(1-p)^(n-k);10 # PMF(DiscreteUniformDistribution(_a,_b), x_Number?) <-- Decide(x<?a Or? x>?b, 0 ,1/(b-a+1));11 # PMF(DiscreteUniformDistribution(_a,_b), _x) <-- Hold(Decide(x<?a Or? x>?b, 0 ,1/(b-a+1)));10 # PMF(PoissonDistribution(_l), n_Number?) <-- Decide(n<?0,0,Exp(-l)*l^n/n!);11 # PMF(PoissonDistribution(_l),_n) <-- Exp(-l)*l^n/n!;10 # PMF(GeometricDistribution(_p),_n) <--Decide(n<?0,0,p*(1-p)^n);10 # PMF(DiscreteDistribution( dom_List?, prob_List?), _x)_( Length(dom)=?Length(prob) And? Simplify(Add(prob))=?1) <-- { Local(i); i:=Find(dom,x); Decide(i =? -1,0,prob[i]); }; 10 # PMF(HypergeometricDistribution( N_Number?, M_Number?, n_Number?), x_Number?)_(M <=? N And? n <=? N) <-- (BinomialCoefficient(M,x) * BinomialCoefficient(N-M, n-x))/BinomialCoefficient(N,n);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"RForm\",[expression]);RulebaseHoldArguments(\"RForm\",[expression, precedence]);Function (\"RFormBracketIf\", [predicate, string]){ Check(Boolean?(predicate) And? String?(string), \"Argument\", \"RForm internal error: non-boolean and/or non-string argument of RFormBracketIf\"); Decide(predicate, ConcatStrings(\"( \", string, \") \"), string);};RFormMaxPrec() := 60000; 100 # RForm(_x) <-- RForm(x, RFormMaxPrec());110 # RForm(x_Integer?, _p) <-- ToString(x);111 # RForm(x_Zero?, _p) <-- \"0.\";112 # RForm(x_Number?, _p) <-- ToString(x);190 # RForm(False, _p) <-- \"false\";190 # RForm(True, _p) <-- \"true\";190 # RForm(Pi, _p) <-- \"pi\";200 # RForm(x_Atom?, _p) <-- ToString(x);100 # RForm(x_String?, _p) <-- ConcatStrings(\"\\\"\", x, \"\\\"\");LocalSymbols(rFormRegularOps) { rFormRegularOps :=  [  [\"+\",\"+\"], [\"-\",\"-\"], [\"*\",\"*\"], [\"/\",\"/\"], [\"/\",\"/\"], [\"^\",\"^\"], [\"=?\",\"==\"], [\">=?\",\">=\"], [\">?\",\">\"], [\"<=?\",\"<=\"], [\"<?\",\"<\"], [\"!=\",\"!=\"], [\"..\",\":\"], [\"Not?\",\"!\"], [\":=\",\"<-\"], [\"sequence\",\":\"], [\"True\",\"TRUE\"], [\"Modulo\",\"%%\"], ]; RFormRegularOps() := rFormRegularOps;}; LocalSymbols(rFormMathFunctions) { rFormMathFunctions := [ [\"NthRoot\",\"root\"], [\"Infinite\",\"Inf\"], [\"Undefined\",\"NaN\"], [\"Sin\",\"sin\"], [\"Cos\",\"cos\"], [\"Tan\",\"tan\"], [\"ArcSin\",\"asin\"], [\"ArcCos\",\"acos\"], [\"ArcTan\",\"atan\"], [\"ArcSinh\",\"asinh\"], [\"ArcCosh\",\"acosh\"], [\"ArcTanh\",\"atanh\"], [\"ArcCsc\",\"acsc\"], [\"ArcCsch\",\"acsch\"], [\"ArcSec\",\"asec\"], [\"ArcSech\",\"asech\"], [\"ArcCot\",\"acot\"], [\"ArcCoth\",\"acoth\"], [\"Exp\",\"exp\"], [\"Ln\",\"log\"], [\"Sqrt\",\"sqrt\"], [\"Bin\",\"choose\"], [\"Gamma\",\"gamma\"], [\"!\",\"factorial\"], [\"Limit\",\"limit\"], [\"Deriv\",\"deriv\"], [\"Integrate\",\"integrate\"], [\"Taylor\",\"?\"], [\"List\",\"list\"], ]; RFormMathFunctions() := rFormMathFunctions;};  100 # RForm(+ _y, _p) <-- RFormBracketDecide(p<?PrecedenceGet(\"+\"), ConcatStrings(\" + \", RForm(y, RightPrecedenceGet(\"+\")) ) ); 100 # RForm(- _y, _p) <-- RFormBracketDecide(p<?PrecedenceGet(\"-\"), ConcatStrings(\" - \", RForm(y, RightPrecedenceGet(\"-\")) ) ); 100 # RForm(_x ^ _y, _p) <-- RFormBracketDecide(p <=? PrecedenceGet(\"^\"), ConcatStrings(RForm(x, RFormMaxPrec()), \"^\", RForm(y, RFormMaxPrec())) );100 # RForm(If(_pred)_body, _p) <-- \"If(\"~RForm(pred,60000)~\") \"~RForm(body);100 # RForm(_left Else _right, _p) <-- RForm(left)~\" Else \"~RForm(right);120 # RForm(expr_Function?, _p)_(ArgumentsCount(expr)=?2 And? Contains?(AssocIndices(RFormRegularOps()), Type(expr)) ) <-- RFormBracketDecide(p<?PrecedenceGet(Type(expr)), ConcatStrings(RForm(FunctionToList(expr)[2], LeftPrecedenceGet(Type(expr))), RFormRegularOps()[Type(expr)], RForm(FunctionToList(expr)[3], RightPrecedenceGet(Type(expr))) ) );120 # RForm(expr_Function?, _p) _ (ArgumentsCount(expr)=?1 And? Contains?(AssocIndices(RFormMathFunctions()), Type(expr)) ) <-- ConcatStrings(RFormMathFunctions()[Type(expr)], \"(\", RForm( FunctionToList(expr)[2], RFormMaxPrec()),\")\" );RFormArgs(list_List?) <--{ Local(i,nr,result); result:=\"\"; nr:=Length(list); For (i:=1,i<=?nr,i++) { result:=result~RForm(list[i]); Decide(i<?nr, result:=result~\", \"); }; result;};200 # RForm(_x, _p)_(Function?(x)) <--{ ConcatStrings(Type(x), \"(\", RFormArgs(Rest(FunctionToList(x))),\")\" );};100 # RForm(Complex(0, 1), _p) <-- \"I\";100 # RForm(Complex(_x, 0), _p) <-- RForm(x, p);110 # RForm(Complex(_x, 1), _p) <-- RForm(x+Hold(I), p);110 # RForm(Complex(0, _y), _p) <-- RForm(Hold(I)*y, p);120 # RForm(Complex(_x, _y), _p) <-- RForm(x+Hold(I)*y, p);100 # RForm(Modulo(_x, _y), _p) <-- RFormBracketDecide(p<?PrecedenceGet(\"/\"), ConcatStrings(RForm(x, PrecedenceGet(\"/\")), \" % \", RForm(y, PrecedenceGet(\"/\")) ) );100 # RForm(Nth(_x, _i), _p) <-- ConcatStrings(RForm(x, RFormMaxPrec()), \"[\", RForm(i, RFormMaxPrec()), \"]\");LocalSymbols(RIndent) { RIndent:=1; RNlIndented():= { Local(result); result:=\"\"; Local(i); For(i:=1,i<?RIndent,i++) { result:=result~\" \"; }; result; }; RIndent() := { (RIndent++); \"\"; }; RUndent() := { (RIndent--); \"\"; };}; RFormStatement(_x) <-- RForm(x) ~ \";\" ~ RNlIndented();120 # RForm(_x,_p)_(Type(x) =? \"Block\") <--{ Local(result); result:=RIndent()~\"[\"~RNlIndented(); ForEach(item,Rest(FunctionToList(x))) { result:=result~RFormStatement(item); }; result:=result~\"]\"~RUndent()~RNlIndented(); result;};120 # RForm(For(_from,_to,_step)_body,_p) <-- \"for(\" ~ RForm(from,RFormMaxPrec()) ~ \";\" ~ RForm(to,RFormMaxPrec()) ~ \";\" ~ RForm(step,RFormMaxPrec()) ~ \")\" ~ RIndent() ~ RNlIndented() ~ RFormStatement(body) ~ RUndent();120 # RForm(While(_pred)_body, _p) <-- \"while(\" ~ RForm(pred,RFormMaxPrec()) ~ \")\" ~ RIndent() ~ RNlIndented() ~ RFormStatement(body) ~ RUndent();";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # MakeNewPoly( _poly ) <-- MakeNewPoly(poly,[x,y,z]);  20 # MakeNewPoly( _poly, polyVars_List? ) <-- {      Decide(InVerboseMode(),Tell(\" MakeNewPoly\",poly)); Local(p,nTerms,ii,dexp,npVars,pVars,ind,polyExp,pwrProd,pwrProdT,newDexp); npVars := polyVars; nVars := Length(npVars); dexp := DisassembleExpression(poly,npVars); nTerms := Length(dexp[3]); pVars := dexp[1]; Decide(InVerboseMode(), { Tell(\" \",nTerms);  Tell(\" \",ReassembleListTerms(dexp));  Tell(\" \",[pVars,npVars]); } ); newDexp := FlatCopy(dexp); newDexp[1] := npVars; pwrProd := dexp[2]; Decide(InVerboseMode(), {Tell(\" \",newDexp); Tell(\" \",pwrProd);}); pwrProdT := Transpose(pwrProd); polyExp := Concat([dexp[3]],pwrProdT); polyExp := Push(Transpose(polyExp),npVars); polyExp := Push(polyExp,NP); Decide(InVerboseMode(),Tell(\" \",polyExp)); polyExp; }; UnFence(\"MakeNewPoly\",2);";
        scriptMap.put("MakeNewPoly",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Retract(\"npOrderTerms\",*);Retract(\"LEXgt\",*);Retract(\"LEXlt\",*);Retract(\"GRLEXgt\",*);Retract(\"GRLEXlt\",*);Retract(\"GREVLEXgt\",*);Retract(\"GREVLEXlt\",*); 10 # npOrderTerms( expr_NP?, _termOrder ) <-- {      Decide(InVerboseMode(),{Tell(\"npOrderTerms\",expr); Tell(\" \",termOrder);}); Local(np,v,out); v := expr[2]; np := npTerms(expr); out := NP~v~npOrderTerms(np,termOrder); };]; 10 # npOrderTerms( expr_List?, _termOrder ) <-- {      Decide(InVerboseMode(),{Tell(\"npOrderTerms\",expr); Tell(\" \",termOrder);}); Local(np,out); np := FlatCopy(expr);  If( termOrder =? LEX ) { Decide(InVerboseMode(),Tell(\" ordering terms by LEX\")); out := Sort( np, LEXgt ); } Else If( termOrder =? GRLEX ) { Decide(InVerboseMode(),Tell(\" ordering terms by GRLEX\"));  out := Sort( np, GRLEXgt ); } Else If( termOrder =? GREVLEX ) { Decide(InVerboseMode(),Tell(\" ordering terms by GREVLEX\")); out := Sort( np, GREVLEXgt ); } Else  { Check(False,\"TermOrder\",\"npOrderTerms should never reach this point\"); }; out; };]; 20 # npOrderTerms( expr_NP? ) <-- npOrderTerms(expr,LEX); 25 # npOrderTerms( expr_List? ) <-- npOrderTerms(expr,LEX);           10 # LEXgt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(nonzero); nonzero := Select(Rest(term1-term2),\"NotZero?\"); Length(nonzero) !=? 0 And? First(nonzero) >? 0; }; 10 # LEXlt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(nonzero); nonzero := Select(Rest(term1-term2),\"NotZero?\"); Length(nonzero) !=? 0 And? First(nonzero) <? 0; }; 10 # GRLEXgt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(sum1,sum2,nonzero); sum1 := Sum(Rest(term1)); sum2 := Sum(Rest(term2)); nonzero := Select(Rest(term1-term2),\"NotZero?\"); Decide(sum1 >? sum2, True, Decide(sum1=?sum2 And? (Length(nonzero)!=?0 And? First(nonzero)>?0),True,False) ); }; 10 # GRLEXlt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(sum1,sum2,nonzero); sum1 := Sum(Rest(term1)); sum2 := Sum(Rest(term2)); nonzero := Select(Rest(term1-term2),\"NotZero?\"); Decide(sum1 <? sum2, True, Decide(sum1=?sum2 And? (Length(nonzero)!=?0 And? First(nonzero)<?0),True,False) ); }; 10 # GREVLEXgt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(sum1,sum2,revnonzero); sum1 := Sum(Rest(term1)); sum2 := Sum(Rest(term2)); revnonzero := Reverse(Select(Rest(term1-term2),\"NotZero?\")); Decide(sum1 >? sum2, True, Decide(sum1=?sum2 And? (Length(revnonzero)!=?0 And? First(revnonzero)<?0),True,False) ); }; 10 # GREVLEXlt(term1_List?,term2_List?)_(Length(term1)=?Length(term2)) <-- { Local(sum1,sum2,revnonzero); sum1 := Sum(Rest(term1)); sum2 := Sum(Rest(term2)); revnonzero := Reverse(Select(Rest(term1-term2),\"NotZero?\")); Decide(sum1 <? sum2, True, Decide(sum1=?sum2 And? (Length(revnonzero)!=?0 And? First(revnonzero)>?0),True,False) ); }; ";
        ...
        scriptMap.put("NP?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " Retract(\"+$\", *); Retract(\"-$\", *); Retract(\"/$\", *); Retract(\"*$\", *); Retract(\"^$\", *);  Infix(\"+$\",70); Infix(\"-$\",70); RightPrecedenceSet(\"-$\",40); Infix(\"/$\",30); Infix(\"*$\",40); Infix(\"^$\",20); LeftPrecedenceSet(\"^$\",19);  RightAssociativeSet(\"^$\"); Prefix(\"+$\",50); Prefix(\"-$\",50); RightPrecedenceSet(\"-$\",40);   RulebaseEvaluateArguments(\"+$\",[lhs, rhs]); RulebaseEvaluateArguments(\"-$\",[lhs, rhs]); RulebaseEvaluateArguments(\"/$\",[lhs, rhs]); RulebaseEvaluateArguments(\"*$\",[lhs, rhs]); RulebaseEvaluateArguments(\"^$\",[lhs, rhs]); RulebaseEvaluateArguments(\"+$\",[operand]); RulebaseEvaluateArguments(\"-$\",[operand]); Position(subTerm, term) :={ Local(positionList);  Check(Function?(term), \"Argument\", \"The argument must be an expression that contains a function.\");  positionList := PositionHelper(subTerm, term,[False]);  positionList := Rest(positionList);  Rest(Reverse(positionList));};PositionHelper(subTerm, term, list) :={  Local(argPosition); If(Atom?(term) And? (subTerm =? term)) { list[1] := True;  list; } Else If(Atom?(term) ) { list; } Else { argPosition := Length(term);  While(argPosition >? 0) {  list := PositionHelper(subTerm, term[argPosition], list);  Decide(list[1] =? True, {list := Append(list, argPosition);argPosition := -1;});  argPosition--; };  list; };};Occurrence(subTerm, term) :={ Check(Function?(term), \"Argument\", \"The argument must be an expression that contains a function.\"); Count(VarListAll(term), subTerm);};SingleOccurrence?(subTerm, term) :={ Occurrence(subTerm, term) =? 1;};Retract(\"SolveEquation\",*);RulebaseListedHoldArguments(\"SolveEquation\",[equation, variable, optionsList]);5 # SolveEquation(_equation, _variable) <-- SolveEquation(equation, variable, []);10 # SolveEquation(_equation, _variable, optionsList_List?) <--{ Local(options, path, steps);  options := OptionsToAssociativeList(optionsList);  equation := AddDollarSigns(equation);  steps := Isolate2(variable, equation);};20 # SolveEquation(_equation, _variable, _singleOption) <-- SolveEquation(equation, variable, [singleOption]);Isolate2(variable, equation) :={ Local(result, rest, currentEquation, path, showTree); result := [];  path := Position(variable, equation);  DestructiveAppend(result, [equation,\"Equation\", \"The original equation.\", variable]);  rest := [equation]; ForEach(pathNumber, path) { currentEquation := First(rest);  rest := Isolax(pathNumber, currentEquation);  DestructiveAppend(result, rest); };  result;};Retract(\"StepsView\",*);RulebaseListedHoldArguments(\"StepsView\",[steps, optionsList]);5 # StepsView(_steps) <-- StepsView(steps, []);10 # StepsView(_steps, optionsList_List?) <--{ Local(result, currentEquation, options, problemMessage);  result := [];  options := OptionsToAssociativeList(optionsList);  problemMessage := \"Solve\\\\ \" ~ ToString(TexFormNoDollarSigns(steps[1][1])) ~ \"\\\\ for\\\\ \" ~ ToString(steps[1][4]) ~ \".\";  DestructiveAppend(result, LatexView(problemMessage));   Decide(options[\"ShowTree\"] =? \"True\", DestructiveAppend(result, TreeView(RemoveDollarSigns(steps[1][1]), Scale -> 2)));   steps := Rest(steps);  ForEach(rest, steps) {  If(Not? List?(rest[1])) { result := Compose(result, rest[1], rest[3], rest[4], Length(rest) =? 4, options[\"ShowTree\"]); } Else { result := Compose(result, rest[1][1], rest[3][1], rest[4][1], Length(rest) =? 4, options[\"ShowTree\"]);  result := Compose(result, rest[1][2], rest[3][2], rest[4][2], Length(rest) =? 4, options[\"ShowTree\"]); };  };  result;};20 # StepsView(_steps, _singleOption) <-- StepsView(steps, [singleOption]);Compose(result, newExpression, explanation, beforeSimplification, lengthFour?, showTree?) :={ DestructiveAppend(result, LatexView(\" \\\\vspace{.75in}\")); DestructiveAppend(result, LatexView(ToLatex(explanation)));  If(lengthFour?) { DestructiveAppend(result, LatexView(\" \\\\vspace{.10in}\")); DestructiveAppend(result, LatexView(TexFormNoDollarSigns(beforeSimplification))); };  DestructiveAppend(result, LatexView(\" \\\\vspace{.10in}\"));  DestructiveAppend(result, LatexView(TexFormNoDollarSigns(newExpression)));  Decide(showTree? =? \"True\", DestructiveAppend(result, TreeView(RemoveDollarSigns(newExpression), Scale -> 2)));  result; };RemoveDollarSigns(equation) :={ equation := Subst(ToAtom(\"+$\"),ToAtom(\"+\")) equation; equation := Subst(ToAtom(\"-$\"),ToAtom(\"-\")) equation; equation := Subst(ToAtom(\"*$\"),ToAtom(\"*\")) equation; equation := Subst(ToAtom(\"/$\"),ToAtom(\"/\")) equation; equation := Subst(ToAtom(\"^$\"),ToAtom(\"^\")) equation;};AddDollarSigns(equation) :={ equation := Subst(ToAtom(\"+\"),ToAtom(\"+$\")) equation; equation := Subst(ToAtom(\"-\"),ToAtom(\"-$\")) equation; equation := Subst(ToAtom(\"*\"),ToAtom(\"*$\")) equation; equation := Subst(ToAtom(\"/\"),ToAtom(\"/$\")) equation; equation := Subst(ToAtom(\"^\"),ToAtom(\"^$\")) equation;};TexFormNoDollarSigns(equation) := { ToAtom(ListToString(Remove(StringToList(UnparseLatex(RemoveDollarSigns(equation))),\"$\")));};ToLatex(string) :={ Local(oldList, newList);  oldList := StringToList(string);  oldList := Remove(oldList, \"$\");  newList := []; ForEach(character, oldList) { DestructiveAppend(newList, Decide(character =? \" \", \"\\\\ \", character)); };  ListToString(newList);};Isolax(side, equation) :={ side : equation /: [  1: -$ _lhs == _rhs <- [lhs == -$ rhs, \"UnaryMinus\", \"Multiply both sides by \" ~ ExpressionToString(-1) ~ \":\", -1 *$ -$ lhs == rhs *$ -1],  1: _term1 +$ _term2 == _rhs <- [term1 == rhs -$ term2, \"Addition1\", \"Subtract \" ~ ExpressionToString(term2) ~ \" from both sides:\", -$ term2 +$ term1 +$ term2 == rhs -$ term2], 2: _term1 +$ _term2 == _rhs <- [term2 == rhs -$ term1, \"Addition2\", \"Subtract \" ~ ExpressionToString(term1) ~ \" from both sides:\", -$ term1 +$ term1 +$ term2 == rhs -$ term1],  1: _term1 -$ _term2 == _rhs <- [term1 == rhs +$ term2, \"Subtraction1\", \"Add \" ~ ExpressionToString(term2) ~ \" to both sides:\", term2 +$ term1 -$ term2 == rhs +$ term2], 2: _term1 -$ _term2 == _rhs <- [term2 == term1 -$ rhs, \"Subtraction2\", \"Add \" ~ ExpressionToString(term1) ~ \" to both sides:\", term1 +$ term1 -$ term2 == rhs +$ term1],  1: _term1 *$ _term2 == _rhs <- [term1 == rhs /$ term2, \"Multiplication1\", \"Divide both sides by \" ~ ExpressionToString(term2) ~ \":\", (term1 *$ term2) /$ term2 == rhs /$ term2], 2: _term1 *$ _term2 == _rhs <- [term2 == rhs /$ term1, \"Multiplication2\", \"Divide both sides by \" ~ ExpressionToString(term1) ~ \":\", (term1 *$ term2) /$ term1 == rhs /$ term1],  1: _term1 /$ _term2 == _rhs <- [term1 == rhs *$ term2, \"Division1\", \"Multiply both sides by \" ~ ExpressionToString(term2) ~ \":\", term2 *$ (term1 /$ term2) == rhs *$ term2], 2: _term1 /$ _term2 == _rhs <- [[rhs *$ term2 == term1, term2 == term1 /$ rhs], \"Division2\", [\"Multiply both sides by \" ~ ExpressionToString(term2) ~ \" and exchange sides:\", \"Divide both sides by \" ~ ToString(TexFormNoDollarSigns(rhs)) ~ \":\"], [(term1 /$ term2) *$ term2 == rhs *$ term2, (rhs *$ term2) /$ rhs == term1 /$ rhs]],      1: _term1 ^$ _term2 == _rhs <- [term1 == rhs^$(1/$term2), \"Exponentiation1b\", \"Take root \" ~ ExpressionToString(term2) ~ \" of both sides:\", (term1 ^$ term2)^$(1/term2) == rhs^$(1/$ term2)],   ]; };";
        ...
        scriptMap.put("Isolate2",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Pslq(x, precision) :={ Local (ndigits, gam, A, B, H, n, i, j, k, s, y, tmp, t, m, maxi, gami, t0, t1, t2, t3, t4, mini, Confidence, norme,result); n:=Length(x); ndigits:=BuiltinPrecisionGet(); BuiltinPrecisionSet(precision+10);  Confidence:=10^(-FloorN(NM(Eval(precision/3)))); gam:=NM(Sqrt(4/3)); For (i:=1, i<=?n,i++) x[i]:=NM(Eval(x[i])); A:=Identity(n);  B:=Identity(n);  s:=ZeroVector(n); y:=ZeroVector(n); For(k:=1,k<=?n,k++) { tmp:=0; For (j:=k,j<=?n,j++) tmp:=tmp + NM(x[j]^2); s[k]:=SqrtN(tmp); }; tmp:=NM(Eval(s[1])); For (k:= 1,k<=? n,k++) { y[k]:=NM(Eval(x[k]/tmp)); s[k]:=NM(Eval(s[k]/tmp)); }; H:=ZeroMatrix(n, n-1); For (i:=1,i<=? n,i++) { If(i <=? n-1) { H[i][i]:=NM(s[i + 1]/s[i]); }; For (j:= 1,j<=?i-1,j++) { H[i][j]:= NM(-(y[i]*y[j])/(s[j]*s[j + 1])); }; }; For (i:=2,i<=?n,i++) { For (j:=i-1,j>=? 1,j--) { t:=Round(H[i][j]/H[j][j]); y[j]:=y[j] + t*y[i]; For (k:=1,k<=?j,k++) { H[i][k]:=H[i][k]-t*H[j][k]; }; For (k:=1,k<=?n,k++) { A[i][k]:=A[i][k]-t*A[j][k]; B[k][j]:=B[k][j] + t*B[k][i]; }; }; }; Local(found); found:=False; While (Not?(found)) { m:=1; maxi:=NM(gam*Abs(H[1][1])); gami:=gam; For (i:= 2,i<=? n-1,i++) { gami:=gami*gam; tmp:=NM(gami*Abs(H[i][i])); If(maxi <? tmp) { maxi:=tmp; m:=i; }; }; tmp:=y[m + 1]; y[m + 1]:=y[m]; y[m]:=tmp; For (i:= 1,i<=?n,i++) { tmp:=A[m + 1][ i]; A[m + 1][ i]:=A[m][ i]; A[m][ i]:=tmp; tmp:=B[i][ m + 1]; B[i][ m + 1]:=B[i][ m]; B[i][ m]:=tmp; }; For (i:=1,i<=?n-1,i++) { tmp:=H[m + 1][ i]; H[m + 1][ i]:=H[m][ i]; H[m][ i]:=tmp; }; If(m <? n-1) { t0:=NM(Eval(Sqrt(H[m][ m]^2 + H[m][ m + 1]^2))); t1:=H[m][ m]/t0; t2:=H[m][ m + 1]/t0; For (i:=m,i<=?n,i++) { t3:=H[i][ m]; t4:=H[i][ m + 1]; H[i][ m]:=t1*t3 + t2*t4; H[i][ m + 1]:= -t2*t3 + t1*t4; }; }; For (i:= 1,i<=? n,i++) { For (j := Minimum(i-1, m + 1),j>=? 1,j--) { t:=Round(H[i][ j]/H[j][ j]); y[j]:=y[j] + t*y[i]; For (k:=1,k<=?j,k++) H[i][ k]:=H[i][ k]-t*H[j][ k]; For (k:= 1,k<=?n,k++) { A[i][ k]:=A[i][ k]-t*A[j][ k]; B[k][ j]:=B[k][ j] + t*B[k][ i]; }; }; };  maxi := NM(Dot(H[1], H[1])); For (j:=2,j<=?n,j++) { tmp:=NM(Dot(H[j], H[j]),10); If(maxi <? tmp) { maxi:=tmp; }; }; norme:=NM(Eval(1/Sqrt(maxi))); m:=1; mini:=NM(Eval(Abs(y[1]))); maxi:=mini; For (j:=2,j<=?n,j++) { tmp:=NM(Eval(Abs(y[j]))); If(tmp <? mini) { mini:=tmp; m:=j; }; If(tmp >? maxi) { maxi:=tmp; }; };  If((mini/maxi) <? Confidence)  {  BuiltinPrecisionSet(ndigits); result:=Transpose(B)[m]; found:=True; } Else { maxi:=Abs(A[1][ 1]); For (i:=1,i<=?n,i++) { For (j:=1,j<=?n,j++) { tmp:=Abs(A[i][ j]); If(maxi <? tmp) { maxi:=tmp;}; }; }; If(maxi >? 10^(precision)) { BuiltinPrecisionSet(ndigits); result:=Fail; found:=True; }; BuiltinPrecisionSet(precision+2); }; }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RadSimp(_n)_(Length(VarList(n))<?1) <--{ Local(max, result); Assign(max, CeilN(NM(Eval(n^2)))); Assign(result,0); Assign(result,RadSimpTry(n,0,1,max)); If(CheckRadicals(n,result)) result Else n;};20 # RadSimp(_n) <-- n;CheckRadicals(_n,_test) <-- Abs(NM(Eval(n-test),20)) <? 0.000001;10 # ClampRadicals(_r)_(NM(Eval(Abs(r)), 20)<?0.000001) <-- 0;20 # ClampRadicals(_r) <-- r;RadSimpTry(_n,_result,_current,_max)<--{ If(LessThan?(NM(Eval(result-n)), 0)) { Local(i);  i:=BSearch(max,Hold([[try],ClampRadicals(NM(Eval((result+Sqrt(try))-n),20))])); Decide(i>?0, { Assign(result,result+Sqrt(i)); Assign(i,AddN(max,1)); Assign(current,AddN(max,1)); });  If(LessThan?(NM(Eval(result-n)), 0)) { For (Assign(i,current),i<=?max,Assign(i,AddN(i,1))) { Local(new, test); Assign(test,result+Sqrt(i)); Assign(new,RadSimpTry(n,test,i,max)); If(CheckRadicals(n,new)) { Assign(result,new); Assign(i,AddN(max,1)); }; }; }; }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(assumptions) {assumptions := [];10 # Assume( L_List? ) <--{ Decide(InVerboseMode(),Tell(\"AssumeLst\",L)); Local(len,s); len := Length(L); Decide( len >? 0, ForEach(s,L) { Assume(s); } ); assumptions;};10 # Assume( _x -> _y ) <--{  Decide(InVerboseMode(),Tell(\"AssumeItem\",[x,y])); Local(key,value); key := Hold(x);  value := Hold(y); Decide(InVerboseMode(),Tell(\" \",[key,value]));  DestructiveAppend(assumptions,[Eval(key),Eval(value)]); assumptions;};10 # AssumptionsGet() <-- assumptions;10 # AssumptionsAbout(_key) <--{ Local(props); props := Select(assumptions,Lambda([X],X[1]=?key)); Decide( Length(props) >? 0, Transpose(props)[2], [] );};10 # Assumed?( _key, _valueExpected ) <-- Contains?(AssumptionsAbout(key),valueExpected);10 # UnAssume( _x )_(Contains?(AssocIndices(assumptions),x)) <--{ Local(lst,len,jj); lst := Lambda([X],Decide(List?(X),X[1])) /@ assumptions; jj := Find(lst,x); Decide( jj >? 0, DestructiveDelete(assumptions,jj) ); lst := Lambda([X],Decide(List?(X),X[1])) /@ assumptions; jj := Find(lst,x); Decide( jj >? 0, UnAssume(x), True );};}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CheckSolution( _expr, _var, solution_List? )_(Not? FreeOf?(var,expr)) <-- { Local(expr0,result,s,r); Decide( Equation?(expr), Assign(expr0,EquationLeft(expr)-EquationRight(expr)), Assign(expr0,expr) ); result := []; ForEach(s,solution)  { r := ( expr0 Where s ); Decide(r=?0,Push(result,s)); }; Reverse(result); };  20 # CheckSolution( _expr, _var, _solution ) <-- False;";
        scriptMap.put("CheckSolution",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Newton\",[function,variable,initial,accuracy]){  `Newton(@function,@variable,initial,accuracy,-Infinity,Infinity);};Function(\"Newton\",[function,variable,initial,accuracy,min,max]){ Local(result,adjust,delta,requiredPrec); MacroLocal(variable); requiredPrec := BuiltinPrecisionGet(); accuracy:=NM((accuracy/10)*10);  BuiltinPrecisionSet(requiredPrec+2); function:=NM(function); adjust:= -function/Apply(\"Differentiate\",[variable,function]); delta:=10000; result:=initial; While (result >? min And? result <? max  And? NM(Eval( Maximum(Re(delta), -Re(delta), Im(delta), -Im(delta)) ) ) >? accuracy) { MacroAssign(variable,result); delta:=NM(Eval(adjust)); result:=result+delta; }; BuiltinPrecisionSet(requiredPrec); result:=NM(Eval((result/10)*10));  If(result <=? min Or? result >=? max) {result := Fail;}; result;};";
        scriptMap.put("Newton",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SolveSystem(_eqns, _vars) <-- SolveSimpleBackSubstitution(eqns,vars);10 # SolveSimpleBackSubstitutionFindAlternativeForms((_lx) == (_rx)) <--{ Local(newEq); newEq := (Simplify(lx) == Simplify(rx)); Decide(newEq !=? (lx == rx) And? newEq !=? (0==0),DestructiveAppend(eq,newEq)); newEq := (Simplify(lx - rx) == 0); Decide(newEq !=? (lx == rx) And? newEq !=? (0==0),DestructiveAppend(eq,newEq));};20 # SolveSimpleBackSubstitutionFindAlternativeForms(_equation) <--{};UnFence(\"SolveSimpleBackSubstitutionFindAlternativeForms\",1);10 # SolveSimpleBackSubstitution(eq_List?,var_List?) <--{ Decide(InVerboseMode(), Echo([\"Entering SolveSimpleBackSubstitution\"])); Local(result,i,j,nrvar,nreq,sub,nrSet,origEq); eq:=FlatCopy(eq); origEq:=FlatCopy(eq); nrvar:=Length(var); result:=[FlatCopy(var)]; nrSet := 0; ForEach(equation,origEq) { SolveSimpleBackSubstitutionFindAlternativeForms(equation); }; nreq:=Length(eq);  For(j:=1,j<=?nreq And? nrSet <? nrvar,j++) { Local(vlist); vlist:=VarListAll(eq[j],`Lambda([pt],Contains?(@var,pt))); For(i:=1,i<=?nrvar And? nrSet <? nrvar,i++) { Decide(Count(vlist,var[i]) =? 1, { sub := FunctionToList(eq[j]); sub := sub[2]-sub[3]; sub:=SuchThat(sub,var[i]); Decide(InVerboseMode(), Echo([\"From \",eq[j],\" it follows that \",var[i],\" = \",sub])); Decide(SolveFullSimplify=?True, result:=Simplify(Subst(var[i],sub)result), result[1][i]:=sub ); nrSet++; Local(k,reset); reset:=False; For(k:=1,k<=?nreq And? nrSet <? nrvar,k++) Decide(Contains?(VarListAll(eq[k],`Lambda([pt],Contains?(@var,pt))),var[i]), { Local(original); original:=eq[k]; eq[k]:=Subst(var[i],sub)eq[k]; Decide(Simplify(Simplify(eq[k])) =? (0 == 0), eq[k] := (0 == 0), SolveSimpleBackSubstitutionFindAlternativeForms(eq[k]) ); Decide(original!=?(0==0) And? eq[k] =? (0 == 0),reset:=True); Decide(InVerboseMode(), Echo([\" \",original,\" simplifies to \",eq[k]])); }); nreq:=Length(eq); vlist:=VarListAll(eq[j],`Lambda([pt],Contains?(@var,pt))); i:=nrvar+1;  Decide(reset,j:=1); }); }; }; Local(zeroeq,tested); tested:=[]; ForEach(item,result) { DestructiveAppend(tested,Map(\"==\",[var,item])); }; Decide(InVerboseMode(), Echo([\"Leaving SolveSimpleBackSubstitution\"])); tested;};10 # OldSolve(eq_List?,var_List?) <-- SolveSimpleBackSubstitution(eq,var);90 # OldSolve((left_List?) == right_List?,_var) <-- OldSolve(Map(\"==\",[left,right]),var);100 # OldSolve(_left == _right,_var) <-- SuchThat(left - right , 0 , var);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"SolveMatrix\",[matrix,vector]){ Decide(InVerboseMode(),Tell(\" SolveMatrix\",[matrix,vector])); Local(perms,indices,inv,det,n); n:=Length(matrix); indices:=BuildList(i,i,1,n,1); perms:=PermutationsList(indices); inv:=ZeroVector(n); det:=0; ForEach(item,perms) { Local(i,lc); lc := LeviCivita(item); det:=det+Product(i,1,n,matrix[i][item[i] ])* lc; For(i:=1,i<=?n,i++) { inv[i] := inv[i]+ Product(j,1,n, Decide(item[j] =? i,vector[j ],matrix[j][item[j] ]))*lc; }; }; Check(det !=? 0, \"Math\", \"Zero determinant\"); (1/det)*inv;};";
        scriptMap.put("SolveMatrix",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # SolveSystem( eqns_List?, vars_List? )_(Length(eqns)=?1 And? Length(vars)=?1) <--{ [Solve(eqns[1],vars[1])];};12 # SolveSystem( eqns_List?, vars_List? ) <--{  Decide(InVerboseMode(),Tell(\"SolveSystem\",[eqns,vars]));  Local(eq,expr,exprns,VaD,isLinearSet,ans);  exprns := []; ForEach(eq,eqns) { expr := Decide( Equation?(eq), EquationLeft(eq)-EquationRight(eq), eq ); DestructiveAppend(exprns,expr); }; Decide(InVerboseMode(),Tell(\" \",exprns));   VaD := VarsAndDegs(exprns,vars); Decide(InVerboseMode(),Tell(\" \",VaD)); isLinearSet := Maximum(Flatten(VaD,\"List\"))=?1; Decide(InVerboseMode(),Tell(\" \",isLinearSet));  Decide( isLinearSet, ans:=SolveLinearSystem( exprns, vars ), ans:=SolveNonlinearSystem( exprns, vars ) ); Decide(ans !=? [], ans := Map(\"==\",[vars,ans])); Decide(InVerboseMode(),Tell(\"\",ans)); ans;};10 # VarsAndDegs(exs_List?,vars_List?) <--{ Local(ex,v,result); result := []; ForEach(ex,exs) { Local(res); res := []; ForEach(v,vars) { DestructiveAppend(res,Apply(\"Degree\",[ex,v])); }; DestructiveAppend(result,res); }; result;};10 # SolveLinearSystem( polys_List?, vars_List? ) <--{ Local(A, E); Decide(InVerboseMode(),Tell(\" SolveLinearSystem\",[polys,vars]));  Local(lhs,rhs,zeros); lhs := MakeCoefMatrix(polys,vars); Decide(InVerboseMode(),Tell(\" \",lhs)); zeros := ZeroVector(Length(vars)); rhs := -WithValue(vars,zeros,polys); Decide(InVerboseMode(),Tell(\" \",rhs));  A := Transpose(Concat(Transpose(lhs),[rhs])); E := RREF(A); Decide(Contains?(E,BaseVector(Dimensions(E)[2],Dimensions(E)[2])), [], MatrixColumn(E,Dimensions(E)[2]) - (ExtractSubMatrix(E, 1, 1, Length(E), Length(E)) - Identity(Length(E))) * vars);};10 # MakeCoefMatrix(polys_List?,vars_List?) <--{ Decide(InVerboseMode(),Tell(\" MakeCoefMatrix\",[polys,vars])); Local(p,v,result); result := []; ForEach(p,polys) { Local(res); res := []; ForEach(v,vars) { DestructiveAppend(res,Apply(\"Coef\",[p,v,1])); }; DestructiveAppend(result,res); }; result;};";
        scriptMap.put("SolveSystem",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " RulebaseHoldArguments(\"xPSolve\",[uni]);RuleHoldArguments(\"xPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 1){ Decide(iDebug,Tell(\" xPSolve_1\",uni)); [-Coef(uni,0)/Coef(uni,1)];};RuleHoldArguments(\"xPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 2){ Decide(iDebug,Tell(\" xPSolve_2\",uni)); Local(a,b,c,d,q,r); c:=Coef(uni,0); b:=Coef(uni,1); a:=Coef(uni,2); Decide(iDebug,Tell(\" \",[a,b,c])); d:=b*b-4*a*c; Decide(iDebug,Tell(\" \",d));  q:=Sqrt(d)/(2*a); Decide(iDebug,Tell(\" \",q)); r:=Simplify(-b/(2*a)); Decide(iDebug,Tell(\" \",r)); [r+q,r-q];};RuleHoldArguments(\"xPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 3 ){ Decide(iDebug,Tell(\" xPSolve_3\",uni)); Local(p,q,r,s,t,w1,w2,a,b); Local(c0,c1,c3,adjust);  w1 := (1/2)*Complex(-1, Sqrt(3)); w2 := Conjugate(w1); Decide( iDebug, Tell(\" \",[w1,w2]) );   adjust := (-uni[3][3])/(3*uni[3][4]); Decide( iDebug, Tell(\" \",adjust)); c3 := uni[3][4]; c1 := (3*uni[3][4]*adjust+2*uni[3][3])*adjust+uni[3][2]; c0 :=((uni[3][4]*adjust+uni[3][3])*adjust+uni[3][2])*adjust+uni[3][1]; Decide( iDebug, Tell(\" \",[c0,c1,c3]));   Assert(\"Invariant\", \"Coefficients Must be Real\") And?(Im(c0)=?0,Im(c1)=?0,Im(c2)=?0); Decide( Error?(\"Invariant\"), DumpErrors() ); p :=c3; q :=c1/p; r :=c0/p; Decide( iDebug, Tell(\" \",[p,q,r])); Local(a3,b3,qq,r1,r2,r3); qq := Sqrt(q^3/27 + r^2/4); a3 := -r/2 + qq; b3 := -r/2 - qq;  Decide( iDebug, {Tell(\" \",[qq,a3,b3]); Tell(\" \",NM(a3+b3+r)); Tell(\" \",NM(a3-b3-2*qq));}); a := (a3)^(1/3); b := (b3)^(1/3); Decide( iDebug, Tell(\" \",[a,b])); r1 := a+b+adjust; r2 := w1*a+w2*b+adjust; r3 := w2*a+w1*b+adjust;  Decide( iDebug, { Tell(\" \",r1); Tell(\" \",r2); Tell(\" \",r3); } ); [r1,r2,r3];};RuleHoldArguments(\"xPSolve\",1,1,UniVar?(uni) And? Degree(uni) =? 4 ){ Decide(iDebug,Tell(\" xPSolve_4\",uni)); Local(coef4,a1,a2,a3,a4,y,y1,z,t,s); coef4:=Coef(uni,4); a1:=Coef(uni,3)/coef4; a2:=Coef(uni,2)/coef4; a3:=Coef(uni,1)/coef4; a4:=Coef(uni,0)/coef4; Decide( iDebug, Tell(\" \",[a1,a2,a3,a4]));  Local(ys); ys := xPSolveCubic(y^3-a2*y^2+(a1*a3-4*a4)*y+(4*a2*a4-a3^2-a1^2*a4)); Decide( iDebug, {NewLine(); Tell(\" \",ys[1]);} ); y1:=First(ys); Decide( iDebug, Tell(\" \",y1)); t := Sqrt(y1^2/4-a4); Decide( iDebug, Tell(\" \",t)); Decide(t=?0, s:=Sqrt(y1+a1^2/4-a2), s:=(a1*y1-2*a3)/(4*t)); Decide( iDebug, Tell(\" \",s));  Local(q11,q12,q21,q2,quad1,quad2); q11 := a1/2+s; q12 := y1/2+t; q21 := a1/2-s; q22 := y1/2-t; Decide( iDebug, Tell(\" \",[q11,q12])); Decide( iDebug, Tell(\" \",[q21,q22])); quad1 := z^2 + q11*z + q12; quad2 := z^2 + q21*z + q22; Decide( iDebug, Tell(\" \",[quad1,quad2]));  Local(r1,r2,r3,r4); [r1,r2] := xPSolve( quad1, z ); [r3,r4] := xPSolve( quad2, z ); r1 := NearRational(NM(r1,10),8); r2 := NearRational(NM(r2,10),8); r3 := NearRational(NM(r3,10),8); r4 := NearRational(NM(r4,10),8);  [r1,r2,r3,r4];};Function(\"xPSolve\",[expr,var]){ Decide( Not? Assigned?(iDebug), iDebug := False ); Decide(iDebug,Tell(\"xPSolve_notUni\",[expr,var]));  Local(lhs,rhs,cc,pp,uni,solnpp,solncc,soln); Decide( Equation?(expr),  { Decide(iDebug,Tell(\" is Equation\")); lhs := EquationLeft(expr); rhs := EquationRight(expr); expr := lhs - rhs; } ); Decide(iDebug,Tell(\" \",expr)); cc := xContent(expr); pp := xPrimitivePart(expr,cc); Decide(iDebug,Tell(\" \",[cc,pp])); solnpp := xPSolve(MakeUni(pp,var)); Decide(iDebug,Tell(\" \",solnpp)); Decide( Length(VarList(cc)) >? 0 And? Contains?(VarList(cc),var ), { solncc := xPSolve(MakeUni(cc,var)); Decide(iDebug,Tell(\" \",solncc)); soln := Concat(solncc,solnpp); }, { soln := solnpp; } ); soln;};10 # xPSolveCubic( poly_Polynomial? )_ (Length(VarList(poly))=?1 And? Degree(poly)=?3) <--{ Decide( iDebug, Tell(\" xPSolveCubic\",poly) ); Local(var,coeffs,ans); var := VarList(poly)[1]; coeffs := Coef(poly,var,3 .. 0); Decide( iDebug, Tell(\" \",[var,coeffs])); ans := xPSC1(coeffs); };UnFence(\"xPSolveCubic\",1);10 # xPSC1( coeffs_List? ) <--{ Decide( iDebug, Tell(\" xPSC1\",coeffs) );  Local(f,g,h,j,iType,ans); f := coeffs[2]/coeffs[1]/3; g := coeffs[3]/coeffs[1]/3 - f^2; h := coeffs[4]/coeffs[1]/2 + f^3 - f * coeffs[3]/coeffs[1]/2; j := g^3 + h^2; Decide( iDebug, Tell(\" \",[f,g,h,j]) ); ans := xPSC2( [f,g,h,j] );};10 # xPSC2( xs_List? )_(xs[4]=?0) <--{ Decide( iDebug, Tell(\" Type 1\",xs) );  Local(f,g,h,j,m,r1,r2,r3,ans); [f,g,h,j] := FlatCopy(xs); m := 2*(-h)^(1/3); r1 := NearRational(NM(m - f,10),8); r2 := NearRational(NM(-m/2 - f,10),8); r3 := NearRational(NM(-m/2 - f,10),8); ans := [r1,r2,r3];};10 # xPSC2( xs_List? )_(xs[4]>?0) <--{ Decide( iDebug, Tell(\" Type 2\",xs) );  Local(f,g,h,j,k,l1,l2,m,n,r1,r2,r3,ans);  [f,g,h,j] := FlatCopy(xs); k := Sqrt(j); l1 := (-h + k)^(1/3); l2 := (-h - k)^(1/3); m := l1 + l2; n := (l1 - l2)*Sqrt(3)/2; r1 := NearRational(NM(m - f,10),8); r2 := NearRational(NM(-m/2 - f + I*n,10),8); r3 := NearRational(NM(Conjugate(r2),10),8); ans := [r1,r2,r3]; };10 # xPSC2( xs_List? )_(xs[4]<?0 And? xs[3]=?0) <--{ Decide( iDebug, Tell(\" Type 3a\",xs) ); Local(f,g,h,j,p,r1,r2,r3,ans); [f,g,h,j] := FlatCopy(xs); p := 2*Sqrt(-g); r1 := NearRational(NM(-f,10),8); r2 := NearRational(NM( p*Sqrt(3)/2 - f,10),8); r3 := NearRational(NM(-p*Sqrt(3)/2 - f,10),8); ans := [r1,r2,r3];};10 # xPSC2( xs_List? )_(xs[4]<?0 And? xs[3]>?0) <--{ Decide( iDebug, Tell(\" Type 3b\",xs) ); Local(p,x,alpha,beta,gama,r1,r2,r3,ans); [f,g,h,j] := FlatCopy(xs); p := 2*Sqrt(-g); k := Sqrt(-j); alpha := ArcTan(k/(-h));  beta := Pi + alpha; gama := beta / 3; Decide( iDebug, { Tell(\" \",[p,k]); Tell(\" \",[alpha,beta,gama]); Tell(\" \",57.2957795*NM([alpha,beta,gama])); Tell(\" \",NM(Cos(gama))); } ); r1 := NearRational(NM(p * Cos(gama) - f,10),8); r2 := NearRational(NM(p * Cos(gama+2*Pi/3) - f,10),8); r3 := NearRational(NM(p * Cos(gama+4*Pi/3) - f,10),8); ans := [r1,r2,r3];};10 # xPSC2( xs_List? )_(xs[4]<?0 And? xs[3]<?0) <--{ Decide( iDebug, Tell(\" Type 3c\",xs) ); Local(f,g,h,j,p,k,alpha,beta,gama,r1,r2,r3,ans); [f,g,h,j] := FlatCopy(xs); p := 2*Sqrt(-g); k := Sqrt(-j); alpha := ArcTan(k/(-h));  beta := alpha; gama := beta / 3; Decide(iDebug,{Tell(\" \",[p,k]); Tell(\" \",[alpha,beta,gama]);}); r1 := NearRational(NM(p * Cos(gama) - f,10),8); r2 := NearRational(NM(p * Cos(gama+2*Pi/3) - f,10),8); r3 := NearRational(NM(p * Cos(gama+4*Pi/3) - f,10),8); ans := [r1,r2,r3];};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SmallSort(_list, _first, _last, _compare) _ (last=?first) <-- list;SmallSort(_list, _first, _last, _compare) _ (last=?first+1) <--{ Local(temp); temp := list[first]; Decide( Apply(compare,[temp,list[last]]), list, { list[first] := list[last]; list[last] := temp; }  ); list;};SmallSort(_list, _first, _last, _compare) _ (last=?first+2) <--{ Local(temp); temp := list[first]; Decide( Apply(compare,[list[first+1],temp]), { list[first] := list[first+1]; list[first+1] := temp; }  );  temp := list[last]; Decide( Apply(compare,[list[first],temp]), Decide(  Apply(compare,[list[first+1],temp]), list, { list[last] := list[first+1]; list[first+1] := temp; }  ), {  list[last] := list[first+1]; list[first+1] := list[first]; list[first] := temp; } ); list;};";
        scriptMap.put("SmallSort",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SmallSortIndexed(_list, _first, _last, _compare)_(last=?first) <-- list;SmallSortIndexed(_list, _first, _last, _compare)_(last=?first+1) <--{ Local(temp); temp := list[first]; Decide( Apply(compare,[temp[1],list[last][1]]), list, { list[first] := list[last]; list[last] := temp; }  ); list;};SmallSortIndexed(_list, _first, _last, _compare)_(last=?first+2) <--{ Local(temp); temp := list[first]; Decide( Apply(compare,[list[first+1][1],temp[1]]), { list[first] := list[first+1]; list[first+1] := temp; }  );  temp := list[last]; Decide( Apply(compare,[list[first][1],temp[1]]), Decide(  Apply(compare,[list[first+1][1],temp[1]]), list, { list[last] := list[first+1]; list[first+1] := temp; }  ), {  list[last] := list[first+1]; list[first+1] := list[first]; list[first] := temp; } ); list;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"ApproxInfSum\",[expr,start,x]){ ApproxInfSum(expr,start,x,[0]);};Function(\"ApproxInfSum\",[expr,start,x,c]){ Local(term,result,k); Local(prec,eps,tmp); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+2);  eps:=10^(-prec); term:=1; k:=start; result:=0; While( NM(Abs(term) >=? eps) ){ term:=NM(Eval(expr));  k:=k+1; result:=result+term; }; Decide(InVerboseMode(), Echo(\"ApproxInfSum: Info: using \", k, \" terms of the series\")); BuiltinPrecisionSet(prec);   result;};";
        scriptMap.put("ApproxInfSum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Bernoulli(0) <-- 1;10 # Bernoulli(1) <-- -1/2;15 # Bernoulli(n_Integer?)_(n<?0) <-- Undefined;30 # Bernoulli(n_Odd?) <-- 0;20 # Bernoulli(n_Even?)_(n<=?Bernoulli1Threshold()) <-- InternalBernoulliArray(n)[n+1];20 # Bernoulli(n_Even?)_(n>?Bernoulli1Threshold()) <-- Bernoulli1(n);LocalSymbols(bernoulli1Threshold) {  Decide(Not? Assigned?(bernoulli1Threshold), bernoulli1Threshold := 20); Bernoulli1Threshold() := bernoulli1Threshold; SetBernoulli1Threshold(threshold) := { bernoulli1Threshold := threshold;};} ; Bernoulli(n_Integer?, _x) <-- { Local(B, i, result); B := InternalBernoulliArray(n); result := B[1]; For(i:=n-1, i>=?0, i--) { result := result * x + B[n-i+1]*BinomialCoefficient(n,i); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "CatalanConstNum1() :={ Local(prec,Aterm,Bterm,nterms,result,n); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(10);  nterms := 1+Floor(NM((prec*Ln(10)+Ln(prec*Ln(10)/Ln(2)))/Ln(2))); BuiltinPrecisionSet(prec+5); Aterm:=NM(1/2); result:= Aterm; Bterm:=Aterm; For(n:=1, n<=?nterms, n++ ) { Bterm:=DivideN(MultiplyN(Bterm,n), 2*n+1);  Aterm:=DivideN(MultiplyN(Aterm,n)+Bterm, 2*n+1);  result := result + Aterm; }; BuiltinPrecisionSet(prec); RoundTo(result,prec);};CatalanConstNum() :={ Local(prec, n, result); prec:=BuiltinPrecisionGet();  n := 1+Quotient(prec*1068+642,643);  BuiltinPrecisionSet(prec+2);  result := NM(1/(2*n+1)); While(n>?0) { result := DivideN(MultiplyN(result, n), 4*n+2)+DivideN(1,2*n-1); n := n-1; }; result := MultiplyNum(result, 3/8) + NM(Pi/8*Ln(2+Sqrt(3))); BuiltinPrecisionSet(prec); RoundTo(result,prec);};CatalanConstNum2() :={ Local(prec, n, result1, result2); prec:=BuiltinPrecisionGet();   n := 1+Quotient(prec*534+642,643);  BuiltinPrecisionSet(prec+2);  result1 := 0; While(n>=?0) { result1 := DivideN(result1, 16)+NM( +1/(8*n+1)^2 -1/(8*n+2)^2 +1/2/(8*n+3)^2 -1/4/(8*n+5)^2 +1/4/(8*n+6)^2 -1/8/(8*n+7)^2 ); n := n-1; };   n := 1+Quotient(prec*178+642,643);  BuiltinPrecisionSet(prec+2);  result2 := 0; While(n>=?0) { result2 := DivideN(result2, 4096)+NM( +1/(8*n+1)^2 +1/2/(8*n+2)^2 +1/8/(8*n+3)^2 -1/64/(8*n+5)^2 -1/128/(8*n+6)^2 -1/512/(8*n+7)^2 ); n := n-1; }; result1 := MultiplyNum(result1, 3/2) - MultiplyNum(result2, 1/4); BuiltinPrecisionSet(prec); RoundTo(result1,prec);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # DirichletBeta(1) <-- Pi/4;5 # DirichletBeta(2) <-- Catalan;5 # DirichletBeta(3) <-- Pi^3/32;6 # DirichletBeta(n_Odd?) <-- { Local(k); k:=(n-1)/2; (-1)^k*Euler(2*k)*(Pi/2)^(2*k+1)/(2*(2*k)!);};10 # DirichletBeta(x_RationalOrNumber?)_(NumericMode?() And? x>=?1 ) <-- { Local(prec,eps,term,result,k); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+3); eps:=10^(-prec); result:=0; term:=1; For(k:=0, Abs(term) >? eps, k++ ){ term:=(-1)^k/(2*k+1)^x; Echo(\"term is \",term); result:=result+term; }; BuiltinPrecisionSet(prec); RoundTo(result,prec);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Erf(0) <-- 0;10 # Erf(Infinity) <-- 1;10 # Erf(Undefined) <-- Undefined;10 # Erf(x_Number?)_(x<?0) <-- -Erf(-x);LocalSymbols(k){ 40 # Erf(_x)_(NumericMode?() And? (Number?(x) Or? Complex?(x)) And? Abs(x) <=? 1) <--{ Local(prec); prec := BuiltinPrecisionGet();  2 / SqrtN(InternalPi()) * x * SumTaylorNum(x^2, 1, [[k], -(2*k-1)/(2*k+1)/k],  NM(1+87/32*Exp(LambertW(prec*421/497)), 20) );};}; LocalSymbols(nmax, k){ 50 # Erf(_x)_(NumericMode?() And? (Number?(x) Or? Complex?(x)) And? ( {  nmax := 0; Re(x^2) >? BuiltinPrecisionGet()*3295/1431+0.121; } Or? {  nmax := NM(Minimum((BuiltinPrecisionGet()*3295/1431+0.121)/InternalLnNum(Abs(x)), 2*InternalLnNum(Abs(x))), 10); 2*Abs(x)+Re(x^2) >? BuiltinPrecisionGet()*3295/1431+0.121; } Or? {   nmax := NM(([[k], k+InternalLnNum(k)] @ BuiltinPrecisionGet()*3295/1431)/2 - 3/2, 10); Abs(x) >? nmax+3/2; } ) ) <-- Decide(Re(x)!=?0, Sign(Re(x)), 0) - Exp(-x^2)/x/SqrtN(InternalPi())  * SumTaylorNum(1/x^2, 1, [[k], -(2*k-1)/2 ], Maximum(0, Floor(nmax)));}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FresnelCos(0) <-- 0;10 # FresnelCos(Infinity) <-- 1/2;10 # FresnelCos(x_Number?)_(x<?0) <-- -FresnelCos(x);40 # FresnelCos(x_Number?)_(Abs(x) <=? 1) <-- NM(Sqrt(2/Pi)*ApproxInfSum((-1)^(k+1)*x^(4*k-3)/((4*k-3) * (2*k-2)! ),1,x));";
        scriptMap.put("FresnelCos",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # FresnelSin(0) <-- 0;10 # FresnelSin(Infinity) <-- 1/2;10 # FresnelSin(x_Number?)_(x<?0) <-- -FresnelSin(x);40 # FresnelSin(x_Number?)_(Abs(x) <=? 1) <-- NM(Sqrt(2/Pi)*ApproxInfSum((-1)^(k+1)*x^(2*k+1)/(k! * (2*k+1)),1,x));";
        scriptMap.put("FresnelSin",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Gamma(Infinity) <-- Infinity;10 # Gamma(_n)_(Integer?(n) And? n<=?0) <-- Infinity;20 # Gamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- (Round(2*n)/2-1)!;30 # Gamma(x_Constant?)_(NumericMode?()) <-- InternalGammaNum(NM(Eval(x)));";
        scriptMap.put("Gamma",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LambertW(0) <-- 0;10 # LambertW(Infinity) <-- Infinity;10 # LambertW(Undefined) <-- Undefined;10 # LambertW(-Infinity) <-- Infinity + I*Pi;10 # LambertW(-Exp(-1)) <-- -1;20 # LambertW(_x * Ln(_x)) <-- Ln(x);20 # LambertW(Ln(_x) * _x) <-- Ln(x);30 # LambertW(x_Constant?) _ NumericMode?() <-- InternalLambertWNum(Eval(x));10 # InternalLambertWNum(x_Number?)_(x <? -ExpN(-1)) <-- Undefined;20 # InternalLambertWNum(x_Number?) <--{ Local(W); NewtonNum( `Hold( [ [W], { Local(a); a:=W- @x*ExpN(-W); W-a/(W+1-(W+2)/(W+1)*a/2.); }]),  Decide( x<?0, x*ExpN(1) / (1+1 / (1 / SqrtN(2*(x*ExpN(1)+1)) - 1 / SqrtN(2) + 1/(ExpN(1)-1))), InternalLnNum(1+x)*(1-InternalLnNum(1+InternalLnNum(1+x))/(2+InternalLnNum(1+x))) ), 10,  3  );};";
        scriptMap.put("LambertW",scriptString);
        scriptMap.put("InternalLambertWNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # LnGamma(_n)_(Integer?(n) And? n<=?0) <-- Infinity;20 # LnGamma(n_RationalOrNumber?)_(PositiveInteger?(n) Or? FloatIsInt?(2*n)) <-- Ln((Round(2*n)/2-1)!);30 # LnGamma(x_Constant?)_(NumericMode?()) <-- InternalLnGammaNum(NM(Eval(x)));";
        scriptMap.put("LnGamma",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # PolyLog(_n,0) <-- 0;10 # PolyLog(1,1/2) <-- Ln(2);10 # PolyLog(_n,1) <-- Zeta(n);10 # PolyLog(_n,_m)_(m=? -1) <-- DirichletEta(n);10 # PolyLog(_n,_x)_(n<? 0) <-- (1/((1-x)^(-n+1)))*Sum(i,0,-n,Eulerian(-n,i)*x^(-n-i) );10 # PolyLog(0,_x) <-- x/(1-x);10 # PolyLog(1,_x) <-- -Ln(1-x);10 # PolyLog(2,1/2) <-- (Pi^2 - 6*Ln(2)^2)/12;10 # PolyLog(3,1/2) <-- (4*Ln(2)^3 - 2*Pi^2*Ln(2)+21*Zeta(3))/24;10 # PolyLog(2,2) <-- Pi^2/4 - Pi*I*Ln(2);20 # PolyLog(_n,_x)_(NumericMode?() And? x <? -1 ) <-- { Local(prec,result); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+5); Echo(\"Warning: PolyLog is only currently accurate for x in [-1,1]\"); result:= (-1)^(n-1)*PolyLog(n,1/x) - ((Ln(-x))^n)/n! - Sum(r,1,Round(n/2), 2^(2*r-2)*Pi^(2*r)*Abs(Bernoulli(2*r))*Ln(-x)^(n-2*r)/( (2*r)! * (n - 2*r)! ) ); BuiltinPrecisionSet(prec); RoundTo(NM(result),prec);};20 # PolyLog(_n,_x)_(NumericMode?() And? x>=? -1 And? x <? 0 ) <-- {   Local(prec,result); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(prec+5); result:=PolyLog(n,x^2)/2^(n-1) - PolyLog(n,-x) ; BuiltinPrecisionSet(prec); RoundTo(NM(result),prec);};20 # PolyLog(_n,_x)_(NumericMode?() And? x >? 0 And? x <? 1) <--{  Local(prec, result, terms); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(15);  terms := Floor(-prec*Ln(10)/Ln(x)); terms := Floor(-(prec*Ln(10)-(n-1)*Ln(terms))/Ln(x)); Decide(terms <? 4, terms := 4); BuiltinPrecisionSet(prec+2*IntLog(prec,10)+5); result := x*SumTaylorNum(x, [[k], 1/(k+1)^n], terms); BuiltinPrecisionSet(prec); RoundTo(result, prec);};";
        scriptMap.put("PolyLog",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Sinc(_x) <-- Decide(x=?0,1,Sin(x)/x);";
        scriptMap.put("Sinc",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # Zeta(1) <-- Infinity;10 # Zeta(0) <-- -1/2; 10 # Zeta(3)_NumericMode?() <-- Zeta3(); 10 # Zeta(n_Even?)_(n>?0) <-- Pi^n*(2^(n-1)/n! *Abs(Bernoulli(n)));10 # Zeta(n_Integer?)_(n<?0) <-- -Bernoulli(-n+1)/(-n+1);11 # Zeta(n_Infinity?) <-- 1;20 # Zeta(s_Constant?)_(NumericMode?()) <-- InternalZetaNum(NM(Eval(s)));";
        scriptMap.put("Zeta",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Bernoulli1(n_Even?)_(n>=?2) <-- { Local(B, prec); prec := BuiltinPrecisionGet();   BuiltinPrecisionSet(10); BuiltinPrecisionSet( Ceil(NM((1/2*Ln(8*Pi*n)-n+n*Ln(n/2/Pi))/Ln(10)))+3  ); Decide(InVerboseMode(), Echo([\"Bernoulli: using zeta funcion, precision \", BuiltinPrecisionSet(), \", n = \", n])); B := Floor(NM(  Decide(  n>?250,  InternalZetaNum2(n, n/17+1),    InternalZetaNum1(n, n/17+1)  ) *NM(2*n! /(2*Pi)^n)))  * (2*Modulo(n/2,2)-1)  + BernoulliFracPart(n);  BuiltinPrecisionSet(prec);  B;};";
        scriptMap.put("Bernoulli1",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BernoulliFracPart(n_Even?)_(n>=?2) <-- { Local(p, sum);  sum := 1/2+1/3;  Decide(Prime?(n+1), sum := sum+1/(n+1)); Decide(Prime?(n/2+1), sum := sum+1/(n/2+1));   For(p:=5, p<=?n/3+1, p:=NextPrime(p)) Decide(Modulo(n, p-1)=?0, sum := sum + 1/p);   Quotient(Numerator(sum), Denominator(sum)) - sum + Modulo(n/2,2);  };";
        scriptMap.put("BernoulliFracPart",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # InternalBernoulliArray(n_Integer?)_(n=?0 Or? n=?1) <-- { Local(B); B:=ArrayCreate(n+1,0); B[1] := 1; Decide(n=?1, B[2] := -1/2); B;};20 # InternalBernoulliArray(n_Integer?) <-- { Local(B, i, k, k2, bin); Decide(InVerboseMode(), Echo([\"InternalBernoulliArray: using direct recursion, n = \", n])); B:=ArrayCreate(n+1, 0);     B[1] := 1; B[2] := -1/2; B[3] := 1/6; For(i:=4, i<=?n, i := i+2)  {  bin := 1;   B[i+1] := 1/2-1/(i+1)*(1 + Sum(k, 1, i/2-1, { bin := bin * (i+3-2*k) * (i+2-2*k)/ (2*k-1) / (2*k); B[2*k+1]*bin;  } ) ); }; B;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BesselJN0\",[x]){ Local(ax,z,xx,y,result,res1,res2); Local(c1,c2,c3,c4);   c1:=[57568490574.0,-13362590354.0,651619640.7, -11214424.18,77392.33017,-184.9052456]; c2:=[57568490411.0,1029532985.0,9494680.718, 59272.64853,267.8532712];   c3:=[-0.001098628627,0.00002734510407,-0.000002073370639, 0.0000002093887211]; c4:=[-0.01562499995,0.0001430488765,-0.000006911147651, 0.0000007621095161,0.0000000934935152]; ax:=Abs(x); Decide( ax <? 8.0,{ y:=x^2; res1:=c1[1]+y*(c1[2]+y*c1[3]+y*(c1[4]+y*(c1[5]+y*(c1[6])))); res2:=c1[1]+y*(c2[2]+y*c2[3]+y*(c2[4]+y*(c2[5]+y*1.0))); result:=res1/res2; },{ z:=8/ax; y:=z^2; xx:=ax-0.785398164; res1:=1.0+y*(c3[1]+y*(c3[2]+y*(c3[3]+y*c4[4]))); res2:=c4[1]+y*(c4[2]+y*(c4[3]+y*(c4[4]-y*c4[5]))); result:=Sqrt(2/(Pi*x))*(Cos(xx)*res1-z*Sin(xx)*res2); } );};";
        scriptMap.put("BesselJN0",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"BesselNsmall\",[n,x,modified]){ Local(term,result,k); Local(prec,eps,tmp); prec:=BuiltinPrecisionGet(); BuiltinPrecisionSet(Ceil(1.2*prec));  eps:=5*10^(-prec); term:=1; k:=0; result:=0; While( Abs(term) >=? eps ){ term:=x^(2*k+n);   Decide( k%2=?1 And? modified=?0 , term:=term*-1 ); term:=NM(term/(2^(2*k+n)* k! * Gamma(k+n+1) ));  result:=result+term; k:=k+1; }; BuiltinPrecisionSet(prec);   RoundTo(result,prec);};";
        scriptMap.put("BesselNsmall",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "InternalGammaNum(z) := NM(Exp(InternalLnGammaNum(z)));InternalGammaNum(z,a) := NM(Exp(InternalLnGammaNum(z,a)));";
        scriptMap.put("InternalGammaNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # InternalLnGammaNum(_z, _a)_(NM(Re(z))<?0) <-- { Decide(InVerboseMode(), Echo([\"InternalLnGammaNum: using 1-z identity\"])); NM(Ln(Pi/Sin(Pi*z)) - InternalLnGammaNum(1-z, a));};20 # InternalLnGammaNum(_z, _a) <-- { Local(e, k, tmpcoeff, coeff, result); a := Maximum(a, 4);  Decide(InVerboseMode(), Echo([\"InternalLnGammaNum: precision parameter = \", a])); e := NM(Exp(1)); k:=Ceil(a);  result := 0;   While(k>?1) {  k:=k-1; result := NM( PowerN(a-k,k)/((z+k)*Sqrt(a-k))-result/(e*k) ); }; NM(Ln(1+Exp(a-1)/Sqrt(2*Pi)*result) + Ln(2*Pi)/2 -a-z+(z+1/2)*Ln(z+a) - Ln(z));};InternalLnGammaNum(z) := { Local(a, prec, result); prec := BuiltinPrecisionGet(); a:= Quotient((prec-IntLog(prec,10))*659, 526) + 0.4;     BuiltinPrecisionSet(Ceil(prec*1.4));  result := InternalLnGammaNum(z,a); BuiltinPrecisionSet(prec); result;};";
        scriptMap.put("InternalLnGammaNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GammaConstNum() :={ Local(k, n, A, B, Uold, U, Vold, V, prec, result); prec:=BuiltinPrecisionGet(); NonNM({ BuiltinPrecisionSet(prec+IntLog(prec,10)+3);  n:= 1+Ceil(prec*0.5757+0.2862);  A:= -InternalLnNum(n); B:=1; U:=A; V:=1; k:=0; Uold := 0;  Vold := 0; While(Uold-U !=? 0 Or? Vold-V !=? 0) { k++; Uold:=U; Vold:=V;  B:=MultiplyNum(B,n^2/k^2);   A:=MultiplyNum(MultiplyNum(A,n^2/k)+B, 1/k);  U:=U+A; V:=V+B; }; Decide(InVerboseMode(), Echo(\"GammaConstNum: Info: used\", k, \"iterations at working precision\", BuiltinPrecisionGet())); result:=DivideN(U,V);  }); BuiltinPrecisionSet(prec);  RoundTo(result, prec); };";
        scriptMap.put("GammaConstNum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "InternalZetaNum(_s, n_Integer?) <-- { Local(result, j, sign); Decide(InVerboseMode(), Echo([\"InternalZetaNum: Borwein's method, precision \", BuiltinPrecisionGet(), \", n = \", n])); result := 0; sign := 1;  For(j:=0, j<=?2*n-1, j++) {  result := NM(result + sign*InternalZetaNumCoeffEj(j,n)/(1+j)^s ); sign := -sign; }; NM(result/(2^n)/(1-2^(1-s)));};InternalZetaNum1(s, limit) := { Local(i, sum); Decide(InVerboseMode(), Echo([\"InternalZetaNum: direct method (sum), precision \", BuiltinPrecisionGet(), \", N = \", limit])); sum := 0; limit := Ceil(NM(limit)); For(i:=2, i<=?limit, i++) sum := sum+NM(1/PowerN(i, s)); sum+1; };InternalZetaNum2(s, limit) :={ Local(i, prod); Decide(InVerboseMode(), Echo([\"InternalZetaNum: direct method (product), precision \", BuiltinPrecisionGet(), \", N = \", limit])); prod := NM( (1-1/PowerN(2, s))*(1-1/PowerN(3,s)) ); limit := Ceil(NM(limit)); For(i:=5, i<=?limit, i:= NextPrime(i)) prod := prod*NM(1-1/PowerN(i, s)); 1/prod;};InternalZetaNumCoeffEj(j,n) := { Local(k); 2^n-Decide(j<?n, 0, Sum(k,0,j-n,BinomialCoefficient(n,k))  );};Zeta3() :={ Local(result, oldresult, k, term); NM({ For( { k:=1; result := 1; oldresult := -1; term := 1; }, oldresult!=?result, k++ ) { oldresult := result; term := -term * k^2 / ((2*k+1)*(2*k)); result := result + term/(k+1)^2; }; result := 5/4*result; }, BuiltinPrecisionGet()+IntLog(BuiltinPrecisionGet(),10)+1); result;};10 # InternalZetaNum(_s) _ (NM(s)=?0) <-- -0.5;10 # InternalZetaNum(_s) _ (NM(s)=?1) <-- Infinity;20 # InternalZetaNum(_s) <-- { Local(n, prec, result); prec := BuiltinPrecisionGet(); Decide(  NM(Re(s)) <? 0.5,  { Decide(InVerboseMode(), Echo([\"InternalZetaNum: using s->1-s identity, s=\", s, \", precision \", prec])); result := 2*Exp(InternalLnGammaNum(1-s)-(1-s)*Ln(2*InternalPi()))*Sin(InternalPi()*s/2) * InternalZetaNum(1-s); },  Decide(NM(Re(s)) >? NM(1+(prec*Ln(10))/(Ln(prec)+0.1), 6), {  n:= NM(10^(prec/(s-1)), 6)+2;  BuiltinPrecisionSet(prec+2);  result := InternalZetaNum1(s, n); }, {  n := Ceil( NM( prec*Ln(10)/Ln(8) + 2, 6 ) );  BuiltinPrecisionSet(prec+2);  result := InternalZetaNum(s, n); } ) ); BuiltinPrecisionSet(prec); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BinomialDistribution(_p, _n)_ (Decide(RationalOrNumber?(p),p<?0 Or? p>?1, False) Or? (Constant?(n) And? Not? PositiveInteger?(n)) ) <-- Undefined;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # ArcCos(x_Number?)_NumericMode?() <-- InternalPi()/2-ArcSin(x); 200 # ArcCos(0) <-- Pi/2;200 # ArcCos(1/2) <-- Pi/3;200 # ArcCos(Sqrt(1/2)) <-- Pi/4;200 # ArcCos(Sqrt(3/4)) <-- Pi/6;200 # ArcCos(1) <-- 0;200 # ArcCos(_n)_(n =? -1) <-- Pi;200 # ArcCos(_n)_(-n =? Sqrt(3/4)) <-- 5/6*Pi;200 # ArcCos(_n)_(-n =? Sqrt(1/2)) <-- 3/4*Pi;200 # ArcCos(_n)_(-n =? 1/2) <-- 2/3*Pi;200 # ArcCos(Undefined) <-- Undefined;ArcCos(xlist_List?) <-- MapSingle(\"ArcCos\",xlist);110 # ArcCos(Complex(_r,_i)) <-- (- I)*Ln(Complex(r,i) + (Complex(r,i)^2 - 1)^(1/2));";
        scriptMap.put("ArcCos",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcCosh(_x)_(NumericMode?() And? (Number?(x) Or? Type(x) =? \"Complex\")) <-- NM(Eval( Ln(x+Sqrt(x^2-1)) ));200 # ArcCosh(Infinity) <-- Infinity;200 # ArcCosh(-Infinity) <-- Infinity+I*Pi/2;200 # ArcCosh(Undefined) <-- Undefined;ArcCosh(xlist_List?) <-- MapSingle(\"ArcCosh\",xlist);";
        scriptMap.put("ArcCosh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "2 # ArcSin(x_Number?)_(NumericMode?() And? Abs(x)<=?1) <-- ArcSinNum(x);3 # ArcSin(x_Number?)_NumericMode?() <-- Sign(x)*(Pi/2+I*ArcCosh(x));110 # ArcSin(Complex(_r,_i)) <-- (- I) * Ln((I*Complex(r,i)) + ((1-(Complex(r,i)^2))^(1/2)));150 # ArcSin(- _x)_(Not? Constant?(x)) <-- -ArcSin(x);160 # (ArcSin(x_Constant?))_(NegativeNumber?(NM(Eval(x)))) <-- -ArcSin(-x);200 # ArcSin(0) <-- 0;200 # ArcSin(1/2) <-- Pi/6;200 # ArcSin(Sqrt(1/2)) <-- Pi/4;200 # ArcSin(Sqrt(3/4)) <-- Pi/3;200 # ArcSin(1) <-- Pi/2;200 # ArcSin(_n)_(n =? -1) <-- -Pi/2;200 # ArcSin(_n)_(-n =? Sqrt(3/4)) <-- -Pi/3;200 # ArcSin(_n)_(-n =? Sqrt(1/2)) <-- -Pi/4;200 # ArcSin(_n)_(-n =? 1/2) <-- -Pi/6;ArcSin(xlist_List?) <-- MapSingle(\"ArcSin\",xlist);200 # ArcSin(Undefined) <-- Undefined;";
        scriptMap.put("ArcSin",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcSinh(_x)_(NumericMode?() And? (Number?(x) Or? Type(x) =? \"Complex\")) <-- NM(Eval( Ln(x+Sqrt(x^2+1)) ));200 # ArcSinh(Infinity) <-- Infinity;200 # ArcSinh(-Infinity) <-- -Infinity;200 # ArcSinh(Undefined) <-- Undefined;ArcSinh(xlist_List?) <-- MapSingle(\"ArcSinh\",xlist);";
        scriptMap.put("ArcSinh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # (ArcTan(x_Constant?))_(NegativeNumber?(NM(Eval(x)))) <-- -ArcTan(-x);4 # ArcTan(-Tan(_x)) <-- -ArcTan(Tan(x));110 # ArcTan(Complex(_r,_i)) <-- (- I*0.5)*Ln(Complex(1,Complex(r,i))/ Complex(1, - Complex(r,i)));150 # ArcTan(- _x)_(Not? Constant?(x)) <-- -ArcTan(x);160 # (ArcTan(x_Constant?))_(NegativeNumber?(NM(Eval(x)))) <-- -ArcTan(-x);200 # ArcTan(Sqrt(3)) <-- Pi/3;200 # ArcTan(-Sqrt(3)) <-- -Pi/3;200 # ArcTan(1) <-- Pi/4;200 # ArcTan(0) <-- 0;200 # ArcTan(_n)_(n =? -1) <-- -Pi/4;200 # ArcTan(Infinity) <-- Pi/2;200 # ArcTan(-Infinity) <-- -Pi/2;200 # ArcTan(Undefined) <-- Undefined;ArcTan(xlist_List?) <-- MapSingle(\"ArcTan\",xlist);2 # ArcTan(x_Number?)_NumericMode?() <-- ArcTanNum(x);";
        scriptMap.put("ArcTan",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # ArcTanh(_x)_(NumericMode?() And? (Number?(x) Or? Type(x) =? \"Complex\")) <-- NM(Eval( Ln((1+x)/(1-x))/2 ));200 # ArcTanh(Infinity) <-- Infinity+I*Pi/2;200 # ArcTanh(-Infinity) <-- -Infinity-I*Pi/2; 200 # ArcTanh(Undefined) <-- Undefined;ArcTanh(xlist_List?) <-- MapSingle(\"ArcTanh\",xlist);";
        scriptMap.put("ArcTanh",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "1 # CosMap( _n )_(Not?(RationalOrNumber?(n))) <-- ListToFunction([ToAtom(\"Cos\"),n*Pi]);2 # CosMap( _n )_(n<?0) <-- CosMap(-n);2 # CosMap( _n )_(n>?2) <-- CosMap(Modulo(n,2));3 # CosMap( _n )_(n>?1) <-- CosMap(2-n);4 # CosMap( _n )_(n>?1/2) <-- -CosMap(1-n);5 # CosMap( 0 ) <-- 1;5 # CosMap( 1/6 ) <-- Sqrt(3)/2;5 # CosMap( 1/4 ) <-- Sqrt(2)/2;5 # CosMap( 1/3 ) <-- 1/2;5 # CosMap( 1/2 ) <-- 0;5 # CosMap( 2/5 ) <-- (Sqrt(5)-1)/4;10 # CosMap(_n) <-- ListToFunction([ToAtom(\"Cos\"),n*Pi]);2 # Cos(x_Number?)_NumericMode?() <-- CosNum(x);4 # Cos(ArcCos(_x)) <-- x;4 # Cos(ArcSin(_x)) <-- Sqrt(1-x^2);4 # Cos(ArcTan(_x)) <-- 1/Sqrt(1+x^2);5 # Cos(- _x)_(Not? Constant?(x)) <-- Cos(x);6 # (Cos(x_Constant?))_(NegativeNumber?(NM(Eval(x)))) <-- Cos(-x);110 # Cos(Complex(_r,_i)) <-- (Exp(I*Complex(r,i)) + Exp(- I*Complex(r,i))) / (2) ;6 # Cos(x_Infinity?) <-- Undefined;6 # Cos(Undefined) <-- Undefined;200 # Cos(v_CanBeUni(Pi))_(Not?(NumericMode?()) And? Degree(v,Pi) <? 2 And? Coef(v,Pi,0) =? 0) <-- CosMap(Coef(v,Pi,1));400 # Cos(x_RationalOrNumber?) <-- { Local(ll); ll:= FloorN(NM(Eval(x/Pi))); Decide(Even?(ll),x:=(x - Pi*ll),x:=(-x + Pi*(ll+1))); ListToFunction([Cos,x]); };400 # Cos(x_RationalOrNumber?) <-- { Local(ll); ll:= FloorN(NM(Eval(Abs(x)/Pi))); Decide(Even?(ll),x:=(Abs(x) - Pi*ll),x:=(-Abs(x) + Pi*(ll+1))); ListToFunction([Cos,x]); };Cos(xlist_List?) <-- MapSingle(\"Cos\",xlist);";
        scriptMap.put("Cos",scriptString);
        scriptMap.put("CosMap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Cosh(- _x) <-- Cosh(x);200 # Cosh(0) <-- 1;200 # Cosh(Infinity) <-- Infinity;200 # Cosh(-Infinity) <-- Infinity;200 # Cosh(ArcCosh(_x)) <-- x;200 # Cosh(ArcSinh(_x)) <-- Sqrt(1+x^2);200 # Cosh(ArcTanh(_x)) <-- 1/Sqrt(1-x^2);200 # Cosh(Undefined) <-- Undefined;Cosh(xlist_List?) <-- MapSingle(\"Cosh\",xlist);2 # Cosh(_x)_(NumericMode?() And? (Number?(x) Or? Type(x) =? \"Complex\")) <-- NM(Eval( (Exp(x)+Exp(-x))/2 ));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # MathBitCount(0) <-- 1;20 # MathBitCount(_x) _ (x<?0) <-- MathBitCount(-x);30 # MathBitCount(_value) <--{ Local(nbits); nbits:=0; Decide(value<?1, {  nbits := 1; While(value<?1) { nbits--; value := MathMul2Exp(value,1); }; }, {  While(value>=?1) { nbits++; value := MathMul2Exp(value, -1); }; }); nbits;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SumTaylorNum0(_x, _nthtermfunc, _nterms) <-- SumTaylorNum0(x, nthtermfunc, [], nterms);SumTaylorNum1(_x, _nthtermfunc, _nterms) <-- SumTaylorNum1(x, nthtermfunc, [], nterms);SumTaylorNum(_x, _nthtermfunc, _nterms) <--Decide( nterms >=? 30,   SumTaylorNum1(x, nthtermfunc, nterms), SumTaylorNum0(x, nthtermfunc, nterms));SumTaylorNum(_x, _nthtermfunc, _nexttermfactor, _nterms) <--Decide( nterms >=? 5,  SumTaylorNum1(x, nthtermfunc, nexttermfactor, nterms), SumTaylorNum0(x, nthtermfunc, nexttermfactor, nterms));1# SumTaylorNum0(_x, _nthtermfunc, [], _nterms) <--{ Local(sum, k); NM({  x:=Eval(x); sum := 0; For(k:=nterms, k>=?0, k--) sum := AddN(sum*x, nthtermfunc @ k); }); sum;};2# SumTaylorNum0(_x, _nthtermfunc, _nexttermfactor, _nterms) <--{ Local(sum, k, term, delta); NM({ x:=Eval(x);  Decide(Constant?(nthtermfunc), term := nthtermfunc, term := (nthtermfunc @ [0]), ); sum := term;  }); NonNM({ delta := 1; For(k:=1, k<=?nterms And? delta !=? 0, k++) { term := MultiplyNum(term, nexttermfactor @ [k], x);  delta := sum; sum := sum + term;  delta := Abs(sum-delta);  }; }); sum;};SumTaylorNum0(_x, _nthtermfunc, _nterms) <-- SumTaylorNum0(x, nthtermfunc, [], nterms);SumTaylorNum1(x, nthtermfunc, nexttermfactor, nterms) :={  Local(sum, rows, cols, rowstmp, lastpower, i, j, xpower, termtmp); NM({  x:=Eval(x);  rows := IntNthRoot(nterms+1, 2); cols := Quotient(nterms+rows, rows);  Check(rows>?1 And? cols>?1, \"Argument\", \"SumTaylorNum1: Internal error: number of Taylor sum terms must be at least 4\"); rowstmp := ArrayCreate(rows, 0); xpower := x ^ rows;      Decide( nexttermfactor =? [], termtmp := 1,  Decide(Constant?(nthtermfunc), termtmp := nthtermfunc, termtmp := (nthtermfunc @ [0]), ) ); }); NonNM({   For(i:=0, i<?cols, i++) {  For(j:=0, j<?rows And? (i<?cols-1 Or? i*rows+j<=?nterms), j++)  {   Decide( nexttermfactor =? [],  { rowstmp[j+1] := rowstmp[j+1] + MultiplyNum(termtmp, nthtermfunc @ [i*rows+j]); }, { rowstmp[j+1] := rowstmp[j+1] + termtmp;  termtmp := MultiplyNum(termtmp, nexttermfactor @ [i*rows+j+1]);  } ); };  termtmp := termtmp*xpower;  };   For({j:=rows; sum:=0;}, j>?0, j--) sum := sum*x + rowstmp[j]; }); sum;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Round(Infinity) <-- Infinity;5 # Round(-Infinity) <-- -Infinity;5 # Round(Undefined) <-- Undefined;10 # Round(x_RationalOrNumber?) <-- FloorN(NM(x+0.5));10 # Round(x_List?) <-- MapSingle(\"Round\",x);20 # Round(x_Complex?) _ (RationalOrNumber?(Re(x)) And? RationalOrNumber?(Im(x)) ) <-- FloorN(NM(Re(x)+0.5)) + FloorN(NM(Im(x)+0.5))*I;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"Taylor3MultiplyCoefs\",[coefs1,coefs2,degree]){ Local(result,i,j,jset,ilimit,jlimit); Assign(result, ArrayCreate(AddN(degree,1),0)); Assign(i,1); Assign(ilimit,AddN(degree,2)); While (Not? Equal?(i,ilimit)) { Assign(j,1); Assign(jlimit,AddN(degree,SubtractN(3,i))); While (Not? Equal?(j,jlimit)) { Assign(jset,AddN(i,SubtractN(j,1))); ArraySet(result,jset,ArrayGet(result,jset) + ArrayGet(coefs1,i)*ArrayGet(coefs2,j)); Assign(j,AddN(j,1)); }; Assign(i,AddN(i,1)); }; result;};10 # (Taylor3TaylorCoefs(_var,_degree)(_var)) <--{ Local(result); Assign(result,ArrayCreate(degree+1,0)); ArraySet(result,2, 1); result;};20 # (Taylor3TaylorCoefs(_var,_degree)(_atom))_(FreeOf?(var,atom)) <-- { Local(result); Assign(result,ArrayCreate(degree+1,0)); ArraySet(result,1, atom); result; };30 # (Taylor3TaylorCoefs(_var,_degree)(_X + _Y)) <-- { Local(result,add,i); Assign(result,Taylor3TaylorCoefs(var,degree)(X)); Assign(add, Taylor3TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) { ArraySet(result,i,ArrayGet(result,i)+ArrayGet(add,i)); }; result; };30 # (Taylor3TaylorCoefs(_var,_degree)(_X - _Y)) <-- { Local(result,add,i); Assign(result,Taylor3TaylorCoefs(var,degree)(X)); Assign(add, Taylor3TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) { ArraySet(result,i,ArrayGet(result,i)-ArrayGet(add,i)); }; result; };30 # (Taylor3TaylorCoefs(_var,_degree)( - _Y)) <-- { Local(result,add,i); Assign(result,Taylor3TaylorCoefs(var,degree)(Y)); For(i:=1,i<=?degree+1,i++) { ArraySet(result,i,-ArrayGet(result,i)); }; result; };30 # (Taylor3TaylorCoefs(_var,_degree)(_X * _Y)) <-- Taylor3MultiplyCoefs( Taylor3TaylorCoefs(var,degree)(X), Taylor3TaylorCoefs(var,degree)(Y), degree);30 # (Taylor3TaylorCoefs(_var,_degree)((_X) ^ N_PositiveInteger?)) <--{ Local(result,factor); factor:=Taylor3TaylorCoefs(var,degree)(X); result:=ArrayCreate(degree+1,0); result[1] := 1;  While(N>?0) { result:=Taylor3MultiplyCoefs(result,factor,degree); N--; }; result;};60 # Taylor3UniFunction(\"Exp\") <-- True;60 # Taylor3CompCoeff(\"Exp\", _n) <-- 1/n!;80 # Taylor3UniFunction(\"Ln\") <-- False; 80 # Taylor3CompCoeff(\"Ln\", 0) <-- 0;81 # Taylor3CompCoeff(\"Ln\", _n) <-- (-1)^(n+1)/n;90 # Taylor3UniFunction(\"Sin\") <-- True;90 # Taylor3CompCoeff(\"Sin\", n_Odd?) <-- (-1)^((n-1)/2) / n!;90 # Taylor3CompCoeff(\"Sin\", n_Even?) <-- 0;100 # Taylor3UniFunction(\"Cos\") <-- True;100 # Taylor3CompCoeff(\"Cos\", n_Odd?) <-- 0;100 # Taylor3CompCoeff(\"Cos\", n_Even?) <-- (-1)^(n/2) / n!;210 # Taylor3UniFunction(_any)_ ( { Local(result); result:= Deriv(var)ListToFunction([ToAtom(any),var]); Type(result) !=? \"Deriv\"; } ) <-- { True; };210 # Taylor3CompCoeff(_any, n_Integer?) <-- { Limit(var,0)(Deriv(var,n)(ListToFunction([ToAtom(any),var]))/n!); };60000 # Taylor3UniFunction(_any) <-- False;Taylor3FuncCoefs(_fname,_degree) <--{ Local(sins,i); Assign(sins, ArrayCreate(degree+1,0)); For (i:=0,i<=?degree,Assign(i,i+1)) { ArraySet(sins,i+1, Taylor3CompCoeff(fname,i)); }; sins;};100 # (Taylor3TaylorCoefs(_var,_degree)(Ln(_f)))_(Simplify(f-1) =? var) <-- Taylor3FuncCoefs(\"Ln\",degree);110 # (Taylor3TaylorCoefs(_var,_degree)(f_Function?))_(ArgumentsCount(f) =? 1 And? (Taylor3UniFunction(Type(f)))) <--{ Local(sins,i,j,result,xx,expr,sinfact); expr := f[1]; sins:=Taylor3FuncCoefs(Type(f),degree); expr:=Taylor3TaylorCoefs(var,degree)expr; result:=ArrayCreate(degree+1,0); ArraySet(result,1, ArrayGet(sins,1)); xx:=expr; For (i:=2,i<=?degree+1,i++) { Assign(sinfact,sins[i]); For (j:=1,j<=?degree+1,j++) { ArraySet(result,j,ArrayGet(result,j) + (ArrayGet(xx,j) * sinfact)); }; Assign(xx,Taylor3MultiplyCoefs(xx,expr,degree)); }; result;};(Taylor3(_var,_degree)(_expr)) <-- Add((Taylor3TaylorCoefs(var,degree)(expr))[1 .. degree+1]*var^(0 .. degree));10 # (Taylor3(_x, 0, _n) _y) <-- Taylor3(x,n) y;20 # (Taylor3(_x, _a, _n) _y) <-- Subst(x,x-a) Taylor3(x,n) Subst(x,x+a) y;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"TSum\",[indices,body]);10 # (TSum([])(_body)) <-- body;20 # (TSum(_indices)(_body))_(Integer?(Ndim)) <-- LocalSymbols(index,i,sum) { Local(index,i,sum); index:=indices[1]; sum:=0; MacroLocal(index); For(i:=1,i<=?Ndim,i++) { MacroAssign(index,i); sum:=sum+Eval(TSum(Rest(indices))body); }; sum; };UnFence(\"TSum\",2);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # EqualAsSets( A_List?, B_List? )_(Length(A)=?Length(B)) <--{ Local(Acopy,b,nba,result); Acopy := FlatCopy(A); result := True; ForEach(b,B) { nba := Find(Acopy,b); Decide( nba <? 0, { result := False; Break(); } ); DestructiveDelete(Acopy,nba); }; Decide( Not? result, result := Length(Acopy)=?0 ); result;};20 # EqualAsSets( _A, _B ) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CollectOn(_var,_expr)_(CanBeUni(var,expr)) <--{ Decide(InVerboseMode(),Echo(\"<< Collect on: \",var,\" in expression \",expr));  Local(u,a); u := MakeUni(expr,var); Decide( u[2] >? 0,  { a := FillList(0,u[2]); u[3] := Concat(a,u[3]); u[2] := 0; } ); u[3];};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function (\"UniVariateBinomial\",[x,q,a]){Local(L,i);L := [a];For (i:=1,i<?q,i++) DestructiveAppend(L,0);DestructiveAppend(L,1);UniVariate(x,0,L);};Function(\"SubstituteInUniVar\",[p,k]){ Local(c,i,d,j,NL); L := p[3];  NL := [];  d := Degree(p); i :=d; ForEach(c,L) {   Decide(i<?d, For (j:=1,j<?k,j++) DestructiveAppend(NL,0));  DestructiveAppend(NL,Decide(Even?(i),c,-c)); i--; }; UniVariate(First(p),0,NL);};Function(\"SubstituteAndExpandInUniVar\",[p,k]){ Local(result,i,var,first,coefs,c,nc,exponent); result:=0; var := p[1]; first:= p[2]; coefs:= p[3]; For(i:=Length(coefs),i>?0,i--) { Local(term); exponent := first+i-1; c:= coefs[i]; nc := Decide(Even?(exponent),c,-c); term:=NormalForm(nc*var^(exponent*k)); result:=result+term; }; result;};CyclotomicDivisorsList(n_PositiveInteger?) <--{ Local(nFactors,f,result,oldresult,x); nFactors:= Factors(n); result := [[1,nFactors[1][1],1]]; nFactors := Rest(nFactors); ForEach (f,nFactors) { oldresult := result; ForEach (x,oldresult) result:=Append(result,[x[1]*f[1],x[2]*f[1],-x[3]]); }; result;};CyclotomicFactor(_a,_b) <--{ Local(coef,p,i,j,result); p := a/b; result:= [[b*(p-1),1]]; For (i:= p-2,i>=?0,i--) DestructiveAppend(result,[b*i,1]); result;};10 # InternalCyclotomic(n_Even?,_x) <-- { Local(k,m,result,p,t); k := 1; m := n; While(Even?(m)) { k := k*2; m := m/2; }; k := k/2 ; Decide(m>?1, { p:= InternalCyclotomic(m,x)[2];  result:=[]; ForEach(t,p) DestructiveAppend(result, [t[1]*k,Decide(Even?(t[1]),t[2],-t[2])]); }, result := [[k,1],[0,1]]  ); SparseUniVar(x,result); };20 # InternalCyclotomic(n_Odd?,_x)_(n>?1) <--{ Local(divisors,poly1,poly2,q,d,f,coef,i,j,result); divisors := CyclotomicDivisorsList(n); poly1 := [[0,1]]; poly2 := [[0,1]]; ForEach (d,divisors) { Decide(InVerboseMode(),Echo(\"d=\",d)); f:= CyclotomicFactor(n/d[1],n/d[2]); Decide(d[3]=?1,poly1:=MultiplyTerms(poly1,f),poly2:=MultiplyTerms(poly2,f)); Decide(InVerboseMode(), { Echo(\"poly1=\",poly1); Echo(\"poly2=\",poly2); }); }; Decide(InVerboseMode(),Echo(\"End ForEach\")); result := Decide(poly2=?[[0,1]],poly1,DivTermList(poly1,poly2)); SparseUniVar(x,result);};10 # Cyclotomic(1,_x) <-- x-1;20 # Cyclotomic(n_Integer?,_x) <-- ExpandSparseUniVar(InternalCyclotomic(n,x));";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # OldInternalCyclotomic(n_Even?,_x,WantNormalForm_Boolean?) <-- { Local(k,m,p); k := 1; m := n; While(Even?(m)) { k := k*2; m := m/2; }; k := k/2 ; Decide(m>?1, { p := OldInternalCyclotomic(m,x,False); Decide(WantNormalForm, SubstituteAndExpandInUniVar(p,k),SubstituteInUniVar(p,k)); }, Decide(WantNormalForm, x^k+1, UniVariateBinomial(x,k,1)) ); };20 # OldInternalCyclotomic(n_Odd?,_x,WantNormalForm_Boolean?)_(n>?1) <--{ Local(divisors,poly1,poly2,q,d,f,result); divisors := MoebiusDivisorsList(n); poly1 :=1 ; poly2 := 1; ForEach (d,divisors) { q:=n/d[1]; f:=UniVariateBinomial(x,q,-1); Decide(d[2]=?1,poly1:=poly1*f,poly2:=poly2*f); }; result := Quotient(poly1,poly2); Decide(WantNormalForm,NormalForm(result),result);};10 # OldCyclotomic(1,_x) <-- _x-1;20 # OldCyclotomic(n_Integer?,_x) <-- OldInternalCyclotomic(n,x,True);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FindRealRoots(_p) <--{ Decide(iDebug,Tell(\"FindRealRoots\",p)); Local(vars,var,cc,pp,vlcc,zeroRoot,minb,maxb,rr); vars := VarList(p); var := vars[1]; cc := Content(p); pp := PrimitivePart(p); Decide(iDebug,Tell(\" \",[cc,pp])); vlcc := VarList(cc); Decide(Length(vlcc)>?0 And? Contains?(vlcc,var), zeroRoot:=True,zeroRoot:=False); p:=SquareFree(Rationalize(pp)); Decide(iDebug,Tell(\" after sqf\",p)); minb := MinimumBound(p); maxb := MaximumBound(p); Decide(iDebug,Tell(\" \",[minb,maxb])); rr := FindRealRoots(p,minb,maxb); Decide(zeroRoot,DestructiveAppend(rr,0)); rr;};FindRealRoots(_p,_Mmin,_Mmax) <--{ Decide(iDebug,Tell(\" FindRealRoots3\",[p,Mmin,Mmax])); Local(bounds,result,i,prec,requiredPrec); bounds := BoundRealRoots(p,Mmin,Mmax); Decide(iDebug,Tell(\" \",[bounds,Length(bounds)])); result:=FillList(0,Length(bounds)); requiredPrec := BuiltinPrecisionGet(); BuiltinPrecisionSet(BuiltinPrecisionGet()+4); prec:=10^-(requiredPrec+1); For(i:=1,i<=?Length(bounds),i++) { Decide(iDebug,Tell(i)); Local(a,b,c,r); [a,b] := bounds[i]; c:=NM(Eval((a+b)/2)); Decide(iDebug,Tell(\" \",[a,b,c])); r := Fail; Decide(iDebug,Tell(\" newt1\",`Hold(Newton(@p,x,@c,@prec,@a,@b)))); If(a !=? b) {r := `Newton(@p,x,@c,prec,a,b);}; Decide(iDebug,Tell(\" newt2\",r)); If(r =? Fail) { Local(c,cold,pa,pb,pc); pa:=(p Where x==a); pb:=(p Where x==b); c:=((a+b)/2); cold := a; While (Abs(cold-c)>?prec) { pc:=(p Where x==c); Decide(iDebug,Tell(\" \",[a,b,c])); If(Abs(pc) <? prec) { a:=c; b:=c; } Else If(pa*pc <? 0) { b:=c; pb:=pc; } Else { a:=c; pa:=pc; }; cold:=c; c:=((a+b)/2); }; r:=c; }; result[i] := NM(Eval((r/10)*(10)),requiredPrec); }; BuiltinPrecisionSet(requiredPrec); result;};";
        scriptMap.put("FindRealRoots",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 5 # MaximumBound(_p)_(Zero?(p Where x==0)) <-- MaximumBound(Simplify(p/x));10 # MaximumBound(_p)_(Degree(p)>?0) <--{ Local(an); an:=Coef(p,(Degree(p)-1) .. 0)/Coef(p,Degree(p)); an := NM(Eval(Abs(an)^(1/(1 .. Degree(p))))); Rationalize(2*Maximum(an));};20 # MaximumBound(_p) <-- Infinity;";
        scriptMap.put("MaximumBound",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # MinimumBound(_p)_(Zero?(p Where x==0)) <-- 0;20 # MinimumBound(_p)_(Degree(p)>?0) <--{ Local(an,result); an:=Coef(p,1 .. (Degree(p)))/Coef(p,0); an := NM(Eval(Abs(an)^(1/(1 .. Degree(p))))); result:=0; an:=2*Maximum(an); If(Not? Zero?(an)) {result := 1/an;}; Simplify(Rationalize(result));};30 # MinimumBound(_p) <-- -Infinity;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(UnparseCAllFunctions) {    CUnparsable?(_expr) <-- `CUnparsable?(@expr, []);  CUnparsable?(_expr, funclist_List?) <-- { Local(badfunctions); badfunctions := Difference(`FuncList(@expr), Concat(UnparseCAllFunctions, funclist)); Decide(Length(badfunctions)=?0, True, { Decide(InVerboseMode(), Echo(Concat([\"CUnparsable?: Info: unexportable function(s): \"], badfunctions)) ); False; } ); }; HoldArgumentNumber(\"CUnparsable?\", 1, 1); HoldArgumentNumber(\"CUnparsable?\", 2, 1);  UnparseCAllFunctions := MapSingle(ToAtom, Concat(AssocIndices(UnparseCMathFunctions()), AssocIndices(UnparseCRegularOps()),  [ \"For\", \"While\", \"Block\", \"Nth\", \"Modulo\", \"Complex\", \"If\", \"Else\", \"++\", \"--\", ] ));}; ";
        scriptMap.put("CUnparsable?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"UnparseC\",[expression]);RulebaseHoldArguments(\"UnparseC\",[expression, precedence]);Function (\"UnparseCBracketIf\", [predicate, string]){ Check(Boolean?(predicate) And? String?(string), \"Argument\", \"UnparseC internal error: non-boolean and/or non-string argument of UnparseCBracketIf\"); Decide(predicate, ConcatStrings(\"( \", string, \") \"), string);};UnparseCDoublePrecisionNumber(x_Number?) <--{ Local(i,n,s,f); s := ToString(x); n := Length(s); f := False; For(i := 1, i <=? n, i++) { Decide(s[i] =? \"e\" Or? s[i] =? \".\", f := True); }; Decide(f, s, s ~ \".\");};UnparseCMaxPrec() := 60000; 100 # UnparseC(_x) <-- UnparseC(x, UnparseCMaxPrec());110 # UnparseC(x_Integer?, _p) <-- ToString(x);111 # UnparseC(x_Zero?, _p) <-- \"0.\";112 # UnparseC(x_Number?, _p) <-- UnparseCDoublePrecisionNumber(x);190 # UnparseC(False, _p) <-- \"false\";190 # UnparseC(True, _p) <-- \"true\";200 # UnparseC(x_Atom?, _p) <-- ToString(x);100 # UnparseC(x_String?, _p) <-- ConcatStrings(\"\\\"\", x, \"\\\"\");LocalSymbols(cUnparserRegularOps) { cUnparserRegularOps := [ [\"+\",\" + \"], [\"-\",\" - \"], [\"*\",\" * \"], [\"/\",\" / \"], [\":=\",\" = \"], [\"==\",\" == \"], [\"=?\",\" == \"], [\"!=?\",\" != \"], [\"<=?\",\" <=? \"], [\">=?\",\" >= \"], [\"<?\",\" < \"], [\">?\",\" > \"], [\"And?\",\" && \"], [\"Or?\",\" || \"], [\">>\", \" >> \"], [ \"<<\", \" << \" ], [ \"&\", \" & \" ], [ \"|\", \" | \" ], [ \"%\", \" % \" ], [ \"^\", \" ^ \" ], ]; UnparseCRegularOps() := cUnparserRegularOps;};   100 # UnparseC(+ _y, _p) <-- UnparseCBracketIf(p<?PrecedenceGet(\"+\"), ConcatStrings(\" + \", UnparseC(y, RightPrecedenceGet(\"+\")) ) ); 100 # UnparseC(- _y, _p) <-- UnparseCBracketIf(p<?PrecedenceGet(\"-\"), ConcatStrings(\" - \", UnparseC(y, RightPrecedenceGet(\"-\")) ) ); 100 # UnparseC(_x ^ _y, _p) <-- UnparseCBracketIf(p<=?PrecedenceGet(\"^\"), ConcatStrings(\"pow(\", UnparseC(x, UnparseCMaxPrec()), \", \", UnparseC(y, UnparseCMaxPrec()), \")\" ) );100 # UnparseC(If(_pred)_body, _p) <-- \"If(\"~UnparseC(pred,60000)~\") \"~UnparseC(body);100 # UnparseC(_left Else _right, _p) <-- UnparseC(left)~\" Else \"~UnparseC(right);LocalSymbols(cUnparserMathFunctions) { cUnparserMathFunctions := [ [\"Sqrt\",\"sqrt\"], [\"Cos\",\"cos\"], [\"Sin\",\"sin\"], [\"Tan\",\"tan\"], [\"Cosh\",\"cosh\"], [\"Sinh\",\"sinh\"], [\"Tanh\",\"tanh\"], [\"Exp\",\"exp\"], [\"Ln\",\"log\"], [\"ArcCos\",\"acos\"], [\"ArcSin\",\"asin\"], [\"ArcTan\",\"atan\"], [\"ArcCosh\",\"acosh\"], [\"ArcSinh\",\"asinh\"], [\"ArcTanh\",\"atanh\"], [\"Maximum\",\"max\"], [\"Minimum\",\"min\"], [\"Abs\",\"fabs\"], [\"Floor\",\"floor\"], [\"Ceil\",\"ceil\"], [\"!\",\"factorial\"] ]; UnparseCMathFunctions() := cUnparserMathFunctions;}; 120 # UnparseC(expr_Function?, _p)_(ArgumentsCount(expr)=?2 And? Contains?(AssocIndices(UnparseCRegularOps()), Type(expr)) ) <-- UnparseCBracketIf(p<?PrecedenceGet(Type(expr)), ConcatStrings(UnparseC(FunctionToList(expr)[2], LeftPrecedenceGet(Type(expr))), UnparseCRegularOps()[Type(expr)], UnparseC(FunctionToList(expr)[3], RightPrecedenceGet(Type(expr))) ) );120 # UnparseC(expr_Function?, _p) _ (ArgumentsCount(expr)=?1 And? Contains?(AssocIndices(UnparseCMathFunctions()), Type(expr)) ) <-- ConcatStrings(UnparseCMathFunctions()[Type(expr)], \"(\", UnparseC( FunctionToList(expr)[2], UnparseCMaxPrec()),\")\" );UnparseCArgs(list_List?) <--{ Local(i,nr,result); result:=\"\"; nr:=Length(list); For (i:=1,i<=?nr,i++) { result:=result~UnparseC(list[i]); Decide(i<?nr, result:=result~\", \"); }; result;};200 # UnparseC(_x, _p)_(Function?(x)) <--{ ConcatStrings(Type(x), \"(\", UnparseCArgs(Rest(FunctionToList(x))),\")\" );};100 # UnparseC(Complex(0, 1), _p) <-- \"I\";100 # UnparseC(Complex(_x, 0), _p) <-- UnparseC(x, p);110 # UnparseC(Complex(_x, 1), _p) <-- UnparseC(x+Hold(I), p);110 # UnparseC(Complex(0, _y), _p) <-- UnparseC(Hold(I)*y, p);120 # UnparseC(Complex(_x, _y), _p) <-- UnparseC(x+Hold(I)*y, p);100 # UnparseC(Modulo(_x, _y), _p) <-- UnparseCBracketIf(p<?PrecedenceGet(\"/\"), ConcatStrings(UnparseC(x, PrecedenceGet(\"/\")), \" % \", UnparseC(y, PrecedenceGet(\"/\")) ) );100 # UnparseC(Nth(_x, _i), _p) <-- ConcatStrings(UnparseC(x, UnparseCMaxPrec()), \"[\", UnparseC(i, UnparseCMaxPrec()), \"]\");LocalSymbols(cindent) { cindent:=1; NlIndented():= { Local(result); result:=\"\"; Local(i); For(i:=1,i<?cindent,i++) { result:=result~\" \"; }; result; }; CIndent() := { (cindent++); \"\"; }; CUndent() := { (cindent--); \"\"; };}; UnparseCStatement(_x) <-- UnparseC(x) ~ \";\" ~ NlIndented();120 # UnparseC(_x,_p)_(Type(x) =? \"Block\") <--{ Local(result); result:=CIndent()~\"{\"~NlIndented(); ForEach(item,Rest(FunctionToList(x))) { result:=result~UnparseCStatement(item); }; result:=result~\"}\"~CUndent()~NlIndented(); result;};120 # UnparseC(For(_from,_to,_step)_body,_p) <-- \"for(\" ~ UnparseC(from,UnparseCMaxPrec()) ~ \";\" ~ UnparseC(to,UnparseCMaxPrec()) ~ \";\" ~ UnparseC(step,UnparseCMaxPrec()) ~ \")\" ~ CIndent() ~ NlIndented() ~ UnparseCStatement(body) ~ CUndent();120 # UnparseC(While(_pred)_body, _p) <-- \"while(\" ~ UnparseC(pred,UnparseCMaxPrec()) ~ \")\" ~ CIndent() ~ NlIndented() ~ UnparseCStatement(body) ~ CUndent();";
        scriptMap.put("UnparseC",scriptString);
        scriptMap.put("UnparseCDoublePrecisionNumber",scriptString);
        scriptMap.put("UnparseCArgs",scriptString);
        scriptMap.put("UnparseCStatement",scriptString);
        scriptMap.put("UnparseCMathFunctions",scriptString);
