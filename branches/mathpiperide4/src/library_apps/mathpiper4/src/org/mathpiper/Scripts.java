...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(\"CForm\",[expression]);RulebaseHoldArguments(\"CForm\",[expression, precedence]);Function (\"CFormBracketIf\", [predicate, string]){ Check(Boolean?(predicate) And? String?(string), \"Argument\", \"CForm internal error: non-boolean and/or non-string argument of CFormBracketIf\"); Decide(predicate, ConcatStrings(\"( \", string, \") \"), string);};CFormDoublePrecisionNumber(x_Number?) <--{ Local(i,n,s,f); s := ToString(x); n := Length(s); f := False; For(i := 1, i <=? n, i++) { Decide(s[i] =? \"e\" Or? s[i] =? \".\", f := True); }; Decide(f, s, s ~ \".\");};CFormMaxPrec() := 60000; 100 # CForm(_x) <-- CForm(x, CFormMaxPrec());110 # CForm(x_Integer?, _p) <-- ToString(x);111 # CForm(x_Zero?, _p) <-- \"0.\";112 # CForm(x_Number?, _p) <-- CFormDoublePrecisionNumber(x);190 # CForm(False, _p) <-- \"false\";190 # CForm(True, _p) <-- \"true\";200 # CForm(x_Atom?, _p) <-- ToString(x);100 # CForm(x_String?, _p) <-- ConcatStrings(\"\\\"\", x, \"\\\"\");LocalSymbols(cformRegularOps) { cformRegularOps := [ [\"+\",\" + \"], [\"-\",\" - \"], [\"*\",\" * \"], [\"/\",\" / \"], [\":=\",\" = \"], [\"==\",\" == \"], [\"=?\",\" == \"], [\"!=?\",\" != \"], [\"<=?\",\" <=? \"], [\">=?\",\" >= \"], [\"<?\",\" < \"], [\">?\",\" > \"], [\"And?\",\" && \"], [\"Or?\",\" || \"], [\">>\", \" >> \"], [ \"<<\", \" << \" ], [ \"&\", \" & \" ], [ \"|\", \" | \" ], [ \"%\", \" % \" ], [ \"^\", \" ^ \" ], ]; CFormRegularOps() := cformRegularOps;};   100 # CForm(+ _y, _p) <-- CFormBracketDecide(p<?PrecedenceGet(\"+\"), ConcatStrings(\" + \", CForm(y, RightPrecedenceGet(\"+\")) ) ); 100 # CForm(- _y, _p) <-- CFormBracketDecide(p<?PrecedenceGet(\"-\"), ConcatStrings(\" - \", CForm(y, RightPrecedenceGet(\"-\")) ) ); 100 # CForm(_x ^ _y, _p) <-- CFormBracketDecide(p<=?PrecedenceGet(\"^\"), ConcatStrings(\"pow(\", CForm(x, CFormMaxPrec()), \", \", CForm(y, CFormMaxPrec()), \")\" ) );100 # CForm(If(_pred)_body, _p) <-- \"If(\"~CForm(pred,60000)~\") \"~CForm(body);100 # CForm(_left Else _right, _p) <-- CForm(left)~\" Else \"~CForm(right);LocalSymbols(cformMathFunctions) { cformMathFunctions := [ [\"Sqrt\",\"sqrt\"], [\"Cos\",\"cos\"], [\"Sin\",\"sin\"], [\"Tan\",\"tan\"], [\"Cosh\",\"cosh\"], [\"Sinh\",\"sinh\"], [\"Tanh\",\"tanh\"], [\"Exp\",\"exp\"], [\"Ln\",\"log\"], [\"ArcCos\",\"acos\"], [\"ArcSin\",\"asin\"], [\"ArcTan\",\"atan\"], [\"ArcCosh\",\"acosh\"], [\"ArcSinh\",\"asinh\"], [\"ArcTanh\",\"atanh\"], [\"Maximum\",\"max\"], [\"Minimum\",\"min\"], [\"Abs\",\"fabs\"], [\"Floor\",\"floor\"], [\"Ceil\",\"ceil\"], [\"!\",\"factorial\"] ]; CFormMathFunctions() := cformMathFunctions;}; 120 # CForm(expr_Function?, _p)_(ArgumentsCount(expr)=?2 And? Contains?(AssocIndices(CFormRegularOps()), Type(expr)) ) <-- CFormBracketDecide(p<?PrecedenceGet(Type(expr)), ConcatStrings(CForm(FunctionToList(expr)[2], LeftPrecedenceGet(Type(expr))), CFormRegularOps()[Type(expr)], CForm(FunctionToList(expr)[3], RightPrecedenceGet(Type(expr))) ) );120 # CForm(expr_Function?, _p) _ (ArgumentsCount(expr)=?1 And? Contains?(AssocIndices(CFormMathFunctions()), Type(expr)) ) <-- ConcatStrings(CFormMathFunctions()[Type(expr)], \"(\", CForm( FunctionToList(expr)[2], CFormMaxPrec()),\")\" );CFormArgs(list_List?) <--{ Local(i,nr,result); result:=\"\"; nr:=Length(list); For (i:=1,i<=?nr,i++) { result:=result~CForm(list[i]); Decide(i<?nr, result:=result~\", \"); }; result;};200 # CForm(_x, _p)_(Function?(x)) <--{ ConcatStrings(Type(x), \"(\", CFormArgs(Rest(FunctionToList(x))),\")\" );};100 # CForm(Complex(0, 1), _p) <-- \"I\";100 # CForm(Complex(_x, 0), _p) <-- CForm(x, p);110 # CForm(Complex(_x, 1), _p) <-- CForm(x+Hold(I), p);110 # CForm(Complex(0, _y), _p) <-- CForm(Hold(I)*y, p);120 # CForm(Complex(_x, _y), _p) <-- CForm(x+Hold(I)*y, p);100 # CForm(Modulo(_x, _y), _p) <-- CFormBracketDecide(p<?PrecedenceGet(\"/\"), ConcatStrings(CForm(x, PrecedenceGet(\"/\")), \" % \", CForm(y, PrecedenceGet(\"/\")) ) );100 # CForm(Nth(_x, _i), _p) <-- ConcatStrings(CForm(x, CFormMaxPrec()), \"[\", CForm(i, CFormMaxPrec()), \"]\");LocalSymbols(cindent) { cindent:=1; NlIndented():= { Local(result); result:=\"\"; Local(i); For(i:=1,i<?cindent,i++) { result:=result~\" \"; }; result; }; CIndent() := { (cindent++); \"\"; }; CUndent() := { (cindent--); \"\"; };}; CFormStatement(_x) <-- CForm(x) ~ \";\" ~ NlIndented();120 # CForm(_x,_p)_(Type(x) =? \"Block\") <--{ Local(result); result:=CIndent()~\"{\"~NlIndented(); ForEach(item,Rest(FunctionToList(x))) { result:=result~CFormStatement(item); }; result:=result~\"}\"~CUndent()~NlIndented(); result;};120 # CForm(For(_from,_to,_step)_body,_p) <-- \"for(\" ~ CForm(from,CFormMaxPrec()) ~ \";\" ~ CForm(to,CFormMaxPrec()) ~ \";\" ~ CForm(step,CFormMaxPrec()) ~ \")\" ~ CIndent() ~ NlIndented() ~ CFormStatement(body) ~ CUndent();120 # CForm(While(_pred)_body, _p) <-- \"while(\" ~ CForm(pred,CFormMaxPrec()) ~ \")\" ~ CIndent() ~ NlIndented() ~ CFormStatement(body) ~ CUndent();";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(CFormAllFunctions) {    CFormable?(_expr) <-- `CFormable?(@expr, []);  CFormable?(_expr, funclist_List?) <-- { Local(badfunctions); badfunctions := Difference(`FuncList(@expr), Concat(CFormAllFunctions, funclist)); Decide(Length(badfunctions)=?0, True, { Decide(InVerboseMode(), Echo(Concat([\"CFormable?: Info: unexportable function(s): \"], badfunctions)) ); False; } ); }; HoldArgumentNumber(\"CFormable?\", 1, 1); HoldArgumentNumber(\"CFormable?\", 2, 1);  CFormAllFunctions := MapSingle(ToAtom, Concat(AssocIndices(CFormMathFunctions()), AssocIndices(CFormRegularOps()),  [ \"For\", \"While\", \"Block\", \"Nth\", \"Modulo\", \"Complex\", \"If\", \"Else\", \"++\", \"--\", ] ));}; ";
