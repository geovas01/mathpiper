...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString[0] = null;
        scriptString[1] = "RulebaseHoldArguments(MathPiperInitLoad,[]);RuleHoldArguments(MathPiperInitLoad, 0, 1, True){  StandardOperatorsLoad(); Constant(True);  Constant(False);  Constant(Empty);  Constant(Infinity);};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # xPrimitivePart(poly_CanBeUni) <-- { Local(cont,pp); Decide(InVerboseMode(),Tell(\" xPrimitivePart1\",poly)); cont := xContent(poly); pp := poly / cont; };Macro(\"xPrimitivePart\",[poly,xcont]){ Local(pp); Decide(InVerboseMode(),Tell(\" xPrimitivePart2\",[poly,xcont]));  Decide( Bound?(@xcont),  { pp := Eval(@poly) / Eval(@xcont); }, { Local(xCont); xCont := xContent(Eval(@poly)); @xcont := xCont; pp := Eval(@poly) / xCont; } ); pp; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnAugment( M_Matrix?, v_Vector? )_(Length(v)=?Dimensions(M)[2]) <--[ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnAugment,[M,v]) ); Local(mRows,nCols,newMat,ir); Local(MT,MA); MT := Transpose(M); MT := MatrixRowStack(MT,v); MA := Transpose(MT);];";
        scriptMap.put("MatrixColumnSwap",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnReplace( M_Matrix?, jCol_PositiveInteger?, v_Vector? )_(Length(v)=?Dimensions(M)[2]) <--{ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnReplace,[M,jCol,v]) ); Local(mRows,nCols,MT); [mRows,nCols] := Dimensions(M); Decide( jCol <=? nCols, { MT:=Transpose(M); DestructiveReplace(MT,jCol,v); M:=Transpose(MT); } ); M;};";
        scriptMap.put("MatrixColumnReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixColumnSwap( M_Matrix?, jCol1_PositiveInteger?, jCol2_PositiveInteger? )_ (And?(jCol1<=?Dimensions(M)[2],jCol2<=?Dimensions(M)[2])) <--{ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixColumnSwap,[M,jCol1,jCol2]) ); Local(MT); MT := Transpose(M); MT := MatrixRowSwap(MT,jCol1,jCol2); M := Transpose(MT);};";
        scriptMap.put("MatrixColumnSwap",scriptString);
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowReplace( M_Matrix?, iRow_PositiveInteger?, v_Vector? )_(Length(v)=?Length(M[1])) <--{ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixRowReplace,[M,iRow,v]) ); Local(mRows,nCols); [mRows,nCols] := Dimensions(M); Decide( iRow <=? mRows, DestructiveReplace(M,iRow,v) ); M;};";
        scriptMap.put("MatrixRowReplace",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowStack( M_Matrix?, v_Vector? )_(Length(v)=?Dimensions(M)[1]) <--{ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixRowStack,[M,v]) ); Local(mRows,nCols,newMat,ir); [mRows,nCols] := Dimensions(M); newMat := ZeroMatrix(mRows+1,nCols); For(ir:=1,ir<?mRows+1,ir++) { MatrixRowReplace(newMat,ir,MatrixRow(M,ir)); }; MatrixRowReplace(newMat,mRows+1,v); newMat;};";
        scriptMap.put("MatrixRowStack",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "MatrixRowSwap( M_Matrix?, iRow1_PositiveInteger?, iRow2_PositiveInteger? )_ (And?(iRow1<=?Dimensions(M)[1],iRow2<=?Dimensions(M)[1])) <--{ Decide( Not? Bound?(iDebug), iDebug := False ); Decide( iDebug, Tell(MatrixRowSwap,[M,iRow1,iRow2]) ); Local(row1,row2); Decide( iRow1 !=? iRow2, { row1 := MatrixRow(M,iRow1); row2 := MatrixRow(M,iRow2); DestructiveReplace(M,iRow1,row2); DestructiveReplace(M,iRow2,row1); } ); M;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(mathExpThreshold) {  mathExpThreshold := Decide(Not? Bound?(mathExpThreshold), 500); MathExpThreshold() := mathExpThreshold; SetMathExpThreshold(threshold) := {mathExpThreshold:= threshold; };};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # IntPowerNum(_x, 0, _func, _unity) <-- unity;10 # IntPowerNum(_x, n_Integer?, _func, _unity) <--{  Local(result);  While(n >? 0) { Decide( (n&1) =? 1, Decide( Bound?(result),  result := Apply(func, [result,x]), result := x,  ) ); x := Apply(func, [x,x]); n := n>>1; }; result;};";
