...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"MathPi\",[]){    Local(initialPrec,curPrec,result,aPrecision); Assign(aPrecision,BuiltinPrecisionGet()); Assign(initialPrec, aPrecision);  Assign(curPrec, 40);  Assign(result, 3.141592653589793238462643383279502884197169399);   While (GreaterThan?(initialPrec, MultiplyN(curPrec,3))) { Assign(initialPrec, FloorN(DivideN(AddN(initialPrec,2),3))); }; Assign(curPrec, initialPrec); While (Not?(GreaterThan?(curPrec, aPrecision))) {   BuiltinPrecisionSet(curPrec); Assign(result,AddN(result,SinN(result)));    If (Equal?(curPrec, aPrecision),  { Assign(curPrec, AddN(aPrecision,1));  }, { Assign(curPrec, MultiplyN(curPrec,3));   If (GreaterThan?(curPrec, aPrecision), { Assign(curPrec, aPrecision);  }); }); }; BuiltinPrecisionSet(aPrecision); result;};";
        scriptMap.put("MathPi",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Defun(\"PositiveIntPower\", [x,n]){ Local(result,unit); If(LessThan?(n,0), False, { Assign(unit,1);  Assign(result, unit); If(Equal?(n,0),unit, If(Equal?(n,1),x, { While(GreaterThan?(n,0)) { If( Equal?(BitAnd(n,1), 1), Assign(result, MultiplyN(result,x)) ); Assign(x, MultiplyN(x,x)); Assign(n,ShiftRight(n,1)); }; result; } ) ); });};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "TemplateFunction(\"ForEach\",[item,listOrString,body]){ If(And?(Equal?(Generic?(listOrString),True), Equal?(GenericTypeName(listOrString),\"Array\") ), `ForEachInArray(@item,listOrString,@body), { MacroLocal(item);  If(String?(listOrString), {  Local(index, stringLength);  stringLength := Length(listOrString);  index := 1; While(index <=? stringLength ) { MacroAssign(item,listOrString[index] );  Eval(body);  index++; }; }, { Local(foreachtail); Assign(foreachtail,listOrString); While(Not?(Equal?(foreachtail,[]))) { MacroAssign(item,First(foreachtail)); Eval(body); Assign(foreachtail,Rest(foreachtail)); }; }); });};UnFence(\"ForEach\",3);HoldArgumentNumber(\"ForEach\",3,1);HoldArgumentNumber(\"ForEach\",3,3);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(ErrorTableau) {  Assign(ErrorTableau, []); GetErrorTableau() := ErrorTableau; ClearErrors() <-- Assign(ErrorTableau, []);  CheckErrorTableau() <-- If( Not? List?(ErrorTableau), Assign(ErrorTableau, [[\"general\", \"corrupted ErrorTableau\"]]) );}; GetError(errorclass_String?) <--{ Local(error); error := GetErrorTableau()[errorclass]; If( error !=? Empty, error, False );};ClearError(errorclass_String?) <-- AssocDelete(GetErrorTableau(), errorclass);";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(max,f,result){ BSearch(max,f) := { Local(result); Assign(result, FindIsq(max,f)); If(Apply(f,[result])!=?0,Assign(result,-1)); result; };};UnFence(\"BSearch\",2);";
        scriptMap.put("BSearch",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Contains?\",[list,element]){ Local(result); Assign(result,False); While(And?(Not?(result), Not?(Equal?(list, [])))) { If(Equal?(First(list),element), Assign(result, True), Assign(list, Rest(list)) ); }; result;};";
        scriptMap.put("Contains?",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Count\",[list,element]){ Local(result); Assign(result,0); ForEach(item,list) If(Equal?(item, element), Assign(result,AddN(result,1))); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Difference\",[list1,list2]){ Local(l2,index,result); l2:=FlatCopy(list2); result:=FlatCopy(list1); ForEach(item,list1) { Assign(index,Find(l2,item)); If(index>?0, { DestructiveDelete(l2,index); DestructiveDelete(result,Find(result,item)); } ); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "LocalSymbols(max,f,low,high,mid,current){FindIsq(max,f) :={ Local(low,high,mid,current); low:=1; high:=max+1; Assign(mid,((high+low)>>1)); While(high>?low And? mid>?1) { Assign(mid,((high+low)>>1)); Assign(current,Apply(f,[mid])); If(current =? 0, high:=low-1, If(current >? 0, Assign(high,mid), Assign(low,mid+1) ) ); }; mid;};};UnFence(\"FindIsq\",2);";
        scriptMap.put("FindIsq",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"FindPredicate\",[list,predicate]){ Local(result,count); Assign(result, -1); Assign(count, 1); While(And?(result<?0, Not?(Equal?(list, [])))) { If(Apply(predicate,[First(list)]), Assign(result, count) ); Assign(list,Rest(list)); Assign(count,AddN(count,1)); }; result;}; ";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Remove(list, expression) :={ Local(result); Assign(result,[]); ForEach(item,list) If(item !=? expression, DestructiveAppend(result,item)); result;};";
        scriptMap.put("Remove",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"RemoveDuplicates\",[list]){ Local(result); Assign(result,[]); ForEach(item,list) If(Not?(Contains?(result,item)),DestructiveAppend(result,item)); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = " 10 # LogicSimplify(_proposition, _level)_(level<?2) <-- CNF(proposition);20 # LogicSimplify(_proposition, _level) <--{ Local(cnf, list, clauses); Check(level >? 1, \"Argument\", \"Wrong level\");  Assign(cnf, CNF(proposition)); If(level <=? 1, cnf, { Assign(list, Flatten(cnf, \"And?\")); Assign(clauses, []); ForEach(clause, list) { Local(newclause);  Assign(newclause, LogicRemoveTautologies(Flatten(clause, \"Or?\"))); If(newclause !=? [True], DestructiveAppend(clauses, newclause)); };  Assign(clauses, RemoveDuplicates(clauses)); If(Equal?(level, 3) And? (Length(clauses) !=? 0), { Assign(clauses, DoUnitSubsumptionAndResolution(clauses)); Assign(clauses, LogicCombine(clauses)); }); Assign(clauses, RemoveDuplicates(clauses)); If(Equal?(Length(clauses), 0), True, {  Local(result); Assign(result, True); ForEach(item,clauses) { Assign(result, result And? UnFlatten(item, \"Or?\", False)); }; result; }); });};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # CheckSolution( _expr, _var, solution_List? )_(Not? FreeOf?(var,expr)) <-- { Local(expr0,result,s,r); If( Equation?(expr), Assign(expr0,EquationLeft(expr)-EquationRight(expr)), Assign(expr0,expr) ); result := []; ForEach(s,solution)  { r := ( expr0 Where s ); If(r=?0,Push(result,s)); }; Reverse(result); };  20 # CheckSolution( _expr, _var, _solution ) <-- False;";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Ceil(Infinity) <-- Infinity;5 # Ceil(-Infinity) <-- -Infinity;5 # Ceil(Undefined) <-- Undefined;10 # Ceil(x_RationalOrNumber?) <-- { x:=N(x); Local(prec,result,n); Assign(prec,BuiltinPrecisionGet()); If(Zero?(x),Assign(n,2), If(x>?0, Assign(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Assign(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); If(n>?prec,BuiltinPrecisionSet(n)); Assign(result,CeilN(x)); BuiltinPrecisionSet(prec); result; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Floor(Infinity) <-- Infinity;5 # Floor(-Infinity) <-- -Infinity;5 # Floor(Undefined) <-- Undefined;10 # Floor(x_RationalOrNumber?) <-- { x:=N(Eval(x)); Local(prec,result,n); Assign(prec,BuiltinPrecisionGet()); If(Zero?(x), Assign(n,2), If(x>?0, Assign(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Assign(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); If(n>?prec,BuiltinPrecisionSet(n)); Assign(result,FloorN(x)); BuiltinPrecisionSet(prec); result; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Taylor\",[taylorvariable,taylorat,taylororder,taylorfunction]) Taylor1(taylorvariable,taylorat,taylororder)(taylorfunction);Function(\"Taylor1\",[taylorvariable,taylorat,taylororder,taylorfunction]){ Local(n,result,dif,polf); { MacroLocal(taylorvariable); { MacroLocal(taylorvariable); MacroAssign(taylorvariable, taylorat); result:=Eval(taylorfunction); }; If(result=?Undefined, { result:=Apply(\"Limit\",[taylorvariable,taylorat,taylorfunction]); }); }; dif:=taylorfunction; polf:=(taylorvariable-taylorat); For(n:=1,result !=? Undefined And? n<=?taylororder,n++) { dif:= Deriv(taylorvariable) dif; Local(term); MacroLocal(taylorvariable); { MacroLocal(taylorvariable); MacroAssign(taylorvariable, taylorat); term:=Eval(dif); }; If(term=?Undefined, { term:=Apply(\"Limit\",[taylorvariable,taylorat,dif]); }); result:=result+(term/(n!))*(polf^n); }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"KnownFailure\",[expr]){ Local(rfail); Echo(\"Known failure: \", expr); Assign(rfail,Eval(expr)); If(rfail,Echo([\"Failure resolved!\"]));};HoldArgument(\"KnownFailure\",expr);";
