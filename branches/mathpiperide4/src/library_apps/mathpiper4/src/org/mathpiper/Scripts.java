...
public class Scripts {
    ...
    public Scripts() {
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function(\"Sparsity\",[matrix]){ Local(rows,cols,nonzero); nonzero:=0; rows:=Length(matrix); cols:=Length(matrix[1]); ForEach(i, 1 .. rows ) ForEach(j, 1 .. cols ) Decide(matrix[i][j] !=? 0, nonzero:=nonzero+1 ); N(1 - nonzero/(rows*cols));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "BracketRational(r,eps):={ Local(n,cflist, r1, r2); cflist := ContFracList(r); n:=2; r1 := ContFracEval(Take(cflist,n)); r2 := -r1;  While (n<?Length(cflist) And? ( Abs(N(Eval(r2-r1))) >? Abs(N(Eval(eps*r)) ) ) ) { r2 := r1; n++; r1 := ContFracEval(Take(cflist,n)); };   Decide( n=?Length(cflist), [],  Decide(N(Eval(r-r1))>?0, [r1, r2],  [r2, r1] ) );};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "GuessRational(_x) <-- GuessRational(x, Floor(1/2*BuiltinPrecisionGet()));GuessRational(x_RationalOrNumber?, prec_Integer?) <-- { Local(denomestimate, cf, i); denomestimate := 1; cf := ContFracList(x); For(i:=2, i<=?Length(cf) And? denomestimate <? 10^prec, i++) {  denomestimate := denomestimate * Decide( cf[i] =? 1, Decide( i+2<=?Length(cf),  RoundTo(N(Eval(cf[i]+1/(cf[i+1]+1/cf[i+2]))), 3),  RoundTo(N(Eval(cf[i]+1/cf[i+1])), 3) ),  cf[i] ); }; Decide(denomestimate <? 10^prec,  i--  ); i--;   ContFracEval(Take(cf, i));};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # GaussianGcd(n_GaussianInteger?,m_GaussianInteger?) <--{ Decide(N(Abs(m))=?0,n, GaussianGcd(m,n - m*Round(n/m) ) );};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "FloatIsInt?(_x) <-- { x:=N(Eval(x)); Local(prec,result,n); Assign(prec,BuiltinPrecisionGet()); Decide(Zero?(x),Assign(n,2), Decide(x>?0, Assign(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Assign(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); BuiltinPrecisionSet(n+prec); Assign(result,Zero?(RoundTo(x-Floor(x),prec)) Or? Zero?(RoundTo(x-Ceil(x),prec))); BuiltinPrecisionSet(prec); result; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "ErrorFunction(x) :={ Local(a1,a2,a3,a4,a5,p,sign,t,y);  a1 := 0.254829592; a2 := -0.284496736; a3 := 1.421413741; a4 := -1.453152027; a5 := 1.061405429; p := 0.3275911;  sign := 1;  Decide(x <? 0, sign := -1);  x := AbsN(x);  t := 1.0/(1.0 + p*x); y := N(1.0 - (((((a5*t + a4)*t) + a3)*t + a2)*t + a1)*t*Exp(-x*x)); sign*y;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "10 # RadSimp(_n)_(Length(VarList(n))<?1) <--{ Local(max, result); Assign(max, CeilN(N(Eval(n^2)))); Assign(result,0); Assign(result,RadSimpTry(n,0,1,max)); If(CheckRadicals(n,result)) result Else n;};20 # RadSimp(_n) <-- n;CheckRadicals(_n,_test) <-- Abs(N(Eval(n-test),20)) <? 0.000001;10 # ClampRadicals(_r)_(N(Eval(Abs(r)), 20)<?0.000001) <-- 0;20 # ClampRadicals(_r) <-- r;RadSimpTry(_n,_result,_current,_max)<--{ If(LessThan?(N(Eval(result-n)), 0)) { Local(i);  i:=BSearch(max,Hold([[try],ClampRadicals(N(Eval((result+Sqrt(try))-n),20))])); Decide(i>?0, { Assign(result,result+Sqrt(i)); Assign(i,AddN(max,1)); Assign(current,AddN(max,1)); });  If(LessThan?(N(Eval(result-n)), 0)) { For (Assign(i,current),i<=?max,Assign(i,AddN(i,1))) { Local(new, test); Assign(test,result+Sqrt(i)); Assign(new,RadSimpTry(n,test,i,max)); If(CheckRadicals(n,new)) { Assign(result,new); Assign(i,AddN(max,1)); }; }; }; }; result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Bernoulli1(n_Even?)_(n>=?2) <-- { Local(B, prec); prec := BuiltinPrecisionGet();   BuiltinPrecisionSet(10); BuiltinPrecisionSet( Ceil(N((1/2*Ln(8*Pi*n)-n+n*Ln(n/2/Pi))/Ln(10)))+3  ); Decide(InVerboseMode(), Echo([\"Bernoulli: using zeta funcion, precision \", BuiltinPrecisionSet(), \", n = \", n])); B := Floor(N(  Decide(  n>?250,  InternalZetaNum2(n, n/17+1),    InternalZetaNum1(n, n/17+1)  ) *N(2*n! /(2*Pi)^n)))  * (2*Modulo(n/2,2)-1)  + BernoulliFracPart(n);  BuiltinPrecisionSet(prec);  B;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "InternalZetaNum(_s, n_Integer?) <-- { Local(result, j, sign); Decide(InVerboseMode(), Echo([\"InternalZetaNum: Borwein's method, precision \", BuiltinPrecisionGet(), \", n = \", n])); result := 0; sign := 1;  For(j:=0, j<=?2*n-1, j++) {  result := N(result + sign*InternalZetaNumCoeffEj(j,n)/(1+j)^s ); sign := -sign; }; N(result/(2^n)/(1-2^(1-s)));};InternalZetaNum1(s, limit) := { Local(i, sum); Decide(InVerboseMode(), Echo([\"InternalZetaNum: direct method (sum), precision \", BuiltinPrecisionGet(), \", N = \", limit])); sum := 0; limit := Ceil(N(limit)); For(i:=2, i<=?limit, i++) sum := sum+N(1/PowerN(i, s)); sum+1; };InternalZetaNum2(s, limit) :={ Local(i, prod); Decide(InVerboseMode(), Echo([\"InternalZetaNum: direct method (product), precision \", BuiltinPrecisionGet(), \", N = \", limit])); prod := N( (1-1/PowerN(2, s))*(1-1/PowerN(3,s)) ); limit := Ceil(N(limit)); For(i:=5, i<=?limit, i:= NextPrime(i)) prod := prod*N(1-1/PowerN(i, s)); 1/prod;};InternalZetaNumCoeffEj(j,n) := { Local(k); 2^n-Decide(j<?n, 0, Sum(k,0,j-n,BinomialCoefficient(n,k))  );};Zeta3() :={ Local(result, oldresult, k, term); N({ For( { k:=1; result := 1; oldresult := -1; term := 1; }, oldresult!=?result, k++ ) { oldresult := result; term := -term * k^2 / ((2*k+1)*(2*k)); result := result + term/(k+1)^2; }; result := 5/4*result; }, BuiltinPrecisionGet()+IntLog(BuiltinPrecisionGet(),10)+1); result;};10 # InternalZetaNum(_s) _ (N(s)=?0) <-- -0.5;10 # InternalZetaNum(_s) _ (N(s)=?1) <-- Infinity;20 # InternalZetaNum(_s) <-- { Local(n, prec, result); prec := BuiltinPrecisionGet(); Decide(  N(Re(s)) <? 0.5,  { Decide(InVerboseMode(), Echo([\"InternalZetaNum: using s->1-s identity, s=\", s, \", precision \", prec])); result := 2*Exp(InternalLnGammaNum(1-s)-(1-s)*Ln(2*InternalPi()))*Sin(InternalPi()*s/2) * InternalZetaNum(1-s); },  Decide(N(Re(s)) >? N(1+(prec*Ln(10))/(Ln(prec)+0.1), 6), {  n:= N(10^(prec/(s-1)), 6)+2;  BuiltinPrecisionSet(prec+2);  result := InternalZetaNum1(s, n); }, {  n := Ceil( N( prec*Ln(10)/Ln(8) + 2, 6 ) );  BuiltinPrecisionSet(prec+2);  result := InternalZetaNum(s, n); } ) ); BuiltinPrecisionSet(prec); result;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "SumTaylorNum0(_x, _nthtermfunc, _nterms) <-- SumTaylorNum0(x, nthtermfunc, [], nterms);SumTaylorNum1(_x, _nthtermfunc, _nterms) <-- SumTaylorNum1(x, nthtermfunc, [], nterms);SumTaylorNum(_x, _nthtermfunc, _nterms) <--Decide( nterms >=? 30,   SumTaylorNum1(x, nthtermfunc, nterms), SumTaylorNum0(x, nthtermfunc, nterms));SumTaylorNum(_x, _nthtermfunc, _nexttermfactor, _nterms) <--Decide( nterms >=? 5,  SumTaylorNum1(x, nthtermfunc, nexttermfactor, nterms), SumTaylorNum0(x, nthtermfunc, nexttermfactor, nterms));1# SumTaylorNum0(_x, _nthtermfunc, [], _nterms) <--{ Local(sum, k); N({  x:=Eval(x); sum := 0; For(k:=nterms, k>=?0, k--) sum := AddN(sum*x, nthtermfunc @ k); }); sum;};2# SumTaylorNum0(_x, _nthtermfunc, _nexttermfactor, _nterms) <--{ Local(sum, k, term, delta); N({ x:=Eval(x);  Decide(Constant?(nthtermfunc), term := nthtermfunc, term := (nthtermfunc @ [0]), ); sum := term;  }); NonN({ delta := 1; For(k:=1, k<=?nterms And? delta !=? 0, k++) { term := MultiplyNum(term, nexttermfactor @ [k], x);  delta := sum; sum := sum + term;  delta := Abs(sum-delta);  }; }); sum;};SumTaylorNum0(_x, _nthtermfunc, _nterms) <-- SumTaylorNum0(x, nthtermfunc, [], nterms);SumTaylorNum1(x, nthtermfunc, nexttermfactor, nterms) :={  Local(sum, rows, cols, rowstmp, lastpower, i, j, xpower, termtmp); N({  x:=Eval(x);  rows := IntNthRoot(nterms+1, 2); cols := Quotient(nterms+rows, rows);  Check(rows>?1 And? cols>?1, \"Argument\", \"SumTaylorNum1: Internal error: number of Taylor sum terms must be at least 4\"); rowstmp := ArrayCreate(rows, 0); xpower := x ^ rows;      Decide( nexttermfactor =? [], termtmp := 1,  Decide(Constant?(nthtermfunc), termtmp := nthtermfunc, termtmp := (nthtermfunc @ [0]), ) ); }); NonN({   For(i:=0, i<?cols, i++) {  For(j:=0, j<?rows And? (i<?cols-1 Or? i*rows+j<=?nterms), j++)  {   Decide( nexttermfactor =? [],  { rowstmp[j+1] := rowstmp[j+1] + MultiplyNum(termtmp, nthtermfunc @ [i*rows+j]); }, { rowstmp[j+1] := rowstmp[j+1] + termtmp;  termtmp := MultiplyNum(termtmp, nexttermfactor @ [i*rows+j+1]);  } ); };  termtmp := termtmp*xpower;  };   For({j:=rows; sum:=0;}, j>?0, j--) sum := sum*x + rowstmp[j]; }); sum;};";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Ceil(Infinity) <-- Infinity;5 # Ceil(-Infinity) <-- -Infinity;5 # Ceil(Undefined) <-- Undefined;10 # Ceil(x_RationalOrNumber?) <-- { x:=N(x); Local(prec,result,n); Assign(prec,BuiltinPrecisionGet()); Decide(Zero?(x),Assign(n,2), Decide(x>?0, Assign(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Assign(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); Decide(n>?prec,BuiltinPrecisionSet(n)); Assign(result,CeilN(x)); BuiltinPrecisionSet(prec); result; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "5 # Floor(Infinity) <-- Infinity;5 # Floor(-Infinity) <-- -Infinity;5 # Floor(Undefined) <-- Undefined;10 # Floor(x_RationalOrNumber?) <-- { x:=N(Eval(x)); Local(prec,result,n); Assign(prec,BuiltinPrecisionGet()); Decide(Zero?(x), Assign(n,2), Decide(x>?0, Assign(n,2+FloorN(N(FastLog(x)/FastLog(10)))), Assign(n,2+FloorN(N(FastLog(-x)/FastLog(10)))) )); Decide(n>?prec,BuiltinPrecisionSet(n)); Assign(result,FloorN(x)); BuiltinPrecisionSet(prec); result; };";
        ...
        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Maximum(l1, l2, l3, ...);10 # Maximum(_l1, _l2, l3_List?) <-- Maximum(Concat([l1, l2], l3));20 # Maximum(_l1, _l2, _l3) <-- Maximum([l1, l2, l3]);10 # Maximum(l1_List?,l2_List?) <-- Map(\"Maximum\",[l1,l2]);20 # Maximum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- Decide(l1>?l2,l1,l2);30 # Maximum(l1_Constant?,l2_Constant?) <-- Decide(N(Eval(l1-l2))>?0,l1,l2);10 # Maximum([]) <-- Undefined;20 # Maximum(list_List?) <--{ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Maximum(result,item); result;};30 # Maximum(_x) <-- x;";
        scriptMap.put("Maximum",scriptString);

        scriptString = new String[2];
        scriptString[0] = null;
        scriptString[1] = "Function() Minimum(l1, l2, l3, ...);10 # Minimum(_l1, _l2, l3_List?) <-- Minimum(Concat([l1, l2], l3));20 # Minimum(_l1, _l2, _l3) <-- Minimum([l1, l2, l3]);10 # Minimum(l1_List?,l2_List?) <-- Map(\"Minimum\",[l1,l2]);20 # Minimum(l1_RationalOrNumber?,l2_RationalOrNumber?) <-- Decide(l1<?l2,l1,l2);30 # Minimum(l1_Constant?,l2_Constant?) <-- Decide(N(Eval(l1-l2))<?0,l1,l2);10 # Minimum([]) <-- Undefined;20 # Minimum(list_List?) <--{ Local(result); result:= list[1]; ForEach(item,Rest(list)) result:=Minimum(result,item); result;};30 # Minimum(_x) <-- x;";
