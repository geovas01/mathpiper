...
public class aAnimationEnvironment extends Panel {
     ...
     * <code>aAnimationObject</code>s use the value of this attribute to
     * determine their appearance. Changing the magnification sends update
     * notification to the objects of the environment, which update their
     * display coordinates accordingly. Note: Not all Animation objects may
     * scale as expected (e.g., font size and display).
     * 
     * @param m
     *            The magnification level (1.0 is 100% - no magnification, 2.0
     *            is 200%, 0.5 is 50%).
     */
    ...
    /**
     * Draws the entire environment, including all <code>aAnimationObject</code>
     * s. Drawing is done in the order of the animation object's level (smaller
     * levels are drawn first, larger levels are drawn later).
     * 
     * @param g
     *            The <code>Graphics</code> environment for <code>Panel</code>s'
     *            <code>paint</code> function.
     */
    ...
    /**
     * Enumerate all <code>aAnimationObject</code>s belonging to the
     * environment.
     * 
     ...
     * 
     * @param ao
     *            The <code>aAnimationObject</code> to add.
     */
    public void addObject(aAnimationObject ao) {
	int pos, max = objects.size();

	for (pos = 0; pos < max; pos++)
	    if (((aAnimationObject) objects.elementAt(pos)).getLevel() > ao
		    .getLevel())
		break;
     ...
     * 
     * @param ao
     *            The <code>aAnimationObject</code> to remove.
     */
	...
	if (offscreen_buffer == null
		|| offscreen_buffer.getWidth(this) != dim.width
