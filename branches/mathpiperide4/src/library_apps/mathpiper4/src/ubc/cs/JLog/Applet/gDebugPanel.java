...
public class gDebugPanel extends Panel {
    ...
    public gDebugPanel(jPrologServices ps, boolean show_buttons) {
	{
		...
		public void handleEvent(jPrologServiceEvent e) {// during a
								// query we want
								// this to
								// remain at the
								// original
	...
	{// create card for proved stack
	    proved_stack = new gProverStackPanel("Proved", info, prolog,
		    goals_stack);
	}
	...
	{// create menu and items for workingarea
	    menu = new gButtonTabMenu(gButtonTabMenu.HORIZONTAL, new Font(
	...
	if (traceall.getState() || run_state == STATE_STOP
		|| run_state == STATE_STEPIN)
	    return true;
