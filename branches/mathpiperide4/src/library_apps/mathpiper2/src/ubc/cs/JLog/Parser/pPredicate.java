...
class pPredicate extends pPacket {
    ...
    // if generic_predicate is true, then only create jPredicate for use as rule
    // head.
	...
	if (params == null)
	    ...
	    cterm = makeCompoundTerm(new jCompoundTerm(), params.getInside(),
		    vars, phash);

	{
		...
		throw new SyntaxErrorException(
			"Valid name required for predicate at ",
	...
	if (!generic_predicate
		&& (pe = predicates.getPredicate(getName(), cterm.size())) != null)
